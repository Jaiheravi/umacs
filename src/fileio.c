...
static void check_vfs_filename(Lisp_Object encoded, const char* reason) {
#if defined HAVE_ANDROID && !defined ANDROID_STUBIFY
    const char* name;
...
static bool selinux_enabled_p(const char* file) {
...
bool file_access_p(char const* file, int amode) {
#ifdef MSDOS
    if (amode & W_OK) {
...
Lisp_Object get_file_errno_data(char const* string, Lisp_Object name,
                                int errorno) {
    ...
    if (errorno == EEXIST)
        ...
        return Fcons(errorno == ENOENT ? Qfile_missing
                                       : (errorno == EACCES ? Qpermission_denied
                                                            : Qfile_error),
                     Fcons(build_string(string), errdata));
}

void report_file_errno(char const* string, Lisp_Object name, int errorno) {
...
void report_file_error(char const* string, Lisp_Object name) {
...
void report_file_notify_error(const char* string, Lisp_Object name) {
...
static Lisp_Object file_metadata_errno(char const* action, Lisp_Object file,
                                       int err) {
...
Lisp_Object file_attribute_errno(Lisp_Object file, int err) {
    return file_metadata_errno("Getting attributes", file, err);
}

void close_file_unwind(int fd) { emacs_close(fd); }

static void close_file_unwind_emacs_fd(void* ptr) {
    emacs_fd* fd;
...
void restore_point_unwind(Lisp_Object location) {
    Fgoto_char(location);
...
(Lisp_Object filename, Lisp_Object operation) {
    ...
    for (chain = Vfile_name_handler_alist; CONSP(chain); chain = XCDR(chain)) {
        Lisp_Object elt;
        elt = XCAR(chain);
        if (CONSP(elt)) {
...
static Lisp_Object file_name_directory(Lisp_Object filename) {
...
(Lisp_Object filename) {
    ...
    if (!NILP(handler)) {
        Lisp_Object handled_name =
            calln(handler, Qfile_name_directory, filename);
...
(Lisp_Object filename) {
    ...
    if (!NILP(handler)) {
        Lisp_Object handled_name =
            calln(handler, Qfile_name_nondirectory, filename);
...
(Lisp_Object filename) {
    ...
    if (!NILP(handler)) {
        Lisp_Object handled_name =
            calln(handler, Qunhandled_file_name_directory, filename);
        return STRINGP(handled_name) ? handled_name : Qnil;
    }
...
static ptrdiff_t file_name_as_directory(char* dst, const char* src,
                                        ptrdiff_t srclen, bool multibyte) {
    if (srclen == 0) {
        dst[0] = '.';
        dst[1] = '/';
        dst[2] = '\0';
...
(Lisp_Object file) {
    ...
    if (!NILP(handler)) {
        Lisp_Object handled_name =
            calln(handler, Qfile_name_as_directory, file);
        if (STRINGP(handled_name))
            return handled_name;
        error("Invalid handler in `file-name-handler-alist'");
    }
...
static ptrdiff_t directory_file_name(char* dst, char* src, ptrdiff_t srclen,
                                     bool multibyte) {
...
static Lisp_Object expand_cp_target(Lisp_Object file, Lisp_Object newname) {
...
(Lisp_Object directory) {
    ...
    if (!NILP(handler)) {
        Lisp_Object handled_name =
            calln(handler, Qdirectory_file_name, directory);
        if (STRINGP(handled_name))
            return handled_name;
        error("Invalid handler in `file-name-handler-alist'");
    }
 ...
 Lisp_Object text) {
    ...
    int kind = (NILP(dir_flag)                          ? GT_FILE
                    : BASE_EQ(dir_flag, make_fixnum(0)) ? GT_NOCREATE
                                                        : GT_DIR);
    // int fd = gen_tempname (data, suffix_len, O_CLOEXEC, kind);
    int fd;
    if (kind == GT_DIR) {
        fd = mkdtemp(data) ? 0 : -1;
    }
    else if (kind == GT_NOCREATE) {
        /* Just need a unique name — mktemp generates it without creating */
        fd = mktemp(data) ? 0 : -1;
    }
    else {
        if (suffix_len > 0)
            fd = mkstemps(data, suffix_len);
        else
            fd = mkstemp(data);
        if (fd >= 0)
...
(ptrdiff_t nargs, Lisp_Object* args) {
    ...
    /* Convert if needed. */
    if ((multibytes != 0 && multibytes != nargs) || eargs != nargs) {
        int j = 0;
...
static bool file_name_absolute_no_tilde_p(Lisp_Object name) {
...
static char* user_homedir(char const* name) {
    ptrdiff_t length;
...
(Lisp_Object name, Lisp_Object default_directory) {
    ...
    if (!NILP(handler)) {
        handled_name =
            calln(handler, Qexpand_file_name, name, default_directory);
        if (STRINGP(handled_name))
            return handled_name;
        error("Invalid handler in `file-name-handler-alist'");
    }
   ...
   To avoid this, we use the root of the current drive.  */
       ...
       of the expanded name.  */
        if (STRINGP(dir)) {
            if (file_name_absolute_no_tilde_p(dir)) {
                ...
                Lisp_Object absdir = STRINGP(Vinvocation_directory) &&
...

    {
        ...
        /* Make sure DEFAULT_DIRECTORY is properly expanded.
           ...
           default_directory.  Unfortunately, calling Fexpand_file_name
           recursively could invoke GC, and the strings might be relocated. This
           would be annoying because we have pointers into strings lying around
        ...
        /* Save time in some common cases - as long as default_directory
           ...
           DOS or Windows).  */
            ...
            /* The above expansion might have produced a remote file name,
               ...
               remote.  */
            handler =
                Ffind_file_name_handler(default_directory, Qexpand_file_name);
            if (!NILP(handler)) {
                handled_name =
                    calln(handler, Qexpand_file_name, name, default_directory);
                if (STRINGP(handled_name))
        ...
        if (multibyte) {
            ...
            if (name_ascii_p || *p != '\0') {
               ...
               multibyte string out of DEFAULT_DIRECTORY's data.  */
                default_directory = make_multibyte_string(
                    SSDATA(default_directory), SCHARS(default_directory),
            ...
            if (*p == '\0') {
               ...
               NAME's data.  */
                ...
                default_directory = make_unibyte_string(
                    SSDATA(default_directory), SBYTES(default_directory));
        }
    }
    ...
    if (nm[0] == '~' /* prefix ~ */
        ...
        if (IS_DIRECTORY_SEP(nm[1]) || nm[1] == 0) /* ~ by itself */
        {
            Lisp_Object tem;
    ...
    if (newdir) {
        ...
        if (
            /* Detect MSDOS file names with drive specifiers.  */
            !(IS_DRIVE(newdir[0]) && IS_DEVICE_SEP(newdir[1]) &&
            ...
            if (!IS_DIRECTORY_SEP(nm[0])) {
                ...
                char* tmp = SAFE_ALLOCA(
                    newdirlen + file_name_as_directory_slop + nmlen + 1);
                ptrdiff_t dlen =
        ...
        if (IS_DIRECTORY_SEP(nm[0]) && collapse_newdir) {
            ...
            if (IS_DIRECTORY_SEP(newdir[0]) && IS_DIRECTORY_SEP(newdir[1]) &&
                !IS_DIRECTORY_SEP(newdir[2])) {
                char* adir =
                    strcpy(SAFE_ALLOCA(newdirlim - newdir + 1), newdir);
                char* p = adir + 2;
    ...
    *target = 0;
    ...
    if (!NILP(handler)) {
        handled_name =
            calln(handler, Qexpand_file_name, result, default_directory);
        if (!STRINGP(handled_name))
            error("Invalid handler in `file-name-handler-alist'");
        result = handled_name;
    }
...
char* splice_dir_file(char* buf, char const* dir, char const* file) {
...
char const* get_homedir(void) {
    ...
    if (IS_DRIVE(*home) && IS_DEVICE_SEP(home[1]) &&
    ...
    if (ahomesize <= ahomelenbound)
        ahome =
            xpalloc(ahome, &ahomesize, ahomelenbound + 1 - ahomesize, -1, 1);
...
static char* search_embedded_absfilename(char* nm, char* endp) {
    char* p = nm + 1;
...
(Lisp_Object filename) {
    ...
    if (!NILP(handler)) {
        Lisp_Object handled_name =
            calln(handler, Qsubstitute_in_file_name, filename);
        if (STRINGP(handled_name))
            return handled_name;
        error("Invalid handler in `file-name-handler-alist'");
    }
    ...
    {
        Lisp_Object result = (Fsubstitute_in_file_name(
...
Lisp_Object expand_and_dir_to_file(Lisp_Object filename) {
...
static void barf_or_query_if_file_exists(Lisp_Object absname,
                                         bool known_to_exist,
                                         const char* querystring,
                                         bool interactive, bool quick) {
    ...
    if (!known_to_exist &&
        (emacs_fstatat(AT_FDCWD, SSDATA(encoded_filename), &statbuf,
                       AT_SYMLINK_NOFOLLOW) == 0)) {
        if (S_ISDIR(statbuf.st_mode))
            xsignal2(Qfile_error, build_string("File is a directory"), absname);
        known_to_exist = true;
    }

    if (known_to_exist) {
        if (!interactive)
            xsignal2(Qfile_already_exists, build_string("File already exists"),
                     absname);
...
static ptrdiff_t emacs_full_read(emacs_fd fd, void* buf, ptrdiff_t bufsize) {
    char* b = buf;
    ptrdiff_t nread = 0, r;
...
static bool clone_file(int dest, int source) {
#ifdef FICLONE
...
(Lisp_Object file, Lisp_Object newname, Lisp_Object ok_if_already_exists,
 ...
 Lisp_Object preserve_permissions) {
    ...
    if (!NILP(handler))
        return calln(handler, Qcopy_file, file, newname, ok_if_already_exists,
    ...
    case -3:
        xsignal2(Qfile_date_error, build_string("Cannot set file date"),
                 newname);
    case -4:
    ...
    if (!NILP(preserve_permissions)) {
        ...
        if (selinux_enabled_p(SSDATA(encoded_file))
            ...
            conlength = fgetfilecon(emacs_fd_to_int(ifd), &con);
            if (conlength == -1)
    ...
    if (emacs_fd_to_int(ifd) == -1 || !clone_file(ofd, emacs_fd_to_int(ifd))) {
        ...
        if (emacs_fd_to_int(ifd) != -1) {
            for (ssize_t copied;; newsize += copied) {
               ...
               surely aligned well.  */
                ...
                copied = copy_file_range(emacs_fd_to_int(ifd), NULL, ofd, NULL,
                                         copy_max, 0);
                if (copied <= 0)
                    break;
                maybe_quit();
       ...
       precisely than copy_file_range would.  */
        ...
        for (ptrdiff_t copied; (copied = emacs_full_read(ifd, buf, sizeof buf));
             newsize += copied) {
            if (copied < 0)
       ...
       owner and group.  */
    {
        ...
        switch ((!NILP(preserve_permissions) && emacs_fd_to_int(ifd) != -1)
                    ? qcopy_acl(SSDATA(encoded_file), emacs_fd_to_int(ifd),
...
(Lisp_Object filename) {
    ...
    if (emacs_unlink(SSDATA(encoded_file)) != 0 && errno != ENOENT)
...
static Lisp_Object internal_delete_file_1(Lisp_Object ignore) { return Qt; }

/* Delete file FILENAME, returning true if successful.
   This ignores `delete-by-moving-to-trash'.  */

bool internal_delete_file(Lisp_Object filename) {
    Lisp_Object tem;

    tem = internal_condition_case_1(Fdelete_file_internal, filename, Qt,
...
static int file_name_case_insensitive_err(Lisp_Object file) {
...
(Lisp_Object filename) {
    ...
    while (true) {
        ...
        /* Avoid infinite loop if the root has trouble (if that's even
       possible). Without a parent, we just don't know and return nil as well.
     */
...
(Lisp_Object file, Lisp_Object newname, Lisp_Object ok_if_already_exists) {
    ...
    if (!NILP(handler))
        return calln(handler, Qrename_file, file, newname,
    ...
    bool plain_rename =
        (case_only_rename ||
         (!NILP(ok_if_already_exists) && !FIXNUMP(ok_if_already_exists)));
    int rename_errno UNINIT;
    if (!plain_rename) {
        if (emacs_renameat_noreplace(AT_FDCWD, SSDATA(encoded_file), AT_FDCWD,
    ...
    if (plain_rename) {
        if (emacs_rename(SSDATA(encoded_file), SSDATA(encoded_newname)) == 0)
            return Qnil;
        rename_errno = errno;
        /* Don't prompt again.  */
    ...
    if (!dirp) {
        if (emacs_fstatat(AT_FDCWD, SSDATA(encoded_file), &file_st,
                          AT_SYMLINK_NOFOLLOW) != 0)
    ...
    if (dirp)
        ...
        Lisp_Object target = emacs_readlinkat(AT_FDCWD, SSDATA(encoded_file));
        if (!NILP(target))
...
(Lisp_Object file, Lisp_Object newname, Lisp_Object ok_if_already_exists) {
    ...
    if (!NILP(handler))
        return calln(handler, Qadd_name_to_file, file, newname,
                     ok_if_already_exists);
    ...
    encoded_file = ENCODE_FILE(file);
    encoded_newname = ENCODE_FILE(newname);
    check_vfs_filename(encoded_file,
                       "Trying to create hard link to "
                       "file within special directory");
    check_vfs_filename(encoded_newname,
                       "Trying to create hard link"
...
(Lisp_Object target, Lisp_Object linkname, Lisp_Object ok_if_already_exists) {
    ...
    if (!NILP(handler))
        return calln(handler, Qmake_symbolic_link, target, linkname,
                     ok_if_already_exists);

    encoded_target = ENCODE_FILE(target);
    encoded_linkname = ENCODE_FILE(linkname);

    if (emacs_symlink(SSDATA(encoded_target), SSDATA(encoded_linkname)) == 0)
        return Qnil;
    ...
    if (errno == EEXIST) {
        ...
        if (emacs_symlink(SSDATA(encoded_target), SSDATA(encoded_linkname)) ==
            0)
            return Qnil;
    }
...
bool file_name_absolute_p(char const* filename) {
...
static Lisp_Object check_file_access(Lisp_Object file, Lisp_Object operation,
                                     int amode) {
    file = Fexpand_file_name(file, Qnil);
...
(Lisp_Object filename, Lisp_Object string) {
    ...
    if (sys_faccessat(AT_FDCWD, SSDATA(encoded_filename), R_OK, AT_EACCESS) !=
        0)
...
static Lisp_Object emacs_readlinkat(int fd, char const* filename) {
...
Lisp_Object check_emacs_readlinkat(int fd, Lisp_Object file,
                                   char const* encoded_file) {
...
bool file_directory_p(Lisp_Object file) {
#ifdef DOS_NT
    /* This is cheaper than 'stat'.  */
    bool retval = sys_faccessat(AT_FDCWD, SSDATA(file), D_OK, AT_EACCESS) == 0;
...
bool file_accessible_directory_p(Lisp_Object file) {
#ifdef DOS_NT
#ifdef WINDOWSNT
...
(Lisp_Object filename) {
    ...
    /* If the file name has special constructs in it,
       call the corresponding file name handler.  */
    Lisp_Object handler =
        Ffind_file_name_handler(absname, Qfile_selinux_context);
    if (!NILP(handler))
...
(Lisp_Object filename, Lisp_Object acl_string) {
    ...
    if (STRINGP(acl_string)) {
        ...
        fail =
            (acl_set_file(SSDATA(encoded_absname), ACL_TYPE_ACCESS, acl) != 0);
        acl_free(acl);
...
static int symlink_nofollow_flag(Lisp_Object flag) {
...
(Lisp_Object filename, Lisp_Object mode, Lisp_Object flag) {
    ...
    int nofollow = symlink_nofollow_flag(flag);
    Lisp_Object absname =
        Fexpand_file_name(filename, BVAR(current_buffer, directory));
...
(Lisp_Object filename, Lisp_Object timestamp, Lisp_Object flag) {
    ...
    Lisp_Object encoded_absname = ENCODE_FILE(absname);
    check_vfs_filename(encoded_absname,
                       "Trying to set access times of"
                       " file within special directory");
...
(Lisp_Object file1, Lisp_Object file2) {
    ...
    /* If the file name has special constructs in it,
       call the corresponding file name handler.  */
    Lisp_Object handler =
        Ffind_file_name_handler(absname1, Qfile_newer_than_file_p);
    if (NILP(handler))
...
static void decide_coding_unwind(Lisp_Object unwind_data) {
...
static Lisp_Object read_non_regular(Lisp_Object state) {
    union read_non_regular* data = XFIXNUMPTR(state);
    intmax_t nbytes = emacs_fd_read(
        data->s.fd,
        (data->s.buf
             ? data->s.buf
             : ((char*)BEG_ADDR + PT_BYTE - BEG_BYTE + data->s.inserted)),
        data->s.bufsize);
...
static Lisp_Object read_non_regular_quit(Lisp_Object ignore) { return Qnil; }

/* Return the file offset that VAL represents, checking for type
   errors and overflow.  */
static off_t file_offset(Lisp_Object val) {
    if (INTEGERP(val)) {
        intmax_t v;
...
static struct timespec time_error_value(int errnum) {
    int ns = (errnum == ENOENT || errnum == ENOTDIR ? NONEXISTENT_MODTIME_NSECS
                                                    : UNKNOWN_MODTIME_NSECS);
    return make_timespec(0, ns);
}

static Lisp_Object get_window_points_and_markers(void) {
    ...
    /* Window markers (and point) are handled specially: rather than move to
       ...
       harmless and is convenient for the end user when most of the file is
       unmodified, except for a few minor details near the beginning and near
       the end.  */
...
static void restore_window_points(Lisp_Object window_markers,
...
static void maybe_move_gap(struct buffer* b) {
...
static off_t xlseek(emacs_fd fd, off_t pos, int whence, Lisp_Object filename) {
...
(Lisp_Object filename, Lisp_Object visit, Lisp_Object beg, Lisp_Object end,
 Lisp_Object replace) {
    ...
    enum
    {
        INSERT_READ_SIZE_MAX = min(1024 * 1024, SYS_BUFSIZE_MAX)
    };
    ...
    bool empty_undo_list_p =
        (!NILP(visit) && NILP(BVAR(current_buffer, undo_list)) && BEG == Z);
    ...
    if (!NILP(handler)) {
        val = calln(handler, Qinsert_file_contents, filename, visit, beg, end,
                    replace);
    ...
    if (!NILP(replace)) {
        ...
        record_unwind_protect(restore_point_unwind, XCAR(XCAR(window_markers)));
    }

    {
        ...
        if (regular | memory_object) {
            ...
            /* On some ancient platforms, a regular file with a negative size
               means the actual file size is greater than the maximum off_t
               value. Fail now rather than doing a lot of work and exhausting
               memory.  */
            if (st_size < 0)
                buffer_overflow();
        }

        mtime = (memory_object ? make_timespec(0, UNKNOWN_MODTIME_NSECS)
                               : get_stat_mtime(&st));
    }
    ...
    if (!regular) {
        if (!NILP(visit) || !NILP(replace)) {
            if (!NILP(visit) || !EQ(replace, Qif_regular))
                xsignal2(Qfile_error, build_string("not a regular file"),
                         orig_filename);
            else
        ...
        if (!NILP(Vcoding_system_for_read))
            ...
            if (regular && !NILP(Vset_auto_coding_function)) {
                if (!NILP(beg))
                    ...
                    beg_offset = curpos =
                        xlseek(fd, 0, SEEK_CUR, orig_filename);
                    end_offset = max(end_offset, beg_offset);
                }
               ...
               do not use st_size or report any SEEK_END failure.  */
                ...
                ptrdiff_t trial = min(end_offset - beg_offset, 4 * 1024),
                          nread =
                              trial ? emacs_full_read(fd, read_buf, trial) : 0;
                curpos += nread;
                ...
                if (nread == 4 * 1024 && curpos < end_offset) {
                    ...
                    if (curpos < 0)
                        ...
                        if (tailbeg != curpos)
                            curpos =
                                xlseek(fd, tailbeg, SEEK_SET, orig_filename);
                    }
                    ...
                    ptrdiff_t trial =
                        min(end_offset - curpos, sizeof read_buf - 1024);
                ...
                if (nread < 0)
                    ...
                    TEMP_SET_PT_BOTH(BEG, BEG_BYTE);
                    coding_system = calln(Vset_auto_coding_function, filename,
                                          make_fixnum(nread));
    ...
    if ((!NILP(replace) && !BASE_EQ(replace, Qunbound)) && BEGV < ZV &&
        (NILP(coding_system) || !CODING_REQUIRE_DECODING(&coding))) {
       ...
       match the text at the beginning of the buffer.  */
        while (true) {
            ...
            if (CODING_REQUIRE_DETECTION(&coding)) {
                coding_system =
                    detect_coding_system((unsigned char*)read_buf, nread, nread,
                                         1, 0, coding_system);
                setup_coding_system(coding_system, &coding);
            }
    ...
    if ((!NILP(replace) && !BASE_EQ(replace, Qunbound)) && !replace_handled &&
        BEGV < ZV) {
       ...
       CONVERSION_BUFFER.  */
        ...
        while (true) {
            ...
            ptrdiff_t trial =
                min(end_offset - curpos, sizeof read_buf - unprocessed);
       ...
       we are taking from the decoded string.  */
        ...
        /* Insert from the file at the proper position.  */
        SET_PT_BOTH(temp, same_at_start);
        same_at_start_charpos = buf_bytepos_to_charpos(
            XBUFFER(conversion_buffer),
            same_at_start - BEGV_BYTE +
                BUF_BEG_BYTE(XBUFFER(conversion_buffer)));
        eassert(same_at_start_charpos == temp - (BEGV - BEG));
        inserted_chars = (buf_bytepos_to_charpos(
                              XBUFFER(conversion_buffer),
                              same_at_start + inserted - BEGV_BYTE +
                                  BUF_BEG_BYTE(XBUFFER(conversion_buffer))) -
                          same_at_start_charpos);
        insert_from_buffer(XBUFFER(conversion_buffer), same_at_start_charpos,
                           inserted_chars, 0);
    ...
    if (NILP(visit) && total > 0) {
        if (!NILP(BVAR(current_buffer, file_truename))
            ...
            && !NILP(BVAR(current_buffer, filename)) && SAVE_MODIFF >= MODIFF)
            we_locked_file = true;
        ...
        while (inserted < total) {
            ...
            if (!regular) {
                ...
                nbytes = internal_condition_case_1(
                    read_non_regular, make_pointer_integer(&data), Qerror,
            ...
            if (buf == read_buf) {
                if (gap_size < this) {
                    ...
                    make_gap(
                        max(GAP_SIZE >> 1, this - gap_size + sizeof read_buf));
                    gap_size = GAP_SIZE - inserted;
                }
                memcpy(BEG_ADDR + PT_BYTE - BEG_BYTE + inserted, read_buf,
                       this);
            }
            gap_size -= this;
            inserted += this;
    ...
    if (NILP(coding_system)) {
       ...
       before the insertion.  */
        ...
        if (!NILP(Vcoding_system_for_read))
            ...
            Lisp_Object unwind_data = Fcons(
                multibyte,
                Fcons(BVAR(current_buffer, undo_list), Fcurrent_buffer()));
            ...
            if (NILP(coding_system)) {
               ...
               file-coding-system-alist.  */
                coding_system =
                    CALLN(Ffind_operation_coding_system, Qinsert_file_contents,
                          orig_filename, visit, beg, end, Qnil);
                if (CONSP(coding_system))
                    coding_system = XCAR(coding_system);
            }
...
handled:
    ...
    if (inserted > 0)
        restore_window_points(window_markers, inserted,
                              BYTE_TO_CHAR(same_at_start), same_at_end_charpos);
    ...
    if (!NILP(Ffboundp(Qafter_insert_file_set_coding))) {
        insval =
            calln(Qafter_insert_file_set_coding, make_fixnum(inserted), visit);
        if (!NILP(insval)) {
    ...
    if (inserted > 0 && !NILP(Ffboundp(Qformat_decode))) {
        ...
        if (NILP(replace)) {
            insval = calln(Qformat_decode, Qnil, make_fixnum(inserted), visit);
            if (!RANGED_FIXNUMP(0, insval, ZV - PT))
                wrong_type_argument(Qinserted_chars, insval);
            inserted = XFIXNAT(insval);
        }
            ...
            insval = calln(Qformat_decode, Qnil, make_fixnum(oinserted), visit);
            if (!RANGED_FIXNUMP(0, insval, ZV - PT))
                wrong_type_argument(Qinserted_chars, insval);
        ...
        if (!empty_undo_list_p) {
            ...
            if (CONSP(old_undo) && inserted != old_inserted) {
               ...
               the format conversion.  */
                ...
                if (CONSP(tem) && FIXNUMP(XCAR(tem)) && FIXNUMP(XCDR(tem)) &&
    ...
    if (current_buffer->base_buffer &&
        ...
        invalidate_region_cache(current_buffer, current_buffer->newline_cache,
                                PT - BEG, Z - PT - inserted);
...
static void build_annotations_unwind(Lisp_Object arg) {
...
static Lisp_Object
choose_write_coding_system(Lisp_Object start, Lisp_Object end,
                           Lisp_Object filename, Lisp_Object append,
                           Lisp_Object visit, Lisp_Object lockname,
                           struct coding_system* coding) {
    Lisp_Object val;
    ...
    if (auto_saving &&
        ...
        if (coding_system_require_warning &&
            ...
            val = calln(Vselect_safe_coding_system_function, start, end,
                        list2(Qt, val), Qnil, filename);
    }
    else {
       ...
       tells that it is not Unix-like (LF only) format.  */
        ...
        if (!force_raw_text &&
            !NILP(Ffboundp(Vselect_safe_coding_system_function))) {
            /* Confirm that VAL can surely encode the current region.  */
            val = calln(Vselect_safe_coding_system_function, start, end, val,
                        Qnil, filename);
       ...
       format, we use that of `buffer-file-coding-system'.  */
        if (!using_default_coding) {
            Lisp_Object dflt =
                BVAR(&buffer_defaults, buffer_file_coding_system);
...
Lisp_Object write_region(Lisp_Object start, Lisp_Object end,
                         Lisp_Object filename, Lisp_Object append,
                         Lisp_Object visit, Lisp_Object lockname,
    ...
    if (!NILP(handler)) {
        Lisp_Object val;
        val = calln(handler, Qwrite_region, start, end, filename, append, visit,
                    lockname, mustbenew);
    ...
    Vlast_coding_system_used = choose_write_coding_system(
        start, end, filename, append, visit, lockname, &coding);
    ...
    if (timespec_valid_p(modtime) &&
        ...
        if (emacs_fstatat(AT_FDCWD, fn, &st1, 0) == 0 &&
            ...
            bool use_heuristic =
                ((open_flags & (O_EXCL | O_TRUNC)) != 0 && st.st_size != 0 &&
                 modtime.tv_nsec % 100 != 0);

            struct timespec modtime1 = get_stat_mtime(&st1);
            if (use_heuristic && timespec_cmp(modtime, modtime1) == 0 &&
    ...
    bool auto_saving_into_visited_file = auto_saving &&
        !NILP(Fstring_equal(BVAR(current_buffer, filename),
                            BVAR(current_buffer, auto_save_file_name)));
    if (visiting) {
        SAVE_MODIFF = MODIFF;
    ...
    if (!auto_saving && !noninteractive)
        message_with_string((NUMBERP(append)     ? "Updated %s"
                                 : !NILP(append) ? "Added to %s"
                                                 : "Wrote %s"),
                            visit_file, 1);
...
loop_over_p:
    FOR_EACH_TAIL(p) {
        ...
        if (EQ(Qt, XCAR(p)) &&
            !used_global) { /* Use the global value of the hook.  */
            used_global = true;
            p = CALLN(Fappend, Fdefault_value(Qwrite_region_annotate_functions),
                      XCDR(p));
            goto loop_over_p;
        }
       ...
       been dealt with by this function.  */
        if (current_buffer != given_buffer) {
            Vwrite_region_annotation_buffers =
                Fcons(Fcurrent_buffer(), Vwrite_region_annotation_buffers);
            XSETFASTINT(start, BEGV);
            XSETFASTINT(end, ZV);
            annotations = Qnil;
        }
...
static bool a_write(int desc, Lisp_Object string, ptrdiff_t pos,
...
static bool e_write(int desc, Lisp_Object string, ptrdiff_t start,
                    ptrdiff_t end, struct coding_system* coding) {
    if (STRINGP(string)) {
        start = 0;
        end = SCHARS(string);
    }
    ...
    while (start < end) {
        if (STRINGP(string)) {
            ...
            if (CODING_REQUIRE_ENCODING(coding)) {
                ...
                encode_coding_object(coding, Fcurrent_buffer(), start,
                                     start_byte, start + nchars,
                                     CHAR_TO_BYTE(start + nchars), Qt);
            }
            else {
        ...
        if (coding->produced > 0) {
            char* buf =
                (coding->raw_destination
                     ? (char*)coding->destination
...
(Lisp_Object buf) {
    ...
    if (!STRINGP(BVAR(b, filename)))
        return Qt;
    if (b->modtime.tv_nsec == UNKNOWN_MODTIME_NSECS)
        return Qt;
    ...
    if (timespec_cmp(mtime, b->modtime) == 0 &&
        (b->modtime_size < 0 || st.st_size == b->modtime_size))
        return Qt;
    return Qnil;
}

Lisp_Object buffer_visited_file_modtime(struct buffer* buf) {
...
See Info node `(elisp)Modification Time' for more details.  */)
(void) { return buffer_visited_file_modtime(current_buffer); }
...
static Lisp_Object auto_save_error(Lisp_Object error_val) {
...
static Lisp_Object auto_save_1(void) {
    struct stat st;
    Lisp_Object modes;
    ...
    if (!NILP(BVAR(current_buffer, filename))) {
        if (emacs_fstatat(AT_FDCWD, SSDATA(BVAR(current_buffer, filename)), &st,
                          0) == 0)
    ...
    return Fwrite_region(
        Qnil, Qnil, BVAR(current_buffer, auto_save_file_name), Qnil,
        NILP(Vauto_save_visited_file_name) ? Qlambda : Qt, Qnil, Qnil);
}
...
static void do_auto_save_unwind(void* arg) {
...
static Lisp_Object do_auto_save_make_dir(Lisp_Object dir) {
    Lisp_Object result;
...
static Lisp_Object do_auto_save_eh(Lisp_Object ignore) {
    auto_saving_dir_umask = 0;
    return Qnil;
}
...
(Lisp_Object no_message, Lisp_Object current_only) {
    ...
    if (STRINGP(Vauto_save_list_file_name)) {
        ...
        if (!NILP(Vrun_hooks)) {
            ...
            if (NILP(Ffile_directory_p(dir)))
                internal_condition_case_1(do_auto_save_make_dir, dir, Qt,
                                          do_auto_save_eh);
        }
    ...
    for (do_handled_files = 0; do_handled_files < 2; do_handled_files++)
        FOR_EACH_LIVE_BUFFER(tail, buf) {
            ...
            if (STRINGP(BVAR(b, auto_save_file_name)) && stream != NULL &&
                do_handled_files == 0) {
                block_input();
            ...
            if (!NILP(current_only) && b != current_buffer)
                continue;
            ...
            if (STRINGP(BVAR(b, auto_save_file_name)) &&
                ...
                if (NILP(Vauto_save_include_big_deletions) &&
                    ...
                    && !NILP(BVAR(b, filename)) && NILP(no_message)) {
                    /* It has shrunk too much; turn off auto-saving here.  */
                    minibuffer_auto_raise = orig_minibuffer_auto_raise;
                    message_with_string(
                        "Buffer %s has shrunk a lot; auto save disabled in "
                        "that buffer until next real save",
                        BVAR(b, name), 1);
                    minibuffer_auto_raise = 0;
...
(Lisp_Object stream, Lisp_Object mode) {
    ...
    defined STAT_STATFS4 || defined STAT_STATVFS || defined STAT_STATVFS64

/* Yield a Lisp number equal to BLOCKSIZE * BLOCKS, with the result
   negated if NEGATE.  */
static Lisp_Object blocks_to_bytes(uintmax_t blocksize, uintmax_t blocks,
                                   bool negate) {
    intmax_t n;
...
(Lisp_Object filename) { return Qnil; }
...
behaves as if file names were encoded in `utf-8'.  */);
    ...
    Fput(Qfile_error, Qerror_message, build_string("File error"));
    ...
    Fput(Qfile_missing, Qerror_message, build_string("File is missing"));
    ...
    Fput(Qremote_file_error, Qerror_message, build_string("Remote file error"));
...
that were already called.  See also `write-region-annotate-functions'.  */);
    ...
    DEFVAR_LISP("inhibit-file-name-handlers", Vinhibit_file_name_handlers, doc:
                    /* A list of file name handlers that temporarily should not
    be used. This applies only to the operation `inhibit-file-name-operation'.
  */);
...
a non-nil value.  */);
    ...
    DEFVAR_LISP("auto-save-visited-file-name", Vauto_save_visited_file_name,
                doc:
                    /* Non-nil says auto-save a buffer in the file it is
    visiting, when practical. Normally auto-save files are written under other
    names.  */);
    Vauto_save_visited_file_name = Qnil;

    DEFVAR_LISP("auto-save-include-big-deletions",
                Vauto_save_include_big_deletions, doc:
                    /* If non-nil, auto-save even if a large part of the text is
    deleted. If nil, deleting a substantial portion of the text disables
    auto-save in the buffer; this is the default behavior, because the
    auto-save file is usually more useful if it contains the deleted text.
  */);
    Vauto_save_include_big_deletions = Qnil;

    DEFVAR_BOOL("write-region-inhibit-fsync", write_region_inhibit_fsync, doc:
                    /* Non-nil means don't call fsync in `write-region'.
    This variable affects calls to `write-region' as well as save commands.
    By default, it is non-nil.

    Although setting this to nil may avoid data loss if the system loses
    power, it can be a significant performance hit in the usual case, and it
    doesn't necessarily cause file-save operations to actually survive a
    crash.  */);
