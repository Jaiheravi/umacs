...
static bool trillion_factor(Lisp_Object hz) {
    if (FASTER_TIMEFNS) {
        if (FIXNUMP(hz))
...
struct timeval make_timeval(struct timespec t) {
    struct timeval tv;
    tv.tv_sec = t.tv_sec;
...
struct timespec monotonic_coarse_timespec(void) {
    struct timespec ts;
...
static long int tm_gmtoff(struct tm* a) {
#if HAVE_TM_GMTOFF
    return a->tm_gmtoff;
#else
    return 0;
...
static struct tm* emacs_localtime_rz(timezone_t tz, time_t const* t,
                                     struct tm* tm) {
#ifdef WINDOWSNT
...
static timezone_t tzlookup(Lisp_Object zone, bool settz) {
    static char const tzbuf_format[] = "<%+.*" pI "d>%s%" pI "d:%02d:%02d";
    char const* trailing_tzbuf_format =
        tzbuf_format + sizeof "<%+.*" pI "d" - 1;
    ...
    if (NILP(zone))
        ...
        else if (plain_integer ||
                 (CONSP(zone) && FIXNUMP(XCAR(zone)) && CONSP(XCDR(zone)))) {
            ...
            if (plain_integer) {
                ...
                AUTO_STRING_WITH_LEN(trailing, tzbuf,
                                     sprintf(tzbuf, trailing_tzbuf_format,
                                             &"-"[XFIXNUM(zone) < 0], hour, min,
                                             sec));
                zone_string =
                    SSDATA(concat3(leading, ENCODE_SYSTEM(abbr), trailing));
            }
        }
        else
...
static AVOID time_overflow(void) {
    error("Specified time is not representable");
}

static AVOID time_spec_invalid(void) { error("Invalid time specification"); }

static AVOID time_error(int err) {
    switch (err) {
    case ENOMEM:
        memory_full(SIZE_MAX);
    case EOVERFLOW:
...
static double frac_to_double(Lisp_Object numerator, Lisp_Object denominator) {
    intmax_t intmax_numerator, intmax_denominator;
    if (FASTER_TIMEFNS && integer_to_intmax(numerator, &intmax_numerator) &&
       ...
       so add FLT_RADIX to its absolute value as per round-to-even.  */
        ...
        incr = ((lo_digit > FLT_RADIX / 2 ||
                 (lo_digit == FLT_RADIX / 2 && FLT_RADIX % 2 == 0 &&
                  ((lo_2digits / FLT_RADIX) & 1 || mpz_sgn(*r) != 0)))
                    ? FLT_RADIX
                    : 0);
    }
...
static bool mpz_time(mpz_t const z, time_t* t) {
...
static struct timespec s_ns_to_timespec(intmax_t s, long int ns) {
    time_t sec;
...
static struct timespec ticks_hz_to_timespec(Lisp_Object ticks, Lisp_Object hz) {
    int ns;
    mpz_t* q = &mpz[0];
    mpz_t const* qt = q;
    ...
    /* Check that Q fits in time_t, not merely in RESULT.tv_sec.  With some
       MinGW versions, tv_sec is a 64-bit type, whereas time_t is a 32-bit type.
     */
    time_t sec;
...
static union c_time decode_ticks_hz(Lisp_Object ticks, Lisp_Object hz,
                                    enum cform cform) {
    switch (cform) {
    case CFORM_DOUBLE:
...
static union c_time decode_float_time(double t, enum cform cform) {
...
static Lisp_Object ticks_hz_list4(Lisp_Object ticks, Lisp_Object hz) {
    ...
    return list4(make_integer_mpz(), make_fixnum(lo), make_fixnum(us),
                 make_fixnum(ps));
}

/* Set ROP to T.  */
static void mpz_set_time(mpz_t rop, time_t t) {
    if (EXPR_SIGNED(t))
...
static void timespec_mpz(struct timespec t) {
...
static Lisp_Object timespec_ticks(struct timespec t) {
...
static EMACS_INT emacs_gcd(EMACS_INT a, EMACS_INT b) {
...
static Lisp_Object ticks_hz_hz_ticks(struct ticks_hz t, Lisp_Object hz) {
...
static Lisp_Object ticks_hz_seconds(struct ticks_hz t) {
    ...
    mpz_fdiv_q(mpz[0], *bignum_integer(&mpz[0], t.ticks),
               *bignum_integer(&mpz[1], t.hz));
    return make_integer_mpz();
}

/* Convert T to a Lisp timestamp.  */
Lisp_Object make_lisp_time(struct timespec t) {
...
Lisp_Object timespec_to_lisp(struct timespec t) {
...
static struct err_time decode_time_components(Lisp_Object high, Lisp_Object low,
                                              Lisp_Object usec,
                                              Lisp_Object psec, Lisp_Object hz,
                                              enum cform cform) {
...
static union c_time current_time_in_cform(enum cform cform) {
...
static union c_time decode_lisp_time(Lisp_Object specified_time,
                                     enum cform cform) {
    ...
    */
    ...
    if (NILP(specified_time))
        ...
        if (CONSP(low)) {
            ...
            /* (TICKS . HZ) */
            if (!(INTEGERP(high) &&
                  (FIXNUMP(low) ? XFIXNUM(low) > 0 : !NILP(Fnatnump(low)))))
                time_spec_invalid();
...
/* Convert a non-float Lisp timestamp SPECIFIED_TIME to double.
   Signal an error if unsuccessful.  */
double float_time(Lisp_Object specified_time) {
...
struct timespec list4_to_timespec(Lisp_Object high, Lisp_Object low,
...
struct timespec lisp_time_argument(Lisp_Object specified_time) {
...
static time_t lisp_seconds_argument(Lisp_Object specified_time) {
...
static Lisp_Object lispint_arith(Lisp_Object a, Lisp_Object b, bool subtract) {
    bool mpz_done = false;
    ...
    if (FASTER_TIMEFNS && FIXNUMP(b)) {
        ...
        /* For speed, use EMACS_INT arithmetic if it will do.  */
        if (FIXNUMP(a))
            return make_int(subtract ? XFIXNUM(a) - XFIXNUM(b)
                                     : XFIXNUM(a) + XFIXNUM(b));
    ...
    if (!mpz_done)
        (subtract ? mpz_sub : mpz_add)(mpz[0], *bignum_integer(&mpz[0], a),
...
static Lisp_Object time_arith(Lisp_Object a, Lisp_Object b, bool subtract) {
    ...
    return (BASE_EQ(hz, make_fixnum(1)) ? ticks
                : (!current_time_list || (CONSP(a) && !CONSP(XCDR(a))) ||
                   (CONSP(b) && !CONSP(XCDR(b))) || !trillion_factor(hz))
                ? Fcons(ticks, hz)
...
(Lisp_Object a, Lisp_Object b) { return time_arith(a, b, false); }
...
static EMACS_INT time_cmp(Lisp_Object a, Lisp_Object b) {
...
See `format-time-string' for the various forms of a time value.
For example, nil stands for the current time.  */)
(Lisp_Object a, Lisp_Object b) { return time_cmp(a, b) < 0 ? Qt : Qnil; }
...
/* Write information into buffer S of size MAXSIZE, according to the
   ...
   bytes in FORMAT.  */
static ptrdiff_t emacs_nmemftime(char* s, size_t maxsize, const char* format,
                                 size_t format_len, const struct tm* tp,
...
static Lisp_Object format_time_string(char const* format, ptrdiff_t formatlen,
                                      struct timespec t, Lisp_Object zone,
                                      struct tm* tmp) {
    char buffer[4000];
    char* buf = buffer;
    ptrdiff_t len = -1;
    ...
    if (tmp) {
        ...
        int ns = t.tv_nsec;
        len = emacs_nmemftime(buffer, sizeof buffer, format, formatlen, tmp, tz,
                              ns);
        if (len < 0 && errno == ERANGE) {
            /* Buffer was too small, so make it bigger and try again.  */
            len =
                emacs_nmemftime(NULL, SIZE_MAX, format, formatlen, tmp, tz, ns);
            if (0 <= len && len < STRING_BYTES_BOUND) {
                buf = SAFE_ALLOCA(len + 1);
                len = emacs_nmemftime(buf, len + 1, format, formatlen, tmp, tz,
                                      ns);
            }
        }
    }
    ...
    Lisp_Object result =
        code_convert_string_norecord(bufstring, Vlocale_coding_system, 0);
    SAFE_FREE();
    return result;
}
...
(Lisp_Object format_string, Lisp_Object timeval, Lisp_Object zone) {
    ...
    format_string =
        code_convert_string_norecord(format_string, Vlocale_coding_system, 1);
    return format_time_string(SSDATA(format_string), SBYTES(format_string), t,
                              zone, &tm);
}
...
(Lisp_Object specified_time, Lisp_Object zone, Lisp_Object form) {
    ...
    if (BASE_EQ(hz, make_fixnum(1)))
       ...
       and SEC = (TICKS . HZ).  */
        ...
        if (FASTER_TIMEFNS && FIXNUMP(th.ticks) && FIXNUMP(hz) &&
            ...
            !ckd_add(&n, n,
                     (XFIXNUM(th.ticks) % XFIXNUM(hz) +
                      (XFIXNUM(th.ticks) % XFIXNUM(hz) < 0 ? XFIXNUM(hz) : 0))))
            ticks = make_int(n);
        else {
            mpz_fdiv_r(mpz[0], *bignum_integer(&mpz[0], th.ticks),
    ...
    return CALLN(Flist, sec, make_fixnum(local_tm.tm_min),
                 make_fixnum(local_tm.tm_hour), make_fixnum(local_tm.tm_mday),
                 make_fixnum(local_tm.tm_mon + 1), year,
                 make_fixnum(local_tm.tm_wday),
                 (local_tm.tm_isdst < 0        ? make_fixnum(-1)
                      : local_tm.tm_isdst == 0 ? Qnil
                                               : Qt),
                 (HAVE_TM_GMTOFF ? make_fixnum(tm_gmtoff(&local_tm))
...
static int check_tm_member(Lisp_Object obj, int offset) {
...
(ptrdiff_t nargs, Lisp_Object* args) {
    ...
    if (FASTER_TIMEFNS && BASE_EQ(hz, make_fixnum(1))) {
        ...
        mpz_fdiv_qr(mpz[0], mpz[1], *bignum_integer(&mpz[0], th.ticks),
                    *bignum_integer(&mpz[1], hz));
    ...
    if (BASE_EQ(hz, make_fixnum(1)))
        return (current_time_list ? list2(hi_time(value), lo_time(value))
...
(void) { return Fcons(make_int(clock()), make_int(CLOCKS_PER_SEC)); }
#endif
...
(Lisp_Object specified_time, Lisp_Object zone) {
    ...
    static char const wday_name[][4] = {"Sun", "Mon", "Tue", "Wed",
                                        "Thu", "Fri", "Sat"};
    static char const mon_name[][4] = {"Jan", "Feb", "Mar", "Apr",
                                       "May", "Jun", "Jul", "Aug",
                                       "Sep", "Oct", "Nov", "Dec"};
    ...
    int len = sprintf(buf, "%s %s%3d %02d:%02d:%02d %" PRIdMAX,
                      ...
                      tm.tm_hour, tm.tm_min, tm.tm_sec, tm.tm_year + year_base);
...
(Lisp_Object specified_time, Lisp_Object zone) {
    ...
    zone_name =
        format_time_string("%Z", sizeof "%Z" - 1, value, zone, &local_tm);
    ...
    if (HAVE_TM_GMTOFF || gmtime_r(&tsec, &gmt_tm)) {
        long int offset = (HAVE_TM_GMTOFF ? tm_gmtoff(&local_tm)
...
static void syms_of_timefns_for_pdumper(void) {
