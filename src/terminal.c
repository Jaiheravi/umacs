...
static void tset_param_alist(struct terminal* t, Lisp_Object val) {
    t->param_alist = val;
}


void ring_bell(struct frame* f) {
...
void set_terminal_window(struct frame* f, int size) {
...
void cursor_to(struct frame* f, int vpos, int hpos) {
...
void raw_cursor_to(struct frame* f, int row, int col) {
    struct terminal* term = FRAME_TERMINAL(f);
...
void clear_to_end(struct frame* f) {
...
void clear_end_of_line(struct frame* f, int first_unused_hpos) {
...
void write_glyphs(struct frame* f, struct glyph* string, int len) {
...
void insert_glyphs(struct frame* f, struct glyph* start, int len) {
    if (len <= 0)
        return;
...
void delete_glyphs(struct frame* f, int n) {
...
void ins_del_lines(struct frame* f, int vpos, int n) {
...
static struct terminal* decode_terminal(Lisp_Object terminal) {
    struct terminal* t;
...
struct terminal* decode_live_terminal(Lisp_Object terminal) {
...
struct terminal* decode_tty_terminal(Lisp_Object terminal) {
    struct terminal* t = decode_live_terminal(terminal);

    return (t->type == output_termcap || t->type == output_msdos_raw) ? t
                                                                      : NULL;
}
...
struct terminal* get_named_terminal(const char* name) {
    struct terminal* t;
...
static struct terminal* allocate_terminal(void) {
...
struct terminal* create_terminal(enum output_method type,
...
void delete_terminal(struct terminal* terminal) {
...
void delete_terminal_internal(struct terminal* terminal) {
    struct terminal** tp;
...
(Lisp_Object terminal, Lisp_Object force) {
    ...
    if (NILP(Vrun_hooks))
        ...
        pending_funcalls = Fcons(
            list3(Qrun_hook_with_args, Qdelete_terminal_functions, terminal),
            pending_funcalls);
    else
...
static Lisp_Object store_terminal_param(struct terminal* t,
...
static void calculate_glyph_code_table(struct terminal* t) {
...
Lisp_Object terminal_glyph_code(struct terminal* t, int ch) {
#if HAVE_STRUCT_UNIPAIR_UNICODE
...
static void initial_free_frame_resources(struct frame* f) {
...
struct terminal* init_initial_terminal(void) {
...
static void delete_initial_terminal(struct terminal* terminal) {
