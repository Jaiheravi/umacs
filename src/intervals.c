...
static void set_interval_left(INTERVAL i, INTERVAL left) { i->left = left; }

static void set_interval_right(INTERVAL i, INTERVAL right) { i->right = right; }

/* Make the parent of D be whatever the parent of S is, regardless
   of the type.  This is used when balancing an interval tree.  */

static void copy_interval_parent(INTERVAL d, INTERVAL s) {
    d->up = s->up;
    d->up_obj = s->up_obj;
}
...
create_root_interval(Lisp_Object parent) {
    ...
    if (!STRINGP(parent)) {
        new->total_length = (BUF_Z(XBUFFER(parent)) - BUF_BEG(XBUFFER(parent)));
...
void copy_properties(INTERVAL source, INTERVAL target) {
...
static void merge_properties(register INTERVAL source,
...
static bool intervals_equal_1(INTERVAL i0, INTERVAL i1, bool use_equal) {
    ...
    while (CONSP(i0_cdr) && CONSP(i1_cdr)) {
        ...
        if (!CONSP(i1_val) || (i1_val = XCDR(i1_val), !CONSP(i1_val)) ||
                    use_equal
                ? NILP(Fequal(XCAR(i1_val), XCAR(i0_cdr)))
...
bool intervals_equal(INTERVAL i0, INTERVAL i1) {
...
void traverse_intervals_noorder(INTERVAL tree,
...
void traverse_intervals(INTERVAL tree, ptrdiff_t position,
                        void (*function)(INTERVAL, void*), void* arg) {
    while (tree) {
...
static INTERVAL rotate_right(INTERVAL A) {
    INTERVAL B = A->left;
    INTERVAL c = B->right;
...
static INTERVAL rotate_left(INTERVAL A) {
    INTERVAL B = A->right;
    INTERVAL c = B->left;
    ptrdiff_t old_total = A->total_length;
...
static INTERVAL balance_an_interval(INTERVAL i) {
...
static INTERVAL balance_possible_root_interval(INTERVAL interval) {
    Lisp_Object parent;
...
static INTERVAL balance_intervals_internal(register INTERVAL tree) {
...
static void buffer_balance_intervals(struct buffer* b) {
    INTERVAL i;
...
static int interval_start_pos(INTERVAL source) {
    Lisp_Object parent;
...
#define SET_PARENT_POSITION(i)                                                 \
    if (AM_LEFT_CHILD(i))                                                      \
        ...
        INTERVAL_PARENT(i)->position =                                         \
            i->position - LEFT_TOTAL_LENGTH(i) - LENGTH(INTERVAL_PARENT(i))
    ...
    while (1) {
        if (pos < i->position) {
            ...
            if (pos < INTERVAL_LAST_POS(i) + RIGHT_TOTAL_LENGTH(i)) {
                i->right->position =
                    INTERVAL_LAST_POS(i) + LEFT_TOTAL_LENGTH(i->right);
...
static INTERVAL adjust_intervals_for_insertion(INTERVAL tree,
                                               ptrdiff_t position,
                                               ptrdiff_t length) {
    INTERVAL i;
    INTERVAL temp;
    ...
    if (position == i->position || eobp) {
       ...
       off a part later, if stickiness demands it.  */
        for (temp = prev ? prev : i; temp;
             temp = INTERVAL_PARENT_OR_NULL(temp)) {
...
static Lisp_Object merge_properties_sticky(Lisp_Object pleft,
                                           Lisp_Object pright) {
    ...
    for (tail1 = pright; CONSP(tail1); tail1 = Fcdr(XCDR(tail1))) {
       ...
       stickiness to SYM.  */
        ...
        use_left = (lpresent &&
                    !(TMEM(sym, lrear) || (CONSP(tmp) && !NILP(XCDR(tmp)))));
        use_right = (TMEM(sym, rfront) || (CONSP(tmp) && NILP(XCDR(tmp))));
    ...
    if (!NILP(front) &&
       ...
       we don't need to set up a detailed one.  */
        !(!NILP(cat) && SYMBOLP(cat) && EQ(Fget(cat, Qfront_sticky), Qt)))
...
static INTERVAL delete_node(register INTERVAL i) {
...
static void delete_interval(register INTERVAL i) {
...
static ptrdiff_t interval_deletion_adjustment(register INTERVAL tree,
    ...
    if (relative_position < LEFT_TOTAL_LENGTH(tree)) {
        ptrdiff_t subtract =
            interval_deletion_adjustment(tree->left, relative_position, amount);
    ...
    /* Right branch.  */
        ...
        relative_position -= (tree->total_length - RIGHT_TOTAL_LENGTH(tree));
        subtract = interval_deletion_adjustment(tree->right, relative_position,
                                                amount);
        tree->total_length -= subtract;
        eassert(LENGTH(tree) > 0);
        return subtract;
    ...
    /* Here -- this node.  */
        ...
        ptrdiff_t my_amount = ((tree->total_length - RIGHT_TOTAL_LENGTH(tree)) -
                               relative_position);
...
static void adjust_intervals_for_deletion(struct buffer* buffer,
    ...
    while (left_to_delete > 0) {
        left_to_delete -=
            interval_deletion_adjustment(tree, start - offset, left_to_delete);
...
void offset_intervals(struct buffer* buffer, ptrdiff_t start,
                      ptrdiff_t length) {
    if (!buffer_intervals(buffer) || length == 0)
        return;

    if (length > 0)
        adjust_intervals_for_insertion(buffer_intervals(buffer), start, length);
    else
...
static INTERVAL merge_interval_right(register INTERVAL i) {
...
static INTERVAL reproduce_interval(INTERVAL source) {
...
static INTERVAL reproduce_tree(INTERVAL source, INTERVAL parent) {
    INTERVAL target = reproduce_interval(source);
    set_interval_parent(target, parent);
    return target;
}

static INTERVAL reproduce_tree_obj(INTERVAL source, Lisp_Object parent) {
    INTERVAL target = reproduce_interval(source);
...
void graft_intervals_into_buffer(INTERVAL source, ptrdiff_t position,
    ...
    if (!source) {
        ...
        if (!inherit && tree && length > 0) {
            ...
            set_text_properties_1(make_fixnum(position),
                                  make_fixnum(position + length), Qnil, buf,
...
Lisp_Object textget(Lisp_Object plist, register Lisp_Object prop) {
    return lookup_char_property(plist, prop, 1);
}

Lisp_Object lookup_char_property(Lisp_Object plist, Lisp_Object prop,
                                 bool textprop) {
...
void temp_set_point_both(struct buffer* buffer, ptrdiff_t charpos,
                         ptrdiff_t bytepos) {
...
void temp_set_point(struct buffer* buffer, ptrdiff_t charpos) {
...
static ptrdiff_t adjust_for_invis_intang(ptrdiff_t pos, ptrdiff_t test_offs,
                                         ptrdiff_t adj, bool test_intang) {
    ...
    invis_propval = get_char_property_and_overlay(test_pos, Qinvisible, Qnil,
                                                  &invis_overlay);
    ...
    if ((!test_intang ||
       ...
       such that an insertion at POS would inherit it.  */
        && (NILP(invis_overlay)
                ...
                ? (text_property_stickiness(Qinvisible, make_fixnum(pos),
                                            Qnil) == (test_offs == 0 ? 1 : -1))
                /* Invisible property is from an overlay.  */
                : (test_offs == 0 ? !OVERLAY_FRONT_ADVANCE_P(invis_overlay)
...
void set_point_both(ptrdiff_t charpos, ptrdiff_t bytepos) {
    ...
    if (NILP(Vinhibit_point_motion_hooks) &&
       ...
       or end of the buffer, so don't bother checking in that case.  */
        ...
        if (backwards) {
            ...
            intangible_propval =
                Fget_char_property(make_fixnum(charpos - 1), Qintangible, Qnil);
    ...
    if (NILP(Vinhibit_point_motion_hooks) &&
        (!intervals_equal(from, to) || !intervals_equal(fromprev, toprev))) {
        ...
        if (!EQ(leave_after, enter_after) && !NILP(leave_after))
            calln(leave_after, make_fixnum(old_position), make_fixnum(charpos));

        if (!EQ(enter_before, leave_before) && !NILP(enter_before))
            calln(enter_before, make_fixnum(old_position),
                  make_fixnum(charpos));
        if (!EQ(enter_after, leave_after) && !NILP(enter_after))
            calln(enter_after, make_fixnum(old_position), make_fixnum(charpos));
    }
}
...
void move_if_not_intangible(ptrdiff_t position) {
    Lisp_Object pos;
    Lisp_Object intangible_propval;
...
bool get_property_and_range(ptrdiff_t pos, Lisp_Object prop, Lisp_Object* val,
    ...
    *val = textget(i->plist, prop);
    ...
    prev = previous_interval(i);
    while (prev && EQ(*val, textget(prev->plist, prop)))
...
Lisp_Object get_local_map(ptrdiff_t position, struct buffer* buffer,
                          Lisp_Object type) {
...
copy_intervals(INTERVAL tree, ptrdiff_t start, ptrdiff_t length) {
    ...
    if ((start - i->position + 1 + length) < LENGTH(i) && DEFAULT_INTERVAL_P(i))
        return NULL;
...
void copy_intervals_to_string(Lisp_Object string, struct buffer* buffer,
                              ptrdiff_t position, ptrdiff_t length) {
    INTERVAL interval_copy =
        copy_intervals(buffer_intervals(buffer), position, length);
    if (!interval_copy)
        return;
...
bool compare_string_intervals(Lisp_Object s1, Lisp_Object s2) {
    INTERVAL i1, i2;
    ptrdiff_t pos = 0;
...
static void set_intervals_multibyte_1(INTERVAL i, bool multi_flag,
    ...
    if (i->right) {
        ...
        set_intervals_multibyte_1(i->right, multi_flag, right_start,
                                  right_start_byte, end, end_byte);
    }
...
void set_intervals_multibyte(bool multi_flag) {
