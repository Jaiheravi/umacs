...
static void fill_up_glyph_row_area_with_spaces(struct frame*, struct glyph_row*,
                                               int);
static int scrolling_window(struct window*, int);
static bool update_window_line(struct window*, int, bool*);
static void mirror_make_current(struct window*, int);
#ifdef GLYPH_DEBUG
static void check_matrix_pointers(struct glyph_matrix*, struct glyph_matrix*);
#endif
...
static void add_window_display_history(struct window* w, const char* msg) {
    char* buf;
    void* ptr = w;
    ...
    snprintf(
        buf, sizeof redisplay_history[0].trace,
        "%" PRIuMAX ": window %p %s\n%s", history_tick++, ptr,
        ((BUFFERP(w->contents) && STRINGP(BVAR(XBUFFER(w->contents), name)))
...
static void add_frame_display_history(struct frame* f, bool paused_p) {
    char* buf;
    void* ptr = f;
    ...
    sprintf(buf, "%" PRIuMAX ": update frame %p%s", history_tick++, ptr,
            paused_p ? " ***paused***" : "");
}
...
void __executable_start(void) { emacs_abort(); }
#endif
...
static struct glyph_matrix* new_glyph_matrix(struct glyph_pool* pool) {
...
static void free_glyph_matrix(struct glyph_matrix* matrix) {
    if (matrix) {
        int i;
...
static int margin_glyphs_to_reserve(struct window* w, int total_glyphs,
                                    int margin) {
    if (margin > 0) {
...
static bool verify_row_hash(struct glyph_row* row) {
...
static void adjust_glyph_matrix(struct window* w, struct glyph_matrix* matrix,
    ...
    if (matrix->pool == NULL) {
        ...
        if (!marginal_areas_changed_p && !XFRAME(w->frame)->fonts_changed &&
    ...
    if (matrix->rows_allocated < dim.height) {
        ...
        matrix->rows = xpalloc(matrix->rows, &matrix->rows_allocated, new_rows,
    ...
    if (matrix->pool) {
        ...
        if (w) {
            left = margin_glyphs_to_reserve(w, dim.width, w->left_margin_cols);
            right =
                margin_glyphs_to_reserve(w, dim.width, w->right_margin_cols);
        }
        else
            left = right = 0;
        ...
        for (i = 0; i < dim.height; ++i) {
            ...
            row->glyphs[LEFT_MARGIN_AREA] =
                (matrix->pool->glyphs + (y + i) * matrix->pool->ncolumns + x);
       ...
       Allocate glyph memory from the heap.  */
        if (dim.width > matrix->matrix_w || new_rows || tab_line_changed_p ||
            header_line_changed_p || marginal_areas_changed_p) {
...
static void reverse_rows(struct glyph_matrix* matrix, int start, int end) {
    int i, j;
...
void rotate_matrix(struct glyph_matrix* matrix, int first, int last, int by) {
    if (by < 0) {
...
void increment_matrix_positions(struct glyph_matrix* matrix, int start, int end,
...
void clear_glyph_matrix_rows(struct glyph_matrix* matrix, int start, int end) {
    eassert(start <= end);
    eassert(start >= 0 &&
            (start < matrix->nrows
...
void clear_glyph_matrix(struct glyph_matrix* matrix) {
    if (matrix) {
...
void shift_glyph_matrix(struct window* w, struct glyph_matrix* matrix,
...
void clear_current_matrices(register struct frame* f) {
...
void clear_desired_matrices(register struct frame* f) {
    if (f->desired_matrix)
...
static void clear_window_matrices(struct window* w, bool desired_p) {
    while (w) {
...
void clear_glyph_row(struct glyph_row* row) {
    enum
    {
...
void blank_row(struct window* w, struct glyph_row* row, int y) {
    int min_y, max_y;
...
static void increment_row_positions(struct glyph_row* row, ptrdiff_t delta,
...
static void swap_glyph_pointers(struct glyph_row* a, struct glyph_row* b) {
    int i;
...
static void copy_row_except_pointers(struct glyph_row* to,
...
static void assign_row(struct glyph_row* to, struct glyph_row* from) {
...
static bool glyph_row_slice_p(struct glyph_row* window_row,
...
void prepare_desired_row(struct window* w, struct glyph_row* row,
                         bool mode_line_p) {
    if (!row->enabled_p) {
    ...
    if (mode_line_p) {
       ...
       and not tab/header/mode line.  */
        ...
        if (w->right_margin_cols > 0 &&
            ...
            /* Leave room for a border glyph.  */
            if (!FRAME_WINDOW_P(XFRAME(w->frame)) && !WINDOW_RIGHTMOST_P(w) &&
                right > 0)
                row->glyphs[RIGHT_MARGIN_AREA] -= 1;
        }
    }
...
static unsigned line_hash_code(struct frame* f, struct glyph_row* row) {
    unsigned hash = 0;
...
static int line_draw_cost(struct frame* f, struct glyph_matrix* matrix,
                          int vpos) {
...
static bool row_equal_p(struct glyph_row* a, struct glyph_row* b,
                        bool mouse_face_p) {
    ...
    if (a == b)
        ...
        if (a->fill_line_p != b->fill_line_p ||
            ...
            || a->x != b->x
            /* Different height.  */
            || a->ascent != b->ascent || a->phys_ascent != b->phys_ascent ||
...
static struct glyph_pool* ATTRIBUTE_MALLOC new_glyph_pool(void) {
...
static void free_glyph_pool(struct glyph_pool* pool) {
    if (pool) {
#if defined GLYPH_DEBUG && defined ENABLE_CHECKING
...
static bool realloc_glyph_pool(struct glyph_pool* pool, struct dim matrix_dim) {
    ptrdiff_t needed;
    bool changed_p;
...
void check_matrix_pointer_lossage(struct glyph_matrix* matrix) {
    int i, j;
...
struct glyph_row* matrix_row(struct glyph_matrix* matrix, int row) {
       ...
       on whether this is a combination or a leaf window.  */
        if (WINDOWP(w->contents))
            ...
            if (x != w->desired_matrix->matrix_x ||
                ...
                (margin_glyphs_to_reserve(w, dim.width, w->left_margin_cols) !=
...
static int required_matrix_height(struct window* w) {
#ifdef HAVE_WINDOW_SYSTEM
...
static int required_matrix_width(struct window* w) {
#ifdef HAVE_WINDOW_SYSTEM
    struct frame* f = XFRAME(w->frame);
    if (FRAME_WINDOW_P(f)) {
        /* https://lists.gnu.org/r/emacs-devel/2015-11/msg00194.html  */
...
static void allocate_matrices_for_window_redisplay(struct window* w) {
    while (w) {
        if (WINDOWP(w->contents))
...
void adjust_frame_glyphs(struct frame* f) {
    ...
    if (FRAME_WINDOW_P(f))
        ...
        eassert(FRAME_INITIAL_P(f) || noninteractive || !initialized ||
                ...
                || (f->current_matrix && f->current_matrix->nrows > 0 &&
                    ...
                    f->desired_matrix->nrows > 0 && f->desired_matrix->rows));
    }
...
static bool showing_window_margins_p(struct window* w) {
    while (w) {
        if (WINDOWP(w->contents)) {
...
static void fake_current_matrices(Lisp_Object window) {
    struct window* w;
...
static struct glyph_matrix* save_current_matrix(struct frame* f) {
    int i;
    ...
    for (i = 0; i < saved->nrows; ++i) {
        ...
        if (from->used[LEFT_MARGIN_AREA]) {
            ...
            memcpy(to->glyphs[LEFT_MARGIN_AREA], from->glyphs[LEFT_MARGIN_AREA],
                   nbytes);
...
static void restore_current_matrix(struct frame* f,
...
static void adjust_frame_glyphs_for_frame_redisplay(struct frame* f) {
    ...
    matrix_dim = allocate_matrices_for_frame_redisplay(
        FRAME_ROOT_WINDOW(f), 0, top_window_y, 1, &window_change_flags);
    ...
    if (pool_changed_p || window_change_flags) {
        ...
        allocate_matrices_for_frame_redisplay(
            FRAME_ROOT_WINDOW(f), 0, top_window_y, 0, &window_change_flags);
...
static void adjust_frame_glyphs_for_window_redisplay(struct frame* f) {
...
#if defined(HAVE_WINDOW_SYSTEM)
    {
        ...
        w->pixel_top =
            (FRAME_MENU_BAR_HEIGHT(f) +
             (!NILP(Vtab_bar_position) ? FRAME_TOOL_BAR_TOP_HEIGHT(f) : 0));
        w->top_line =
            (FRAME_MENU_BAR_LINES(f) +
             (!NILP(Vtab_bar_position) ? FRAME_TOOL_BAR_TOP_LINES(f) : 0));
        w->total_cols = FRAME_TOTAL_COLS(f);
        w->pixel_width =
            (FRAME_PIXEL_WIDTH(f) - 2 * FRAME_INTERNAL_BORDER_WIDTH(f));
...
#if defined(HAVE_WINDOW_SYSTEM) && !defined(HAVE_EXT_TOOL_BAR)
    {
        ...
        if (EQ(FRAME_TOOL_BAR_POSITION(f), Qbottom)) {
            w->pixel_top = (FRAME_PIXEL_HEIGHT(f) - FRAME_TOOL_BAR_HEIGHT(f));
...
static void adjust_decode_mode_spec_buffer(struct frame* f) {
    int frame_message_buf_size = FRAME_MESSAGE_BUF_SIZE(f);

    eassert(frame_message_buf_size >= 0);
    f->decode_mode_spec_buffer =
        xrealloc(f->decode_mode_spec_buffer, frame_message_buf_size + 1);
}
...
void free_glyphs(struct frame* f) {
...
void free_window_matrices(struct window* w) {
    while (w) {
        if (WINDOWP(w->contents))
...
static void build_frame_matrix(struct frame* f) {
    int i;
...
static void build_frame_matrix_from_window_tree(struct glyph_matrix* matrix,
                                                struct window* w) {
    while (w) {
        if (WINDOWP(w->contents))
...
static void
build_frame_matrix_from_leaf_window(struct glyph_matrix* frame_matrix,
                                    struct window* w) {
    ...
    while (window_y < window_matrix->nrows) {
       ...
       window row.  */
        ...
        /* If only part of W's desired matrix has been built, and
           ...
           row instead.  */
        if (window_matrix == w->desired_matrix && !window_row->enabled_p) {
...
void spec_glyph_lookup_face(struct window* w, GLYPH* glyph) {
...
static void fill_up_glyph_row_with_spaces(struct frame* f,
...
static void fill_up_glyph_row_area_with_spaces(struct frame* f,
                                               struct glyph_row* row,
                                               int area) {
...
void fill_up_frame_row_with_spaces(struct frame* f, struct glyph_row* row,
                                   int upto) {
...
static void make_current(struct frame* f, struct window* w, int row) {
...
static void mirror_make_current(struct window* w, int frame_row) {
    while (w) {
        if (WINDOWP(w->contents))
...
void mirrored_line_dance(struct frame* f, int unchanged_at_top, int nlines,
    ...
    mirror_line_dance(XWINDOW(f->root_window), unchanged_at_top, nlines,
...
static void sync_window_with_frame_matrix_rows(struct window* w) {
    struct frame* f = XFRAME(w->frame);
...
static struct window* frame_row_to_window(struct window* w, int row) {
...
static void mirror_line_dance(struct window* w, int unchanged_at_top,
                              int nlines, int* copy_from, char* retained_p) {
    while (w) {
        if (WINDOWP(w->contents))
            mirror_line_dance(XWINDOW(w->contents), unchanged_at_top, nlines,
                              copy_from, retained_p);
        else {
...
static void check_window_matrix_pointers(struct window* w) {
    struct frame* f = XFRAME(w->frame);
...
static void check_matrix_pointers(struct glyph_matrix* window_matrix,
    ...
    while (i < window_matrix->nrows) {
        if (!glyph_row_slice_p(window_matrix->rows + i, frame_matrix->rows + j))
            emacs_abort();
        ++i, ++j;
    }
}
...
static int window_to_frame_vpos(struct window* w, int vpos) {
...
static int window_to_frame_hpos(struct window* w, int hpos) {
    eassert(!FRAME_WINDOW_P(XFRAME(w->frame)));
...
void redraw_frame(struct frame* f) {
...
static bool rect_intersect(struct rect* r, struct rect r1, struct rect r2) {
    int x = max(r1.x, r2.x);
    int y = max(r1.y, r2.y);
...
static struct rect frame_rect_abs(struct frame* f) {
    int x, y;
    root_xy(f, 0, 0, &x, &y);
...
static bool frame_ancestors_visible_p(struct frame* f) {
    while (f) {
...
static Lisp_Object frames_with_root(struct frame* root, bool visible_only) {
...
static Lisp_Object frames_with_parent(struct frame* parent, bool visible_only) {
    Lisp_Object list = Qnil;
    Lisp_Object tail, frame;
    FOR_EACH_FRAME(tail, frame) {
        struct frame* f = XFRAME(frame);
...
static int frame_z_order_cmp(struct frame* f1, struct frame* f2) {
    if (f1 == f2)
        return 0;
...
DEFUN("frame--z-order-lessp", Fframe__z_order_lessp, Sframe__z_order_lessp, 2,
      2, 0, doc:/* Internal frame sorting function A < B.  */)
...
Lisp_Object frames_in_reverse_z_order(struct frame* f, bool visible_only) {
...
static int first_enabled_row(struct glyph_matrix* matrix) {
...
static void make_matrix_current(struct frame* f) {
...
static struct glyph_row* prepare_desired_root_row(struct frame* root, int y) {
...
static void make_glyph_space(struct glyph* glyph) {
    glyph->u.ch = ' ';
    glyph->pixel_width = 1;
    glyph->padding_p = 0;
}
...
static void neutralize_wide_char(struct frame* root, struct glyph_row* row,
                                 int x) {
...
static bool box_from_display_table(struct frame* f, enum box box, GLYPH* g) {
...
static void box_default(struct frame* f, enum box box, GLYPH* g) {
    int dflt UNINIT;
    switch (box) {
    case BOX_VERTICAL:
        dflt = '|';
...
static GLYPH box_glyph(struct frame* f, enum box box) {
    GLYPH g;
...
static void produce_box_glyphs(enum box box, struct glyph_row* row, int x,
                               int n, struct frame* child) {
...
static void produce_box_sides(enum box left, enum box right,
...
static void produce_box_line(struct frame* root, struct frame* child, int x,
    ...
    if (first)
        ...
        produce_box_sides(BOX_UP_RIGHT, BOX_UP_LEFT, root_row, x, w, root,
                          child);
...
static void copy_child_glyphs(struct frame* root, struct frame* child) {
    ...
    if (!FRAME_UNDECORATED(child)) {
        ...
        for (int y = r.y; y < r.y + r.h; ++y) {
            struct glyph_row* root_row = prepare_desired_root_row(root, y);
            if (root_row)
                produce_box_sides(BOX_VERTICAL, BOX_VERTICAL, root_row, r.x,
                                  r.w, root, child);
        }
...
static void update_menu_bar(struct frame* f) {
#if defined HAVE_WINDOW_SYSTEM && !defined HAVE_EXT_MENU_BAR
    if (WINDOWP(f->menu_bar_window))
...
#ifdef HAVE_WINDOW_SYSTEM
static void update_bar_window(Lisp_Object window, Lisp_Object* current,
                              Lisp_Object* desired) {
    if (WINDOWP(window)) {
...
static void update_tab_bar(struct frame* f) {
#if defined(HAVE_WINDOW_SYSTEM)
...
static void update_tool_bar(struct frame* f) {
#if defined(HAVE_WINDOW_SYSTEM) && !defined(HAVE_EXT_TOOL_BAR)
...
static void update_window_frame(struct frame* f) {
    eassert(FRAME_WINDOW_P(f));
    update_begin(f);
    update_menu_bar(f);
    update_tab_bar(f);
...
static void update_initial_frame(struct frame* f) {
    build_frame_matrix(f);
    struct window* root_window = XWINDOW(f->root_window);
    set_window_update_flags(root_window, false);
}

static void flush_terminal(struct frame* f) {
    if (FRAME_TTY(f)->termscript)
        fflush(FRAME_TTY(f)->termscript);
    fflush(FRAME_TTY(f)->output);
}

static void update_tty_frame(struct frame* f) { build_frame_matrix(f); }
...
static bool abs_cursor_pos(struct frame* f, int* x, int* y) {
...
static bool is_in_matrix(struct frame* f, int x, int y) {
    struct frame* root = root_frame(f);
...
static struct frame* frame_selected_window_frame(struct frame* f) {
    /* Paranoia.  It should not happen that window or frame not valid.  */
    Lisp_Object frame;
    if (WINDOWP(f->selected_window) &&
        (frame = XWINDOW(f->selected_window)->frame, FRAMEP(frame)))
        return XFRAME(frame);
    return NULL;
}
...
static void terminal_cursor_magic(struct frame* root,
...
void combine_updates_for_frame(struct frame* f, bool inhibit_scrolling) {
    struct frame* root = root_frame(f);
...
void combine_updates(Lisp_Object roots) {
...
void update_frame(struct frame* f, bool inhibit_scrolling) {
    if (FRAME_WINDOW_P(f))
...
void update_frame_with_menu(struct frame* f, int row, int col) {
    struct window* root_window = XWINDOW(f->root_window);
...
int update_mouse_position(struct frame* f, int x, int y) {
...
static void update_window_tree(struct window* w) {
    while (w) {
        if (WINDOWP(w->contents))
...
void update_single_window(struct window* w) {
    if (w->must_be_updated_p) {
...
static void redraw_overlapped_rows(struct window* w, int yb) {
    int i;
    struct frame* f = XFRAME(WINDOW_FRAME(w));
...
static void redraw_overlapping_rows(struct window* w, int yb) {
    int i, bottom_y;
    struct glyph_row* row;
...
static void check_current_matrix_flags(struct window* w) {
    bool last_seen_p = 0;
...
static void update_window(struct window* w) {
    ...
    if (mode_line_row->mode_line_p && mode_line_row->enabled_p) {
        ...
        update_window_line(w, MATRIX_ROW_VPOS(mode_line_row, desired_matrix),
    ...
    if (row < end && !desired_matrix->no_scrolling_p) {
        int rc = scrolling_window(w,
                                  (tab_line_row != NULL ? 1 : 0) +
    ...
    for (; row < end; ++row)
        ...
        if (row->enabled_p) {
            ...
            changed_p |= update_window_line(w, vpos, &mouse_face_overwritten_p);
...
void gui_update_window_begin(struct window* w) {
    struct frame* f = XFRAME(WINDOW_FRAME(w));
...
void gui_update_window_end(struct window* w, bool cursor_on_p,
...
static void update_marginal_area(struct window* w,
    ...
    if (desired_row->used[area])
        rif->write_glyphs(w, updated_row, desired_row->glyphs[area], area,
                          desired_row->used[area]);
...
static bool update_text_area(struct window* w, struct glyph_row* updated_row,
       ...
       in common.  */
        while (i < stop) {
            ...
            if (overlapping_glyphs_p && i > 0) {
                ...
                rif->get_glyph_overhangs(glyph, XFRAME(w->frame), &left,
                                         &right);
...
static bool update_window_line(struct window* w, int vpos,
                               bool* mouse_face_overwritten_p) {
    struct glyph_row* current_row = MATRIX_ROW(w->current_matrix, vpos);
    struct glyph_row* desired_row = MATRIX_ROW(w->desired_matrix, vpos);
    struct redisplay_interface* rif = FRAME_RIF(XFRAME(WINDOW_FRAME(w)));
...
static struct row_entry* add_row_entry(struct glyph_row* row) {
...
static int scrolling_window(struct window* w, int tab_line_p) {
    struct glyph_matrix* desired_matrix = w->desired_matrix;
    ...
    i = last_new;
    j = last_old;
    while (i - 1 > first_new && j - 1 > first_old &&
    ...
    if (current_matrix->nrows > old_lines_size)
        old_lines = xpalloc(old_lines, &old_lines_size,
                            current_matrix->nrows - old_lines_size, INT_MAX,
                            sizeof *old_lines);

    if (desired_matrix->nrows > new_lines_size)
        new_lines = xpalloc(new_lines, &new_lines_size,
                            desired_matrix->nrows - new_lines_size, INT_MAX,
                            sizeof *new_lines);
    ...
    for (i = 0; i < nruns; ++i)
        if (runs[i]->nrows > 0) {
            ...
            for (j = nruns - 1; j > i; --j) {
                ...
                if (p->nrows > 0 && p->desired_y < r->desired_y + r->height &&
                    ...
                    if (p->desired_y < r->desired_y) {
                        ...
                        int nrows_copied =
                            (r->desired_vpos + r->nrows - p->desired_vpos);
...
static void write_matrix(struct frame* f, bool inhibit_id_p,
...
static void scrolling(struct frame* frame) {
    ...
    if ((!FRAME_SCROLL_REGION_OK(frame) && changed_lines < baud_rate / 2400) ||
        unchanged_at_bottom == height) {
        SAFE_FREE();
        return;
    }
    ...
    if ((FRAME_SCROLL_REGION_OK(frame) || window_size < 18 ||
         ...
         (window_size < 10 *
              scrolling_max_lines_saved(unchanged_at_top,
                                        height - unchanged_at_bottom, old_hash,
                                        new_hash, draw_cost))) &&
        2 <= window_size)
...
static int count_blanks(struct frame* f, struct glyph* r, int len) {
    int i;
...
static int count_match(struct glyph* str1, struct glyph* end1,
                       struct glyph* str2, struct glyph* end2) {
    struct glyph* p1 = str1;
    struct glyph* p2 = str2;

    while (p1 < end1 && p2 < end2 && GLYPH_CHAR_AND_FACE_EQUAL_P(p1, p2))
        ++p1, ++p2;
...
static void write_row(struct frame* f, int vpos, bool updating_menu_p) {
       ...
       a glyph in the current row, and write the rest from there on.  */
        for (i = 0; i < nlen; i++) {
            if (i >= olen || !GLYPH_EQUAL_P(nbody + i, obody + i)) {
                ...
                j = i + 1;
                while (j < nlen &&
                       (j >= olen || !GLYPH_EQUAL_P(nbody + j, obody + j) ||
    ...
    begmatch =
        count_match(obody + osp, obody + olen, nbody + nsp, nbody + nlen);
...
Lisp_Object buffer_posn_from_coords(struct window* w, int* x, int* y,
                                    struct display_pos* pos,
                                    Lisp_Object* object, int* dx, int* dy,
    ...
    /* If we are hscrolling only the current line, and Y is at the line
       ...
       current line.  */
    if (it.line_wrap == TRUNCATE && EQ(automatic_hscrolling, Qcurrent_line) &&
        IT_CHARPOS(it) < PT) {
        struct it it2 = it;
    ...
    if (it.what == IT_COMPOSITION && it.cmp_it.nchars > 1 &&
       ...
       cluster.  */
        ...
        if (STRINGP(it.string))
            ...
            BYTEPOS(pos->pos) =
                buf_charpos_to_bytepos(XBUFFER(w->contents), CHARPOS(pos->pos));
    }
...
Lisp_Object mode_line_string(struct window* w, enum window_part part, int* x,
...
Lisp_Object marginal_area_string(struct window* w, enum window_part part,
                                 int* x, int* y, ptrdiff_t* charpos,
                                 Lisp_Object* object, int* dx, int* dy,
                                 int* width, int* height) {
...
static void handle_window_change_signal(int sig) {
    int width, height;
...
static void deliver_window_change_signal(int sig) {
...
void do_pending_window_change(bool safe) {
    ...
    while (delayed_size_change) {
        ...
        FOR_EACH_FRAME(tail, frame) {
            ...
            if (f->new_size_p && (f->new_height >= 0 || f->new_width >= 0))
                change_frame_size(f, f->new_width, f->new_height, false, false,
                                  safe);
        }
    }
}
...
static void change_frame_size_1(struct frame* f, int new_width, int new_height,
                                bool pretend, bool delay, bool safe) {
    if (delay || (redisplaying_p && !safe)) {
        if (CONSP(frame_size_history) &&
            ((new_width != f->new_width || new_height != f->new_height ||
              new_width != FRAME_PIXEL_WIDTH(f) ||
              new_height != FRAME_PIXEL_HEIGHT(f))))
            frame_size_history_extra(
                f, build_string("change_frame_size_1, delayed"),
                FRAME_PIXEL_WIDTH(f), FRAME_PIXEL_HEIGHT(f), new_width,
                new_height, f->new_width, f->new_height);
       ...
       small frames, the resulting values may be negative though.  */
        adjust_frame_size(f, FRAME_PIXEL_TO_TEXT_WIDTH(f, new_width),
                          FRAME_PIXEL_TO_TEXT_HEIGHT(f, new_height), 5, pretend,
                          Qchange_frame_size);
    }
}
...
void change_frame_size(struct frame* f, int new_width, int new_height,
                       bool pretend, bool delay, bool safe) {
    Lisp_Object tail, frame;
    ...
    if (FRAME_MSDOS_P(f) && !FRAME_PARENT_FRAME(f)) {
        ...
        if (!FRAME_WINDOW_P(XFRAME(frame)) &&
            ...
            change_frame_size_1(XFRAME(frame), new_width, new_height, pretend,
                                delay, safe);
    }
    else
...
bool frame_size_change_delayed(struct frame* f) {
...
Lisp_Object sit_for(Lisp_Object timeout, bool reading, int display_option) {
    intmax_t sec;
    int nsec;
...
static void init_faces_initial(void) {
...
static void init_display_interactive(void) {
    char* terminal_type;
...
#ifndef HAVE_ANDROID
    {
        ...
        change_frame_size(XFRAME(selected_frame),
                          ...
                          FrameRows(t->display_info.tty), false, false, true);
...
See `buffer-display-table' for more information.  */);
    ...
    DEFVAR_LISP("tab-bar-position", Vtab_bar_position, doc:
                    /* Specify on which side from the tool bar the tab bar shall
    be. Possible values are t (below the tool bar), nil (above the tool
    bar). This option affects only builds where the tool bar is not external.
  */);
