...
/* If BYTE_CODE_THREADED is defined, then the interpreter will be
   ...
   BYTE_CODE_METER.  */
#if (defined __GNUC__ && !defined __STRICT_ANSI__ && !BYTE_CODE_SAFE &&        \
     !defined BYTE_CODE_METER)
...
#define METER_2(code1, code2) (*aref_addr(AREF(Vbyte_code_meter, code1), code2))
...
#define BYTE_CODES                                                             \
    DEFINE(Bstack_ref,                                                         \
           0) /* Actually, Bstack_ref+0 is not implemented: use dup.  */       \
...
void init_bc_thread(struct bc_thread_state* bc) {
...
void free_bc_thread(struct bc_thread_state* bc) { xfree(bc->stack); }

void mark_bytecode(struct bc_thread_state* bc) {
...
DEFUN("internal-stack-stats", Finternal_stack_stats, Sinternal_stack_stats, 0,
      0, 0, doc:/* internal */)
(void) {
...
static bool valid_sp(struct bc_thread_state* bc, Lisp_Object* sp) {
    struct bc_frame* fp = bc->fp;
...
Lisp_Object exec_byte_code(Lisp_Object fun, ptrdiff_t args_template,
...
#undef DEFINE
        ...
        FIRST {
            ...
            CASE(Bvarref) :
                ...
                CASE(Bvarref2) :
                CASE(Bvarref3) :
                CASE(Bvarref4) : CASE(Bvarref5) : arg = op - Bvarref;
            goto varref;

            /* This seems to be the most frequently executed byte-code
               among the Bvarref's, so avoid a goto here.  */
            CASE(Bvarref6) : arg = FETCH;
        varref:
            {
            ...
            CASE(Bcar) : if (CONSP(TOP)) TOP = XCAR(TOP);
            else if (!NILP(TOP)) {
            ...
            CASE(Bvarset) :
                ...
                CASE(Bvarset2) :
                CASE(Bvarset3) :
                CASE(Bvarset4) : CASE(Bvarset5) : arg = op - Bvarset;
            goto varset;

            CASE(Bvarset7) : arg = FETCH2;
            goto varset;

            CASE(Bvarset6) : arg = FETCH;
        varset:
            {
            ...
            CASE(Bvarbind6) : arg = FETCH;
            goto varbind;

            CASE(Bvarbind7) : arg = FETCH2;
            goto varbind;
            ...
            CASE(Bvarbind) :
                ...
                CASE(Bvarbind2) :
                CASE(Bvarbind3) :
                CASE(Bvarbind4) : CASE(Bvarbind5) : arg = op - Bvarbind;
        varbind:
            /* Specbind can signal and thus GC.  */
            specbind(vectorp[arg], POP);
            NEXT;

            CASE(Bcall6) : arg = FETCH;
            goto docall;

            CASE(Bcall7) : arg = FETCH2;
            goto docall;

            CASE(Bcall) :
                CASE(Bcall1) :
                CASE(Bcall2) :
                CASE(Bcall3) : CASE(Bcall4) : CASE(Bcall5) : arg = op - Bcall;
        docall:
            {
                DISCARD(arg);
#ifdef BYTE_CODE_METER
                if (byte_metering_on && SYMBOLP(TOP)) {
                    ...
                    if (FIXNUMP(v2) && XFIXNUM(v2) < MOST_POSITIVE_FIXNUM) {
            ...
            CASE(Bunbind6) : arg = FETCH;
            goto dounbind;

            CASE(Bunbind7) : arg = FETCH2;
            goto dounbind;
            ...
            CASE(Bunbind) :
                ...
                CASE(Bunbind2) :
                CASE(Bunbind3) :
                CASE(Bunbind4) : CASE(Bunbind5) : arg = op - Bunbind;
        dounbind:
            unbind_to(specpdl_ref_add(SPECPDL_INDEX(), -arg), Qnil);
            NEXT;

            CASE(Bgoto) : arg = FETCH2;
        op_branch:
            {
            ...
            CASE(Bgotoifnonnil) : arg = FETCH2;
            if (!NILP(POP))
                goto op_branch;
            NEXT;

            CASE(Bgotoifnilelsepop) : arg = FETCH2;
            if (NILP(TOP))
                goto op_branch;
            DISCARD(1);
            NEXT;

            CASE(Bgotoifnonnilelsepop) : arg = FETCH2;
            if (!NILP(TOP))
                goto op_branch;
            DISCARD(1);
            NEXT;
            ...
            CASE(Bdiscard) : DISCARD(1);
            NEXT;

            CASE(Bconstant2) : PUSH(vectorp[FETCH2]);
            NEXT;

            CASE(Bsave_excursion) : record_unwind_protect_excursion();
            NEXT;

            CASE(Bsave_current_buffer_OBSOLETE) : /* Obsolete since 20.  */
                CASE(Bsave_current_buffer) : record_unwind_current_buffer();
            NEXT;
        ...
        pushhandler:
            {
                ...
                if (sys_setjmp(c->jmp)) {
                    /* No need to restore old quitcounter; just check at the
            ...
            CASE(Bsymbolp) : TOP = SYMBOLP(TOP) ? Qt : Qnil;
            NEXT;

            CASE(Bconsp) : TOP = CONSP(TOP) ? Qt : Qnil;
            NEXT;

            CASE(Bstringp) : TOP = STRINGP(TOP) ? Qt : Qnil;
            NEXT;

            CASE(Blistp) : TOP = CONSP(TOP) || NILP(TOP) ? Qt : Qnil;
            NEXT;

            CASE(Bnot) : TOP = NILP(TOP) ? Qt : Qnil;
            NEXT;
            ...
            CASE(Blist3) : DISCARD(2);
            TOP = list3(TOP, top[1], top[2]);
            NEXT;

            CASE(Blist4) : DISCARD(3);
            ...
            CASE(Bsymbol_value) : TOP = Fsymbol_value(TOP);
            NEXT;

            CASE(Bsymbol_function) : TOP = Fsymbol_function(TOP);
            NEXT;
            ...
            CASE(Bconcat2) : DISCARD(1);
            TOP = Fconcat(2, &TOP);
            NEXT;

            CASE(Bconcat3) : DISCARD(2);
            TOP = Fconcat(3, &TOP);
            NEXT;

            CASE(Bconcat4) : DISCARD(3);
            TOP = Fconcat(4, &TOP);
            NEXT;
            ...
            CASE(Bdiff) : {
                ...
                EMACS_INT res;
                if (FIXNUMP(v1) && FIXNUMP(v2) &&
                    (res = XFIXNUM(v1) - XFIXNUM(v2), !FIXNUM_OVERFLOW_P(res)))
                    TOP = make_fixnum(res);
                else
                    TOP = Fminus(2, &TOP);
                NEXT;
            ...
            CASE(Bpoint) : PUSH(make_fixed_natnum(PT));
            NEXT;

            CASE(Bgoto_char) : TOP = Fgoto_char(TOP);
            NEXT;

            CASE(Binsert) : TOP = Finsert(1, &TOP);
            NEXT;
            ...
            CASE(Bpoint_max) : PUSH(make_fixed_natnum(ZV));
            NEXT;

            CASE(Bpoint_min) : PUSH(make_fixed_natnum(BEGV));
            NEXT;

            CASE(Bchar_after) : TOP = Fchar_after(TOP);
            NEXT;

            CASE(Bfollowing_char) : PUSH(Ffollowing_char());
            NEXT;

            CASE(Bpreceding_char) : PUSH(Fprevious_char());
            NEXT;

            CASE(Bcurrent_column) : PUSH(make_fixed_natnum(current_column()));
            NEXT;

            CASE(Bindent_to) : TOP = Findent_to(TOP, Qnil);
            NEXT;

            CASE(Beolp) : PUSH(Feolp());
            NEXT;

            CASE(Beobp) : PUSH(Feobp());
            NEXT;

            CASE(Bbolp) : PUSH(Fbolp());
            NEXT;

            CASE(Bbobp) : PUSH(Fbobp());
            NEXT;

            CASE(Bcurrent_buffer) : PUSH(Fcurrent_buffer());
            NEXT;

            CASE(Bset_buffer) : TOP = Fset_buffer(TOP);
            NEXT;
            ...
            CASE(Bforward_char) : TOP = Fforward_char(TOP);
            NEXT;

            CASE(Bforward_word) : TOP = Fforward_word(TOP);
            NEXT;
            ...
            CASE(Bforward_line) : TOP = Fforward_line(TOP);
            NEXT;

            CASE(Bchar_syntax) : TOP = Fchar_syntax(TOP);
            NEXT;
            ...
            CASE(Bwiden) : PUSH(Fwiden());
            NEXT;

            CASE(Bend_of_line) : TOP = Fend_of_line(TOP);
            NEXT;
            ...
            CASE(Bmatch_beginning) : TOP = Fmatch_beginning(TOP);
            NEXT;

            CASE(Bmatch_end) : TOP = Fmatch_end(TOP);
            NEXT;

            CASE(Bupcase) : TOP = Fupcase(TOP);
            NEXT;

            CASE(Bdowncase) : TOP = Fdowncase(TOP);
            NEXT;
            ...
            CASE(Bnreverse) : TOP = Fnreverse(TOP);
            NEXT;
            ...
            CASE(Bcar_safe) : TOP = CAR_SAFE(TOP);
            NEXT;

            CASE(Bcdr_safe) : TOP = CDR_SAFE(TOP);
            NEXT;

            CASE(Bnconc) : DISCARD(1);
            TOP = Fnconc(2, &TOP);
            NEXT;

            CASE(Bnumberp) : TOP = NUMBERP(TOP) ? Qt : Qnil;
            NEXT;

            CASE(Bintegerp) : TOP = INTEGERP(TOP) ? Qt : Qnil;
            NEXT;
        ...
        CASE_ABORT:
            /* Actually this is Bstack_ref with offset 0, but we use Bdup
               for that instead.  */
            /* CASE (Bstack_ref): */
            error("Invalid byte opcode: op=%d, ptr=%" pD "d", pc[-1],
                  pc - 1 - bytestr_data);

            /* Handy byte-codes for lexical binding.  */
            CASE(Bstack_ref1) :
                CASE(Bstack_ref2) :
                CASE(Bstack_ref3) : CASE(Bstack_ref4) : CASE(Bstack_ref5) : {
            ...
            CASE(Bswitch) : {
               ...
               search as the jump table.  */
                ...
                /* Do a linear search if there are few cases and the test is
                   `eq'. (The table is assumed to be sized exactly; all entries
                   are consecutive at the beginning.)
...
Lisp_Object get_byte_code_arity(Lisp_Object args_template) {
    ...
    return Fcons(make_fixnum(mandatory), rest ? Qmany : make_fixnum(nonrest));
}
...
executed in succession.  */);
    ...
    DEFVAR_BOOL("byte-metering-on", byte_metering_on, doc:
                    /* If non-nil, keep profiling information on byte code
    usage. The variable byte-code-meter indicates how often each byte opcode
    is used. If a symbol has a property named `byte-code-meter' whose value is
    an integer, it is incremented each time that symbol's function is called.
  */);
