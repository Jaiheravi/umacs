...
static void diag(OFFSET xoff, OFFSET xlim, OFFSET yoff, OFFSET ylim,
                 bool find_minimal, struct partition* part,
                 struct context* ctxt) {
    OFFSET* const fd = ctxt->fdiag; /* Give the compiler a chance. */
    OFFSET* const bd = ctxt->bdiag; /* Additional help for the compiler. */
#ifdef ELEMENT
    ELEMENT const* const xv =
        ctxt->xvec; /* Still more help for the compiler. */
            ...
            if (200 < c && big_snake && heuristic) {
...

                {
                    ...
                    for (d = bmax; d >= bmin; d -= 2) {
                        ...
                        if (v > 12 * (c + (dd < 0 ? -dd : dd))) {
                            if (v > best && xoff < x &&
                                x <= xlim - SNAKE_LIMIT && yoff < y &&
                                y <= ylim - SNAKE_LIMIT) {
                                /* We have a good enough best diagonal; now
                                   insist that it end with a significant snake.
                                 */
                                for (int k = 0; XREF_YREF_EQUAL(x + k, y + k);
                                     k++)
                                    if (k == SNAKE_LIMIT - 1) {
                                        best = v;
                                        part->xmid = x;
