...
/* Copy the value from V into the memory location pointed to by *CP,
   ...
   * (__typeof__ (v) *) cp = v  */
static void set_uint64(char* cp, u64 v) { memcpy(cp, &v, sizeof v); }
...
void sha512_process_bytes(const void* buffer, size_t len,
                          struct sha512_ctx* ctx) {
...
void sha512_process_block(const void* buffer, size_t len,
                          struct sha512_ctx* ctx) {
...
#define M(I)                                                                   \
    (x[(I) & 15] =                                                             \
         u64plus(x[(I) & 15],                                                  \
                 u64plus(S1(x[((I) - 2) & 15]),                                \
                         u64plus(x[((I) - 7) & 15], S0(x[((I) - 15) & 15])))))

#define R(A, B, C, D, E, F, G, H, K, M)                                        \
    do {                                                                       \
        u64 t0 = u64plus(SS0(A), F2(A, B, C));                                 \
        u64 t1 =                                                               \
            u64plus(H, u64plus(SS1(E), u64plus(F1(E, F, G), u64plus(K, M))));  \
        D = u64plus(D, t1);                                                    \
        H = u64plus(t0, t1);                                                   \
    } while (0)
