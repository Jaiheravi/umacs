...
static size_t divide_round_up(size_t x, size_t y) { return (x + y - 1) / y; }

static const char dump_magic[16] = {'D', 'U', 'M', 'P', 'E', 'D', 'G',
                                    'N', 'U', 'E', 'M', 'A', 'C', 'S'};
...
enum
{
    EMACS_INT_XDIGITS = (EMACS_INT_WIDTH + 3) / 4
};

static void ATTRIBUTE_FORMAT_PRINTF(1, 2) dump_trace(const char* fmt, ...) {
    if (0) {
        va_list args;
        va_start(args, fmt);
...
static dump_off ptrdiff_t_to_dump_off(ptrdiff_t value) {
...
/* Worst-case allocation granularity on any system that might load
   this dump.  */
static int dump_get_max_page_size(void) { return 64 * 1024; }
...
static void emacs_reloc_set_type(struct emacs_reloc* reloc,
...
static void dump_reloc_set_type(struct dump_reloc* reloc,
                                enum dump_reloc_type type) {
    reloc->type = type;
    eassert(reloc->type == type);
}

static dump_off dump_reloc_get_offset(struct dump_reloc reloc) {
    return reloc.raw_offset << DUMP_RELOC_ALIGNMENT_BITS;
}

static void dump_reloc_set_offset(struct dump_reloc* reloc, dump_off offset) {
    eassert(offset >= 0);
...
void dump_fingerprint(FILE* output, char const* label,
...
static struct link_weight const WEIGHT_NONE = {.value = 0},
...
static void dump_grow_buffer(struct dump_context* ctx) {
    ctx->buf = xrealloc(ctx->buf,
                        ctx->buf_size = (ctx->buf_size ? (ctx->buf_size * 2)
                                                       : 8 * 1024 * 1024));
}
...
static Lisp_Object dump_push(Lisp_Object* where, Lisp_Object newelt) {
    *where = Fcons(newelt, *where);
    return newelt;
}

/* Like the Lisp function `pop'.  */
static Lisp_Object dump_pop(Lisp_Object* where) {
    Lisp_Object ret = XCAR(*where);
    *where = XCDR(*where);
    return ret;
}

static bool dump_tracking_referrers_p(struct dump_context* ctx) {
    return !NILP(ctx->referrers);
}

static void dump_set_have_current_referrer(struct dump_context* ctx,
                                           bool have) {
...
static bool dump_set_referrer(struct dump_context* ctx) {
...
static void dump_clear_referrer(struct dump_context* ctx) {
    eassert(ctx->have_current_referrer);
...
static Lisp_Object dump_ptr_referrer(const char* label, void const* address) {
    char buf[128];
...
static void print_paths_to_root(struct dump_context* ctx, Lisp_Object object);

static void dump_remember_cold_op(struct dump_context* ctx, enum cold_op op,
                                  Lisp_Object arg);

static AVOID error_unsupported_dump_object(struct dump_context* ctx,
                                           Lisp_Object object,
                                           const char* msg) {
    if (dump_tracking_referrers_p(ctx))
...
static uintptr_t emacs_basis(void) { return (uintptr_t)&Vpurify_flag; }

static void* emacs_ptr_at(const ptrdiff_t offset) {
    /* TODO: assert somehow that the result is actually in the Emacs
       image.  */
    return (void*)(emacs_basis() + offset);
}

static dump_off emacs_offset(const void* emacs_ptr) {
...
static bool dump_builtin_symbol_p(Lisp_Object object) {
...
*/
static bool dump_object_self_representing_p(Lisp_Object object) {
    return FIXNUMP(object) || dump_builtin_symbol_p(object);
}

static intmax_t intmax_t_from_lisp(Lisp_Object value) {
    intmax_t n;
...
static Lisp_Object intmax_t_to_lisp(intmax_t value) {
    return INT_TO_INTEGER(value);
}

static dump_off dump_off_from_lisp(Lisp_Object value) {
    intmax_t n = intmax_t_from_lisp(value);
    eassert(DUMP_OFF_MIN <= n && n <= DUMP_OFF_MAX);
    return n;
}

static Lisp_Object dump_off_to_lisp(dump_off value) {
    return INT_TO_INTEGER(value);
}

static void dump_write(struct dump_context* ctx, const void* buf,
                       dump_off nbyte) {
...
static Lisp_Object make_eq_hash_table(void) {
    return CALLN(Fmake_hash_table, QCtest, Qeq);
}

static void dump_tailq_init(struct dump_tailq* tailq) {
    tailq->head = tailq->tail = Qnil;
    tailq->length = 0;
}

static intptr_t dump_tailq_length(const struct dump_tailq* tailq) {
    return tailq->length;
}

static void dump_tailq_prepend(struct dump_tailq* tailq, Lisp_Object value) {
...
static bool dump_tailq_empty_p(struct dump_tailq* tailq) {
    return NILP(tailq->head);
}

static Lisp_Object dump_tailq_peek(struct dump_tailq* tailq) {
    eassert(!dump_tailq_empty_p(tailq));
    return XCAR(tailq->head);
}

static Lisp_Object dump_tailq_pop(struct dump_tailq* tailq) {
    eassert(!dump_tailq_empty_p(tailq));
...
static void dump_seek(struct dump_context* ctx, dump_off offset) {
...
static void dump_write_zero(struct dump_context* ctx, dump_off nbytes) {
    while (nbytes > 0) {
        uintmax_t zero = 0;
...
static void dump_align_output(struct dump_context* ctx, int alignment) {
    if (ctx->offset % alignment != 0)
        dump_write_zero(ctx, alignment - (ctx->offset % alignment));
}

static dump_off dump_object_start(struct dump_context* ctx, void* out,
                                  dump_off outsz) {
...
static dump_off dump_object_finish(struct dump_context* ctx, const void* out,
                                   dump_off sz) {
...
/* Return offset at which OBJECT has been dumped, or one of the
   dump_object_special_offset negative values, or DUMP_OBJECT_NOT_SEEN.  */
static dump_off dump_recall_object(struct dump_context* ctx,
                                   Lisp_Object object) {
    Lisp_Object dumped = ctx->objects_dumped;
    return dump_off_from_lisp(
        Fgethash(object, dumped, make_fixnum(DUMP_OBJECT_NOT_SEEN)));
}

static void dump_remember_object(struct dump_context* ctx, Lisp_Object object,
                                 dump_off offset) {
    Fputhash(object, dump_off_to_lisp(offset), ctx->objects_dumped);
}

static void dump_note_reachable(struct dump_context* ctx, Lisp_Object object) {
    eassert(ctx->have_current_referrer);
    if (!dump_tracking_referrers_p(ctx))
        return;
...
static void* dump_object_emacs_ptr(Lisp_Object lv) {
...
static void dump_queue_init(struct dump_queue* dump_queue) {
...
static bool dump_queue_empty_p(struct dump_queue* dump_queue) {
...
static void dump_queue_push_weight(Lisp_Object* weight_list, dump_off basis,
                                   struct link_weight weight) {
    if (EQ(*weight_list, Qt))
        *weight_list = Qnil;
    dump_push(weight_list,
              Fcons(dump_off_to_lisp(basis), dump_off_to_lisp(weight.value)));
}

static void dump_queue_enqueue(struct dump_queue* dump_queue,
                               Lisp_Object object, dump_off basis,
                               struct link_weight weight) {
    ...
    if (NILP(weights)) {
        ...
        Fputhash(object, dump_off_to_lisp(dump_queue->next_sequence_number++),
...
static float dump_calc_link_score(dump_off basis, dump_off link_basis,
...
/* Compute the score for a queued object.
   ...
   score.  */
static float dump_queue_compute_score(struct dump_queue* dump_queue,
                                      Lisp_Object object, dump_off basis) {
    float score = 0;
...
static float dump_queue_scan_fancy(struct dump_queue* dump_queue,
                                   dump_off basis,
    ...
    while (!NILP(*cons_ptr)) {
        ...
        float score =
            dump_queue_compute_score(dump_queue, queued_object, basis);
...
static dump_off dump_queue_sequence(struct dump_queue* dump_queue,
                                    Lisp_Object object) {
...
static void dump_queue_find_score_of_one_weight_queue(
    struct dump_queue* dump_queue, dump_off basis,
    struct dump_tailq* one_weight_queue, float* out_score, int* out_sequence) {
        ...
        if (dump_tailq_empty_p(one_weight_queue)) {
            ...
            *out_sequence = dump_queue_sequence(dump_queue, head);
            if (*out_sequence < 0)
                ...
                *out_score = dump_queue_compute_score(dump_queue, head, basis);
        }
...
static Lisp_Object dump_queue_dequeue(struct dump_queue* dump_queue,
                                      dump_off basis) {
    ...
    dump_trace(("dump_queue_dequeue basis=%" PRIdDUMP_OFF " fancy=%" PRIdPTR
                " zero=%" PRIdPTR " normal=%" PRIdPTR " strong=%" PRIdPTR
                " hash=%td\n"),
               basis, dump_tailq_length(&dump_queue->fancy_weight_objects),
               dump_tailq_length(&dump_queue->zero_weight_objects),
               dump_tailq_length(&dump_queue->one_weight_normal_objects),
               dump_tailq_length(&dump_queue->one_weight_strong_objects),
...
#define nr_candidates 3
    ...
    do {
        ...
        candidates[0].score =
            dump_queue_scan_fancy(dump_queue, basis, &fancy_cons);
        candidates[0].sequence = candidates[0].score > -INFINITY
            ? dump_queue_sequence(dump_queue, XCAR(*fancy_cons))
            : 0;
    } while (candidates[0].sequence < 0);

    dump_queue_find_score_of_one_weight_queue(
        dump_queue, basis, &dump_queue->one_weight_normal_objects,
        &candidates[1].score, &candidates[1].sequence);

    dump_queue_find_score_of_one_weight_queue(
        dump_queue, basis, &dump_queue->one_weight_strong_objects,
        &candidates[2].score, &candidates[2].sequence);

    int best = -1;
    for (int i = 0; i < nr_candidates; ++i) {
        eassert(candidates[i].sequence >= 0);
        if (candidates[i].score > -INFINITY &&
            (best < 0 || candidates[i].score > candidates[best].score ||
...
#undef nr_candidates
    ...
    dump_trace("  result score=%f src=%s object=%0*" pI "x\n",
               best < 0 ? -1.0 : (double)candidates[best].score, src,
               EMACS_INT_XDIGITS, uresult);

    {
        ...
        while (!NILP(weights) && CONSP(weights)) {
            ...
            dump_trace(
                ...
                link_basis, basis - link_basis, link_weight,
...
static bool dump_object_needs_dumping_p(Lisp_Object object) {
    return !(FIXNUMP(object));
}

static void dump_enqueue_object(struct dump_context* ctx, Lisp_Object object,
                                struct link_weight weight) {
...
static void print_paths_to_root_1(struct dump_context* ctx, Lisp_Object object,
                                  int level) {
...
static void dump_reloc_dump_to_emacs_ptr_raw(struct dump_context* ctx,
                                             dump_off dump_offset) {
    if (ctx->flags.dump_object_contents)
...
static void dump_reloc_dump_to_dump_lv(struct dump_context* ctx,
                                       dump_off dump_offset,
                                       enum Lisp_Type type) {
    if (!ctx->flags.dump_object_contents)
        return;
    ...
    dump_push(&ctx->dump_relocs[EARLY_RELOCS],
              list2(make_fixnum(reloc_type), dump_off_to_lisp(dump_offset)));
}
...
/* Add a dump relocation that points a raw pointer back at the dump.
   ...
   value at DUMP_OFFSET in the dump file should contain the offset of
   the target object relative to the start of the dump.  */
static void dump_reloc_dump_to_dump_ptr_raw(struct dump_context* ctx,
                                            dump_off dump_offset) {
    if (ctx->flags.dump_object_contents)
        dump_push(&ctx->dump_relocs[EARLY_RELOCS],
...
/* Add a dump relocation that points to a Lisp object in Emacs.
   ...
   Lisp value.  */
static void dump_reloc_dump_to_emacs_lv(struct dump_context* ctx,
                                        dump_off dump_offset,
                                        enum Lisp_Type type) {
    if (!ctx->flags.dump_object_contents)
        return;
...
/* Add an Emacs relocation that copies arbitrary bytes from the dump.
   ...
   dump file).  */
static void dump_emacs_reloc_copy_from_dump(struct dump_context* ctx,
                                            dump_off dump_offset,
    ...
    dump_push(&ctx->emacs_relocs,
              list4(make_fixnum(RELOC_EMACS_COPY_FROM_DUMP),
                    ...
                    dump_off_to_lisp(dump_offset), dump_off_to_lisp(size)));
}
...
/* Add an Emacs relocation that sets values to arbitrary bytes.
   ...
   from VALUE_PTR.
 */
static void dump_emacs_reloc_immediate(struct dump_context* ctx,
    ...
    dump_push(&ctx->emacs_relocs,
              list4(make_fixnum(RELOC_EMACS_IMMEDIATE),
                    dump_off_to_lisp(emacs_offset(emacs_ptr)),
                    intmax_t_to_lisp(value), dump_off_to_lisp(size)));
}

#define DEFINE_EMACS_IMMEDIATE_FN(fnname, type)                                \
    static void fnname(struct dump_context* ctx, const type* emacs_ptr,        \
                       type value) {                                           \
        dump_emacs_reloc_immediate(ctx, emacs_ptr, &value, sizeof(value));     \
    }
...
/* Add an emacs relocation that makes a raw pointer in Emacs point
   into the dump.  */
static void dump_emacs_reloc_to_dump_ptr_raw(struct dump_context* ctx,
                                             const void* emacs_ptr,
                                             dump_off dump_offset) {
    if (!ctx->flags.dump_object_contents)
        return;
...
static void dump_emacs_reloc_to_lv(struct dump_context* ctx,
        ...
        if (ctx->flags.dump_object_contents)
            ...
            dump_push(&ctx->emacs_relocs,
                      list3(make_fixnum(dump_object_emacs_ptr(value)
                                            ? RELOC_EMACS_EMACS_LV
                                            : RELOC_EMACS_DUMP_LV),
                            dump_off_to_lisp(emacs_offset(emacs_ptr)), value));
...
static void dump_emacs_reloc_to_emacs_ptr_raw(struct dump_context* ctx,
                                              void* emacs_ptr,
 ...
 */
static void dump_remember_fixup_lv(struct dump_context* ctx,
                                   dump_off dump_offset, Lisp_Object value,
    ...
    dump_push(&ctx->fixups,
              list3(make_fixnum(fixup_subtype == LV_FIXUP_LISP_OBJECT
                                    ? DUMP_FIXUP_LISP_OBJECT
                                    : DUMP_FIXUP_LISP_OBJECT_RAW),
                    dump_off_to_lisp(dump_offset), value));
}
...
static void dump_remember_fixup_ptr_raw(struct dump_context* ctx,
                                        dump_off dump_offset,
...
static void dump_root_visitor(Lisp_Object const* root_ptr,
                              enum gc_root_type type, void* data) {
    ...
    if (type == GC_ROOT_C_SYMBOL) {
        ...
        if (type == GC_ROOT_STATICPRO)
            Fputhash(dump_off_to_lisp(emacs_offset(root_ptr)), Qt,
                     ctx->staticpro_table);
...
static void dump_roots(struct dump_context* ctx) {
    struct gc_root_visitor visitor = {.visit = dump_root_visitor, .data = ctx};
    visit_static_gc_roots(visitor);
}

enum
{
    PDUMPER_MAX_OBJECT_SIZE = 2048
};

static dump_off field_relpos(const void* in_start, const void* in_field) {
...
static void cpyptr(void* out, const void* in) {
    memcpy(out, in, sizeof(void*));
}

/* Convenience macro for regular assignment.  */
#define DUMP_FIELD_COPY(out, in, name) ((out)->name = (in)->name)

static void
dump_field_lv_or_rawptr(struct dump_context* ctx, void* out,
                        const void* in_start, const void* in_field,
    ...
    if (DANGEROUS && target_offset > 0 &&
        dump_object_emacs_ptr(value) == NULL) {
        ...
        dump_remember_fixup_lv(
            ctx, out_field_offset, value,
            (is_ptr_raw ? LV_FIXUP_RAW_POINTER : LV_FIXUP_LISP_OBJECT));
 ...
 */
static void dump_field_lv_rawptr(struct dump_context* ctx, void* out,
                                 const void* in_start, const void* in_field,
                                 enum Lisp_Type type,
                                 struct link_weight weight) {
...
static void dump_field_lv(struct dump_context* ctx, void* out,
                          const void* in_start, const Lisp_Object* in_field,
                          struct link_weight weight) {
...
/* Note that we're going to add a manual fixup for the given field
   later.  */
static void dump_field_fixup_later(struct dump_context* ctx, void* out,
                                   const void* in_start, const void* in_field) {
...
static void dump_field_ptr_to_dump_offset(struct dump_context* ctx, void* out,
                                          const void* in_start,
                                          const void* in_field,
 ...
 */
static void dump_field_emacs_ptr(struct dump_context* ctx, void* out,
                                 const void* in_start, const void* in_field) {
    eassert(ctx->obj_offset > 0);
    if (!ctx->flags.dump_object_contents)
        return;
...
static dump_off finish_dump_pvec(struct dump_context* ctx,
                                 union vectorlike_header* out_hdr) {
...
static dump_off dump_cons(struct dump_context* ctx,
                          const struct Lisp_Cons* cons) {
...
static dump_off dump_interval_tree(struct dump_context* ctx, INTERVAL tree,
...
#error "interval changed. See CHECK_STRUCTS comment in config.h."
    ...
    if (tree->left)
        dump_remember_fixup_ptr_raw(
            ctx, offset + dump_offsetof(struct interval, left),
            dump_interval_tree(ctx, tree->left, offset));
    if (tree->right)
        dump_remember_fixup_ptr_raw(
            ctx, offset + dump_offsetof(struct interval, right),
            dump_interval_tree(ctx, tree->right, offset));
    return offset;
}

static dump_off dump_string(struct dump_context* ctx,
                            const struct Lisp_String* string) {
...
#error "Lisp_String changed. See CHECK_STRUCTS comment in config.h."
    ...
    if (string->u.s.size_byte == -2)
        ...
        dump_remember_cold_op(ctx, COLD_OP_STRING,
    ...
    if (string->u.s.intervals)
        dump_remember_fixup_ptr_raw(
            ctx, offset + dump_offsetof(struct Lisp_String, u.s.intervals),
            dump_interval_tree(ctx, string->u.s.intervals, 0));

    return offset;
}

static dump_off dump_marker(struct dump_context* ctx,
                            const struct Lisp_Marker* marker) {
...
#error "Lisp_Marker changed. See CHECK_STRUCTS comment in config.h."
    ...
    if (marker->buffer) {
        dump_field_lv_rawptr(ctx, out, marker, &marker->buffer, Lisp_Vectorlike,
                             WEIGHT_NORMAL);
        dump_field_lv_rawptr(ctx, out, marker, &marker->next, Lisp_Vectorlike,
                             WEIGHT_STRONG);
...
static dump_off dump_interval_node(struct dump_context* ctx,
...
#error "itree_node changed. See CHECK_STRUCTS comment in config.h."
    ...
    if (node->parent)
        dump_remember_fixup_ptr_raw(
            ctx, offset + dump_offsetof(struct itree_node, parent),
            dump_interval_node(ctx, node->parent));
    if (node->left)
        dump_remember_fixup_ptr_raw(
            ctx, offset + dump_offsetof(struct itree_node, left),
            dump_interval_node(ctx, node->left));
    if (node->right)
        dump_remember_fixup_ptr_raw(
            ctx, offset + dump_offsetof(struct itree_node, right),
            dump_interval_node(ctx, node->right));
    return offset;
}

static dump_off dump_overlay(struct dump_context* ctx,
                             const struct Lisp_Overlay* overlay) {
...
#error "Lisp_Overlay changed. See CHECK_STRUCTS comment in config.h."
    ...
    dump_remember_fixup_ptr_raw(
        ctx, offset + dump_offsetof(struct Lisp_Overlay, interval),
        dump_interval_node(ctx, overlay->interval));
    return offset;
}

static void dump_field_finalizer_ref(struct dump_context* ctx, void* out,
    ...
    if (*field == &finalizers || *field == &doomed_finalizers)
        ...
        dump_field_lv_rawptr(ctx, out, finalizer, field, Lisp_Vectorlike,
                             WEIGHT_NORMAL);
}

static dump_off dump_finalizer(struct dump_context* ctx,
                               const struct Lisp_Finalizer* finalizer) {
...
static dump_off dump_treesit_compiled_query(struct dump_context* ctx,
...
static dump_off dump_bignum(struct dump_context* ctx, Lisp_Object object) {
...
#error "Lisp_Bignum changed. See CHECK_STRUCTS comment in config.h."
    ...
    if (ctx->flags.dump_object_contents) {
        ...
        dump_off value_offset =
            (bignum_offset + (dump_off)offsetof(struct Lisp_Bignum, value));
        dump_push(&ctx->fixups,
                  list3(make_fixnum(DUMP_FIXUP_BIGNUM_DATA),
                        dump_off_to_lisp(value_offset), object));
        ...
        dump_push(
            &ctx->dump_relocs[EARLY_RELOCS],
            list2(make_fixnum(RELOC_BIGNUM), dump_off_to_lisp(bignum_offset)));
    }

    return bignum_offset;
}

static dump_off dump_float(struct dump_context* ctx,
                           const struct Lisp_Float* lfloat) {
...
static void dump_field_fwd(struct dump_context* ctx, void* out,
                           const void* in_start, const lispfwd* in_field) {
...
static dump_off dump_blv(struct dump_context* ctx,
...
static dump_off dump_recall_symbol_aux(struct dump_context* ctx,
                                       Lisp_Object symbol) {
...
static void dump_remember_symbol_aux(struct dump_context* ctx,
                                     Lisp_Object symbol, dump_off offset) {
...
static void dump_pre_dump_symbol(struct dump_context* ctx,
...
static dump_off dump_symbol(struct dump_context* ctx, Lisp_Object object,
                            dump_off offset) {
...
#error "symbol_redirect changed. See CHECK_STRUCTS comment in config.h."
    ...
    case SYMBOL_PLAINVAL:
        dump_field_lv(ctx, &out, symbol, &symbol->u.s.val.value, WEIGHT_NORMAL);
        break;
    case SYMBOL_VARALIAS:
        dump_field_lv_rawptr(ctx, &out, symbol, &symbol->u.s.val.alias,
    ...
    case SYMBOL_LOCALIZED:
        ...
        dump_remember_fixup_ptr_raw(
            ctx, offset + dump_offsetof(struct Lisp_Symbol, u.s.val.blv),
            (aux_offset ? aux_offset : dump_blv(ctx, symbol->u.s.val.blv)));
        break;
    default:
        break;
    }
...
static dump_off dump_vectorlike_generic(struct dump_context* ctx,
...
#error "vectorlike_header changed. See CHECK_STRUCTS comment in config.h."
    ...
    dump_off skip_start =
        ptrdiff_t_to_dump_off((char*)&v->contents[skip] - (char*)v);
...
static Lisp_Object* hash_table_contents(struct Lisp_Hash_Table* h) {
    ptrdiff_t size = h->count;
    Lisp_Object* key_and_value =
        hash_table_alloc_bytes(2 * size * sizeof *key_and_value);
    ptrdiff_t n = 0;
...
static void dump_hash_table_list(struct dump_context* ctx) {
...
/* Compact contents and discard inessential information from a hash table,
   ...
   state. */
static void hash_table_freeze(struct Lisp_Hash_Table* h) {
...
static dump_off dump_hash_table_contents(struct dump_context* ctx,
...
static dump_off dump_hash_table(struct dump_context* ctx, Lisp_Object object) {
...
#error "Lisp_Hash_Table changed. See CHECK_STRUCTS comment in config.h."
    ...
    if (hash->key_and_value)
        dump_remember_fixup_ptr_raw(
            ctx, offset + dump_offsetof(struct Lisp_Hash_Table, key_and_value),
            dump_hash_table_contents(ctx, hash));
    return offset;
}

static dump_off dump_obarray_buckets(struct dump_context* ctx,
                                     const struct Lisp_Obarray* o) {
    dump_align_output(ctx, DUMP_ALIGNMENT);
    dump_off start_offset = ctx->offset;
...
static dump_off dump_obarray(struct dump_context* ctx, Lisp_Object object) {
...
#error "Lisp_Obarray changed. See CHECK_STRUCTS comment in config.h."
    ...
    dump_off offset = finish_dump_pvec(ctx, &out->header);
    dump_remember_fixup_ptr_raw(
        ctx, offset + dump_offsetof(struct Lisp_Obarray, buckets),
        dump_obarray_buckets(ctx, oa));
    return offset;
}

static dump_off dump_buffer(struct dump_context* ctx,
                            const struct buffer* in_buffer) {
...
#error "buffer changed. See CHECK_STRUCTS comment in config.h."
    ...
    if (buffer->base_buffer) {
        ...
        base_offset = dump_object_for_offset(
            ctx, make_lisp_ptr(buffer->base_buffer, Lisp_Vectorlike));
    }
    ...
    memcpy(out->local_flags, &buffer->local_flags, sizeof(out->local_flags));
    ...
    dump_field_lv(ctx, out, buffer, &buffer->undo_list_, WEIGHT_STRONG);
    dump_off offset = finish_dump_pvec(ctx, &out->header);
    if (!buffer->base_buffer && buffer->own_text.intervals)
        dump_remember_fixup_ptr_raw(
            ctx, offset + dump_offsetof(struct buffer, own_text.intervals),
            dump_interval_tree(ctx, buffer->own_text.intervals, 0));

    return offset;
}

static dump_off dump_bool_vector(struct dump_context* ctx,
                                 const struct Lisp_Vector* v) {
...
static dump_off dump_subr(struct dump_context* ctx,
                          const struct Lisp_Subr* subr) {
...
#error "Lisp_Subr changed. See CHECK_STRUCTS comment in config.h."
    ...
    if (non_primitive) {
        ...
        dump_remember_cold_op(ctx, COLD_OP_NATIVE_SUBR,
    ...
    if (non_primitive && ctx->flags.dump_object_contents)
        ...
        dump_push(
            &ctx->dump_relocs[VERY_LATE_RELOCS],
            list2(make_fixnum(RELOC_NATIVE_SUBR), dump_off_to_lisp(subr_off)));
#endif
    return subr_off;
}

#ifdef HAVE_NATIVE_COMP
static dump_off dump_native_comp_unit(struct dump_context* ctx,
...
static void fill_pseudovec(union vectorlike_header* header, Lisp_Object item) {
...
static dump_off dump_nilled_pseudovec(struct dump_context* ctx,
...
static dump_off dump_vectorlike(struct dump_context* ctx, Lisp_Object lv,
                                dump_off offset) {
...
static dump_off dump_charset(struct dump_context* ctx, int cs_i) {
...
#error "charset changed. See CHECK_STRUCTS comment in config.h."
    ...
    if (cs_i < charset_table_used && cs->code_space_mask)
        dump_remember_cold_op(
            ctx, COLD_OP_CHARSET,
            Fcons(dump_off_to_lisp(cs_i), dump_off_to_lisp(offset)));
    return offset;
}

static dump_off dump_charset_table(struct dump_context* ctx) {
    struct dump_flags old_flags = ctx->flags;
    ctx->flags.pack_objects = true;
...
static void dump_finalizer_list_head_ptr(struct dump_context* ctx,
    ...
    if (value != &finalizers && value != &doomed_finalizers)
        dump_emacs_reloc_to_dump_ptr_raw(
            ctx, ptr,
            dump_object_for_offset(ctx, make_lisp_ptr(value, Lisp_Vectorlike)));
}

static void dump_metadata_for_pdumper(struct dump_context* ctx) {
...
static void dump_sort_copied_objects(struct dump_context* ctx) {
    ...
    /* Sort the objects into the order in which they'll appear in the
       ...
       most effective.  */
    ctx->copied_queue = CALLN(Fsort, queue_reversed,
                              Qdump_emacs_portable__sort_predicate_copied);
}

/* Dump parts of copied objects we need at runtime.  */
static void dump_hot_parts_of_discardable_objects(struct dump_context* ctx) {
...
static void dump_drain_copied_objects(struct dump_context* ctx) {
    Lisp_Object copied_queue = ctx->copied_queue;
...
static void dump_cold_string(struct dump_context* ctx, Lisp_Object string) {
    ...
    dump_remember_fixup_ptr_raw(
        ctx, string_offset + dump_offsetof(struct Lisp_String, u.s.data),
        ctx->offset);
    dump_write(ctx, XSTRING(string)->u.s.data, total_size);
}

static void dump_cold_charset(struct dump_context* ctx, Lisp_Object data) {
    /* Dump charset lookup tables.  */
    int cs_i = XFIXNUM(XCAR(data));
    dump_off cs_dump_offset = dump_off_from_lisp(XCDR(data));
    dump_remember_fixup_ptr_raw(
        ctx, cs_dump_offset + dump_offsetof(struct charset, code_space_mask),
        ctx->offset);
    struct charset* cs = charset_table + cs_i;
    dump_write(ctx, cs->code_space_mask, 256);
}

static void dump_cold_buffer(struct dump_context* ctx, Lisp_Object data) {
    /* Dump buffer text.  */
    ...
    dump_remember_fixup_ptr_raw(
        ctx, buffer_offset + dump_offsetof(struct buffer, own_text.beg),
        ctx->offset);
    dump_write(ctx, b->own_text.beg, ptrdiff_t_to_dump_off(nbytes));
}

static void dump_cold_bignum(struct dump_context* ctx, Lisp_Object object) {
...
#ifdef HAVE_NATIVE_COMP
static void dump_cold_native_subr(struct dump_context* ctx, Lisp_Object subr) {
    /* Dump subr contents.  */
    dump_off subr_offset = dump_recall_object(ctx, subr);
    eassert(subr_offset > 0);
    dump_remember_fixup_ptr_raw(
        ctx, subr_offset + dump_offsetof(struct Lisp_Subr, symbol_name),
        ctx->offset);
    const char* symbol_name = XSUBR(subr)->symbol_name;
    dump_write(ctx, symbol_name, 1 + strlen(symbol_name));

    dump_remember_fixup_ptr_raw(
        ctx, subr_offset + dump_offsetof(struct Lisp_Subr, native_c_name),
        ctx->offset);
...
static void dump_drain_cold_data(struct dump_context* ctx) {
...
static void read_ptr_raw_and_lv(const void* mem, enum Lisp_Type type,
                                void** out_ptr, Lisp_Object* out_lv) {
...
static void dump_drain_user_remembered_data_hot(struct dump_context* ctx) {
    for (int i = 0; i < nr_remembered_data; ++i) {
...
static void dump_drain_user_remembered_data_cold(struct dump_context* ctx) {
    for (int i = 0; i < nr_remembered_data; ++i) {
        void* mem = remembered_data[i].mem;
        int sz = remembered_data[i].sz;
        if (sz > 0) {
...
static void dump_unwind_cleanup(void* data) {
    struct dump_context* ctx = data;
    if (ctx->fd >= 0)
        emacs_close(ctx->fd);
#ifdef REL_ALLOC
...
static void dump_check_dump_off(struct dump_context* ctx,
                                dump_off dump_offset) {
    eassert(dump_offset > 0);
    eassert(!ctx || dump_offset < ctx->end_heap);
}

static void dump_check_emacs_off(dump_off emacs_off) {
    eassert(labs(emacs_off) <= 60 * 1024 * 1024);
}

static struct dump_reloc dump_decode_dump_reloc(Lisp_Object lreloc) {
...
static void dump_emit_dump_reloc(struct dump_context* ctx, Lisp_Object lreloc) {
    eassert(ctx->flags.pack_objects);
    struct dump_reloc reloc;
...
#ifdef ENABLE_CHECKING
static Lisp_Object dump_check_overlap_dump_reloc(Lisp_Object lreloc_a,
...
/* Translate a Lisp Emacs-relocation descriptor (a list whose first
   ...
   dump file.
*/
static struct emacs_reloc decode_emacs_reloc(struct dump_context* ctx,
                                             Lisp_Object lreloc) {
    ...
    case RELOC_EMACS_EMACS_LV:
        {
            ...
            if (type == RELOC_EMACS_EMACS_LV) {
                ...
                if (reloc.u.dump_offset <= 0) {
                    Lisp_Object repr =
                        Fprin1_to_string(target_value, Qnil, Qnil);
...
static void dump_emit_emacs_reloc(struct dump_context* ctx,
                                  Lisp_Object lreloc) {
    eassert(ctx->flags.pack_objects);
...
static Lisp_Object dump_merge_emacs_relocs(Lisp_Object lreloc_a,
                                           Lisp_Object lreloc_b) {
...
static void drain_reloc_list(struct dump_context* ctx,
                             drain_reloc_handler handler,
                             drain_reloc_merger merger, Lisp_Object* reloc_list,
                             struct dump_table_locator* out_locator) {
    struct dump_flags old_flags = ctx->flags;
    Lisp_Object list_reversed, relocs;
    ctx->flags.pack_objects = true;
    list_reversed = Fnreverse(*reloc_list);
    relocs = CALLN(Fsort, list_reversed, Qdump_emacs_portable__sort_predicate);
    *reloc_list = Qnil;
    dump_align_output(
        ctx, max(alignof(struct dump_reloc), alignof(struct emacs_reloc)));
...
static void dump_do_fixup(struct dump_context* ctx, Lisp_Object fixup,
                          Lisp_Object prev_fixup) {
    enum dump_fixup_type type = (enum dump_fixup_type)XFIXNUM(dump_pop(&fixup));
...
static void dump_do_fixups(struct dump_context* ctx) {
    dump_off saved_offset = ctx->offset;
    Lisp_Object fixups_reversed = Fnreverse(ctx->fixups);
    Lisp_Object fixups =
        CALLN(Fsort, fixups_reversed, Qdump_emacs_portable__sort_predicate);
...
static void dump_drain_normal_queue(struct dump_context* ctx) {
    while (!dump_queue_empty_p(&ctx->dump_queue))
        dump_object(ctx, dump_queue_dequeue(&ctx->dump_queue, ctx->offset));
}

static void dump_drain_deferred_hash_tables(struct dump_context* ctx) {
    struct dump_flags old_flags = ctx->flags;
...
static void dump_drain_deferred_symbols(struct dump_context* ctx) {
    struct dump_flags old_flags = ctx->flags;
...
DEFUN("dump-emacs-portable", Fdump_emacs_portable, Sdump_emacs_portable, 1, 2,
      0, doc:
...
(Lisp_Object filename, Lisp_Object track_referrers) {
...

    {
        ...
        if (len >= 5 && !strcmp(filename_1 + len - 5, ".pdmp")) {
    ...
    do {
        ...
        dump_drain_normal_queue(ctx);
               ...
               NILP(ctx->deferred_hash_tables) && NILP(ctx->deferred_symbols)));
    ...
    dump_off header_bytes = header_end - header_start,
             ...
             discardable_bytes =
                 discardable_end - ctx->header.discardable_start,
...
DEFUN("dump-emacs-portable--sort-predicate",
      ...
      Sdump_emacs_portable__sort_predicate, 2, 2, 0,
...
DEFUN("dump-emacs-portable--sort-predicate-copied",
      ...
      Sdump_emacs_portable__sort_predicate_copied, 2, 2, 0,
      doc:/* Internal relocation sorting function.  */)
(Lisp_Object a, Lisp_Object b) {
...
void pdumper_do_now_and_after_load_impl(pdumper_hook hook) {
...
void pdumper_do_now_and_after_late_load_impl(pdumper_hook hook) {
...
static void pdumper_remember_user_data_1(void* mem, int nbytes) {
...
void pdumper_remember_scalar_impl(void* mem, ptrdiff_t nbytes) {
    eassert(0 <= nbytes && nbytes <= INT_MAX);
    if (nbytes > 0)
        pdumper_remember_user_data_1(mem, (int)nbytes);
}

void pdumper_remember_lv_ptr_raw_impl(void* ptr, enum Lisp_Type type) {
...
static void*
dump_anonymous_allocate_w32(void* base, size_t size,
    ...
    if (ret == NULL)
        errno =
            (base && GetLastError() == ERROR_INVALID_ADDRESS) ? EBUSY : EPERM;
    return ret;
}
#endif
...
static void*
dump_anonymous_allocate_posix(void* base, size_t size,
                              enum dump_memory_protection protection) {
    void* ret;
    int mem_prot;
    ...
    do {
        ...
        if (ret == MAP_FAILED && errno == EINVAL &&
            (mem_flags & MAP_POPULATE)) {
...
static void dump_anonymous_release(void* addr, size_t size) {
    eassert(size >= 0);
#if VM_SUPPORTED == VM_MS_WINDOWS
    (void)size;
...
static void* dump_anonymous_allocate(void* base, const size_t size,
                                     enum dump_memory_protection protection) {
    void* val;
...
#if VM_SUPPORTED == VM_MS_WINDOWS
static void* dump_map_file_w32(void* base, int fd, off_t offset, size_t size,
                               enum dump_memory_protection protection) {
    void* ret = NULL;
    HANDLE section = NULL;
    HANDLE file;
    ...
    ret = MapViewOfFileEx(section, map_access, offset_high, offset_low, size,
                          base);
...
#if VM_SUPPORTED == VM_POSIX
static void* dump_map_file_posix(void* base, int fd, off_t offset, size_t size,
                                 enum dump_memory_protection protection) {
    void* ret;
    int mem_prot;
    int mem_flags;
...
static void* dump_map_file(void* base, int fd, off_t offset, size_t size,
                           enum dump_memory_protection protection) {
#if VM_SUPPORTED == VM_POSIX
...
static void dump_unmap_file(void* addr, size_t size) {
    eassert(size >= 0);
#if !VM_SUPPORTED
    (void)addr;
    (void)size;
...
static void dump_discard_mem(void* mem, size_t size) {
#if VM_SUPPORTED == VM_MS_WINDOWS
    /* Discard COWed pages.  */
...
static void dump_mmap_discard_contents(struct dump_memory_map* map) {
    if (map->mapping)
        dump_discard_mem(map->mapping, map->spec.size);
}

static void dump_mmap_reset(struct dump_memory_map* map) {
    map->mapping = NULL;
    map->release = NULL;
    map->private = NULL;
}

static void dump_mmap_release(struct dump_memory_map* map) {
    if (map->release)
        map->release(map);
    dump_mmap_reset(map);
}
...
static void dump_mmap_release_heap(struct dump_memory_map* map) {
    dump_mm_heap_cb_release(map->private);
}

/* Implement dump_mmap using malloc and read.  */
static bool dump_mmap_contiguous_heap(struct dump_memory_map* maps, int nr_maps,
                                      size_t total_size) {
    bool ret = false;
...
static void dump_mmap_release_vm(struct dump_memory_map* map) {
    if (map->spec.fd < 0)
...
static bool needs_mmap_retry_p(void) {
...
static bool dump_mmap_contiguous_vm(struct dump_memory_map* maps, int nr_maps,
                                    size_t total_size) {
    int save_errno;
    bool ret = false;
    void* resv = NULL;
    ...
    do {
        ...
        eassert(resv == NULL);
        resv =
            dump_anonymous_allocate(NULL, total_size, DUMP_MEMORY_ACCESS_NONE);
        if (!resv)
            goto out;
        ...
        for (int i = 0; i < nr_maps; ++i) {
            ...
            if (spec.fd < 0)
                map->mapping =
                    dump_anonymous_allocate(mem, spec.size, spec.protection);
            else
...
static bool dump_mmap_contiguous(struct dump_memory_map* maps, int nr_maps) {
    if (!nr_maps)
        return true;
...
static bool dump_bitsets_init(struct dump_bitset bitset[2],
                              size_t number_bits) {
    int xword_size = sizeof(dump_bitset_word);
    ptrdiff_t words_needed =
        divide_round_up(number_bits, DUMP_BITSET_WORD_WIDTH);
    ...
    bitset[1].bits =
        memset(bits + words_needed, UCHAR_MAX, words_needed * xword_size);
    return true;
}

static dump_bitset_word* dump_bitset__bit_slot(const struct dump_bitset* bitset,
                                               size_t bit_number) {
...
static bool dump_bitset_bit_set_p(const struct dump_bitset* bitset,
                                  size_t bit_number) {
...
static void dump_bitset__set_bit_value(struct dump_bitset* bitset,
                                       size_t bit_number, bool bit_is_set) {
...
static void dump_bitset_set_bit(struct dump_bitset* bitset, size_t bit_number) {
    dump_bitset__set_bit_value(bitset, bit_number, true);
}

static void dump_bitset_clear(struct dump_bitset* bitset) {
...
/* Return a pointer to offset OFFSET within the dump, which begins at
   ...
   using this function.  */
static void* dump_ptr(uintptr_t dump_base, dump_off offset) {
...
static uintptr_t dump_read_word_from_dump(uintptr_t dump_base,
                                          dump_off offset) {
    uintptr_t value;
...
/* Write a word to the dump. DUMP_BASE and OFFSET are as for
   ...
   offset.  */
static void dump_write_word_to_dump(uintptr_t dump_base, dump_off offset,
                                    uintptr_t value) {
...
/* Write a Lisp_Object to the dump. DUMP_BASE and OFFSET are as for
   ...
   given offset.  */
static void dump_write_lv_to_dump(uintptr_t dump_base, dump_off offset,
                                  Lisp_Object value) {
    /* The compiler optimizes this memcpy into a write.  */
    memcpy(dump_ptr(dump_base, offset), &value, sizeof(value));
}
...
static bool dump_loaded_p(void) { return dump_public.start != 0; }

bool pdumper_cold_object_p_impl(const void* obj) {
...
int pdumper_find_object_type_impl(const void* obj) {
    eassert(pdumper_object_p(obj));
    dump_off offset = ptrdiff_t_to_dump_off((uintptr_t)obj - dump_public.start);
...
bool pdumper_marked_p_impl(const void* obj) {
    eassert(pdumper_object_p(obj));
...
void pdumper_set_marked_impl(const void* obj) {
    eassert(pdumper_object_p(obj));
    ptrdiff_t offset = (uintptr_t)obj - dump_public.start;
    eassert(offset % DUMP_ALIGNMENT == 0);
    eassert(offset < dump_private.header.cold_start);
...
void pdumper_clear_marks_impl(void) {
...
/* Return the number of bytes written when we perform the given
   relocation.  */
static int dump_reloc_size(const struct dump_reloc reloc) {
...
static Lisp_Object dump_make_lv_from_reloc(const uintptr_t dump_base,
                                           const struct dump_reloc reloc) {
...
static inline void dump_do_dump_relocation(const uintptr_t dump_base,
                                           const struct dump_reloc reloc) {
    const dump_off reloc_offset = dump_reloc_get_offset(reloc);
    ...
    case RELOC_NATIVE_SUBR:
        {
            ...
            if (!comp_u->handle)
                error("NULL handle in compilation unit %s",
                      SSDATA(comp_u->file));
            const char* c_name = subr->native_c_name;
            eassert(c_name);
            void* func = dynlib_sym(comp_u->handle, c_name);
            if (!func)
                error("can't find function \"%s\" in compilation unit %s",
                      c_name, SSDATA(comp_u->file));
            subr->function.a0 = func;
            Lisp_Object lambda_data_idx = Fgethash(
                build_string(c_name), comp_u->lambda_c_name_idx_h, Qnil);
...
dump_do_all_dump_reloc_for_phase(const struct dump_header* const header,
    ...
    struct dump_reloc* r =
        dump_ptr(dump_base, header->dump_relocs[phase].offset);
...
static void dump_do_emacs_relocation(const uintptr_t dump_base,
    ...
    case RELOC_EMACS_COPY_FROM_DUMP:
        ...
        memcpy(emacs_ptr_at(reloc.emacs_offset),
               dump_ptr(dump_base, reloc.u.dump_offset), reloc.length);
        break;
    case RELOC_EMACS_IMMEDIATE:
        eassume(reloc.length > 0);
        eassume(reloc.length <= sizeof(reloc.u.immediate));
        memcpy(emacs_ptr_at(reloc.emacs_offset), &reloc.u.immediate,
               reloc.length);
        break;
    case RELOC_EMACS_DUMP_PTR_RAW:
...
static void pdumper_set_emacs_execdir(char* emacs_executable) {
    char* p = emacs_executable + strlen(emacs_executable);

    while (p > emacs_executable && !IS_DIRECTORY_SEP(p[-1]))
        --p;
...
int pdumper_load(const char* dump_filename, char* argv0) {
    ...
    if (dump_fd < 0) {
        err =
            (errno == ENOENT || errno == ENOTDIR ? PDUMPER_LOAD_FILE_NOT_FOUND
    ...
    err = PDUMPER_LOAD_BAD_FILE_TYPE;
    if (dump_read_all(dump_fd, header, sizeof(*header)) < sizeof(*header))
        goto out;
    ...
    if ((dump_base + dump_size) & ~VALMASK) {
        fprintf(stderr, "Failed to load dump file: 0x%p+0x%p & ~0x%p != 0\n",
                (void*)dump_base, (void*)dump_size, (void*)(uintptr_t)VALMASK);
        goto out;
    }
#endif /* !USE_LSB_TAG */
...
void pdumper_record_wd(const char* wd) {
    if (wd && !file_name_absolute_p(dump_private.dump_filename)) {
        char* dfn =
            xmalloc(strlen(wd) + 1 + strlen(dump_private.dump_filename) + 1);
...
void syms_of_pdumper(void) {
    ...
    DEFVAR_LISP("pdumper-fingerprint", Vpdumper_fingerprint, doc:
                    /* The fingerprint of this Emacs binary.
    It is a string that is supposed to be unique to each build of
    Emacs.  */);
    Vpdumper_fingerprint = make_unibyte_string((char*)hexbuf, sizeof hexbuf);
#endif /* HAVE_PDUMPER */
}
