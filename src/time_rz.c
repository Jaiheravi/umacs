...
enum
{
    ABBR_SIZE_MIN = DEFAULT_MXFAST - offsetof(struct tm_zone, abbrs)
};
...
timezone_t tzalloc(char const* name) {
    ...
    size_t abbr_size =
        name_size < ABBR_SIZE_MIN ? ABBR_SIZE_MIN : name_size + 1;
...
timezone_t set_tz(timezone_t tz) {
    ...
    if (env_tz ? tz->tz_is_set && streq(tz->abbrs, env_tz) : !tz->tz_is_set)
        return local_tz;
    else {
