...
enum
{
    GMP_NUMB_BITS = TYPE_WIDTH(mp_limb_t)
};
#endif
...
INLINE struct Lisp_Bignum* XBIGNUM(Lisp_Object a) {
    eassert(BIGNUMP(a));
    return XUNTAG(a, Lisp_Vectorlike, struct Lisp_Bignum);
}

INLINE void ARG_NONNULL((1)) mpz_set_intmax(mpz_t result, intmax_t v) {
...
INLINE void ARG_NONNULL((1)) mpz_set_uintmax(mpz_t result, uintmax_t v) {
    unsigned long int i;
    if (FASTER_BIGNUM && !ckd_add(&i, v, 0))
        mpz_set_ui(result, i);
    else
...
INLINE mpz_t const* bignum_val(struct Lisp_Bignum const* i) {
    return &i->value;
}
INLINE mpz_t const* xbignum_val(Lisp_Object i) {
...
INLINE mpz_t const* bignum_integer(mpz_t* tmp, Lisp_Object i) {
    if (FIXNUMP(i)) {
...
/* Set RESULT to the value stored in the Lisp integer I.  If I is a
   ...
   initialized.  */
INLINE void mpz_set_integer(mpz_t result, Lisp_Object i) {
    if (FIXNUMP(i))
