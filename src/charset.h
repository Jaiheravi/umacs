...
#define CHARSET_SUBSET(charset) CHARSET_ATTR_SUBSET(CHARSET_ATTRIBUTES(charset))
...
INLINE void set_charset_attr(struct charset* charset,
                             enum charset_attr_index idx, Lisp_Object val) {
...
#define CHAR_CHARSET(c)                                                        \
    ((c) < 0x80 ? CHARSET_FROM_ID(charset_ascii) : char_charset(c, Qnil, NULL))
...
#define CHAR_CHARSET_SET(c) CHAR_TABLE_REF(Vchar_charset_set, c)
...
#define ENCODE_CHAR(charset, c)                                                \
    (verify_expr(                                                              \
        sizeof(c) <= sizeof(int),                                              \
        (ASCII_CHAR_P(c) && (charset)->ascii_compatible_p ? (unsigned)(c)      \
             : ((charset)->unified_p ||                                        \
             ...
             ? (((charset)->compact_codes_p &&                                 \
                 CHAR_TABLE_P(CHARSET_ENCODER(charset)))                       \
                    ? (charset_work =                                          \
                       ...
                       (NILP(charset_work) ? (charset)->invalid_code           \
...
extern void map_charset_chars(void (*)(Lisp_Object, Lisp_Object), Lisp_Object,
                              Lisp_Object, struct charset*, unsigned, unsigned);
