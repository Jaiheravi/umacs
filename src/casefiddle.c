...
static void prepare_casing_context(struct casing_context* ctx,
...
static int case_character_impl(struct casing_str_buf* buf,
...
static bool case_character(struct casing_str_buf* buf,
    ...
    /* If we have just down-cased a capital sigma and the next character no
       longer has a word syntax (i.e. current character is end of word), use
       final sigma.  */
...
static inline int make_char_unibyte(int c) {
    return ASCII_CHAR_P(c) ? c : CHAR_TO_BYTE8(c);
}

static Lisp_Object do_casify_natnum(struct casing_context* ctx,
                                    Lisp_Object obj) {
    int flagbits = (CHAR_ALT | CHAR_SUPER | CHAR_HYPER | CHAR_SHIFT | CHAR_CTL |
                    CHAR_META);
    int ch = XFIXNAT(obj);

    /* If the character has higher bits set above the flags, return it
       unchanged. It is not a real character.  */
    if (!(0 <= ch && ch <= flagbits))
        return obj;
    ...
    /* FIXME: Even if enable-multibyte-characters is nil, we may manipulate
       multibyte chars.  This means we have a bug for latin-1 chars since when
       we receive an int 128-255 we can't tell whether it's an eight-bit byte or
       a latin-1 char.  */
    bool multibyte =
...
static Lisp_Object do_casify_multibyte_string(struct casing_context* ctx,
                                              Lisp_Object obj) {
...
static int ascii_casify_character(bool downcase, int c) {
    Lisp_Object cased = CHAR_TABLE_REF(
        downcase ? uniprop_table(Qlowercase) : uniprop_table(Quppercase), c);
    return FIXNATP(cased) ? XFIXNAT(cased) : c;
}

static Lisp_Object do_casify_unibyte_string(struct casing_context* ctx,
                                            Lisp_Object obj) {
...
static Lisp_Object casify_object(enum case_action flag, Lisp_Object obj) {
...
static ptrdiff_t do_casify_unibyte_region(struct casing_context* ctx,
...
static ptrdiff_t do_casify_multibyte_region(struct casing_context* ctx,
                                            ptrdiff_t* startp,
                                            ptrdiff_t* endp) {
    ptrdiff_t first = -1, last = -1; /* Position of first and last changes.  */
    ...
    for (; size; --size) {
        ...
        if (buf.len_chars == 1 && buf.len_bytes == len)
            ...
            replace_range_2(pos, pos_byte, pos + 1, pos_byte + len,
                            (const char*)buf.data, buf.len_chars, buf.len_bytes,
                            0);
...
static ptrdiff_t casify_region(enum case_action flag, Lisp_Object b,
                               Lisp_Object e) {
    ptrdiff_t added;
    struct casing_context ctx;
    ...
    struct ts_linecol start_linecol = treesit_linecol_maybe(
        start, start_byte, BUF_TS_LINECOL_POINT(current_buffer));
    struct ts_linecol old_end_linecol = treesit_linecol_maybe(
        end, old_end_byte, BUF_TS_LINECOL_POINT(current_buffer));
#endif
    ...
    treesit_record_change(start_byte, old_end_byte, new_end_byte, start_linecol,
                          old_end_linecol, new_end);
#endif
...
static Lisp_Object casify_pnc_region(enum case_action flag, Lisp_Object beg,
                                     Lisp_Object end,
...
static Lisp_Object casify_word(enum case_action flag, Lisp_Object arg) {
    CHECK_FIXNUM(arg);
...
(Lisp_Object arg) { return casify_word(CASE_UP, arg); }
...
With negative argument, convert previous words but do not move.  */)
(Lisp_Object arg) { return casify_word(CASE_DOWN, arg); }
...
(Lisp_Object arg) { return casify_word(CASE_CAPITALIZE, arg); }
