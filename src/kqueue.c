...
static Lisp_Object kqueue_directory_listing(Lisp_Object directory_files) {
...
static void kqueue_generate_event(Lisp_Object watch_object, Lisp_Object actions,
    ...
    if (!NILP(actions)) {
        ...
        event.arg =
            list2(Fcons(XCAR(watch_object),
                        Fcons(actions,
                              NILP(file1) ? list1(file) : list2(file, file1))),
...
/* This compares two directory listings in case of a `write' event for
   ...
   function.  */
static void kqueue_compare_dir_list(Lisp_Object watch_object) {
    ...
    while (1) {
        ...
        if (!NILP(new_entry)) {
            ...
            if (strcmp(SSDATA(XCAR(XCDR(old_entry))),
                ...
                /* Modification time has been changed, the file has been
                 * written.  */
                if (NILP(Fequal(Fnth(make_fixnum(2), old_entry),
                                Fnth(make_fixnum(2), new_entry))))
                    kqueue_generate_event(watch_object, Fcons(Qwrite, Qnil),
                ...
                if (NILP(Fequal(Fnth(make_fixnum(3), old_entry),
                    ...
                    kqueue_generate_event(watch_object, Fcons(Qattrib, Qnil),
                                          XCAR(XCDR(old_entry)), Qnil);
            }
            else {
                /* The file has been renamed.  */
                kqueue_generate_event(watch_object, Fcons(Qrename, Qnil),
                                      XCAR(XCDR(old_entry)),
                                      XCAR(XCDR(new_entry)));
        ...
        if (NILP(new_entry)) {
            ...
            kqueue_generate_event(watch_object, Fcons(Qdelete, Qnil),
                                  XCAR(XCDR(old_entry)), Qnil);
        }
        else {
            /* The file has been renamed.  */
    ...
    while (1) {
        ...
        entry = XCAR(dl);
        kqueue_generate_event(watch_object, Fcons(Qcreate, Qnil),
                              XCAR(XCDR(entry)), Qnil);
...
static void kqueue_callback(int fd, void* data) {
    for (;;) {
        struct kevent kev;
...
(Lisp_Object file, Lisp_Object flags, Lisp_Object callback) {
    ...
    if (!NILP(Fmember(Qdelete, flags)))
        fflags |= NOTE_DELETE;
    if (!NILP(Fmember(Qwrite, flags)))
        fflags |= NOTE_WRITE;
    if (!NILP(Fmember(Qextend, flags)))
        fflags |= NOTE_EXTEND;
    if (!NILP(Fmember(Qattrib, flags)))
        fflags |= NOTE_ATTRIB;
    if (!NILP(Fmember(Qlink, flags)))
        fflags |= NOTE_LINK;
    if (!NILP(Fmember(Qrename, flags)))
        fflags |= NOTE_RENAME;
    if (!NILP(Fmember(Qrevoke, flags)))
        fflags |= NOTE_REVOKE;
    ...
    if (NILP(Ffile_directory_p(file)))
        ...
        dir_list =
            directory_files_internal(file, Qnil, Qnil, Qnil, true, Qnil, Qnil);
...
void globals_of_kqueue(void) { watch_list = Qnil; }

void syms_of_kqueue(void) {
