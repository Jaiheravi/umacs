...
static void init_and_cache_system_name(void) {
    init_system_name();
...
(Lisp_Object string) {
    ...
    return make_fixnum(STRING_MULTIBYTE(string) ? STRING_CHAR(SDATA(string))
                                                : SREF(string, 0));
}
...
static Lisp_Object region_limit(bool beginningp) {
    Lisp_Object m;
    ...
    if (!BUFFERP(object))
        ...
        ITREE_FOREACH(node, b->overlays, posn - 1, posn + 1, ASCENDING) {
            ...
            if (NILP(tem)
                ...
                || ((node->begin == posn && OVERLAY_FRONT_ADVANCE_P(ol)) ||
                    (node->end == posn && !OVERLAY_REAR_ADVANCE_P(ol)) ||
                    node->begin > posn || node->end < posn))
...
void find_field(Lisp_Object pos, Lisp_Object merge_at_boundary,
                Lisp_Object beg_limit, ptrdiff_t* beg, Lisp_Object end_limit,
                ptrdiff_t* end) {
    ...
    after_field = get_char_property_and_overlay(pos, Qfield, Qnil, NULL);
    before_field =
        (XFIXNAT(pos) > BEGV
    ...
    if (end) {
        ...
        /* Find the next field boundary.  */
        {
            ...
            pos =
                Fnext_single_char_property_change(pos, Qfield, Qnil, end_limit);
        ...
        (NILP(inhibit_capture_property)
         ...
         (NILP(Fget_pos_property(old_pos, inhibit_capture_property, Qnil)) &&
          (XFIXNAT(old_pos) <= BEGV ||
           ...
           NILP(
               Fget_char_property(prev_old, inhibit_capture_property, Qnil))))))
        ...
        if (/* See if ESCAPE_FROM_EDGE caused FIELD_BOUND to jump to the
            ...
            && (NILP(only_in_line)
               ...
               there's an intervening newline or not.  */
                || (find_newline(XFIXNAT(new_pos), -1, XFIXNAT(field_bound), -1,
...
static ptrdiff_t bol(Lisp_Object n, ptrdiff_t* out_count) {
...
static ptrdiff_t eol(Lisp_Object n) {
    ptrdiff_t count;
...
(Lisp_Object n) {
    ...
    return Fconstrain_to_field(make_fixnum(eol(n)), make_fixnum(PT), Qnil, Qt,
                               Qnil);
}
...
void save_excursion_restore(Lisp_Object marker, Lisp_Object window) {
...
DEFUN("save-current-buffer", Fsave_current_buffer, Ssave_current_buffer, 0,
      UNEVALLED, 0, doc:
          /* Record which buffer is current; execute BODY; make that buffer
current. BODY is executed just like `progn'. usage: (save-current-buffer &rest
BODY)  */)
(Lisp_Object args) {
    specpdl_ref count = SPECPDL_INDEX();
...
(Lisp_Object buffer) {
    if (NILP(buffer))
        ...
        CHECK_BUFFER(buffer);
        return make_fixnum(BUF_Z(XBUFFER(buffer)) - BUF_BEG(XBUFFER(buffer)));
    }
}
...
static void general_insert_function(
    ...
    for (argnum = 0; argnum < nargs; argnum++) {
        ...
        if (CHARACTERP(val)) {
            ...
            (*insert_from_string_func)(val, 0, 0, SCHARS(val), SBYTES(val),
                                       inherit);
        }
        else
...
void insert1(Lisp_Object arg) { Finsert(1, &arg); }
...
(ptrdiff_t nargs, Lisp_Object* args) {
    general_insert_function(insert_and_inherit, insert_from_string, 1, nargs,
                            args);
    return Qnil;
}
...
(ptrdiff_t nargs, Lisp_Object* args) {
    general_insert_function(insert_before_markers,
                            insert_from_string_before_markers, 0, nargs, args);
    return Qnil;
}
...
(ptrdiff_t nargs, Lisp_Object* args) {
    general_insert_function(insert_before_markers_and_inherit,
                            insert_from_string_before_markers, 1, nargs, args);
    return Qnil;
}
...
Lisp_Object make_buffer_string(ptrdiff_t start, ptrdiff_t end, bool props) {
...
Lisp_Object make_buffer_string_both(ptrdiff_t start, ptrdiff_t start_byte,
    ...
    if (!NILP(BVAR(current_buffer, enable_multibyte_characters)))
        result =
            make_uninit_multibyte_string(end - start, end_byte - start_byte);
    else
...
static void update_buffer_properties(ptrdiff_t start, ptrdiff_t end) {
    ...
    if (!NILP(Vbuffer_access_fontify_functions)) {
       ...
       has already been done.  */
        if (!NILP(Vbuffer_access_fontified_property)) {
            Lisp_Object tem = Ftext_property_any(
                make_fixnum(start), make_fixnum(end),
                Vbuffer_access_fontified_property, Qnil, Qnil);
            if (NILP(tem))
                return;
        }
...
(Lisp_Object buffer1, Lisp_Object start1, Lisp_Object end1, Lisp_Object buffer2,
 Lisp_Object start2, Lisp_Object end2) {
    register EMACS_INT begp1, endp1, begp2, endp2, temp;
    register struct buffer *bp1, *bp2;
    register Lisp_Object trt =
        (!NILP(Vcase_fold_search) ? BVAR(current_buffer, case_canon_table)
                                  : Qnil);
    ptrdiff_t chars = 0;
    ...
    if (!(BUF_BEGV(bp1) <= begp1 && begp1 <= endp1 && endp1 <= BUF_ZV(bp1)))
    ...
    if (!(BUF_BEGV(bp2) <= begp2 && begp2 <= endp2 && endp2 <= BUF_ZV(bp2)))
...
(Lisp_Object beg, Lisp_Object end, Lisp_Object source, Lisp_Object max_secs,
 Lisp_Object max_costs, Lisp_Object inherit) {
    ...
    if (STRINGP(source)) {
       ...
       <https://gcc.gnu.org/bugzilla/show_bug.cgi?id=21161>.  */
        ...
        CHECK_TYPE(VECTORP(source), list(Qor, Qstring, Qbuffer, Qvector),
                   source);
    ...
    if (a_empty && b_empty)
        return Qt;
    else if (a_empty || b_empty || EQ(max_secs, make_fixnum(0)) ||
    ...
    if (!NILP(max_secs)) {
        struct timespec tlim = timespec_add(current_timespec(),
                                            lisp_time_argument(max_secs)),
                        tmax = make_timespec(TYPE_MAXIMUM(time_t),
                                             TIMESPEC_HZ - 1);
    ...
    if (early_abort) {
        Lisp_Object src =
            CALLN(Fvector, source_buffer, make_fixnum(BUF_BEGV(b)),
...
static void set_bit(unsigned char* a, ptrdiff_t i) {
    eassume(0 <= i);
    a[i / CHAR_BIT] |= (1 << (i % CHAR_BIT));
}

static bool bit_is_set(const unsigned char* a, ptrdiff_t i) {
    eassume(0 <= i);
...
static bool buffer_chars_equal(struct context* ctx, ptrdiff_t pos_a,
                               ptrdiff_t pos_b) {
    if (!++ctx->quitcounter) {
        maybe_quit();
...
static bool compareseq_early_abort(struct context* ctx) {
...
static void subst_char_in_region_unwind(Lisp_Object arg) {
    bset_undo_list(current_buffer, arg);
}

static void subst_char_in_region_unwind_1(Lisp_Object arg) {
...
(Lisp_Object start, Lisp_Object end, Lisp_Object fromchar, Lisp_Object tochar,
 Lisp_Object noundo) {
    ...
    bool multibyte_p = !NILP(BVAR(current_buffer, enable_multibyte_characters));
    int fromc, toc;
    ...
    struct ts_linecol start_linecol = treesit_linecol_maybe(
        start_char, start_byte, BUF_TS_LINECOL_POINT(current_buffer));
    struct ts_linecol old_end_linecol = treesit_linecol_maybe(
        old_end_char, old_end_byte, BUF_TS_LINECOL_POINT(current_buffer));
#endif
    ...
    while (1) {
        ...
        if (pos_byte_next - pos_byte == len && p[0] == fromstr[0] &&
            (len == 1 ||
             (p[1] == fromstr[1] &&
              (len == 2 ||
               (p[2] == fromstr[2] && (len == 3 || p[3] == fromstr[3])))))) {
            if (changed < 0)
...
static Lisp_Object check_translation(ptrdiff_t pos, ptrdiff_t pos_byte,
                                     ptrdiff_t end, Lisp_Object val) {
    int initial_buf[16];
    int* buf = initial_buf;
        ...
        if (oc < translatable_chars) {
            ...
            if (nc != oc && nc >= 0) {
                ...
                if (len != str_len) {
                    ...
                    replace_range(pos, pos + 1, string, true, false, false);
                    len = str_len;
                }
                else {
#ifdef HAVE_TREE_SITTER
                    ...
                    struct ts_linecol old_end_linecol = treesit_linecol_maybe(
                        pos + 1, pos_byte + len, start_linecol);
#endif
                    record_change(pos, 1);
                    while (str_len-- > 0)
                        *p++ = *str++;
...
static void labeled_restrictions_add(Lisp_Object buf,
                                     Lisp_Object restrictions) {
    labeled_restrictions =
        nconc2(list1(list2(buf, restrictions)), labeled_restrictions);
}

/* Remove BUF and its list of labeled restrictions from the
   labeled_restrictions alist.  Do nothing if BUF is not present in
   labeled_restrictions.  */
static void labeled_restrictions_remove(Lisp_Object buf) {
    labeled_restrictions =
        Fdelq(Fassoc(buf, labeled_restrictions, Qnil), labeled_restrictions);
}
...
static Lisp_Object labeled_restrictions_get_bound(Lisp_Object buf, bool begv,
                                                  bool outermost) {
...
/* Retrieve the label of the innermost labeled restriction in BUF.
   ...
   buffer.  */
static Lisp_Object labeled_restrictions_peek_label(Lisp_Object buf) {
    if (NILP(Fbuffer_live_p(buf)))
        return Qnil;
    Lisp_Object restrictions = assq_no_quit(buf, labeled_restrictions);
    if (NILP(restrictions))
...
/* Add a labeled RESTRICTION for BUF in the labeled_restrictions
   alist.  */
static void labeled_restrictions_push(Lisp_Object buf,
                                      Lisp_Object restriction) {
    Lisp_Object restrictions = assq_no_quit(buf, labeled_restrictions);
    if (NILP(restrictions))
        labeled_restrictions_add(buf, list1(restriction));
    else
        XSETCDR(restrictions,
                list1(nconc2(list1(restriction), XCAR(XCDR(restrictions)))));
}

/* Remove the innermost labeled restriction in BUF from the
   labeled_restrictions alist.  Do nothing if BUF is not present in
   labeled_restrictions.  */
static void labeled_restrictions_pop(Lisp_Object buf) {
    Lisp_Object restrictions = assq_no_quit(buf, labeled_restrictions);
    if (NILP(restrictions))
        return;
...
void labeled_restrictions_remove_in_current_buffer(void) {
    labeled_restrictions_remove(Fcurrent_buffer());
}

static void unwind_reset_outermost_restriction(Lisp_Object buf) {
    ...
    if (!NILP(begv) && !NILP(zv)) {
        SET_BUF_BEGV_BOTH(XBUFFER(buf), marker_position(begv),
                          marker_byte_position(begv));
        SET_BUF_ZV_BOTH(XBUFFER(buf), marker_position(zv),
                        marker_byte_position(zv));
    }
    else
        labeled_restrictions_remove(buf);
}
...
void reset_outermost_restrictions(void) {
    Lisp_Object val, buf;
...
static Lisp_Object labeled_restrictions_save(void) {
...
static void labeled_restrictions_restore(Lisp_Object buf_and_restrictions) {
...
static void unwind_labeled_narrow_to_region(Lisp_Object label) {
    Finternal__labeled_widen(label);
}

/* Narrow current_buffer to BEGV-ZV with a restriction labeled with
   LABEL.  */
void labeled_narrow_to_region(Lisp_Object begv, Lisp_Object zv,
                              Lisp_Object label) {
...
(void) {
    ...
    if (NILP(label)) {
        ...
        if (begv_charpos != BEGV || zv_charpos != ZV)
            current_buffer->clip_changed = 1;
        SET_BUF_BEGV_BOTH(current_buffer, begv_charpos,
                          marker_byte_position(begv));
        SET_BUF_ZV_BOTH(current_buffer, zv_charpos, marker_byte_position(zv));
...
(Lisp_Object start, Lisp_Object end, Lisp_Object label) {
    ...
    Lisp_Object outermost_restriction =
        list3(Qoutermost_restriction, Fpoint_min_marker(), Fpoint_max_marker());
    Fnarrow_to_region(start, end);
    if (NILP(labeled_restrictions_peek_label(buf)))
        labeled_restrictions_push(buf, outermost_restriction);
    labeled_restrictions_push(
        buf, list3(label, Fpoint_min_marker(), Fpoint_max_marker()));
    return Qnil;
}
...
static Lisp_Object save_restriction_save_1(void) {
    if (BEGV == BEG && ZV == Z)
...
static void save_restriction_restore_1(Lisp_Object data) {
    ...
    if (buf && buf != current_buffer &&
        !NILP(BVAR(buf,
                   pt_marker))) { /* If `buf' uses markers to keep track of PT,
                                 BEGV, and ZV (as is the case if it is or has an
                                 indirect buffer), then make sure it is current
                                 before we update BEGV, so set_buffer_internal
                                 takes care of managing those markers.  */
        cur = current_buffer;
...
void save_restriction_restore(Lisp_Object data) {
...
static ptrdiff_t str2num(char* str, char** str_end) {
    ptrdiff_t n = 0;
...
static Lisp_Object styled_format(ptrdiff_t nargs, Lisp_Object* args,
                                 bool message) {
    enum
    {
    ...
    if (ckd_mul(&info_size, nspec_bound, sizeof *info) ||
        ckd_add(&alloca_size, formatlen, info_size) || SIZE_MAX < alloca_size)
        memory_full(SIZE_MAX);
...
retry:
    ...
    while (format != end) {
     ...
     2 < __GLIBC__ + (35 <= __GLIBC_MINOR__))
                    enum
                    {
                        HAVE_PRINTF_B_CONVERSION_SPECIFIER = true
                    };
#else
                    enum
                    {
                        HAVE_PRINTF_B_CONVERSION_SPECIFIER = false
                    };
#endif
                ...
                /* Now the length of the formatted item is known, except it
               omits padding and excess precision.  Deal with excess precision
               ...
               with enough precision.  */
                ...
                if (excess_precision != 0 && float_conversion) {
                    if (!c_isdigit(p[sprintf_bytes - 1]) ||
                        (conversion == 'g' && !(sharp_flag && strchr(p, '.'))))
                        excess_precision = 0;
               ...
               excess precision and padding.  */
                ...
                if (convbytes <= buf + bufsize - p) {
                    ...
                    /* Insert padding and excess-precision zeros.  The output
                       contains the following components, in left-to-right
                       order:
               ...
               up to this position.  */
                for (; position < pos; bytepos++) {
                    if (!discarded[bytepos])
                        ...
                        position++;
                        if (fieldn < nspec && bytepos >= info[fieldn].fbeg &&
...
static void transpose_markers(ptrdiff_t start1, ptrdiff_t end1,
    ...
    if (PT < start1)
        ...
        TEMP_SET_PT_BOTH(PT + (end2 - end1), PT_BYTE + (end2_byte - end1_byte));
    else if (PT < start2)
        TEMP_SET_PT_BOTH(
            PT + (end2 - start2) - (end1 - start1),
            (PT_BYTE + (end2_byte - start2_byte) - (end1_byte - start1_byte)));
    else if (PT < end2)
...
ring.  */)
(Lisp_Object startr1, Lisp_Object endr1, Lisp_Object startr2, Lisp_Object endr2,
 Lisp_Object leave_markers) {
...
#ifdef HAVE_TREE_SITTER
    struct ts_linecol start_linecol = treesit_linecol_maybe(
        start1, start1_byte, BUF_TS_LINECOL_POINT(current_buffer));
    struct ts_linecol old_end_linecol = treesit_linecol_maybe(
        end2, end2_byte, BUF_TS_LINECOL_POINT(current_buffer));
#endif
    ...
    if (end1 == start2) {
        if (count_combining_before(BYTE_POS_ADDR(start2_byte), len2_byte,
                                   start1, start1_byte) ||
            count_combining_before(BYTE_POS_ADDR(start1_byte), len1_byte, end2,
                                   start2_byte + len2_byte) ||
            count_combining_after(BYTE_POS_ADDR(start1_byte), len1_byte, end2,
                                  start2_byte + len2_byte))
            emacs_abort();
    }
    else {
        if (count_combining_before(BYTE_POS_ADDR(start2_byte), len2_byte,
                                   start1, start1_byte) ||
            count_combining_before(BYTE_POS_ADDR(start1_byte), len1_byte,
                                   start2, start2_byte) ||
            count_combining_after(BYTE_POS_ADDR(start2_byte), len2_byte, end1,
    ...
    {
        graft_intervals_into_buffer(tmp_interval_mid, start1 + len2, len_mid,
                                    current_buffer, 0);
    }
    graft_intervals_into_buffer(tmp_interval1, end2 - len1, len1,
                                current_buffer, 0);
    graft_intervals_into_buffer(tmp_interval2, start1, len2, current_buffer, 0);
    ...
    if (NILP(leave_markers)) {
        /* FIXME: Since the undo info doesn't record the transposition as its
       own operation, we won't enjoy 'transpose_markers' during undo :-(  */
...
#ifdef HAVE_TREE_SITTER
    treesit_record_change(start1_byte, end2_byte, end2_byte, start_linecol,
                          old_end_linecol, end2);
#endif
