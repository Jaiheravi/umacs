...
static Lisp_Object store_in_keymap(Lisp_Object, Lisp_Object, Lisp_Object, bool);
...
static void CHECK_VECTOR_OR_CHAR_TABLE(Lisp_Object x) {
...
Lisp_Object get_keymap(Lisp_Object object, bool error_if_not_keymap,
                       bool autoload) {
autoload_retry:
    if (NILP(object))
        goto end;
...
static Lisp_Object keymap_parent(Lisp_Object keymap, bool autoload) {
...
static bool keymap_memberp(Lisp_Object map, Lisp_Object maps) {
    if (NILP(map))
        return 0;
...
static Lisp_Object access_keymap_1(Lisp_Object map, Lisp_Object idx, bool t_ok,
                                   bool noinherit, bool autoload) {
            ...
            if (EQ(binding, Qkeymap)) {
                if (noinherit || NILP(retval))
                    ...
                    parent_entry = get_keymap(
                        access_keymap_1(tail, idx, t_ok, 0, autoload), 0,
                        autoload);
...
Lisp_Object access_keymap(Lisp_Object map, Lisp_Object idx, bool t_ok,
                          bool noinherit, bool autoload) {
    Lisp_Object val = access_keymap_1(map, idx, t_ok, noinherit, autoload);
    return BASE_EQ(val, Qunbound) ? Qnil : val;
}

static void map_keymap_item(map_keymap_function_t fun, Lisp_Object args,
                            Lisp_Object key, Lisp_Object val, void* data) {
    if (EQ(val, Qt))
        val = Qnil;
...
static void map_keymap_char_table_item(Lisp_Object args, Lisp_Object key,
                                       Lisp_Object val) {
    if (!NILP(val)) {
...
static Lisp_Object map_keymap_internal(Lisp_Object map,
                                       map_keymap_function_t fun,
                                       Lisp_Object args, void* data) {
    Lisp_Object tail = (CONSP(map) && EQ(Qkeymap, XCAR(map))) ? XCDR(map) : map;
...
static void map_keymap_call(Lisp_Object key, Lisp_Object val, Lisp_Object fun,
                            void* dummy) {
    calln(fun, key, val);
}
...
void map_keymap(Lisp_Object map, map_keymap_function_t fun, Lisp_Object args,
...
void map_keymap_canonical(Lisp_Object map, map_keymap_function_t fun,
                          Lisp_Object args, void* data) {
    /* map_keymap_canonical may be used from redisplay (e.g. when building
       menus) so be careful to ignore errors and to inhibit redisplay.  */
...
static Lisp_Object get_keyelt(Lisp_Object object, bool autoload) {
    while (1) {
        if (!(CONSP(object)))
...
static Lisp_Object store_in_keymap(Lisp_Object keymap, register Lisp_Object idx,
    ...
    if (CONSP(def) && (EQ(XCAR(def), Qmenu_item) || STRINGP(XCAR(def))))
    ...
    /* Scan the keymap for a binding of idx.  */
    {
        ...
        for (tail = XCDR(keymap); CONSP(tail); tail = XCDR(tail)) {
            ...
            if (VECTORP(elt)) {
                ...
                if (EQ(Qkeymap,
                       XCAR(elt))) { /* A sub keymap.  This might be due to a
                                    lookup that found two matching bindings
                                    (maybe because of a sub keymap). It almost
                                    never happens (since the second binding
                                    normally only happens in the inherited part
                                    of the keymap), but if it does, we want to
                                    update the sub-keymap since the main one
                                    might be temporary (built by access_keymap).
                                  */
...
static Lisp_Object copy_keymap_item(Lisp_Object elt, int depth) {
    Lisp_Object res, tem;
...
static void copy_keymap_set_char_table(Lisp_Object chartable_and_depth,
                                       Lisp_Object idx, Lisp_Object elt) {
...
is not copied.  */)
(Lisp_Object keymap) { return copy_keymap_1(keymap, 0); }


/* Simple Keymap mutators and accessors.				*/

static Lisp_Object possibly_translate_key_sequence(Lisp_Object key,
                                                   ptrdiff_t* length) {
    if (VECTORP(key) && ASIZE(key) == 1 && STRINGP(AREF(key, 0))) {
       ...
       format.  */
        if (NILP(Ffboundp(Qkey_valid_p)))
            xsignal2(Qerror,
                     build_string("`key-valid-p' is not defined, so this "
                                  "syntax can't be used: %s"),
                     key);
...
(Lisp_Object keymap, Lisp_Object key, Lisp_Object def, Lisp_Object remove) {
    ...
    int meta_bit =
        (VECTORP(key) || (STRINGP(key) && STRING_MULTIBYTE(key)) ? meta_modifier
                                                                 : 0x80);

    if (VECTORP(def) && ASIZE(def) > 0 &&
        CONSP(AREF(
            def, 0))) { /* DEF is apparently an XEmacs-style keyboard macro.  */
    ...
    while (1) {
        ...
        if (!CONSP(keymap)) {
            ...
            error("Key sequence %s starts with non-prefix key %s%s",
                  ...
                  SDATA(Fkey_description(
                      Fsubstring(key, make_fixnum(0), make_fixnum(idx)), Qnil)),
                  trailing_esc);
        }
    }
}
...
static Lisp_Object lookup_key_1(Lisp_Object keymap, Lisp_Object key,
    ...
    for (int tbl_num = 0; tbl_num < 2; tbl_num++) {
        ...
        for (int i = 0; i < key_len; i++) {
            ...
            if (!SYMBOLP(item))
                ...
                if (!STRING_MULTIBYTE(key_item))
                    ...
                    new_item = make_multibyte_string(
                        (char*)dst, SCHARS(key_item), SBYTES(key_item));
                    SAFE_FREE();
                }
       ...
       "foo-bar-baz".  */
        for (int i = 0; i < key_len; i++) {
            ...
            Lisp_Object new_it = make_multibyte_string(
                (char*)dst, SCHARS(lc_key), SBYTES(lc_key));
...
static Lisp_Object define_as_prefix(Lisp_Object keymap, Lisp_Object c) {
...
static Lisp_Object append_key(Lisp_Object key_sequence, Lisp_Object key) {
...
static void silly_event_symbol_error(Lisp_Object c) {
    ...
    if (!NILP(assoc)) {
        ...
        *p = 0;
        ...
        error("To bind the key %s, use [?%s], not [%s]", SDATA(SYMBOL_NAME(c)),
              SDATA(keystring), SDATA(SYMBOL_NAME(c)));
    }
}
...
ptrdiff_t current_minor_maps(Lisp_Object** modeptr, Lisp_Object** mapptr) {
    ptrdiff_t i = 0;
    ...
    for (int list_number = 0; list_number < 2; list_number++) {
        ...
        for (; CONSP(alist); alist = XCDR(alist))
            if ((assoc = XCAR(alist), CONSP(assoc)) &&
                ...
                /* If a variable has an entry in
                   Vminor_mode_overriding_map_alist, and also an entry in
                   Vminor_mode_map_alist, ignore the latter.  */
                if (list_number == 1) {
                ...
                if (i >= cmm_size) {
                    ...
                    if (min(PTRDIFF_MAX, SIZE_MAX) / (2 * sizeof *newmodes) -
                            3 <
                        cmm_size)
                        break;
...
static ptrdiff_t click_position(Lisp_Object position) {
    ...
    if (NILP(XCDR(keymaps))) {
       ...
       but that can be overridden by a `local-map' property.  */
        ...
        if (CONSP(position)) {
            ...
            if (FIXNUMP(buffer_posn) && XFIXNUM(buffer_posn) >= BEG &&
                XFIXNUM(buffer_posn) <= Z) {
                ...
                if (NILP(keymap))
                    keymap = get_local_map(XFIXNUM(buffer_posn), current_buffer,
                                           Qkeymap);
            }
...
DEFUN("key-binding", Fkey_binding, Skey_binding, 1, 4, 0, doc:
...
(Lisp_Object key, Lisp_Object accept_default, Lisp_Object no_remap,
 Lisp_Object position) {
    ...
    Lisp_Object value =
        Flookup_key(Fcurrent_active_maps(Qt, position), key, accept_default);
...
(void) { return BVAR(current_buffer, keymap); }
...
static void accessible_keymaps_1(Lisp_Object key, Lisp_Object cmd,
                                 Lisp_Object args, void* data)
/* Use void * data to be compatible with map_keymap_function_t.  */
{
    ...
    while (!NILP(tem = Frassq(cmd, maps))) {
        ...
        if (lim <= XFIXNUM(Flength(thisseq))) { /* This keymap was already seen
                                                   with a smaller prefix.  */
            ptrdiff_t i = 0;
            while (i < lim &&
                   EQ(Faref(prefix, make_fixnum(i)),
    ...
    if (is_metized) {
       ...
       after this one.  */
        XSETCDR(tail, Fcons(Fcons(tem, cmd), XCDR(tail)));
    }
    else {
...
char* push_key_description(EMACS_INT ch, char* p) {
    int c, c2;
    bool tab_as_ci;
...
static char* push_text_char_description(register unsigned int c,
                                        register char* p) {
    if (c < 040) {
        *p++ = '^';
        *p++ = c + 64; /* 'A' - 1 */
...
/* Return 0 if SEQ uses non-preferred modifiers or non-char events.
   ...
   and 1 otherwise.  */
static int preferred_sequence_p(Lisp_Object seq) {
    EMACS_INT i;
...
static Lisp_Object shadow_lookup(Lisp_Object keymap, Lisp_Object key,
                                 Lisp_Object accept_default, bool remap) {
...
static Lisp_Object where_is_internal(Lisp_Object definition,
                                     Lisp_Object keymaps, bool noindirect,
                                     bool nomenus) {
    ...
    for (; CONSP(maps); maps = XCDR(maps)) {
       ...
       is the meta-prefix char.  */
        ...
        last_is_meta =
            (XFIXNUM(last) >= 0 && EQ(Faref(this, last), meta_prefix_char));
    ...
    if (nomenus && !noindirect) { /* Remember for which keymaps this cache was
                                 built. We do it here (late) because we want to
                                 keep where_is_cache_keymaps set to t while the
                                 cache isn't fully filled.  */
...
(Lisp_Object definition, Lisp_Object keymap, Lisp_Object firstonly,
 Lisp_Object noindirect, Lisp_Object no_remap) {
    ...
    if (SYMBOLP(definition) && !NILP(firstonly) &&
    ...
    sequences = Freverse(
        where_is_internal(definition, keymaps, !NILP(noindirect), nomenus));
    ...
    while (CONSP(sequences)
           /* If we're at the end of the `sequences' list and we haven't
              ...
              process them.  */
           || (!remapped &&
               (sequences = remapped_sequences, remapped = true,
                CONSP(sequences)))) {
       ...
       which run COMMAND, and use those sequences instead.  */
        if (NILP(no_remap) && !remapped && VECTORP(sequence) &&
            ASIZE(sequence) == 2 && EQ(AREF(sequence, 0), Qremap) &&
       ...
       been seen (as could happen when inheriting keymaps).  */
        if (NILP(Fmember(sequence, found))
            ...
            && !(VECTORP(sequence) && ASIZE(sequence) == 1 &&
    ...
    if (NILP(firstonly))
        ...
        return Fcar(found);
    else { /* Maybe we did not find a preferred_modifier binding, but we did
          find some ASCII binding.  */
...
--------        -----------\n";
    ...
    if (STRINGP(KVAR(current_kboard, Vkeyboard_translate_table)) &&
        ...
        const unsigned char* translate =
            SDATA(KVAR(current_kboard, Vkeyboard_translate_table));
        int translate_len =
    ...
    if (!NILP(Vkey_translation_map)) {
        ...
        calln(Qhelp__describe_map_tree, Vkey_translation_map, Qnil, Qnil,
              prefix, msg, nomenu, Qt, Qnil, Qnil, buffer);
    }
    ...
    if (!NILP(start1)) {
        ...
        calln(Qhelp__describe_map_tree, start1, Qt, shadow, prefix, msg, nomenu,
              Qnil, Qnil, Qnil, buffer);
    ...
    if (!NILP(start1)) {
       ...
       minor modes correctly.  */
        ...
        start1 =
            get_local_map(BUF_PT(XBUFFER(buffer)), XBUFFER(buffer), Qkeymap);
        if (!NILP(start1)) {
            Lisp_Object msg =
                build_unibyte_string("\f\n`keymap' Property Bindings");
            calln(Qhelp__describe_map_tree, start1, Qt, shadow, prefix, msg,
                  nomenu, Qnil, Qnil, Qnil, buffer);
            shadow = Fcons(start1, shadow);
        }
        ...
        for (int i = 0; i < nmaps; i++) {
            ...
            *p = 0;
            ...
            calln(Qhelp__describe_map_tree, maps[i], Qt, shadow, prefix, msg,
                  nomenu, Qnil, Qnil, Qnil, buffer);
        ...
        start1 =
            get_local_map(BUF_PT(XBUFFER(buffer)), XBUFFER(buffer), Qlocal_map);
        if (!NILP(start1)) {
    ...
    Lisp_Object msg = build_unibyte_string("\f\nGlobal Bindings");
    calln(Qhelp__describe_map_tree, current_global_map, Qt, shadow, prefix, msg,
          nomenu, Qnil, Qt, Qnil, buffer);
    ...
    if (!NILP(KVAR(current_kboard, Vinput_decode_map))) {
        ...
        calln(Qhelp__describe_map_tree, KVAR(current_kboard, Vinput_decode_map),
              Qnil, Qnil, prefix, msg, nomenu, Qt, Qnil, Qnil, buffer);
    }
    return Qnil;
}

static void describe_vector_princ(Lisp_Object elt, Lisp_Object fun) {
    Findent_to(make_fixnum(16), make_fixnum(1));
    calln(fun, elt);
    Fterpri(Qnil, Qnil);
}

static void describe_vector_basic(Lisp_Object elt, Lisp_Object fun) {
    calln(fun, elt);
}
...
(Lisp_Object vector, Lisp_Object describer) {
    ...
    describe_vector(vector, Qnil, describer, describe_vector_princ, 0, Qnil,
                    Qnil, 0, 0);
...
static Lisp_Object
describe_key_maybe_fontify(Lisp_Object str, Lisp_Object prefix, bool keymap_p) {
    Lisp_Object key_desc = Fkey_description(str, prefix);
    if (keymap_p)
        Fadd_text_properties(make_fixnum(0), make_fixnum(SCHARS(key_desc)),
                             fontify_key_properties, key_desc);
    return key_desc;
}
...
static void describe_vector(Lisp_Object vector, Lisp_Object prefix,
                            Lisp_Object args,
                            void (*elt_describer)(Lisp_Object, Lisp_Object),
                            bool partial, Lisp_Object shadow,
                            Lisp_Object entire_map, bool keymap_p,
                            bool mention_shadow) {
...
void syms_of_keymap(void) {
    ...
    exclude_keys = list(Fcons(build_string("DEL"), build_string("\\d")),
...
that describe key bindings.  That is why the default is nil.  */);
    ...
    Vmouse_events =
        list(Qmenu_bar, Qtab_bar, Qtool_bar, Qtab_line, Qheader_line,
             Qmode_line, intern_c_string("mouse-1"), intern_c_string("mouse-2"),
             intern_c_string("mouse-3"), intern_c_string("mouse-4"),
    ...
    fontify_key_properties =
        Fcons(Qfont_lock_face, Fcons(Qhelp_key_binding, Qnil));
