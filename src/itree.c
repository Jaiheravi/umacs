...
static struct itree_stack* itree_stack_create(intmax_t initial_size) {
...
static void itree_stack_destroy(struct itree_stack* stack) {
    if (!stack)
        return;
    if (stack->nodes)
        xfree(stack->nodes);
...
static inline void itree_stack_ensure_space(struct itree_stack* stack,
                                            uintmax_t nelements) {
    if (nelements > stack->size) {
        stack->size = (nelements + 1) * 2;
        stack->nodes =
            xrealloc(stack->nodes, stack->size * sizeof(*stack->nodes));
    }
}

/* Push NODE on the STACK, while settings its visited flag to FLAG. */

static inline void itree_stack_push(struct itree_stack* stack,
...
static inline struct itree_node* itree_stack_pop(struct itree_stack* stack) {
    if (stack->length == 0)
        return NULL;
...
static int itree_max_height(const struct itree_tree* tree) {
...
check_subtree(struct itree_node* node, bool check_red_black_invariants,
              uintmax_t tree_otick, ptrdiff_t offset, ptrdiff_t min_begin,
              ptrdiff_t max_begin) {
    struct check_subtree_result result = {
        .size = 0, .limit = PTRDIFF_MIN, .black_height = 0};
    if (node == NULL)
        return result;
    ...
    struct check_subtree_result left_result =
        check_subtree(node->left, check_red_black_invariants, tree_otick,
                      offset, min_begin, begin);
    struct check_subtree_result right_result =
        check_subtree(node->right, check_red_black_invariants, tree_otick,
                      offset, begin, max_begin);
...
*/
static bool check_tree(struct itree_tree* tree,
...
static bool null_safe_is_red(struct itree_node* node) {
    return node != NULL && node->red;
}

static bool null_safe_is_black(struct itree_node* node) {
    return node == NULL || !node->red; /* NULL nodes are black */
}

static inline ptrdiff_t itree_newlimit(struct itree_node* node) {
    eassert(node != NULL);
    return max(node->end,
               max(node->left == NULL ? PTRDIFF_MIN
...
static void itree_update_limit(struct itree_node* node) {
    if (node == NULL)
        return;
...
static void itree_inherit_offset(uintmax_t otick, struct itree_node* node) {
...
static void itree_propagate_limit(struct itree_node* node) {
    ptrdiff_t newlimit;
...
static struct itree_node* itree_validate(struct itree_tree* tree,
                                         struct itree_node* node) {
...
void itree_node_init(struct itree_node* node, bool front_advance,
                     bool rear_advance, Lisp_Object data) {
    node->parent = NULL;
    node->left = NULL;
    node->right = NULL;
    node->begin = -1;
...
ptrdiff_t itree_node_begin(struct itree_tree* tree, struct itree_node* node) {
...
ptrdiff_t itree_node_end(struct itree_tree* tree, struct itree_node* node) {
    itree_validate(tree, node);
    return node->end;
}
...
void itree_clear(struct itree_tree* tree) {
    tree->root = NULL;
    tree->otick = 1;
    tree->size = 0;
}
...
static void itree_init(struct itree_tree* tree) { itree_clear(tree); }
#endif

/* Release a tree, freeing its allocated memory.  */
void itree_destroy(struct itree_tree* tree) {
...
intmax_t itree_size(struct itree_tree* tree) { return tree->size; }

/* Perform the familiar left-rotation on node NODE.  */

static void itree_rotate_left(struct itree_tree* tree,
                              struct itree_node* node) {
...
static void itree_rotate_right(struct itree_tree* tree,
                               struct itree_node* node) {
...
static void itree_insert_fix(struct itree_tree* tree, struct itree_node* node) {
...
static void itree_insert_node(struct itree_tree* tree,
                              struct itree_node* node) {
    eassert(node && node->begin <= node->end);
    eassert(node->left == NULL && node->right == NULL && node->parent == NULL);
    eassert(check_tree(tree, true)); /* FIXME: Too expensive.  */
...
void itree_node_set_region(struct itree_tree* tree, struct itree_node* node,
                           ptrdiff_t begin, ptrdiff_t end) {
    itree_validate(tree, node);
    if (begin != node->begin) {
...
static bool itree_contains(struct itree_tree* tree, struct itree_node* node) {
    eassert(node);
...
static bool itree_limit_is_stable(struct itree_node* node) {
    if (node == NULL)
        return true;
...
static struct itree_node* itree_subtree_min(uintmax_t otick,
                                            struct itree_node* node) {
    if (node == NULL)
        return node;
    while ((itree_inherit_offset(otick, node), node->left != NULL))
        node = node->left;
    return node;
}
...
static void itree_remove_fix(struct itree_tree* tree, struct itree_node* node,
...
static ptrdiff_t itree_total_offset(struct itree_node* node) {
    eassert(node != NULL);
    ptrdiff_t offset = 0;
...
static void itree_replace_child(struct itree_tree* tree,
...
/* Replace DEST with SOURCE in the tree.  Copies the following fields
   ...
   effective 'offset'. */
static void itree_transplant(struct itree_tree* tree, struct itree_node* source,
                             struct itree_node* dest) {
...
struct itree_node* itree_remove(struct itree_tree* tree,
                                struct itree_node* node) {
    ...
    itree_inherit_offset(tree->otick, node);
    struct itree_node* splice = (node->left == NULL || node->right == NULL)
        ? node
...
void itree_insert_gap(struct itree_tree* tree, ptrdiff_t pos, ptrdiff_t length,
                      bool before_markers) {
    ...
    if (tree->root != NULL) {
        ...
        while ((node = itree_stack_pop(stack))) {
            ...
            if (before_markers ? node->begin >= pos
                               : node->begin >
                        pos) /* node->begin == pos => !front-advance  */
                node->begin += length;
            if (node->end > pos ||
...
void itree_delete_gap(struct itree_tree* tree, ptrdiff_t pos,
                      ptrdiff_t length) {
    if (!tree || length <= 0 || tree->root == NULL)
        return;
...
static inline bool itree_node_intersects(const struct itree_node* node,
                                         ptrdiff_t begin, ptrdiff_t end) {
...
itree_iter_next_in_subtree(struct itree_node* node,
    ...
    case ITREE_DESCENDING:
        ...
        if (!next ||
            ...
            while ((next = node->parent) && next->left == node)
                node = next;
            if (!next)
                return NULL; /* No more nodes to visit. */
            node = next;
            ...
            node = next;
            while (node->begin <= iter->end && (next = node->right)) {
                itree_inherit_offset(iter->otick, next), node = next;
            }
        }
        return node;
    ...
    case ITREE_POST_ORDER:
        ...
        while (((next = node->left) &&
                (itree_inherit_offset(iter->otick, next),
                 iter->begin <= next->limit)) ||
               (node->begin <= iter->end && (next = node->right) &&
...
struct itree_iterator* itree_iterator_start(struct itree_iterator* iter,
...
struct itree_node* itree_iterator_next(struct itree_iterator* iter) {
    struct itree_node* node = iter->node;
    while (node && !itree_node_intersects(node, iter->begin, iter->end)) {
...
void itree_iterator_narrow(struct itree_iterator* g, ptrdiff_t begin,
                           ptrdiff_t end) {
    eassert(g);
