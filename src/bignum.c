...
static void* xrealloc_for_gmp(void* ptr, size_t ignore, size_t size) {
    return xrealloc(ptr, size);
}

static void xfree_for_gmp(void* ptr, size_t ignore) { xfree(ptr); }
...
double bignum_to_double(Lisp_Object n) {
...
Lisp_Object double_to_integer(double d) {
    if (!isfinite(d))
        overflow_error();
    mpz_set_d(mpz[0], d);
...
static Lisp_Object make_bignum_bits(size_t bits) {
...
/* Return a Lisp integer equal to -N, which must not be in fixnum range.  */
Lisp_Object make_neg_biguint(uintmax_t n) {
...
/* Return a Lisp integer with value taken from mpz[0].
   Set mpz[0] to a junk value.  */
Lisp_Object make_integer_mpz(void) {
    ...
    if (!(FASTER_BIGNUM && FIXNUM_OVERFLOW_P(LONG_MIN) &&
...
void mpz_set_intmax_slow(mpz_t result, intmax_t v) {
...
void mpz_set_uintmax_slow(mpz_t result, uintmax_t v) {
...
bool mpz_to_intmax(mpz_t const z, intmax_t* pi) {
    if (FASTER_BIGNUM) {
...
bool mpz_to_uintmax(mpz_t const z, uintmax_t* pi) {
    if (FASTER_BIGNUM) {
...
intmax_t bignum_to_intmax(Lisp_Object x) {
    intmax_t i;
    return mpz_to_intmax(*xbignum_val(x), &i) ? i : 0;
}
uintmax_t bignum_to_uintmax(Lisp_Object x) {
    uintmax_t i;
...
static int emacs_mpz_size(mpz_t const op) {
...
void emacs_mpz_mul(mpz_t rop, mpz_t const op1, mpz_t const op2) {
    if (NLIMBS_LIMIT - emacs_mpz_size(op1) < emacs_mpz_size(op2))
        overflow_error();
    mpz_mul(rop, op1, op2);
}

void emacs_mpz_mul_2exp(mpz_t rop, mpz_t const op1, EMACS_INT op2) {
...
void emacs_mpz_pow_ui(mpz_t rop, mpz_t const base, unsigned long exp) {
    /* This fudge factor is derived from GMP 6.1.2, to avoid an abort in
       mpz_n_pow_ui (look for the '5' in its source code).  */
    enum
    {
        pow_ui_extra_limbs = 5
    };
    enum
    {
        lim = min(NLIMBS_LIMIT, GMP_NLIMBS_MAX - pow_ui_extra_limbs)
    };
...
static ptrdiff_t mpz_bufsize(mpz_t const num, int base) {
    return mpz_sizeinbase(num, base) + 2;
}
ptrdiff_t bignum_bufsize(Lisp_Object num, int base) {
    return mpz_bufsize(*xbignum_val(num), base);
}

/* Convert NUM to a nearest double, as opposed to mpz_get_d which
   truncates toward zero.  */
double mpz_get_d_rounded(mpz_t const num) {
...
ptrdiff_t bignum_to_c_string(char* buf, ptrdiff_t size, Lisp_Object num,
                             int base) {
...
Lisp_Object bignum_to_string(Lisp_Object num, int base) {
...
Lisp_Object make_bignum_str(char const* num, int base) {
    struct Lisp_Bignum* b =
        ALLOCATE_PLAIN_PSEUDOVECTOR(struct Lisp_Bignum, PVEC_BIGNUM);
    mpz_init(b->value);
...
intmax_t check_integer_range(Lisp_Object x, intmax_t lo, intmax_t hi) {
    CHECK_INTEGER(x);
    intmax_t i;
...
uintmax_t check_uinteger_max(Lisp_Object x, uintmax_t hi) {
    CHECK_INTEGER(x);
    uintmax_t i;
...
int check_int_nonnegative(Lisp_Object x) {
    CHECK_INTEGER(x);
...
static mp_limb_t get_random_limb(void) {
...
static mp_limb_t get_random_limb_lim(mp_limb_t lim) {
...
Lisp_Object get_random_bignum(struct Lisp_Bignum const* limit) {
