...
#define ISBLANK(c) (IS_REAL_ASCII(c) ? ((c) == ' ' || (c) == '\t') : blankp(c))
...
static void STORE_NUMBER(unsigned char* destination, int number) {
...
static int extract_number(re_char* source) {
    signed char leading_byte = source[1];
    return leading_byte * 256 + source[0];
}

static re_char* extract_address(re_char* source) {
...
static int extract_number_and_incr(re_char** source) {
...
static void debug_putchar(FILE* dest, int c) {
    if (c >= 32 && c <= 126)
        putc(c, dest);
    else {
        unsigned int uc = c;
...
static void print_fastmap(FILE* dest, char* fastmap) {
...
static void print_partial_compiled_pattern(FILE* dest, re_char* start,
                                           re_char* end) {
    int mcnt, mcnt2;
    re_char* p = start;
    re_char* pend = end;
...
void print_compiled_pattern(FILE* dest, struct re_pattern_buffer* bufp) {
    if (!dest)
        dest = stderr;
...
static void print_double_string(re_char* where, re_char* string1,
                                ptrdiff_t size1, re_char* string2,
                                ptrdiff_t size2) {
    if (where == NULL)
        fputs("(null)", stderr);
    else {
...
#define GROW_FAIL_STACK(fail_stack)                                            \
    (((fail_stack).size >= emacs_re_max_failures * TYPICAL_FAILURE_SIZE)       \
         ? 0                                                                   \
         : ((fail_stack).stack = REGEX_REALLOCATE(                             \
                (fail_stack).stack,                                            \
 ...
 */
re_wctype_t re_wctype_parse(const unsigned char** strp, ptrdiff_t limit) {
    ...
    *strp = (const unsigned char*)(it + 2);
    ...
    /* Sort tests in the length=five case by frequency the classes to minimize
       ...
       class names used in Emacs sources as of 2016-07-27:
...
bool re_iswctype(int ch, re_wctype_t cc) {
    switch (cc) {
    case RECC_ALNUM:
        return ISALNUM(ch) != 0;
    case RECC_ALPHA:
    ...
    case RECC_ASCII:
        return IS_REAL_ASCII(ch) != 0;
    case RECC_NONASCII:
        return !IS_REAL_ASCII(ch);
    case RECC_UNIBYTE:
        return ISUNIBYTE(ch) != 0;
    case RECC_MULTIBYTE:
        return !ISUNIBYTE(ch);
    case RECC_WORD:
        return ISWORD(ch) != 0;
...

            {
                ...
                if (greedy) {
                    if (many_times_ok) {
                        ...
                        if (!zero_times_ok &&
                            simple) { /* Since simple * loops can be made faster
                                     by using on_failure_keep_string_jump, we
                                     turn P+ into PP* if P is simple. We can't
                                     use `top: <BODY>; OFJS exit; J top; exit:`
                                     because the OFJS needs to be at the
                                     beginning so we can replace top: OFJS exit;
                                     <BODY>; J top; exit with OFKSJ exit; loop:
                                     <BODY>; J loop; exit i.e. a single OFJ at
                                     the beginning of the loop rather than once
                                     per iteration.  */
                            unsigned char *p1, *p2;
                        ...
                        if (!zero_times_ok)
                            ...
                            /* Simple * loops can use
                           on_failure_keep_string_jump depending on what
                           follows.  But since we don't know that yet, we leave
                           the decision up to on_failure_jump_smart.  */
                    ...
                    if (many_times_ok) {
                        ...
                        if (zero_times_ok) {
                            /* The repeat...until naturally matches one or more.
                               To also match zero times, we need to first jump
                               to the end of the loop (its conditional jump). */
                ...
                for (;;) {
                    ...
                    if (cc != -1) {
                        ...
                        /* Most character classes in a multibyte match just set
                           a flag.  Exceptions are is_blank, is_digit, is_cntrl,
                           and is_xdigit, since they can only match ASCII
                           characters. We don't need to handle them for
                           multibyte.  */
                        ...
                        /* In most cases the matching rule for char classes only
                           uses the syntax table for multibyte chars, so that
                           the content of the syntax-table is not hardcoded in
                           the range_table.  SPACE and WORD are the two
                           exceptions.  */
            ...
            case '(':
                {
                    ...
                    if (!shy)
                        regnum = ++bufp->re_nsub;
                    else if (regnum) { /* It's actually not shy, but explicitly
                                          numbered.  */
                        shy = false;
                        if (regnum > bufp->re_nsub)
                            bufp->re_nsub = regnum;
                        else if (regnum > bufp->re_nsub
                                 /* Ideally, we'd want to check that the
                                specified group can't have matched (i.e. all
                                subgroups using the same regnum are in other
                                branches of OR patterns), but we don't currently
                                keep track of enough info to do that easily.  */
                                 ||
                    ...
                    if (COMPILE_STACK_FULL)
                        compile_stack.stack =
                            xpalloc(compile_stack.stack, &compile_stack.size, 1,
                                    -1, sizeof *compile_stack.stack);
    ...
    while (true) {
        ...
        if (p == pend)
            ...
            case jump_n:
                ...
                if (newp1 > p) { /* Forward jump, boring.  */
                    p = newp1;
                    continue;
                }
                switch (*newp1) {
            ...
            case on_failure_jump_smart:
                ...
                if ((re_opcode_t)*newp2 == jump) {
                    ...
                    /* Only recognize this pattern if one of the two
                       destinations is going forward, otherwise we'll fall into
                       the pessimistic "Both destinations go backward" below.
            ...
            do_twoway_jump:
                ...
                if (!forall_firstchar_1(newp2, pend, loop_beg, loop_end, f,
                                        arg))
                    return false;
            ...
            do_jump:
                ...
                if (newp1 <= p_orig) {
                    if (newp1 < loop_beg) {
                        ...
                        /* If we jump backward to the entry point of the current
                           loop it means it's a zero-length cycle through that
                           loop; this cycle itself does not break safety.  */
                        return true;
                    else
                        /* We jump backward to a new loop, nested within the
                           current one. `newp1` is the entry point and `newp2`
                           the exit of that inner loop.  */
                        /* `p` gets smaller, but termination is still ensured
                           because `loop_beg` gets bigger. */
...
/* Iterate through all the char-matching operations directly reachable from P.
   ...
   of [P...PEND].  */
static bool forall_firstchar(struct re_pattern_buffer* bufp, re_char* p,
                             re_char* pend, bool f(re_char* p, void* arg),
                             void* arg) {
...
static bool analyze_first_fastmap(const re_char* p, void* arg) {
    ...
    case charset_not:
        {
            ...
            for (j = CHARSET_BITMAP_SIZE(p) * BYTEWIDTH; j < (1 << BYTEWIDTH);
                 j++)
                data->fastmap[j] = 1;
        }
        FALLTHROUGH;
...
static bool analyze_first_null(const re_char* p, void* arg) {
    switch (*p) {
    case succeed:
...
static bool analyze_first(struct re_pattern_buffer* bufp, re_char* p,
                          re_char* pend, char* fastmap) {
    eassert(pend);
    struct anafirst_data data = {bufp ? bufp->multibyte : false, fastmap,
                                 false};
    bool safe = forall_firstchar(
        bufp->used ? bufp : NULL, p, pend,
        fastmap ? analyze_first_fastmap : analyze_first_null, &data);
    return !safe;
}
...
ptrdiff_t re_search(struct re_pattern_buffer* bufp, const char* string,
                    ptrdiff_t size, ptrdiff_t startpos, ptrdiff_t range,
...
ptrdiff_t re_search_2(struct re_pattern_buffer* bufp, const char* str1,
                      ptrdiff_t size1, const char* str2, ptrdiff_t size2,
...
static bool execute_charset(re_char** pp, int c, int corig, bool unibyte,
    ...
    if (unibyte && c < (1 << BYTEWIDTH)) { /* Lookup bitmap.  */
...
static bool mutually_exclusive_exactn(struct re_pattern_buffer* bufp,
...
static bool mutually_exclusive_charset(struct re_pattern_buffer* bufp,
                                       re_char* p1, re_char* p2) {
...
static bool mutually_exclusive_one(re_char* p2, void* arg) {
...
static bool mutually_exclusive_p(struct re_pattern_buffer* bufp, re_char* p1,
                                 re_char* p2) {
...
ptrdiff_t re_match_2(struct re_pattern_buffer* bufp, char const* string1,
...
static void unwind_re_match(void* ptr) {
...
static ptrdiff_t re_match_2_internal(struct re_pattern_buffer* bufp,
    ...
    if (pos >= size1) {
        ...
        else { /* It's important to use this code when STOP == SIZE so that
    ...
    for (;;) {
        ...
        succeed_label:
            ...
            if (regs) {
               ...
               since we haven't kept track of any info for it.  */
                ...
                for (ptrdiff_t reg = 1; reg < num_regs; reg++) {
                    eassert(REG_UNSET(regstart[reg]) <= REG_UNSET(regend[reg]));
        ...
        case duplicate:
            {
                ...
                for (;;) {
                    ...
                    /* Compare that many; failure if mismatch, else move
                       past them.  */
                    if (!NILP(translate)
                            ? bcmp_translate(d, d2, dcnt, translate,
                                             target_multibyte)
                            : memcmp(d, d2, dcnt)) {
                        d = dfail;
                        goto fail;
        ...
        on_failure:
                        ...
                        p + mcnt);
            {
                ...
                if (cycle)
                    /* If there's a cycle, get out of the loop, as if the
                   matching had failed.  We used to just 'goto fail' here, but
                   that was aborting the search a bit too early: we want to keep
                   the empty-loop-match and keep matching after the loop. We
                   want (x?)*y\1z to match both xxyz and xxyxz.  */
                    p += mcnt;
                else
                    PUSH_FAILURE_POINT(p - 3, d);
            }
        ...
        case on_failure_jump_smart:
                        ...
                        p + mcnt);
            {
                ...
                unsigned char* p3 =
                    (unsigned char*)p - 3; /* opcode location.  */
        ...
        case notwordbound:
            {
                ...
                if (AT_STRINGS_BEG(d) || AT_STRINGS_END(d))
                    ...
                    ptrdiff_t charpos =
                        RE_SYNTAX_TABLE_BYTE_TO_CHAR(offset) - 1;
...
static bool bcmp_translate(re_char* s1, re_char* s2, ptrdiff_t len,
...
const char* re_compile_pattern(const char* pattern, ptrdiff_t length,
    ...
    reg_errcode_t ret = regex_compile(
        (re_char*)pattern, length, posix_backtracking, whitespace_regexp, bufp);
