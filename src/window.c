...
static ptrdiff_t get_leaf_windows(struct window*, struct window**, ptrdiff_t);
...
static void foreach_window(struct frame*, bool (*fn)(struct window*, void*),
                           void*);
static bool foreach_window_1(struct window*, bool (*fn)(struct window*, void*),
                             void*);
...
static void CHECK_WINDOW_CONFIGURATION(Lisp_Object x) {
    CHECK_TYPE(WINDOW_CONFIGURATIONP(x), Qwindow_configuration_p, x);
}

/* These setters are used only in this file, so they can be private.  */
static void wset_combination_limit(struct window* w, Lisp_Object val) {
    w->combination_limit = val;
}

static void wset_dedicated(struct window* w, Lisp_Object val) {
    w->dedicated = val;
}

static void wset_display_table(struct window* w, Lisp_Object val) {
    w->display_table = val;
}

static void wset_new_normal(struct window* w, Lisp_Object val) {
    w->new_normal = val;
}

static void wset_new_total(struct window* w, Lisp_Object val) {
    w->new_total = val;
}

static void wset_normal_cols(struct window* w, Lisp_Object val) {
    w->normal_cols = val;
}

static void wset_normal_lines(struct window* w, Lisp_Object val) {
    w->normal_lines = val;
}

static void wset_parent(struct window* w, Lisp_Object val) { w->parent = val; }

static void wset_pointm(struct window* w, Lisp_Object val) { w->pointm = val; }

static void wset_old_pointm(struct window* w, Lisp_Object val) {
    w->old_pointm = val;
}

static void wset_start(struct window* w, Lisp_Object val) { w->start = val; }

static void wset_temslot(struct window* w, Lisp_Object val) {
    w->temslot = val;
}

static void wset_vertical_scroll_bar_type(struct window* w, Lisp_Object val) {
    w->vertical_scroll_bar_type = val;
}

static void wset_window_parameters(struct window* w, Lisp_Object val) {
    w->window_parameters = val;
}

static void wset_combination(struct window* w, bool horflag, Lisp_Object val) {
...
bool window_outdated(struct window* w) {
    struct buffer* b = XBUFFER(w->contents);
    return (w->last_modified < BUF_MODIFF(b) ||
            w->last_overlay_modified < BUF_OVERLAY_MODIFF(b));
}

struct window* decode_live_window(register Lisp_Object window) {
    if (NILP(window))
...
struct window* decode_any_window(register Lisp_Object window) {
    struct window* w;
...
static struct window* decode_valid_window(register Lisp_Object window) {
    struct window* w;
...
void wset_buffer(struct window* w, Lisp_Object val) {
...
static void wset_old_buffer(struct window* w, Lisp_Object val) {
    w->old_buffer = val;
}

DEFUN("windowp", Fwindowp, Swindowp, 1, 1, 0,
      doc:/* Return t if OBJECT is a window and nil otherwise.  */)
(Lisp_Object object) { return WINDOWP(object) ? Qt : Qnil; }
...
(Lisp_Object object) { return WINDOW_VALID_P(object) ? Qt : Qnil; }
...
(Lisp_Object object) { return WINDOW_LIVE_P(object) ? Qt : Qnil; }

/* Frames and windows.  */
DEFUN ("window-frame", Fwindow_frame, Swindow_frame, 0, 1, 0,
       doc: /* Return the frame that window WINDOW is on.
WINDOW must be a valid window and defaults to the selected one.  */)
(Lisp_Object window) { return decode_valid_window(window)->frame; }
...
(Lisp_Object frame) { return FRAME_MINIBUF_WINDOW(decode_live_frame(frame)); }
...
(Lisp_Object frame_or_window) {
    ...
    if (NILP(frame_or_window))
        ...
        window =
            XFRAME(WINDOW_FRAME(XWINDOW(frame_or_window)))->selected_window;
    else {
        CHECK_LIVE_FRAME(frame_or_window);
...
functions were run.  */)
(void) { return old_selected_window; }
...
static Lisp_Object select_window(Lisp_Object window, Lisp_Object norecord,
    ...
    if (EQ(window, selected_window) && !inhibit_point_swap)
        /* `switch-to-buffer' uses (select-window (selected-window)) as a
           "clever" way to call record_buffer from Elisp, so it's important that
           we call record_buffer before returning here.  */
        goto record_and_return;

    if (NILP(norecord) ||
        EQ(norecord,
           Qmark_for_redisplay)) { /* Mark the window for redisplay since the
                                  selected-window has a different mode-line.  */
    ...
    return (NILP(w->old_buffer)
                ...
                : (w->change_stamp != WINDOW_XFRAME(w)->change_stamp)
                    ...
                    /* A window that was live the last time seen by window
                       change functions or was deleted.  */
                    : w->old_buffer);
}
...
(Lisp_Object window) { return decode_valid_window(window)->parent; }
...
(Lisp_Object window) { return decode_valid_window(window)->next; }
...
(Lisp_Object window) { return decode_valid_window(window)->prev; }
...
(Lisp_Object window) {
    return (make_fixnum(decode_valid_window(window)->old_pixel_width));
}
...
zero if WINDOW was created after that.  */)
(Lisp_Object window) {
    return (make_fixnum(decode_valid_window(window)->old_pixel_height));
}
...
(Lisp_Object window) { return decode_valid_window(window)->new_total; }
...
(Lisp_Object window) { return decode_valid_window(window)->new_normal; }
...
(Lisp_Object window) { return decode_valid_window(window)->new_pixel; }
...
static enum window_body_unit window_body_unit_from_symbol(Lisp_Object unit) {
    return EQ(unit, Qremap)
        ? WINDOW_BODY_IN_REMAPPED_CHARS
        : (NILP(unit) ? WINDOW_BODY_IN_CANONICAL_CHARS : WINDOW_BODY_IN_PIXELS);
}
...
int window_body_height(struct window* w, enum window_body_unit pixelwise) {
    int height =
        (w->pixel_height - WINDOW_TAB_LINE_HEIGHT(w) -
         WINDOW_HEADER_LINE_HEIGHT(w) -
         (WINDOW_HAS_HORIZONTAL_SCROLL_BAR(w) ? WINDOW_SCROLL_BAR_AREA_HEIGHT(w)
                                              : 0) -
         WINDOW_MODE_LINE_HEIGHT(w) - WINDOW_BOTTOM_DIVIDER_WIDTH(w));
...
/* Return the number of columns/pixels of W's body.  Don't count columns
   ...
   pixelwise.  */
int window_body_width(struct window* w, enum window_body_unit pixelwise) {
    struct frame* f = XFRAME(WINDOW_FRAME(w));

    int width = (w->pixel_width - WINDOW_RIGHT_DIVIDER_WIDTH(w) -
                 (WINDOW_HAS_VERTICAL_SCROLL_BAR(w)
                      ? WINDOW_SCROLL_BAR_AREA_WIDTH(w)
                      : (/* A vertical bar is either 1 or 0.  */
                         !FRAME_WINDOW_P(f) && !WINDOW_RIGHTMOST_P(w) &&
...
(Lisp_Object window, Lisp_Object pixelwise) {
    return (make_fixnum(window_body_width(
        decode_live_window(window), window_body_unit_from_symbol(pixelwise))));
}
...
pixels.  */)
(Lisp_Object window, Lisp_Object pixelwise) {
    return (make_fixnum(window_body_height(
        decode_live_window(window), window_body_unit_from_symbol(pixelwise))));
}
...
(Lisp_Object window) {
    return (make_fixnum(decode_live_window(window)->old_body_pixel_width));
}
...
is zero if WINDOW was created after that.  */)
(Lisp_Object window) {
    return (make_fixnum(decode_live_window(window)->old_body_pixel_height));
}
...
(Lisp_Object window) {
    return make_fixnum(
        WINDOW_SCROLL_BAR_AREA_WIDTH(decode_live_window(window)));
}
...
(Lisp_Object window) {
    return make_fixnum(
        WINDOW_SCROLL_BAR_AREA_HEIGHT(decode_live_window(window)));
}
...
static Lisp_Object set_window_hscroll(struct window* w, EMACS_INT hscroll) {
...
static enum window_part coordinates_in_window(register struct window* w, int x,
                                              int y) {
    struct frame* f = XFRAME(WINDOW_FRAME(w));
    ...
    if (WINDOW_BOTTOM_DIVIDER_WIDTH(w) > 0 &&
        y >= (bottom_y - WINDOW_BOTTOM_DIVIDER_WIDTH(w)) && y <= bottom_y)
        return ON_BOTTOM_DIVIDER;
    /* On vertical window divider?  */
    else if (!WINDOW_RIGHTMOST_P(w) && WINDOW_RIGHT_DIVIDER_WIDTH(w) > 0 &&
             x >= right_x - WINDOW_RIGHT_DIVIDER_WIDTH(w) && x <= right_x)
    ...
    /* On the horizontal scroll bar?  (Including the empty space at its
       right!)  */
              ...
              y >= (bottom_y - WINDOW_SCROLL_BAR_AREA_HEIGHT(w) -
    ...
    /* On the mode or header/tab line?   */
             ...
             (window_wants_tab_line(w) &&
              y < top_y + CURRENT_TAB_LINE_HEIGHT(w) && (part = ON_TAB_LINE)) ||
             (window_wants_header_line(w) &&
              y < top_y + CURRENT_HEADER_LINE_HEIGHT(w) +
                      (window_wants_tab_line(w) ? CURRENT_TAB_LINE_HEIGHT(w)
                                                : 0) &&
              (part = ON_HEADER_LINE))) {
       ...
       must be eventually resized is that on the left of WINDOW.  */
        if ((WINDOW_RIGHT_DIVIDER_WIDTH(w) == 0) &&
            ((WINDOW_HAS_VERTICAL_SCROLL_BAR_ON_LEFT(w) &&
              !WINDOW_LEFTMOST_P(w) && eabs(x - left_x) < grabbable_width) ||
             (!WINDOW_HAS_VERTICAL_SCROLL_BAR_ON_LEFT(w) &&
              !WINDOW_RIGHTMOST_P(w) && eabs(x - right_x) < grabbable_width)))
            return ON_VERTICAL_BORDER;
        else
            return part;
    }
    ...
    if (FRAME_WINDOW_P(f)) {
        if (!w->pseudo_window_p && WINDOW_RIGHT_DIVIDER_WIDTH(w) == 0 &&
            !WINDOW_HAS_VERTICAL_SCROLL_BAR(w) && !WINDOW_RIGHTMOST_P(w) &&
            (eabs(x - right_x) < grabbable_width))
            return ON_VERTICAL_BORDER;
    }
    /* Need to say "x > right_x" rather than >=, since on character
       terminals, the vertical line's x coordinate is right_x.  */
    else if (!w->pseudo_window_p && WINDOW_RIGHT_DIVIDER_WIDTH(w) == 0 &&
...
int window_relative_x_coord(struct window* w, enum window_part part, int x) {
    ...
    case ON_RIGHT_MARGIN:
        return (x + 1 -
                ((w->pseudo_window_p) ? WINDOW_PIXEL_WIDTH(w)
...
static bool check_window_containing(struct window* w, void* user_data) {
...
Lisp_Object window_from_coordinates(struct frame* f, int x, int y,
    ...
    window = Qnil;
    cw.window = &window, cw.x = x, cw.y = y;
    cw.part = part;
    ...
    /* If not found above, see if it's in the menu bar window, if a menu
       bar exists.  */
    if (NILP(window) && menu_bar_p && WINDOWP(f->menu_bar_window) &&
    ...
    /* If not found above, see if it's in the tab bar window, if a tab
       bar exists.  */
    if (NILP(window) && tab_bar_p && WINDOWP(f->tab_bar_window) &&
    ...
    /* If not found above, see if it's in the tool bar window, if a tool
       bar exists.  */
    if (NILP(window) && tool_bar_p && WINDOWP(f->tool_bar_window) &&
...
(Lisp_Object x, Lisp_Object y, Lisp_Object frame) {
    ...
    if (XFIXNUM(x) < 0 || XFIXNUM(x) > FRAME_PIXEL_WIDTH(f) || XFIXNUM(y) < 0 ||
        XFIXNUM(y) > FRAME_PIXEL_HEIGHT(f))
        return Qnil;

    CHECK_NUMBER(x);
    CHECK_NUMBER(y);

    return window_from_coordinates(
        f, (FRAME_PIXEL_X_FROM_CANON_X(f, x) + FRAME_INTERNAL_BORDER_WIDTH(f)),
        (FRAME_PIXEL_Y_FROM_CANON_Y(f, y) + FRAME_INTERNAL_BORDER_WIDTH(f)), 0,
        false, false, false);
}

ptrdiff_t window_point(struct window* w) {
    return (w == XWINDOW(selected_window) ? BUF_PT(XBUFFER(w->contents))
...
(Lisp_Object pos, Lisp_Object window, Lisp_Object partially) {
    ...
    if ((EQ(pos, Qt) || (posint >= CHARPOS(top) && posint <= BUF_ZV(buf))) &&
        CHARPOS(top) >= BUF_BEGV(buf) && CHARPOS(top) <= BUF_ZV(buf) &&
    ...
    if (!NILP(in_window) && !NILP(partially)) {
        ...
        in_window = Fcons(make_fixnum(x), Fcons(make_fixnum(y), part));
    }
...
(Lisp_Object line, Lisp_Object window) {
    ...
    if (!w->window_end_valid || windows_or_buffers_changed || b->clip_changed ||
    ...
    if (EQ(line, Qmode_line)) {
        ...
        return (row->enabled_p ? list4i(row->height, 0, /* not accurate */
...
(Lisp_Object window, Lisp_Object first, Lisp_Object last, Lisp_Object body,
 Lisp_Object inverse, Lisp_Object left) {
    ...
    int window_width = NILP(body) ? w->pixel_width
    ...
    row = (!NILP(first)     ? MATRIX_ROW(w->current_matrix,
                                         check_integer_range(
                                         first, 0, w->current_matrix->nrows))
               : NILP(body) ? MATRIX_ROW(w->current_matrix, 0)
                            : MATRIX_FIRST_TEXT_ROW(w->current_matrix));
    end_row = (!NILP(last) ? MATRIX_ROW(w->current_matrix,
                                        check_integer_range(
                                            last, 0, w->current_matrix->nrows))
                   : NILP(body)
                   ? MATRIX_ROW(w->current_matrix, w->current_matrix->nrows)
                   : MATRIX_BOTTOM_TEXT_ROW(w->current_matrix, w));

    while (row <= end_row && row->enabled_p && row->y + row->height < max_y) {

        if (left_flag) {
            struct glyph* glyph = row->glyphs[TEXT_AREA];

            rows = Fcons(
                Fcons(make_fixnum(invert ? glyph->pixel_width
            ...
            rows =
                Fcons(Fcons(make_fixnum(invert ? window_width - row->pixel_width
                                               : row->pixel_width),
                            make_fixnum(row->y + row->height - subtract)),
                      rows);
        row++;
...
(Lisp_Object window) { return decode_live_window(window)->dedicated; }
...
(Lisp_Object window) { return decode_live_window(window)->prev_buffers; }
...
WINDOW must be a live window and defaults to the selected one.  */)
(Lisp_Object window) { return decode_live_window(window)->next_buffers; }
...
Lisp_Object window_parameter(struct window* w, Lisp_Object parameter) {
...
(Lisp_Object window, Lisp_Object parameter, Lisp_Object value) {
    ...
    if (SYMBOLP(parameter) && WINDOW_LIVE_P(window) &&
...
WINDOW must be a live window and defaults to the selected one.  */)
(Lisp_Object window) { return decode_live_window(window)->display_table; }
...
struct Lisp_Char_Table* window_display_table(struct window* w) {
...
static void unshow_buffer(register struct window* w) {
    ...
    if (!EQ(buf, XWINDOW(selected_window)->contents)
       ...
       useful in connection with bug#12208).
        ...
        temp_set_point_both(
            ...
            clip_to_bounds(BUF_BEGV_BYTE(b), marker_byte_position(w->pointm),
                           BUF_ZV_BYTE(b)));
...
static void replace_window(Lisp_Object old, Lisp_Object new, bool setflag) {
    Lisp_Object tem;
...
static void recombine_windows(Lisp_Object window) {
    struct window *w, *p, *c;
    ...
    if (!NILP(parent) && NILP(w->combination_limit)) {
           ...
           combination.  */
        {
            ...
            while (c) {
                ...
                if (horflag)
                    wset_normal_cols(c,
                                     make_float((double)c->pixel_width /
                                                (double)p->pixel_width));
                else
                    wset_normal_lines(c,
                                      make_float((double)c->pixel_height /
...
static void delete_deletable_window(Lisp_Object window) {
...
static bool add_window_to_list(struct window* w, void* user_data) {
...
static bool candidate_window_p(Lisp_Object window, Lisp_Object owindow,
...
static void decode_next_window_args(Lisp_Object* window, Lisp_Object* minibuf,
    ...
    if (WINDOW_LIVE_P(miniwin) && NILP(*minibuf))
        *minibuf =
            (this_minibuffer_depth(XWINDOW(miniwin)->contents) ? miniwin
                                                               : Qlambda);
    ...
    if (NILP(*all_frames))
        *all_frames
           ...
           before including it (Bug#47207).  */
            = ((WINDOW_LIVE_P(miniwin) && !EQ(*minibuf, Qlambda)) ? miniwin
                                                                  : Qnil);
    else if (EQ(*all_frames, Qvisible))
        ;
...
static Lisp_Object next_window(Lisp_Object window, Lisp_Object minibuf,
                               Lisp_Object all_frames, bool next_p) {
    specpdl_ref count = SPECPDL_INDEX();
    ...
    if (FRAMEP(all_frames) && !EQ(all_frames, XWINDOW(window)->frame))
    ...
    if (next_p) {
        ...
        if (!CONSP(list))
            for (list = Vwindow_list; CONSP(list) && !EQ(XCAR(list), window);
                 list = XCDR(list))
                if (candidate_window_p(XCAR(list), window, minibuf, all_frames))
                    break;
...
static Lisp_Object window_list_1(Lisp_Object window, Lisp_Object minibuf,
                                 Lisp_Object all_frames) {
...
(Lisp_Object frame, Lisp_Object minibuf, Lisp_Object window) {
    if (NILP(window))
        window =
            FRAMEP(frame) ? XFRAME(frame)->selected_window : selected_window;
    CHECK_WINDOW(window);
    if (NILP(frame))
        frame = selected_frame;
...
Lisp_Object mru_rooted_frame(struct frame* f) {
...
static Lisp_Object window_loop(enum window_loop type, Lisp_Object obj,
                               bool mini, Lisp_Object frames) {
        ...
        if (!MINI_WINDOW_P(w)
            ...
            case REPLACE_BUFFER_IN_WINDOWS_SAFELY:
                ...
                if (EQ(w->contents, obj)) {
                    ...
                    set_window_buffer(window, other_buffer_safely(w->contents),
                                      false, false);
...
void check_all_windows(void) { window_loop(CHECK_ALL_WINDOWS, Qnil, true, Qt); }
...
static Lisp_Object resize_root_window(Lisp_Object window, Lisp_Object delta,
                                      Lisp_Object horizontal,
                                      Lisp_Object ignore,
                                      Lisp_Object pixelwise) {
    return calln(Qwindow__resize_root_window, window, delta, horizontal, ignore,
                 pixelwise);
}


static Lisp_Object window_pixel_to_total(Lisp_Object frame,
                                         Lisp_Object horizontal) {
...
static Lisp_Object window_discard_buffer_from_alist(Lisp_Object buffer,
                                                    Lisp_Object alist) {
...
/** Remove all occurrences of BUFFER from LIST.  Return changed
    LIST.  */
static Lisp_Object window_discard_buffer_from_list(Lisp_Object buffer,
                                                   Lisp_Object list) {
...
static void window_discard_buffer_from_window(Lisp_Object buffer,
                                              Lisp_Object window, bool all) {
    struct window* w = XWINDOW(window);

    wset_prev_buffers(
        w, window_discard_buffer_from_alist(buffer, w->prev_buffers));
    wset_next_buffers(w,
                      window_discard_buffer_from_list(buffer, w->next_buffers));
...
/** Remove BUFFER from the lists of previous and next buffers and the
    ...
    WINDOW.  */
void window_discard_buffer_from_dead_windows(Lisp_Object buffer) {
...
(Lisp_Object window, Lisp_Object root) {
    ...
    /* Get SIBLING below (on the right of) WINDOW.  */
    {
        ...
        s = XWINDOW(sibling);
        wset_prev(s, Qnil);
        wset_combination(XWINDOW(w->parent), XWINDOW(w->parent)->horizontal,
                         sibling);
    }
    ...
    if (BUFFERP(w->contents) && !resize_failed) {
       ...
       when the display is not current, due to typeahead).  */
        new_top =
            WINDOW_TOP_EDGE_LINE(w) - FRAME_TOP_MARGIN(XFRAME(WINDOW_FRAME(w)));
        if (new_top != top && startpos >= BUF_BEGV(XBUFFER(w->contents)) &&
...
void replace_buffer_in_windows(Lisp_Object buffer) {
...
static void run_funs(Lisp_Object funs) {
    for (; CONSP(funs); funs = XCDR(funs))
        if (!EQ(XCAR(funs), Qt))
            call0(XCAR(funs));
}

static void select_window_norecord(Lisp_Object window) {
    if (WINDOW_LIVE_P(window))
        Fselect_window(window, Qt);
}

static void select_frame_norecord(Lisp_Object frame) {
 ...
 */
static void run_window_configuration_change_hook(struct frame* f) {
    specpdl_ref count = SPECPDL_INDEX();
    Lisp_Object frame,
        global_wcch = Fdefault_value(Qwindow_configuration_change_hook);
    XSETFRAME(frame, f);

    if (NILP(Vrun_hooks) || !f->can_set_window_size || !f->after_make_frame)
        return;
 ...
 */
static Lisp_Object window_sub_list(Lisp_Object window, Lisp_Object windows) {
 ...
 * NUMBER is 0.
 */
static ptrdiff_t window_change_record_windows(Lisp_Object window, int stamp,
                                              ptrdiff_t number) {
    struct window* w = XWINDOW(window);
 ...
 * old_selected_frame.
 */
static void window_change_record(void) {
    if (window_change_record_frames) {
        ...
        FOR_EACH_FRAME(tail, frame) {
            ...
            f->number_of_windows = window_change_record_windows(
                f->root_window, f->change_stamp, 0);
 ...
 */
static void run_window_change_functions_locally(Lisp_Object symbol,
                                                Lisp_Object buffer,
                                                Lisp_Object window) {
...
static void run_window_change_functions_globally(Lisp_Object symbol,
                                                 Lisp_Object frame) {
    ...
    FOR_EACH_FRAME(tail, frame) {
        ...
        bool frame_selected_change =
            (selected_frame_change &&
             (EQ(frame, old_selected_frame) || EQ(frame, selected_frame)));
        ...
        if (!FRAME_LIVE_P(f) || !f->can_set_window_size ||
            !f->after_make_frame || FRAME_TOOLTIP_P(f) ||
            !(frame_window_change || frame_selected_change ||
              frame_selected_window_change || frame_window_state_change))
       ...
       local hooks right away.  */
        for (; CONSP(windows); windows = XCDR(windows)) {
            ...
            window_buffer_change = (frame_window_change &&
            ...
            if ((window_buffer_change || window_size_change ||
       ...
       windows has changed size.  */
        if (frame_size_change && FRAME_LIVE_P(f))
            run_window_change_functions_globally(Qwindow_size_change_functions,
                                                 frame);
       ...
       input method composition state.  */
        ...
        if ((frame_selected_window_change || frame_buffer_change) &&
            FRAME_LIVE_P(f) && FRAME_WINDOW_P(f))
       ...
       (de-)selected or its window state change flag was set.  */
        if ((frame_selected_change || frame_selected_window_change ||
            ...
            FRAME_LIVE_P(f)) {
            run_window_change_functions_globally(Qwindow_state_change_functions,
                                                 frame);
...
void set_window_buffer(Lisp_Object window, Lisp_Object buffer, bool run_hooks_p,
                       bool keep_margins_p) {
    struct window* w = XWINDOW(window);
    ...
    if (!(keep_margins_p &&
          samebuf)) { /* If we're not actually changing the buffer, don't reset
                     hscroll and vscroll.  Resetting hscroll and vscroll here is
                     problematic for things like image-mode and doc-view-mode
                     since it resets the image's position whenever we resize the
                     frame.  */
        ...
        set_marker_restricted(w->start, make_fixnum(b->last_window_start),
                              buffer);
        w->start_at_line_beg = false;
        w->force_start = false;
    ...
    XMARKER(w->old_pointm)->insertion_type =
...
(register Lisp_Object window, Lisp_Object buffer_or_name,
...
static Lisp_Object display_buffer(Lisp_Object buffer,
...
/* Obsolete since 24.3.  */
void temp_output_buffer_show(register Lisp_Object buf) {
...
static bool window_resize_check(struct window* w, bool horflag) {
    struct frame* f = XFRAME(w->frame);
    struct window* c;
    ...
    /* W is a horizontal combination.  */
    {
        ...
        return (XFIXNUM(w->new_pixel) >=
                (horflag ? 2 * FRAME_COLUMN_WIDTH(f) : FRAME_LINE_HEIGHT(f)));
}
...
static void window_resize_apply(struct window* w, bool horflag) {
    struct window* c;
    int edge;
    int unit = (horflag ? FRAME_COLUMN_WIDTH(WINDOW_XFRAME(w))
...
/* Set w->total_lines (w->total_cols if HORFLAG) to
   ...
   positions of each of these windows.  */
static void window_resize_apply_total(struct window* w, bool horflag) {
    struct window* c;
    int edge;
...
(Lisp_Object frame, Lisp_Object horizontal) {
    ...
    if (!window_resize_check(r, horflag) ||
        (XFIXNUM(r->new_pixel) != (horflag ? r->pixel_width : r->pixel_height)))
        return Qnil;
...
void resize_frame_windows(struct frame* f, int size, bool horflag) {
...
static Lisp_Object make_parent_window(Lisp_Object frame) {
    Lisp_Object parent;
...
static void window_set_parent_and_normal_sizes(Lisp_Object parent) {
    ...
    if (p->horizontal) {
        while (true) {
            ...
            if (NILP(w->next)) {
                ...
                wset_normal_cols(w,
                                 make_float((double)w->pixel_width /
                                            (double)p->pixel_width));
        ...
        while (true) {
            ...
            if (NILP(w->next)) {
                ...
                wset_normal_lines(w,
                                  make_float((double)w->pixel_height /
                                             (double)p->pixel_height));
...
(Lisp_Object old, Lisp_Object pixel_size, Lisp_Object side,
 Lisp_Object normal_size, Lisp_Object refer) {
    ...
    EMACS_INT total_size = XFIXNUM(pixel_size) /
        (horflag ? FRAME_COLUMN_WIDTH(f) : FRAME_LINE_HEIGHT(f));
    ...
    bool combination_limit =
        (EQ(Vwindow_combination_limit, Qt) || NILP(o->parent) ||
         (horflag ? WINDOW_VERTICAL_COMBINATION_P(XWINDOW(o->parent))
       ...
       is a deleted former parent window.  */
           ...
           contents can be used.  */
        {
            if (!BUFFERP(r->old_buffer))
                error("REFER must specify a former live window (must have "
                      "shown a buffer)");
            else if (!BUFFER_LIVE_P(XBUFFER(r->old_buffer)))
       ...
       proportionally.  */
    {
        ...
        wset_new_pixel(
            op,
            make_fixnum((horflag ? op->pixel_width : op->pixel_height) -
                        XFIXNUM(pixel_size)));
    ...
    if (combination_limit) {
       ...
       p->new_normal.  */
        ...
        wset_new_pixel(p,
                       make_fixnum(horflag ? o->pixel_width : o->pixel_height));
        wset_new_total(p,
                       make_fixnum(horflag ? o->total_cols : o->total_lines));
    ...
    wset_new_total(
        n, make_fixnum((horflag ? p->total_cols : p->total_lines) - sum));
    ...
    if (dead) {
        ...
        set_marker_restricted(n->start, make_fixnum(XMARKER(n->start)->charpos),
                              n->contents);
        set_marker_restricted(
            n->pointm, make_fixnum(XMARKER(n->pointm)->charpos), n->contents);
        set_marker_restricted(n->old_pointm,
                              make_fixnum(XMARKER(n->old_pointm)->charpos),
                              n->contents);

        Vwindow_list = Qnil;
        /* Remove window from the table of dead windows.  */
        Fremhash(make_fixnum(n->sequence_number), window_dead_windows_table);
    }
    ...
    /* We can delete WINDOW now.  */
    {
       ...
       highlight, invalidate that slot to be safe (Bug#9904).  */
        ...
        if (WINDOWP(w->contents)) {
            ...
            Fputhash(make_fixnum(w->sequence_number), window,
 ...
 */
static void resize_mini_window_apply(struct window* w, int delta) {
    struct frame* f = XFRAME(w->frame);
    Lisp_Object root = FRAME_ROOT_WINDOW(f);
    struct window* r = XWINDOW(root);
 ...
 * line of text.
 */
void grow_mini_window(struct window* w, int delta, int unit) {
    struct frame* f = XFRAME(w->frame);
    ...
    if (delta != 0) {
        ...
        grow = calln(Qwindow__resize_root_window_vertically, root,
                     make_fixnum(-delta), Qt);
        ...
        if (FIXNUMP(grow)
            ...
            && XFIXNUM(grow) != 0 && window_resize_check(r, false))
 ...
 * line of text.
 */
void shrink_mini_window(struct window* w, int unit) {
    struct frame* f = XFRAME(w->frame);
...
(Lisp_Object window) {
    ...
    if (window_resize_check(r, false) && XFIXNUM(w->new_pixel) > 0 &&
...
void mark_window_cursors_off(struct window* w) {
    while (w) {
        if (WINDOWP(w->contents))
 ...
 */
bool window_wants_mode_line(struct window* w) {
    Lisp_Object window_mode_line_format =
        window_parameter(w, Qmode_line_format);

    return (WINDOW_LEAF_P(w) && !MINI_WINDOW_P(w) && !WINDOW_PSEUDO_P(w) &&
 ...
 */
bool window_wants_header_line(struct window* w) {
    ...
    return (WINDOW_LEAF_P(w) && !MINI_WINDOW_P(w) && !WINDOW_PSEUDO_P(w) &&
            ...
            (WINDOW_PIXEL_HEIGHT(w) > (window_wants_mode_line(w)
...
bool window_wants_tab_line(struct window* w) {
    Lisp_Object window_tab_line_format = window_parameter(w, Qtab_line_format);

    return (
        WINDOW_LEAF_P(w) && !MINI_WINDOW_P(w) && !WINDOW_PSEUDO_P(w) &&
        !EQ(window_tab_line_format, Qnone) &&
        (!NILP(window_tab_line_format) ||
         !NILP(BVAR(XBUFFER(WINDOW_BUFFER(w)), tab_line_format))) &&
        (WINDOW_PIXEL_HEIGHT(w) > (((window_wants_mode_line(w) ? 1 : 0) +
                                    (window_wants_header_line(w) ? 1 : 0) + 1) *
                                   WINDOW_FRAME_LINE_HEIGHT(w))));
}
...
static void window_scroll(Lisp_Object window, EMACS_INT n, bool whole,
                          bool noerror) {
    struct window* w = XWINDOW(window);
    struct buffer* b = XBUFFER(w->contents);
    ...
    if (b->long_line_optimizations_p && !long_lines_truncated &&
...
int window_scroll_margin(struct window* window, enum margin_unit unit) {
    if (scroll_margin > 0) {
        ...
        int max_margin =
            min((window_lines - 1) / 2, (int)(window_lines * ratio));
        int margin = clip_to_bounds(0, scroll_margin, max_margin);
        return (unit == MARGIN_IN_PIXELS) ? margin * frame_line_height : margin;
    }
    else
        return 0;
}

static int sanitize_next_screen_context_lines(void) {
...
static void window_scroll_for_long_lines(struct window* w, int n,
                                         bool noerror) {
    ...
    if (!(PT >= startpos && PT <= ZV && startpos <= ZV &&
    ...
    if (pos.bufpos < ZV) {
        set_marker_restricted_both(w->start, w->contents, pos.bufpos,
                                   pos.bytepos);
...
static void window_scroll_pixel_based(Lisp_Object window, int n, bool whole,
                                      bool noerror) {
    struct it it;
    struct window* w = XWINDOW(window);
    ...
    bool adjust_old_pointm =
        !NILP(Fequal(Fwindow_point(window), Fwindow_old_point(window)));
    ...
    if (whole) {
       ...
       we would end up at the start of the line ending at ZV.  */
        if (dy <= 0) {
            ...
            while (start_pos == IT_CHARPOS(it) && start_pos > BEGV)
                move_it_by_lines(&it, -1);
        }
        else if (dy > 0) {
            goal_y = it.current_y + dy;
            ...
            /* Ensure we actually do move, e.g. in case we are currently
               looking at an image that is taller that the window height.  */
            while (start_pos == IT_CHARPOS(it) && start_pos < ZV)
                move_it_by_lines(&it, 1);
        }
    }
    else
    ...
    if (n > 0) {
       ...
       in the scroll margin at the bottom.  */
        move_it_to(&it, PT, -1,
                   ...
                   (it.last_visible_y - WINDOW_TAB_LINE_HEIGHT(w) -
                    WINDOW_HEADER_LINE_HEIGHT(w) - partial_line_height(&it) -
       ...
       reached PT before the scroll margin.  */
        ...
        if (it.what == IT_EOB)
            ...
            move_it_by_lines(&it, 1);
            partial_p = it.current_y > it.last_visible_y - this_scroll_margin -
                    WINDOW_TAB_LINE_HEIGHT(w) - WINDOW_HEADER_LINE_HEIGHT(w);
        }
        ...
        if (charpos == PT && !partial_p &&
            ...
            move_it_to(&it, -1, window_scroll_pixel_based_preserve_x, goal_y,
                       -1, MOVE_TO_Y | MOVE_TO_X);
            SET_PT_BOTH(IT_CHARPOS(it), IT_BYTEPOS(it));
        }
        else {
...
static void window_scroll_line_based(Lisp_Object window, int n, bool whole,
                                     bool noerror) {
    struct window* w = XWINDOW(window);
    ...
    if (!NILP(Vscroll_preserve_screen_position)) {
        if (window_scroll_preserve_vpos <= 0 ||
            ...
            struct position posit =
                *compute_motion(startpos, startbyte, 0, 0, false, PT, ht, 0, -1,
                                w->hscroll, 0, w);
       ...
       that it is outside the scroll margin.  */
            ...
            if (bottom_margin > marker_position(opoint_marker))
                ...
                if (!NILP(Vscroll_preserve_screen_position)) {
                    ...
                    Fvertical_motion(
                        Fcons(make_fixnum(window_scroll_preserve_hpos),
                              make_fixnum(nlines)),
                        window, Qnil);
                }
                else
...
static void scroll_command(Lisp_Object window, Lisp_Object n, int direction) {
    struct window* w;
    bool other_window;
    specpdl_ref count = SPECPDL_INDEX();
...
(register Lisp_Object arg, Lisp_Object set_minimum) {
    ...
    EMACS_INT requested_arg =
        (NILP(arg) ? window_body_width(w, WINDOW_BODY_IN_CANONICAL_CHARS) - 2
...
(void) {
    if (minibuf_level > 0 && MINI_WINDOW_P(XWINDOW(selected_window)) &&
...
static int displayed_window_lines(struct window* w) {
    struct it it;
    struct text_pos start;
...
(Lisp_Object arg, Lisp_Object redisplay) {
    ...
    if (NILP(arg)) {
        if (!NILP(redisplay) && !NILP(Vrecenter_redisplay) &&
    ...
    w->start_at_line_beg =
        (bytepos == BEGV_BYTE || FETCH_BYTE(bytepos - 1) == '\n');
...
(Lisp_Object object) { return WINDOW_CONFIGURATIONP(object) ? Qt : Qnil; }
 ...
 Lisp_Object dont_set_miniwindow) {
    ...
    if (!BUFFER_LIVE_P(XBUFFER(new_current_buffer)))
        ...
        if (XBUFFER(new_current_buffer) == current_buffer)
            ...
            if (EQ(XWINDOW(data->current_window)->contents,
                   new_current_buffer) &&
                WINDOWP(selected_window) &&
                EQ(XWINDOW(selected_window)->contents, new_current_buffer) &&
                !EQ(selected_window, data->current_window))
                old_point =
                    marker_position(XWINDOW(data->current_window)->pointm);
            else
                old_point = PT;
        else
    ...
    if (FRAME_LIVE_P(f)) {
       ...
       code and is thus potentially unsafe while input is blocked.  */
        for (k = 0; k < saved_windows->header.size; k++) {
            ...
            if (BUFFERP(w->contents) && !EQ(w->contents, p->buffer) &&
       ...
       dead.  */
        ...
        for (k = 0; k < saved_windows->header.size; k++) {
            ...
            if (!NILP(p->parent))
                wset_parent(
                    w,
                    SAVED_WINDOW_N(saved_windows, XFIXNAT(p->parent))->window);
            else
                wset_parent(w, Qnil);
               ...
               minibuffer we explicitly prohibit.  */
            {
                if (!EQ(w->contents, p->buffer)) {
                    ...
                    window_discard_buffer_from_window(w->contents, window,
                                                      false);
                }
                ...
                set_marker_restricted(w->old_pointm, p->old_pointm,
                                      w->contents);
               ...
               current when the window configuration was recorded.  */
                if (!EQ(p->buffer, new_current_buffer) &&
                    ...
                    Fgoto_char(w->pointm);
                     ...
                     BUFFER_LIVE_P(XBUFFER(w->contents)))
            /* Keep window's old buffer; make sure the markers are real.  */
            {
                ...
                if (XMARKER(w->pointm)->buffer == 0)
                    set_marker_restricted_both(
                        w->pointm, w->contents, BUF_PT(XBUFFER(w->contents)),
                        BUF_PT_BYTE(XBUFFER(w->contents)));
                    ...
                    if (FUNCTIONP(window_restore_killed_buffer_windows))
                        kept_windows =
                            Fcons(list(window, p->buffer,
                                       ...
                                       p->dedicated, Qnil),
                                  kept_windows);
                    else if (EQ(window_restore_killed_buffer_windows,
                                Qdelete) ||
                             (!NILP(p->dedicated) &&
       ...
       selected window.  */
        if (FRAME_LIVE_P(XFRAME(data->selected_frame)))
            do_switch_frame(NILP(dont_set_frame) ? data->selected_frame
                                                 : old_frame,
                            0, 0, Qnil);
    }
    ...
    if (!NILP(Vrun_hooks) && FUNCTIONP(window_restore_killed_buffer_windows))
        safe_calln(window_restore_killed_buffer_windows, frame, kept_windows,
                   Qconfiguration);

    return FRAME_LIVE_P(f) ? Qt : Qnil;
}

void restore_window_configuration(Lisp_Object configuration) {
...
void delete_all_child_windows(Lisp_Object window) {
    register struct window* w;
...
static ptrdiff_t count_windows(struct window* window) {
    ptrdiff_t count = 1;
    if (!NILP(window->next))
...
static ptrdiff_t get_leaf_windows(struct window* w, struct window** flat,
                                  ptrdiff_t i) {
    while (w) {
        if (WINDOWP(w->contents))
...
static ptrdiff_t save_window_save(Lisp_Object window,
                                  struct Lisp_Vector* vector, ptrdiff_t i) {
    struct saved_window* p;
    struct window* w;
                ...
                if (CONSP(pers) && !NILP(XCDR(pers))) {
                    ...
                    if (NILP(par))
                        ...
                        /* If the window has a value for the parameter,
                           save it.  */
                        p->window_parameters = Fcons(
                            Fcons(XCAR(par), XCDR(par)), p->window_parameters);
                }
            }
        }

        if (BUFFERP(w->contents)) {
            bool window_point_insertion_type = !NILP(
...
(Lisp_Object frame) {
    ...
    struct save_window_data* data = ALLOCATE_PSEUDOVECTOR(
        struct save_window_data, saved_windows, PVEC_WINDOW_CONFIGURATION);
    ...
    data->minibuf_scroll_window =
        minibuf_level > 0 ? Vminibuf_scroll_window : Qnil;
    data->minibuf_selected_window =
        minibuf_level > 0 ? minibuf_selected_window : Qnil;
...
static void apply_window_adjustment(struct window* w) {
    eassert(w);
...
static int extract_dimension(Lisp_Object dimension) {
    if (NILP(dimension))
        return -1;
    return check_integer_range(dimension, 0, INT_MAX);
}

static struct window* set_window_margins(struct window* w,
    ...
    if (w->left_margin_cols != left || w->right_margin_cols != right) {
        ...
        if ((WINDOW_PIXEL_WIDTH(w) - WINDOW_FRINGES_WIDTH(w) -
             WINDOW_SCROLL_BAR_AREA_WIDTH(w) - (left + right) * unit) >=
...
(Lisp_Object window, Lisp_Object left_width, Lisp_Object right_width) {
    struct window* w =
        set_window_margins(decode_live_window(window), left_width, right_width);
...
(Lisp_Object window) {
    struct window* w = decode_live_window(window);
    return Fcons(w->left_margin_cols ? make_fixnum(w->left_margin_cols) : Qnil,
                 w->right_margin_cols ? make_fixnum(w->right_margin_cols)
                                      : Qnil);
}
...
static struct window* set_window_fringes(struct window* w,
                                         Lisp_Object left_width,
                                         Lisp_Object right_width,
    ...
    if (!FRAME_WINDOW_P(WINDOW_XFRAME(w)))
       ...
       fit the window's dimensions.  */
        if ((WINDOW_PIXEL_WIDTH(w) - WINDOW_MARGINS_WIDTH(w) -
             WINDOW_SCROLL_BAR_AREA_WIDTH(w) - WINDOW_RIGHT_DIVIDER_WIDTH(w) -
...
(Lisp_Object window, Lisp_Object left_width, Lisp_Object right_width,
 Lisp_Object outside_margins, Lisp_Object persistent) {
    struct window* w =
        set_window_fringes(decode_live_window(window), left_width, right_width,
                           outside_margins, persistent);
    return w ? (apply_window_adjustment(w), Qt) : Qnil;
}
...
(Lisp_Object window, Lisp_Object type) {
    ...
    if (!(NILP(type) || EQ(type, Qt) || EQ(type, Qbox) || EQ(type, Qhollow) ||
          EQ(type, Qbar) || EQ(type, Qhbar) ||
          (CONSP(type) &&
           (EQ(XCAR(type), Qbox) || EQ(XCAR(type), Qbar) ||
            EQ(XCAR(type), Qhbar)) &&
           INTEGERP(XCDR(type)))))
...
WINDOW must be a live window and defaults to the selected one.  */)
(Lisp_Object window) { return decode_live_window(window)->cursor_type; }
...
(Lisp_Object window) {
    ...
    if (phys_cursor_glyph && phys_cursor_glyph->type == STRETCH_GLYPH &&
    ...
    return CALLN(Fvector, w->cursor_type, make_fixnum(w->phys_cursor.x),
                 make_fixnum(w->phys_cursor.y), make_fixnum(phys_cursor_width),
...
set_window_scroll_bars(struct window* w, Lisp_Object width,
    ...
    if (!FRAME_WINDOW_P(WINDOW_XFRAME(w)))
        ...
        if (new_width == 0)
            vertical_type = Qnil;
        else if (!(NILP(vertical_type) || EQ(vertical_type, Qleft) ||
                   EQ(vertical_type, Qright) || EQ(vertical_type, Qt)))
            error("Invalid type of vertical scroll bar");

        /* Check dimension of new scroll bar.  Make changes only if it
       fit the window's dimensions.  */
        if ((WINDOW_PIXEL_WIDTH(w) - WINDOW_MARGINS_WIDTH(w) -
             WINDOW_FRINGES_WIDTH(w) - WINDOW_RIGHT_DIVIDER_WIDTH(w) -
        ...
        if (!(NILP(horizontal_type) || EQ(horizontal_type, Qbottom) ||
              EQ(horizontal_type, Qt)))
            error("Invalid type of horizontal scroll bar");

        /* Don't change anything if new scroll bar won't fit.  */
        if ((WINDOW_PIXEL_HEIGHT(w) - WINDOW_TAB_LINE_HEIGHT(w) -
             WINDOW_HEADER_LINE_HEIGHT(w) - WINDOW_MODE_LINE_HEIGHT(w) -
             (new_height == -1 ? FRAME_SCROLL_BAR_AREA_HEIGHT(f)
                               : new_height)) >=
...
(Lisp_Object window, Lisp_Object width, Lisp_Object vertical_type,
 Lisp_Object height, Lisp_Object horizontal_type, Lisp_Object persistent) {
    struct window* w =
        set_window_scroll_bars(decode_live_window(window), width, vertical_type,
                               height, horizontal_type, persistent);
    return w ? (apply_window_adjustment(w), Qt) : Qnil;
}
...
(Lisp_Object window) {
    ...
    return Fcons(
        ((w->scroll_bar_width >= 0) ? make_fixnum(w->scroll_bar_width) : Qnil),
...
(Lisp_Object window, Lisp_Object pixels_p) {
    ...
    if (FRAME_WINDOW_P(f))
        result = (NILP(pixels_p) ? FRAME_CANON_Y_FROM_PIXEL_Y(f, -w->vscroll)
...
(Lisp_Object window, Lisp_Object vscroll, Lisp_Object pixels_p,
 Lisp_Object preserve_vscroll_p) {
    ...
    if (FRAME_WINDOW_P(f)) {
        ...
        w->vscroll =
            -(NILP(pixels_p) ? FRAME_LINE_HEIGHT(f) * XFLOATINT(vscroll)
                             : XFLOATINT(vscroll));
...
static void foreach_window(struct frame* f, bool (*fn)(struct window*, void*),
                           void* user_data) {
...
static bool foreach_window_1(struct window* w,
...
static bool compare_window_configurations(Lisp_Object configuration1,
    ...
    for (i = 0; i < sws1->header.size; i++) {
        ...
        if (
            ...
            /* The next two disjuncts check the window structure for
               equality.  */
            || !EQ(sw1->parent, sw2->parent) || !EQ(sw1->prev, sw2->prev) ||
...
void init_window(void) { Vwindow_list = Qnil; }
...
:/* Non-nil means it is the window that C-M-v in minibuffer should scroll.  */);
    ...
    DEFVAR_BOOL("mode-line-in-non-selected-windows",
                mode_line_in_non_selected_windows, doc:
                    /* Non-nil means to use `mode-line-inactive' face in
    non-selected windows. If the minibuffer is active, the
    `minibuffer-scroll-window' mode line is displayed in the `mode-line' face.
  */);
    mode_line_in_non_selected_windows = true;

    DEFVAR_LISP("other-window-scroll-buffer", Vother_window_scroll_buffer, doc:/* If this is a live buffer, \\[scroll-other-window] should scroll its window.  */);
...
See `marker-insertion-type' for the meaning of the possible values.  */);
    ...
    DEFVAR_LISP("window-buffer-change-functions",
                Vwindow_buffer_change_functions, doc:
                    /* Functions called during redisplay when window buffers
    have changed. The value should be a list of functions that take one
    argument.
...
is affected by a non-nil value of `window-combination-limit'.  */);
    ...
    DEFVAR_LISP("window-combination-limit", Vwindow_combination_limit, doc:
                    /* If non-nil, splitting a window makes a new parent
