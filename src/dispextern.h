...
#define CLIP_TEXT_POS_FROM_MARKER(POS, MARKER)                                 \
    (CHARPOS(POS) = clip_to_bounds(BEGV, marker_position(MARKER), ZV),         \
     BYTEPOS(POS) =                                                            \
...
#define TEXT_POS_EQUAL_P(POS1, POS2)                                           \
    ((POS1).charpos == (POS2).charpos && (POS1).bytepos == (POS2).bytepos)
...
INLINE int GLYPH_CODE_CHAR(Lisp_Object gc) {
    return (CONSP(gc) ? XFIXNUM(XCAR(gc)) : XFIXNUM(gc) & MAX_CHAR);
}

INLINE int GLYPH_CODE_FACE(Lisp_Object gc) {
...
struct glyph {
    /* Position from which this glyph was drawn.  If `object' below is a
       ...
       newline stretch glyph at left edge of R2L lines     buffer position
       of newline  */
    ptrdiff_t charpos;
...
#define FACE_ID_BITS 20
    ...
    union {
        ...
        struct {
            /* Value is an enum of the type
             * glyphless_display_method.  */
            unsigned method : 2;
            /* True iff this glyph is for a character of no font. */
            bool_bf for_no_font : 1;
            /* Length of acronym or hexadecimal code string (at most
             * 8).  */
            unsigned len : 4;
...
#define GLYPH_EQUAL_P(X, Y)                                                    \
    ((X)->type == (Y)->type && (X)->u.val == (Y)->u.val &&                     \
     ...
     (X)->voffset == (Y)->voffset && (X)->pixel_width == (Y)->pixel_width)
...
#define SET_CHAR_GLYPH_FROM_GLYPH(FRAME, GLYPH, FROM)                          \
    SET_CHAR_GLYPH(FRAME, GLYPH, GLYPH_CHAR(FROM), GLYPH_FACE(FROM), false)
...
#define MATRIX_MODE_LINE_ROW(MATRIX) ((MATRIX)->rows + (MATRIX)->nrows - 1)
...
#define MATRIX_ROW_USED(MATRIX, ROW) (MATRIX_ROW(MATRIX, ROW)->used[TEXT_AREA])
...
#define MATRIX_BOTTOM_TEXT_ROW(MATRIX, W)                                      \
    ((MATRIX)->rows + (MATRIX)->nrows - (window_wants_mode_line(W) ? 1 : 0))
...
#define MATRIX_ROW_ENABLED_P(MATRIX, ROW) (MATRIX_ROW(MATRIX, ROW)->enabled_p)
...
#define MR_PARTIALLY_VISIBLE(ROW) ((ROW)->height != (ROW)->visible_height)
...
#define MATRIX_ROW_PARTIALLY_VISIBLE_AT_BOTTOM_P(W, ROW)                       \
    (MR_PARTIALLY_VISIBLE(ROW) && MR_PARTIALLY_VISIBLE_AT_BOTTOM(W, ROW))
...
#define MATRIX_ROW_ENDS_IN_MIDDLE_OF_CHAR_P(ROW)                               \
    ((ROW)->end.dpvec_index > 0 || (ROW)->end.overlay_string_index >= 0 ||     \
...
#define MATRIX_ROW_OVERLAPS_PRED_P(ROW) ((ROW)->phys_ascent > (ROW)->ascent)

/* True means ROW overlaps its successor.  */

#define MATRIX_ROW_OVERLAPS_SUCC_P(ROW)                                        \
    ((ROW)->phys_height - (ROW)->phys_ascent > (ROW)->height - (ROW)->ascent)
...
#define MATRIX_MODE_LINE_HEIGHT(MATRIX)                                        \
    ((MATRIX) && (MATRIX)->rows ? MATRIX_MODE_LINE_ROW(MATRIX)->height : 0)
...
#define MATRIX_HEADER_LINE_HEIGHT(MATRIX)                                      \
    ((MATRIX) && (MATRIX)->rows ? MATRIX_HEADER_LINE_ROW(MATRIX)->height : 0)
...
#define MATRIX_TAB_LINE_HEIGHT(MATRIX)                                         \
    ((MATRIX) && (MATRIX)->rows ? MATRIX_TAB_LINE_ROW(MATRIX)->height : 0)
...
#define CURRENT_HEADER_LINE_ACTIVE_FACE_ID(W)                                  \
    CURRENT_HEADER_LINE_ACTIVE_FACE_ID_3(W, XWINDOW(selected_window), W)
...
#define DESIRED_MODE_LINE_HEIGHT(W) MATRIX_MODE_LINE_HEIGHT((W)->desired_matrix)
...
#define DESIRED_TAB_LINE_HEIGHT(W) MATRIX_TAB_LINE_HEIGHT((W)->desired_matrix)
...
INLINE bool face_tty_specified_color(unsigned long color) {
...
/* Return the id of the realized face on frame F that is like the face
   ...
   CHAR.  */
INLINE int FACE_FOR_CHAR(struct frame* f, struct face* face, int character,
                         ptrdiff_t pos, Lisp_Object object) {
    return face->id;
}

/* Return true if G contains a valid character code.  */
INLINE bool GLYPH_CHAR_VALID_P(GLYPH g) { return CHAR_VALID_P(GLYPH_CHAR(g)); }
...
INLINE bool GLYPH_CODE_P(Lisp_Object gc) {
    return (
        CONSP(gc)
            ? (CHARACTERP(XCAR(gc)) && RANGED_FIXNUMP(0, XCDR(gc), MAX_FACE_ID))
            : (RANGED_FIXNUMP(
                  0, gc,
...
INLINE void reset_mouse_highlight(Mouse_HLInfo* hlinfo) {
...
struct redisplay_interface {
    ...
    /* Set *LEFT and *RIGHT to the left and right overhang of GLYPH on
       frame F.  */
    void (*get_glyph_overhangs)(struct glyph* glyph, struct frame* f, int* left,
                                int* right);
...
struct glyph_row* row_containing_pos(struct window*, ptrdiff_t,
                                     struct glyph_row*, struct glyph_row*, int);
...
void window_box(struct window*, enum glyph_row_area, int*, int*, int*, int*);
...
void init_iterator(struct it*, struct window*, ptrdiff_t, ptrdiff_t,
                   struct glyph_row*, enum face_id);
...
void init_iterator_to_row_start(struct it*, struct window*, struct glyph_row*);
...
void move_it_in_display_line(struct it* it, ptrdiff_t to_charpos, int to_x,
...
extern ptrdiff_t compute_display_string_end(ptrdiff_t, struct bidi_string_data*,
                                            struct window*);
...
extern void tty_draw_row_with_mouse_face(struct window*, struct glyph_row*, int,
...
void update_face_from_frame_parameter(struct frame*, Lisp_Object, Lisp_Object);
extern bool tty_defined_color(struct frame*, const char*, Emacs_Color*, bool,
                              bool);
bool parse_color_spec(const char*, unsigned short*, unsigned short*,
                      unsigned short*);

Lisp_Object tty_color_name(struct frame*, int);
void clear_face_cache(bool);
char* choose_face_font(struct frame*, Lisp_Object*, Lisp_Object, int*);
...
int lookup_derived_face(struct window*, struct frame*, Lisp_Object, int, bool);
void init_frame_faces(struct frame*);
void free_frame_faces(struct frame*);
void recompute_basic_faces(struct frame*);
int face_at_buffer_position(struct window*, ptrdiff_t, ptrdiff_t*, ptrdiff_t,
                            bool, int, enum lface_attribute_index);
...
extern Lisp_Object buffer_posn_from_coords(struct window*, int*, int*,
                                           struct display_pos*, Lisp_Object*,
                                           int*, int*, int*, int*);
extern Lisp_Object mode_line_string(struct window*, enum window_part, int*,
                                    int*, ptrdiff_t*, Lisp_Object*, int*, int*,
                                    int*, int*);
extern Lisp_Object marginal_area_string(struct window*, enum window_part, int*,
                                        int*, ptrdiff_t*, Lisp_Object*, int*,
                                        int*, int*, int*);
...
void shift_glyph_matrix(struct window*, struct glyph_matrix*, int, int, int);
void rotate_matrix(struct glyph_matrix*, int, int, int);
void increment_matrix_positions(struct glyph_matrix*, int, int, ptrdiff_t,
                                ptrdiff_t);
...
extern void do_line_insertion_deletion_costs(struct frame*, const char*,
                                             ...
                                             const char*, const char*,
                                             const char*, int);
void scrolling_1(struct frame*, int, int, int, int*, int*, unsigned*, unsigned*,
                 int);
