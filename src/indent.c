...
struct Lisp_Char_Table* buffer_display_table(void) {
    Lisp_Object thisbuf;
...
static int character_width(int c, struct Lisp_Char_Table* dp) {
    Lisp_Object elt;
...
bool disptab_matches_widthtab(struct Lisp_Char_Table* disptab,
    ...
    for (i = 0; i < 256; i++)
        if (character_width(i, disptab) != XFIXNAT(widthtab->contents[i]))
            return 0;
...
static struct region_cache* width_run_cache_on_off(void) {
...
ptrdiff_t skip_invisible(ptrdiff_t pos, ptrdiff_t* next_boundary_p,
    ...
    /* Otherwise, scan for the next `invisible' property change.  */
        ...
        if (XFIXNAT(overlay_limit) < XFIXNAT(proplimit))
            proplimit = overlay_limit;
        tmp = Fnext_single_property_change(position, Qinvisible, buffer,
                                           proplimit);
        end = XFIXNAT(tmp);
#if 0
    ...
    prop = Fget_char_property(
        position, Qinvisible,
        (!NILP(window) && EQ(XWINDOW(window)->contents, buffer)) ? window
                                                                 : buffer);
...
void invalidate_current_column(void) { last_known_column_point = 0; }

ptrdiff_t current_column(void) {
    ptrdiff_t col;
    ...
    if (PT == last_known_column_point && MODIFF == last_known_column_modified)
        return last_known_column;

    ptrdiff_t line_beg =
        find_newline(PT, PT_BYTE, BEGV, BEGV_BYTE, -1, NULL, NULL, 1);
    ...
    if (buffer_intervals(current_buffer) || buffer_has_overlays() ||
        Z != Z_BYTE)
...
/* Check the presence of a display property and compute its width.
   ...
   the selected window.  */
static int check_display_width(Lisp_Object window, ptrdiff_t pos,
                               ptrdiff_t pos_byte, ptrdiff_t col,
                               ptrdiff_t* endpos) {
    ...
    if (!NILP(val = get_char_property_and_overlay(make_fixnum(pos), Qdisplay,
        ...
        if (CONSP(val)) {
            ...
            if (EQ(Qspace, xcar)) { /* FIXME: Use calc_pixel_width_or_height. */
                Lisp_Object prop;
...
static void scan_for_column(ptrdiff_t* endpos, EMACS_INT* goalcol,
                            ptrdiff_t* prevpos, ptrdiff_t* prevbpos,
                            ptrdiff_t* prevcol) {
    int tab_width = SANE_TAB_WIDTH(current_buffer);
    bool ctl_arrow = !NILP(BVAR(current_buffer, ctl_arrow));
    struct Lisp_Char_Table* dp = buffer_display_table();
    ...
    while (scan < end) {
        ...
        while (scan == next_boundary) {
            ...
            if (scan > cmp_it.stop_pos && cmp_it.id < 0)
                composition_reseat_it(&cmp_it, scan, scan_byte, end, w, -1,
                                      NULL, Qnil);
        }
       ...
       character on which the cursor will appear.  */
        ...
        { /* Check display property.  */
            ptrdiff_t endp;
            int width =
                check_display_width(window, scan, scan_byte, col, &endp);
            if (width >= 0) {
                col += width;
                if (endp >
                    scan) /* Avoid infinite loops with 0-width overlays.  */
                {
                    scan = endp;
                    scan_byte = CHAR_TO_BYTE(scan);
                    continue;
        ...
        if (cmp_it.id >= 0 ||
            (scan == cmp_it.stop_pos &&
             composition_reseat_it(&cmp_it, scan, scan_byte, end, w, -1, NULL,
                                   Qnil)))
...
(Lisp_Object column, Lisp_Object minimum) {
    ...
    if (mincol < XFIXNUM(column))
        mincol = XFIXNUM(column);
...
static ptrdiff_t position_indentation(ptrdiff_t pos_byte) {
...
bool indented_beyond_p(ptrdiff_t pos, ptrdiff_t pos_byte, EMACS_INT column) {
    while (pos > BEGV && FETCH_BYTE(pos_byte) == '\n') {
        dec_both(&pos, &pos_byte);
        pos = find_newline(pos, pos_byte, BEGV, BEGV_BYTE, -1, NULL, &pos_byte,
                           0);
    }
...
struct position* compute_motion(ptrdiff_t from, ptrdiff_t frombyte,
    ...
    EMACS_INT selective =
        (FIXNUMP(BVAR(current_buffer, selective_display))
             ...
             : !NILP(BVAR(current_buffer, selective_display)) ? -1
                                                              : 0);
    ...
    if (dp == buffer_display_table()) {
        width_table =
            (VECTORP(BVAR(current_buffer, width_table))
    ...
    while (true) {
        ...
        while (pos == next_boundary) {
            ...
            if (vpos > tovpos || (vpos == tovpos && hpos >= tohpos)) {
                if (contin_hpos && prev_hpos == 0 && hpos > tohpos &&
                    (contin_hpos == width ||
                     wide_column_end_hpos >
                         width)) { /* Line breaks because we can't put the
                                      character at the previous line any more.
                                      It is not the multi-column character
                                      continued in middle.  Go back to previous
                                      buffer position, screen position, and set
                                      tab offset to previous value.  It's the
               ...
               to be changed here.  */
            {
                ...
                hpos +=
                    ((multibyte && ovlen > 0) ? strwidth((char*)ovstr, ovlen)
                                              : ovlen);
            }
            did_motion = 0;
            ...
            if (newpos > cmp_it.stop_pos && cmp_it.id < 0)
                composition_reseat_it(&cmp_it, pos, pos_byte, to, win, -1, NULL,
                                      Qnil);
       ...
       And we find the continuation AFTER it occurs.
        ...
        if (pos > to) {
            ...
            if (contin_hpos && prev_hpos == 0 && contin_hpos < width &&
                !wide_column_end_hpos) {
        ...
        if (vpos > tovpos || (vpos == tovpos && hpos >= tohpos)) {
            if (contin_hpos && prev_hpos == 0 && hpos > tohpos &&
                (contin_hpos == width ||
                 wide_column_end_hpos >
                     width)) { /* Line breaks because we can't put the character
                              at the previous line any more.  It is not the
                              multi-column character continued in middle.  Go
                              back to previous buffer position, screen position,
                              and set tab offset to previous value.  It's the
                              beginning of the line.  */
                pos = prev_pos;
                pos_byte = prev_pos_byte;
                hpos = prev_hpos;
        ...
        /* We have to scan the text character-by-character.  */
            ...
            if (cmp_it.id >= 0 ||
                (pos == cmp_it.stop_pos &&
                 composition_reseat_it(&cmp_it, pos, pos_byte, to, win, -1,
                                       NULL, Qnil)))
            ...
            if (width_cache) {
               ...
               different position, or a different width.  */
                    ...
                    if (width_run_start < width_run_end && width_run_width == 1)
...
after_loop:
    ...
    if (width_cache && width_run_width == 1 && width_run_start < width_run_end)
        know_region_cache(cache_buffer, width_cache, width_run_start,
                          width_run_end);
...
(Lisp_Object from, Lisp_Object frompos, Lisp_Object to, Lisp_Object topos,
 Lisp_Object width, Lisp_Object offsets, Lisp_Object window) {
    ...
    if (!NILP(offsets)) {
        ...
        if (!(0 <= XFIXNUM(XCAR(offsets)) &&
              XFIXNUM(XCAR(offsets)) <= PTRDIFF_MAX &&
    ...
    pos = compute_motion(
        XFIXNUM(from), CHAR_TO_BYTE(XFIXNUM(from)), XFIXNUM(XCDR(frompos)),
        XFIXNUM(XCAR(frompos)), 0, XFIXNUM(to),
        (NILP(topos) ? window_internal_height(w) : XFIXNUM(XCDR(topos))),
        (NILP(topos) ? (window_body_width(w, WINDOW_BODY_IN_CANONICAL_CHARS) -
                        (
#ifdef HAVE_WINDOW_SYSTEM
                            FRAME_WINDOW_P(XFRAME(w->frame)) ? 0 :
                                                             ...
                                                             1))
                     : XFIXNUM(XCAR(topos))),
        (NILP(width) ? -1 : XFIXNUM(width)), hscroll, tab_offset, w);
...
struct position* vmotion(ptrdiff_t from, ptrdiff_t from_byte, EMACS_INT vtarget,
            ...
            while (prevline > BEGV &&
                   ...
                   When moving upward, check the newline before.  */
                    ...
                    (propval = Fget_char_property(make_fixnum(prevline - 1),
                                                  Qinvisible, text_prop_object),
                     TEXT_PROP_MEANS_INVISIBLE(propval)))) {
                dec_both(&prevline, &bytepos);
                prevline =
                    find_newline_no_quit(prevline, bytepos, -1, &bytepos);
            }
            pos = *compute_motion(prevline, bytepos, 0, lmargin, 0, from,
                                  /* Don't care for VPOS...  */
                                  1 << (SHRT_WIDTH - 1),
                                  /* ... nor HPOS.  */
                                  1 << (SHRT_WIDTH - 1), -1, hscroll, 0, w);
            vpos -= pos.vpos;
            first = 0;
            from = prevline;
            from_byte = bytepos;
    ...
    if (from > BEGV && FETCH_BYTE(from_byte - 1) != '\n') {
        ...
        while (
            prevline > BEGV &&
            ((selective > 0 && indented_beyond_p(prevline, bytepos, selective))
             /* Watch out for newlines with `invisible' property.
                When moving downward, check the newline after.  */
             || (propval = Fget_char_property(make_fixnum(prevline), Qinvisible,
                                              text_prop_object),
                 TEXT_PROP_MEANS_INVISIBLE(propval)))) {
            dec_both(&prevline, &bytepos);
            prevline = find_newline_no_quit(prevline, bytepos, -1, &bytepos);
    ...
    return compute_motion(from, from_byte, vpos, pos.hpos, did_motion, ZV,
                          vtarget, -(1 << (SHRT_WIDTH - 1)), -1, hscroll, 0, w);
}
...
static int window_column_x(struct window* w, Lisp_Object window, double col,
                           Lisp_Object column) {
...
static void restore_window_buffer(Lisp_Object list) {
    struct window* w = decode_live_window(XCAR(list));
    list = XCDR(list);
    wset_buffer(w, XCAR(list));
    list = XCDR(list);
    set_marker_both(w->pointm, w->contents, XFIXNAT(XCAR(list)),
...
(Lisp_Object lines, Lisp_Object window, Lisp_Object cur_col) {
    ...
    if (XBUFFER(w->contents) != current_buffer) {
        ...
        Lisp_Object old =
            list4(window, w->contents, make_fixnum(marker_position(w->pointm)),
                  make_fixnum(marker_byte_position(w->pointm)));
        record_unwind_protect(restore_window_buffer, old);
        wset_buffer(w, Fcurrent_buffer());
        set_marker_both(w->pointm, w->contents, BUF_PT(current_buffer),
       ...
       backtrack.  */
        if (IT_CHARPOS(it) > it_start) {
            /* We need to backtrack also if the Lisp string contains no
               ...
               before the newline.  */
            if (it_overshoot_count < 0 && it.method == GET_FROM_BUFFER &&
                it.c == '\n')
                it_overshoot_count = 1;
        ...
        else if (IT_CHARPOS(it) == PT - 1 && FETCH_BYTE(PT_BYTE - 1) == '\n' &&
                 nlines <= 0) {
