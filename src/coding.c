...
#define CODING_ISO_INITIAL(coding, reg)                                        \
  XFIXNUM(                                                                     \
      AREF(AREF(CODING_ID_ATTRS((coding)->id), coding_attr_iso_initial), reg))
...
#define CATEGORY_MASK_ANY                                                      \
  (CATEGORY_MASK_ISO_7 | CATEGORY_MASK_ISO_7_TIGHT | CATEGORY_MASK_ISO_8_1 |   \
   CATEGORY_MASK_ISO_8_2 | CATEGORY_MASK_ISO_7_ELSE |                          \
   CATEGORY_MASK_ISO_8_ELSE | CATEGORY_MASK_UTF_8_AUTO |                       \
   CATEGORY_MASK_UTF_8_NOSIG | CATEGORY_MASK_UTF_8_SIG |                       \
   CATEGORY_MASK_UTF_16_AUTO | CATEGORY_MASK_UTF_16_BE |                       \
   CATEGORY_MASK_UTF_16_LE | CATEGORY_MASK_UTF_16_BE_NOSIG |                   \
   CATEGORY_MASK_UTF_16_LE_NOSIG | CATEGORY_MASK_CHARSET |                     \
   CATEGORY_MASK_SJIS | CATEGORY_MASK_BIG5 | CATEGORY_MASK_CCL |               \
   CATEGORY_MASK_EMACS_MULE)


#define CATEGORY_MASK_ISO_7BIT (CATEGORY_MASK_ISO_7 | CATEGORY_MASK_ISO_7_TIGHT)

#define CATEGORY_MASK_ISO_8BIT (CATEGORY_MASK_ISO_8_1 | CATEGORY_MASK_ISO_8_2)
...
#define CATEGORY_MASK_ISO                                                      \
  (CATEGORY_MASK_ISO_7BIT | CATEGORY_MASK_ISO_8BIT | CATEGORY_MASK_ISO_ELSE)
...
static int encode_inhibit_flag(Lisp_Object flag)
{
...
static bool inhibit_flag(int encoded_flag, bool var)
{
...
static bool growable_destination(struct coding_system* coding)
{
  ...
  {                                                                            \
    ...
    {                                                                          \
      ...
      {                                                                        \
        ...
        record_conversion_result(coding, CODING_RESULT_INVALID_SRC);           \
      }                                                                        \
    }                                                                          \
    consumed_chars++;                                                          \
  }                                                                            \
...
#define CODING_DECODE_CHAR(coding, src, src_base, src_end, charset, code, c)   \
  ...
  {                                                                            \
    ...
    if (charset_map_loaded && (offset = coding_change_source(coding)))         \
    {                                                                          \
      src += offset;                                                           \
      src_base += offset;                                                      \
      src_end += offset;                                                       \
...
#define CODING_ENCODE_CHAR(coding, dst, dst_end, charset, c, code)             \
  ...
  {                                                                            \
    ...
    if (charset_map_loaded && (offset = coding_change_destination(coding)))    \
    {                                                                          \
      dst += offset;                                                           \
      dst_end += offset;                                                       \
    }                                                                          \
...
static void coding_set_source(struct coding_system* coding)
{
...
static ptrdiff_t coding_change_source(struct coding_system* coding)
{
...
static void coding_set_destination(struct coding_system* coding)
{
  if (BUFFERP(coding->dst_object))
  {
    if (BUFFERP(coding->src_object) && coding->src_pos < 0)
    {
      ...
      coding->dst_bytes =
          (GAP_END_ADDR - (coding->src_bytes - coding->consumed) -
           coding->destination);
    }
    else
    {
      ...
      coding->dst_bytes =
          (BUF_GAP_END_ADDR(XBUFFER(coding->dst_object)) - coding->destination);
    }
  }
  else
  {
...
static ptrdiff_t coding_change_destination(struct coding_system* coding)
{
...
static void coding_alloc_by_realloc(struct coding_system* coding,
                                    ptrdiff_t bytes)
{
  ptrdiff_t newbytes;
  if (ckd_add(&newbytes, coding->dst_bytes, bytes) || SIZE_MAX < newbytes)
    string_overflow();
  coding->destination = xrealloc(coding->destination, newbytes);
  coding->dst_bytes = newbytes;
}

static void coding_alloc_by_making_gap(struct coding_system* coding,
...
static unsigned char* alloc_destination(struct coding_system* coding,
                                        ptrdiff_t nbytes, unsigned char* dst)
{
...
static bool detect_coding_utf_8(struct coding_system* coding,
                                struct coding_detection_info* detect_info)
{
  ...
  if (src == coding->source /* BOM should be at the head.  */
      ...
      && src[0] == UTF_8_BOM_1 && src[1] == UTF_8_BOM_2 &&
      src[2] == UTF_8_BOM_3)
  {
    bom_found = 1;
    src += 3;
...
no_more_source:
  ...
  if (bom_found)
  {
    ...
    detect_info->found |= CATEGORY_MASK_UTF_8_AUTO | CATEGORY_MASK_UTF_8_SIG |
...
static void decode_coding_utf_8(struct coding_system* coding)
{
  const unsigned char* src = coding->source + coding->consumed;
  const unsigned char* src_end = coding->source + coding->src_bytes;
  const unsigned char* src_base;
  ...
  if (bom != utf_without_bom)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          if ((c1 != UTF_8_BOM_1) || (c2 != UTF_8_BOM_2) || (c3 != UTF_8_BOM_3))
            src = src_base;
          else
  ...
  while (1)
  {
    ...
    if (multibytep && !eol_dos && charbuf < charbuf_end - 6 &&
        src < src_end - 6)
    {
     ...
     and they mess up read/write invariance.  */
      ...
      else
      {
        ...
        else
        {
          ...
          {
            c = (((c1 & 0x7) << 18) | ((c2 & 0x3F) << 12) | ((c3 & 0x3F) << 6) |
                 (c4 & 0x3F));
            if (c < 0x10000)
              goto invalid_code;
          }
...
bool encode_coding_utf_8(struct coding_system* coding)
{
  bool multibytep = coding->dst_multibyte;
  int* charbuf = coding->charbuf;
...
static bool detect_coding_utf_16(struct coding_system* coding,
                                 struct coding_detection_info* detect_info)
{
  const unsigned char* src = coding->source;
  const unsigned char* src_end = coding->source + coding->src_bytes;
  ...
  if (coding->mode & CODING_MODE_LAST_BLOCK && (coding->src_chars & 1))
  {
  ...
  if ((c1 == 0xFF) && (c2 == 0xFE))
  {
    detect_info->found |= (CATEGORY_MASK_UTF_16_LE | CATEGORY_MASK_UTF_16_AUTO);
    detect_info->rejected |=
        (CATEGORY_MASK_UTF_16_BE | CATEGORY_MASK_UTF_16_BE_NOSIG |
         CATEGORY_MASK_UTF_16_LE_NOSIG);
  }
  else if ((c1 == 0xFE) && (c2 == 0xFF))
  {
    detect_info->found |= (CATEGORY_MASK_UTF_16_BE | CATEGORY_MASK_UTF_16_AUTO);
    detect_info->rejected |=
        (CATEGORY_MASK_UTF_16_LE | CATEGORY_MASK_UTF_16_BE_NOSIG |
         CATEGORY_MASK_UTF_16_LE_NOSIG);
  }
...
static void decode_coding_utf_16(struct coding_system* coding)
{
  const unsigned char* src = coding->source + coding->consumed;
  const unsigned char* src_end = coding->source + coding->src_bytes;
  const unsigned char* src_base;
  ...
  if (bom == utf_with_bom)
  {
    ...
    if (endian == utf_16_big_endian ? c != 0xFEFF : c != 0xFFFE)
    {
 ...
 detect_coding.  */
  ...
  while (1)
  {
    ...
    c = (endian == utf_16_big_endian ? ((c1 << 8) | c2) : ((c2 << 8) | c1));
...
static bool encode_coding_utf_16(struct coding_system* coding)
{
  bool multibytep = coding->dst_multibyte;
  int* charbuf = coding->charbuf;
  int* charbuf_end = charbuf + coding->charbuf_used;
...
static bool detect_coding_emacs_mule(struct coding_system* coding,
                                     struct coding_detection_info* detect_info)
{
  const unsigned char *src = coding->source, *src_base;
  const unsigned char* src_end = coding->source + coding->src_bytes;
...
static int emacs_mule_char(struct coding_system* coding,
                           const unsigned char* src, int* nbytes, int* nchars,
                           int* id, struct composition_status* cmp_status)
{
  const unsigned char* src_end = coding->source + coding->src_bytes;
  ...
  else
  {
    if (c >= 0xA0)
    {
      if (cmp_status->state != COMPOSING_NO && cmp_status->old_form)
      {
...
static int emacs_mule_finish_composition(int* charbuf,
...
static void decode_coding_emacs_mule(struct coding_system* coding)
{
  const unsigned char* src = coding->source + coding->consumed;
  const unsigned char* src_end = coding->source + coding->src_bytes;
  const unsigned char* src_base;
  ...
  while (1)
  {
    ...
    else
    {
      ...
      c = emacs_mule_char(coding, src_base, &nbytes, &nchars, &id, cmp_status);
...
static bool encode_coding_emacs_mule(struct coding_system* coding)
{
  bool multibytep = coding->dst_multibyte;
  int* charbuf = coding->charbuf;
  int* charbuf_end = charbuf + coding->charbuf_used;
    ...
    if (c < 0)
    {
      ...
      case CODING_ANNOTATE_CHARSET_MASK:
        ...
        if (preferred_charset_id >= 0 &&
            NILP(Fmemq(make_fixnum(preferred_charset_id), charset_list)))
    ...
    else
    {
      ...
      if (preferred_charset_id >= 0)
      {
        ...
        if (result)
          ...
          CODING_CHAR_CHARSET(coding, dst, dst_end, c, charset_list, &code,
                              charset);
      }
      else
        CODING_CHAR_CHARSET(coding, dst, dst_end, c, charset_list, &code,
...
#define SAFE_CHARSET_P(coding, id)                                             \
  ((id) <= (coding)->max_charset_id && (coding)->safe_charsets[id] != 255)
...
static bool detect_coding_iso_2022(struct coding_system* coding,
                                   struct coding_detection_info* detect_info)
{
  ...
  while (rejected != CATEGORY_MASK_ISO)
  {
    ...
    case ISO_CODE_ESC:
      ...
      else
      {
        ...
        if (SAFE_CHARSET_P(&coding_categories[coding_category_iso_7], id))
          found |= CATEGORY_MASK_ISO_7;
        else
          rejected |= CATEGORY_MASK_ISO_7;
        if (SAFE_CHARSET_P(&coding_categories[coding_category_iso_7_tight], id))
          found |= CATEGORY_MASK_ISO_7_TIGHT;
        else
          rejected |= CATEGORY_MASK_ISO_7_TIGHT;
        if (SAFE_CHARSET_P(&coding_categories[coding_category_iso_7_else], id))
          found |= CATEGORY_MASK_ISO_7_ELSE;
        else
          rejected |= CATEGORY_MASK_ISO_7_ELSE;
        if (SAFE_CHARSET_P(&coding_categories[coding_category_iso_8_else], id))
    ...
    default:
      ...
      if (c >= 0xA0)
      {
        ...
        if (!single_shifting && !(rejected & CATEGORY_MASK_ISO_8_2))
        {
          ptrdiff_t len = 1;
          while (src < src_end)
          {
...
static int finish_composition(int* charbuf,
                              struct composition_status* cmp_status)
{
  int idx = -cmp_status->length;
  int new_chars;
  ...
  charbuf[idx++] =
      (cmp_status->method == COMPOSITION_RELATIVE        ? '0'
...
static void decode_coding_iso_2022(struct coding_system* coding)
{
  const unsigned char* src = coding->source + coding->consumed;
  const unsigned char* src_end = coding->source + coding->src_bytes;
  const unsigned char* src_base;
  ...
  while (1)
  {
    ...
    if (cmp_status->state == COMPOSING_NO && charset->id != charset_ascii &&
        last_id != charset->id)
    {
      if (last_id != charset_ascii)
        ADD_CHARSET_DATA(charbuf, char_offset - last_offset, last_id);
    ...
    else if (cmp_status->state == COMPOSING_NO)
    {
      *charbuf++ = c;
      char_offset++;
                  ...
                  ? cmp_status->nchars
                  : cmp_status->ncomps) >= MAX_COMPOSITION_COMPONENTS)
    {
...
encode_invocation_designation(struct charset* charset,
                              struct coding_system* coding, unsigned char* dst,
                              ptrdiff_t* p_nchars)
{
  bool multibytep = coding->dst_multibyte;
...
static ptrdiff_t encode_designation_at_bol(struct coding_system* coding,
                                           ...
                                           unsigned char* dst)
{
  ...
  {
    for (reg = 0; reg < 4; reg++)
      if (r[reg] >= 0 && CODING_ISO_DESIGNATION(coding, reg) != r[reg])
...
static bool encode_coding_iso_2022(struct coding_system* coding)
{
  bool multibytep = coding->dst_multibyte;
  int* charbuf = coding->charbuf;
  int* charbuf_end = charbuf + coding->charbuf_used;
  ...
  ascii_compatible =
      (!NILP(CODING_ATTR_ASCII_COMPAT(attrs)) &&
       !(CODING_ISO_FLAGS(coding) &
         (CODING_ISO_FLAG_DESIGNATION | CODING_ISO_FLAG_LOCKING_SHIFT)));
  ...
  while (charbuf < charbuf_end)
  {
    ...
    if (bol_designation)
    {
      ...
      charset_map_loaded = 0;
      nbytes =
          encode_designation_at_bol(coding, charbuf, charbuf_end, desig_buf);
      if (charset_map_loaded && (offset = coding_change_destination(coding)))
      {
        dst += offset;
        dst_end += offset;
    ...
    if (c < 0x20 || c == 0x7F)
    {
      if (c == '\n' || (c == '\r' && EQ(eol_type, Qmac)))
      {
        ...
        bol_designation = ((CODING_ISO_FLAGS(coding) &
                            CODING_ISO_FLAG_DESIGNATE_AT_BOL) != 0);
      }
    ...
    else
    {
      ...
      if (preferred_charset_id >= 0)
      {
        ...
        CODING_CHAR_CHARSET_P(coding, dst, dst_end, c, charset, result);
        if (!result)
          CODING_CHAR_CHARSET(coding, dst, dst_end, c, charset_list, NULL,
                              charset);
      }
      else
        CODING_CHAR_CHARSET(coding, dst, dst_end, c, charset_list, NULL,
...
static bool detect_coding_sjis(struct coding_system* coding,
                               struct coding_detection_info* detect_info)
{
  const unsigned char *src = coding->source, *src_base;
  const unsigned char* src_end = coding->source + coding->src_bytes;
...
static bool detect_coding_big5(struct coding_system* coding,
                               struct coding_detection_info* detect_info)
{
  const unsigned char *src = coding->source, *src_base;
  const unsigned char* src_end = coding->source + coding->src_bytes;
...
static void decode_coding_sjis(struct coding_system* coding)
{
  const unsigned char* src = coding->source + coding->consumed;
  const unsigned char* src_end = coding->source + coding->src_bytes;
  const unsigned char* src_base;
...
static void decode_coding_big5(struct coding_system* coding)
{
  const unsigned char* src = coding->source + coding->consumed;
  const unsigned char* src_end = coding->source + coding->src_bytes;
  const unsigned char* src_base;
...
static bool encode_coding_sjis(struct coding_system* coding)
{
  bool multibytep = coding->dst_multibyte;
  int* charbuf = coding->charbuf;
  int* charbuf_end = charbuf + coding->charbuf_used;
  ...
  while (charbuf < charbuf_end)
  {
    ...
    else
    {
      ...
      else if (charset_kanji2 && charset == charset_kanji2)
      {
        ...
        c1 = code >> 8;
        if (c1 == 0x21 || (c1 >= 0x23 && c1 <= 0x25) || c1 == 0x28 ||
...
static bool encode_coding_big5(struct coding_system* coding)
{
  bool multibytep = coding->dst_multibyte;
  int* charbuf = coding->charbuf;
  int* charbuf_end = charbuf + coding->charbuf_used;
...
static bool detect_coding_ccl(struct coding_system* coding,
                              struct coding_detection_info* detect_info)
{
  const unsigned char *src = coding->source, *src_base;
  const unsigned char* src_end = coding->source + coding->src_bytes;
...
static void decode_coding_ccl(struct coding_system* coding)
{
  const unsigned char* src = coding->source + coding->consumed;
  const unsigned char* src_end = coding->source + coding->src_bytes;
  int* charbuf = coding->charbuf + coding->charbuf_used;
...
static bool encode_coding_ccl(struct coding_system* coding)
{
  struct ccl_program* ccl = &coding->spec.ccl->ccl;
  bool multibytep = coding->dst_multibyte;
  int* charbuf = coding->charbuf;
  ...
  do
  {
    ...
    /* As ccl_driver calls DECODE_CHAR, buffer may be relocated.  */
    charset_map_loaded = 0;
    ccl_driver(ccl, charbuf, destination_charbuf, charbuf_end - charbuf, 1024,
               charset_list);
    if (charset_map_loaded && (offset = coding_change_destination(coding)))
      dst += offset;
    if (multibytep)
    ...
    if (ccl->status == CCL_STAT_QUIT || ccl->status == CCL_STAT_INVALID_CMD)
      break;
  }
  while (charbuf < charbuf_end);
...
static void decode_coding_raw_text(struct coding_system* coding)
{
  bool eol_dos =
      !inhibit_eol_conversion && EQ(CODING_ID_EOL_TYPE(coding->id), Qdos);
  ...
  if (eol_dos && coding->src_bytes > 0 /* empty source text? */
...
static bool encode_coding_raw_text(struct coding_system* coding)
{
  bool multibytep = coding->dst_multibyte;
  int* charbuf = coding->charbuf;
...
static bool detect_coding_charset(struct coding_system* coding,
                                  struct coding_detection_info* detect_info)
{
  const unsigned char *src = coding->source, *src_base;
  const unsigned char* src_end = coding->source + coding->src_bytes;
  ...
  if (strncmp(SSDATA(SYMBOL_NAME(name)), "iso-8859-",
              sizeof("iso-8859-") - 1) == 0 ||
      strncmp(SSDATA(SYMBOL_NAME(name)), "iso-latin-",
              sizeof("iso-latin-") - 1) == 0)
    check_latin_extra = 1;
...
static void decode_coding_charset(struct coding_system* coding)
{
  const unsigned char* src = coding->source + coding->consumed;
  const unsigned char* src_end = coding->source + coding->src_bytes;
  const unsigned char* src_base;
...
static bool encode_coding_charset(struct coding_system* coding)
{
  bool multibytep = coding->dst_multibyte;
  int* charbuf = coding->charbuf;
  int* charbuf_end = charbuf + coding->charbuf_used;
    ...
    else
    {
      ...
      if (charset)
      {
        if (CHARSET_DIMENSION(charset) == 1)
          ...
          EMIT_FOUR_BYTES(code >> 24, (code >> 16) & 0xFF, (code >> 8) & 0xFF,
                          code & 0xFF);
      }
      else
      {
...
void setup_coding_system(Lisp_Object coding_system,
                         struct coding_system* coding)
{
  ...
  else if (EQ(coding_type, Qemacs_mule))
  {
        ...
        !EQ(CODING_ATTR_CHARSET_LIST(attrs), Vemacs_mule_charset_list))
    {
      ...
      for (tail = Vemacs_mule_charset_list; CONSP(tail); tail = XCDR(tail))
...
Lisp_Object coding_charset_list(struct coding_system* coding)
{
  Lisp_Object attrs, charset_list;
...
Lisp_Object coding_system_charset_list(Lisp_Object coding_system)
{
  ptrdiff_t id;
  Lisp_Object attrs, charset_list;
...
Lisp_Object raw_text_coding_system(Lisp_Object coding_system)
{
...
bool raw_text_coding_system_p(struct coding_system* coding)
{
...
Lisp_Object coding_inherit_eol_type(Lisp_Object coding_system,
                                    Lisp_Object parent)
{
...
Lisp_Object complement_process_encoding_system(Lisp_Object coding_system)
{
...
static ptrdiff_t check_ascii(struct coding_system* coding)
{
  ...
  if (inhibit_eol_conversion || SYMBOLP(eol_type))
  {
...
static ptrdiff_t check_utf_8(struct coding_system* coding)
{
  const unsigned char *src, *end;
  int eol_seen;
  ptrdiff_t nchars = coding->head_ascii;
  ...
  while (src < end)
  {
    ...
    else if (UTF_8_2_OCTET_LEADING_P(c))
    {
      if (c < 0xC2 /* overlong sequence */
          || src + 1 >= end || !UTF_8_EXTRA_OCTET_P(src[1]))
        return -1;
      src += 2;
    }
    ...
    {
      if (src + 2 >= end ||
          !(UTF_8_EXTRA_OCTET_P(src[1]) && UTF_8_EXTRA_OCTET_P(src[2])))
        return -1;
      c = (((c & 0xF) << 12) | ((src[1] & 0x3F) << 6) | (src[2] & 0x3F));
...
Lisp_Object make_string_from_utf8(const char* text, ptrdiff_t nbytes)
{
#if 0
...
static int detect_eol(const unsigned char* source, ptrdiff_t src_bytes,
                      enum coding_category category)
{
  ...
  {
    bool msb = category ==
        (coding_category_utf_16_le | coding_category_utf_16_le_nosig);
    bool lsb = !msb;
    ...
    while (src + 1 < src_end)
    {
      ...
      if (src[msb] == 0 && (c == '\n' || c == '\r'))
      {
        ...
        else if (src + 3 >= src_end || src[msb + 2] != 0 ||
                 src[lsb + 2] != '\n')
...
static void detect_coding(struct coding_system* coding)
{
  ...
  if (EQ(CODING_ATTR_TYPE(CODING_ID_ATTRS(coding->id)), Qundecided))
  {
    ...
    for (src = coding->source; src < src_end; src++)
    {
      ...
      else if (c < 0x20)
      {
        if ((c == ISO_CODE_ESC || c == ISO_CODE_SI || c == ISO_CODE_SO) &&
            !inhibit_ied && !detect_info.checked)
        {
        ...
        else if (!disable_ascii_optimization && !inhibit_eol_conversion)
        {
          if (c == '\r')
          {
    ...
    if (null_byte_found || eight_bit_found ||
        coding->head_ascii < coding->src_bytes || detect_info.found)
    {
      enum coding_category category;
      ...
      else
      {
        ...
        else if (prefer_utf_8 && detect_coding_utf_8(coding, &detect_info))
        {
          detect_info.checked |= ~CATEGORY_MASK_UTF_8;
          detect_info.rejected |= ~CATEGORY_MASK_UTF_8;
        }
      ...
      if (i < coding_category_raw_text)
      {
        if (category == coding_category_utf_8_auto)
        {
          ...
          coding_systems = AREF(CODING_ID_ATTRS(this->id), coding_attr_utf_bom);
      ...
      else if ((detect_info.rejected & CATEGORY_MASK_ANY) == CATEGORY_MASK_ANY)
        found = Qraw_text;
           ...
           coding_category_utf_8_auto)
  {
    ...
    {
      struct coding_detection_info detect_info = {0};
      if (CONSP(coding_systems) && detect_coding_utf_8(coding, &detect_info))
      {
        if (detect_info.found & CATEGORY_MASK_UTF_8_SIG)
          found = XCAR(coding_systems);
        else
...
static void decode_eol(struct coding_system* coding)
{
  Lisp_Object eol_type;
...
enum
{
  MAX_LOOKUP_MAX = min(INT_MAX, MAX_ALLOCA / sizeof(int))
};
...
static Lisp_Object get_translation_table(Lisp_Object attrs, bool encodep,
                                         int* max_lookup)
{
...
static Lisp_Object get_translation(Lisp_Object trans, int* buf, int* buf_end,
                                   ptrdiff_t* nchars)
{
...
static int produce_chars(struct coding_system* coding,
                         Lisp_Object translation_table, bool last_block)
{
  unsigned char* dst = coding->destination + coding->produced;
  unsigned char* dst_end = coding->destination + coding->dst_bytes;
  ptrdiff_t produced;
  ...
  if (!coding->chars_at_source)
  {
    ...
    if (EQ(coding->src_object, coding->dst_object) && !NILP(coding->dst_object))
    {
    ...
    while (buf < buf_end)
    {
      ...
      if (c >= 0)
      {
        ...
        for (i = 0; i < to_nchars; i++)
        {
          ...
          if (coding->dst_multibyte || !CHAR_BYTE8_P(c))
            CHAR_STRING_ADVANCE_NO_UNIFY(c, dst);
          else
            *dst++ = CHAR_TO_BYTE8(c);
        }
  ...
  if (BUFFERP(coding->dst_object) && produced_chars > 0)
    insert_from_gap(produced_chars, produced, 0, coding->insert_before_markers);
...
static void produce_composition(struct coding_system* coding, int* charbuf,
                                ptrdiff_t pos)
{
  int len;
  ptrdiff_t to;
...
static void produce_charset(struct coding_system* coding, int* charbuf,
                            ptrdiff_t pos)
{
  ptrdiff_t from = pos - charbuf[2];
  struct charset* charset = CHARSET_FROM_ID(charbuf[3]);

  Fput_text_property(make_fixnum(from), make_fixnum(pos), Qcharset,
                     CHARSET_NAME(charset), coding->dst_object);
}
...
#define ALLOC_CONVERSION_WORK_AREA(coding, size)                               \
  do                                                                           \
  {                                                                            \
    ptrdiff_t units = min((size) + MAX_CHARBUF_EXTRA_SIZE, MAX_CHARBUF_SIZE);  \
    coding->charbuf = SAFE_ALLOCA(units * sizeof(int));                        \
    coding->charbuf_size = units;                                              \
  }                                                                            \
  while (0)

static void produce_annotation(struct coding_system* coding, ptrdiff_t pos)
{
  int* charbuf = coding->charbuf;
  int* charbuf_end = charbuf + coding->charbuf_used;
...
static void decode_coding(struct coding_system* coding)
{
  Lisp_Object attrs;
  ...
  if (BUFFERP(coding->src_object) && coding->src_pos > 0 &&
      coding->src_pos < GPT && coding->src_pos + coding->src_chars > GPT)
 ...
 record_first_change if needed.  */
  ...
  if (coding->consumed < coding->src_bytes)
  {
    ...
    if (coding->mode & CODING_MODE_LAST_BLOCK)
    {
     ...
     text as single characters.  */
        if (coding->src_multibyte && CHAR_BYTE8_HEAD_P(*src) && nbytes > 0)
        {
          c = string_char_advance(&src);
          nbytes--;
        }
  ...
  if (!EQ(CODING_ID_EOL_TYPE(coding->id), Qunix) && !inhibit_eol_conversion)
    decode_eol(coding);
  if (BUFFERP(coding->dst_object))
  {
...
static int* handle_composition_annotation(ptrdiff_t pos, ptrdiff_t limit,
                                          struct coding_system* coding,
                                          int* buf, ptrdiff_t* stop)
{
  ptrdiff_t start, end;
  Lisp_Object prop;
  ...
  else
  {
    if (start == pos)
    {
      ...
      if (method != COMPOSITION_RELATIVE)
      {
        ...
        else if (STRINGP(components))
        {
          ...
          i = i_byte = 0;
          while (i < len)
            *buf++ = fetch_string_char_advance(components, &i, &i_byte);
        }
        ...
        {
          for (len = 0; CONSP(components); len++, components = XCDR(components))
    ...
    if (find_composition(end, limit, &start, &end, &prop, coding->src_object) &&
        end <= limit)
      *stop = start;
    else
      *stop = limit;
...
static int* handle_charset_annotation(ptrdiff_t pos, ptrdiff_t limit,
                                      ...
                                      ptrdiff_t* stop)
{
  ...
  next = Fnext_single_property_change(make_fixnum(pos), Qcharset,
                                      coding->src_object, make_fixnum(limit));
  *stop = XFIXNUM(next);
  return buf;
}


static void consume_chars(struct coding_system* coding,
                          Lisp_Object translation_table, int max_lookup)
{
  int* buf = coding->charbuf;
  ...
  while (buf < buf_end)
  {
    ...
    if (pos == stop)
    {
      ...
      if (pos == stop_composition)
        buf = handle_composition_annotation(pos, end_pos, coding, buf,
                                            &stop_composition);
      if (pos == stop_charset)
        buf =
            handle_charset_annotation(pos, end_pos, coding, buf, &stop_charset);
    ...
    else
    {
      ...
      trans = get_translation(trans, lookup_buf, lookup_buf_end, &from_nchars);
      if (FIXNUMP(trans))
        c = XFIXNUM(trans);
      else if (VECTORP(trans))
      {
...
static void encode_coding(struct coding_system* coding)
{
  Lisp_Object attrs;
  Lisp_Object translation_table;
  int max_lookup;
...
static void code_conversion_restore(Lisp_Object arg)
{
...
Lisp_Object code_conversion_save(bool with_work_buf, bool multibyte)
{
...
static void coding_restore_undo_list(Lisp_Object arg)
{
...
void decode_coding_gap(struct coding_system* coding, ptrdiff_t bytes)
{
  specpdl_ref count = SPECPDL_INDEX();
  Lisp_Object attrs;
  ...
  attrs = CODING_ID_ATTRS(coding->id);
  if (!disable_ascii_optimization && !coding->src_multibyte &&
      !NILP(CODING_ATTR_ASCII_COMPAT(attrs)) &&
      ...
      NILP(get_translation_table(attrs, 0, NULL)))
  {
    ...
    if (chars != bytes)
    {
          ...
          coding->detected_utf8_bytes == coding->src_bytes)
      {
        ...
        if (CODING_UTF_8_BOM(coding) != utf_without_bom &&
            coding->head_ascii == 0 && coding->source[0] == UTF_8_BOM_1 &&
  ...
  if (!NILP(CODING_ATTR_POST_READ(attrs)))
  {
    ...
    val =
        calln(CODING_ATTR_POST_READ(attrs), make_fixnum(coding->produced_char));
    CHECK_FIXNAT(val);
...
void decode_coding_object(struct coding_system* coding, Lisp_Object src_object,
                          ptrdiff_t from, ptrdiff_t from_byte, ptrdiff_t to,
                          ptrdiff_t to_byte, Lisp_Object dst_object)
{
  specpdl_ref count = SPECPDL_INDEX();
  ...
  if (EQ(dst_object, Qt) ||
      (!NILP(CODING_ATTR_POST_READ(attrs)) && NILP(dst_object)))
  {
...
void encode_coding_object(struct coding_system* coding, Lisp_Object src_object,
                          ptrdiff_t from, ptrdiff_t from_byte, ptrdiff_t to,
                          ptrdiff_t to_byte, Lisp_Object dst_object)
{
  specpdl_ref count = SPECPDL_INDEX();
  ...
  if (!NILP(CODING_ATTR_PRE_WRITE(attrs)))
  {
    ...
    safe_calln(CODING_ATTR_PRE_WRITE(attrs), make_fixnum(BEG), make_fixnum(Z));
  ...
  else if (BUFFERP(src_object))
  {
    ...
    if (same_buffer)
    {
      ...
      coding->src_object =
          del_range_2(from, CHAR_TO_BYTE(from), to, CHAR_TO_BYTE(to), true);
      coding->src_pos = 0;
  ...
  if (EQ(dst_object, Qt))
  {
    ...
    {
      coding->dst_object =
          make_unibyte_string((char*)coding->destination, coding->produced);
...
Lisp_Object from_unicode(Lisp_Object str)
{
  CHECK_STRING(str);
  if (!STRING_MULTIBYTE(str) && SBYTES(str) & 1)
  {
...
Lisp_Object from_unicode_buffer(const wchar_t* wstr)
{
  ...
  words[i] = '\0';
  AUTO_STRING_WITH_LEN(str, (char*)words, (length - 1) * sizeof *words);
  return unbind_to(sa_count, from_unicode(str));
#endif
}

wchar_t* to_unicode(Lisp_Object str, Lisp_Object* buf)
{
...
(Lisp_Object prompt)
{
  ...
  do
  {
    val = Fcompleting_read(prompt, Vcoding_system_alist, Qnil, Qt, Qnil,
                           Qcoding_system_history, Qnil, Qnil);
  }
  while (SCHARS(val) == 0);
...
(Lisp_Object prompt, Lisp_Object default_coding_system)
{
  ...
  val = Fcompleting_read(prompt, Vcoding_system_alist, Qnil, Qt, Qnil,
                         Qcoding_system_history, default_coding_system, Qnil);
...
Lisp_Object detect_coding_system(const unsigned char* src, ptrdiff_t src_chars,
                                 ptrdiff_t src_bytes, bool highest,
        ...
        coding.head_ascii < coding.src_bytes || detect_info.found)
    {
      ...
      else
      {
        ...
        for (i = 0; i < coding_category_raw_text; i++)
        {
          ...
          {
            if (highest && (detect_info.found & (1 << category)))
              break;
          }
          else if ((*(this->detector))(&coding, &detect_info) && highest &&
                   (detect_info.found & (1 << category)))
          {
    ...
    if (VECTORP(eol_type))
    {
      if (detect_info.found & ~CATEGORY_MASK_UTF_16)
      {
        if (null_byte_found)
          ...
          normal_eol =
              detect_eol(coding.source, src_bytes, coding_category_raw_text);
      }
      if (detect_info.found &
          (CATEGORY_MASK_UTF_16_BE | CATEGORY_MASK_UTF_16_BE_NOSIG))
        utf_16_be_eol =
            detect_eol(coding.source, src_bytes, coding_category_utf_16_be);
      if (detect_info.found &
          (CATEGORY_MASK_UTF_16_LE | CATEGORY_MASK_UTF_16_LE_NOSIG))
        utf_16_le_eol =
            detect_eol(coding.source, src_bytes, coding_category_utf_16_le);
    }
    else
    {
      if (EQ(eol_type, Qunix))
...
(Lisp_Object start, Lisp_Object end, Lisp_Object highest)
{
  ...
  return detect_coding_system(
      BYTE_POS_ADDR(from_byte), to - from, to_byte - from_byte, !NILP(highest),
      !NILP(BVAR(current_buffer, enable_multibyte_characters)), Qnil);
}
...
(Lisp_Object string, Lisp_Object highest)
{
  ...
  return detect_coding_system(SDATA(string), SCHARS(string), SBYTES(string),
                              !NILP(highest), STRING_MULTIBYTE(string), Qnil);
}


static bool char_encodable_p(int c, Lisp_Object attrs)
{
  Lisp_Object tail;
  struct charset* charset;
  Lisp_Object translation_table;
  ...
  for (tail = CODING_ATTR_CHARSET_LIST(attrs); CONSP(tail); tail = XCDR(tail))
  {
...
(Lisp_Object start, Lisp_Object end, Lisp_Object exclude)
{
  ...
  if (STRINGP(start))
  {
    if (!STRING_MULTIBYTE(start) || SCHARS(start) == SBYTES(start))
      return Qt;
    start_byte = 0;
  ...
  for (tail = Vcoding_system_list; CONSP(tail); tail = XCDR(tail))
    if (NILP(exclude) || NILP(Fmemq(XCAR(tail), exclude)))
    {
      Lisp_Object attrs;
  ...
  while (p < pend && ASCII_CHAR_P(*p))
    p++;
  while (p < pend && ASCII_CHAR_P(*(pend - 1)))
    pend--;
 ...
 Lisp_Object count, Lisp_Object string)
{
  ...
  while (1)
  {
    ...
    c = string_char_advance(&p);
    if (!(ASCII_CHAR_P(c) && ascii_compatible) &&
        !char_charset(translate_char(translation_table, c), charset_list, NULL))
    {
...
(Lisp_Object start, Lisp_Object end, Lisp_Object coding_system_list)
{
  ...
  for (tail = coding_system_list; CONSP(tail); tail = XCDR(tail))
  {
    ...
    attrs = AREF(spec, 0);
    ASET(attrs, coding_attr_trans_tbl, get_translation_table(attrs, 1, NULL));
  ...
  for (; CONSP(tail); tail = XCDR(tail))
  {
    ...
    if (CONSP(XCDR(XCDR(elt))))
      list = Fcons(Fcons(XCAR(elt), Fnreverse(XCDR(XCDR(elt)))), list);
  }
...
static Lisp_Object code_convert_region(Lisp_Object start, Lisp_Object end,
                                       Lisp_Object coding_system,
                                       Lisp_Object dst_object, bool encodep,
                                       bool norecord)
{
  struct coding_system coding;
  ...
  return (BUFFERP(dst_object) ? make_fixnum(coding.produced_char)
                              : coding.dst_object);
}
...
bool string_ascii_p(Lisp_Object string)
{
                                ...
                                bool nocopy, bool norecord)
{
  ...
  if (EQ(dst_object, Qt))
  {
    ...
    if (!NILP(CODING_ATTR_ASCII_COMPAT(attrs)) &&
        ...
        (EQ(CODING_ID_EOL_TYPE(coding.id), Qunix) || inhibit_eol_conversion ||
...
Lisp_Object code_convert_string_norecord(Lisp_Object string,
                                         Lisp_Object coding_system,
                                         bool encodep)
{
...
static unsigned char* get_buffer_gap_address(Lisp_Object buffer,
                                             ptrdiff_t nbytes)
{
...
static unsigned char* get_char_bytes(int c, int* len)
{
...
Lisp_Object encode_string_utf_8(Lisp_Object string, Lisp_Object buffer,
     ...
     the next scan.  */
        if (num_8_bit > 0)
        {
          if (CHARACTERP(handle_8_bit))
            replace_8_bit =
                get_char_bytes(XFIXNUM(handle_8_bit), &replace_8_bit_len);
        ...
        if (num_over_4 + num_over_5 > 0)
        {
          if (CHARACTERP(handle_over_uni))
            replace_over_uni =
                get_char_bytes(XFIXNUM(handle_over_uni), &replace_over_uni_len);
...
Lisp_Object decode_string_utf_8(Lisp_Object string, const char* str,
                                ptrdiff_t str_len, Lisp_Object buffer,
                                bool nocopy, Lisp_Object handle_8_bit,
                                Lisp_Object handle_over_uni)
{
...
#define UTF_8_SEQUENCE_LENGTH(c)                                               \
  ...
  if (!change_byte_sequence && NILP(buffer))
  {
  ...
  {
    if (nocopy && (num_8_bit + num_over_4 + num_over_5) == 0 && STRINGP(string))
      return string;
    val = make_uninit_multibyte_string(outchars, outbytes);
    dst = SDATA(val);
  }
  ...
  if (change_byte_sequence)
  {
    ...
    while (p < pend)
    {
      ...
      {
        int mlen;
        for (mlen = 1; mlen < len && UTF_8_EXTRA_OCTET_P(p[mlen]); mlen++)
          ;
        if (mlen == len &&
            (len <= 3 || (len == 4 && STRING_CHAR(p) <= MAX_UNICODE_CHAR) ||
             EQ(handle_over_uni, Qt)))
        {
          p += len;
          continue;
...
DEFUN("internal-encode-string-utf-8", Finternal_encode_string_utf_8,
      Sinternal_encode_string_utf_8, 7, 7, 0, doc:/* Internal use only.*/)
 ...
 Lisp_Object encode_method, Lisp_Object count)
{
  ...
  if (!NILP(handle_8_bit) && !EQ(handle_8_bit, Qt) &&
      !EQ(handle_8_bit, Qignored) && !CHARACTERP(handle_8_bit) &&
      (!STRINGP(handle_8_bit) || STRING_MULTIBYTE(handle_8_bit)))
    return Qnil;
  if (!NILP(handle_over_uni) && !EQ(handle_over_uni, Qt) &&
      !EQ(handle_over_uni, Qignored) && !CHARACTERP(handle_over_uni) &&
  ...
  if (NILP(encode_method))
  {
    for (int i = 0; i < repeat_count; i++)
      val = encode_string_utf_8(string, buffer, !NILP(nocopy), handle_8_bit,
                                handle_over_uni);
  }
  else
  {
    for (int i = 0; i < repeat_count; i++)
      val = code_convert_string(string, Qutf_8_unix, Qnil, true, !NILP(nocopy),
                                true);
  }
  return val;
}
...
DEFUN("internal-decode-string-utf-8", Finternal_decode_string_utf_8,
      Sinternal_decode_string_utf_8, 7, 7, 0, doc:/* Internal use only.*/)
(Lisp_Object string, Lisp_Object buffer, Lisp_Object nocopy,
 Lisp_Object handle_8_bit, Lisp_Object handle_over_uni,
...
/* Encode or decode STRING using CODING_SYSTEM, with the possibility of
   ...
   Do not set Vlast_coding_system_used.  */
static Lisp_Object convert_string_nocopy(Lisp_Object string,
                                         Lisp_Object coding_system,
                                         bool encodep)
{
...
Lisp_Object decode_file_name(Lisp_Object fname)
{
#ifdef WINDOWSNT
...
static Lisp_Object encode_file_name_1(Lisp_Object fname)
{
...
Lisp_Object encode_file_name(Lisp_Object fname)
{
...
(Lisp_Object string, Lisp_Object coding_system, Lisp_Object nocopy,
 Lisp_Object buffer)
{
  return code_convert_string(string, coding_system, buffer, 1, !NILP(nocopy),
                             0);
}
...
(Lisp_Object code)
{
  ...
  if (ASCII_CHAR_P(ch) && !NILP(CODING_ATTR_ASCII_COMPAT(attrs)))
    return code;
  ...
  else
  {
    ...
    if (c1 < 0x81 || (c1 > 0x9F && c1 < 0xE0) || c1 > 0xEF || c2 < 0x40 ||
        c2 == 0x7F || c2 > 0xFC)
...
(Lisp_Object ch)
{
  ...
  if (ASCII_CHAR_P(c) && !NILP(CODING_ATTR_ASCII_COMPAT(attrs)))
    return ch;
...
(Lisp_Object code)
{
  ...
  else
  {
    EMACS_INT b1 = ch >> 8;
    int b2 = ch & 0x7F;
    if (b1 < 0xA1 || b1 > 0xFE || b2 < 0x40 || (b2 > 0x7E && b2 < 0xA1) ||
        b2 > 0xFE)
      error("Invalid code: %" pI "d", ch);
    c = ch;
    charset = charset_big5;
...
(Lisp_Object terminal)
{
  return CODING_ID_NAME(
      TERMINAL_KEYBOARD_CODING(decode_live_terminal(terminal))->id);
}
...
(ptrdiff_t nargs, Lisp_Object* args)
{
  ...
  chain =
      ((EQ(operation, Qinsert_file_contents) || EQ(operation, Qwrite_region))
  ...
  for (; CONSP(chain); chain = XCDR(chain))
  {
    ...
    elt = XCAR(chain);
    if (CONSP(elt) &&
        ((STRINGP(target) && STRINGP(XCAR(elt)) &&
...
static Lisp_Object make_subsidiaries(Lisp_Object base)
{
...
(ptrdiff_t nargs, Lisp_Object* args)
{
 ...
 charsets.  A charset of smaller dimension comes first. */
    ...
    for (Lisp_Object tail = charset_list; CONSP(tail); tail = XCDR(tail))
    {
      ...
      for (int i = charset->code_space[idx]; i <= charset->code_space[idx + 1];
           i++)
      {
  ...
  else if (EQ(coding_type, Qiso_2022))
  {
    ...
    if (i & CODING_ISO_FLAG_SEVEN_BITS)
      category =
          ((i & (CODING_ISO_FLAG_LOCKING_SHIFT | CODING_ISO_FLAG_SINGLE_SHIFT))
               ? coding_category_iso_7_else
    ...
    for (int i = 0; i < 3; i++)
    {
      ...
      if (NILP(val))
        Vcoding_system_alist =
            Fcons(Fcons(Fsymbol_name(this_name), Qnil), Vcoding_system_alist);
    }
  }
...
(Lisp_Object alias, Lisp_Object coding_system)
{
  ...
  if (VECTORP(eol_type))
  {
    ...
    for (i = 0; i < 3; i++)
      Fdefine_coding_system_alias(AREF(subsidiaries, i), AREF(eol_type, i));
  }
...
DEFUN("coding-system-plist", Fcoding_system_plist, Scoding_system_plist, 1, 1,
      0, doc:/* Return the property list of CODING-SYSTEM.  */)
(Lisp_Object coding_system)
{
  Lisp_Object spec, attrs;
...
DEFUN("coding-system-aliases", Fcoding_system_aliases, Scoding_system_aliases,
      1, 1, 0, doc:/* Return the list of aliases of CODING-SYSTEM.  */)
(Lisp_Object coding_system)
{
  Lisp_Object spec;
...
updated by `define-coding-system-alias'.  */);
  ...
  DEFVAR_LISP("coding-category-list", Vcoding_category_list, doc:
...
The default value is `select-safe-coding-system' (which see).  */);
  ...
  DEFVAR_BOOL("coding-system-require-warning", coding_system_require_warning,
              doc:
                  /* Internal use only.
...
decode text as usual.  */);
  ...
  DEFVAR_BOOL("disable-ascii-optimization", disable_ascii_optimization, doc:
                  /* If non-nil, Emacs does not optimize code decoder for ASCII
    files. Internal use only.  Remove after the experimental optimizer becomes
    stable.  */);
...
static void reset_coding_after_pdumper_load(void)
{
  ...
  for (struct coding_system* this = &coding_categories[0];
       this < &coding_categories[coding_category_max]; ++this)
  {
    int id = this->id;
    if (id >= 0)
    {
