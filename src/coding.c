...
#define CODING_ISO_INITIAL(coding, reg)                                        \
    XFIXNUM(AREF(AREF(CODING_ID_ATTRS((coding)->id), coding_attr_iso_initial), \
                 reg))
...
#define CODING_ISO_INVOKED_CHARSET(coding, plane)                              \
    (CODING_ISO_INVOCATION(coding, plane) < 0                                  \
         ...
         : CODING_ISO_DESIGNATION(coding,                                      \
                                  CODING_ISO_INVOCATION(coding, plane)))
        ...
        if (!UTF_8_3_OCTET_LEADING_P(c1))
            ...
            if (!UTF_8_EXTRA_OCTET_P(c2))
                ...
                if (!UTF_8_EXTRA_OCTET_P(c3))
                    ...
                    if ((c1 != UTF_8_BOM_1) || (c2 != UTF_8_BOM_2) ||
                        (c3 != UTF_8_BOM_3))
                        src = src_base;
                    else
...
static bool detect_coding_utf_16(struct coding_system* coding,
    ...
    if ((c1 == 0xFF) && (c2 == 0xFE)) {
        detect_info->found |=
            (CATEGORY_MASK_UTF_16_LE | CATEGORY_MASK_UTF_16_AUTO);
        ...
        detect_info->found |=
            (CATEGORY_MASK_UTF_16_BE | CATEGORY_MASK_UTF_16_AUTO);
        detect_info->rejected |=
            (CATEGORY_MASK_UTF_16_LE | CATEGORY_MASK_UTF_16_BE_NOSIG |
             CATEGORY_MASK_UTF_16_LE_NOSIG);
    }
        ...
        if (c < 0) {
            ...
            case CODING_ANNOTATE_CHARSET_MASK:
                ...
                if (preferred_charset_id >= 0 &&
                    NILP(
                        Fmemq(make_fixnum(preferred_charset_id), charset_list)))
    ...
    while (rejected != CATEGORY_MASK_ISO) {
        ...
        case ISO_CODE_ESC:
            ...
            if (c == 'N' || c == 'O') {
                ...
                if (SAFE_CHARSET_P(
                        &coding_categories[coding_category_iso_7_tight], id))
                    found |= CATEGORY_MASK_ISO_7_TIGHT;
                else
                    rejected |= CATEGORY_MASK_ISO_7_TIGHT;
                if (SAFE_CHARSET_P(
                        &coding_categories[coding_category_iso_7_else], id))
                    found |= CATEGORY_MASK_ISO_7_ELSE;
                else
                    rejected |= CATEGORY_MASK_ISO_7_ELSE;
                if (SAFE_CHARSET_P(
                        &coding_categories[coding_category_iso_8_else], id))
        ...
        label_escape_sequence:
            ...
            case '$': /* designation of 2-byte character set */
                    ...
                    goto invalid_code;
                {
                    ...
                    ONE_MORE_BYTE(c1);
                    if (c1 >= '@' &&
                        c1 <= 'B') { /* designation of JISX0208.1978,
                                      GB2312.1980, or JISX0208.1980 */
                        reg = 0, chars96 = 0;
                    }
                    else if (c1 >= 0x28 &&
                             c1 <= 0x2B) { /* designation of DIMENSION2_CHARS94
                                              character set */
                        reg = c1 - 0x28, chars96 = 0;
                        ONE_MORE_BYTE(c1);
                    }
                    else if (c1 >= 0x2C &&
                             c1 <= 0x2F) { /* designation of DIMENSION2_CHARS96
                                              character set */
            ...
            default:
                    ...
                    goto invalid_code;
                {
                    ...
                    if (c1 >= 0x28 &&
                        c1 <= 0x2B) { /* designation of DIMENSION1_CHARS94
                                         character set */
                        reg = c1 - 0x28, chars96 = 0;
                        ONE_MORE_BYTE(c1);
                    }
                    else if (c1 >= 0x2C &&
                             c1 <= 0x2F) { /* designation of DIMENSION1_CHARS96
                                              character set */
                        reg = c1 - 0x2C, chars96 = 1;
                        ONE_MORE_BYTE(c1);
                    }
...
static bool detect_coding_sjis(struct coding_system* coding,
    ...
    max_first_byte_of_2_byte_code =
        list_length(charset_list) <= 3 ? 0xEF : 0xFC;
...
static bool encode_coding_ccl(struct coding_system* coding) {
    ...
    do {
        ...
        /* As ccl_driver calls DECODE_CHAR, buffer may be relocated.  */
        charset_map_loaded = 0;
        ccl_driver(ccl, charbuf, destination_charbuf, charbuf_end - charbuf,
                   1024, charset_list);
...
static void decode_coding_charset(struct coding_system* coding) {
    ...
    while (1) {
        ...
        if (FIXNUMP(val)) {
            ...
            CODING_DECODE_CHAR(coding, src, src_base, src_end, charset, code,
                               c);
        }
        else {
            ...
            while (CONSP(val)) {
                ...
                CODING_DECODE_CHAR(coding, src, src_base, src_end, charset,
                                   code, c);
                if (c >= 0)
                    break;
                val = XCDR(val);
        ...
        if (ascii_compatible && ASCII_CHAR_P(c))
            ...
            if (charset) {
                if (CHARSET_DIMENSION(charset) == 1)
                    ...
                    EMIT_THREE_BYTES(code >> 16, (code >> 8) & 0xFF,
                                     code & 0xFF);
                else
        ...
        if (null_byte_found || eight_bit_found ||
            ...
            if (i < coding_category_raw_text) {
                if (category == coding_category_utf_8_auto) {
                    ...
                    coding_systems =
                        AREF(CODING_ID_ATTRS(this->id), coding_attr_utf_bom);
...
#define ALLOC_CONVERSION_WORK_AREA(coding, size)                               \
    do {                                                                       \
        ptrdiff_t units =                                                      \
            min((size) + MAX_CHARBUF_EXTRA_SIZE, MAX_CHARBUF_SIZE);            \
        ...
        if (coding->mode & CODING_MODE_LAST_BLOCK) {
            ...
            while (nbytes-- > 0) {
             ...
             text as single characters.  */
                if (coding->src_multibyte && CHAR_BYTE8_HEAD_P(*src) &&
                    nbytes > 0) {
                    c = string_char_advance(&src);
                    nbytes--;
                }
...
static int* handle_composition_annotation(ptrdiff_t pos, ptrdiff_t limit,
    ...
    if (!find_composition(pos, limit, &start, &end, &prop,
                          coding->src_object) ||
        end > limit)
        *stop = limit;
    else if (start > pos)
        ...
        if (start == pos) {
            ...
            if (method != COMPOSITION_RELATIVE) {
                ...
                if (VECTORP(components)) {
                    ...
                    while (i < len)
                        *buf++ =
                            fetch_string_char_advance(components, &i, &i_byte);
                }
        ...
        if (pos == stop) {
            ...
            if (pos == stop_composition)
                buf = handle_composition_annotation(pos, end_pos, coding, buf,
                                                    &stop_composition);
            if (pos == stop_charset)
                buf = handle_charset_annotation(pos, end_pos, coding, buf,
                                                &stop_charset);
            ...
            for (tail = BUF_MARKERS(current_buffer); tail; tail = tail->next)
                if (tail->need_adjustment) {
                    ...
                    if (tail->insertion_type) {
                        ...
                        tail->charpos = (NILP(BVAR(current_buffer,
...
void encode_coding_object(struct coding_system* coding, Lisp_Object src_object,
    ...
    if (!NILP(CODING_ATTR_PRE_WRITE(attrs))) {
        ...
        safe_calln(CODING_ATTR_PRE_WRITE(attrs), make_fixnum(BEG),
                   make_fixnum(Z));
        ...
        if (same_buffer) {
            ...
            coding->src_object = del_range_2(from, CHAR_TO_BYTE(from), to,
                                             CHAR_TO_BYTE(to), true);
            coding->src_pos = 0;
    ...
    if (EQ(dst_object, Qt)) {
        if (BUFFERP(coding->dst_object))
            ...
            coding->dst_object = make_unibyte_string((char*)coding->destination,
                                                     coding->produced);
                    ...
                    if (this->id < 0) {
                        ...
                        if (highest && (detect_info.found & (1 << category)))
                            break;
                             ...
                             highest && (detect_info.found & (1 << category))) {
...
(Lisp_Object start, Lisp_Object end, Lisp_Object highest) {
    ...
    return detect_coding_system(
        BYTE_POS_ADDR(from_byte), to - from, to_byte - from_byte,
        !NILP(highest),
...
static bool char_encodable_p(int c, Lisp_Object attrs) {
    ...
    for (tail = CODING_ATTR_CHARSET_LIST(attrs); CONSP(tail);
         tail = XCDR(tail)) {
...
(Lisp_Object start, Lisp_Object end, Lisp_Object exclude) {
    ...
    for (tail = coding_attrs_list; CONSP(tail); tail = XCDR(tail))
        if (!NILP(XCAR(tail)))
            safe_codings =
                Fcons(CODING_ATTR_BASE_NAME(XCAR(tail)), safe_codings);
...
(Lisp_Object start, Lisp_Object end, Lisp_Object coding_system,
 Lisp_Object count, Lisp_Object string) {
    ...
    while (1) {
        ...
        c = string_char_advance(&p);
        if (!(ASCII_CHAR_P(c) && ascii_compatible) &&
            !char_charset(translate_char(translation_table, c), charset_list,
                          NULL)) {
             ...
             the next scan.  */
                ...
                if (num_over_4 + num_over_5 > 0) {
                    if (CHARACTERP(handle_over_uni))
                        replace_over_uni = get_char_bytes(
                            XFIXNUM(handle_over_uni), &replace_over_uni_len);
...
Lisp_Object decode_file_name(Lisp_Object fname) {
    ...
    if (!NILP(Vfile_name_coding_system))
        ...
        return convert_string_nocopy(fname, Vdefault_file_name_coding_system,
                                     0);
    else
        return fname;
#endif
...
(ptrdiff_t nargs, Lisp_Object* args) {
    ...
    for (i = coding_category_max; i-- > 0;)
        Vcoding_category_list = Fcons(
            AREF(Vcoding_category_table, priorities[i]), Vcoding_category_list);
...
(ptrdiff_t nargs, Lisp_Object* args) {
    ...
    if (EQ(coding_type, Qcharset)) {
     ...
     charsets.  A charset of smaller dimension comes first. */
        ...
        category = (CONSP(bom) ? coding_category_utf_16_auto
                        : NILP(bom)
                        ? (EQ(endian, Qbig) ? coding_category_utf_16_be_nosig
                                            : coding_category_utf_16_le_nosig)
    ...
    if (NILP(eol_type)) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (NILP(val))
                Vcoding_system_alist = Fcons(
                    Fcons(Fsymbol_name(this_name), Qnil), Vcoding_system_alist);
        }
    }
...
updated by `define-coding-system-alias'.  */);
    ...
    DEFVAR_LISP(
        "coding-category-list", Vcoding_category_list, doc:
...
Coding system used in the latest file or process I/O.  */);
    ...
    DEFVAR_LISP("last-code-conversion-error", Vlast_code_conversion_error, doc:
                    /*
...
decode text as usual.  */);
    ...
    DEFVAR_BOOL(
        "disable-ascii-optimization", disable_ascii_optimization, doc:
