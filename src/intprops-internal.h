...
#define _GL_INT_OP_WRAPV(a, b, r, op, overflow)                                \
    (_Generic(*(r),                                                            \
         ...
         unsigned short int: _GL_INT_OP_CALC(a, b, r, op, overflow,            \
                                             unsigned int, unsigned short int, \
                                             0, USHRT_MAX),                    \
         int: _GL_INT_OP_CALC(a, b, r, op, overflow, unsigned int, int,        \
                              INT_MIN, INT_MAX),                               \
         ...
         unsigned long int: _GL_INT_OP_CALC(a, b, r, op, overflow,             \
                                            unsigned long int,                 \
                                            unsigned long int, 0, ULONG_MAX),  \
         long long int: _GL_INT_OP_CALC(a, b, r, op, overflow,                 \
                                        unsigned long long int, long long int, \
                                        LLONG_MIN, LLONG_MAX),                 \
         unsigned long long int: _GL_INT_OP_CALC(                              \
             a, b, r, op, overflow, unsigned long long int,                    \
...
#if _GL_HAVE___TYPEOF__
#define _GL_INT_OP_WRAPV_SMALLISH(a, b, r, op, overflow, st, smin, smax, ut,   \
                                  umax)                                        \
...
#define _GL_INT_OP_WRAPV_SMALLISH(a, b, r, op, overflow, st, smin, smax, ut,   \
    ...
    (overflow(a, b, smin, smax)                                                \
         ? (overflow(a, b, 0, umax) ? (*(r) = _GL_INT_OP_WRAPV_VIA_UNSIGNED(   \
                                           a, b, op, unsigned, st),            \
                                       1)                                      \
                                    : (*(r) = _GL_INT_OP_WRAPV_VIA_UNSIGNED(   \
                                           a, b, op, unsigned, st)) < 0)       \
         : (overflow(a, b, 0, umax) ? (*(r) = _GL_INT_OP_WRAPV_VIA_UNSIGNED(   \
                                           a, b, op, unsigned, st)) >= 0       \
                                    : (*(r) = _GL_INT_OP_WRAPV_VIA_UNSIGNED(   \
                                           a, b, op, unsigned, st),            \
...
#define _GL_INT_OP_WRAPV(a, b, r, op, overflow)                                \
    (sizeof *(r) == sizeof(signed char)                                        \
         ? _GL_INT_OP_WRAPV_SMALLISH(a, b, r, op, overflow, signed char,       \
                                     SCHAR_MIN, SCHAR_MAX, unsigned char,      \
                                     UCHAR_MAX)                                \
         : sizeof *(r) == sizeof(short int)                                    \
         ? _GL_INT_OP_WRAPV_SMALLISH(a, b, r, op, overflow, short int,         \
                                     SHRT_MIN, SHRT_MAX, unsigned short int,   \
                                     USHRT_MAX)                                \
         : sizeof *(r) == sizeof(int)                                          \
         ? (_GL_EXPR_SIGNED(*(r))                                              \
                ? _GL_INT_OP_CALC(a, b, r, op, overflow, unsigned int, int,    \
                                  INT_MIN, INT_MAX)                            \
    ...
    (sizeof *(r) == sizeof(long int)                                           \
         ? (_GL_EXPR_SIGNED(*(r))                                              \
                ...
                : _GL_INT_OP_CALC(a, b, r, op, overflow, unsigned long int,    \
                                  unsigned long int, 0, ULONG_MAX))            \
         : (_GL_EXPR_SIGNED(*(r))                                              \
                ? _GL_INT_OP_CALC(a, b, r, op, overflow,                       \
                                  unsigned long long int, long long int,       \
                                  LLONG_MIN, LLONG_MAX)                        \
                : _GL_INT_OP_CALC(a, b, r, op, overflow,                       \
...
#define _GL_INT_ADD_RANGE_OVERFLOW(a, b, tmin, tmax)                           \
    ((b) < 0                                                                   \
         ? (((tmin) ? ((_GL_EXPR_SIGNED(_GL_INT_CONVERT(a, (tmin) - (b))) ||   \
                        (b) < (tmin)) &&                                       \
                       (a) < (tmin) - (b))                                     \
                    : (a) <= -1 - (b)) ||                                      \
            ((_GL_EXPR_SIGNED(a) ? 0 <= (a) : (tmax) < (a)) &&                 \
             (tmax) < (a) + (b)))                                              \
         : (a) < 0                                                             \
         ? (((tmin) ? ((_GL_EXPR_SIGNED(_GL_INT_CONVERT(b, (tmin) - (a))) ||   \
                        (a) < (tmin)) &&                                       \
                       (b) < (tmin) - (a))                                     \
                    : (b) <= -1 - (a)) ||                                      \
...
#define _GL_INT_SUBTRACT_RANGE_OVERFLOW(a, b, tmin, tmax)                      \
    (((a) < 0) == ((b) < 0)                                                    \
         ? ((a) < (b) ? !(tmin) || -1 - (tmin) < (b) - (a) - 1                 \
                      : (tmax) < (a) - (b))                                    \
         : (a) < 0 ? ((!_GL_EXPR_SIGNED(_GL_INT_CONVERT((a) - (tmin), b)) &&   \
                       (a) - (tmin) < 0) ||                                    \
                      (a) - (tmin) < (b))                                      \
    ...
    ((b) < 0 ? ((a) < 0 ? (_GL_EXPR_SIGNED(_GL_INT_CONVERT(tmax, b))           \
                               ...
                               : ((_GL_INT_NEGATE_OVERFLOW(b)                  \
                                       ? _GL_INT_CONVERT(b, tmax) >>           \
                                           (_GL_TYPE_WIDTH(+(b)) - 1)          \
                                       : (tmax) / -(b)) <= -1 - (a)))          \
                    : _GL_INT_NEGATE_OVERFLOW(_GL_INT_CONVERT(b, tmin)) &&     \
                        (b) == -1                                              \
