...
static void byte_char_debug_check(struct buffer* b, ptrdiff_t charpos,
                                  ptrdiff_t bytepos) {
    ptrdiff_t nchars;
    ...
    if (bytepos > BUF_GPT_BYTE(b))
        ...
        nchars =
            multibyte_chars_in_text(BUF_BEG_ADDR(b), bytepos - BUF_BEG_BYTE(b));
...
#define byte_char_debug_check(b, charpos, bytepos)                             \
    do {                                                                       \
    } while (0)

#endif /* MARKER_DEBUG */

void clear_charpos_cache(struct buffer* b) {
    if (cached_buffer == b)
        cached_buffer = 0;
}
    ...
    {                                                                          \
        ...
        if (changed) {                                                         \
            if (best_above - best_below ==                                     \
                best_above_byte - best_below_byte) {                           \
...
ptrdiff_t buf_charpos_to_bytepos(struct buffer* b, ptrdiff_t charpos) {
    ...
    for (tail = BUF_MARKERS(b);
         ...
         tail &&
         !(best_above - charpos < distance || charpos - best_below < distance);
         tail = tail->next, distance += BYTECHAR_DISTANCE_INCREMENT)
...
ptrdiff_t buf_bytepos_to_charpos(struct buffer* b, ptrdiff_t bytepos) {
    ...
    for (tail = BUF_MARKERS(b);
         /* If we are down to a range of DISTANCE bytes,
            ...
            scan the intervening chars directly now.  */
         tail &&
         !(best_above_byte - bytepos < distance ||
...
static void attach_marker(struct Lisp_Marker* m, struct buffer* b,
...
static struct buffer* live_buffer(Lisp_Object buffer) {
...
static Lisp_Object set_marker_internal(Lisp_Object marker, Lisp_Object position,
    ...
    /* Optimize the special case where we are copying the position of
       ...
       is a marker and so we know the bytepos already.  */
        ...
        charpos = clip_to_bounds(restricted ? BUF_BEGV(b) : BUF_BEG(b), charpos,
       ...
       between character and byte positions.  */
        if (bytepos == -1 ||
            ...
            bytepos = clip_to_bounds(
                restricted ? BUF_BEGV_BYTE(b) : BUF_BEG_BYTE(b), bytepos,
                restricted ? BUF_ZV_BYTE(b) : BUF_Z_BYTE(b));
    ...
    if (m->buffer && EQ(marker, BVAR(m->buffer, mark)) &&
        !NILP(BVAR(m->buffer, mark_active)) && buffer_window_count(m->buffer))
...
Lisp_Object set_marker_restricted(Lisp_Object marker, Lisp_Object position,
                                  Lisp_Object buffer) {
...
Lisp_Object set_marker_both(Lisp_Object marker, Lisp_Object buffer,
                            ptrdiff_t charpos, ptrdiff_t bytepos) {
...
Lisp_Object set_marker_restricted_both(Lisp_Object marker, Lisp_Object buffer,
                                       ptrdiff_t charpos, ptrdiff_t bytepos) {
    register struct Lisp_Marker* m;
    register struct buffer* b = live_buffer(buffer);
    ...
    if (b) {
        attach_marker(
            m, b, clip_to_bounds(BUF_BEGV(b), charpos, BUF_ZV(b)),
...
void detach_marker(Lisp_Object marker) { Fset_marker(marker, Qnil, Qnil); }

/* Remove MARKER from the chain of whatever buffer it is in.  Set its
   buffer NULL.  */

void unchain_marker(register struct Lisp_Marker* marker) {
...
ptrdiff_t marker_position(Lisp_Object marker) {
...
ptrdiff_t marker_byte_position(Lisp_Object marker) {
    register struct Lisp_Marker* m = XMARKER(marker);
    register struct buffer* buf = m->buffer;
...
(register Lisp_Object marker, Lisp_Object type) {
    ...
    if (!NILP(marker))
        CHECK_TYPE(FIXNUMP(marker) || MARKERP(marker), Qinteger_or_marker_p,
                   marker);

    new = Fmake_marker();
    Fset_marker(new, marker, (MARKERP(marker) ? Fmarker_buffer(marker) : Qnil));
...
int count_markers(struct buffer* buf) {
    int total = 0;
    struct Lisp_Marker* tail;
...
ptrdiff_t verify_bytepos(ptrdiff_t charpos) {
    ptrdiff_t below = BEG;
