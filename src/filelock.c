...
static time_t get_boot_sec(void) { return 0; }
...
static int rename_lock_file(char const* old, char const* new, bool force) {
#ifdef WINDOWSNT
    ...
    if (!force) {
        ...
        int r = emacs_renameat_noreplace(AT_FDCWD, old, AT_FDCWD, new);
        if (!(r < 0 && errno == ENOSYS))
            return r;
        if (link(old, new) == 0)
...
static int create_lock_file(char* lfname, char* lock_info_str, bool force) {
#ifdef WINDOWSNT
    ...
    if (err == ENOSYS || err == LINKS_MIGHT_NOT_WORK || err == ENAMETOOLONG) {
        ...
        if (fd < 0)
            ...
            if (emacs_write(fd, lock_info_str, lock_info_len) !=
                    lock_info_len ||
...
static int lock_file_1(Lisp_Object lfname, bool force) {
    ...
    int suffixlen =
        snprintf(lock_info_str + len, room, lock_info_fmt, pid, boot);
...
static bool within_one_second(intmax_t a, time_t b) {
    intmax_t diff;
...
static ptrdiff_t read_lock_data(char* lfname, char lfinfo[MAX_LFINFO + 1]) {
    ptrdiff_t nbytes;

    while ((nbytes = readlinkat(AT_FDCWD, lfname, lfinfo, MAX_LFINFO + 1)) <
               0 &&
           errno == EINVAL) {
...
/* Whether the string S starts with a decimal integer, optionally
   negative.  */
static bool integer_prefixed(char const* s) {
...
static int current_lock_owner(lock_info_type* owner, Lisp_Object lfname) {
    ...
    if (lfinfolen) {
        ...
        case '\357':
            /* Treat "\357\200\242" (U+F022 in UTF-8) like ":" (Bug#24656).
               This works around a bug in the Linux CIFS kernel client, which
               can mistakenly transliterate ':' to U+F022 in symlink contents.
       ...
       .#test.txt -> larsi@.118961:1646577954) is an empty string.  */
        ...
        if (VALID_PROCESS_ID(pid) && !(kill(pid, 0) < 0 && errno != EPERM) &&
            (boot_time == 0 || within_one_second(boot_time, get_boot_sec())))
            return ANOTHER_OWNS_IT;
    }

    /* The owner process is dead or has a strange pid, or the lock file is
       empty. Try to zap the lockfile.  If the lock file is empty, this assumes
...
static int lock_if_free(lock_info_type* clasher, Lisp_Object lfname) {
    int err;
...
static Lisp_Object make_lock_file_name(Lisp_Object fn) {
...
static Lisp_Object lock_file(Lisp_Object fn) {
    ...
    if (!NILP(subject_buf) && NILP(Fverify_visited_file_modtime(subject_buf)) &&
    ...
    if (!NILP(lfname)) {
       ...
       lock_if_free returns an errno value.  */
        if (lock_if_free(&lock_info, lfname) == ANOTHER_OWNS_IT) {
            ...
            attack =
                calln(Qask_user_about_lock, fn, build_string(lock_info.user));
...
static Lisp_Object unlock_file(Lisp_Object fn) {
    ...
    if (!(err == 0 || err == ANOTHER_OWNS_IT ||
          (err == I_OWN_IT &&
           (emacs_unlink(SSDATA(lfname)) == 0 || (err = errno) == ENOENT))))
        report_file_errno("Unlocking file", fn, err);

    return Qnil;
}

static Lisp_Object unlock_file_handle_error(Lisp_Object err) {
...
(Lisp_Object file) {
    ...
    internal_condition_case_1(unlock_file, file, list1(Qfile_error),
...
(Lisp_Object file) {
    if (NILP(file))
        ...
        CHECK_STRING(file);
    if (SAVE_MODIFF < MODIFF && !NILP(file))
        Flock_file(file);
    return Qnil;
}
...
error did not occur.  */)
(void) {
    if (SAVE_MODIFF < MODIFF && STRINGP(BVAR(current_buffer, file_truename)))
...
(Lisp_Object filename) {
    ...
    case 0:
        ...
        break;
    default:
        report_file_errno("Testing file lock", filename, owner);
    }
