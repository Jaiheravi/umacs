...
static bool would_block(int errnum) {
#ifdef EWOULDBLOCK
...
static int accept4(int sockfd, struct sockaddr* addr, socklen_t* addrlen,
                   int flags) {
    return close_on_exec(accept(sockfd, addr, addrlen));
}

static int process_socket(int domain, int type, int protocol) {
...
#define DATAGRAM_CONN_P(proc)                                                  \
    (PROCESSP(proc) && XPROCESS(proc)->infd >= 0 &&                            \
...
static void pset_buffer(struct Lisp_Process* p, Lisp_Object val) {
    p->buffer = val;
}
static void pset_command(struct Lisp_Process* p, Lisp_Object val) {
    p->command = val;
}
static void pset_decode_coding_system(struct Lisp_Process* p, Lisp_Object val) {
    p->decode_coding_system = val;
}
static void pset_decoding_buf(struct Lisp_Process* p, Lisp_Object val) {
    p->decoding_buf = val;
}
static void pset_encode_coding_system(struct Lisp_Process* p, Lisp_Object val) {
    p->encode_coding_system = val;
}
static void pset_encoding_buf(struct Lisp_Process* p, Lisp_Object val) {
    p->encoding_buf = val;
}
static void pset_filter(struct Lisp_Process* p, Lisp_Object val) {
    p->filter = NILP(val) ? Qinternal_default_process_filter : val;
}
static void pset_log(struct Lisp_Process* p, Lisp_Object val) { p->log = val; }
static void pset_mark(struct Lisp_Process* p, Lisp_Object val) {
    p->mark = val;
}
static void pset_thread(struct Lisp_Process* p, Lisp_Object val) {
    p->thread = val;
}
static void pset_name(struct Lisp_Process* p, Lisp_Object val) {
    p->name = val;
}
static void pset_plist(struct Lisp_Process* p, Lisp_Object val) {
    p->plist = val;
}
static void pset_sentinel(struct Lisp_Process* p, Lisp_Object val) {
    p->sentinel = NILP(val) ? Qinternal_default_process_sentinel : val;
}
static void pset_tty_name(struct Lisp_Process* p, Lisp_Object val) {
    p->tty_name = val;
}
static void pset_type(struct Lisp_Process* p, Lisp_Object val) {
    p->type = val;
}
static void pset_write_queue(struct Lisp_Process* p, Lisp_Object val) {
    p->write_queue = val;
}
static void pset_stderrproc(struct Lisp_Process* p, Lisp_Object val) {
    p->stderrproc = val;
}


static Lisp_Object make_lisp_proc(struct Lisp_Process* p) {
...
static void clear_fd_callback_data(struct fd_callback_data* elem) {
    elem->func = NULL;
    elem->data = NULL;
    elem->flags = 0;
    elem->thread = NULL;
...
void add_read_fd(int fd, fd_callback func, void* data) {
...
void add_non_keyboard_read_fd(int fd, fd_callback func, void* data) {
    add_read_fd(fd, func, data);
    fd_callback_info[fd].flags &= ~KEYBOARD_FD;
}

static void add_process_read_fd(int fd) {
...
static void add_non_blocking_write_fd(int fd) {
    eassert(fd >= 0 && fd < FD_SETSIZE);
    eassert(fd_callback_info[fd].func == NULL);
...
static void recompute_max_desc(void) {
    int fd;
...
static void compute_input_wait_mask(fd_set* mask) {
    int fd;
...
static void compute_non_process_wait_mask(fd_set* mask) {
    int fd;
...
static void compute_non_keyboard_wait_mask(fd_set* mask) {
    int fd;
...
static void compute_write_mask(fd_set* mask) {
    int fd;
...
static void clear_waiting_thread_info(void) {
    int fd;
...
static void update_status(struct Lisp_Process* p) {
...
static bool connecting_status(Lisp_Object status) {
...
static void decode_status(Lisp_Object l, Lisp_Object* symbol, Lisp_Object* code,
                          bool* coredump) {
    Lisp_Object tem;
...
static Lisp_Object status_message(struct Lisp_Process* p) {
...
static int allocate_pty(char pty_name[PTY_NAME_SIZE]) {
#ifdef HAVE_PTYS
    int fd;
...
#endif
    {
        ...
        if (fd >= 0) {
            ...
            if (sys_faccessat(AT_FDCWD, pty_name, R_OK | W_OK, AT_EACCESS) !=
                0) {
                emacs_close(fd);
                continue;
            }
...
static struct Lisp_Process* allocate_process(void) {
    return ALLOCATE_ZEROED_PSEUDOVECTOR(struct Lisp_Process, thread,
                                        PVEC_PROCESS);
}

static Lisp_Object make_process(Lisp_Object name) {
...
static void remove_process(register Lisp_Object proc) {
...
void update_processes_for_thread_death(Lisp_Object dying_thread) {
    Lisp_Object pair;
...
static void free_dns_request(Lisp_Object proc) {
...
(Lisp_Object object) { return PROCESSP(object) ? Qt : Qnil; }
...
void record_deleted_pid(pid_t pid, Lisp_Object filename) {
...
static void update_process_mark(struct Lisp_Process* p) {
    Lisp_Object buffer = p->buffer;
    if (BUFFERP(buffer) && XMARKER(p->mark)->buffer != XBUFFER(buffer))
        set_marker_both(p->mark, buffer, BUF_ZV(XBUFFER(buffer)),
...
DEFUN("process-mark", Fprocess_mark, Sprocess_mark, 1, 1, 0,
...
static void set_process_filter_masks(struct Lisp_Process* p) {
...
static bool is_pty_from_symbol(Lisp_Object symbol) {
    if (EQ(symbol, Qpty))
        return true;
...
static void set_proc_thread(struct Lisp_Process* proc,
                            struct thread_state* thrd) {
...
DEFUN("process-query-on-exit-flag", Fprocess_query_on_exit_flag,
      Sprocess_query_on_exit_flag, 1, 1, 0,
...
(Lisp_Object process, Lisp_Object key, Lisp_Object no_block) {
    ...
    if (DATAGRAM_CONN_P(process) && (EQ(key, Qt) || EQ(key, QCremote)))
        contact =
            plist_put(contact, QCremote, Fprocess_datagram_address(process));
#endif
    ...
    if (NILP(key) && NETCONN_P(process))
        return list2(plist_get(contact, QChost), plist_get(contact, QCservice));
    if (NILP(key) && SERIALCONN_P(process))
        return list2(plist_get(contact, QCport), plist_get(contact, QCspeed));
...
DEFUN("process-plist", Fprocess_plist, Sprocess_plist, 1, 1, 0,
...
DEFUN("set-process-plist", Fset_process_plist, Sset_process_plist, 2, 2, 0,
...
(void) { return Fmapcar(Qcdr, Vprocess_alist); }


static Lisp_Object get_required_string_keyword_param(Lisp_Object kwargs,
                                                     Lisp_Object keyword) {
...
(ptrdiff_t nargs, Lisp_Object* args) {
    ...
    if (!NILP(plist_get(contact, QCfile_handler))) {
        Lisp_Object file_handler = Ffind_file_name_handler(
            BVAR(current_buffer, directory), Qmake_process);
    ...
    if (PROCESSP(xstderr)) {
        ...
        CHECK_STRING(program);
        stderrproc = CALLN(Fmake_pipe_process, QCname,
                           concat2(name, build_string(" stderr")), QCbuffer,
                           Fget_buffer_create(xstderr, Qnil), QCnoquery,
    ...
    XPROCESS(proc)->adaptive_read_buffering =
        (NILP(Vprocess_adaptive_read_buffering)         ? 0
             : EQ(Vprocess_adaptive_read_buffering, Qt) ? 1
                                                        : 2);
...

    {
        ...
        if (NILP(val)) {
            if (EQ(coding_systems, Qt)) {
                ...
                if (!NILP(program))
                    coding_systems =
                        Ffind_operation_coding_system(nargs2, args2);
            }
            if (CONSP(coding_systems))
    ...
    if (!NILP(program)) {
       ...
       Put the name we will really use in TEM.  */
        if (!IS_DIRECTORY_SEP(SREF(program, 0)) &&
            !(SCHARS(program) > 1 && IS_DEVICE_SEP(SREF(program, 1)))) {
            tem = Qnil;
...
static void close_process_fd(int* fd_addr) {
    int fd = *fd_addr;
    if (0 <= fd) {
        *fd_addr = -1;
        emacs_close(fd);
...
void dissociate_controlling_tty(void) {
    if (setsid() < 0) {
#ifdef TIOCNOTTY
...
static void create_process(Lisp_Object process, char** new_argv,
...
Lisp_Object conv_sockaddr_to_lisp(struct sockaddr* sa, ptrdiff_t len) {
    Lisp_Object address;
    unsigned char* cp;
...
static Lisp_Object conv_addrinfo_to_lisp(struct addrinfo* res) {
    Lisp_Object protocol = make_fixnum(res->ai_protocol);
    eassert(XFIXNUM(protocol) == res->ai_protocol);
    return Fcons(protocol,
                 conv_sockaddr_to_lisp(res->ai_addr, res->ai_addrlen));
}


/* Get family and required size for sockaddr structure to hold ADDRESS.  */

static ptrdiff_t get_lisp_to_sockaddr_size(Lisp_Object address, int* familyp) {
    struct Lisp_Vector* p;
...
static void conv_lisp_to_sockaddr(int family, Lisp_Object address,
...
static int set_socket_option(int s, Lisp_Object opt, Lisp_Object val) {
    char* name;
    ...
    case SOPT_BOOL:
        {
            int optval;
            optval = NILP(val) ? 0 : 1;
            ret = setsockopt(s, sopt->optlevel, sopt->optnum, &optval,
                             sizeof(optval));
            break;
        }
    ...
    case SOPT_IFNAME:
        {
            ...
            if (STRINGP(val))
                ...
                error("Bad option value for %s", name);
            ret =
                setsockopt(s, sopt->optlevel, sopt->optnum, devname, IFNAMSIZ);
            break;
        }
#endif
    ...
    case SOPT_LINGER:
        {
            ...
            ret = setsockopt(s, sopt->optlevel, sopt->optnum, &linger,
                             sizeof(linger));
            break;
        }
#endif
...
If PROCESS is a non-blocking network process that hasn't been fully
set up yet, this function will block until socket setup has completed. */)
(Lisp_Object process, Lisp_Object option, Lisp_Object value,
...
static void set_network_socket_coding_system(Lisp_Object proc, Lisp_Object host,
                                             Lisp_Object service,
                                             Lisp_Object name) {
    Lisp_Object tem;
    struct Lisp_Process* p = XPROCESS(proc);
    ...
    if (!NILP(tem)) {
        ...
        if (NILP(host) || NILP(service))
            ...
            coding_systems =
                CALLN(Ffind_operation_coding_system, Qopen_network_stream, name,
                      p->buffer, host, service);
        if (CONSP(coding_systems))
            val = XCAR(coding_systems);
        else if (CONSP(Vdefault_process_coding_system))
            val = XCAR(Vdefault_process_coding_system);
    ...
    if (!NILP(tem)) {
        ...
        if (EQ(coding_systems, Qt)) {
            if (NILP(host) || NILP(service))
                ...
                coding_systems =
                    CALLN(Ffind_operation_coding_system, Qopen_network_stream,
                          name, p->buffer, host, service);
        }
        if (CONSP(coding_systems))
            val = XCDR(coding_systems);
        else if (CONSP(Vdefault_process_coding_system))
...
#ifdef HAVE_GNUTLS
static void finish_after_tls_connection(Lisp_Object proc) {
    struct Lisp_Process* p = XPROCESS(proc);
    Lisp_Object contact = p->childp;
    Lisp_Object result = Qt;

    if (!NILP(Ffboundp(Qnsm_verify_connection)))
        result = calln(Qnsm_verify_connection, proc, plist_get(contact, QChost),
    ...
    if (NILP(result)) {
        pset_status(
            ...
            list2(Qfailed,
                  build_string(
                      "The Network Security Manager stopped the connections")));
        deactivate_process(proc);
    }
    else if (p->outfd < 0) {
    ...
    else if ((fd_callback_info[p->outfd].flags & NON_BLOCKING_CONNECT_FD) ==
             0) {
...
static void connect_network_socket(Lisp_Object proc, Lisp_Object addrinfos,
            ...
            if (family != AF_LOCAL)
                ...
                if (!(optbits & (1 << OPIX_REUSEADDR))) {
                    ...
                    if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR, &optval,
                                   sizeof optval))
                        report_file_error(
                            "Cannot set reuse option on server socket", Qnil);
                }
        ...
        if (p->socktype == SOCK_DGRAM) {
            ...
            if (p->is_server) {
                ...
                if (remote = plist_get(contact, QCremote), !NILP(remote)) {
                    ...
                    if (rlen != 0 && rfamily == family && rlen == addrlen)
    ...
    if (s < 0) {
        const char* err = (p->is_server ? "make server process failed"
    ...
    if (p->is_non_blocking_client) {
       ...
       connection.  */
        if (!(connecting_status(p->status) && EQ(XCDR(p->status), addrinfos)))
    ...
    if (!NILP(p->gnutls_boot_parameters)) {
        ...
        if (p->gnutls_initstage == GNUTLS_STAGE_READY) {
            ...
            if (NILP(boot))
                pset_status(
                    p, list2(Qfailed, build_string("TLS negotiation failed")));
            else
       ...
       Convert to a C string for later use by getaddrinfo.  */
        ...
        if (nowait) {
            ...
            struct req {
                ...
                FLEXSIZEOF(struct req, str, hostlen + 1 + portstringlen + 1));
            ...
            if (ret)
                error("%s/%s getaddrinfo_a error %d", SSDATA(host), portstring,
                      ret);
    ...
    if (EQ(service, Qt))
        ...
        if (SBYTES(service) != 0) {
            ...
            if (service_end == SSDATA(service) + SBYTES(service))
                ...
                struct servent* svc_info = getservbyname(
                    SSDATA(service), socktype == SOCK_DGRAM ? "udp" : "tcp");
                if (svc_info)
...
static Lisp_Object network_interface_list(bool full, unsigned short match) {
...
static Lisp_Object network_interface_info(Lisp_Object ifname) {
    struct ifreq rq;
    Lisp_Object res = Qnil;
    Lisp_Object elt;
    int s;
    ...
    if (ioctl(s, SIOCGIFFLAGS, &rq) == 0) {
        ...
        /* If flags is smaller than int (i.e. short) it may have the high bit
           set due to IFF_MULTICAST.  In that case, sign extending it into an
           int is wrong.  */
    ...
    if (ioctl(s, SIOCGIFHWADDR, &rq) == 0) {
        ...
        for (int n = 0; n < 6; n++)
            p->contents[n] =
                make_fixnum(((unsigned char*)&rq.ifr_hwaddr.sa_data[0])[n]);
    ...
    if (getifaddrs(&ifap) != -1) {
        ...
        for (struct ifaddrs* it = ifap; it != NULL; it = it->ifa_next) {
            ...
            if (it->ifa_addr->sa_family != AF_LINK ||
                strcmp(it->ifa_name, SSDATA(ifname)) != 0 || sdl->sdl_alen != 6)
                continue;
...
static Lisp_Object network_lookup_address_info_1(Lisp_Object host,
                                                 const char* service,
                                                 struct addrinfo* hints,
                                                 struct addrinfo** res) {
    Lisp_Object msg = Qt;
    ...
    if (ret) {
        ...
        msg =
            CALLN(Fformat, format, host, build_string(service), make_int(ret));
#endif
    }
    return msg;
}
...
static void deactivate_process(Lisp_Object proc) {
    int inchannel;
    struct Lisp_Process* p = XPROCESS(proc);
    int i;
...
(Lisp_Object process, Lisp_Object seconds, Lisp_Object millisec,
 Lisp_Object just_this_one) {
    ...
    if (!NILP(process)) {
       ...
       thread.  */
        if (!NILP(proc->thread) && !BASE_EQ(proc->thread, Fcurrent_thread())) {
            ...
            error(
                "Attempt to accept output from process %s locked to thread %s",
                SDATA(proc->name),
    ...
    if (!NILP(millisec)) { /* Obsolete calling convention using integers rather
                              than floats.  */
    ...
    return (
        (wait_reading_process_output(secs, nsecs, 0, 0, Qnil,
                                     !NILP(process) ? XPROCESS(process) : NULL,
                                     (NILP(just_this_one)           ? 0
                                          : !FIXNUMP(just_this_one) ? 1
                                                                    : -1)) <= 0)
            ? Qnil
            : Qt);
}
...
static void server_accept_connection(Lisp_Object server, int channel) {
    Lisp_Object buffer;
...
#define HOST_FORMAT_IN6 "%x:%x:%x:%x:%x:%x:%x:%x"
    ...
    if (!NILP(service))
        contact = plist_put(contact, QCservice, service);
    contact =
        plist_put(contact, QCremote, conv_sockaddr_to_lisp(&saddr.sa, len));
#ifdef HAVE_GETSOCKNAME
    len = sizeof saddr;
    if (getsockname(s, &saddr.sa, &len) == 0)
        contact =
            plist_put(contact, QClocal, conv_sockaddr_to_lisp(&saddr.sa, len));
#endif
...
#ifdef HAVE_GETADDRINFO_A
static Lisp_Object check_for_dns(Lisp_Object proc) {
    struct Lisp_Process* p = XPROCESS(proc);
    Lisp_Object addrinfos = Qnil;
...
static void wait_for_socket_fds(Lisp_Object process, char const* name) {
...
static void wait_while_connecting(Lisp_Object process) {
...
static void wait_for_tls_negotiation(Lisp_Object process) {
#ifdef HAVE_GNUTLS
...
static void wait_reading_process_output_unwind(int data) {
    clear_waiting_thread_info();
    waiting_for_user_input_p = data;
}

/* This is here so breakpoints can be put on it.  */
static void wait_reading_process_output_1(void) {}

#if defined HAVE_ANDROID && !defined ANDROID_STUBIFY && defined THREADS_ENABLED
...
static int android_select_wrapper(int nfds, fd_set* readfds, fd_set* writefds,
                                  fd_set* exceptfds,
...
int wait_reading_process_output(intmax_t time_limit, int nsecs, int read_kbd,
                                bool do_display, Lisp_Object wait_for_cell,
    ...
    uintmax_t prev_wait_proc_nbytes_read =
        wait_proc ? wait_proc->nbytes_read : 0;
#if defined HAVE_GETADDRINFO_A || defined HAVE_GNUTLS
    bool retry_for_async;
#endif
    specpdl_ref count = SPECPDL_INDEX();

    eassert(wait_proc == NULL || NILP(wait_proc->thread) ||
    ...
    while (1) {
       ...
       and those callers cannot handle running arbitrary Lisp code here.  */
        if (NILP(wait_for_cell) && just_wait_proc >= 0) {
            ...
            if (read_kbd != 0 && requeued_command_events_pending_p())
                break;
       ...
       timeout to get our attention.  */
        if (update_tick != process_tick) {
            ...
            if ((thread_select(pselect, max_desc + 1, &Atemp,
                               (num_pending_connects > 0 ? &Ctemp : NULL), NULL,
                               &timeout, NULL) <= 0)) {
       ...
       read whatever data has already been received.  */
        ...
        if (wait_proc && !EQ(wait_proc->status, Qrun) &&
       ...
       nfds to 1 to avoid breaking the loop.  */
        ...
        if ((read_kbd
           ...
           And if so, we need to skip the select which could block. */
            ...
            for (channel = 0; channel < FD_SETSIZE; ++channel)
                if (!NILP(chan_process[channel]) &&
                    ...
                    if (p && p->gnutls_p && p->gnutls_state &&
                        emacs_gnutls_record_check_pending(p->gnutls_state) >
                            0) {
                        tls_nfds++;
            ...
            nfds = android_select(max_desc + 1, &Available,
                                  (check_write ? &Writeok : 0), NULL, &timeout);
#else /* THREADS_ENABLED */
            nfds = thread_select(android_select_wrapper, max_desc + 1,
                                 &Available, (check_write ? &Writeok : 0), NULL,
                                 &timeout, NULL);
            ...
            nfds =
                xg_select(max_desc + 1, &Available,
                          (check_write ? &Writeok : 0), NULL, &timeout, NULL);
#elif defined HAVE_NS
            /* And NS builds call thread_select in ns_select. */
            nfds =
                ns_select(max_desc + 1, &Available,
                          (check_write ? &Writeok : 0), NULL, &timeout, NULL);
#else /* !HAVE_GLIB */
	  ...
	  if (detect_input_pending ())
       ...
       but select says there is input.  */
        ...
        if (read_kbd && interrupt_input && keyboard_bit_set(&Available) &&
            !noninteractive)
#ifdef USABLE_SIGIO
       ...
       obey it now if we should.  */
        ...
        for (channel = 0; channel <= max_desc; ++channel) {
            ...
            if (d->func &&
                ((d->flags & FOR_READ && FD_ISSET(channel, &Available)) ||
                 ((d->flags & FOR_WRITE) && FD_ISSET(channel, &Writeok))))
        ...
        for (channel = channel_start, wrapped = false;
            ...
            if (FD_ISSET(channel, &Available) &&
                ((fd_callback_info[channel].flags &
                  (KEYBOARD_FD | PROCESS_FD)) == PROCESS_FD)) {
                int nread;
               ...
               process gone just because its pipe is closed.  */
                         ...
                         !SERIALCONN_P(proc) && !PIPECONN_P(proc))
#ifdef WINDOWSNT
                    ;
#else
                    delete_read_fd(channel);
            ...
            if (FD_ISSET(channel, &Writeok) &&
                (fd_callback_info[channel].flags & NON_BLOCKING_CONNECT_FD) !=
                    0) {
                struct Lisp_Process* p;
...
#ifndef WINDOWSNT
                {
                    ...
                    if (getsockopt(channel, SOL_SOCKET, SO_ERROR, &xerrno,
                                   &xlen))
                        xerrno = errno;
                }
#else
    ...
    /* Timers and/or process filters that we have run could have themselves
       called `accept-process-output' (and by that indirectly this function),
       thus possibly reading some (or all) output of wait_proc without us
       noticing it. This could potentially lead to an endless wait (dealt with
       earlier in the function) and/or a wrong return value (dealt with here).
     */
    if (wait_proc && wait_proc->nbytes_read != prev_wait_proc_nbytes_read)
        got_some_output =
            min(INT_MAX, (wait_proc->nbytes_read - prev_wait_proc_nbytes_read));
...
static Lisp_Object read_process_output_error_handler(Lisp_Object error_val) {
...
static void read_and_dispose_of_process_output(struct Lisp_Process*, char*,
                                               ssize_t, struct coding_system*);

static void read_and_insert_process_output(struct Lisp_Process*, char*, ssize_t,
...
static int read_process_output(Lisp_Object proc, int channel) {
    ssize_t nbytes;
    struct Lisp_Process* p = XPROCESS(proc);
    eassert(0 <= channel && channel < FD_SETSIZE);
    ...
    if (DATAGRAM_CHAN_P(channel)) {
        ...
        do
            nbytes = recvfrom(channel, chars + carryover, readmax, 0,
                              datagram_address[channel].sa, &len);
...
static void read_process_output_before_insert(
    struct Lisp_Process* p, Lisp_Object* old_read_only, ptrdiff_t* old_begv,
    ptrdiff_t* old_zv, ptrdiff_t* before, ptrdiff_t* before_byte,
...
static void read_process_output_after_insert(
    struct Lisp_Process* p, Lisp_Object* old_read_only, ptrdiff_t old_begv,
    ptrdiff_t old_zv, ptrdiff_t before, ptrdiff_t before_byte, ptrdiff_t opoint,
    ...
    if (BUFFERP(p->buffer) && (b = XBUFFER(p->buffer), b != current_buffer))
...
static void
read_and_insert_process_output(struct Lisp_Process* p, char* buf, ssize_t nread,
    ...
    if (NILP(BVAR(XBUFFER(p->buffer), enable_multibyte_characters)) &&
    ...
    else { /* We have to decode the input.  */
        Lisp_Object curbuf;
        specpdl_ref count1 = SPECPDL_INDEX();
       ...
       TEMP_SET_PT_BOTH below.  */
        ...
        decode_coding_c_string(process_coding, (unsigned char*)buf, nread,
                               curbuf);
        ...
        signal_after_change(PT - process_coding->produced_char, 0,
...
static void read_and_dispose_of_process_output(struct Lisp_Process* p,
                                               char* chars, ssize_t nbytes,
                                               struct coding_system* coding) {
...
(Lisp_Object proc, Lisp_Object text) {
    ...
    if (!NILP(p->buffer) && BUFFER_LIVE_P(XBUFFER(p->buffer))) {
        ...
        if (NILP(BVAR(current_buffer, enable_multibyte_characters)) !=
            ...
            text = (STRING_MULTIBYTE(text) ? Fstring_as_unibyte(text)
...
static void write_queue_push(struct Lisp_Process* p, Lisp_Object input_obj,
...
static bool write_queue_pop(struct Lisp_Process* p, Lisp_Object* obj,
...
static void send_process(Lisp_Object proc, const char* buf, ptrdiff_t len,
                         Lisp_Object object) {
    struct Lisp_Process* p = XPROCESS(proc);
    ssize_t rv;
    struct coding_system* coding;
    ...
    if (p->raw_status_new)
        update_status(p);
    if (!EQ(p->status, Qrun))
        error("Process %s not running: %s", SDATA(p->name),
              SDATA(status_message(p)));
    if (p->outfd < 0)
    ...
    if ((STRINGP(object) && STRING_MULTIBYTE(object)) ||
       ...
       or one of the subsidiary if we have not yet done it.  */
        if (CODING_REQUIRE_ENCODING(coding)) {
            ...
            setup_coding_system(
                raw_text_coding_system(Vlast_coding_system_used), coding);
            coding->src_multibyte = 0;
        }
    }
    ...
    if (CODING_REQUIRE_ENCODING(coding)) {
        ...
        if (BUFFERP(object)) {
            ...
            encode_coding_object(coding, object, from, from_byte, to,
                                 from_byte + len, Qt);
        ...
        while (cur_len > 0) {
            ...
            if (rv < 0) {
                   ...
                   to finish doing output and read more.  */
                {
                    ...
                    wait_reading_process_output(0, 20 * 1000 * 1000, 0, 0, Qnil,
                                                NULL, 0);
...
(Lisp_Object process, Lisp_Object start, Lisp_Object end) {
    ...
    send_process(proc, (char*)BYTE_POS_ADDR(start_byte), end_byte - start_byte,
                 Fcurrent_buffer());
...
(Lisp_Object process, Lisp_Object string) {
    CHECK_STRING(string);
    Lisp_Object proc = get_process(process);
    send_process(proc, SSDATA(string), SBYTES(string), string);
    return Qnil;
}

/* Return the foreground process group for the tty/pty that
   the process P uses.  */
static pid_t emacs_get_tty_pgrp(struct Lisp_Process* p) {
    pid_t gid = -1;
...
(Lisp_Object process) {
    ...
    if (!EQ(p->type, Qreal))
        error("Process %s is not a subprocess", SDATA(p->name));
    if (p->infd < 0)
        error("Process %s is not active", SDATA(p->name));
...
static void process_send_signal(Lisp_Object process, int signo,
                                Lisp_Object current_group, bool nomsg) {
    Lisp_Object proc;
    struct Lisp_Process* p;
    pid_t gid;
    bool no_pgrp = 0;
...
(Lisp_Object process, Lisp_Object current_group) {
    return CALLN(Frun_hook_with_args_until_success,
                 Qinterrupt_process_functions, process, current_group);
}
...
(Lisp_Object process, Lisp_Object current_group) {
    if (PROCESSP(process) &&
        (NETCONN_P(process) || SERIALCONN_P(process) || PIPECONN_P(process))) {
        struct Lisp_Process* p;

        p = XPROCESS(process);
        if (NILP(p->command) && p->infd >= 0)
            delete_read_fd(p->infd);
        pset_command(p, Qt);
        return process;
    }
...
static int abbr_to_signal(char const* name) {
    int i, signo;
...
(Lisp_Object process) {
    ...
    if (XPROCESS(proc)->raw_status_new)
        update_status(XPROCESS(proc));
    if (!EQ(XPROCESS(proc)->status, Qrun))
        error("Process %s not running: %s", SDATA(XPROCESS(proc)->name),
              SDATA(status_message(XPROCESS(proc))));
    ...
    if (XPROCESS(proc)->pty_in)
       ...
       Then we just can't win.)  */
        if (0 <= old_outfd && (EQ(p->type, Qnetwork) || p->infd == old_outfd))
...
void child_signal_init(void) {
    ...
    if (FD_SETSIZE <= fds[0]) {
       ...
       into an `fd_set'.  */
        emacs_close(fds[0]);
        emacs_close(fds[1]);
        report_file_errno("Creating pipe for child signal", Qnil, EMFILE);
    }
...
static void child_signal_read(int fd, void* data) {
    eassert(0 <= fd);
...
static Lisp_Object exec_sentinel_error_handler(Lisp_Object error_val) {
...
static void exec_sentinel(Lisp_Object proc, Lisp_Object reason) {
  ...
  okeymap = BVAR (current_buffer, keymap);
    ...
    internal_condition_case_1(
        read_process_output_call, list3(sentinel, proc, reason),
        !NILP(Vdebug_on_error) ? Qnil : Qerror, exec_sentinel_error_handler);
...
static int status_notify(struct Lisp_Process* deleting_process,
                         struct Lisp_Process* wait_proc) {
    Lisp_Object proc;
    ...
    FOR_EACH_PROCESS(tail, proc) {
        ...
        if (p->tick != p->update_tick) {
            ...
            while (!EQ(p->filter, Qt) && !connecting_status(p->status) &&
                   !EQ(p->status, Qlisten)
                   /* Network or serial process not stopped:  */
                   && !EQ(p->command, Qt) && p->infd >= 0 &&
                   p != deleting_process) {
...
(Lisp_Object proc, Lisp_Object msg) {
    ...
    if (!EQ(symbol, Qrun) && !NILP(buffer)) {
       ...
       (probably that's why the process is dead, too).  */
        ...
        if (NILP(BVAR(current_buffer, enable_multibyte_characters)))
            msg = (code_convert_string_norecord(msg, Vlocale_coding_system, 1));
...
DEFUN("process-coding-system", Fprocess_coding_system, Sprocess_coding_system,
      1, 1, 0,
      doc
...
void add_gpm_wait_descriptor(int desc) { add_keyboard_wait_descriptor(desc); }

void delete_gpm_wait_descriptor(int desc) {
...
static bool keyboard_bit_set(fd_set* mask) {
    int fd;
...
/* Defined in msdos.c.  */
extern int sys_select(int, fd_set*, fd_set*, fd_set*, struct timespec*, void*);
...
int wait_reading_process_output(intmax_t time_limit, int nsecs, int read_kbd,
    ...
    while (1) {
       ...
       reprocess its display stuff.  */
        ...
        if (read_kbd && detect_input_pending_run_timers(do_display)) {
            swallow_events(do_display);
            if (detect_input_pending_run_timers(do_display))
                break;
        }
        ...
        /* If there is unread keyboard input, also return.  */
        if (read_kbd && requeued_command_events_pending_p())
            break;
       ...
       Try changing this after version 19.31.)  */
        if (!NILP(wait_for_cell) && detect_input_pending()) {
            swallow_events(do_display);
            if (detect_input_pending())
                break;
        }
...
void add_timer_wait_descriptor(int fd) {
    eassert(0 <= fd && fd < FD_SETSIZE);
...
Lisp_Object remove_slash_colon(Lisp_Object name) {
    return (SREF(name, 0) == '/' && SREF(name, 1) == ':'
...
void add_keyboard_wait_descriptor(int desc) {
...
void delete_keyboard_wait_descriptor(int desc) {
#ifdef subprocesses
    eassert(desc >= 0 && desc < FD_SETSIZE);
...
void setup_process_coding_systems(Lisp_Object process) {
#ifdef subprocesses
    struct Lisp_Process* p = XPROCESS(process);
    int inch = p->infd;
    int outch = p->outfd;
    ...
    if (EQ(p->filter, Qinternal_default_process_filter) && BUFFERP(p->buffer)) {
        if (NILP(BVAR(XBUFFER(p->buffer), enable_multibyte_characters)))
...
DEFUN("waiting-for-user-input-p", Fwaiting_for_user_input_p,
      Swaiting_for_user_input_p, 0, 0, 0, doc:
...
void hold_keyboard_input(void) { kbd_is_on_hold = 1; }

/* Resume reading input from keyboard sources.  */

void unhold_keyboard_input(void) { kbd_is_on_hold = 0; }

/* Return true if keyboard input is on hold, zero otherwise.  */

bool kbd_on_hold_p(void) { return kbd_is_on_hold; }
...
(void) {
    Lisp_Object handler = Ffind_file_name_handler(
        BVAR(current_buffer, directory), Qlist_system_processes);
    if (!NILP(handler))
...
(Lisp_Object pid) {
    Lisp_Object handler = Ffind_file_name_handler(
        BVAR(current_buffer, directory), Qprocess_attributes);
    if (!NILP(handler))
...
DEFUN("num-processors", Fnum_processors, Snum_processors, 0, 1, 0,
      doc:/* Return the number of processors. */)
(Lisp_Object query) {
    (void)query;
    return make_fixnum(1);
}
...
void catch_child_signal(void) {
    ...
    if (old_action.sa_handler != deliver_child_signal)
        lib_child_handler = (old_action.sa_handler == SIG_DFL ||
                                     old_action.sa_handler == SIG_IGN
                                 ? dummy_handler
...
int open_channel_for_module(Lisp_Object process) {
    CHECK_PROCESS(process);
...
void init_process_emacs(int sockfd) {
#ifdef subprocesses
    int i;
...
void syms_of_process(void) {
    ...
    DEFSYM(Qinternal_default_process_filter, "internal-default-process-filter");
#endif
    DEFSYM(Qpri, "pri");
    DEFSYM(Qnice, "nice");
    ...
    DEFVAR_BOOL("delete-exited-processes", delete_exited_processes, doc:
                    /* Non-nil means delete processes immediately when they
    exit. A value of nil means don't delete them until `list-processes' is
    run.  */);
...
The variable takes effect when `start-process' is called.  */);
    ...
    DEFVAR_BOOL("process-prioritize-lower-fds", process_prioritize_lower_fds,
                doc:
                    /* Whether to start checking for subprocess output from
    first file descriptor. Emacs loops through file descriptors to check for
    output from subprocesses. If this variable is nil, the default, then after
    accepting output from a subprocess, Emacs will continue checking the rest
    of descriptors, starting from the one following the descriptor it just
    read.  If this variable is non-nil, Emacs will always restart the loop
    from the first file descriptor, thus favoring processes with lower
    descriptors.  */);
...
sentinel or a process filter function has an error.  */);
    ...
    DEFSYM(Qinternal_default_signal_process, "internal-default-signal-process");
