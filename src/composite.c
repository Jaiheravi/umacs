...
2009, 2010, 2011 National Institute of Advanced Industrial Science and
Technology (AIST) Registration Number H14PRO021 Copyright (C) 2003, 2006
...
ptrdiff_t get_composition_id(ptrdiff_t charpos, ptrdiff_t bytepos,
                             ptrdiff_t nchars, Lisp_Object prop,
                             Lisp_Object string) {
    ...
    */
    ...
    if (VECTORP(components) && ASIZE(components) >= 2 &&
    ...
    method = (NILP(components) ? COMPOSITION_RELATIVE
                               : ((FIXNUMP(components) || STRINGP(components))
                                      ? COMPOSITION_WITH_ALTCHARS
                                      : COMPOSITION_WITH_RULE_ALTCHARS));

    glyph_len = (method == COMPOSITION_WITH_RULE_ALTCHARS ? (ASIZE(key) + 1) / 2
                                                          : ASIZE(key));
    ...
    if (cmp->method != COMPOSITION_WITH_RULE_ALTCHARS) {
        ...
        for (ptrdiff_t i = 1; i < glyph_len; i += 2) {
            ...
            */
            ...
            this_left = (leftmost + (gref % 3) * (rightmost - leftmost) / 2.0 -
...
bool find_composition(ptrdiff_t pos, ptrdiff_t limit, ptrdiff_t* start,
                      ptrdiff_t* end, Lisp_Object* prop, Lisp_Object object) {
    Lisp_Object val;
...
static void run_composition_function(ptrdiff_t from, ptrdiff_t to,
                                     Lisp_Object prop) {
    Lisp_Object func;
    ptrdiff_t start, end;
    ...
    if (from > BEGV &&
        ...
        from = start;
    if (to < ZV && find_composition(to, -1, &start, &end, &prop, Qnil) &&
        !composition_valid_p(start, end, prop))
        to = end;
...
void update_compositions(ptrdiff_t from, ptrdiff_t to, int check_mask) {
    Lisp_Object prop;
    ptrdiff_t start, end;
        ...
        if (from > BEGV &&
            ...
            if (from < end)
                Fput_text_property(make_fixnum(from), make_fixnum(end),
                                   Qcomposition, Fcons(XCAR(prop), XCDR(prop)),
                                   Qnil);
    ...
    if (check_mask & CHECK_INSIDE) {
        ...
        while (from < to - 1 &&
               ...
               composition_valid_p(start, from, prop) && from < to - 1)
            run_composition_function(start, from, prop);
    }
...
void make_composition_value_copy(Lisp_Object list) {
...
void compose_text(ptrdiff_t start, ptrdiff_t end, Lisp_Object components,
                  Lisp_Object modification_func, Lisp_Object string) {
    Lisp_Object prop;

    prop =
        Fcons(Fcons(make_fixnum(end - start), components), modification_func);
    Fput_text_property(make_fixnum(start), make_fixnum(end), Qcomposition, prop,
                       string);
}
...
Lisp_Object composition_gstring_lookup_cache(Lisp_Object header) {
    return Fgethash(header, gstring_hash_table, Qnil);
}

Lisp_Object composition_gstring_put_cache(Lisp_Object gstring, ptrdiff_t len) {
...
Lisp_Object composition_gstring_from_id(ptrdiff_t id) {
    struct Lisp_Hash_Table* h = XHASH_TABLE(gstring_hash_table);
...
void composition_gstring_cache_clear_font(Lisp_Object font_object) {
    struct Lisp_Hash_Table* h = XHASH_TABLE(gstring_hash_table);
...
bool composition_gstring_p(Lisp_Object gstring) {
    Lisp_Object header;
    ptrdiff_t i;
...
int composition_gstring_width(Lisp_Object gstring, ptrdiff_t from, ptrdiff_t to,
    ...
    for (glyph = lgstring_glyph_addr(gstring, from); from < to;
         from++, glyph++) {
        int x;
...
void composition_gstring_adjust_zero_width(Lisp_Object gstring) {
    ptrdiff_t from = 0;
    int width = 0;
    ...
    for (ptrdiff_t i = 0;; i++) {
        ...
        if (NILP(glyph) || from != LGLYPH_FROM(glyph)) {
            ...
            if (width == 0) {
                if (NILP(LGLYPH_ADJUSTMENT(last)))
                    LGLYPH_SET_ADJUSTMENT(
                        last,
                        CALLN(Fvector, make_fixnum(0), make_fixnum(0),
                              make_fixnum(LGLYPH_WIDTH(last) + 1)));
                else
        ...
        width += (NILP(LGLYPH_ADJUSTMENT(glyph)) ? LGLYPH_WIDTH(glyph)
...
static Lisp_Object fill_gstring_header(ptrdiff_t from, ptrdiff_t from_byte,
                                       ptrdiff_t to, Lisp_Object font_object,
                                       Lisp_Object string) {
    ptrdiff_t len = to - from;
    if (len == 0)
        error("Attempt to shape zero-length text");
    eassume(0 < len);
    Lisp_Object header = (len <= 8 ? AREF(gstring_work_headers, len - 1)
                                   : make_uninit_vector(len + 1));

    ASET(header, 0, font_object);
    for (ptrdiff_t i = 0; i < len; i++) {
        int c = (NILP(string) ? fetch_char_advance_no_check(&from, &from_byte)
                              : fetch_string_char_advance_no_check(
                                    string, &from, &from_byte));
        ASET(header, i + 1, make_fixnum(c));
    }
    return header;
}

static void fill_gstring_body(Lisp_Object gstring) {
    Lisp_Object font_object = LGSTRING_FONT(gstring);
...
static Lisp_Object autocmp_chars(Lisp_Object rule, ptrdiff_t charpos,
                                 ptrdiff_t bytepos, ptrdiff_t limit,
                                 struct window* win, struct face* face,
...
#endif
    lgstring =
        Fcomposition_get_gstring(pos, make_fixnum(to), font_object, string);
    if (NILP(LGSTRING_ID(lgstring))) {
        /* Save point as marker before calling out to lisp.  */
        if (NILP(string))
            record_unwind_protect(restore_point_unwind,
                                  build_marker(current_buffer, pt, pt_byte));
        lgstring = safe_calln(Vauto_composition_function, AREF(rule, 2), pos,
                              make_fixnum(to), font_object, string, direction);
    }
...
static bool char_composable_p(int c) {
    Lisp_Object val;
    return (c >= ' ' &&
...
static inline bool inhibit_auto_composition(void) {
...
void composition_compute_stop_pos(struct composition_it* cmp_it,
                                  ptrdiff_t charpos, ptrdiff_t bytepos,
                                  ptrdiff_t endpos, Lisp_Object string,
                                  bool include_static) {
    ptrdiff_t start, end;
    int c;
    ...
    if (charpos < endpos) {
        ...
        if (endpos < 0) {
            ...
            if (current_buffer->long_line_optimizations_p)
                endpos =
                    get_small_narrowing_begv(cmp_it->parent_it->w, charpos);
        }
    }
    cmp_it->id = -1;
    cmp_it->ch = -2;
    ...
    if (include_static && charpos < endpos &&
        find_composition(charpos, endpos, &start, &end, &prop, string) &&
        start >= charpos && composition_valid_p(start, end, prop)) {
...
bool composition_reseat_it(struct composition_it* cmp_it, ptrdiff_t charpos,
                           ptrdiff_t bytepos, ptrdiff_t endpos,
                           struct window* w, signed char bidi_level,
    ...
    if (cmp_it->ch < 0) {
        ...
        cmp_it->id =
            get_composition_id(charpos, bytepos, end - start, prop, string);
        if (cmp_it->id < 0)
            goto no_composition;
        ...
        if (charpos < endpos) {
            ...
            for (; CONSP(val); val = XCDR(val)) {
                elt = XCAR(val);
                if (!VECTORP(elt) || ASIZE(elt) != 3 || !FIXNUMP(AREF(elt, 1)))
                    continue;
                if (XFIXNAT(AREF(elt, 1)) != cmp_it->lookback)
                    goto no_composition;
                lgstring = autocmp_chars(elt, charpos, bytepos, endpos, w, face,
...
int composition_update_it(struct composition_it* cmp_it, ptrdiff_t charpos,
...
bool find_automatic_composition(ptrdiff_t pos, ptrdiff_t limit,
                                ptrdiff_t backlim, ptrdiff_t* start,
                                ptrdiff_t* end, Lisp_Object* gstring,
                                Lisp_Object string) {
        ...
        while (cur.pos < fore_check_limit) {
            ...
            for (val = CHAR_TABLE_REF(Vcomposition_function_table, c);
                ...
                if (VECTORP(elt) && ASIZE(elt) == 3 && FIXNATP(AREF(elt, 1))) {
                    ...
                    if (check_pos < head ||
                        (limit <= pos ? pos < check_pos : limit <= check_pos))
                        continue;
...
ptrdiff_t composition_adjust_point(ptrdiff_t last_pt, ptrdiff_t new_pt) {
    ptrdiff_t i, beg, end;
    Lisp_Object val;
    ...
    if (!find_automatic_composition(new_pt, (ptrdiff_t)-1, (ptrdiff_t)-1, &beg,
                                    &end, &val, Qnil) ||
        beg == new_pt)
        return new_pt;
    for (i = 0; i < LGSTRING_GLYPH_LEN(val); i++) {
        ...
        if (beg + LGLYPH_TO(glyph) >= new_pt)
            return (new_pt < last_pt ? beg + LGLYPH_FROM(glyph)
...
should be ignored.  */)
(Lisp_Object from, Lisp_Object to, Lisp_Object font_object,
 Lisp_Object string) {
    ...
    header = fill_gstring_header(frompos, frombyte, topos, font_object, string);
...
(Lisp_Object start, Lisp_Object end, Lisp_Object components,
 Lisp_Object modification_func) {
    validate_region(&start, &end);
    if (!NILP(components) && !FIXNUMP(components) && !CONSP(components) &&
        !STRINGP(components))
        CHECK_VECTOR(components);

    compose_text(XFIXNUM(start), XFIXNUM(end), components, modification_func,
                 Qnil);
    return Qnil;
}
...
(Lisp_Object pos, Lisp_Object limit, Lisp_Object string, Lisp_Object detail_p) {
    ...
    if (!find_composition(from, to, &start, &end, &prop, string)) {
        if (((NILP(string) &&
            ...
            !inhibit_auto_composition() &&
            find_automatic_composition(from, to, (ptrdiff_t)-1, &start, &end,
                                       &gstring, string))
    ...
    if (!(start <= fixed_pos && fixed_pos < end)) {
        ...
        if (find_automatic_composition(from, to, (ptrdiff_t)-1, &s, &e,
                                       &gstring, string) &&
    ...
    if (composition_registered_p(prop))
        ...
        ptrdiff_t start_byte =
            (NILP(string) ? CHAR_TO_BYTE(start)
    ...
    if (id >= 0) {
        ...
        relative_p = (method == COMPOSITION_WITH_RULE_ALTCHARS ? Qnil : Qt);
...
static int compare_composition_rules(const void* r1, const void* r2) {
