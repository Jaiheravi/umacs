...
INLINE bool SINGLE_BYTE_CHAR_P(intmax_t c) { return 0 <= c && c < 0x100; }

/* True iff C is a character that corresponds to a raw 8-bit
   byte.  */
INLINE bool CHAR_BYTE8_P(int c) { return MAX_5_BYTE_CHAR < c; }

/* Return the character code for raw 8-bit byte BYTE.  */
INLINE int BYTE8_TO_CHAR(int byte) { return byte + 0x3FFF00; }

INLINE int UNIBYTE_TO_CHAR(int byte) {
    return ASCII_CHAR_P(byte) ? byte : BYTE8_TO_CHAR(byte);
}

/* Return the raw 8-bit byte for character C.  */
INLINE int CHAR_TO_BYTE8(int c) {
    return CHAR_BYTE8_P(c) ? c - 0x3FFF00 : c & 0xFF;
}

/* Return the raw 8-bit byte for character C,
   or -1 if C doesn't correspond to a byte.  */
INLINE int CHAR_TO_BYTE_SAFE(int c) {
...
INLINE bool CHAR_BYTE8_HEAD_P(int byte) { return byte == 0xC0 || byte == 0xC1; }

/* If C is not ASCII, make it multibyte.  Assumes C < 256.  */
INLINE int make_char_multibyte(int c) {
...
INLINE bool CHAR_VALID_P(intmax_t c) { return 0 <= c && c <= MAX_CHAR; }

/* Nonzero iff X is a character.  */
INLINE bool CHARACTERP(Lisp_Object x) {
    return FIXNUMP(x) && CHAR_VALID_P(XFIXNUM(x));
}

/* Check if Lisp object X is a character or not.  */
INLINE void CHECK_CHARACTER(Lisp_Object x) {
    CHECK_TYPE(CHARACTERP(x), Qcharacterp, x);
}

INLINE void CHECK_CHARACTER_CAR(Lisp_Object x) { CHECK_CHARACTER(XCAR(x)); }

INLINE void CHECK_CHARACTER_CDR(Lisp_Object x) { CHECK_CHARACTER(XCDR(x)); }

/* True if character C has a printable glyph.  */
INLINE bool CHAR_PRINTABLE_P(int c) {
    return ((32 <= c && c < 127) || !NILP(CHAR_TABLE_REF(Vprintable_chars, c)));
}

/* Return byte length of multibyte form for character C.  */
INLINE int CHAR_BYTES(int c) {
...
INLINE int CHAR_LEADING_CODE(int c) {
...
INLINE int CHAR_STRING(int c, unsigned char* p) {
    eassume(0 <= c);
...
INLINE int BYTE8_STRING(int b, unsigned char* p) {
    p[0] = 0xC0 | ((b >> 6) & 0x01);
    p[1] = 0x80 | (b & 0x3F);
    return 2;
}
...
INLINE bool LEADING_CODE_P(int byte) { return (byte & 0xC0) == 0xC0; }

/* True iff BYTE is a trailing code of a non-ASCII character in a
   multibyte form.  */
INLINE bool TRAILING_CODE_P(int byte) { return (byte & 0xC0) == 0x80; }

/* True iff BYTE starts a character in a multibyte form.
   This is equivalent to:
    (ASCII_CHAR_P (byte) || LEADING_CODE_P (byte))  */
INLINE bool CHAR_HEAD_P(int byte) { return (byte & 0xC0) != 0x80; }
...
INLINE int BYTES_BY_CHAR_HEAD(int byte) {
    return (!(byte & 0x80)       ? 1
                : !(byte & 0x20) ? 2
                : !(byte & 0x10) ? 3
                : !(byte & 0x08) ? 4
...
INLINE int multibyte_length(unsigned char const* p, unsigned char const* pend,
...
INLINE int raw_prev_char_len(unsigned char const* p) {
    for (int len = 1;; len++)
...
INLINE int string_char_and_length(unsigned char const* p, int* length) {
    int c = p[0];
    if (!(c & 0x80)) {
        *length = 1;
        return c;
...
INLINE int STRING_CHAR(unsigned char const* p) {
    int len;
...
INLINE int string_char_advance(unsigned char const** pp) {
...
INLINE int fetch_string_char_advance(Lisp_Object string, ptrdiff_t* charidx,
                                     ptrdiff_t* byteidx) {
    int output;
    ptrdiff_t b = *byteidx;
...
INLINE int fetch_string_char_as_multibyte_advance(Lisp_Object string,
                                                  ptrdiff_t* charidx,
                                                  ptrdiff_t* byteidx) {
    int output;
    ptrdiff_t b = *byteidx;
...
INLINE int fetch_string_char_advance_no_check(Lisp_Object string,
                                              ptrdiff_t* charidx,
                                              ptrdiff_t* byteidx) {
    ptrdiff_t b = *byteidx;
    unsigned char* chp = SDATA(string) + b;
...
INLINE int CHAR_VARIATION_SELECTOR_P(int c) {
    return (c < 0xFE00         ? 0
                : c <= 0xFE0F  ? c - 0xFE00 + 1
                : c < 0xE0100  ? 0
...
INLINE bool char_surrogate_p(int c) { return 0xD800 <= c && c <= 0xDFFF; }
...
extern ptrdiff_t c_string_width(const unsigned char*, ptrdiff_t, int,
                                ptrdiff_t*, ptrdiff_t*);
extern ptrdiff_t lisp_string_width(Lisp_Object, ptrdiff_t, ptrdiff_t, ptrdiff_t,
...
INLINE int char_table_translate(Lisp_Object obj, int ch) {
...
INLINE int char_hexdigit(int c) {
