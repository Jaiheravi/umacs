...
static void malloc_initialize_hook(void)
{
...
static void* ATTRIBUTE_NO_SANITIZE_ADDRESS no_sanitize_memcpy(void* dest,
                                                              void const* src,
                                                              size_t size)
{
...
static bool deadp(Lisp_Object x) { return BASE_EQ(x, dead_object()); }
...
static ATTRIBUTE_NO_SANITIZE_UNDEFINED void* XPNTR(Lisp_Object a)
{
...
static void XFLOAT_INIT(Lisp_Object f, double n) { XFLOAT(f)->u.data = n; }
...
static void tally_consing(ptrdiff_t nbytes) { consing_until_gc -= nbytes; }

#ifdef DOUG_LEA_MALLOC
static bool pointers_fit_in_lispobj_p(void)
{
  return (UINTPTR_MAX <= VAL_MAX) || USE_LSB_TAG;
}

static bool mmap_lisp_allowed_p(void)
{
...
void malloc_warning(const char* str) { pending_malloc_warning = str; }

#endif

/* Display an already-pending malloc warning.  */

void display_malloc_warning(void)
{
  calln(Qdisplay_warning, Qalloc, build_string(pending_malloc_warning),
        QCemergency);
...
void buffer_memory_full(ptrdiff_t nbytes)
{
...
void* xnmalloc(ptrdiff_t nitems, ptrdiff_t item_size)
{
...
void* xnrealloc(void* pa, ptrdiff_t nitems, ptrdiff_t item_size)
{
  eassert(0 <= nitems && 0 < item_size);
  ptrdiff_t nbytes;
  if (ckd_mul(&nbytes, nitems, item_size) || SIZE_MAX < nbytes)
              ...
              ptrdiff_t nitems_max, ptrdiff_t item_size)
{
  ...
  if (n - n0 < nitems_incr_min &&
      (ckd_add(&n, n0, nitems_incr_min) ||
       (0 <= nitems_max && nitems_max < n) || ckd_mul(&nbytes, n, item_size)))
    memory_full(SIZE_MAX);
...
char* xlispstrdup(Lisp_Object string)
{
...
void dupstring(char** ptr, char const* string)
{
  char* old = *ptr;
...
void xputenv(char const* string)
{
...
void* record_xmalloc(size_t size)
{
  void* p = xmalloc(size);
...
static void lisp_free(void* block)
{
  if (pdumper_object_p(block))
    return;
...
static void* pointer_align(void* ptr, int alignment)
{
...
static void* lisp_align_malloc(size_t nbytes, enum mem_type type)
{
  void *base, *val;
  struct ablocks* abase;
...
static void lisp_align_free(void* block)
{
  ...
  { /* All the blocks are free.  */
    ...
    {
#if GC_ASAN_POISON_OBJECTS
      __asan_unpoison_memory_region(&(*tem)->x, sizeof((*tem)->x));
#endif
...
#define ASAN_POISON_INTERVAL(i) __asan_poison_memory_region(i, sizeof *(i))
#define ASAN_UNPOISON_INTERVAL(i) __asan_unpoison_memory_region(i, sizeof *(i))
#else
...
static void mark_interval_tree_1(INTERVAL i, void* dummy)
{
...
enum
{
  STRING_BLOCK_SIZE = ((MALLOC_SIZE_NEAR(1024) - sizeof(struct string_block*)) /
...
static ptrdiff_t sdata_size(ptrdiff_t n)
{
  /* Reserve space for the nbytes union member even when N + 1 is less
     than the size of that member.  */
  ptrdiff_t unaligned_size = max(SDATA_DATA_OFFSET + n + 1, sizeof(sdata));
...
static void allocate_string_data(struct Lisp_String* s, EMACS_INT nchars,
                                 EMACS_INT nbytes, bool clearit,
                                 bool immovable);
...
#define ASAN_PREPARE_DEAD_SDATA(s, size)                                       \
  ...
  {                                                                            \
    ...
    __asan_unpoison_memory_region(&(s)->string, sizeof(struct Lisp_String*));  \
    __asan_unpoison_memory_region(&SDATA_NBYTES(s), sizeof SDATA_NBYTES(s));   \
  }                                                                            \
  while (false)
...
#define ASAN_POISON_STRING(s) __asan_poison_memory_region(s, sizeof *(s))
#define ASAN_UNPOISON_STRING(s) __asan_unpoison_memory_region(s, sizeof *(s))
#else
...
{
  ptrdiff_t nbytes = (s->u.s.size_byte < 0 ? s->u.s.size & ~ARRAY_MARK_FLAG
                                           : s->u.s.size_byte);
...
static void check_sblock(struct sblock* b)
{
  sdata* end = b->next_free;

  for (sdata* from = b->data; from < end;)
  {
    ptrdiff_t nbytes = sdata_size(from->string ? string_bytes(from->string)
                                               : SDATA_NBYTES(from));
...
static void check_string_bytes(bool all_p)
{
  if (all_p)
  {
    struct sblock* b;
...
static void check_string_free_list(void)
{
...
static void allocate_string_data(struct Lisp_String* s, EMACS_INT nchars,
                                 EMACS_INT nbytes, bool clearit, bool immovable)
{
  sdata* data;
  struct sblock* b;
 ...
 copying will happen this way.  */
    ...
    do
    {
     ...
     same as the one recorded in the sdata structure.  */
        ...
        sdata* from_end = (sdata*)((char*)from + size + GC_STRING_EXTRA);
        ...
        if (s)
        {
          ...
          sdata* to_end = (sdata*)((char*)to + size + GC_STRING_EXTRA);
          if (to_end > tb_end)
          {
            tb->next_free = to;
            tb = tb->next;
...
void string_overflow(void) { error("Maximum string size exceeded"); }
...
Lisp_Object bool_vector_fill(Lisp_Object a, Lisp_Object init)
{
...
Lisp_Object make_clear_bool_vector(EMACS_INT nbits, bool clearit)
{
  ...
  struct Lisp_Bool_Vector* p =
      (struct Lisp_Bool_Vector*)allocate_clear_vector(needed_elements, clearit);
...
Lisp_Object make_uninit_bool_vector(EMACS_INT nbits)
{
  return make_clear_bool_vector(nbits, false);
}

DEFUN("make-bool-vector", Fmake_bool_vector, Smake_bool_vector, 2, 2, 0, doc:
          /* Return a new bool-vector of length LENGTH, using INIT for each
element. LENGTH must be a number.  INIT matters only in whether it is t or nil.
*/)
...
Lisp_Object make_string(const char* contents, ptrdiff_t nbytes)
{
...
Lisp_Object make_multibyte_string(const char* contents, ptrdiff_t nchars,
                                  ptrdiff_t nbytes)
{
  register Lisp_Object val;
...
Lisp_Object make_string_from_bytes(const char* contents, ptrdiff_t nchars,
                                   ptrdiff_t nbytes)
{
  register Lisp_Object val;
  val = make_uninit_multibyte_string(nchars, nbytes);
...
Lisp_Object make_specified_string(const char* contents, ptrdiff_t nchars,
                                  ptrdiff_t nbytes, bool multibyte)
{
  ...
  {
    if (multibyte)
      nchars = multibyte_chars_in_text((const unsigned char*)contents, nbytes);
    else
      nchars = nbytes;
  }
  val = make_uninit_multibyte_string(nchars, nbytes);
...
static Lisp_Object make_clear_string(EMACS_INT length, bool clearit)
{
  Lisp_Object val;
...
Lisp_Object make_uninit_string(EMACS_INT length)
{
...
static Lisp_Object make_clear_multibyte_string(EMACS_INT nchars,
                                               EMACS_INT nbytes, bool clearit)
{
  Lisp_Object string;
  struct Lisp_String* s;
...
Lisp_Object make_uninit_multibyte_string(EMACS_INT nchars, EMACS_INT nbytes)
{
...
Lisp_Object make_formatted_string(const char* format, ...)
{
  char buf[MAX_ALLOCA];
  char* cstr = buf;
...
void pin_string(Lisp_Object string)
{
  ...
  if (!(size > LARGE_STRING_BYTES || pdumper_object_p(data) ||
        s->u.s.size_byte == -3))
  {
...
#define FLOAT_BLOCK_SIZE                                                       \
  (((BLOCK_BYTES -                                                             \
     sizeof(                                                                   \
         struct float_block*) /* The compiler might add padding at the end. */                                                                            \
...
#define SETMARKBIT(block, n)                                                   \
  ((block)->gcmarkbits[(n) / BITS_PER_BITS_WORD] |= (bits_word)1               \
       << ((n) % BITS_PER_BITS_WORD))
...
#define XFLOAT_MARKED_P(fptr) GETMARKBIT(FLOAT_BLOCK(fptr), FLOAT_INDEX(fptr))

#define XFLOAT_MARK(fptr) SETMARKBIT(FLOAT_BLOCK(fptr), FLOAT_INDEX(fptr))

#define XFLOAT_UNMARK(fptr) UNSETMARKBIT(FLOAT_BLOCK(fptr), FLOAT_INDEX(fptr))

#if GC_ASAN_POISON_OBJECTS
#define ASAN_POISON_FLOAT_BLOCK(fblk)                                          \
  __asan_poison_memory_region((fblk)->floats, sizeof((fblk)->floats))
#define ASAN_UNPOISON_FLOAT_BLOCK(fblk)                                        \
  __asan_unpoison_memory_region((fblk)->floats, sizeof((fblk)->floats))
...
#define CONS_BLOCK_SIZE                                                        \
  (((BLOCK_BYTES -                                                             \
     sizeof(                                                                   \
         struct cons_block*) /* The compiler might add padding at the end.  */ \
...
(Lisp_Object car, Lisp_Object cdr)
{
  ...
  else
  {
    ...
    {
      struct cons_block* new = lisp_align_malloc(sizeof *new, MEM_TYPE_CONS);
      memset(new->gcmarkbits, 0, sizeof new->gcmarkbits);
...
Lisp_Object list1(Lisp_Object arg1) { return Fcons(arg1, Qnil); }

Lisp_Object list2(Lisp_Object arg1, Lisp_Object arg2)
{
  return Fcons(arg1, Fcons(arg2, Qnil));
}


Lisp_Object list3(Lisp_Object arg1, Lisp_Object arg2, Lisp_Object arg3)
{
  return Fcons(arg1, Fcons(arg2, Fcons(arg3, Qnil)));
}

Lisp_Object list4(Lisp_Object arg1, Lisp_Object arg2, Lisp_Object arg3,
                  Lisp_Object arg4)
{
  return Fcons(arg1, Fcons(arg2, Fcons(arg3, Fcons(arg4, Qnil))));
}

Lisp_Object list5(Lisp_Object arg1, Lisp_Object arg2, Lisp_Object arg3,
...
static Lisp_Object cons_listn(ptrdiff_t count, Lisp_Object arg, va_list ap)
{
  eassume(0 < count);
...
Lisp_Object listn(ptrdiff_t count, Lisp_Object arg1, ...)
{
  va_list ap;
  va_start(ap, arg1);
...
static struct Lisp_Vector* next_vector(struct Lisp_Vector* v)
{
  return XUNTAG(v->contents[0], Lisp_Int0, struct Lisp_Vector);
}

static void set_next_vector(struct Lisp_Vector* v, struct Lisp_Vector* p)
{
...
static struct Lisp_Vector* ADVANCE(struct Lisp_Vector* v, ptrdiff_t nbytes)
{
  void* vv = v;
  char* cv = vv;
  void* p = cv + nbytes;
...
static ptrdiff_t VINDEX(ptrdiff_t nbytes)
{
...
static struct Lisp_Vector* large_vector_vec(struct large_vector* p)
{
...
static void setup_on_free_list(struct Lisp_Vector* v, ptrdiff_t nbytes)
{
...
static struct vector_block* allocate_vector_block(void)
{
...
static struct Lisp_Vector* allocate_vector_from_block(ptrdiff_t nbytes);

/* Memory footprint in bytes of a pseudovector other than a bool-vector.  */
static ptrdiff_t pseudovector_nbytes(const union vectorlike_header* hdr)
{
...
static struct Lisp_Vector* allocate_vector_from_block(ptrdiff_t nbytes)
{
  ...
  for (index = max(VINDEX(nbytes + VBLOCK_BYTES_MIN),
    ...
    if (vector_free_lists[index])
    {
      ...
      __asan_unpoison_memory_region(ADVANCE(vector, nbytes), restbytes);
#endif
...
ptrdiff_t vectorlike_nbytes(const union vectorlike_header* hdr)
{
  ...
  if (size & PSEUDOVECTOR_FLAG)
  {
    if (PSEUDOVECTOR_TYPEP(hdr, PVEC_BOOL_VECTOR))
    {
      ...
      ptrdiff_t word_bytes = (bool_vector_words(bv->size) * sizeof(bits_word));
...
#define PSEUDOVEC_STRUCT(p, t)                                                 \
  verify_expr(                                                                 \
      (header_size + VECSIZE(struct t) * word_size <= VBLOCK_BYTES_MAX),       \
      (struct t*)(p))

/* Release extra resources still in use by VECTOR, which may be any
   small vector-like object.  */

static void cleanup_vector(struct Lisp_Vector* vector)
{
  ...
  case PVEC_HASH_TABLE:
    {
      ...
      if (h->table_size > 0)
      {
        ...
        ptrdiff_t bytes = (h->table_size *
                               (2 * sizeof *h->key_and_value + sizeof *h->hash +
                                sizeof *h->next) +
...
#define VECTOR_ELTS_MAX                                                        \
  ((ptrdiff_t)min(                                                             \
      ((min(PTRDIFF_MAX, SIZE_MAX) - header_size - large_vector_offset) /      \
       word_size),                                                             \
...
static struct Lisp_Vector* allocate_vectorlike(ptrdiff_t len, bool clearit)
{
  ...
  else
  {
    struct large_vector* lv =
        lisp_malloc(large_vector_offset + nbytes, clearit, MEM_TYPE_VECTORLIKE);
...
static struct Lisp_Vector* allocate_clear_vector(ptrdiff_t len, bool clearit)
{
  if (len == 0)
...
struct Lisp_Vector* allocate_vector(ptrdiff_t len)
{
...
struct Lisp_Vector* allocate_nil_vector(ptrdiff_t len)
{
...
struct Lisp_Vector* allocate_pseudovector(int memlen, int lisplen, int zerolen,
                                          enum pvec_type tag)
{
  /* Catch bogus values.  */
  enum
...
struct buffer* allocate_buffer(void)
{
  struct buffer* b = ALLOCATE_PSEUDOVECTOR(
...
static struct Lisp_Vector* allocate_record(EMACS_INT count)
{
...
(ptrdiff_t nargs, Lisp_Object* args)
{
  if (!((FIXNUMP(args[CLOSURE_ARGLIST]) || CONSP(args[CLOSURE_ARGLIST]) ||
        ...
        STRINGP(args[CLOSURE_CODE]) && !STRING_MULTIBYTE(args[CLOSURE_CODE]) &&
        VECTORP(args[CLOSURE_CONSTANTS]) && FIXNATP(args[CLOSURE_STACK_DEPTH])))
...
#define ASAN_POISON_SYMBOL(sym) __asan_poison_memory_region(sym, sizeof *(sym))
...
static void set_symbol_name(Lisp_Object sym, Lisp_Object name)
{
...
Lisp_Object make_misc_ptr(void* a)
{
  struct Lisp_Misc_Ptr* p =
      ALLOCATE_PLAIN_PSEUDOVECTOR(struct Lisp_Misc_Ptr, PVEC_MISC_PTR);
  p->pointer = a;
  return make_lisp_ptr(p, Lisp_Vectorlike);
}

/* Return a new symbol with position with the specified SYMBOL and POSITION. */
Lisp_Object build_symbol_with_pos(Lisp_Object symbol, Lisp_Object position)
{
  Lisp_Object val;
...
Lisp_Object build_marker(struct buffer* buf, ptrdiff_t charpos,
                         ptrdiff_t bytepos)
{
...
Lisp_Object make_event_array(ptrdiff_t nargs, Lisp_Object* args)
{
  ptrdiff_t i;
  ...
  for (i = 0; i < nargs; i++)
    ...
    if (!FIXNUMP(args[i]) || (XFIXNUM(args[i]) & ~(-CHAR_META)) >= 0200)
...
Lisp_Object make_user_ptr(void (*finalizer)(void*), void* p)
{
...
static void init_finalizer_list(struct Lisp_Finalizer* head)
{
...
static void finalizer_insert(struct Lisp_Finalizer* element,
...
static void unchain_finalizer(struct Lisp_Finalizer* finalizer)
{
...
static void mark_finalizer_list(struct Lisp_Finalizer* head)
{
  for (struct Lisp_Finalizer* finalizer = head->next; finalizer != head;
...
static void queue_doomed_finalizers(struct Lisp_Finalizer* dest,
                                    struct Lisp_Finalizer* src)
{
  ...
  while (finalizer != src)
  {
    ...
    if (!vectorlike_marked_p(&finalizer->header) && !NILP(finalizer->function))
    {
...
static Lisp_Object run_finalizer_handler(Lisp_Object args)
{
  add_to_log("finalizer failed: %S", args);
  return Qnil;
}

static void run_finalizer_function(Lisp_Object function)
{
...
static void run_finalizers(struct Lisp_Finalizer* finalizers)
{
  struct Lisp_Finalizer* finalizer;
...
static bool vector_marked_p(const struct Lisp_Vector* v)
{
...
static bool vectorlike_marked_p(const union vectorlike_header* header)
{
  return vector_marked_p((const struct Lisp_Vector*)header);
}

static void set_vectorlike_marked(union vectorlike_header* header)
{
  set_vector_marked((struct Lisp_Vector*)header);
}

static bool cons_marked_p(const struct Lisp_Cons* c)
{
  return pdumper_object_p(c) ? pdumper_marked_p(c) : XCONS_MARKED_P(c);
}

static void set_cons_marked(struct Lisp_Cons* c)
{
...
static bool string_marked_p(const struct Lisp_String* s)
{
  return pdumper_object_p(s) ? pdumper_marked_p(s) : XSTRING_MARKED_P(s);
}

static void set_string_marked(struct Lisp_String* s)
{
...
static bool symbol_marked_p(const struct Lisp_Symbol* s)
{
  return pdumper_object_p(s) ? pdumper_marked_p(s) : s->u.s.gcmarkbit;
}

static void set_symbol_marked(struct Lisp_Symbol* s)
{
  if (pdumper_object_p(s))
    pdumper_set_marked(s);
  else
...
static bool interval_marked_p(INTERVAL i)
{
  return pdumper_object_p(i) ? pdumper_marked_p(i) : i->gcmarkbit;
}

static void set_interval_marked(INTERVAL i)
{
...
void memory_full(size_t nbytes)
{
  if (!initialized)
...
void refill_memory_reserve(void)
{
  ...
  if (spare_memory[1] == 0)
    spare_memory[1] =
        lisp_align_malloc(sizeof(struct cons_block), MEM_TYPE_SPARE);
  if (spare_memory[2] == 0)
    spare_memory[2] =
        lisp_align_malloc(sizeof(struct cons_block), MEM_TYPE_SPARE);
  if (spare_memory[3] == 0)
  ...
  if (spare_memory[5] == 0)
    spare_memory[5] =
        lisp_malloc(sizeof(struct string_block), false, MEM_TYPE_SPARE);
  if (spare_memory[6] == 0)
    spare_memory[6] =
        lisp_malloc(sizeof(struct string_block), false, MEM_TYPE_SPARE);
...
static struct mem_node* mem_find(void* start)
{
  struct mem_node* p;
...
static struct mem_node* mem_insert(void* start, void* end, enum mem_type type)
{
...
static void mem_insert_fixup(struct mem_node* x)
{
...
static void mem_rotate_left(struct mem_node* x)
{
  struct mem_node* y;
...
static void mem_rotate_right(struct mem_node* x)
{
...
static void mem_delete(struct mem_node* z)
{
  struct mem_node *x, *y;
...
static void mem_delete_fixup(struct mem_node* x)
{
...
static struct Lisp_String* live_string_holding(struct mem_node* m, void* p)
{
...
static bool live_string_p(struct mem_node* m, void* p)
{
...
static struct Lisp_Cons* live_cons_holding(struct mem_node* m, void* p)
{
  ...
  if (0 <= offset && offset < sizeof b->conses &&
      (b != cons_block || offset / sizeof b->conses[0] < cons_block_index))
  {
...
static bool live_cons_p(struct mem_node* m, void* p)
{
...
static struct Lisp_Symbol* live_symbol_holding(struct mem_node* m, void* p)
{
  ...
  if (0 <= offset && offset < sizeof b->symbols &&
      (b != symbol_block || offset / sizeof b->symbols[0] < symbol_block_index))
  {
...
static bool live_symbol_p(struct mem_node* m, void* p)
{
...
static struct Lisp_Float* live_float_holding(struct mem_node* m, void* p)
{
  ...
  if (0 <= offset && offset < sizeof b->floats)
  {
    ...
    if ((off == Lisp_Float || off == 0) &&
        (b != float_block || offset / sizeof b->floats[0] < float_block_index))
    {
...
static bool live_float_p(struct mem_node* m, void* p)
{
...
static struct Lisp_Vector* live_vector_pointer(struct Lisp_Vector* vector,
                                               void* p)
{
  void* vvector = vector;
  ...
  return ((offset == Lisp_Vectorlike || offset == 0 ||
           (sizeof vector->header <= offset && offset < vector_nbytes(vector) &&
            (!(vector->header.size & PSEUDOVECTOR_FLAG)
...
static struct Lisp_Vector* live_large_vector_holding(struct mem_node* m,
                                                     void* p)
{
...
static bool live_large_vector_p(struct mem_node* m, void* p)
{
...
static struct Lisp_Vector* live_small_vector_holding(struct mem_node* m,
                                                     void* p)
{
...
static bool live_small_vector_p(struct mem_node* m, void* p)
{
...
static void mark_maybe_pointer(void* p, bool symbol_only)
{
  struct mem_node* m;
...
void ATTRIBUTE_NO_SANITIZE_ADDRESS mark_memory(void const* start,
                                               void const* end)
{
  char const* pp;
...
void mark_c_stack(char const* bottom, char const* end)
{
...
NO_INLINE void flush_stack_call_func1(void (*func)(void* arg), void* arg)
{
  void* end;
...
static int valid_pointer_p(void* p)
{
#ifdef WINDOWSNT
...
int valid_lisp_object_p(Lisp_Object obj)
{
  if (FIXNUMP(obj))
    return 1;
...
static void allow_garbage_collection(intmax_t consing)
{
  consing_until_gc = consing - (HI_THRESHOLD - consing_until_gc);
  garbage_collection_inhibited--;
}

specpdl_ref inhibit_garbage_collection(void)
{
  specpdl_ref count = SPECPDL_INDEX();
...
static byte_ct object_bytes(object_ct n, size_t s)
{
  byte_ct b = s;
  return n * b;
}
...
static byte_ct total_bytes_of_live_objects(void)
{
  byte_ct tot = 0;
...
static Lisp_Object compact_font_cache_entry(Lisp_Object entry)
{
  ...
  for (tail = entry; CONSP(tail); tail = XCDR(tail))
  {
    ...
    if (CONSP(obj) && GC_FONT_SPEC_P(XCAR(obj)) &&
        ...
        &&
        (VECTORLIKEP(XCDR(obj)) && !(gc_asize(XCDR(obj)) & PSEUDOVECTOR_FLAG)))
    {
      ...
      for (i = 0; i < size; i++)
      {
        ...
        if (vectorlike_marked_p(&GC_XFONT_ENTITY(AREF(obj_cdr, i))->header))
          break;
...
static Lisp_Object compact_undo_list(Lisp_Object list)
{
  Lisp_Object tail, *prev = &list;

  for (tail = list; CONSP(tail); tail = XCDR(tail))
  {
    if (CONSP(XCAR(tail)) && MARKERP(XCAR(XCAR(tail))) &&
...
static Lisp_Object android_make_lisp_symbol(struct Lisp_Symbol* sym)
{
  intptr_t symoffset;
...
static void visit_vectorlike_root(struct gc_root_visitor visitor,
...
static void visit_buffer_root(struct gc_root_visitor visitor,
                              struct buffer* buffer, enum gc_root_type type)
{
...
void visit_static_gc_roots(struct gc_root_visitor visitor)
{
  visit_buffer_root(visitor, &buffer_defaults, GC_ROOT_BUFFER_LOCAL_DEFAULT);
  visit_buffer_root(visitor, &buffer_local_symbols, GC_ROOT_BUFFER_LOCAL_NAME);
...
static void mark_object_root_visitor(Lisp_Object const* root_ptr,
...
static EMACS_INT consing_threshold(intmax_t threshold, Lisp_Object percentage,
                                   intmax_t since_gc)
{
...
static EMACS_INT bump_consing_until_gc(intmax_t threshold,
                                       Lisp_Object percentage)
{
  ...
  EMACS_INT new_gc_threshold =
      consing_threshold(threshold, percentage, since_gc);
...
static Lisp_Object watch_gc_cons_threshold(Lisp_Object symbol,
                                           Lisp_Object newval,
...
static Lisp_Object watch_gc_cons_percentage(Lisp_Object symbol,
                                            Lisp_Object newval,
                                            Lisp_Object operation,
                                            Lisp_Object where)
{
...
void maybe_garbage_collect(void)
{
...
void garbage_collect(void)
{
  ...
  byte_ct tot_before = (false // <- profiler_memory_running
 ...
 in the undo_list any more, we can finally mark the list.  */
  ...
  if (FLOATP(Vgc_elapsed))
  {
    ...
    gc_elapsed =
        timespec_add(gc_elapsed, timespec_sub(current_timespec(), start));
...
static void mark_glyph_matrix(struct glyph_matrix* matrix)
{
...
static void mark_vectorlike(union vectorlike_header* header)
{
...
static void mark_char_table(struct Lisp_Vector* ptr, enum pvec_type pvectype)
{
...
static void mark_overlay(struct Lisp_Overlay* ov)
{
...
static void mark_overlays(struct itree_node* node)
{
  if (node == NULL)
    return;
...
static void mark_buffer(struct buffer* buffer)
{
  ...
  if (buffer->base_buffer && !vectorlike_marked_p(&buffer->base_buffer->header))
...
static void mark_frame(struct Lisp_Vector* ptr)
{
...
static void mark_window(struct Lisp_Vector* ptr)
{
...
static inline bool mark_stack_empty_p(void) { return mark_stk.sp <= 0; }

/* Pop and return a value from the mark stack (which must be nonempty).  */
static inline Lisp_Object mark_stack_pop(void)
{
...
NO_INLINE static void grow_mark_stack(void)
{
...
static inline void mark_stack_push_value(Lisp_Object value)
{
...
static inline void mark_stack_push_values(Lisp_Object* values, ptrdiff_t n)
{
  eassume(n >= 0);
  if (n == 0)
    return;
...
/* Check that the object pointed to by PO is alive, using predicate
   function LIVEP.  */
static inline void check_live(bool (*livep)(struct mem_node* m, void* p),
                              enum mem_type mtype, void* po, struct mem_node* m)
{
...
static inline void check_allocated_and_live(bool (*livep)(struct mem_node* m,
                                                          void* p),
                                            enum mem_type mtype, void* po)
{
  if (GC_CHECK_MARKED_OBJECTS)
  {
    if (pdumper_object_p(po))
...
static inline void check_allocated_and_live_symbol(void* po,
...
static inline void check_allocated_and_live_vectorlike(void* po,
                                                       Lisp_Object obj)
{
  if (GC_CHECK_MARKED_OBJECTS)
  {
...
static void process_mark_stack(ptrdiff_t base_sp)
{
#if GC_CDR_COUNT
        ...
        case PVEC_HASH_TABLE:
          {
            ...
            if (h->weakness == Weak_None)
              /* The values pushed here may include
                 ...
                 cope with.  */
              mark_stack_push_values(h->key_and_value, 2 * h->table_size);
            else
            {
...
void mark_objects(Lisp_Object* objs, ptrdiff_t n)
{
  ptrdiff_t sp = mark_stk.sp;
...
bool survives_gc_p(Lisp_Object obj)
{
  bool survives_p;
  ...
  case Lisp_Float:
    survives_p = XFLOAT_MARKED_P(XFLOAT(obj)) || pdumper_object_p(XFLOAT(obj));
    break;
...
static void unchain_dead_markers(struct buffer* buffer)
{
...
{
  Lisp_Object handler =
      Ffind_file_name_handler(BVAR(current_buffer, directory), Qmemory_info);
  if (!NILP(handler))
  ...
  if (w32_memory_info(&totalram, &freeram, &totalswap, &freeswap) == 0)
    return list4i((uintmax_t)totalram / 1024, (uintmax_t)freeram / 1024,
                  (uintmax_t)totalswap / 1024, (uintmax_t)freeswap / 1024);
  else
    return Qnil;
#elif defined MSDOS
...
(void)
{
  return list(make_int(cons_cells_consed), make_int(floats_consed),
              make_int(vector_cells_consed), make_int(symbols_consed),
              make_int(string_chars_consed), make_int(intervals_consed),
...
static bool symbol_uses_obj(Lisp_Object symbol, Lisp_Object obj)
{
  struct Lisp_Symbol* sym = XBARE_SYMBOL(symbol);
  Lisp_Object val = find_symbol_value(symbol);
  return (EQ(val, obj) || EQ(sym->u.s.function, obj) ||
          (!NILP(sym->u.s.function) && CLOSUREP(sym->u.s.function) &&
           EQ(AREF(sym->u.s.function, CLOSURE_CODE), obj)) ||
          (!NILP(val) && CLOSUREP(val) && EQ(AREF(val, CLOSURE_CODE), obj)));
}

/* Find at most FIND_MAX symbols which have OBJ as their value or
   function.  This is used in gdbinit's `xwhichsymbols' command.  */

Lisp_Object which_symbols(Lisp_Object obj, EMACS_INT find_max)
{
  struct symbol_block* sblk;
  specpdl_ref gc_count = inhibit_garbage_collection();
...
NO_INLINE static void verify_alloca(void)
{
  int i;
  enum
  {
    ALLOCA_CHECK_MAX = 256
  };
...
If this portion is smaller than `gc-cons-threshold', this is ignored.  */);
  ...
  DEFVAR_INT("pure-bytes-used", pure_bytes_used, doc:/* No longer used.  */);
...
:/* Number of string characters that have been consed so far.  */);
  ...
  DEFVAR_LISP("purify-flag", Vpurify_flag, doc:
                  /* Non-nil means loading Lisp code in order to dump an
    executable. This used to mean that certain objects should be allocated in
    shared (pure) space, but objects are not allocated in pure storage any
    more. This flag is still used in a few places, not to decide where objects
    are allocated, but to know if we're in the preload phase of Emacs's
    build.  */);
...
:/* Non-nil means display messages at start and end of garbage collection.  */);
  ...
  Vmemory_signal_data = list(Qerror,
...
{
  defined_HAVE_X_WINDOWS = true
};
#else
enum defined_HAVE_X_WINDOWS
{
  defined_HAVE_X_WINDOWS = false
};
#endif

#ifdef HAVE_PGTK
enum defined_HAVE_PGTK
{
  defined_HAVE_PGTK = true
};
#else
enum defined_HAVE_PGTK
{
  defined_HAVE_PGTK = false
};
#endif

#ifdef WINDOWSNT
enum defined_WINDOWSNT
{
  defined_WINDOWSNT = true
};
#else
enum defined_WINDOWSNT
{
  defined_WINDOWSNT = false
};
#endif
