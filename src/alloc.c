...
void* xpalloc(void* pa, ptrdiff_t* nitems, ptrdiff_t nitems_incr_min,
    ...
    ptrdiff_t n0 = *nitems;
    eassume(0 < item_size && 0 < nitems_incr_min && 0 <= n0 &&
            -1 <= nitems_max);
...
static void lisp_align_free(void* block) {
    ...
    if (busy < 2) { /* All the blocks are free.  */
        int i = 0;
        bool aligned = busy;
...
Lisp_Object make_specified_string(const char* contents, ptrdiff_t nchars,
    ...
    if (nchars < 0) {
        if (multibyte)
            nchars =
                multibyte_chars_in_text((const unsigned char*)contents, nbytes);
        else
            nchars = nbytes;
    }
    val = make_uninit_multibyte_string(nchars, nbytes);
...
#define FLOAT_BLOCK_SIZE                                                       \
    (((BLOCK_BYTES - sizeof(struct float_block*) /* The compiler might add     \
                                                    padding at the end. */     \
...
#define CONS_BLOCK_SIZE                                                        \
    (((BLOCK_BYTES - sizeof(struct cons_block*) /* The compiler might add      \
                                                   padding at the end.  */     \
...
static void init_vectors(void) {
    /* The normal vector allocation code refuses to allocate a 0-length vector
       ...
       for example by marking it by hand at the beginning of the GC and
       unmarking it by hand at the end.  */
    ...
    case PVEC_FONT:
        {
            ...
            if (((vector->header.size & PSEUDOVECTOR_SIZE_MASK) ==
                ...
                android_finalize_font_entity(
                    PSEUDOVEC_STRUCT(vector, font_entity));
#endif
        }
        break;
    case PVEC_THREAD:
    ...
    case PVEC_USER_PTR:
        {
            struct Lisp_User_Ptr* uptr =
                PSEUDOVEC_STRUCT(vector, Lisp_User_Ptr);
            if (uptr->finalizer)
    ...
    case PVEC_HASH_TABLE:
        {
            struct Lisp_Hash_Table* h =
                PSEUDOVEC_STRUCT(vector, Lisp_Hash_Table);
            if (h->table_size > 0) {
...
void visit_static_gc_roots(struct gc_root_visitor visitor) {
    ...
    visit_buffer_root(visitor, &buffer_local_symbols,
