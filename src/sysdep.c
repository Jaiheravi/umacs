...
static const int baud_convert[] = {0,    50,   75,    110,  135,  150,
...
int maybe_disable_address_randomization(int argc, char** argv) {
...
int emacs_exec_file(char const* file, char* const* argv, char* const* envp) {
#ifdef HAVE_PERSONALITY_ADDR_NO_RANDOMIZE
    if (exec_personality != -1)
...
static void force_open(int fd, int flags) {
...
static char* get_current_dir_name_or_unreachable(void) {
...
char* emacs_get_current_dir_name(void) {
...
void init_baud_rate(int fd) {
    ...
    baud_rate =
        (emacs_ospeed < ARRAYELTS(baud_convert) ? baud_convert[emacs_ospeed]
                                                : 9600);
    if (baud_rate == 0)
        baud_rate = 1200;
}
...
static pid_t get_child_status(pid_t child, int* status, int options,
                              bool interruptible) {
    pid_t pid;
...
bool wait_for_termination(pid_t child, int* status, bool interruptible) {
...
pid_t child_status_changed(pid_t child, int* status, int options) {
...
#ifndef DOS_NT
#ifdef F_SETOWN
static void reset_sigio(int fd) {
#ifdef USABLE_SIGIO
...
void block_child_signal(sigset_t* oldset) {
    sigset_t blocked;
    sigemptyset(&blocked);
...
void unblock_child_signal(sigset_t const* oldset) {
...
/* Block SIGINT.  */
void block_interrupt_signal(sigset_t* oldset) {
    sigset_t blocked;
    sigemptyset(&blocked);
    sigaddset(&blocked, SIGINT);
    pthread_sigmask(SIG_BLOCK, &blocked, oldset);
...
void restore_signal_mask(sigset_t const* oldset) {
    pthread_sigmask(SIG_SETMASK, oldset, 0);
}
...
void init_foreground_group(void) {
    pid_t pgrp = getpgrp();
...
void block_tty_out_signal(sigset_t* oldset) {
#ifdef SIGTTOU
    sigset_t blocked;
    sigemptyset(&blocked);
...
void unblock_tty_out_signal(sigset_t const* oldset) {
#ifdef SIGTTOU
    pthread_sigmask(SIG_SETMASK, oldset, 0);
#endif
}
...
static void tcsetpgrp_without_stopping(int fd, pid_t pgid) {
#ifdef SIGTTOU
    sigset_t oldset;
    block_input();
...
static void narrow_foreground_group(int fd) {
...
static void widen_foreground_group(int fd) {
...
int emacs_set_tty(int fd, struct emacs_tty* settings, bool flushp) {
    ...
    for (i = 0; i < 10; i++)
        if (tcsetattr(fd, flushp ? TCSAFLUSH : TCSADRAIN, &settings->main) <
            0) {
            if (errno == EINTR)
                continue;
            else
             ...
             * not be filled in.
             */
            if (new.c_iflag == settings->main.c_iflag&& new.c_oflag ==
                settings->main.c_oflag&& new.c_cflag ==
                settings->main.c_cflag&& new.c_lflag ==
                settings->main.c_lflag&& memcmp(new.c_cc, settings->main.c_cc,
                                                NCCS) == 0)
                break;
            else
                continue;
...
void init_sys_modes(struct tty_display_info* tty_out) {
...
#ifdef TIOCSTART
    if (!tty_out->flow_control)
        ioctl(fileno(tty_out->input), TIOCSTART, 0);
#endif

#if !defined(DOS_NT)
#ifdef TCOON
    if (!tty_out->flow_control)
        tcflow(fileno(tty_out->input), TCOON);
#endif
#endif
    ...
    const size_t buffer_size =
        (tty_out->output_buffer_size ? tty_out->output_buffer_size : BUFSIZ);
...
void suppress_echo_on_tty(int fd) {
    struct emacs_tty etty;
...
void get_tty_size(int fd, int* widthp, int* heightp) {
...
void reset_sys_modes(struct tty_display_info* tty_out) {
    if (noninteractive) {
        fflush(stdout);
        return;
    }
    ...
    if (tty_out->old_tty)
        while (emacs_set_tty(fileno(tty_out->input), tty_out->old_tty, 0) < 0 &&
               errno == EINTR)
            ;
...
static int emacs_sigaction_flags(void) {
#ifdef SA_RESTART
...
/* Store into *ACTION a signal action suitable for Emacs, with handler
   HANDLER.  */
void emacs_sigaction_init(struct sigaction* action, signal_handler_t handler) {
...
void deliver_process_signal(int sig, signal_handler_t handler) {
...
static void deliver_thread_signal(int sig, signal_handler_t handler) {
    int old_errno = errno;
...
static void handle_fatal_signal(int sig) { terminate_due_to_signal(sig, 40); }

static void deliver_fatal_signal(int sig) {
    deliver_process_signal(sig, handle_fatal_signal);
}

static void deliver_fatal_thread_signal(int sig) {
    deliver_thread_signal(sig, handle_fatal_signal);
}

static AVOID handle_arith_signal(int sig) {
...
static void handle_sigbus(int sig, siginfo_t* siginfo, void* arg) {
...
static max_align_t
    sigsegv_stack[(64 * 1024 + sizeof(max_align_t) - 1) / sizeof(max_align_t)];


/* Return true if SIGINFO indicates a stack overflow.  */

static bool stack_overflow(siginfo_t* siginfo) {
    ...
    enum
    {
        LG_STACK_HEURISTIC = 8
    };

    if (bot < top)
        return 0 <= addr - top &&
            addr - top < (top - bot) >> LG_STACK_HEURISTIC;
    else
        return 0 <= top - addr &&
            top - addr < (bot - top) >> LG_STACK_HEURISTIC;
}
...
static void handle_sigsegv(int sig, siginfo_t* siginfo, void* arg) {
...
static bool init_sigsegv(void) { return 0; }
...
static void maybe_fatal_sig(int sig) {
...
void seed_random(void* seed, ptrdiff_t seed_size) {
    random_seed arg = 0;
...
unsigned long int get_random_ulong(void) {
...
int snprintf(char* buf, size_t bufsize, char const* format, ...) {
...
void emacs_backtrace(int backtrace_limit) {
...
static
#endif
    int renameat_noreplace(int srcfd, char const* src, int dstfd,
                           char const* dst) {
...
#if !defined HAVE_NTGUI && !(defined HAVE_ANDROID && !defined ANDROID_STUBIFY)
void emacs_abort(void) { terminate_due_to_signal(SIGABRT, 40); }
#endif
...
int emacs_fstatat(int dirfd, char const* filename, void* st, int flags) {
    int r;
#if !(defined HAVE_ANDROID && !defined ANDROID_STUBIFY)
...
static int sys_openat(int dirfd, char const* file, int oflags, int mode) {
#ifdef O_PATH
...
int sys_fstat(int fd, struct stat* statb) {
#if !(defined HAVE_ANDROID && !defined ANDROID_STUBIFY)
...
int emacs_openat(int dirfd, char const* file, int oflags, int mode) {
    int fd;
    if (!(oflags & O_TEXT))
        oflags |= O_BINARY;
    oflags |= O_CLOEXEC;
...
int emacs_open(char const* file, int oflags, int mode) {
#if defined HAVE_ANDROID && !defined ANDROID_STUBIFY
    int fd;
#endif
...
int emacs_open_noquit(char const* file, int oflags, int mode) {
    int fd;
    if (!(oflags & O_TEXT))
        oflags |= O_BINARY;
    oflags |= O_CLOEXEC;
...
FILE* emacs_fopen(char const* file, char const* mode) {
    int fd, omode, oflags;
    int bflag = 0;
    char const* m = mode;
...
int emacs_pipe(int fd[2]) { return pipe2(fd, O_BINARY | O_CLOEXEC); }
...
static int posix_close(int fd, int flag) {
...
FILE* emacs_fdopen(int fd, const char* mode) {
#if !(defined HAVE_ANDROID && !defined ANDROID_STUBIFY)
...
#if defined HAVE_ANDROID && !defined ANDROID_STUBIFY
int emacs_fclose(FILE* stream) { return android_fclose(stream); }
#endif
...
int emacs_unlink(const char* name) {
#if !(defined HAVE_ANDROID && !defined ANDROID_STUBIFY)
    return unlink(name);
#else /* !defined HAVE_ANDROID || defined ANDROID_STUBIFY */
...
int emacs_symlink(const char* target, const char* linkname) {
#if !(defined HAVE_ANDROID && !defined ANDROID_STUBIFY)
...
int emacs_rmdir(const char* dirname) {
#if !(defined HAVE_ANDROID && !defined ANDROID_STUBIFY)
...
int emacs_mkdir(const char* dirname, mode_t mode) {
#if !(defined HAVE_ANDROID && !defined ANDROID_STUBIFY)
...
int emacs_renameat_noreplace(int srcfd, const char* src, int dstfd,
                             const char* dst) {
#if !(defined HAVE_ANDROID && !defined ANDROID_STUBIFY)
...
int emacs_rename(const char* src, const char* dst) {
#if !(defined HAVE_ANDROID && !defined ANDROID_STUBIFY)
...
int emacs_fchmodat(int fd, const char* path, mode_t mode, int flags) {
#if !(defined HAVE_ANDROID && !defined ANDROID_STUBIFY)
...
static ptrdiff_t emacs_intr_read(int fd, void* buf, ptrdiff_t nbyte,
                                 bool interruptible) {
...
ptrdiff_t emacs_read(int fd, void* buf, ptrdiff_t nbyte) {
...
/* Write to FILEDES from a buffer BUF with size NBYTE, retrying if
   ...
   other than EINTR.  */
static ptrdiff_t emacs_full_write(int fd, char const* buf, ptrdiff_t nbyte,
                                  int interruptible) {
...
ptrdiff_t emacs_write(int fd, void const* buf, ptrdiff_t nbyte) {
    return emacs_full_write(fd, buf, nbyte, 0);
}

/* Like emacs_write, but also process pending signals.  */
ptrdiff_t emacs_write_sig(int fd, void const* buf, ptrdiff_t nbyte) {
    return emacs_full_write(fd, buf, nbyte, -1);
}

/* Like emacs_write, but also process quits and pending signals.  */
ptrdiff_t emacs_write_quit(int fd, void const* buf, ptrdiff_t nbyte) {
    return emacs_full_write(fd, buf, nbyte, 1);
}
...
void emacs_perror(char const* message) {
    int err = errno;
...
char const* safe_strsignal(int code) {
...
void errwrite(void const* buf, ptrdiff_t nbuf) {
...
void close_output_streams(void) {
    ...
    if (err |
        (ADDRESS_SANITIZER ? fflush(stderr) != 0 || ferror(stderr)
...
int serial_open(Lisp_Object port) {
...
static int cfsetspeed(struct termios* termios_p, speed_t vitesse) {
    return (cfsetispeed(termios_p, vitesse) + cfsetospeed(termios_p, vitesse));
}
#endif
...
/* Convert a numerical speed (e.g., 9600) to a Bnnn constant (e.g.,
   B9600); see bug#49524.  */
static speed_t convert_speed(speed_t speed) {
...
void serial_configure(struct Lisp_Process* p, Lisp_Object contact) {
...
Lisp_Object list_system_processes(void) {
    ...
    proclist =
        directory_files_internal(procdir, Qnil, match, Qt, false, Qnil, Qnil);
...
Lisp_Object list_system_processes(void) { return Qnil; }

#endif /* !defined (WINDOWSNT) */

#if (HAVE_GETRUSAGE || defined __FreeBSD__ || defined DARWIN_OS ||             \
     defined __OpenBSD__)

static Lisp_Object make_lisp_s_us(time_t s, long us) {
...
static Lisp_Object make_lisp_timeval(struct timeval t) {
...
static Lisp_Object time_from_jiffies(unsigned long long ticks, Lisp_Object hz,
                                     Lisp_Object form) {
    return Ftime_convert(Fcons(make_uint(ticks), hz), form);
}

static Lisp_Object put_jiffies(Lisp_Object attrs, Lisp_Object propname,
...
static Lisp_Object get_host_uptime(Lisp_Object hz) {
...
static Lisp_Object procfs_ttyname(int rdev) {
    FILE* fdev;
    char name[PATH_MAX];
...
static uintmax_t procfs_get_total_memory(void) {
    FILE* fmem;
...
Lisp_Object system_process_attributes(Lisp_Object pid) {
    ...
    if (0 < nread) {
        ...
        decoded_cmd =
            code_convert_string_norecord(cmd_str, Vlocale_coding_system, 0);
       ...
       utime stime cutime cstime priority nice thcount . start vsize rss */
        if (q &&
            (sscanf(q + 2,
                    ...
                    &c, &ppid, &pgrp, &sess, &tty, &tpgid, &minflt, &cminflt,
                    &majflt, &cmajflt, &u_time, &s_time, &cutime, &cstime,
                    &priority, &niceness, &thcount, &start, &vsize,
                    &rss) == 20)) {
            char state_str[2];
            state_str[0] = c;
            state_str[1] = '\0';
...
Lisp_Object system_process_attributes(Lisp_Object pid) {
    ...
    if (nread == sizeof pinfo) {
       ...
       range 0 .. 2**15, representing 0.0 .. 1.0.  */
        attrs = Fcons(
            Fcons(Qpcpu, make_float(100.0 / 0x8000 * pinfo.pr_pctcpu)), attrs);
        attrs = Fcons(
            Fcons(Qpmem, make_float(100.0 / 0x8000 * pinfo.pr_pctmem)), attrs);

        AUTO_STRING(fname, pinfo.pr_fname);
        decoded_cmd =
            code_convert_string_norecord(fname, Vlocale_coding_system, 0);
        attrs = Fcons(Fcons(Qcomm, decoded_cmd), attrs);
        AUTO_STRING(psargs, pinfo.pr_psargs);
        decoded_cmd =
            code_convert_string_norecord(psargs, Vlocale_coding_system, 0);
        attrs = Fcons(Fcons(Qargs, decoded_cmd), attrs);
    }
...
Lisp_Object system_process_attributes(Lisp_Object pid) {
    ...
    attrs =
        Fcons(Fcons(Qminflt, INT_TO_INTEGER(proc.ki_rusage.ru_minflt)), attrs);
    attrs =
        Fcons(Fcons(Qmajflt, INT_TO_INTEGER(proc.ki_rusage.ru_majflt)), attrs);
    attrs =
        Fcons(Fcons(Qcminflt, make_fixnum(proc.ki_rusage_ch.ru_minflt)), attrs);
    attrs =
        Fcons(Fcons(Qcmajflt, make_fixnum(proc.ki_rusage_ch.ru_majflt)), attrs);
    ...
    attrs =
        Fcons(Fcons(Qrss, make_fixnum(proc.ki_rssize * pagesize >> 10)), attrs);
    ...
    if (sysctlbyname("hw.availpages", &npages, &len, NULL, 0) == 0) {
        double pmem =
            (proc.ki_flag & P_INMEM ? 100.0 * proc.ki_rssize / npages : 0);
    ...
    if (sysctl(mib, 4, args, &len, NULL, 0) == 0 && len != 0) {
        ...
        decoded_comm =
            code_convert_string_norecord(comm, Vlocale_coding_system, 0);
...
Lisp_Object system_process_attributes(Lisp_Object pid) {
    ...
    attrs = Fcons(
        Fcons(Qcutime, make_lisp_s_us(proc.p_uctime_sec, proc.p_uctime_usec)),
        attrs);
    ...
    attrs = Fcons(
        Fcons(Qstart, make_lisp_s_us(proc.p_ustart_sec, proc.p_ustart_usec)),
        attrs);

    len =
        (proc.p_vm_tsize + proc.p_vm_dsize + proc.p_vm_ssize) * pagesize >> 10;
    ...
    Lisp_Object now = Ftime_convert(Qnil, make_fixnum(1000000));
    Lisp_Object start = make_lisp_s_us(proc.p_ustart_sec, proc.p_ustart_usec);
    Lisp_Object etime = Ftime_convert(Ftime_subtract(now, start), Qnil);
    attrs = Fcons(Fcons(Qetime, etime), attrs);
...
Lisp_Object system_process_attributes(Lisp_Object pid) {
    ...
    decoded_comm = (code_convert_string_norecord(build_unibyte_string(comm),
    ...
    attrs = Fcons(Fcons(Qtpgid, INT_TO_INTEGER(proc.kp_eproc.e_tpgid)), attrs);
    ...
    if (proc_pid_rusage(proc_id, RUSAGE_INFO_CURRENT, (rusage_info_t*)&ri) ==
        ...
        attrs = Fcons(Fcons(Qtime, make_lisp_time(timespec_add(utime, stime))),
                      attrs);
    ...
    if (rusage) {
        attrs = Fcons(Fcons(Qminflt, INT_TO_INTEGER(rusage->ru_minflt)), attrs);
        attrs = Fcons(Fcons(Qmajflt, INT_TO_INTEGER(rusage->ru_majflt)), attrs);
    ...
    if (proc_pidinfo(proc_id, PROC_PIDTASKINFO, 0, &taskinfo,
                     sizeof(taskinfo)) > 0) {
        attrs =
            Fcons(Fcons(Qvsize, make_fixnum(taskinfo.pti_virtual_size / 1024)),
                  attrs);
        attrs = Fcons(
            Fcons(Qrss, make_fixnum(taskinfo.pti_resident_size / 1024)), attrs);
        attrs =
            Fcons(Fcons(Qthcount, make_fixnum(taskinfo.pti_threadnum)), attrs);
    }
...
Lisp_Object system_process_attributes(Lisp_Object pid) { return Qnil; }
...
static locale_t newlocale(int category_mask, char const* locale, locale_t loc) {
    return locale;
}

static void freelocale(locale_t loc) {}

static char* emacs_setlocale(int category, char const* locale) {
    errno = 0;
...
static int wcscoll_l(wchar_t const* a, wchar_t const* b, locale_t loc) {
    int result = 0;
...
static wint_t towlower_l(wint_t wc, locale_t loc) {
    wint_t result = wc;
...
int str_collate(Lisp_Object s1, Lisp_Object s2, Lisp_Object locale,
    ...
    if (STRINGP(locale)) {
        locale_t loc =
            newlocale(LC_COLLATE_MASK | LC_CTYPE_MASK, SSDATA(locale), 0);
        if (!loc)
            error("Invalid locale %s: %s", SSDATA(locale),
...
#ifndef HAVE_NEWLOCALE
    if (err)
        error("Invalid locale or string for collation: %s",
              emacs_strerror(err));
#else
    if (err)
...
void syms_of_sysdep(void) { defsubr(&Sget_internal_run_time); }
