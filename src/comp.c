...
static bool init_gccjit_functions(void) {
...
static bool load_gccjit_if_necessary(bool mandatory) {
#ifdef WINDOWSNT
...
#define CALLNI(fun, ...) calln(intern_c_string(STR(fun)), __VA_ARGS__)

#define DECL_BLOCK(name, func)                                                 \
    gcc_jit_block*(name) = gcc_jit_function_new_block(func, STR(name))
...
typedef struct {
    ...
    gcc_jit_lvalue*
        scratch; /* Used as scratch slot for some code sequence (switch).  */
...
static Lisp_Object helper_sanitizer_assert(Lisp_Object, Lisp_Object);

/* Note: helper_link_table must match the list created by
   `declare_runtime_imported_funcs'.  */
static void* helper_link_table[] = {wrong_type_argument,
...
static Lisp_Object comp_hash_string(Lisp_Object string) {
...
static Lisp_Object comp_hash_source_file(Lisp_Object filename) {
...
#ifdef HAVE_ZLIB
    int res = is_gz ? md5_gz_stream(f, SSDATA(digest))
...
static void freloc_check_fill(void) {
    if (freloc.size)
        return;
...
static void bcall0(Lisp_Object f) { calln(f); }

static gcc_jit_block* retrieve_block(Lisp_Object block_name) {
...
static void declare_block(Lisp_Object block_name) {
...
static gcc_jit_lvalue* emit_mvar_lval(Lisp_Object mvar) {
    Lisp_Object mvar_slot = CALLNI(comp - mvar - slot, mvar);

    if (EQ(mvar_slot, Qscratch)) {
        if (!comp.scratch)
            comp.scratch = gcc_jit_function_new_local(
                comp.func, NULL, comp.lisp_obj_type, "scratch");
        return comp.scratch;
    }
...
static void register_emitter(Lisp_Object key, void* func) {
    Lisp_Object value = make_mint_ptr(func);
    Fputhash(key, value, comp.emitter_dispatcher);
}

static imm_reloc_t obj_to_reloc(Lisp_Object obj) {
    imm_reloc_t reloc;
    Lisp_Object idx;
...
found:
    ...
    reloc.idx = gcc_jit_context_new_rvalue_from_int(
        comp.ctxt, comp.ptrdiff_type, XFIXNUM(idx));
    return reloc;
}

static void emit_comment(const char* str) {
    if (comp.debug)
        gcc_jit_block_add_comment(comp.block, NULL, str);
}
...
*/
static gcc_jit_field* declare_imported_func(Lisp_Object subr_sym,
                                            gcc_jit_type* ret_type, int nargs,
                                            gcc_jit_type** types) {
    USE_SAFE_ALLOCA;
    ...
    Lisp_Object f_ptr_name = calln(intern_c_string("comp-c-func-name"),
                                   subr_sym, make_string("R", 1));

    gcc_jit_type* f_ptr_type =
        gcc_jit_type_get_const(gcc_jit_context_new_function_ptr_type(
            comp.ctxt, NULL, ret_type, nargs, types, 0));
    gcc_jit_field* field = gcc_jit_context_new_field(
        comp.ctxt, NULL, f_ptr_type, SSDATA(f_ptr_name));
...
static gcc_jit_rvalue* emit_call(Lisp_Object func, gcc_jit_type* ret_type,
                                 ptrdiff_t nargs, gcc_jit_rvalue** args,
                                 bool direct) {
    Lisp_Object gcc_func = Fgethash(
        func, direct ? comp.exported_funcs_h : comp.imported_funcs_h, Qnil);

    if (NILP(gcc_func))
        xsignal2(Qnative_ice, build_string("missing function declaration"),
                 func);

    if (direct) {
        emit_comment(format_string("direct call to: %s", SSDATA(func)));
        return gcc_jit_context_new_call(comp.ctxt, NULL,
                                        xmint_pointer(gcc_func), nargs, args);
    }
    else {
       ...
       relying on GCC. */
        gcc_jit_lvalue* f_ptr = gcc_jit_rvalue_dereference_field(
            gcc_jit_lvalue_as_rvalue(comp.func_relocs_local
                                         ? comp.func_relocs_local
                                         : comp.func_relocs),
            NULL, (gcc_jit_field*)xmint_pointer(gcc_func));

        if (!f_ptr)
            xsignal2(Qnative_ice, build_string("missing function relocation"),
                     func);
        emit_comment(
            format_string("calling subr: %s", SSDATA(SYMBOL_NAME(func))));
        return gcc_jit_context_new_call_through_ptr(
            comp.ctxt, NULL, gcc_jit_lvalue_as_rvalue(f_ptr), nargs, args);
    }
}

static gcc_jit_rvalue* emit_call_ref(Lisp_Object func, ptrdiff_t nargs,
                                     gcc_jit_lvalue* base_arg, bool direct) {
    gcc_jit_rvalue* args[] = {gcc_jit_context_new_rvalue_from_int(
                                  comp.ctxt, comp.ptrdiff_type, nargs),
...
static void emit_cond_jump(gcc_jit_rvalue* test, gcc_jit_block* then_target,
                           gcc_jit_block* else_target) {
    if (gcc_jit_rvalue_get_type(test) == comp.bool_type)
        gcc_jit_block_end_with_conditional(comp.block, NULL, test, then_target,
                                           else_target);
    else
        ...
        gcc_jit_block_end_with_conditional(
            comp.block, NULL,
            gcc_jit_context_new_unary_op(comp.ctxt, NULL,
...
#ifndef LIBGCCJIT_HAVE_gcc_jit_context_new_bitcast
static int type_to_cast_index(gcc_jit_type* type) {
...
static gcc_jit_rvalue* emit_coerce(gcc_jit_type* new_type,
    ...
    if (new_type == comp.lisp_obj_type) {
        gcc_jit_rvalue* lwordobj = emit_coerce(comp.lisp_word_type, obj);
        ...
        gcc_jit_block_add_assignment(
            comp.block, NULL,
            gcc_jit_lvalue_access_field(tmp_s, NULL, comp.lisp_obj_i),
            lwordobj);
    ...
    return gcc_jit_context_new_call(
        comp.ctxt, NULL, comp.cast_functions_from_to[old_index][new_index], 1,
        &obj);
#endif
}

static gcc_jit_rvalue* emit_binary_op(enum gcc_jit_binary_op op,
                                      gcc_jit_type* result_type,
                                      gcc_jit_rvalue* a, gcc_jit_rvalue* b) {
    /* FIXME Check here for possible UB.  */
    return gcc_jit_context_new_binary_op(comp.ctxt, NULL, op, result_type,
...
static gcc_jit_rvalue* emit_rvalue_from_long_long(gcc_jit_type* type,
                                                  long long n) {
    emit_comment(format_string("emit long long: %lld", n));

    gcc_jit_rvalue* high = gcc_jit_context_new_rvalue_from_long(
        comp.ctxt, comp.unsigned_long_long_type, (unsigned long long)n >> 32);
    gcc_jit_rvalue* low = emit_binary_op(
        GCC_JIT_BINARY_OP_RSHIFT, comp.unsigned_long_long_type,
        emit_binary_op(GCC_JIT_BINARY_OP_LSHIFT, comp.unsigned_long_long_type,
                       gcc_jit_context_new_rvalue_from_long(
                           comp.ctxt, comp.unsigned_long_long_type, n),
                       gcc_jit_context_new_rvalue_from_int(
                           comp.ctxt, comp.unsigned_long_long_type, 32)),
        gcc_jit_context_new_rvalue_from_int(comp.ctxt,
                                            comp.unsigned_long_long_type, 32));

    return emit_coerce(
        type,
        emit_binary_op(
            GCC_JIT_BINARY_OP_BITWISE_OR, comp.unsigned_long_long_type,
            emit_binary_op(GCC_JIT_BINARY_OP_LSHIFT,
                           comp.unsigned_long_long_type, high,
                           gcc_jit_context_new_rvalue_from_int(
                               comp.ctxt, comp.unsigned_long_long_type, 32)),
            low));
}

static gcc_jit_rvalue* emit_rvalue_from_emacs_uint(EMACS_UINT val) {
#ifdef WIDE_EMACS_INT
    if (val > ULONG_MAX)
        return emit_rvalue_from_long_long(comp.emacs_uint_type, val);
#endif
    return gcc_jit_context_new_rvalue_from_long(comp.ctxt, comp.emacs_uint_type,
                                                val);
}

static gcc_jit_rvalue* emit_rvalue_from_emacs_int(EMACS_INT val) {
...
static gcc_jit_rvalue* emit_rvalue_from_lisp_word_tag(Lisp_Word_tag val) {
#ifdef WIDE_EMACS_INT
    if (val > ULONG_MAX)
        return emit_rvalue_from_long_long(comp.lisp_word_tag_type, val);
#endif
    return gcc_jit_context_new_rvalue_from_long(comp.ctxt,
                                                comp.lisp_word_tag_type, val);
}

static gcc_jit_rvalue* emit_rvalue_from_lisp_word(Lisp_Word val) {
#if LISP_WORDS_ARE_POINTERS
    return gcc_jit_context_new_rvalue_from_ptr(comp.ctxt, comp.lisp_word_type,
                                               val);
#else
    if (val > LONG_MAX || val < LONG_MIN)
        ...
        return gcc_jit_context_new_rvalue_from_long(comp.ctxt,
                                                    comp.lisp_word_type, val);
#endif
}

static gcc_jit_rvalue* emit_rvalue_from_lisp_obj(Lisp_Object obj) {
#ifdef LISP_OBJECT_IS_STRUCT
    return emit_coerce(comp.lisp_obj_type, emit_rvalue_from_lisp_word(obj.i));
#else
...
static gcc_jit_rvalue* emit_ptr_arithmetic(gcc_jit_rvalue* ptr,
    ...
    gcc_jit_rvalue* offset =
        emit_binary_op(GCC_JIT_BINARY_OP_MULT, comp.uintptr_type,
                       gcc_jit_context_new_rvalue_from_int(
                           comp.ctxt, comp.uintptr_type, size_of_ptr_ref),
                       i);

    return emit_coerce(
        ptr_type,
        emit_binary_op(GCC_JIT_BINARY_OP_PLUS, comp.uintptr_type, ptr, offset));
}

static gcc_jit_rvalue* emit_XLI(gcc_jit_rvalue* obj) {
    emit_comment("XLI");
    return emit_coerce(comp.emacs_int_type, obj);
}

static gcc_jit_rvalue* emit_XLP(gcc_jit_rvalue* obj) {
    emit_comment("XLP");

    return emit_coerce(comp.void_ptr_type, obj);
}

static gcc_jit_rvalue* emit_XUNTAG(gcc_jit_rvalue* a, gcc_jit_type* type,
    ...
    return emit_coerce(
        ...
        emit_binary_op(GCC_JIT_BINARY_OP_MINUS, comp.uintptr_type, emit_XLP(a),
                       emit_rvalue_from_lisp_word_tag(lisp_word_tag)));
}

static gcc_jit_rvalue* emit_XCONS(gcc_jit_rvalue* a) {
    emit_comment("XCONS");

    return emit_XUNTAG(a, gcc_jit_struct_as_type(comp.lisp_cons_s),
                       LISP_WORD_TAG(Lisp_Cons));
}

static gcc_jit_rvalue* emit_BASE_EQ(gcc_jit_rvalue* x, gcc_jit_rvalue* y) {
    emit_comment("BASE_EQ");

    return gcc_jit_context_new_comparison(
        comp.ctxt, NULL, GCC_JIT_COMPARISON_EQ, emit_XLI(x), emit_XLI(y));
}

static gcc_jit_rvalue* emit_AND(gcc_jit_rvalue* x, gcc_jit_rvalue* y) {
    return gcc_jit_context_new_binary_op(
        comp.ctxt, NULL, GCC_JIT_BINARY_OP_LOGICAL_AND, comp.bool_type, x, y);
}

static gcc_jit_rvalue* emit_OR(gcc_jit_rvalue* x, gcc_jit_rvalue* y) {
    return gcc_jit_context_new_binary_op(
        comp.ctxt, NULL, GCC_JIT_BINARY_OP_LOGICAL_OR, comp.bool_type, x, y);
}

static gcc_jit_rvalue* emit_TAGGEDP(gcc_jit_rvalue* obj, Lisp_Word_tag tag) {
    /* (! (((unsigned) (XLI (a) >> (USE_LSB_TAG ? 0 : VALBITS)) \
     - (unsigned) (tag)) \
     & ((1 << GCTYPEBITS) - 1))) */
    emit_comment("TAGGEDP");

    gcc_jit_rvalue* sh_res = emit_binary_op(
        GCC_JIT_BINARY_OP_RSHIFT, comp.emacs_int_type, emit_XLI(obj),
        gcc_jit_context_new_rvalue_from_int(comp.ctxt, comp.emacs_int_type,
                                            (USE_LSB_TAG ? 0 : VALBITS)));

    gcc_jit_rvalue* minus_res =
        emit_binary_op(GCC_JIT_BINARY_OP_MINUS, comp.unsigned_type, sh_res,
                       gcc_jit_context_new_rvalue_from_int(
                           comp.ctxt, comp.unsigned_type, tag));

    gcc_jit_rvalue* res = gcc_jit_context_new_unary_op(
        comp.ctxt, NULL, GCC_JIT_UNARY_OP_LOGICAL_NEGATE, comp.int_type,
        emit_binary_op(
            GCC_JIT_BINARY_OP_BITWISE_AND, comp.unsigned_type, minus_res,
            gcc_jit_context_new_rvalue_from_int(comp.ctxt, comp.unsigned_type,
                                                ((1 << GCTYPEBITS) - 1))));

    return res;
}

static gcc_jit_rvalue* emit_VECTORLIKEP(gcc_jit_rvalue* obj) {
    emit_comment("VECTORLIKEP");

    return emit_TAGGEDP(obj, Lisp_Vectorlike);
}

static gcc_jit_rvalue* emit_CONSP(gcc_jit_rvalue* obj) {
    emit_comment("CONSP");

    return emit_TAGGEDP(obj, Lisp_Cons);
}

static gcc_jit_rvalue* emit_SYMBOL_WITH_POS_P(gcc_jit_rvalue* obj) {
    ...
    return gcc_jit_context_new_call(comp.ctxt, NULL, comp.pseudovectorp, 2,
                                    args);
}

static gcc_jit_rvalue* emit_slow_eq(gcc_jit_rvalue* x, gcc_jit_rvalue* y) {
    gcc_jit_rvalue* args[] = {emit_coerce(comp.lisp_obj_type, x),
                              emit_coerce(comp.lisp_obj_type, y)};

    return emit_call(intern_c_string("slow_eq"), comp.bool_type,
                     ARRAYELTS(args), args, false);
}

static gcc_jit_rvalue* emit_EQ(gcc_jit_rvalue* x, gcc_jit_rvalue* y) {
    gcc_jit_rvalue* base_eq = emit_BASE_EQ(x, y);
    gcc_jit_rvalue* symbols_with_pos_enabled_rval = gcc_jit_lvalue_as_rvalue(
        gcc_jit_rvalue_dereference(comp.f_symbols_with_pos_enabled_ref, NULL));
    ...
    if (!NILP(version) && XFIXNUM(XCAR(version)) >= 14) {
        gcc_jit_rvalue* expect_args[] = {
            ...
            gcc_jit_context_new_rvalue_from_int(comp.ctxt, comp.long_type, 0)};

        symbols_with_pos_enabled_rval = emit_coerce(
            comp.bool_type,
            gcc_jit_context_new_call(comp.ctxt, NULL,
                                     gcc_jit_context_get_builtin_function(
                                         comp.ctxt, "__builtin_expect"),
                                     2, expect_args));
    }

    return emit_OR(base_eq,
                   emit_AND(symbols_with_pos_enabled_rval, emit_slow_eq(x, y)));
}

static gcc_jit_rvalue* emit_FLOATP(gcc_jit_rvalue* obj) {
    emit_comment("FLOATP");

    return emit_TAGGEDP(obj, Lisp_Float);
}

static gcc_jit_rvalue* emit_BIGNUMP(gcc_jit_rvalue* obj) {
    /* PSEUDOVECTORP (x, PVEC_BIGNUM); */
    emit_comment("BIGNUMP");

    gcc_jit_rvalue* args[] = {obj,
                              gcc_jit_context_new_rvalue_from_int(
                                  comp.ctxt, comp.int_type, PVEC_BIGNUM)};

    return gcc_jit_context_new_call(comp.ctxt, NULL, comp.pseudovectorp, 2,
                                    args);
}

static gcc_jit_rvalue* emit_FIXNUMP(gcc_jit_rvalue* obj) {
    /* (! (((unsigned) (XLI (x) >> (USE_LSB_TAG ? 0 : FIXNUM_BITS))
      - (unsigned) (Lisp_Int0 >> !USE_LSB_TAG))
      & ((1 << INTTYPEBITS) - 1)))  */
    emit_comment("FIXNUMP");

    gcc_jit_rvalue* sh_res = USE_LSB_TAG
        ? obj
        : emit_binary_op(GCC_JIT_BINARY_OP_RSHIFT, comp.emacs_int_type,
                         emit_XLI(obj),
                         gcc_jit_context_new_rvalue_from_int(
                             comp.ctxt, comp.emacs_int_type, FIXNUM_BITS));

    gcc_jit_rvalue* minus_res = emit_binary_op(
        GCC_JIT_BINARY_OP_MINUS, comp.unsigned_type, sh_res,
        gcc_jit_context_new_rvalue_from_int(comp.ctxt, comp.unsigned_type,
...
static gcc_jit_rvalue* emit_XFIXNUM(gcc_jit_rvalue* obj) {
    ...
    if (!USE_LSB_TAG) {
        i = emit_binary_op(GCC_JIT_BINARY_OP_LSHIFT, comp.emacs_uint_type, i,
                           comp.inttypebits);

        return emit_binary_op(GCC_JIT_BINARY_OP_RSHIFT, comp.emacs_int_type, i,
                              comp.inttypebits);
    }
    else
        return emit_coerce(comp.emacs_int_type,
                           emit_binary_op(GCC_JIT_BINARY_OP_RSHIFT,
                                          comp.emacs_int_type, i,
                                          comp.inttypebits));
}

static gcc_jit_rvalue* emit_INTEGERP(gcc_jit_rvalue* obj) {
    emit_comment("INTEGERP");

    return emit_binary_op(GCC_JIT_BINARY_OP_LOGICAL_OR, comp.bool_type,
                          emit_FIXNUMP(obj), emit_BIGNUMP(obj));
}

static gcc_jit_rvalue* emit_NUMBERP(gcc_jit_rvalue* obj) {
    emit_comment("NUMBERP");

    return emit_binary_op(GCC_JIT_BINARY_OP_LOGICAL_OR, comp.bool_type,
                          emit_INTEGERP(obj), emit_FLOATP(obj));
}

static gcc_jit_rvalue* emit_make_fixnum_LSB_TAG(gcc_jit_rvalue* n) {
    /*
      EMACS_UINT u = n;
      n = u << INTTYPEBITS;
      n += int0;
    */
    ...
    gcc_jit_rvalue* tmp = emit_binary_op(
        GCC_JIT_BINARY_OP_LSHIFT, comp.emacs_int_type, n, comp.inttypebits);

    tmp = emit_binary_op(GCC_JIT_BINARY_OP_PLUS, comp.emacs_int_type, tmp,
                         comp.lisp_int0);

    return emit_coerce(comp.lisp_obj_type, tmp);
}

static gcc_jit_rvalue* emit_make_fixnum_MSB_TAG(gcc_jit_rvalue* n) {
    /*
      n &= INTMASK;
      n += (int0 << VALBITS);
      return XIL (n);
    */
    ...
    n = emit_binary_op(GCC_JIT_BINARY_OP_BITWISE_AND, comp.emacs_uint_type,
                       intmask, n);

    n = emit_binary_op(GCC_JIT_BINARY_OP_PLUS, comp.emacs_uint_type,
                       emit_binary_op(GCC_JIT_BINARY_OP_LSHIFT,
                                      comp.emacs_uint_type, comp.lisp_int0,
...
static gcc_jit_rvalue* emit_NILP(gcc_jit_rvalue* x) {
    emit_comment("NILP");
    return emit_BASE_EQ(x, emit_lisp_obj_rval(Qnil));
}

static gcc_jit_rvalue* emit_XCAR(gcc_jit_rvalue* c) {
    emit_comment("XCAR");
    ...
    return gcc_jit_rvalue_access_field(
        ...
        gcc_jit_rvalue_access_field(
            ...
            gcc_jit_lvalue_as_rvalue(gcc_jit_rvalue_dereference_field(
                emit_XCONS(c), NULL, comp.lisp_cons_u)),
            NULL, comp.lisp_cons_u_s),
        NULL, comp.lisp_cons_u_s_car);
}

static gcc_jit_lvalue* emit_lval_XCAR(gcc_jit_rvalue* c) {
    ...
    return gcc_jit_lvalue_access_field(
        ...
        gcc_jit_lvalue_access_field(
            /* XCONS (c)->u */
            gcc_jit_rvalue_dereference_field(emit_XCONS(c), NULL,
                                             comp.lisp_cons_u),
            NULL, comp.lisp_cons_u_s),
        NULL, comp.lisp_cons_u_s_car);
}

static gcc_jit_rvalue* emit_XCDR(gcc_jit_rvalue* c) {
    emit_comment("XCDR");
    /* XCONS (c)->u.s.u.cdr */
    return gcc_jit_rvalue_access_field(
        /* XCONS (c)->u.s.u */
...
static gcc_jit_lvalue* emit_lval_XCDR(gcc_jit_rvalue* c) {
...
static void emit_CHECK_CONS(gcc_jit_rvalue* x) {
    emit_comment("CHECK_CONS");

    gcc_jit_rvalue* args[] = {emit_CONSP(x), emit_lisp_obj_rval(Qconsp), x};

    gcc_jit_block_add_eval(
        comp.block, NULL,
        gcc_jit_context_new_call(comp.ctxt, NULL, comp.check_type, 3, args));
}

static void emit_CHECK_SYMBOL_WITH_POS(gcc_jit_rvalue* x) {
    emit_comment("CHECK_SYMBOL_WITH_POS");

    gcc_jit_rvalue* args[] = {
        gcc_jit_context_new_cast(comp.ctxt, NULL, emit_SYMBOL_WITH_POS_P(x),
                                 comp.int_type),
        emit_lisp_obj_rval(Qsymbol_with_pos_p), x};
...
static gcc_jit_rvalue* emit_car_addr(gcc_jit_rvalue* c) {
    emit_comment("car_addr");

    return gcc_jit_lvalue_get_address(emit_lval_XCAR(c), NULL);
}

static gcc_jit_rvalue* emit_cdr_addr(gcc_jit_rvalue* c) {
    emit_comment("cdr_addr");

    return gcc_jit_lvalue_get_address(emit_lval_XCDR(c), NULL);
}

static void emit_XSETCAR(gcc_jit_rvalue* c, gcc_jit_rvalue* n) {
    emit_comment("XSETCAR");

    gcc_jit_block_add_assignment(
        comp.block, NULL, gcc_jit_rvalue_dereference(emit_car_addr(c), NULL),
        n);
}

static void emit_XSETCDR(gcc_jit_rvalue* c, gcc_jit_rvalue* n) {
    emit_comment("XSETCDR");

    gcc_jit_block_add_assignment(
        comp.block, NULL, gcc_jit_rvalue_dereference(emit_cdr_addr(c), NULL),
        n);
}
...
static gcc_jit_rvalue* emit_mvar_rval(Lisp_Object mvar) {
    ...
    if (!NILP(const_vld)) {
        ...
        if (NILP(Fgethash(value,
                          CALLNI(comp - ctxt - non - materializable - objs - h,
                                 Vcomp_ctxt),
                          Qnil))) {
            if (comp.debug > 1) {
                Lisp_Object func =
                    Fgethash(value,
                             CALLNI(comp - ctxt - byte - func - to - func - h,
                                    Vcomp_ctxt),
                             Qnil);

                emit_comment(SSDATA(Fprin1_to_string(
                    NILP(func) ? value : CALLNI(comp - func - c - name, func),
                    Qnil, Qnil)));
            }
            if (FIXNUMP(value)) {
                /* We can still emit directly objects that are self-contained in
               a word (read fixnums).  */
...
static void emit_frame_assignment(Lisp_Object dst_mvar, gcc_jit_rvalue* val) {

    gcc_jit_block_add_assignment(comp.block, NULL, emit_mvar_lval(dst_mvar),
                                 val);
}

static gcc_jit_rvalue* emit_set_internal(Lisp_Object args) {
    /*
    ...
    */
    ...
    if (list_length(args) != 3)
        xsignal2(Qnative_ice, build_string("unexpected arg length for insns"),
                 args);
    ...
    gcc_args[3] = gcc_jit_context_new_rvalue_from_int(comp.ctxt, comp.int_type,
                                                      SET_INTERNAL_SET);
...
static gcc_jit_rvalue* emit_simple_limple_call_lisp_ret(Lisp_Object args) {
    /*
...
static gcc_jit_rvalue* emit_simple_limple_call_void_ret(Lisp_Object args) {
    return emit_simple_limple_call(args, comp.void_type, false);
}

/* Entry point to dispatch emitting (call fun ...).  */

static gcc_jit_rvalue* emit_limple_call(Lisp_Object insn) {
...
static gcc_jit_rvalue* emit_limple_call_ref(Lisp_Object insn, bool direct) {
    ...
    gcc_jit_lvalue* tmp_arr = gcc_jit_function_new_local(
        comp.func, NULL,
        gcc_jit_context_new_array_type(comp.ctxt, NULL, comp.lisp_obj_type,
                                       nargs),
    ...
    FOR_EACH_TAIL(arg) {
        gcc_jit_block_add_assignment(
            ...
            gcc_jit_context_new_array_access(
                comp.ctxt, NULL, gcc_jit_lvalue_as_rvalue(tmp_arr),
                gcc_jit_context_new_rvalue_from_int(comp.ctxt, comp.int_type,
                                                    j)),
    ...
    return emit_call_ref(
        callee, nargs,
        gcc_jit_context_new_array_access(
            comp.ctxt, NULL, gcc_jit_lvalue_as_rvalue(tmp_arr), comp.zero),
        direct);
}

static gcc_jit_rvalue* emit_setjmp(gcc_jit_rvalue* buf) {
#ifndef WINDOWSNT
    ...
    gcc_jit_function* f = gcc_jit_context_new_function(
        comp.ctxt, NULL, GCC_JIT_FUNCTION_IMPORTED, comp.int_type,
        STR(SETJMP_NAME), ARRAYELTS(params), params, false);
...
static void emit_limple_push_handler(gcc_jit_rvalue* handler,
                                     gcc_jit_rvalue* handler_type,
                                     gcc_jit_block* handler_bb,
                                     gcc_jit_block* guarded_bb,
                                     Lisp_Object clobbered_mvar) {
    /* struct handler *c = push_handler (POP, type);  */

    gcc_jit_rvalue* args[] = {handler, handler_type};
    gcc_jit_block_add_assignment(comp.block, NULL, comp.loc_handler,
                                 emit_call(intern_c_string("push_handler"),
                                           comp.handler_ptr_type, 2, args,
                                           false));

    args[0] = gcc_jit_lvalue_get_address(
        gcc_jit_rvalue_dereference_field(
            gcc_jit_lvalue_as_rvalue(comp.loc_handler), NULL,
...
static void emit_limple_insn(Lisp_Object insn) {
    ...
    if (EQ(op, Qjump)) {
        ...
        */
        ...
        gcc_jit_rvalue* n = gcc_jit_context_new_rvalue_from_int(
            comp.ctxt, comp.ptrdiff_type, XFIXNUM(arg[0]));
        ...
        gcc_jit_rvalue* test = gcc_jit_context_new_comparison(
            ...
            gcc_jit_lvalue_as_rvalue(nargs), n);
        ...
        gcc_jit_rvalue* handler_type = gcc_jit_context_new_rvalue_from_int(
            comp.ctxt, comp.int_type, h_num);
      ...
      C: current_thread->m_handlerlist =
        ...
        */
        gcc_jit_lvalue* m_handlerlist = gcc_jit_rvalue_dereference_field(
            ...
            NULL, comp.m_handlerlist);

        gcc_jit_block_add_assignment(
            comp.block, NULL, m_handlerlist,
            gcc_jit_lvalue_as_rvalue(gcc_jit_rvalue_dereference_field(
                gcc_jit_lvalue_as_rvalue(m_handlerlist), NULL,
        ...
        gcc_jit_block_add_eval(comp.block, NULL, emit_limple_call(args));
    }
        ...
        if (EQ(Fcl_type_of(arg1), Qcomp_mvar))
            ...
            xsignal2(Qnative_ice,
                     build_string("LIMPLE inconsistent arg1 for insn"), insn);
        ...
        gcc_jit_rvalue* param = gcc_jit_param_as_rvalue(
            gcc_jit_function_get_param(comp.func, param_n));
      ...
      C: local[1] = *args;
        */
        gcc_jit_rvalue* gcc_args = gcc_jit_lvalue_as_rvalue(
            gcc_jit_param_as_lvalue(gcc_jit_function_get_param(comp.func, 1)));

        gcc_jit_rvalue* res = gcc_jit_lvalue_as_rvalue(
            gcc_jit_rvalue_dereference(gcc_args, NULL));
        ...
        */
        ...
        gcc_jit_rvalue* n = gcc_jit_context_new_rvalue_from_int(
            comp.ctxt, comp.ptrdiff_type, slot_n);
        gcc_jit_lvalue* nargs =
            gcc_jit_param_as_lvalue(gcc_jit_function_get_param(comp.func, 0));
        gcc_jit_lvalue* args =
            gcc_jit_param_as_lvalue(gcc_jit_function_get_param(comp.func, 1));
        ...
        gcc_jit_rvalue* list_args[] = {
            emit_binary_op(GCC_JIT_BINARY_OP_MINUS, comp.ptrdiff_type,
                           gcc_jit_lvalue_as_rvalue(nargs), n),
            gcc_jit_lvalue_as_rvalue(args)};

        res = emit_call(Qlist, comp.lisp_obj_type, 2, list_args, false);
      ...
      C: ++args;
        */
        ...
        gcc_jit_block_add_assignment(
            comp.block, NULL, args,
            emit_ptr_arithmetic(gcc_jit_lvalue_as_rvalue(args),
                                comp.lisp_obj_ptr_type, sizeof(Lisp_Object),
                                comp.one));
    }
        ...
        emit_frame_assignment(
            ...
            gcc_jit_lvalue_as_rvalue(gcc_jit_context_new_array_access(
                comp.ctxt, NULL, reloc.array.r_val, reloc.idx)));
    }
        ...
        gcc_jit_block_end_with_return(comp.block, NULL, emit_mvar_rval(arg[0]));
    }
    else if (EQ(op, Qunreachable)) {
        /* Libgccjit has no __builtin_unreachable.  */
        gcc_jit_block_end_with_return(comp.block, NULL,
                                      emit_lisp_obj_rval(Qnil));
    }
    else {
        xsignal2(Qnative_ice, build_string("LIMPLE op inconsistent"), op);
    }
}
...
static gcc_jit_rvalue* emit_call_with_type_hint(gcc_jit_function* func,
                                                Lisp_Object insn,
                                                Lisp_Object type) {
    bool hint_match = !comp.func_safety &&
        !NILP(
            CALLNI(comp - mvar - type - hint - match - p, SECOND(insn), type));
    gcc_jit_rvalue* args[] = {emit_mvar_rval(SECOND(insn)),
                              gcc_jit_context_new_rvalue_from_int(
                                  comp.ctxt, comp.bool_type, hint_match)};

    return gcc_jit_context_new_call(comp.ctxt, NULL, func, 2, args);
}

/* Same as before but with two args. The type hint is on the 2th.  */
static gcc_jit_rvalue* emit_call2_with_type_hint(gcc_jit_function* func,
                                                 Lisp_Object insn,
                                                 Lisp_Object type) {
    bool hint_match = !comp.func_safety &&
        !NILP(
...
static gcc_jit_rvalue* emit_add1(Lisp_Object insn) {
    return emit_call_with_type_hint(comp.add1, insn, Qfixnum);
}

static gcc_jit_rvalue* emit_sub1(Lisp_Object insn) {
    return emit_call_with_type_hint(comp.sub1, insn, Qfixnum);
}

static gcc_jit_rvalue* emit_negate(Lisp_Object insn) {
    return emit_call_with_type_hint(comp.negate, insn, Qfixnum);
}

static gcc_jit_rvalue* emit_consp(Lisp_Object insn) {
    gcc_jit_rvalue* x = emit_mvar_rval(SECOND(insn));
    gcc_jit_rvalue* res = emit_coerce(comp.bool_type, emit_CONSP(x));
    return gcc_jit_context_new_call(comp.ctxt, NULL, comp.bool_to_lisp_obj, 1,
                                    &res);
}

static gcc_jit_rvalue* emit_eq(Lisp_Object insn) {
    gcc_jit_rvalue* x = emit_mvar_rval(SECOND(insn));
...
static gcc_jit_rvalue* emit_car(Lisp_Object insn) {
    return emit_call_with_type_hint(comp.car, insn, Qcons);
}

static gcc_jit_rvalue* emit_cdr(Lisp_Object insn) {
    return emit_call_with_type_hint(comp.cdr, insn, Qcons);
}

static gcc_jit_rvalue* emit_setcar(Lisp_Object insn) {
    return emit_call2_with_type_hint(comp.setcar, insn, Qcons);
}

static gcc_jit_rvalue* emit_setcdr(Lisp_Object insn) {
    return emit_call2_with_type_hint(comp.setcdr, insn, Qcons);
}

static gcc_jit_rvalue* emit_numperp(Lisp_Object insn) {
    gcc_jit_rvalue* x = emit_mvar_rval(SECOND(insn));
...
static gcc_jit_rvalue* emit_integerp(Lisp_Object insn) {
    gcc_jit_rvalue* x = emit_mvar_rval(SECOND(insn));
...
static gcc_jit_rvalue* emit_maybe_gc_or_quit(Lisp_Object insn) {
...
static void emit_static_object(const char* name, Lisp_Object obj) {
    /* libgccjit has no support for initialized static data.
       ...
       neck in terms of performance at load time will still be the reader. NOTE:
       we can not rely on libgccjit even for valid NULL terminated C strings
       cause of this funny bug that will affect all pre gcc10 era gccs:
    ...
    if (gcc_jit_global_set_initializer) {
        ...
        gcc_jit_lvalue* blob = gcc_jit_context_new_global(
            comp.ctxt, NULL, GCC_JIT_GLOBAL_EXPORTED,
            gcc_jit_context_new_array_type(comp.ctxt, NULL, comp.char_type,
                                           size),
    ...
    gcc_jit_type* a_type = gcc_jit_context_new_array_type(
        comp.ctxt, NULL, comp.char_type, len + 1);
    gcc_jit_field* fields[] = {
        gcc_jit_context_new_field(comp.ctxt, NULL, comp.ptrdiff_type, "len"),
        gcc_jit_context_new_field(comp.ctxt, NULL, a_type, "data")};

    gcc_jit_type* data_struct_t =
        gcc_jit_struct_as_type(gcc_jit_context_new_struct_type(
            comp.ctxt, NULL, format_string("%s_struct", name),
            ARRAYELTS(fields), fields));

    gcc_jit_lvalue* data_struct =
        gcc_jit_context_new_global(comp.ctxt, NULL, GCC_JIT_GLOBAL_INTERNAL,
                                   data_struct_t, format_string("%s_s", name));

    gcc_jit_function* f = gcc_jit_context_new_function(
        comp.ctxt, NULL, GCC_JIT_FUNCTION_EXPORTED,
        gcc_jit_type_get_pointer(data_struct_t), name, 0, NULL, 0);
    DECL_BLOCK(block, f);
    ...
    gcc_jit_lvalue* ptrvar =
        gcc_jit_function_new_local(f, NULL, comp.char_ptr_type, "ptr");

    gcc_jit_block_add_assignment(
        block, NULL, ptrvar,
        gcc_jit_lvalue_get_address(
            gcc_jit_context_new_array_access(
                comp.ctxt, NULL, gcc_jit_lvalue_as_rvalue(arr),
                gcc_jit_context_new_rvalue_from_int(comp.ctxt, comp.int_type,
                                                    0)),
            NULL));
    ...
    for (ptrdiff_t i = 0; i < len;) {
        ...
        if (l != 0) {
            ...
            gcc_jit_rvalue* args[] = {
                ...
                gcc_jit_context_new_rvalue_from_int(comp.ctxt, comp.size_t_type,
                                                    l)};

            gcc_jit_block_add_eval(
                block, NULL,
                gcc_jit_context_new_call(comp.ctxt, NULL, comp.memcpy,
                                         ARRAYELTS(args), args));
            gcc_jit_block_add_assignment(
                block, NULL, ptrvar,
                gcc_jit_lvalue_get_address(
    ...
    gcc_jit_block_add_assignment(
        block, NULL, gcc_jit_lvalue_access_field(data_struct, NULL, fields[0]),
        gcc_jit_context_new_rvalue_from_int(comp.ctxt, comp.ptrdiff_type, len));
    gcc_jit_rvalue* res = gcc_jit_lvalue_get_address(data_struct, NULL);
    gcc_jit_block_end_with_return(block, NULL, res);
}
#pragma GCC diagnostic pop

static reloc_array_t declare_imported_data_relocs(Lisp_Object container,
                                                  const char* code_symbol,
                                                  const char* text_symbol) {
    /* Imported objects.  */
    reloc_array_t res;
    res.len = XFIXNUM(CALLNI(hash - table - count,
    ...
    res.r_val = gcc_jit_lvalue_as_rvalue(gcc_jit_context_new_global(
        comp.ctxt, NULL, GCC_JIT_GLOBAL_EXPORTED,
        gcc_jit_context_new_array_type(comp.ctxt, NULL, comp.lisp_obj_type,
                                       res.len),
        code_symbol));
...
static void declare_imported_data(void) {
    /* Imported objects.  */
    comp.data_relocs = declare_imported_data_relocs(
        CALLNI(comp - ctxt - d - default, Vcomp_ctxt), DATA_RELOC_SYM,
        TEXT_DATA_RELOC_SYM);
    comp.data_relocs_ephemeral = declare_imported_data_relocs(
        CALLNI(comp - ctxt - d - ephemeral, Vcomp_ctxt),
        DATA_RELOC_EPHEMERAL_SYM, TEXT_DATA_RELOC_EPHEMERAL_SYM);
}
...
*/
static Lisp_Object declare_runtime_imported_funcs(void) {
...
#define ADD_IMPORTED(f_name, ret_type, nargs, args)                            \
    ...
    ADD_IMPORTED(helper_GET_SYMBOL_WITH_POSITION,
                 comp.lisp_symbol_with_position_ptr_type, 1, args);
...
*/
static void emit_ctxt_code(void) {
    /* Emit optimize qualities.  */
    Lisp_Object opt_qly[] = {Fcons(Qnative_comp_speed, make_fixnum(comp.speed)),
    ...
    FOR_EACH_TAIL(subr_l) {
        ...
        eassert(n_frelocs < freloc.size);
        fields[n_frelocs++] = declare_imported_func(
            subr_sym, comp.lisp_obj_type, subr->max_args, NULL);
    }

    gcc_jit_struct* f_reloc_struct = gcc_jit_context_new_struct_type(
        comp.ctxt, NULL, "freloc_link_table", n_frelocs, fields);
    ...
    comp.func_relocs = gcc_jit_context_new_global(
        comp.ctxt, NULL, GCC_JIT_GLOBAL_EXPORTED, comp.func_relocs_ptr_type,
        FUNC_LINK_TABLE_SYM);
...
static void define_lisp_cons(void) {
    /*
      union cdr_u
      {
        Lisp_Object cdr;
    ...
    */
    ...
    comp.lisp_cons_s =
        gcc_jit_context_new_opaque_struct(comp.ctxt, NULL, "comp_Lisp_Cons");
    comp.lisp_cons_type = gcc_jit_struct_as_type(comp.lisp_cons_s);
    comp.lisp_cons_ptr_type = gcc_jit_type_get_pointer(comp.lisp_cons_type);

    comp.lisp_cons_u_s_u_cdr =
        gcc_jit_context_new_field(comp.ctxt, NULL, comp.lisp_obj_type, "cdr");

    gcc_jit_field* cdr_u_fields[] = {
        comp.lisp_cons_u_s_u_cdr,
        gcc_jit_context_new_field(comp.ctxt, NULL, comp.lisp_cons_ptr_type,
                                  "chain")};

    gcc_jit_type* cdr_u = gcc_jit_context_new_union_type(
        comp.ctxt, NULL, "comp_cdr_u", ARRAYELTS(cdr_u_fields), cdr_u_fields);

    comp.lisp_cons_u_s_car =
        gcc_jit_context_new_field(comp.ctxt, NULL, comp.lisp_obj_type, "car");
    comp.lisp_cons_u_s_u =
        gcc_jit_context_new_field(comp.ctxt, NULL, cdr_u, "u");
    gcc_jit_field* cons_s_fields[] = {comp.lisp_cons_u_s_car,
                                      comp.lisp_cons_u_s_u};

    gcc_jit_struct* cons_s = gcc_jit_context_new_struct_type(
        comp.ctxt, NULL, "comp_cons_s", ARRAYELTS(cons_s_fields),
        cons_s_fields);

    comp.lisp_cons_u_s = gcc_jit_context_new_field(
        comp.ctxt, NULL, gcc_jit_struct_as_type(cons_s), "s");
    ...
    gcc_jit_type* lisp_cons_u_type =
        gcc_jit_context_new_union_type(comp.ctxt, NULL, "comp_cons_u",
                                       ARRAYELTS(cons_u_fields), cons_u_fields);

    comp.lisp_cons_u =
        gcc_jit_context_new_field(comp.ctxt, NULL, lisp_cons_u_type, "u");
    gcc_jit_struct_set_fields(comp.lisp_cons_s, NULL, 1, &comp.lisp_cons_u);
}

static void define_lisp_symbol_with_position(void) {
    comp.lisp_symbol_with_position_header =
        gcc_jit_context_new_field(comp.ctxt, NULL, comp.ptrdiff_type, "header");
    comp.lisp_symbol_with_position_sym =
        gcc_jit_context_new_field(comp.ctxt, NULL, comp.lisp_obj_type, "sym");
    comp.lisp_symbol_with_position_pos =
        gcc_jit_context_new_field(comp.ctxt, NULL, comp.lisp_obj_type, "pos");
    ...
    comp.lisp_symbol_with_position = gcc_jit_context_new_struct_type(
        comp.ctxt, NULL, "comp_lisp_symbol_with_position", 3, fields);
...
static void define_jmp_buf(void) {
    gcc_jit_field* field = gcc_jit_context_new_field(
        comp.ctxt, NULL,
        gcc_jit_context_new_array_type(comp.ctxt, NULL, comp.char_type,
                                       sizeof(sys_jmp_buf)),
        "stuff");
    comp.jmp_buf_s = gcc_jit_context_new_struct_type(comp.ctxt, NULL,
                                                     "comp_jmp_buf", 1, &field);
}

static void define_memcpy(void) {
    ...
    comp.memcpy = gcc_jit_context_new_function(
        comp.ctxt, NULL, GCC_JIT_FUNCTION_IMPORTED, comp.void_ptr_type,
        "memcpy", ARRAYELTS(params), params, false);
}

/* struct handler definition  */

static void define_handler_struct(void) {
    comp.handler_s =
    ...
    comp.handler_jmp_field = gcc_jit_context_new_field(
        comp.ctxt, NULL, gcc_jit_struct_as_type(comp.jmp_buf_s), "jmp");
    comp.handler_val_field =
        gcc_jit_context_new_field(comp.ctxt, NULL, comp.lisp_obj_type, "val");
    comp.handler_next_field = gcc_jit_context_new_field(
        comp.ctxt, NULL, comp.handler_ptr_type, "next");
    gcc_jit_field* fields[] = {
        gcc_jit_context_new_field(
            comp.ctxt, NULL,
            gcc_jit_context_new_array_type(comp.ctxt, NULL, comp.char_type,
        ...
        gcc_jit_context_new_field(comp.ctxt, NULL,
                                  ...
                                  "pad2")};
    gcc_jit_struct_set_fields(comp.handler_s, NULL, ARRAYELTS(fields), fields);
}

static void define_thread_state_struct(void) {
    ...
    comp.m_handlerlist = gcc_jit_context_new_field(
        comp.ctxt, NULL, comp.handler_ptr_type, "m_handlerlist");
    gcc_jit_field* fields[] = {
        gcc_jit_context_new_field(
            comp.ctxt, NULL,
            gcc_jit_context_new_array_type(
    ...
    comp.thread_state_s = gcc_jit_context_new_struct_type(
...
#ifndef LIBGCCJIT_HAVE_gcc_jit_context_new_bitcast
static gcc_jit_function* define_type_punning(const char* name,
                                             gcc_jit_type* from,
                                             gcc_jit_field* from_field,
                                             gcc_jit_type* to,
                                             gcc_jit_field* to_field) {
    gcc_jit_param* param =
        gcc_jit_context_new_param(comp.ctxt, NULL, from, "arg");
    gcc_jit_function* result = gcc_jit_context_new_function(
        comp.ctxt, NULL, GCC_JIT_FUNCTION_INTERNAL, to, name, 1, &param, 0);

    DECL_BLOCK(entry_block, result);

    gcc_jit_lvalue* tmp_union = gcc_jit_function_new_local(
        result, NULL, comp.cast_union_type, "union_cast");

    gcc_jit_block_add_assignment(
        entry_block, NULL,
        gcc_jit_lvalue_access_field(tmp_union, NULL, from_field),
        gcc_jit_param_as_rvalue(param));

    gcc_jit_block_end_with_return(
        entry_block, NULL,
        gcc_jit_rvalue_access_field(gcc_jit_lvalue_as_rvalue(tmp_union), NULL,
                                    to_field));
...
static void define_cast_functions(void) {
    ...
    cast_union_fields[0] = gcc_jit_context_new_field(
        comp.ctxt, NULL, comp.void_ptr_type, "void_ptr");
    cast_union_fields[1] = gcc_jit_context_new_field(
        comp.ctxt, NULL, comp.uintptr_type, "uintptr");

    comp.cast_union_type = gcc_jit_context_new_union_type(
        comp.ctxt, NULL, "cast_union", 2, cast_union_fields);

    comp.cast_ptr_to_int = define_type_punning(
        "cast_pointer_to_uintptr_t", comp.void_ptr_type, cast_union_fields[0],
        comp.uintptr_type, cast_union_fields[1]);
    comp.cast_int_to_ptr = define_type_punning(
        "cast_uintptr_t_to_pointer", comp.uintptr_type, cast_union_fields[1],
        comp.void_ptr_type, cast_union_fields[0]);
...
static void define_CHECK_TYPE(void) {
    gcc_jit_param* param[] = {
        gcc_jit_context_new_param(comp.ctxt, NULL, comp.int_type, "ok"),
        gcc_jit_context_new_param(comp.ctxt, NULL, comp.lisp_obj_type,
                                  "predicate"),
        gcc_jit_context_new_param(comp.ctxt, NULL, comp.lisp_obj_type, "x")};
    comp.check_type =
        gcc_jit_context_new_function(comp.ctxt, NULL, GCC_JIT_FUNCTION_INTERNAL,
                                     comp.void_type, "CHECK_TYPE", 3, param, 0);
...
static void define_CAR_CDR(void) {
    gcc_jit_function* func[2];
    char const* f_name[] = {"CAR", "CDR"};
    for (int i = 0; i < 2; i++) {
        gcc_jit_param* param[] = {
            gcc_jit_context_new_param(comp.ctxt, NULL, comp.lisp_obj_type, "c"),
            gcc_jit_context_new_param(comp.ctxt, NULL, comp.bool_type,
                                      "cert_cons")};
       ...
       This seems at least to have no perf downside.  */
        func[i] = gcc_jit_context_new_function(
            comp.ctxt, NULL, GCC_JIT_FUNCTION_INTERNAL, comp.lisp_obj_type,
            f_name[i], 2, param, 0);
        ...
        emit_cond_jump(
            emit_binary_op(GCC_JIT_BINARY_OP_LOGICAL_OR, comp.bool_type,
                           gcc_jit_param_as_rvalue(param[1]), emit_CONSP(c)),
            is_cons_b, not_a_cons_b);
        ...
        gcc_jit_rvalue* wrong_type_args[] = {emit_lisp_obj_rval(Qlistp), c};
...
static void define_setcar_setcdr(void) {
    char const* f_name[] = {"setcar", "setcdr"};
    char const* par_name[] = {"new_car", "new_cdr"};

    for (int i = 0; i < 2; i++) {
        gcc_jit_param* cell = gcc_jit_context_new_param(
            comp.ctxt, NULL, comp.lisp_obj_type, "cell");
        gcc_jit_param* new_el = gcc_jit_context_new_param(
            comp.ctxt, NULL, comp.lisp_obj_type, par_name[i]);

        gcc_jit_param* param[] = {cell, new_el,
                                  gcc_jit_context_new_param(comp.ctxt, NULL,
                                                            comp.bool_type,
                                                            "cert_cons")};

        gcc_jit_function** f_ref = !i ? &comp.setcar : &comp.setcdr;
        *f_ref = gcc_jit_context_new_function(
            comp.ctxt, NULL, GCC_JIT_FUNCTION_INTERNAL, comp.lisp_obj_type,
            f_name[i], 3, param, 0);
        ...
        /* return newel;  */
        gcc_jit_block_end_with_return(entry_block, NULL,
                                      gcc_jit_param_as_rvalue(new_el));
    }
}
...
static void define_add1_sub1(void) {
    gcc_jit_block* bb_orig = comp.block;
    gcc_jit_function* func[2];
    char const* f_name[] = {"add1", "sub1"};
    char const* fall_back_func[] = {"1+", "1-"};
    enum gcc_jit_binary_op op[] = {GCC_JIT_BINARY_OP_PLUS,
                                   GCC_JIT_BINARY_OP_MINUS};
    for (ptrdiff_t i = 0; i < 2; i++) {
        gcc_jit_param* param[] = {
            gcc_jit_context_new_param(comp.ctxt, NULL, comp.lisp_obj_type, "n"),
            gcc_jit_context_new_param(comp.ctxt, NULL, comp.bool_type,
                                      "cert_fixnum")};
        comp.func = func[i] = gcc_jit_context_new_function(
            comp.ctxt, NULL, GCC_JIT_FUNCTION_INTERNAL, comp.lisp_obj_type,
            f_name[i], 2, param, 0);
        DECL_BLOCK(entry_block, func[i]);
       ...
       : Fadd1 (n)) */
        ...
        gcc_jit_rvalue* sure_fixnum =
            emit_binary_op(GCC_JIT_BINARY_OP_LOGICAL_OR, comp.bool_type,
                           gcc_jit_param_as_rvalue(param[1]), emit_FIXNUMP(n));
        emit_cond_jump(
            emit_binary_op(
                GCC_JIT_BINARY_OP_LOGICAL_AND, comp.bool_type, sure_fixnum,
                gcc_jit_context_new_comparison(
                    comp.ctxt, NULL, GCC_JIT_COMPARISON_NE, n_fixnum,
                    i == 0 ? emit_rvalue_from_emacs_int(MOST_POSITIVE_FIXNUM)
                           : emit_rvalue_from_emacs_int(MOST_NEGATIVE_FIXNUM))),
            inline_block, fcall_block);

        comp.block = inline_block;
        gcc_jit_rvalue* inline_res =
            emit_binary_op(op[i], comp.emacs_int_type, n_fixnum, comp.one);

        gcc_jit_block_end_with_return(inline_block, NULL,
        ...
        gcc_jit_block_end_with_return(fcall_block, NULL, call_res);
    }
    comp.block = bb_orig;
    comp.add1 = func[0];
    comp.sub1 = func[1];
...
static void define_negate(void) {
    gcc_jit_block* bb_orig = comp.block;
    gcc_jit_param* param[] = {
        gcc_jit_context_new_param(comp.ctxt, NULL, comp.lisp_obj_type, "n"),
        gcc_jit_context_new_param(comp.ctxt, NULL, comp.bool_type,
    ...
    comp.func = comp.negate =
        gcc_jit_context_new_function(comp.ctxt, NULL, GCC_JIT_FUNCTION_INTERNAL,
                                     comp.lisp_obj_type, "negate", 2, param, 0);
    ...
    emit_cond_jump(
        emit_binary_op(GCC_JIT_BINARY_OP_LOGICAL_AND, comp.bool_type,
                       sure_fixnum,
                       gcc_jit_context_new_comparison(
                           comp.ctxt, NULL, GCC_JIT_COMPARISON_NE, n_fixnum,
                           emit_rvalue_from_emacs_int(MOST_NEGATIVE_FIXNUM))),
    ...
    comp.block = inline_block;
    gcc_jit_rvalue* inline_res = gcc_jit_context_new_unary_op(
        comp.ctxt, NULL, GCC_JIT_UNARY_OP_MINUS, comp.emacs_int_type, n_fixnum);
...
static void define_PSEUDOVECTORP(void) {
    gcc_jit_param* param[] = {
        gcc_jit_context_new_param(comp.ctxt, NULL, comp.lisp_obj_type, "a"),
        gcc_jit_context_new_param(comp.ctxt, NULL, comp.int_type, "code")};

    comp.pseudovectorp = gcc_jit_context_new_function(
        comp.ctxt, NULL, GCC_JIT_FUNCTION_INTERNAL, comp.bool_type,
        "PSEUDOVECTORP", 2, param, 0);
    ...
    emit_cond_jump(emit_VECTORLIKEP(gcc_jit_param_as_rvalue(param[0])),
                   call_pseudovector_typep_b, ret_false_b);

    comp.block = ret_false_b;
    gcc_jit_block_end_with_return(
        ret_false_b, NULL,
        gcc_jit_context_new_rvalue_from_int(comp.ctxt, comp.bool_type, false));
...
static void define_GET_SYMBOL_WITH_POSITION(void) {
    ...
    comp.get_symbol_with_position =
        gcc_jit_context_new_function(comp.ctxt, NULL, GCC_JIT_FUNCTION_INTERNAL,
                                     comp.lisp_symbol_with_position_ptr_type,
                                     "GET_SYMBOL_WITH_POSITION", 1, param, 0);
    ...
    gcc_jit_block_end_with_return(
        ...
        emit_call(intern_c_string("helper_GET_SYMBOL_WITH_POSITION"),
                  comp.lisp_symbol_with_position_ptr_type, 1, args, false));
}
...
static void define_SYMBOL_WITH_POS_SYM(void) {
    ...
    gcc_jit_param* param[] = {
        gcc_jit_context_new_param(comp.ctxt, NULL, comp.lisp_obj_type, "a")};
    comp.symbol_with_pos_sym = gcc_jit_context_new_function(
        comp.ctxt, NULL, GCC_JIT_FUNCTION_INTERNAL, comp.lisp_obj_type,
        "SYMBOL_WITH_POS_SYM", 1, param, 0);
    ...
    swp = gcc_jit_context_new_call(comp.ctxt, NULL,
                                   comp.get_symbol_with_position, 1, args);
    tmpl = gcc_jit_rvalue_dereference(swp, NULL);
    tmpr = gcc_jit_lvalue_as_rvalue(tmpl);
    gcc_jit_block_end_with_return(
        entry_block, NULL,
        gcc_jit_rvalue_access_field(tmpr, NULL,
                                    comp.lisp_symbol_with_position_sym));
}

static void define_maybe_gc_or_quit(void) {
    ...
    */
    ...
    gcc_jit_lvalue* quitcounter =
        gcc_jit_context_new_global(comp.ctxt, NULL, GCC_JIT_GLOBAL_INTERNAL,
                                   comp.unsigned_type, "quitcounter");

    comp.func = comp.maybe_gc_or_quit = gcc_jit_context_new_function(
        comp.ctxt, NULL, GCC_JIT_FUNCTION_INTERNAL, comp.void_type,
        "maybe_gc_quit", 0, NULL, 0);
    ...
    gcc_jit_block_add_assignment(
        comp.block, NULL, quitcounter,
        emit_binary_op(GCC_JIT_BINARY_OP_PLUS, comp.unsigned_type,
                       gcc_jit_lvalue_as_rvalue(quitcounter),
                       gcc_jit_context_new_rvalue_from_int(
                           comp.ctxt, comp.unsigned_type, 1)));
    emit_cond_jump(
        emit_binary_op(GCC_JIT_BINARY_OP_RSHIFT, comp.unsigned_type,
                       gcc_jit_lvalue_as_rvalue(quitcounter),
                       gcc_jit_context_new_rvalue_from_int(
                           comp.ctxt, comp.unsigned_type, 9)),
    ...
    gcc_jit_block_add_eval(
        comp.block, NULL,
        emit_call(intern_c_string("maybe_gc"), comp.void_type, 0, NULL, false));
    gcc_jit_block_add_eval(comp.block, NULL,
...
static void define_bool_to_lisp_obj(void) {
    /* x ? Qt : Qnil */
    gcc_jit_param* param =
        gcc_jit_context_new_param(comp.ctxt, NULL, comp.bool_type, "x");
    comp.bool_to_lisp_obj = gcc_jit_context_new_function(
        comp.ctxt, NULL, GCC_JIT_FUNCTION_INTERNAL, comp.lisp_obj_type,
        "bool_to_lisp_obj", 1, &param, 0);
    ...
    emit_cond_jump(gcc_jit_param_as_rvalue(param), ret_t_block, ret_nil_block);

    comp.block = ret_t_block;
    gcc_jit_block_end_with_return(ret_t_block, NULL, emit_lisp_obj_rval(Qt));

    comp.block = ret_nil_block;
    gcc_jit_block_end_with_return(ret_nil_block, NULL,
                                  emit_lisp_obj_rval(Qnil));
}

static gcc_jit_function* declare_lex_function(Lisp_Object func) {
    ...
    if (!nargs) {
        ...
        for (int i = 0; i < max_args; ++i)
            params[i] = gcc_jit_context_new_param(comp.ctxt, NULL, type[i],
                                                  format_string("par_%d", i));
        res = gcc_jit_context_new_function(
            comp.ctxt, NULL, GCC_JIT_FUNCTION_EXPORTED, comp.lisp_obj_type,
            SSDATA(c_name), max_args, params, 0);
    }
    else {
        gcc_jit_param* params[] = {
            gcc_jit_context_new_param(comp.ctxt, NULL, comp.ptrdiff_type,
                                      "nargs"),
            gcc_jit_context_new_param(comp.ctxt, NULL, comp.lisp_obj_ptr_type,
                                      "args")};
        res = gcc_jit_context_new_function(
            comp.ctxt, NULL, GCC_JIT_FUNCTION_EXPORTED, comp.lisp_obj_type,
            SSDATA(c_name), ARRAYELTS(params), params, 0);
    }
    SAFE_FREE();
    return res;
}
...
static void declare_function(Lisp_Object func) {
    gcc_jit_function* gcc_func = !NILP(CALLNI(comp - func - l - p, func))
        ? declare_lex_function(func)
        : gcc_jit_context_new_function(
              comp.ctxt, NULL, GCC_JIT_FUNCTION_EXPORTED, comp.lisp_obj_type,
              SSDATA(CALLNI(comp - func - c - name, func)), 0, NULL, 0);
    Fputhash(CALLNI(comp - func - c - name, func), make_mint_ptr(gcc_func),
             comp.exported_funcs_h);
}

static void compile_function(Lisp_Object func) {
    USE_SAFE_ALLOCA;
    ...
    comp.func_has_non_local =
        !NILP(CALLNI(comp - func - has - non - local, func));
    comp.func_speed = XFIXNUM(CALLNI(comp - func - speed, func));
    comp.func_safety = XFIXNUM(CALLNI(comp - func - safety, func));

    comp.func_relocs_local = gcc_jit_function_new_local(
        comp.func, NULL, comp.func_relocs_ptr_type, "freloc");

    SAFE_NALLOCA(comp.frame, 1, comp.frame_size);
    if (comp.func_has_non_local || !comp.func_speed) {
        /* FIXME: See bug#42360.  */
        gcc_jit_lvalue* arr = gcc_jit_function_new_local(
            comp.func, NULL,
            gcc_jit_context_new_array_type(comp.ctxt, NULL, comp.lisp_obj_type,
                                           comp.frame_size),
            "frame");
        ...
        for (ptrdiff_t i = 0; i < comp.frame_size; ++i)
            comp.frame[i] = gcc_jit_context_new_array_access(
                comp.ctxt, NULL, gcc_jit_lvalue_as_rvalue(arr),
                gcc_jit_context_new_rvalue_from_int(comp.ctxt, comp.int_type,
                                                    i));
    }
    ...
    comp.loc_handler =
        gcc_jit_function_new_local(comp.func, NULL, comp.handler_ptr_type, "c");
    ...
    struct Lisp_Hash_Table* ht =
        XHASH_TABLE(CALLNI(comp - func - blocks, func));
    DOHASH(ht, block_name, block) {
        if (!EQ(block_name, Qentry))
            declare_block(block_name);
    }

    gcc_jit_block_add_assignment(retrieve_block(Qentry), NULL,
                                 comp.func_relocs_local,
    ...
    if (err)
        xsignal3(Qnative_ice, build_string("failing to compile function"),
                 CALLNI(comp - func - name, func), build_string(err));
    SAFE_FREE();
}
...
static Lisp_Object make_directory_wrapper(Lisp_Object directory) {
    CALLNI(make - directory, directory, Qt);
    return Qnil;
}

static Lisp_Object make_directory_wrapper_1(Lisp_Object ignore) { return Qt; }
...
(Lisp_Object filename) {
    ...
    if (file_normalized) {
        filename = DECODE_FILE(
            make_unibyte_string(file_normalized, strlen(file_normalized)));
    ...
    if (NILP(base_dir)) {
        ...
        FOR_EACH_TAIL(eln_load_paths) {
            ...
            if (!NILP(Ffile_exists_p(dir))) {
                ...
                if (NILP(internal_condition_case_1(make_directory_wrapper, dir,
                                                   Qt,
...
(void) {
    ...
    if (comp.ctxt) {
        xsignal1(Qnative_ice, build_string("compiler context already taken"));
        return Qnil;
    }
    ...
    comp.unsigned_type =
        gcc_jit_context_get_type(comp.ctxt, GCC_JIT_TYPE_UNSIGNED_INT);
    ...
    comp.emacs_int_type =
        gcc_jit_context_get_int_type(comp.ctxt, sizeof(EMACS_INT), true);
    comp.emacs_uint_type =
        gcc_jit_context_get_int_type(comp.ctxt, sizeof(EMACS_UINT), false);
#if LISP_WORDS_ARE_POINTERS
    comp.lisp_word_type = gcc_jit_type_get_pointer(gcc_jit_struct_as_type(
        gcc_jit_context_new_opaque_struct(comp.ctxt, NULL, "Lisp_X")));
#else
...
#ifdef LISP_OBJECT_IS_STRUCT
    comp.lisp_obj_i =
        gcc_jit_context_new_field(comp.ctxt, NULL, comp.lisp_word_type, "i");
    comp.lisp_obj_s = gcc_jit_context_new_struct_type(
        comp.ctxt, NULL, "Lisp_Object", 1, &comp.lisp_obj_i);
    ...
    comp.zero =
        gcc_jit_context_new_rvalue_from_int(comp.ctxt, comp.emacs_int_type, 0);
    comp.one =
        gcc_jit_context_new_rvalue_from_int(comp.ctxt, comp.emacs_int_type, 1);
    comp.inttypebits = gcc_jit_context_new_rvalue_from_int(
        comp.ctxt, comp.emacs_uint_type, INTTYPEBITS);
    comp.lisp_int0 = gcc_jit_context_new_rvalue_from_int(
        comp.ctxt, comp.emacs_int_type, Lisp_Int0);
    comp.ptrdiff_type =
        gcc_jit_context_get_int_type(comp.ctxt, sizeof(void*), true);
    comp.uintptr_type =
        gcc_jit_context_get_int_type(comp.ctxt, sizeof(void*), false);
    comp.size_t_type =
        gcc_jit_context_get_int_type(comp.ctxt, sizeof(size_t), false);
...
DEFUN("comp--release-ctxt", Fcomp__release_ctxt, Scomp__release_ctxt, 0, 0, 0,
...
DEFUN("comp-native-driver-options-effective-p",
      ...
      Scomp_native_driver_options_effective_p, 0, 0, 0,
...
DEFUN("comp-native-compiler-options-effective-p",
      ...
      Scomp_native_compiler_options_effective_p, 0, 0, 0,
...
static void add_driver_options(void) {
...
static void add_compiler_options(void) {
...
DEFUN("comp--compile-ctxt-to-file0", Fcomp__compile_ctxt_to_file0,
      Scomp__compile_ctxt_to_file0, 1, 1, 0,
...
(Lisp_Object filename) {
    ...
    comp.compiler_options =
        CALLNI(comp - ctxt - compiler - options, Vcomp_ctxt);
    ...
    if (comp.debug >= 3) {
        ...
        gcc_jit_context_set_logfile(comp.ctxt, logfile, 0, 0);
        gcc_jit_context_set_bool_option(
            comp.ctxt, GCC_JIT_BOOL_OPTION_KEEP_INTERMEDIATES, 1);
        gcc_jit_context_set_bool_option(comp.ctxt,
                                        GCC_JIT_BOOL_OPTION_DUMP_EVERYTHING, 1);
    }

    gcc_jit_context_set_int_option(
        comp.ctxt, GCC_JIT_INT_OPTION_OPTIMIZATION_LEVEL,
        comp.speed < 0 ? 0 : (comp.speed > 3 ? 3 : comp.speed));
    ...
    comp.d_default_idx = CALLNI(comp - data - container - idx,
                                CALLNI(comp - ctxt - d - default, Vcomp_ctxt));
    ...
    Lisp_Object version = Fcomp_libgccjit_version();
    if (NILP(version) || XFIXNUM(XCAR(version)) < 11)
        gcc_jit_context_add_command_line_option(comp.ctxt,
    ...
    const char* err = gcc_jit_context_get_first_error(comp.ctxt);
    if (err)
        xsignal3(Qnative_ice, build_string("failed to compile"), filename,
                 build_string(err));
...
static bool helper_PSEUDOVECTOR_TYPEP_XUNTAG(Lisp_Object a,
                                             enum pvec_type code) {
    return PSEUDOVECTOR_TYPEP(
        XUNTAG(a, Lisp_Vectorlike, union vectorlike_header), code);
}
...
static Lisp_Object helper_sanitizer_assert(Lisp_Object val, Lisp_Object type) {
    if (!comp_sanitizer_active || !NILP((CALLNI(cl - typep, val, type))))
        return Qnil;
...
#ifdef WINDOWSNT
static Lisp_Object return_nil(Lisp_Object arg) { return Qnil; }

static Lisp_Object directory_files_matching(Lisp_Object name,
                                            Lisp_Object match) {
...
void eln_load_path_final_clean_up(void) {
#ifdef WINDOWSNT
    Lisp_Object dir_tail = Vnative_comp_eln_load_path;
    FOR_EACH_TAIL(dir_tail) {
        Lisp_Object files_in_dir = internal_condition_case_2(
            directory_files_matching,
            Fexpand_file_name(Vcomp_native_version_dir, XCAR(dir_tail)),
            build_string("\\.eln\\.old\\'"), Qt, return_nil);
...
static void register_native_comp_unit(Lisp_Object comp_u) {
...
void maybe_defer_native_compilation(Lisp_Object function_name,
...
#include <unistd.h>
  if (!NILP (function_name) &&
    ...
    if (!native_comp_jit_compilation || noninteractive || !NILP(Vpurify_flag) ||
        !CLOSUREP(definition) || !STRINGP(Vload_true_file_name) ||
    ...
    pending_funcalls =
        Fcons(list(Qnative__compile_async, src, Qnil, Qlate), pending_funcalls);
}
...
void fixup_eln_load_path(Lisp_Object eln_filename) {
    ...
    for (int i = 0; i < (preloaded_p ? 2 : 1); i++)
        eln_cache_sys = Ffile_name_directory(
            Fsubstring_no_properties(eln_cache_sys, Qnil, make_fixnum(-1)));
...
static Lisp_Object load_static_obj(struct Lisp_Native_Comp_Unit* comp_u,
                                   const char* name) {
    specpdl_ref count = SPECPDL_INDEX();
    static_obj_t* blob =
    ...
    specbind(intern_c_string("load-file-name"), Vcomp__hashdollar);
    if (blob)
        /* New blob format.  */
...
static bool check_comp_unit_relocs(struct Lisp_Native_Comp_Unit* comp_u) {
...
static void unset_cu_load_ongoing(Lisp_Object comp_u) {
    XNATIVE_COMP_UNIT(comp_u)->load_ongoing = false;
}

Lisp_Object load_comp_unit(struct Lisp_Native_Comp_Unit* comp_u,
    ...
    Lisp_Object (*top_level_run)(Lisp_Object) =
        dynlib_sym(handle, late_load ? "late_top_level_run" : "top_level_run");
    ...
    if (!comp_u->loaded_once) {
        ...
        if (!(current_thread_reloc && f_symbols_with_pos_enabled_reloc &&
              data_relocs && data_eph_relocs && freloc_link_table &&
              top_level_run) ||
    ...
    if (!loading_dump) {
       ...
       run. Guard against sibling call optimization (or any other).  */
...
Lisp_Object native_function_doc(Lisp_Object function) {
...
static Lisp_Object make_subr(Lisp_Object symbol_name, Lisp_Object minarg,
                             Lisp_Object maxarg, Lisp_Object c_name,
                             Lisp_Object type, Lisp_Object doc_idx,
                             Lisp_Object intspec, Lisp_Object command_modes,
                             Lisp_Object comp_u) {
    ...
    union Aligned_Lisp_Subr* x =
        (union Aligned_Lisp_Subr*)allocate_pseudovector(
            VECSIZE(union Aligned_Lisp_Subr), 0,
            VECSIZE(union Aligned_Lisp_Subr), PVEC_SUBR);
    if (CONSP(minarg)) {
        /* Dynamic code.  */
#ifdef HAVE_NATIVE_COMP
...
(Lisp_Object reloc_idx, Lisp_Object c_name, Lisp_Object minarg,
 Lisp_Object maxarg, Lisp_Object type, Lisp_Object rest, Lisp_Object comp_u) {
    ...
    Lisp_Object tem = make_subr(c_name, minarg, maxarg, c_name, type, doc_idx,
...
This gets called by top_level_run during the load phase.  */)
(Lisp_Object name, Lisp_Object c_name, Lisp_Object minarg, Lisp_Object maxarg,
 Lisp_Object type, Lisp_Object rest, Lisp_Object comp_u) {
    Lisp_Object doc_idx = FIRST(rest);
    Lisp_Object intspec = SECOND(rest);
    Lisp_Object command_modes = THIRD(rest);

    Lisp_Object tem = make_subr(SYMBOL_NAME(name), minarg, maxarg, c_name, type,
                                doc_idx, intspec, command_modes, comp_u);
...
static bool file_in_eln_sys_dir(Lisp_Object filename) {
    Lisp_Object eln_sys_dir = Qnil;
    Lisp_Object tmp = Vnative_comp_eln_load_path;
    FOR_EACH_TAIL(tmp)
    eln_sys_dir = XCAR(tmp);
    return !NILP(
        Fstring_match(Fregexp_quote(Fexpand_file_name(eln_sys_dir, Qnil)),
                      Fexpand_file_name(filename, Qnil), Qnil, Qnil));
}
...
(Lisp_Object filename, Lisp_Object late_load) {
    ...
    if (!NILP(Fgethash(filename, Vcomp_loaded_comp_units_h, Qnil)) &&
        !file_in_eln_sys_dir(filename) && !NILP(Ffile_writable_p(filename))) {
        /* If in this session there was ever a file loaded with this
       name, rename it before loading, to make sure we always get a
       new handle!  */
        Lisp_Object tmp_filename = Fmake_temp_file_internal(
            filename, Qnil, build_string(".eln.tmp"), Qnil);
        if (NILP(Ffile_writable_p(tmp_filename)))
            ...
            comp_u->handle =
                dynlib_open_for_eln(SSDATA(ENCODE_FILE(tmp_filename)));
...
natively compiled one.  */);
    ...
    Fput(
        Qnative_lisp_file_inconsistent, Qerror_conditions,
    ...
    DEFVAR_LISP("comp-ctxt", Vcomp_ctxt, doc:/* The compiler context.  */);
    Vcomp_ctxt = Qnil;
...
:/* Hash table recording all loaded compilation units, file -> CU.  */);
    ...
    DEFVAR_LISP("comp-subr-arities-h", Vcomp_subr_arities_h, doc:
                    /* Hash table recording the arity of Lisp primitives.
           This is in case they are redefined so the compiler still knows how
           to compile calls to them. subr-name -> arity For internal use.
         */);
