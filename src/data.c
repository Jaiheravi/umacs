...
static bool BOOLFWDP(lispfwd a) { return XFWDTYPE(a) == Lisp_Fwd_Bool; }
static bool INTFWDP(lispfwd a) { return XFWDTYPE(a) == Lisp_Fwd_Int; }
static bool OBJFWDP(lispfwd a) { return XFWDTYPE(a) == Lisp_Fwd_Obj; }

static bool* XBOOLVAR(lispfwd a)
{
  eassert(BOOLFWDP(a));
  return a->u.boolvar;
}
static int XKBOARD_OFFSET(lispfwd a)
{
  eassert(KBOARD_OBJFWDP(a));
  return a->u.kbdoffset;
}
static intmax_t* XINTVAR(lispfwd a)
{
  eassert(INTFWDP(a));
  return a->u.intvar;
}
static Lisp_Object* XOBJVAR(lispfwd a)
{
  eassert(OBJFWDP(a));
  return a->u.objvar;
}

static void set_blv_found(struct Lisp_Buffer_Local_Value* blv, int found)
{
  eassert(found == !BASE_EQ(blv->defcell, blv->valcell));
  blv->found = found;
}

static Lisp_Object blv_value(struct Lisp_Buffer_Local_Value* blv)
{
  return XCDR(blv->valcell);
}

static void set_blv_value(struct Lisp_Buffer_Local_Value* blv, Lisp_Object val)
{
  XSETCDR(blv->valcell, val);
}

static void set_blv_where(struct Lisp_Buffer_Local_Value* blv, Lisp_Object val)
{
  blv->where = val;
}

static void set_blv_defcell(struct Lisp_Buffer_Local_Value* blv,
                            Lisp_Object val)
{
  blv->defcell = val;
}

static void set_blv_valcell(struct Lisp_Buffer_Local_Value* blv,
                            Lisp_Object val)
{
  blv->valcell = val;
}

static AVOID wrong_length_argument(Lisp_Object a1, Lisp_Object a2,
                                   Lisp_Object a3)
{
...
void args_out_of_range(Lisp_Object a1, Lisp_Object a2)
{
  xsignal2(Qargs_out_of_range, a1, a2);
}

void args_out_of_range_3(Lisp_Object a1, Lisp_Object a2, Lisp_Object a3)
{
  xsignal3(Qargs_out_of_range, a1, a2, a3);
}

void circular_list(Lisp_Object list) { xsignal1(Qcircular_list, list); }
...
DEFUN("closurep", Fclosurep, Sclosurep, 1, 1, 0,
...
DEFUN("byte-code-function-p", Fbyte_code_function_p, Sbyte_code_function_p, 1,
      1, 0, doc:/* Return t if OBJECT is a byte-compiled function object.  */)
(Lisp_Object object)
{
...
DEFUN("integer-or-marker-p", Finteger_or_marker_p, Sinteger_or_marker_p, 1, 1,
      0,
...
DEFUN("condition-variable-p", Fcondition_variable_p, Scondition_variable_p, 1,
      1, 0, doc:/* Return t if OBJECT is a condition variable.  */)
(Lisp_Object object)
{
  if (CONDVARP(object))
    return Qt;
...
(register Lisp_Object symbol, Lisp_Object definition)
{
  ...
  if (!NILP(Vnative_comp_enable_subr_trampolines) && SUBRP(function) &&
...
static void add_to_function_history(Lisp_Object symbol, Lisp_Object olddef)
{
  eassert(!NILP(olddef));
...
void defalias(Lisp_Object symbol, Lisp_Object definition)
{
  {
...
DEFUN("subr-type", Fsubr_type, Ssubr_type, 1, 1, 0,
...
DEFUN("subr-native-comp-unit", Fsubr_native_comp_unit, Ssubr_native_comp_unit,
      1, 1, 0, doc:/* Return the native compilation unit.  */)
(Lisp_Object subr)
{
  CHECK_SUBR(subr);
...
DEFUN("native-comp-unit-file", Fnative_comp_unit_file, Snative_comp_unit_file,
      1, 1, 0, doc:/* Return the file of the native compilation unit.  */)
...
(Lisp_Object cmd)
{
    ...
    if (spec)
      return list2(Qinteractive,
                   (*spec != '(') ? build_string(spec)
                                  : Fcar(Fread_from_string(build_string(spec),
                                                           Qnil, Qnil)));
  }
  ...
  {
    Lisp_Object form = module_function_interactive_form(XMODULE_FUNCTION(fun));
    if (!NILP(form))
      return form;
  }
#endif
...
(Lisp_Object command)
{
  ...
  {
    Lisp_Object form = module_function_command_modes(XMODULE_FUNCTION(fun));
    if (!NILP(form))
      return form;
  }
#endif
...
KBOARD* kboard_for_bindings(void)
{
...
Lisp_Object do_symval_forwarding(lispfwd valcontents)
{
  ...
  case Lisp_Fwd_Buffer_Obj:
    return per_buffer_value(current_buffer, XBUFFER_OFFSET(valcontents));
...
void wrong_choice(Lisp_Object choice, Lisp_Object wrong)
{
...
static void wrong_range(Lisp_Object min, Lisp_Object max, Lisp_Object wrong)
{
  AUTO_STRING(value_should_be_from, "Value should be from ");
  AUTO_STRING(to, " to ");
  xsignal2(Qerror,
           CALLN(Fconcat, value_should_be_from, Fnumber_to_string(min), to,
                 Fnumber_to_string(max)),
           wrong);
}

static void check_choice(Lisp_Object choice, Lisp_Object val)
{
...
static void check_fwd_predicate(enum Lisp_Fwd_Predicate p, Lisp_Object val)
{
  switch (p)
  {
  case FWDPRED_Qnil:
                                    ...
                                    struct buffer* buf)
{
 ...
 and update them.  */
    ...
    {
      int offset = ((char*)XOBJVAR(valcontents) - (char*)&buffer_defaults);
...
void swap_in_global_binding(struct Lisp_Symbol* symbol)
{
...
static void swap_in_symval_forwarding(struct Lisp_Symbol* symbol,
...
void set_internal(Lisp_Object symbol, Lisp_Object newval, Lisp_Object where,
  ...
  case SYMBOL_TRAPPED_WRITE:
    ...
    if (bindflag != SET_INTERNAL_THREAD_SWITCH)
      notify_variable_watchers(
          symbol, (unbinding_p ? Qnil : newval),
          (bindflag == SET_INTERNAL_BIND
               ? Qlet
               : (bindflag == SET_INTERNAL_UNBIND
                      ? Qunlet
                      : (unbinding_p ? Qmakunbound : Qset))),
          where);
    break;
      ...
      if (BUFFER_OBJFWDP(innercontents))
      {
            ...
            !PER_BUFFER_VALUE_P(buf, idx))
        {
          if (let_shadows_buffer_binding_p(sym))
            set_default_internal(symbol, newval, bindflag, NULL);
          else
...
static void set_symbol_trapped_write(Lisp_Object symbol,
...
static void restore_symbol_trapped_write(Lisp_Object symbol)
{
  set_symbol_trapped_write(symbol, SYMBOL_TRAPPED_WRITE);
}

static void harmonize_variable_watchers(Lisp_Object alias,
                                        Lisp_Object base_variable)
{
  if (!EQ(base_variable, alias) && EQ(base_variable, Findirect_variable(alias)))
    set_symbol_trapped_write(alias, XSYMBOL(base_variable)->u.s.trapped_write);
}
...
DEFUN("get-variable-watchers", Fget_variable_watchers, Sget_variable_watchers,
      1, 1, 0, doc:/* Return a list of SYMBOL's active watchers.  */)
(Lisp_Object symbol)
{
...
void notify_variable_watchers(Lisp_Object symbol, Lisp_Object newval,
                              Lisp_Object operation, Lisp_Object where)
{
  ...
  if (NILP(where) && !EQ(operation, Qset_default) &&
      !EQ(operation, Qmakunbound) &&
  ...
  for (Lisp_Object watchers = Fget(symbol, Qwatchers); CONSP(watchers);
...
Lisp_Object default_value(Lisp_Object symbol)
{
  struct Lisp_Symbol* sym;
...
void set_default_internal(Lisp_Object symbol, Lisp_Object value,
  ...
  case SYMBOL_FORWARDED:
    {
      ...
      {
        char* base = (char*)(where ? where : kboard_for_bindings());
        char* p = base + XKBOARD_OFFSET(valcontents);
        *(Lisp_Object*)p = value;
      }
      else
...
static struct Lisp_Buffer_Local_Value* make_blv(struct Lisp_Symbol* sym,
                                                bool forwarded,
                                                union Lisp_Val_Fwd valcontents)
{
  ...
  tem = Fcons(
      symbol,
      (forwarded ? do_symval_forwarding(valcontents.fwd) : valcontents.value));
...
start:
  ...
  if (NILP(tem))
  {
    if (let_shadows_buffer_binding_p(sym))
    {
      AUTO_STRING(format, "Making %s buffer-local while locally let-bound!");
  ...
  case SYMBOL_LOCALIZED:
    {
      ...
      if (BASE_EQ(blv->where, tmp)) /* The binding is already loaded.  */
        ...
        return NILP(assq_no_quit(variable, BVAR(buf, local_var_alist))) ? Qnil
                                                                        : Qt;
    }
  case SYMBOL_FORWARDED:
    {
...
(Lisp_Object object, Lisp_Object noerror) { return indirect_function(object); }
...
static Lisp_Object check_integer_coerce_marker(Lisp_Object x)
{
  if (MARKERP(x))
...
static Lisp_Object check_number_coerce_marker(Lisp_Object x)
{
  if (MARKERP(x))
    return make_fixnum(marker_position(x));
...
static Lisp_Object coerce_marker(Lisp_Object x)
{
  return MARKERP(x) ? make_fixnum(marker_position(x)) : x;
}

static AVOID not_number_or_marker(Lisp_Object x)
{
  wrong_type_argument(Qnumber_or_marker_p, x);
}

cmp_bits_t arithcompare(Lisp_Object num1, Lisp_Object num2)
{
...
static Lisp_Object arithcompare_driver(ptrdiff_t nargs, Lisp_Object* args,
                                       cmp_bits_t cmpmask)
{
...
uintmax_t cons_to_unsigned(Lisp_Object c, uintmax_t max)
{
  bool valid = false;
...
/* Convert the cons-of-integers, integer, or float value C to a signed
   ...
   cons-of-integers or floats, for now this function still accepts the
   obsolete forms in case some old Lisp code still generates them.  */
intmax_t cons_to_signed(Lisp_Object c, intmax_t min, intmax_t max)
{
  bool valid = false;
  intmax_t val UNINIT;
  ...
  else
  {
    ...
    if (valid && CONSP(c))
    {
      ...
      if (top >= INTMAX_MIN >> 24 >> 16 && top <= INTMAX_MAX >> 24 >> 16 &&
          CONSP(rest) && FIXNATP(XCAR(rest)) && XFIXNAT(XCAR(rest)) < 1 << 24 &&
          FIXNATP(XCDR(rest)) && XFIXNAT(XCDR(rest)) < 1 << 16)
      {
...
char* fixnum_to_string(EMACS_INT number, char* buffer, char* end)
{
  EMACS_INT x = number;
  bool negative = x < 0;
  if (negative)
...
static bool floating_point_op(enum arithop code) { return code <= Adiv; }
...
static Lisp_Object floatop_arith_driver(enum arithop code, ptrdiff_t nargs,
                                        Lisp_Object* args, ptrdiff_t argnum,
...
static Lisp_Object float_arith_driver(enum arithop code, ptrdiff_t nargs,
                                      Lisp_Object* args, ptrdiff_t argnum,
                                      double accum, Lisp_Object next)
{
...
static Lisp_Object bignum_arith_driver(enum arithop code, ptrdiff_t nargs,
                                       Lisp_Object* args, ptrdiff_t argnum,
                                       intmax_t iaccum, Lisp_Object val)
{
  mpz_t const* accum;
  if (argnum == 0)
  {
  ...
  while (true)
  {
    ...
    {
    case Aadd:
      mpz_add(mpz[0], *accum, *next);
      break;
    case Asub:
      mpz_sub(mpz[0], *accum, *next);
      break;
    case Amult:
      emacs_mpz_mul(mpz[0], *accum, *next);
      break;
    case Alogand:
      mpz_and(mpz[0], *accum, *next);
      break;
    case Alogior:
      mpz_ior(mpz[0], *accum, *next);
      break;
    case Alogxor:
      mpz_xor(mpz[0], *accum, *next);
      break;
    case Adiv:
      if (mpz_sgn(*next) == 0)
        xsignal0(Qarith_error);
...
static Lisp_Object arith_driver(enum arithop code, ptrdiff_t nargs,
                                Lisp_Object* args, Lisp_Object val)
{
  eassume(2 <= nargs);
...
/* Return NUM % DEN (or NUM mod DEN, if MODULO).  NUM and DEN must be
   integers.  */
static Lisp_Object integer_remainder(Lisp_Object num, Lisp_Object den,
                                     bool modulo)
{
  if (FIXNUMP(den))
  {
...
static Lisp_Object minmax_driver(ptrdiff_t nargs, Lisp_Object* args,
                                 cmp_bits_t cmpmask)
{
...
DEFUN("ash", Fash, Sash, 2, 2, 0, doc:
          /* Return integer VALUE with its bits shifted left by COUNT bit
positions. If COUNT is negative, shift VALUE to the right instead. VALUE and
COUNT must be integers. Mathematically, the return value is VALUE multiplied by
2 to the power of COUNT, rounded down.  If the result is non-zero, its sign is
the same as that of VALUE. In terms of bits, when COUNT is positive, the
function moves the bits of VALUE to the left, adding zero bits on the right;
when COUNT is negative, it moves the bits of VALUE to the right, discarding
bits.  */)
(Lisp_Object value, Lisp_Object count)
{
  ...
  if (!FIXNUMP(count))
  {
    ...
    {
      EMACS_INT v =
          (FIXNUMP(value) ? XFIXNUM(value) : mpz_sgn(*xbignum_val(value)));
...
Lisp_Object expt_integer(Lisp_Object x, Lisp_Object y)
{
  ...
  {
    if (ULONG_MAX <= MOST_POSITIVE_FIXNUM || !mpz_fits_ulong_p(*xbignum_val(y)))
      overflow_error();
...
static bits_word bool_vector_spare_mask(EMACS_INT nr_bits)
{
...
static Lisp_Object bool_vector_binop_driver(Lisp_Object a, Lisp_Object b,
                                            Lisp_Object dest,
...
void syms_of_data(void)
{
  ...
  Fput(Qerror, Qerror_message, build_string("error"));
...
#define PUT_ERROR(sym, tail, msg)                                              \
  ...
  PUT_ERROR(Qvoid_function, error_tail, "Symbol's function definition is void");
...
This variable cannot be set; trying to do so will signal an error.  */);
  ...
  DEFVAR_BOOL("symbols-with-pos-enabled", symbols_with_pos_enabled, doc:
                  /* If non-nil, a symbol with position ordinarily behaves as
its bare symbol. Bind this to non-nil in applications such as the byte
compiler.  */);
