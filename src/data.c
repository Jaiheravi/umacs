...
static void set_blv_value(struct Lisp_Buffer_Local_Value* blv,
                          Lisp_Object val) {
    XSETCDR(blv->valcell, val);
}

static void set_blv_where(struct Lisp_Buffer_Local_Value* blv,
                          Lisp_Object val) {
    blv->where = val;
}
...
static void add_to_function_history(Lisp_Object symbol, Lisp_Object olddef) {
    ...
    if (!NILP(tem)) { /* New def from a file used before.
                         Overwrite the previous record associated with this
                         file.  */
        if (EQ(tem, past))
...
void defalias(Lisp_Object symbol, Lisp_Object definition) {
    {
        ...
        if (!will_dump_p() ||
            !autoload) { /* Only add autoload entries after dumping, because the
                      ones before are not useful and else we get loads of them
                      from the loaddefs.el. That saves us about 110KB in the
                      pdmp file (Jan 2022).  */
...
(register Lisp_Object symbol, Lisp_Object definition, Lisp_Object docstring) {
    ...
    /* We used to return `definition', but now that `defun' and `defmacro'
       expand to a call to `defalias', we return `symbol' for backward
       compatibility (bug#11686).  */
    return symbol;
}
    ...
    case Lisp_Fwd_Obj:
     ...
     and update them.  */
        if (XOBJVAR(valcontents) > (Lisp_Object*)&buffer_defaults &&
            ...
            int offset =
                ((char*)XOBJVAR(valcontents) - (char*)&buffer_defaults);
            ...
            if (!BASE_EQ(blv->where, where)
                ...
                if (NILP(tem1)) {
                    ...
                    /* If it's a local_if_set, being set not bound,
                       ...
                       That means, give this buffer a new assoc for a local
                       value and load that binding.  */
                    else {
    ...
    case SYMBOL_FORWARDED:
        {
            ...
            if (BUFFER_OBJFWDP(valcontents)) {
                ...
                /* If this variable is not always local in all buffers,
                   set it in the buffers that don't nominally have a local
                   value.  */
                if (idx > 0) {
...
make_blv(struct Lisp_Symbol* sym, bool forwarded,
    ...
    tem = Fcons(symbol,
                (forwarded ? do_symval_forwarding(valcontents.fwd)
                           : valcontents.value));
...
intmax_t cons_to_signed(Lisp_Object c, intmax_t min, intmax_t max) {
    ...
    if (FLOATP(c)) {
        ...
        if (valid && CONSP(c)) {
            intmax_t top = val;
            Lisp_Object rest = XCDR(c);
            if (top >= INTMAX_MIN >> 24 >> 16 &&
                top <= INTMAX_MAX >> 24 >> 16 && CONSP(rest) &&
                FIXNATP(XCAR(rest)) && XFIXNAT(XCAR(rest)) < 1 << 24 &&
                FIXNATP(XCDR(rest)) && XFIXNAT(XCDR(rest)) < 1 << 16) {
...
(Lisp_Object number) {
    ...
    if (FLOATP(number))
        return make_unibyte_string(
            buffer, float_to_string(buffer, XFLOAT_DATA(number)));
...
This variable cannot be set; trying to do so will signal an error.  */);
    ...
    DEFVAR_BOOL("symbols-with-pos-enabled", symbols_with_pos_enabled, doc:
                    /* If non-nil, a symbol with position ordinarily behaves
  as its bare symbol. Bind this to non-nil in applications such as the byte
  compiler.  */);
