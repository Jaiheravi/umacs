...
#define GPT_ADDR                                                               \
    (current_buffer->text->beg + current_buffer->text->gpt_byte - BEG_BYTE)
...
extern void temp_set_point_both(struct buffer*, ptrdiff_t, ptrdiff_t);
extern void set_point_from_marker(Lisp_Object);
extern void enlarge_buffer_text(struct buffer*, ptrdiff_t);

INLINE void SET_PT(ptrdiff_t position) { set_point(position); }
INLINE void TEMP_SET_PT(ptrdiff_t position) {
    temp_set_point(current_buffer, position);
}
INLINE void SET_PT_BOTH(ptrdiff_t position, ptrdiff_t byte) {
    set_point_both(position, byte);
}
INLINE void TEMP_SET_PT_BOTH(ptrdiff_t position, ptrdiff_t byte) {
    temp_set_point_both(current_buffer, position, byte);
}
INLINE void BUF_TEMP_SET_PT(struct buffer* buffer, ptrdiff_t position) {
...
INLINE bool BUFFERP(Lisp_Object a) { return PSEUDOVECTORP(a, PVEC_BUFFER); }

INLINE void CHECK_BUFFER(Lisp_Object x) { CHECK_TYPE(BUFFERP(x), Qbufferp, x); }

INLINE struct buffer* XBUFFER(Lisp_Object a) {
    eassert(BUFFERP(a));
...
INLINE void bset_bidi_paragraph_direction(struct buffer* b, Lisp_Object val) {
    b->bidi_paragraph_direction_ = val;
}
INLINE void bset_cache_long_scans(struct buffer* b, Lisp_Object val) {
    b->cache_long_scans_ = val;
}
INLINE void bset_case_canon_table(struct buffer* b, Lisp_Object val) {
    b->case_canon_table_ = val;
}
INLINE void bset_case_eqv_table(struct buffer* b, Lisp_Object val) {
    b->case_eqv_table_ = val;
}
INLINE void bset_directory(struct buffer* b, Lisp_Object val) {
    b->directory_ = val;
}
INLINE void bset_display_count(struct buffer* b, Lisp_Object val) {
    b->display_count_ = val;
}
INLINE void bset_left_margin_cols(struct buffer* b, Lisp_Object val) {
    b->left_margin_cols_ = val;
}
INLINE void bset_right_margin_cols(struct buffer* b, Lisp_Object val) {
    b->right_margin_cols_ = val;
}
INLINE void bset_display_time(struct buffer* b, Lisp_Object val) {
    b->display_time_ = val;
}
INLINE void bset_downcase_table(struct buffer* b, Lisp_Object val) {
    b->downcase_table_ = val;
}
INLINE void bset_enable_multibyte_characters(struct buffer* b,
                                             Lisp_Object val) {
    b->enable_multibyte_characters_ = val;
}
INLINE void bset_filename(struct buffer* b, Lisp_Object val) {
    b->filename_ = val;
}
INLINE void bset_keymap(struct buffer* b, Lisp_Object val) { b->keymap_ = val; }
INLINE void bset_last_selected_window(struct buffer* b, Lisp_Object val) {
    b->last_selected_window_ = val;
}
INLINE void bset_local_var_alist(struct buffer* b, Lisp_Object val) {
    b->local_var_alist_ = val;
}
INLINE void bset_mark_active(struct buffer* b, Lisp_Object val) {
    b->mark_active_ = val;
}
INLINE void bset_point_before_scroll(struct buffer* b, Lisp_Object val) {
    b->point_before_scroll_ = val;
}
INLINE void bset_read_only(struct buffer* b, Lisp_Object val) {
    b->read_only_ = val;
}
INLINE void bset_truncate_lines(struct buffer* b, Lisp_Object val) {
    b->truncate_lines_ = val;
}
INLINE void bset_undo_list(struct buffer* b, Lisp_Object val) {
    b->undo_list_ = val;
}
INLINE void bset_upcase_table(struct buffer* b, Lisp_Object val) {
    b->upcase_table_ = val;
}
INLINE void bset_width_table(struct buffer* b, Lisp_Object val) {
    b->width_table_ = val;
}

INLINE void bset_text_conversion_style(struct buffer* b, Lisp_Object val) {
...
INLINE ptrdiff_t BUFFER_CEILING_OF(ptrdiff_t bytepos) {
    return (bytepos < GPT_BYTE && GPT < ZV ? GPT_BYTE : ZV_BYTE) - 1;
}

INLINE ptrdiff_t BUFFER_FLOOR_OF(ptrdiff_t bytepos) {
...
/* Position of beginning of accessible range of buffer.  */
INLINE ptrdiff_t BUF_BEGV(struct buffer* buf) {
...
INLINE ptrdiff_t BUF_BEGV_BYTE(struct buffer* buf) {
...
INLINE ptrdiff_t BUF_PT(struct buffer* buf) {
...
INLINE ptrdiff_t BUF_PT_BYTE(struct buffer* buf) {
...
/* Position of end of accessible range of buffer.  */
INLINE ptrdiff_t BUF_ZV(struct buffer* buf) {
...
INLINE ptrdiff_t BUF_ZV_BYTE(struct buffer* buf) {
...
/* Position of beginning of buffer.  */
INLINE ptrdiff_t BUF_BEG(struct buffer* buf) { return BEG; }

INLINE ptrdiff_t BUF_BEG_BYTE(struct buffer* buf) { return BEG_BYTE; }

/* Address of beginning of gap of buffer.  */
INLINE unsigned char* BUF_GPT_ADDR(struct buffer* buf) {
    return buf->text->beg + buf->text->gpt_byte - BEG_BYTE;
}

/* Address of end of buffer.  */
INLINE unsigned char* BUF_Z_ADDR(struct buffer* buf) {
    return buf->text->beg + buf->text->gap_size + buf->text->z_byte - BEG_BYTE;
}

/* Address of end of gap in buffer.  */
INLINE unsigned char* BUF_GAP_END_ADDR(struct buffer* buf) {
...
INLINE void BUF_COMPUTE_UNCHANGED(struct buffer* buf, ptrdiff_t start,
                                  ptrdiff_t end) {
...
INLINE void SET_BUF_BEGV(struct buffer* buf, ptrdiff_t charpos) {
    buf->begv_byte = buf_charpos_to_bytepos(buf, charpos);
    buf->begv = charpos;
}

INLINE void SET_BUF_ZV(struct buffer* buf, ptrdiff_t charpos) {
    buf->zv_byte = buf_charpos_to_bytepos(buf, charpos);
    buf->zv = charpos;
}

INLINE void SET_BUF_BEGV_BOTH(struct buffer* buf, ptrdiff_t charpos,
                              ptrdiff_t byte) {
    buf->begv = charpos;
    buf->begv_byte = byte;
}

INLINE void SET_BUF_ZV_BOTH(struct buffer* buf, ptrdiff_t charpos,
                            ptrdiff_t byte) {
    buf->zv = charpos;
    buf->zv_byte = byte;
}

INLINE void SET_BUF_PT_BOTH(struct buffer* buf, ptrdiff_t charpos,
                            ptrdiff_t byte) {
    buf->pt = charpos;
    buf->pt_byte = byte;
}
...
INLINE struct ts_linecol BUF_TS_LINECOL_BEGV(struct buffer* buf) {
    return buf->ts_linecol_begv;
} INLINE struct ts_linecol BUF_TS_LINECOL_POINT(struct buffer* buf) {
...
INLINE void SET_BUF_TS_LINECOL_POINT(struct buffer* buf,
                                     struct ts_linecol linecol) {
    buf->ts_linecol_point = linecol;
}

INLINE void SET_BUF_TS_LINECOL_ZV(struct buffer* buf,
                                  struct ts_linecol linecol) {
...
INLINE struct buffer* decode_buffer(Lisp_Object b) {
...
INLINE void set_buffer_internal(struct buffer* b) {
    if (current_buffer != b)
...
INLINE void record_unwind_current_buffer(void) {
...
#define GET_OVERLAYS_AT(posn, overlays, noverlays, next)                       \
    do {                                                                       \
        ...
        if ((noverlays) > maxlen) {                                            \
            ...
            SAFE_NALLOCA(overlays, 1, maxlen);                                 \
            (noverlays) =                                                      \
                overlays_at(posn, false, &(overlays), &maxlen, next);          \
        }                                                                      \
    } while (false)
...
INLINE INTERVAL buffer_intervals(struct buffer* b) {
    eassert(b->text != NULL);
...
INLINE void set_buffer_intervals(struct buffer* b, INTERVAL i) {
    eassert(b->text != NULL);
    b->text->intervals = i;
}
...
INLINE bool buffer_has_overlays(void) {
...
INLINE int FETCH_MULTIBYTE_CHAR(ptrdiff_t pos) {
...
INLINE int BUF_FETCH_MULTIBYTE_CHAR(struct buffer* buf, ptrdiff_t pos) {
    unsigned char* p = ((pos >= BUF_GPT_BYTE(buf) ? BUF_GAP_SIZE(buf) : 0) +
...
INLINE int FETCH_CHAR_AS_MULTIBYTE(ptrdiff_t pos) {
...
INLINE int FETCH_CHAR(ptrdiff_t pos) {
    return (!NILP(BVAR(current_buffer, enable_multibyte_characters))
                ? FETCH_MULTIBYTE_CHAR(pos)
                : FETCH_BYTE(pos));
}
...
INLINE unsigned char* BUF_BYTE_ADDRESS(struct buffer* buf, ptrdiff_t pos) {
...
INLINE unsigned char* BUF_CHAR_ADDRESS(struct buffer* buf, ptrdiff_t pos) {
...
INLINE ptrdiff_t BUF_PTR_BYTE_POS(struct buffer* buf, unsigned char* ptr) {
...
INLINE unsigned char BUF_FETCH_BYTE(struct buffer* buf, ptrdiff_t n) {
...
INLINE int BUF_FETCH_CHAR_AS_MULTIBYTE(struct buffer* buf, ptrdiff_t pos) {
...
INLINE int buffer_window_count(struct buffer* b) {
    if (b->base_buffer)
        b = b->base_buffer;
...
INLINE ptrdiff_t overlay_start(struct Lisp_Overlay* ov) {
    if (!ov->buffer)
        return -1;
    return itree_node_begin(ov->buffer->overlays, ov->interval);
}

INLINE ptrdiff_t overlay_end(struct Lisp_Overlay* ov) {
    if (!ov->buffer)
        return -1;
...
INLINE ptrdiff_t OVERLAY_START(Lisp_Object ov) {
    return overlay_start(XOVERLAY(ov));
}

/* Return the end of OV in its buffer, or -1. */

INLINE ptrdiff_t OVERLAY_END(Lisp_Object ov) {
    return overlay_end(XOVERLAY(ov));
}

/* Return the plist of overlay OV.  */

INLINE Lisp_Object OVERLAY_PLIST(Lisp_Object ov) { return XOVERLAY(ov)->plist; }

/* Return the buffer of overlay OV. */

INLINE struct buffer* OVERLAY_BUFFER(Lisp_Object ov) {
    return XOVERLAY(ov)->buffer;
}

/* Return true, if OV's rear-advance is set. */

INLINE bool OVERLAY_REAR_ADVANCE_P(Lisp_Object ov) {
    return XOVERLAY(ov)->interval->rear_advance;
}

/* Return true, if OV's front-advance is set. */

INLINE bool OVERLAY_FRONT_ADVANCE_P(Lisp_Object ov) {
...
#define PER_BUFFER_VAR_IDX(VAR) PER_BUFFER_IDX(PER_BUFFER_VAR_OFFSET(VAR))
...
INLINE bool PER_BUFFER_VALUE_P(struct buffer* b, int idx) {
...
INLINE void SET_PER_BUFFER_VALUE_P(struct buffer* b, int idx, bool val) {
    eassert(valid_per_buffer_idx(idx));
...
INLINE int PER_BUFFER_IDX(ptrdiff_t offset) {
...
INLINE Lisp_Object per_buffer_default(int offset) {
    return *(Lisp_Object*)(offset + (char*)&buffer_defaults);
}

INLINE void set_per_buffer_default(int offset, Lisp_Object value) {
...
INLINE Lisp_Object per_buffer_value(struct buffer* b, int offset) {
    return *(Lisp_Object*)(offset + (char*)b);
}

INLINE void set_per_buffer_value(struct buffer* b, int offset,
                                 Lisp_Object value) {
    *(Lisp_Object*)(offset + (char*)b) = value;
}

/* Downcase a character C, or make no change if that cannot be done.  */
INLINE int downcase(int c) {
...
INLINE bool lowercasep(int c) { return !uppercasep(c) && upcase(c) != c; }

/* Return a non-outlandish value for the tab width.  */

INLINE int sanitize_tab_width(Lisp_Object width) {
    return (FIXNUMP(width) && 0 < XFIXNUM(width) && XFIXNUM(width) <= 1000
                ? XFIXNUM(width)
                : 8);
}

INLINE int SANE_TAB_WIDTH(struct buffer* buf) {
    return sanitize_tab_width(BVAR(buf, tab_width));
}

/* Return a non-outlandish value for a character width.  */

INLINE int sanitize_char_width(EMACS_INT width) {
...
INLINE int CHARACTER_WIDTH(int c) {
...
INLINE int fetch_char_advance(ptrdiff_t* charidx, ptrdiff_t* byteidx) {
    int output;
...
INLINE int fetch_char_advance_no_check(ptrdiff_t* charidx, ptrdiff_t* byteidx) {
    int output;
    ptrdiff_t c = *charidx, b = *byteidx;
    c++;
    unsigned char* chp = BYTE_POS_ADDR(b);
...
INLINE int buf_next_char_len(struct buffer* buf, ptrdiff_t pos_byte) {
    unsigned char* chp = BUF_BYTE_ADDRESS(buf, pos_byte);
    return BYTES_BY_CHAR_HEAD(*chp);
}

INLINE int next_char_len(ptrdiff_t pos_byte) {
...
INLINE int buf_prev_char_len(struct buffer* buf, ptrdiff_t pos_byte) {
    unsigned char* chp =
...
INLINE int prev_char_len(ptrdiff_t pos_byte) {
    return buf_prev_char_len(current_buffer, pos_byte);
}

/* Increment both *CHARPOS and *BYTEPOS, each in the appropriate way.  */

INLINE void inc_both(ptrdiff_t* charpos, ptrdiff_t* bytepos) {
    (*charpos)++;
...
INLINE void dec_both(ptrdiff_t* charpos, ptrdiff_t* bytepos) {
    (*charpos)--;
    (*bytepos) -= (!NILP(BVAR(current_buffer, enable_multibyte_characters))
