...
struct region_cache* new_region_cache(void) {
    ...
    c->cache_len = 0;
    c->boundaries =
        xmalloc((c->gap_len + c->cache_len) * sizeof(*c->boundaries));
...
void free_region_cache(struct region_cache* c) {
    xfree(c->boundaries);
    xfree(c);
}
...
static ptrdiff_t find_cache_boundary(struct region_cache* c, ptrdiff_t pos) {
    ...
    eassert(!(BOUNDARY_POS(c, low) > pos ||
              (low + 1 < c->cache_len && BOUNDARY_POS(c, low + 1) <= pos)));
...
static void move_cache_gap(struct region_cache* c, ptrdiff_t pos,
                           ptrdiff_t min_size) {
    ...
    while (gap_start < pos) {
        ...
        c->boundaries[gap_start].pos =
            (buffer_end + c->boundaries[gap_start + gap_len].pos - buffer_beg);
    ...
    if (gap_len < min_size) {
        ...
        c->boundaries = xpalloc(c->boundaries, &nboundaries, min_size - gap_len,
                                -1, sizeof *c->boundaries);
        ...
        for (i = c->cache_len - 1; i >= gap_start; i--) {
            ...
            c->boundaries[i + min_size].value =
                c->boundaries[i + gap_len].value;
        }
...
static void insert_cache_boundary(struct region_cache* c, ptrdiff_t i,
                                  ptrdiff_t pos, int value) {
    ...
    eassert((BOUNDARY_POS(c, i - 1) < pos &&
             (i == c->cache_len || pos < BOUNDARY_POS(c, i))));
...
static void delete_cache_boundaries(struct region_cache* c, ptrdiff_t start,
                                    ptrdiff_t end) {
...
static void set_cache_region(struct region_cache* c, ptrdiff_t start,
                             ptrdiff_t end, int value) {
    eassert(start <= end);
...

    {
        ...
        if (BOUNDARY_POS(c, start_ix) == start) {
            /* Is this boundary necessary?  If no, remove it; if yes, set
               its value.  */
            if (start_ix > 0 && BOUNDARY_VALUE(c, start_ix - 1) == value) {
        ...
        if (end == c->buffer_end)
            ...
            ;
        else if (end_ix >= c->cache_len || end < BOUNDARY_POS(c, end_ix)) {
...
void invalidate_region_cache(struct buffer* buf, struct region_cache* c,
...
static void revalidate_region_cache(struct buffer* buf,
    ...
    /* If the entire buffer is still valid, don't waste time.  Yes, this
       ...
       insertion.  */
    if (c->buffer_beg + c->beg_unchanged > c->buffer_end - c->end_unchanged)
        return;
    ...
    if (c->buffer_beg + c->beg_unchanged == c->buffer_end - c->end_unchanged) {
        ...
        move_cache_gap(
            c, (find_cache_boundary(c, (c->buffer_beg + c->beg_unchanged)) + 1),
            0);
        ...
        /* Now that the cache's basis has been changed, the modified
           ...
           invalidate it.  */
        set_cache_region(c, c->buffer_beg + c->beg_unchanged,
                         c->buffer_end - c->end_unchanged, 0);
    }
    ...
    c->beg_unchanged = c->end_unchanged = c->buffer_end - c->buffer_beg;
}
...
void know_region_cache(struct buffer* buf, struct region_cache* c,
...
int region_cache_forward(struct buffer* buf, struct region_cache* c,
...
void pp_cache(struct region_cache* c) {
    ...
    fprintf(stderr,
            ...
            c->buffer_beg, c->buffer_end, beg_u, end_u);
