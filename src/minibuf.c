...
static bool minibuf_follows_frame(void) {
    return EQ(Fdefault_toplevel_value(Qminibuffer_follows_selected_frame), Qt);
}
...
static bool minibuf_moves_frame_when_opened(void) {
    return !NILP(Fdefault_toplevel_value(Qminibuffer_follows_selected_frame));
}
...
static void zip_minibuffer_stacks(Lisp_Object dest_window,
    ...
    if (!live_minibuffer_p(dw->contents) && NILP(dw->prev_buffers)) {
...
void move_minibuffers_onto_frame(struct frame* of, Lisp_Object frame,
                                 bool for_deletion) {
    ...
    if (!(minibuf_level &&
        ...
        return;
    if (FRAME_LIVE_P(f) && !EQ(f->minibuffer_window, of->minibuffer_window) &&
...
(void) {
    ...
    FOR_EACH_FRAME(frames, frame) {
        f = XFRAME(frame);
        if (FRAME_LIVE_P(f) && WINDOW_LIVE_P(f->minibuffer_window) &&
...
static Lisp_Object string_to_object(Lisp_Object val, Lisp_Object defalt) {
...
static Lisp_Object read_minibuf_noninteractive(Lisp_Object prompt, bool expflag,
                                               Lisp_Object defalt) {
    ptrdiff_t size, len;
    char* line;
    Lisp_Object val;
...
static bool live_minibuffer_p(Lisp_Object buffer) {
    Lisp_Object tem;
    EMACS_INT i;
...
(Lisp_Object buffer, Lisp_Object live) {
    ...
    return (NILP(live) ? !NILP(Fmemq(buffer, Vminibuffer_list))
...
(Lisp_Object buffer) {
    ...
    return BASE_EQ(
               buffer,
               (Fcar(Fnthcdr(make_fixnum(minibuf_level), Vminibuffer_list))))
        ? Qt
        : Qnil;
}
...
DEFUN("minibuffer-innermost-command-loop-p",
      ...
      Sminibuffer_innermost_command_loop_p, 0, 1, 0, doc:
          /* Return t if BUFFER is a minibuffer at the current command loop
level. No argument or nil as argument means use the current buffer as BUFFER.
*/)
(Lisp_Object buffer) {
    EMACS_INT depth;
    if (NILP(buffer))
    ...
    return depth && minibuf_c_loop_level(depth) == command_loop_level ? Qt
                                                                      : Qnil;
}
...
this_minibuffer_depth(Lisp_Object buffer) {
    ...
    if (NILP(buffer))
        buffer = Fcurrent_buffer();
    for (i = 1, bufs = Fcdr(Vminibuffer_list); i <= minibuf_level;
         i++, bufs = Fcdr(bufs))
...
static Lisp_Object read_minibuf(Lisp_Object map, Lisp_Object initial,
                                Lisp_Object prompt, bool expflag,
                                Lisp_Object histvar, Lisp_Object histpos,
                                Lisp_Object defalt, bool allow_props,
    ...
    if (!enable_recursive_minibuffers && minibuf_level > 0) {
        Lisp_Object str = build_string(
    ...
    if (minibuf_level > 1 &&
        ...
        !EQ(XWINDOW(XFRAME(selected_frame)->minibuffer_window)->frame,
            MB_frame) &&
        minibuf_moves_frame_when_opened() && (!minibuf_follows_frame())) {
        ...
        if (MINI_WINDOW_P(XWINDOW(FRAME_SELECTED_WINDOW(of))))
            Fset_frame_selected_window(MB_frame, Fframe_first_window(MB_frame),
                                       Qnil);
    }
    ...
    /* Arrange to restore a number of minibuffer-related variables.
       ...
       specpdl slots.  */
    minibuf_save_list = Fcons(
        Voverriding_local_map,
        Fcons(minibuf_window,
              Fcons(calling_frame, Fcons(calling_window, minibuf_save_list))));
    minibuf_save_list =
                          ...
                          Fcons(Vcurrent_prefix_arg,
                                Fcons(Vminibuffer_history_position,
                                      Fcons(Vminibuffer_history_variable,
                                            minibuf_save_list))))));
    minibuf_save_list = Fcons(Fthis_command_keys_vector(), minibuf_save_list);
    ...
    if (STRINGP(ambient_dir))
        ...
        if (STRINGP(BVAR(XBUFFER(buf), directory))) {
            bset_directory(current_buffer, BVAR(XBUFFER(buf), directory));
            break;
        }
    }
    ...
    /* Erase the buffer.  */
    {
        ...
        bset_enable_multibyte_characters(
            current_buffer, inherit_input_method ? enable_multibyte : Qt);

        /* Insert the prompt, record where it ends.  */
        Finsert(1, &minibuf_prompt);
        if (PT > BEG) {
            Fput_text_property(make_fixnum(BEG), make_fixnum(PT), Qfront_sticky,
                               Qt, Qnil);
            Fput_text_property(make_fixnum(BEG), make_fixnum(PT),
                               Qrear_nonsticky, Qt, Qnil);
            Fput_text_property(make_fixnum(BEG), make_fixnum(PT), Qfield, Qt,
                               Qnil);
                    ...
                    if (CONSP(list)) {
                        ...
                        if (EQ(key, Qface))
                            Fadd_face_text_property(make_fixnum(BEG),
                                                    make_fixnum(PT), val, Qt,
                                                    Qnil);
                        else
                            Fput_text_property(make_fixnum(BEG),
                                               make_fixnum(PT), key, val, Qnil);
                    }
                }
            }
        }
    ...
    if (XWINDOW(minibuf_window)->cursor.vpos >= 0 && !noninteractive &&
    ...
    if (FRAMEP(calling_frame) && FRAME_LIVE_P(XFRAME(calling_frame)) &&
...
bool is_minibuffer(EMACS_INT depth, Lisp_Object buf) {
    Lisp_Object tail = Fnthcdr(make_fixnum(depth), Vminibuffer_list);
    return !NILP(tail) && EQ(Fcar(tail), buf);
}
...
Lisp_Object get_minibuffer(EMACS_INT depth) {
    Lisp_Object tail = Fnthcdr(make_fixnum(depth), Vminibuffer_list);
    Lisp_Object cll_tail =
        Fnthcdr(make_fixnum(depth), Vcommand_loop_level_list);
    if (NILP(tail)) {
        tail = list1(Qnil);
...
static void read_minibuf_unwind(void) {
    ...
    FOR_EACH_FRAME(frames, exp_MB_frame) {
        ...
        if (WINDOW_LIVE_P(window)) {
            w = XWINDOW(window);
            if (EQ(w->frame, exp_MB_frame) && EQ(w->contents, expired_MB))
                goto found;
        }
    }
...
found:
    if (!EQ(exp_MB_frame, saved_selected_frame) && !NILP(exp_MB_frame))
  ...
  if (FRAME_LIVE_P (XFRAME (WINDOW_FRAME (XWINDOW (temp)))))
    ...
    /* In case the previous minibuffer displayed in this miniwindow is
       dead, we may keep displaying this buffer (tho it's inactive), so reset
       it, to make sure we don't leave around bindings and stuff which only made
       sense during the read_minibuf invocation.  */
    call0(Qminibuffer_inactive_mode);
...
void barf_if_interaction_inhibited(void) {
...
(Lisp_Object prompt, Lisp_Object initial_contents, Lisp_Object keymap,
 Lisp_Object read, Lisp_Object hist, Lisp_Object default_value,
 Lisp_Object inherit_input_method) {
    ...
    val = read_minibuf(keymap, initial_contents, prompt, !NILP(read), histvar,
                       histpos, default_value, minibuffer_allow_text_properties,
...
(Lisp_Object prompt, Lisp_Object initial_input, Lisp_Object history,
 Lisp_Object default_value, Lisp_Object inherit_input_method) {
    ...
    val = Fread_from_minibuffer(prompt, initial_input, Qnil, Qnil, history,
                                default_value, inherit_input_method);
...
(Lisp_Object prompt, Lisp_Object default_value) {
    ...
    name = Fcompleting_read(prompt, Vobarray, Qcommandp, Qt, Qnil, Qnil,
                            default_string, Qnil);
    if (NILP(name))
        return name;
...
(Lisp_Object prompt, Lisp_Object default_value) {
    ...
    name = Fcompleting_read(prompt, Vobarray, Qcustom_variable_p, Qt, Qnil,
                            Qcustom_variable_history, default_string, Qnil);
    if (NILP(name))
        return name;
    return Fintern(name, Qnil);
}
    ...
    if (NILP(Vread_buffer_function)) {
        if (!NILP(def)) {
            ...
            if (STRINGP(prompt)) {
                ...
                prompt =
                    make_specified_string(s, -1, len, STRING_MULTIBYTE(prompt));
            }

            prompt =
                calln(Qformat_prompt, prompt, CONSP(def) ? XCAR(def) : def);
        }

        result = Fcompleting_read(prompt, Qinternal_complete_buffer, predicate,
                                  require_match, Qnil, Qbuffer_name_history,
                                  def, Qnil);
    }
    else
        result = (NILP(predicate)
                      /* Partial backward compatibility for older
                     read_buffer_functions which don't expect a `predicate'
                     argument.  */
...
static bool match_regexps(Lisp_Object string, Lisp_Object regexps,
                          bool ignore_case) {
    ptrdiff_t val;
...
(Lisp_Object string, Lisp_Object collection, Lisp_Object predicate) {
    ...
    while (1) {
       ...
       eltstring gets the name to check as a completion.  */
        ...
        {
            while (
                idx < HASH_TABLE_SIZE(XHASH_TABLE(collection)) &&
                hash_unused_entry_key_p(HASH_KEY(XHASH_TABLE(collection), idx)))
                idx++;
        ...
        if (STRINGP(eltstring) && SCHARS(string) <= SCHARS(eltstring) &&
            (tem = Fcompare_strings(eltstring, zero,
                                    make_fixnum(SCHARS(string)), string, zero,
                                    Qnil, completion_ignore_case ? Qt : Qnil),
             EQ(Qt, tem))) {
            ...
            if (NILP(bestmatch)) {
                ...
                tem = Fcompare_strings(bestmatch, zero, lcompare, eltstring,
                                       zero, lcompare,
                ...
                if (bestmatchsize != SCHARS(eltstring) ||
                    ...
                    (completion_ignore_case &&
                     !BASE_EQ(Fcompare_strings(old_bestmatch, zero, lcompare,
                                               eltstring, zero, lcompare, Qnil),
                              Qt)))
                ...
                if (matchsize <= SCHARS(string)
                    ...
                    && !completion_ignore_case && matchcount > 1)
...
(Lisp_Object string, Lisp_Object collection, Lisp_Object predicate) {
    ...
    while (1) {
       ...
       eltstring gets the name to check as a completion.  */
        ...
        if (STRINGP(eltstring) && SCHARS(string) <= SCHARS(eltstring) &&
            (tem =
                 Fcompare_strings(eltstring, zero, make_fixnum(SCHARS(string)),
                                  string, zero, make_fixnum(SCHARS(string)),
                                  completion_ignore_case ? Qt : Qnil),
             EQ(Qt, tem))) {
            /* Ignore this element if it fails to match all the regexps.  */
            if (!match_regexps(eltstring, Vcompletion_regexp_list,
...
(Lisp_Object prompt, Lisp_Object collection, Lisp_Object predicate,
 ...
 Lisp_Object def, Lisp_Object inherit_input_method) {
    return calln(Fsymbol_value(Qcompleting_read_function), prompt, collection,
...
(Lisp_Object string, Lisp_Object collection, Lisp_Object predicate) {
    ...
    if (NILP(collection) || (CONSP(collection) && !FUNCTIONP(collection))) {
        tem = Fassoc_string(string, collection,
                            completion_ignore_case ? Qt : Qnil);
        if (NILP(tem))
            return Qnil;
    }
        ...
        tem = oblookup(collection, SSDATA(string), SCHARS(string),
                       SBYTES(string));
        if (completion_ignore_case && !BARE_SYMBOL_P(tem))
            DOOBARRAY(XOBARRAY(collection), it) {
                Lisp_Object obj = obarray_iter_symbol(&it);
                if (BASE_EQ(Fcompare_strings(string, make_fixnum(0), Qnil,
                                             Fsymbol_name(obj), make_fixnum(0),
                                             Qnil, Qt),
                            Qt)) {
                    tem = obj;
                    break;
        ...
        if (i >= 0) {
            ...
            DOHASH(h, k, v) {
                ...
                if (!STRINGP(strkey))
                    continue;
                if (BASE_EQ(
                        Fcompare_strings(string, Qnil, Qnil, strkey, Qnil, Qnil,
                                         completion_ignore_case ? Qt : Qnil),
                        Qt)) {
                    arg = v;
                    goto found_matching_key;
    ...
    if (!match_regexps(string, Vcompletion_regexp_list, completion_ignore_case))
        return Qnil;

    /* Finally, check the predicate.  */
    if (!NILP(predicate)) {
        return HASH_TABLE_P(collection) ? calln(predicate, tem, arg)
                                        : calln(predicate, tem);
    }
    else
        return Qt;
...
(Lisp_Object string, Lisp_Object predicate, Lisp_Object flag) {
    if (NILP(flag))
        ...
        return list3(Qmetadata, Fcons(Qcategory, Qbuffer),
...
DEFUN("minibuffer-depth", Fminibuffer_depth, Sminibuffer_depth, 0, 0, 0, doc:/* Return current depth of activations of minibuffer, a nonnegative integer.  */)
(void) { return make_fixnum(minibuf_level); }
...
(void) { return Fcopy_sequence(minibuf_prompt); }


void set_initial_minibuffer_mode(void) {
        ...
        for (iter_t j = prev_match; j.x < strlen; j.x++) {
            ...
            if (cmatch) {
                ...
                /* Compute M[i,j]. If we pick M[i-1,j-1] it means that
                   ...
                   match from a gap.  */
                MAT(M, i.x, j.x) =
                    min(MAT(M, i.x - 1, j.x - 1), MAT(D, i.x - 1, j.x - 1));
            }
            ...
            MAT(D, i.x, j.x) = min(MAT(M, i.x, j.x - 1) + gap_open_cost,
...
For example, `eval-expression' uses this.  */);
    ...
    DEFVAR_LISP(
        "read-buffer-function", Vread_buffer_function,
        doc:/* If this is non-nil, `read-buffer' does its work by calling this
function. The function is called with the arguments passed to `read-buffer'.
*/);
...
:/* Within call to `completing-read', this holds the PREDICATE argument.  */);
    ...
    DEFVAR_LISP(
        "minibuffer-completion-confirm", Vminibuffer_completion_confirm,
        doc:/* Whether to demand confirmation of completion before exiting
minibuffer. If nil, confirmation is not required. If the value is `confirm',
the user may exit with an input that is not a valid completion alternative,
but Emacs asks for confirmation. If the value is `confirm-after-completion',
the user may exit with an input that is not a valid completion alternative,
but Emacs asks for confirmation if the user submitted the input right after
any of the completion commands listed in `minibuffer-confirm-exit-commands'.
*/);
...
is added with
 ...
 If the variable is t, no history is recorded.  */);
    ...
    DEFVAR_BOOL(
        "minibuffer-auto-raise", minibuffer_auto_raise, doc:
            /* Non-nil means entering the minibuffer raises the minibuffer's
frame. Some uses of the echo area also raise that frame (since they use it
too).  */);
...
uses to hide passwords.  */);
    ...
    DEFVAR_BOOL(
        "inhibit-interaction", inhibit_interaction, doc:
