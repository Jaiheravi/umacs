...
Copyright (C) 1988, 1993-1994, 1999, 2001-2026 Free Software Foundation,
Inc.
...
DEFUN("asin", Fasin, Sasin, 1, 1, 0, doc:/* Return the inverse sine of ARG.  */)
(Lisp_Object arg) {
    double d = extract_float(arg);
    d = asin(d);
    return make_float(d);
...
DEFUN("cos", Fcos, Scos, 1, 1, 0, doc:/* Return the cosine of ARG.  */)
(Lisp_Object arg) {
    double d = extract_float(arg);
    d = cos(d);
    return make_float(d);
...
DEFUN("sin", Fsin, Ssin, 1, 1, 0, doc:/* Return the sine of ARG.  */)
(Lisp_Object arg) {
    double d = extract_float(arg);
    d = sin(d);
    return make_float(d);
...
DEFUN("tan", Ftan, Stan, 1, 1, 0, doc:/* Return the tangent of ARG.  */)
(Lisp_Object arg) {
    double d = extract_float(arg);
    d = tan(d);
    return make_float(d);
...
DEFUN("sqrt", Fsqrt, Ssqrt, 1, 1, 0, doc:/* Return the square root of ARG.  */)
(Lisp_Object arg) {
    double d = extract_float(arg);
    d = sqrt(d);
    return make_float(d);
...
DEFUN("abs", Fabs, Sabs, 1, 1, 0, doc:/* Return the absolute value of ARG.  */)
(Lisp_Object arg) {
    CHECK_NUMBER(arg);
...
int double_integer_scale(double d) {
...
static mpz_t const* rescale_for_division(Lisp_Object n, mpz_t* t, int nscale,
                                         int dscale) {
    mpz_t const* pn;
...
static EMACS_INT ceiling2(EMACS_INT n, EMACS_INT d) {
    return n / d + ((n % d != 0) & ((n < 0) == (d < 0)));
}

static EMACS_INT floor2(EMACS_INT n, EMACS_INT d) {
    return n / d - ((n % d != 0) & ((n < 0) != (d < 0)));
}

static EMACS_INT truncate2(EMACS_INT n, EMACS_INT d) { return n / d; }

static EMACS_INT round2(EMACS_INT n, EMACS_INT d) {
...
static void rounddiv_q(mpz_t q, mpz_t const n, mpz_t const d) {
...
#else
static double emacs_rint(double d) {
    double d1 = d + 0.5;
    double r = floor(d1);
...
#ifndef HAVE_TRUNC
double trunc(double d) { return (d < 0 ? ceil : floor)(d); }
#endif
...
DEFUN("floor", Ffloor, Sfloor, 1, 2, 0, doc:
...
Lisp_Object fmod_float(Lisp_Object x, Lisp_Object y) {
