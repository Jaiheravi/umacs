...
void move_gap_both(ptrdiff_t charpos, ptrdiff_t bytepos) {
...
static void gap_left(ptrdiff_t charpos, ptrdiff_t bytepos, bool newgap) {
...
static void adjust_suspend_auto_hscroll(ptrdiff_t from, ptrdiff_t to) {
...
static void adjust_markers_for_insert(ptrdiff_t from, ptrdiff_t from_byte,
                                      ptrdiff_t to, ptrdiff_t to_byte,
                                      bool before_markers) {
    struct Lisp_Marker* m;
...
static void adjust_point(ptrdiff_t nchars, ptrdiff_t nbytes) {
...
static void adjust_markers_for_replace(ptrdiff_t from, ptrdiff_t from_byte,
    ...
    if (old_chars == 0) {
       ...
       to marker types.  */
        adjust_markers_for_insert(from, from_byte, from + new_chars,
                                  from_byte + new_bytes, false);
        return;
    }
...
static ptrdiff_t count_bytes(ptrdiff_t pos, ptrdiff_t bytepos,
                             ptrdiff_t endpos) {
    eassert(BEG_BYTE <= bytepos && bytepos <= Z_BYTE && BEG <= endpos &&
            endpos <= Z);
...
void adjust_markers_bytepos(ptrdiff_t from, ptrdiff_t from_byte, ptrdiff_t to,
                            ptrdiff_t to_byte, int to_z) {
    register struct Lisp_Marker* m;
    ...
    if (Z == Z_BYTE || (!to_z && to == to_byte)) {
       ...
       its charpos.  */
        for (m = BUF_MARKERS(current_buffer); m; m = m->next) {
            if (m->bytepos > from_byte && (to_z || m->bytepos <= to_byte))
                m->bytepos = m->charpos;
        }
    }
    else {
        for (m = BUF_MARKERS(current_buffer); m; m = m->next) {
            ...
            if (m->bytepos > from_byte && (to_z || m->bytepos <= to_byte)) {
                if (m->charpos < beg && beg - m->charpos > m->charpos - from) {
                    beg = from;
                    begbyte = from_byte;
                }
...
void buffer_overflow(void) { error("Maximum buffer size exceeded"); }

/* Make the gap NBYTES_ADDED bytes longer.  */

static void make_gap_larger(ptrdiff_t nbytes_added) {
    Lisp_Object tem;
    ...
    nbytes_added =
        min(nbytes_added + GAP_BYTES_DFL, BUF_BYTES_MAX - current_size);
...
static void make_gap_smaller(ptrdiff_t nbytes_removed) {
    Lisp_Object tem;
    ptrdiff_t real_gap_loc;
    ptrdiff_t real_gap_loc_byte;
    ptrdiff_t real_Z;
...
void make_gap(ptrdiff_t nbytes_added) {
    if (nbytes_added >= 0)
        /* With set-buffer-multibyte on a large buffer, we can end up growing
         * the buffer *many* times.  Avoid an O(N^2) behavior by increasing by
         * an amount at least proportional to the size of the buffer. On my test
         * (a 223.9MB zip file on a Thinkpad T61): With /5    =>  24s With /32
         * =>  25s With /64   =>  26s With /128  =>  28s With /1024 =>  51s With
         * /4096 => 131s With /∞    => gave up after 858s Of course, ideally we
         * should never call set-buffer-multibyte on a non-empty buffer (e.g.
         * use buffer-swap-text instead). We chose /64 because it already brings
         * almost the best performance while limiting the potential wasted
         * memory to 1.5%.  */
...
void make_gap_1(struct buffer* b, ptrdiff_t nbytes) {
...
ptrdiff_t copy_text(const unsigned char* from_addr, unsigned char* to_addr,
...
void insert_and_inherit(const char* string, ptrdiff_t nbytes) {
    if (nbytes > 0) {
        ptrdiff_t len = chars_in_text((unsigned char*)string, nbytes), opoint;
...
void insert_string(const char* s) { insert(s, strlen(s)); }
...
void insert_before_markers(const char* string, ptrdiff_t nbytes) {
    if (nbytes > 0) {
        ptrdiff_t len = chars_in_text((unsigned char*)string, nbytes), opoint;
...
void insert_before_markers_and_inherit(const char* string, ptrdiff_t nbytes) {
    if (nbytes > 0) {
        ptrdiff_t len = chars_in_text((unsigned char*)string, nbytes), opoint;
...
int count_combining_before(const unsigned char* string, ptrdiff_t length,
...
int count_combining_after(const unsigned char* string, ptrdiff_t length,
                          ptrdiff_t pos, ptrdiff_t pos_byte) {
...
void insert_1_both(const char* string, ptrdiff_t nchars, ptrdiff_t nbytes,
    ...
    struct ts_linecol start_linecol = treesit_linecol_maybe(
        PT, PT_BYTE, BUF_TS_LINECOL_POINT(current_buffer));
#endif
    ...
    if (!inherit && buffer_intervals(current_buffer))
        set_text_properties(make_fixnum(PT), make_fixnum(PT + nchars), Qnil,
                            Qnil, Qnil);
    ...
    treesit_record_change(PT_BYTE, PT_BYTE, PT_BYTE + nbytes, start_linecol,
                          start_linecol, PT + nchars);
#endif
...
void insert_from_string(Lisp_Object string, ptrdiff_t pos, ptrdiff_t pos_byte,
    ...
    insert_from_string_1(string, pos, pos_byte, length, length_byte, inherit,
                         0);
...
void insert_from_string_before_markers(Lisp_Object string, ptrdiff_t pos,
...
static void insert_from_string_1(Lisp_Object string, ptrdiff_t pos,
    ...
    adjust_markers_for_insert(PT, PT_BYTE, PT + nchars,
                              PT_BYTE + outgoing_nbytes, before_markers);
...
void insert_from_gap_1(ptrdiff_t nchars, ptrdiff_t nbytes,
    ...
    struct ts_linecol start_linecol = treesit_linecol_maybe(
        GPT, GPT_BYTE, BUF_TS_LINECOL_POINT(current_buffer));
#endif
...
void insert_from_gap(ptrdiff_t nchars, ptrdiff_t nbytes, bool text_at_gap_tail,
                     bool before_markers) {
    ...
    adjust_markers_for_insert(ins_charpos, ins_bytepos, ins_charpos + nchars,
                              ins_bytepos + nbytes, before_markers);

    if (buffer_intervals(current_buffer)) {
        offset_intervals(current_buffer, ins_charpos, nchars);
        graft_intervals_into_buffer(NULL, ins_charpos, nchars, current_buffer,
                                    0);
    }
...
void insert_from_buffer(struct buffer* buf, ptrdiff_t charpos, ptrdiff_t nchars,
                        bool inherit) {
    ptrdiff_t opoint = PT;
    ...
    struct ts_linecol start_linecol = treesit_linecol_maybe(
        opoint, obyte, BUF_TS_LINECOL_POINT(current_buffer));
#endif
    ...
    treesit_record_change(obyte, obyte, PT_BYTE, start_linecol, start_linecol,
                          PT);
#endif
}
...
static void insert_from_buffer_1(struct buffer* buf, ptrdiff_t from,
    ...
    if (NILP(BVAR(current_buffer, enable_multibyte_characters)))
        ...
        if (from < BUF_GPT(buf)) {
            ...
            outgoing_before_gap = count_size_as_multibyte(
                BUF_BYTE_ADDRESS(buf, from_byte), chunk);
        }
        else
            chunk = 0;

        if (chunk < incoming_nbytes)
            outgoing_after_gap = count_size_as_multibyte(
                BUF_BYTE_ADDRESS(buf, from_byte + chunk),
    ...
    if (from < BUF_GPT(buf)) {
       ...
       to put the output from the second copy_text.  */
        chunk_expanded =
            copy_text(BUF_BYTE_ADDRESS(buf, from_byte), GPT_ADDR, chunk,
...
static void adjust_after_replace(ptrdiff_t from, ptrdiff_t from_byte,
    ...
    if (nchars_del > 0)
        adjust_markers_for_replace(from, from_byte, nchars_del, nbytes_del, len,
                                   len_byte);
    else
        adjust_markers_for_insert(from, from_byte, from + len,
                                  from_byte + len_byte, false);
...
void adjust_after_insert(ptrdiff_t from, ptrdiff_t from_byte, ptrdiff_t to,
                         ptrdiff_t to_byte, ptrdiff_t newlen) {
...
void replace_range(ptrdiff_t from, ptrdiff_t to, Lisp_Object new,
                   bool run_mod_hooks, bool inherit, bool adjust_match_data) {
    ptrdiff_t inschars;
    ptrdiff_t insbeg;
...
#ifdef HAVE_TREE_SITTER
    struct ts_linecol start_linecol = treesit_linecol_maybe(
        from, from_byte, BUF_TS_LINECOL_POINT(current_buffer));
    struct ts_linecol old_end_linecol = treesit_linecol_maybe(
        to, to_byte, BUF_TS_LINECOL_POINT(current_buffer));
#endif
    ...
    if (NILP(BVAR(current_buffer, enable_multibyte_characters)))
        ...
        outgoing_insbytes = count_size_as_multibyte(insbeg_ptr, insbytes);
    ...
    /* Copy the string text into the buffer, perhaps converting
       between single-byte and multibyte.  */
    copy_text(insbeg_ptr, GPT_ADDR, insbytes, new_is_multibyte,
              !NILP(BVAR(current_buffer, enable_multibyte_characters)));
    ...
    /* Insert those intervals.  */
    graft_intervals_into_buffer(intervals, from, inschars, current_buffer,
                                inherit);
...
void replace_range_2(ptrdiff_t from, ptrdiff_t from_byte, ptrdiff_t to,
                     ptrdiff_t to_byte, const char* ins, ptrdiff_t inschars,
                     ptrdiff_t insbytes, bool markers) {
    ptrdiff_t nbytes_del, nchars_del;
...
void del_range(ptrdiff_t from, ptrdiff_t to) { del_range_1(from, to, 1, 0); }
...
static Lisp_Object safe_del_range_1(void* ptr) {
...
static Lisp_Object safe_del_range_2(enum nonlocal_exit type,
                                    Lisp_Object value) {
    return Qt;
}
...
int safe_del_range(ptrdiff_t from, ptrdiff_t to) {
    struct safe_del_range_context context;
    ...
    return !NILP(
        internal_catch_all(safe_del_range_1, &context, safe_del_range_2));
}

/* Like del_range; PREPARE says whether to call prepare_to_modify_buffer.
   RET_STRING says to return the deleted text. */

Lisp_Object del_range_1(ptrdiff_t from, ptrdiff_t to, bool prepare,
                        bool ret_string) {
    ptrdiff_t from_byte, to_byte;
    Lisp_Object deletion;
...
void del_range_byte(ptrdiff_t from_byte, ptrdiff_t to_byte) {
    ptrdiff_t from, to;
...
void del_range_both(ptrdiff_t from, ptrdiff_t from_byte, ptrdiff_t to,
...
Lisp_Object del_range_2(ptrdiff_t from, ptrdiff_t from_byte, ptrdiff_t to,
    ...
    /* Relocate point as if it were a marker.  */
    if (from < PT)
        adjust_point(from - min(PT, to), from_byte - min(PT_BYTE, to_byte));
    ...
    eassert(from_byte >= 0);
    treesit_record_change(from_byte, to_byte, from_byte, start_linecol,
                          old_end_linecol, from);
#endif
...
void modify_text(ptrdiff_t start, ptrdiff_t end) {
...
/* Signal that we are about to make a change that may result in new
   undo information.
 */
static void run_undoable_change(void) {
    if (EQ(BVAR(current_buffer, undo_list), Qt))
        return;
...
void prepare_to_modify_buffer_1(ptrdiff_t start, ptrdiff_t end,
    ...
    if (!NILP(BVAR(base_buffer, file_truename))
        ...
        && !NILP(BVAR(base_buffer, filename)) && SAVE_MODIFF >= MODIFF)
    ...
    if (!NILP(BVAR(current_buffer, mark_active)) &&
        ...
        (EQ(Vselect_active_regions, Qonly)
             ...
             : (!NILP(Vselect_active_regions) && !NILP(Vtransient_mark_mode))))
        Vsaved_region_selection = calln(Vregion_extract_function, Qnil);
...
void prepare_to_modify_buffer(ptrdiff_t start, ptrdiff_t end,
                              ptrdiff_t* preserve_ptr) {
...
void invalidate_buffer_caches(struct buffer* buf, ptrdiff_t start,
                              ptrdiff_t end) {
    ...
    if (buf->bidi_paragraph_cache) {
        if (start > BUF_BEG(buf)) {
            ...
            if ((start == end) == (prev_char == '\n')) {
                ...
                line_beg =
                    find_newline_no_quit(start, start_byte, -1, &start_byte);
                set_buffer_internal(old);
            }
    ...
    if (buf->newline_cache)
        invalidate_region_cache(buf, buf->newline_cache, start - BUF_BEG(buf),
                                BUF_Z(buf) - end);
    if (buf->width_run_cache)
        invalidate_region_cache(buf, buf->width_run_cache, start - BUF_BEG(buf),
                                BUF_Z(buf) - end);
}
...
#define FETCH_END (!NILP(end_marker) ? Fmarker_position(end_marker) : end)
...
static void reset_var_on_error(void* ptr) {
...
static void signal_before_change(ptrdiff_t start_int, ptrdiff_t end_int,
                                 ptrdiff_t* preserve_ptr) {
    ...
    if (!NILP(Vbefore_change_functions)) {
        ...
        CALLN(Frun_hook_with_args, Qbefore_change_functions, FETCH_START,
              FETCH_END);
    ...
    if (buffer_has_overlays()) {
        PRESERVE_VALUE;
        report_overlay_modification(FETCH_START, FETCH_END, 0, FETCH_START,
                                    FETCH_END, Qnil);
    }
...
void signal_after_change(ptrdiff_t charpos, ptrdiff_t lendel,
                         ptrdiff_t lenins) {
    specpdl_ref count = SPECPDL_INDEX();
    struct rvoe_arg rvoe_arg;
    ...
    if (!NILP(Vcombine_after_change_calls)
        ...
        && (NILP(Vbefore_change_functions) ||
            (CONSP(Vbefore_change_functions) &&
             ...
             CONSP(tmp = XCDR(Vbefore_change_functions)) && NILP(XCDR(tmp)) &&
        ...
        elt = list3i(charpos - BEG, Z - (charpos - lendel + lenins),
                     lenins - lendel);
        combine_after_change_list = Fcons(elt, combine_after_change_list);
    ...
    if (buffer_has_overlays())
        report_overlay_modification(
            make_fixnum(charpos), make_fixnum(charpos + lenins), 1,
            make_fixnum(charpos), make_fixnum(charpos + lenins),
            make_fixnum(lendel));
...
static void Fcombine_after_change_execute_1(Lisp_Object val) {
    Vcombine_after_change_calls = val;
}

DEFUN(
    "combine-after-change-execute",
    Fcombine_after_change_execute, Scombine_after_change_execute, 0, 0, 0, doc:/* This function is for use internally in the function `combine-after-change-calls'.  */)
(void) {
    specpdl_ref count = SPECPDL_INDEX();
    ...
    for (tail = combine_after_change_list; CONSP(tail); tail = XCDR(tail)) {
        Lisp_Object elt;
