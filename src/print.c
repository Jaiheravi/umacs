...
static void print_free_buffer(void) {
...
static void print_unwind(Lisp_Object saved_text) {
...
static inline struct print_context print_prepare(Lisp_Object printcharfun) {
    ...
    if (NILP(printcharfun)) {
        ...
        if (print_buffer.buffer != NULL) {
            Lisp_Object string = make_string_from_bytes(
                print_buffer.buffer, print_buffer.pos, print_buffer.pos_byte);
...
static inline void print_finish(struct print_context* pc) {
    if (NILP(pc->printcharfun)) {
        if (print_buffer.pos != print_buffer.pos_byte &&
            ...
            insert_1_both((char*)temp, print_buffer.pos, print_buffer.pos, 0, 1,
                          0);
            SAFE_FREE();
        }
        else
    ...
    if (pc->old_point >= 0)
        SET_PT_BOTH(
            pc->old_point +
                (pc->old_point >= pc->start_point ? PT - pc->start_point : 0),
            pc->old_point_byte +
...
static void printchar_to_stream(unsigned int ch, FILE* stream) {
...
static void printchar(unsigned int ch, Lisp_Object fun) {
...
static void octalout(unsigned char c, unsigned char* data, ptrdiff_t i,
                     ptrdiff_t size, Lisp_Object printcharfun) {
    int digits =
        (c > '\77' || (i < size && '0' <= data[i] && data[i] <= '7') ? 3
             : c > '\7'                                              ? 2
                                                                     : 1);
...
static void strout(const char* ptr, ptrdiff_t size, ptrdiff_t size_byte,
                   Lisp_Object printcharfun) {
    if (NILP(printcharfun)) {
        ptrdiff_t incr =
            size_byte - (print_buffer.size - print_buffer.pos_byte);
        if (incr > 0)
            print_buffer.buffer =
        ...
        if (DISP_TABLE_P(Vstandard_display_table)) {
            ...
            for (ptrdiff_t i = 0; i < size_byte; i += len) {
                int ch =
                    string_char_and_length((const unsigned char*)ptr + i, &len);
                printchar_to_stream(ch, stdout);
            }
        }
        else
       ...
       job.  */
        ...
        if (size == size_byte) {
            ...
            while (i < size_byte) {
               ...
               PRINTCHAR.  */
                int len,
                    ch = (string_char_and_length((const unsigned char*)ptr + i,
                                                 &len));
                printchar(ch, printcharfun);
                i += len;
            }
...
static void print_string_1(Lisp_Object string, Lisp_Object printcharfun,
                           bool escape_nonascii) {
...
static void print_string(Lisp_Object string, Lisp_Object printcharfun) {
...
static void print_c_string(char const* string, Lisp_Object printcharfun) {
...
static void write_string(const char* data, Lisp_Object printcharfun) {
    struct print_context pc = print_prepare(printcharfun);
...
static void print_bind_all_defaults(void) {
...
static void print_create_variable_mapping(void) {
    Lisp_Object total[] = {
        ...
        list3(intern("escape-multibyte"), intern("print-escape-multibyte"),
              Qnil),
...
static void print_bind_overrides(Lisp_Object overrides) {
    ...
    if (EQ(overrides, Qt))
        ...
        while (!NILP(overrides)) {
            ...
            if (EQ(setting, Qt))
                ...
                Lisp_Object key = XCAR(setting), value = XCDR(setting);
...
void debug_output_compilation_hack(bool x) { print_output_debug_flag = x; }
...
(Lisp_Object file, Lisp_Object append) {
    /* If equal to STDERR_FILENO, stderr has not been duplicated and is OK
       as-is. Otherwise, this is a close-on-exec duplicate of the original
       stderr. */
    ...
    if (!NILP(file)) {
        ...
        fd = emacs_open(
            ...
            (O_WRONLY | O_CREAT | (!NILP(append) ? O_APPEND : O_TRUNC)), 0666);
        if (fd < 0)
...
(Lisp_Object obj) {
    ...
    if (CONSP(obj) && EQ(XCAR(obj), Qerror) && CONSP(XCDR(obj)) &&
        STRINGP(XCAR(XCDR(obj))) && NILP(XCDR(XCDR(obj))))
...
void print_error_message(Lisp_Object data, Lisp_Object stream,
                         const char* context, Lisp_Object caller) {
...
int float_to_string(char* buf, double data) {
    char* cp;
    int width;
    int len;
...
static void print(Lisp_Object obj, Lisp_Object printcharfun, bool escapeflag) {
    ...
    if (!NILP(Vprint_circle)) {
       ...
       This increments print_number_index for the objects added.  */
        ...
        if (HASH_TABLE_P(
                Vprint_number_table)) { /* Remove unnecessary objects, which
                                           appear only once in OBJ; that is,
                                           whose status is Qt.  */
...
static inline bool print_circle_candidate_p(Lisp_Object obj) {
    if (CONSP(obj))
        return true;
    else if (STRINGP(obj))
        return SCHARS(obj) > 0;
        ...
        if (VECTORP(obj))
            ...
            return (CLOSUREP(obj) || CHAR_TABLE_P(obj) ||
                    SUB_CHAR_TABLE_P(obj) || HASH_TABLE_P(obj) || FONTP(obj) ||
                    RECORDP(obj));
    }
    else
        return false;
...
NO_INLINE static void grow_pp_stack(void) {
...
static inline void pp_stack_push_value(Lisp_Object value) {
...
static inline void pp_stack_push_values(Lisp_Object* values, ptrdiff_t n) {
    eassume(n >= 0);
    if (n == 0)
        return;
    if (ppstack.sp >= ppstack.size)
        grow_pp_stack();
    ppstack.stack[ppstack.sp++] =
        (struct print_pp_entry){.n = n, .u.values = values};
}

static inline bool pp_stack_empty_p(void) { return ppstack.sp <= 0; }

static inline Lisp_Object pp_stack_pop(void) {
...
static void print_preprocess(Lisp_Object obj) {
    ...
    for (;;) {
        if (print_circle_candidate_p(obj)) {
            ...
            if (!NILP(num)
                /* If Vprint_continuous_numbering is non-nil and OBJ is a
               gensym, always print the gensym with a number.  This is a special
               for the lisp function byte-compile-output-docform.  */
                || (!NILP(Vprint_continuous_numbering) && SYMBOLP(obj) &&
                    !SYMBOL_INTERNED_P(obj))) { /* OBJ appears more than once.
                                                   Let's remember that.  */
                ...
                case Lisp_Vectorlike:
                    {
                        ...
                        ptrdiff_t start =
                            (SUB_CHAR_TABLE_P(obj) ? SUB_CHAR_TABLE_OFFSET : 0);
                        pp_stack_push_values(vec->contents + start,
                                             size - start);
                        if (HASH_TABLE_P(obj)) {
...
static void print_preprocess_string(INTERVAL interval, void* arg) {
...
static void print_check_string_charset_prop(INTERVAL interval, void* pstring) {
    Lisp_Object val;

    if (NILP(interval->plist) ||
        (print_check_string_result ==
         (PRINT_STRING_NON_CHARSET_FOUND | PRINT_STRING_UNSAFE_CHARSET_FOUND)))
        return;
    ...
    if (!(print_check_string_result & PRINT_STRING_NON_CHARSET_FOUND)) {
        if (!EQ(val, interval->plist) || CONSP(XCDR(XCDR(val))))
            print_check_string_result |= PRINT_STRING_NON_CHARSET_FOUND;
    }
...
static Lisp_Object print_prune_string_charset(Lisp_Object string) {
    ...
    if (NILP(Vprint_charset_text_property) ||
        ...
        if (print_check_string_result & PRINT_STRING_NON_CHARSET_FOUND) {
            ...
            Fremove_text_properties(make_fixnum(0), make_fixnum(SCHARS(string)),
...
static void const* data_from_funcptr(void (*funcptr)(void)) {
...
static void print_pointer(Lisp_Object printcharfun, char* buf,
                          const char* prefix, const void* ptr) {
...
static void print_bignum(Lisp_Object obj, Lisp_Object printcharfun) {
...
static void print_bool_vector(Lisp_Object obj, Lisp_Object printcharfun) {
    ...
    if (FIXNATP(Vprint_length) && XFIXNAT(Vprint_length) < size_in_bytes)
...
static void print_vectorlike_unreadable(Lisp_Object obj,
                                        Lisp_Object printcharfun,
    ...
    case PVEC_OVERLAY:
        ...
        if (!OVERLAY_BUFFER(obj))
            ...
            strout(buf, len, len, printcharfun);
            print_string(BVAR(OVERLAY_BUFFER(obj), name), printcharfun);
        }
        printchar('>', printcharfun);
        return;
    ...
    case PVEC_USER_PTR:
        {
            ...
            int i = sprintf(buf, "ptr=%p finalizer=%p", XUSER_PTR(obj)->p,
...
#ifdef USE_GTK
                int len =
                    sprintf(buf, "#<xwidget %u %p>", XXWIDGET(obj)->xwidget_id,
    ...
    case PVEC_FRAME:
        {
            ...
            print_c_string(
                (FRAME_LIVE_P(XFRAME(obj)) ? "#<frame " : "#<dead frame "),
                printcharfun);
                ...
                for (int i = 0; i < FONT_SPEC_MAX; i++) {
                    ...
                    if (i != FONT_EXTRA_INDEX || !FONT_ENTITY_P(obj)) {
                        ...
                        if (i < FONT_WEIGHT_INDEX || i > FONT_WIDTH_INDEX)
                            print_object(AREF(obj, i), printcharfun,
                                         escapeflag);
                        else
...
static char named_escape(int i) {
    switch (i) {
    case '\b':
        return 'b';
    case '\t':
...
NO_INLINE static void grow_print_stack(void) {
...
static inline void print_stack_push(struct print_stack_entry e) {
    if (prstack.sp >= prstack.size)
        grow_print_stack();
    prstack.stack[prstack.sp++] = e;
}

static void print_stack_push_vector(const char* lbrac, const char* rbrac,
                                    Lisp_Object obj, ptrdiff_t start,
                                    ptrdiff_t size, Lisp_Object printcharfun) {
    print_c_string(lbrac, printcharfun);

    ptrdiff_t print_size =
...
static void print_object(Lisp_Object obj, Lisp_Object printcharfun,
                         bool escapeflag) {
    ptrdiff_t base_depth = print_depth;
    ptrdiff_t base_sp = prstack.sp;
    char buf[max(
        sizeof "from..to..in " + 2 * INT_STRLEN_BOUND(EMACS_INT),
        max(sizeof " . #" + INT_STRLEN_BOUND(intmax_t),
            max((sizeof " with data 0x" + (UINTMAX_WIDTH + 4 - 1) / 4), 40)))];
...
print_obj:
    ...
    if (NILP(Vprint_circle)) {
        ...
        if (FIXNUMP(num)) {
            ...
            if (n < 0) { /* Add a prefix #n= if OBJ has not yet been printed;
    ...
    case Lisp_Int1:
        {
            ...
            if (print_integers_as_characters && i >= 0 &&
                i <= MAX_UNICODE_CHAR &&
                ((escaped_name = named_escape(i)) || graphic_base_p(i))) {
                printchar('?', printcharfun);
                if (escaped_name) {
                    printchar('\\', printcharfun);
                    i = escaped_name;
    ...
    case Lisp_String:
        if (!escapeflag)
               ...
               printchar.  */
                ...
                if (multibyte
                    ...
                    /* When printing a raw 8-bit byte in a multibyte buffer, or
                       (when requested) a non-ASCII character in a unibyte
                       buffer, print single-byte non-ASCII string chars using
                       octal escapes.  */
                    octalout(c, SDATA(obj), i_byte, size_byte, printcharfun);
                    need_nonhex = false;
                }
                else if (multibyte && !ASCII_CHAR_P(c) &&
                         print_escape_multibyte) {
                    ...
                    if (c_isxdigit(c)) {
                        ...
                        octalout(c, SDATA(obj), i_byte, size_byte,
                                 printcharfun);
                    else if (!multibyte && SINGLE_BYTE_CHAR_P(c) &&
                             !ASCII_CHAR_P(c))
            ...
            if (string_intervals(obj)) {
                ...
                traverse_intervals(string_intervals(obj), 0, print_interval,
                                   &pcf);
                printchar(')', printcharfun);
            }
        }
    ...
    case Lisp_Symbol:
        {
            ...
            for (ptrdiff_t i_byte = 0; i_byte < size_byte;) {
                 ...
                 */
                int c =
                    fetch_string_char_as_multibyte_advance(name, &i, &i_byte);
                maybe_quit();
    ...
    case Lisp_Vectorlike:
        ...
        switch (PSEUDOVECTOR_TYPE(XVECTOR(obj))) {
        case PVEC_NORMAL_VECTOR:
            print_stack_push_vector("[", "]", obj, 0, ASIZE(obj), printcharfun);
            goto next_obj;
        case PVEC_RECORD:
        ...
        case PVEC_SUB_CHAR_TABLE:
            {
                print_c_string("#^^[", printcharfun);
                int n = sprintf(buf, "%d %d", XSUB_CHAR_TABLE(obj)->depth,
                                XSUB_CHAR_TABLE(obj)->min_char);
                strout(buf, n, n, printcharfun);
                print_stack_push_vector("", "]", obj, SUB_CHAR_TABLE_OFFSET,
                                        PVSIZE(obj), printcharfun);
                goto next_obj;
            }
        case PVEC_HASH_TABLE:
            {
...
void syms_of_print(void) {
    ...
    DEFVAR_LISP("standard-output", Vstandard_output, doc:
                    /* Output stream `print' uses by default for outputting a
    character. This may be any function of one argument. It may also be a
    buffer (output is inserted before point) or a marker (output is inserted
    and the marker is advanced) or the symbol t (output appears in the echo
    area).  */);
    Vstandard_output = Qt;
...
as numbers the usual way.  */);
    ...
    DEFVAR_LISP("print-length", Vprint_length, doc:
...
where N is a positive decimal integer.  */);
    ...
    DEFVAR_LISP("print-continuous-numbering", Vprint_continuous_numbering, doc:
...
that need to be recorded in the table.  */);
    ...
    DEFVAR_LISP("print-charset-text-property", Vprint_charset_text_property,
                doc:
                    /* A flag to control printing of `charset' text property on
...
Otherwise print just the bare symbol.  */);
    ...
    DEFVAR_LISP("print-unreadable-function", Vprint_unreadable_function, doc:
                    /* If non-nil, a function to call when printing unreadable
    objects. By default, Emacs printing functions (like `prin1') print
    unreadable objects as \"#<...>\", where \"...\" describes the object
    (for instance, \"#<marker in no buffer>\").
