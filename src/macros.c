...
void end_kbd_macro(void) {
    kset_defining_kbd_macro(current_kboard, Qnil);
    update_mode_lines = 20;
    kset_last_kbd_macro(current_kboard,
...
void store_kbd_macro_char(Lisp_Object c) {
    ...
    if (!NILP(KVAR(kb, defining_kbd_macro))) {
        if (kb->kbd_macro_ptr - kb->kbd_macro_buffer == kb->kbd_macro_bufsize) {
            ...
            kb->kbd_macro_buffer =
                xpalloc(kb->kbd_macro_buffer, &kb->kbd_macro_bufsize, 1, -1,
                        sizeof *kb->kbd_macro_buffer);
...
DEFUN("store-kbd-macro-event", Fstore_kbd_macro_event, Sstore_kbd_macro_event,
      1, 1, 0, doc:/* Store EVENT into the keyboard macro being defined.  */)
(Lisp_Object event) {
...
static void pop_kbd_macro(Lisp_Object info) {
    Lisp_Object tem;
...
(Lisp_Object macro, Lisp_Object count, Lisp_Object loopfunc) {
    ...
    tem = Fcons(Vexecuting_kbd_macro,
                Fcons(make_int(executing_kbd_macro_index), Vreal_this_command));
