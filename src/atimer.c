...
static void block_atimers(sigset_t* oldset) {
    sigset_t blocked;
    sigemptyset(&blocked);
...
static void unblock_atimers(sigset_t const* oldset) {
...
static struct atimer* append_atimer_lists(struct atimer*, struct atimer*);
...
struct atimer* start_atimer(enum atimer_type type, struct timespec timestamp,
...
void cancel_atimer(struct atimer* timer) {
    int i;
    sigset_t oldset;
...
static struct atimer* append_atimer_lists(struct atimer* list_1,
                                          struct atimer* list_2) {
    if (list_1 == NULL)
        return list_2;
    else if (list_2 == NULL)
        return list_1;
...
void stop_other_atimers(struct atimer* t) {
    sigset_t oldset;
    block_atimers(&oldset);
...
static void schedule_atimer(struct atimer* t) {
...
static void handle_alarm_signal(int sig) { pending_signals = 1; }
...
void timerfd_callback(int fd, void* arg) {
    ptrdiff_t nbytes;
...
static void debug_timer_callback(struct atimer* t) {
    struct timespec now = current_timespec();
...
(void) {
    ...
    for (i = 0; i < MAXTIMERS; i++) {
        ...
        results[i]->intime = -1;
        timer =
            start_atimer(ATIMER_RELATIVE, t, debug_timer_callback, results[i]);
    }
...
#ifdef HAVE_TIMERFD
static bool have_buggy_timerfd(void) {
#ifdef CYGWIN
    struct utsname name;
...
void init_atimer(void) {
    ...
    timerfd =
        (egetenv("EMACS_IGNORE_TIMERFD") || have_buggy_timerfd()
             ? -1
