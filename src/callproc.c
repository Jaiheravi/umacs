...
/* In order to be able to use `posix_spawn', it needs to support some
   variant of `chdir' as well as `setsid'.  */
    ...
    HAVE_DECL_POSIX_SPAWN_SETSID == 1 /* posix_spawnattr_setflags rejects      \
                                         POSIX_SPAWN_SETSID on Haiku */                                                                      \
    && !defined HAIKU
#include <spawn.h>
...
static CHILD_SETUP_TYPE child_setup(int, int, int, char**, char**, const char*);
...
Lisp_Object get_current_directory(bool encode) {
    ...
    if (encode &&
        (android_is_special_directory(SSDATA(dir), "/assets") ||
         android_is_special_directory(SSDATA(dir), "/content")))
        dir = build_string("~");
...
void record_kill_process(struct Lisp_Process* p, Lisp_Object tempfile) {
#ifndef MSDOS
    sigset_t oldset;
...
static void delete_temp_file(Lisp_Object name) { emacs_unlink(SSDATA(name)); }

static void call_process_kill(void* ptr) {
    int* callproc_fd = ptr;
    int i;
...
static void call_process_cleanup(Lisp_Object buffer) {
    Fset_buffer(buffer);
    ...
    if (synch_process_pid) {
        ...
        synch_process_pid = 0;
        message1(wait_ok ? "Waiting for process to die...done"
...
DEFUN("call-process", Fcall_process, Scall_process, 1, MANY, 0, doc:
...
(ptrdiff_t nargs, Lisp_Object* args) {
    ...
    return unbind_to(
        count, call_process(nargs, args, filefd, make_invalid_specpdl_ref()));
}
...
static Lisp_Object call_process(ptrdiff_t nargs, Lisp_Object* args, int filefd,
    ...
    if (nargs >= 3 && (FIXNUMP(CONSP(args[2]) ? XCAR(args[2]) : args[2])))
    ...
    if (nargs < 3)
       ...
       the next paragraph. */
        ...
        if (CONSP(buffer) && EQ(XCAR(buffer), QCfile)) {
            ...
            CHECK_STRING(ofile);
            output_file =
                Fexpand_file_name(ofile, BVAR(current_buffer, directory));
    ...
    if (STRINGP(error_file) || (NILP(error_file) && !discard_output)) {
        fd_error = emacs_open(
            (STRINGP(error_file) ? SSDATA(error_file) : NULL_DEVICE),
            O_WRONLY | O_CREAT | O_TRUNC | O_TEXT, default_output_mode);
        if (fd_error < 0) {
            int open_errno = errno;
            report_file_errno("Cannot redirect stderr",
                              (STRINGP(error_file) ? DECODE_FILE(error_file)
    ...
    if (status < 0) {
        ...
        return code_convert_string_norecord(build_string(strerror(child_errno)),
    ...
    child_errno = emacs_spawn(&pid, filefd, fd_output, fd_error, new_argv, env,
    ...
    if (0 <= fd0) {
        enum
        {
            CALLPROC_BUFFER_SIZE_MIN = 16 * 1024
        };
        enum
        {
            CALLPROC_BUFFER_SIZE_MAX = 4 * CALLPROC_BUFFER_SIZE_MIN
        };
        ...
        while (1) {
            ...
            while (nread < bufsize - 1024) {
                int this_read =
                    emacs_read_quit(fd0, buf + nread, bufsize - nread);
            ...
            if (!nread)
               ...
               TEMP_SET_PT_BOTH below.  */
                ...
                if (display_on_the_fly &&
                    ...
                    saved_coding.common_flags &= ~CODING_REQUIRE_DETECTION_MASK;
                    continue;
                }
...
static int create_temp_file(ptrdiff_t nargs, Lisp_Object* args,
...
#else /* DOS_NT */
        if ((outf = egetenv("TMPDIR")) || (outf = egetenv("TMP")) ||
            (outf = egetenv("TEMP")))
    ...
    {
        Lisp_Object pattern =
            Fexpand_file_name(Vtemp_file_name_pattern, tmpdir);
        char* tempfile;
...

    {
        ...
        write_region(start, end, filename_string, Qnil, Qlambda, Qnil, Qnil,
                     fd);
...
static char** add_env(char** env, char** new_env, char* string) {
    char** ep;
    bool ok = 1;
    if (string == NULL)
        return new_env;
...
static AVOID exec_failed(char const* name, int err) {
...
static CHILD_SETUP_TYPE child_setup(int in, int out, int err, char** new_argv,
                                    char** env, const char* current_dir) {
#ifdef MSDOS
    char* pwd_var;
    char* temp;
    ptrdiff_t i;
...
static int emacs_posix_spawn_init_actions(posix_spawn_file_actions_t* actions,
    ...
    error = posix_spawn_file_actions_adddup2(actions, std_in, STDIN_FILENO);
    if (error != 0)
        goto out;

    error = posix_spawn_file_actions_adddup2(actions, std_out, STDOUT_FILENO);
    if (error != 0)
        goto out;

    error = posix_spawn_file_actions_adddup2(
        actions, std_err < 0 ? std_out : std_err, STDERR_FILENO);
    if (error != 0)
        goto out;
...
static int emacs_posix_spawn_init_attributes(posix_spawnattr_t* attributes,
...
int emacs_spawn(pid_t* newpid, int std_in, int std_out, int std_err,
                char** argv, char** envp, const char* cwd, const char* pty_name,
                bool pty_in, bool pty_out, const sigset_t* oldset) {
#if defined HAVE_ANDROID && !defined ANDROID_STUBIFY
    ...
    if (use_posix_spawn) {
        ...
        int error = emacs_posix_spawn_init_actions(&actions, std_in, std_out,
                                                   std_err, cwd);
        if (error != 0)
            return error;
...
static bool getenv_internal_1(const char* var, ptrdiff_t varlen, char** value,
...
static bool getenv_internal(const char* var, ptrdiff_t varlen, char** value,
...
char* egetenv_internal(const char* var, ptrdiff_t len) {
    char* value;
    ptrdiff_t valuelen;
...
char** make_environment_block(Lisp_Object current_dir) {
    char** env;
    char* pwd_var;
...

    {
        ...
        for (tem = Vprocess_environment; CONSP(tem) && STRINGP(XCAR(tem));
             tem = XCDR(tem)) {
#ifndef HAVE_ANDROID
            if (strncmp(SSDATA(XCAR(tem)), "DISPLAY", 7) == 0 &&
                (SDATA(XCAR(tem))[7] == '\0' || SDATA(XCAR(tem))[7] == '='))
...
void init_callproc(void) {
    ...
    if (!NILP(Vinstallation_directory)) {
        ...
        Lisp_Object tem;
        tem =
            Fexpand_file_name(build_string("lib-src"), Vinstallation_directory);
#ifndef MSDOS
        ...
        if (data_dir == 0) {
            tem =
                Fexpand_file_name(build_string("etc"), Vinstallation_directory);
    ...
    if (data_dir == 0) {
        ...
        if (!NILP(Vinstallation_directory) ||
            ...
            NILP(Ffile_exists_p(
                Fexpand_file_name(AUTO_STR("NEWS"), Vdata_directory)))) {
            Lisp_Object newdir;
            newdir = Fexpand_file_name(AUTO_STR("../etc/"), lispdir);
            if (!NILP(Ffile_exists_p(
                    Fexpand_file_name(AUTO_STR("NEWS"), newdir))))
...
void set_initial_environment(void) {
    char** envp;
...
void syms_of_callproc(void) {
    ...
    DEFVAR_LISP("shell-file-name", Vshell_file_name, doc:
...
includes this.  */);
    ...
    DEFVAR_LISP("shared-game-score-directory", Vshared_game_score_directory,
                doc:
                    /* Directory of score files for games which come with GNU
    Emacs. If this variable is nil, then Emacs is unable to use a shared
    directory.  */);
...
#if !defined HAVE_ANDROID || defined ANDROID_STUBIFY || defined HAVE_MAILUTILS
