...
#define OUTPUT(tty, a)                                                         \
  emacs_tputs(tty, a, FRAME_TOTAL_LINES(XFRAME(selected_frame)) - curY(tty),   \
              cmputc)
...
static void tty_ring_bell(struct frame* f)
{
  ...
  {
    OUTPUT(tty,
           (tty->TS_visible_bell && visible_bell ? tty->TS_visible_bell
                                                 : tty->TS_bell));
    fflush(tty->output);
  }
}
...
static void tty_send_additional_strings(struct terminal* terminal,
                                        Lisp_Object sym)
{
  ...
  for (Lisp_Object extra_codes =
       ...
       CONSP(extra_codes); extra_codes = XCDR(extra_codes))
  {
    ...
    if (STRINGP(string))
    {
      ...
      ptrdiff_t sbytes =
          (str->u.s.size_byte < 0 ? str->u.s.size & ~ARRAY_MARK_FLAG
                                  : str->u.s.size_byte);
...
static void tty_set_terminal_modes(struct terminal* terminal)
{
  struct tty_display_info* tty = terminal->display_info.tty;
  ...
  if (tty->output)
  {
    ...
    OUTPUT_IF(tty,
              visible_cursor ? tty->TS_cursor_visible : tty->TS_cursor_normal);
...
static void tty_reset_terminal_modes(struct terminal* terminal)
{
  struct tty_display_info* tty = terminal->display_info.tty;
...
static void tty_update_end(struct frame* f)
{
  struct tty_display_info* tty = FRAME_TTY(f);
...
static void tty_set_terminal_window(struct frame* f, int size)
{
  struct tty_display_info* tty = FRAME_TTY(f);
...
static void tty_set_scroll_region(struct frame* f, int start, int stop)
{
  ...
  if (tty->TS_set_scroll_region)
    OUTPUT(tty, tparm(tty->TS_set_scroll_region, start, stop - 1));
  else if (tty->TS_set_scroll_region_1)
    OUTPUT(tty, tparm(tty->TS_set_scroll_region_1, FRAME_TOTAL_LINES(f), start,
                      FRAME_TOTAL_LINES(f) - stop, FRAME_TOTAL_LINES(f)));
  else
    OUTPUT(tty, tparm(tty->TS_set_window, start, 0, stop, FRAME_COLS(f)));

  losecursor(tty);
}


static void tty_turn_on_insert(struct tty_display_info* tty)
{
  if (!tty->insert_mode)
...
void tty_turn_off_insert(struct tty_display_info* tty)
{
  if (tty->insert_mode)
...
static void tty_turn_off_highlight(struct tty_display_info* tty)
{
...
static void tty_turn_on_highlight(struct tty_display_info* tty)
{
  if (!tty->standout_mode)
...
static void tty_toggle_highlight(struct tty_display_info* tty)
{
  if (tty->standout_mode)
    tty_turn_off_highlight(tty);
  else
...
static void tty_highlight_if_desired(struct tty_display_info* tty)
{
  if (inverse_video)
    tty_turn_on_highlight(tty);
  else
...
static void tty_cursor_to(struct frame* f, int vpos, int hpos)
{
  struct tty_display_info* tty = FRAME_TTY(f);
  ...
  if (curY(tty) == vpos && curX(tty) == hpos)
    return;
...
static void tty_raw_cursor_to(struct frame* f, int row, int col)
{
  struct tty_display_info* tty = FRAME_TTY(f);

  if (curY(tty) == row && curX(tty) == col)
    return;
  if (!tty->TF_standout_motion)
    tty_background_highlight(tty);
  if (!tty->TF_insmode_motion)
...
static void tty_clear_to_end(struct frame* f)
{
  register int i;
  struct tty_display_info* tty = FRAME_TTY(f);
...
static void tty_clear_end_of_line(struct frame* f, int first_unused_hpos)
{
  register int i;
  struct tty_display_info* tty = FRAME_TTY(f);
  ...
  { /* have to do it the hard way */
    ...
    if (AutoWrap(tty) && curY(tty) == FrameRows(tty) - 1 &&
...
unsigned char* encode_terminal_code(struct glyph* src, int src_len,
                                    struct coding_system* coding)
{
  ...
  if (encode_terminal_src_size < required)
    encode_terminal_src = xpalloc(
        encode_terminal_src, &encode_terminal_src_size,
        required - encode_terminal_src_size, -1, sizeof *encode_terminal_src);
    ...
    if (src->type == COMPOSITE_GLYPH)
    {
      ...
      if (encode_terminal_src_size - nbytes < required)
      {
        encode_terminal_src =
            xpalloc(encode_terminal_src, &encode_terminal_src_size,
                    required - (encode_terminal_src_size - nbytes), -1, 1);
      ...
      if (!cmp)
        ...
        for (i = 0; i < cmp->glyph_len; i++)
        {
          ...
          if (char_charset(c, charset_list, NULL))
          {
            if (CHARACTER_WIDTH(c) == 0 && i > 0 &&
                COMPOSITION_GLYPH(cmp, i - 1) == '\t')
    ...
    else if (!CHAR_GLYPH_PADDING_P(*src))
    {
     ...
     so process any alias before testing for simpleness.  */
      ...
      if (NILP(string))
      {
        ...
        if (CHAR_BYTE8_P(c) || char_charset(c, charset_list, NULL))
        {
      ...
      else
      {
        ...
        if (encode_terminal_src_size - nbytes < SBYTES(string))
        {
          encode_terminal_src = xpalloc(
              encode_terminal_src, &encode_terminal_src_size,
              (SBYTES(string) - (encode_terminal_src_size - nbytes)), -1, 1);
          buf = encode_terminal_src + nbytes;
        }
  ...
  {
    encode_terminal_dst =
        xrealloc(encode_terminal_dst, encode_terminal_src_size);
...
static void tty_write_glyphs_1(struct frame* f, struct glyph* string, int len)
{
  struct tty_display_info* tty = FRAME_TTY(f);
  tty_turn_off_insert(tty);
  tty_hide_cursor(tty);
  ...
  for (int stringlen = len, n; stringlen; stringlen -= n, string += n)
  {
    ...
    if (n == stringlen)
      ...
      coding->mode |= CODING_MODE_LAST_BLOCK;
    unsigned char* conversion_buffer = encode_terminal_code(string, n, coding);
...
static void tty_write_glyphs_with_face(struct frame* f, struct glyph* string,
                                       int len, struct face* face)
{
  ...
  if (AutoWrap(tty) && curY(tty) + 1 == FRAME_TOTAL_LINES(f) &&
...
static void tty_insert_glyphs(struct frame* f, struct glyph* start, int len)
{
  char* buf;
  ...
  if (tty->TS_ins_multi_chars)
  {
    ...
    // OUTPUT1(tty, buf);
    // xfree(buf);
    OUTPUT1 (tty, tparm (tty->TS_ins_multi_chars, len));
    if (start)
...
static void tty_write_glyphs(struct frame* f, struct glyph* string, int len)
{
  struct tty_display_info* tty = FRAME_TTY(f);
  /* Don't dare write in last column of bottom line, if Auto-Wrap,
     since that would scroll the whole frame on some terminals.  */
  if (AutoWrap(tty) && curY(tty) + 1 == FRAME_TOTAL_LINES(f) &&
      curX(tty) + len == FRAME_COLS(f) && len > 0)
  {
...
static void tty_delete_glyphs(struct frame* f, int n)
{
  char* buf;
  register int i;
  ...
  if (tty->TS_del_multi_chars)
  {
    ...
    // OUTPUT1(tty, buf);
    // xfree(buf);
    OUTPUT1 (tty, tparm (tty->TS_del_multi_chars, n));
  }
  else
    for (i = 0; i < n; i++)
...
static void tty_ins_del_lines(struct frame* f, int vpos, int n)
{
  struct tty_display_info* tty = FRAME_TTY(f);
  const char* multi = n > 0 ? tty->TS_ins_multi_lines : tty->TS_del_multi_lines;
  ...
  if (FRAME_SCROLL_REGION_OK(f) && vpos + i >= tty->specified_window)
    return;
  if (!FRAME_MEMORY_BELOW_FRAME(f) && vpos + i >= FRAME_TOTAL_LINES(f))
    return;
  ...
  if (!FRAME_SCROLL_REGION_OK(f) && FRAME_MEMORY_BELOW_FRAME(f) && n < 0)
  {
...
int string_cost(const char* str)
{
  cost = 0;
#ifndef HAVE_ANDROID
  if (str)
...
static int string_cost_one_line(const char* str)
{
  cost = 0;
#ifndef HAVE_ANDROID
  if (str)
...
int per_line_cost(const char* str)
{
  cost = 0;
#ifndef HAVE_ANDROID
  if (str)
...
static void calculate_ins_del_char_costs(struct frame* f)
{
  struct tty_display_info* tty = FRAME_TTY(f);
  ...
  {
    ins_startup_cost = (30 *
                        (string_cost(tty->TS_insert_mode) +
                         string_cost(tty->TS_end_insert_mode))) /
        100;
...
void calculate_costs(struct frame* frame)
{
  ...
  if (FRAME_TERMCAP_P(frame))
  {
    ...
    register const char* f =
        (tty->TS_set_scroll_region ? tty->TS_set_scroll_region
    ...
    char_ins_del_vector = xrealloc(
        char_ins_del_vector, (sizeof(int) + 2 * sizeof(int) * max_frame_cols));
    ...
    if (f && (!tty->TS_ins_line && !tty->TS_del_line))
      do_line_insertion_deletion_costs(
          frame, tty->TS_rev_scroll, tty->TS_ins_multi_lines,
          tty->TS_fwd_scroll, tty->TS_del_multi_lines, f, f, 1);
    else
      do_line_insertion_deletion_costs(
          frame, tty->TS_ins_line, tty->TS_ins_multi_lines, tty->TS_del_line,
          tty->TS_del_multi_lines, 0, 0, 1);

    calculate_ins_del_char_costs(frame);

    /* Don't use TS_repeat if its padding is worse than sending the chars */
    if (tty->TS_repeat &&
        (baud_rate <= 0 || per_line_cost(tty->TS_repeat) < 9000 / baud_rate))
...
static void term_get_fkeys(char** address, KBOARD* kboard)
{
...
static void append_glyph(struct it* it)
{
...
/* For external use.  */
void tty_append_glyph(struct it* it) { append_glyph(it); }
...
void produce_glyphs(struct it* it)
{
  ...
  eassert(it->what == IT_CHARACTER || it->what == IT_COMPOSITION ||
          it->what == IT_STRETCH || it->what == IT_GLYPHLESS);
        ...
        (it->current_x +
         (it->string_from_prefix_prop_p
              ...
              : (it->continuation_lines_width
                     ? (it->continuation_lines_width - it->wrap_prefix_width)
                     : 0)));
    int x0 = absolute_x;
    /* Adjust for line numbers.  */
    if (!NILP(Vdisplay_line_numbers) && it->line_number_produced_p)
      absolute_x -= it->lnum_pixel_width;
    int next_tab_x = (((1 + absolute_x + it->tab_width - 1) / it->tab_width) *
                      it->tab_width);
    if (!NILP(Vdisplay_line_numbers) && it->line_number_produced_p)
 ...
 a single byte code has 1-column width.  */
  ...
  else
  {
    ...
    if (char_charset(it->char_to_display, charset_list, NULL) &&
        (char_glyph = terminal_glyph_code(t, it->char_to_display),
         NILP(char_glyph) || (FIXNUMP(char_glyph) && XFIXNUM(char_glyph) >= 0)))
    {
  ...
  if (it->area == TEXT_AREA)
  {
    ...
    if (it->continuation_lines_width && it->string_from_prefix_prop_p)
...
static void produce_composite_glyph(struct it* it)
{
  if (it->cmp_it.ch < 0)
  {
...
static void append_glyphless_glyph(struct it* it, int face_id, const char* str)
{
  struct glyph *glyph, *end;
  int i;
...
static void produce_glyphless_glyph(struct it* it, Lisp_Object acronym)
{
...
#define MAY_USE_WITH_COLORS_P(tty, ATTR)                                       \
  (tty->TN_max_colors > 0 ? (tty->TN_no_color_video & (ATTR)) == 0 : 1)
...
static void turn_on_face(struct frame* f, struct face* face)
{
  ...
  if (face->underline && MAY_USE_WITH_COLORS_P(tty, NC_UNDERLINE))
  {
    if (face->underline == FACE_UNDERLINE_SINGLE ||
      ...
      OUTPUT(tty, tparm(tty->TF_set_underline_style, face->underline));
  }
...
bool tty_capable_p(struct tty_display_info* tty, unsigned int caps)
{
#ifndef HAVE_ANDROID
#define TTY_CAPABLE_P_TRY(tty, cap, TS, NC_bit)                                \
  ...
  TTY_CAPABLE_P_TRY(tty, TTY_CAP_UNDERLINE, tty->TS_enter_underline_mode,
                    NC_UNDERLINE);
  TTY_CAPABLE_P_TRY(tty, TTY_CAP_UNDERLINE_STYLED, tty->TF_set_underline_style,
                    NC_UNDERLINE);
...
/* Save or restore the default color-related capabilities of this
   terminal.  */
static void tty_default_color_capabilities(struct tty_display_info* tty,
                                           bool save)
{
...
static void tty_setup_colors(struct tty_display_info* tty, int mode)
{
...
void set_tty_color_mode(struct tty_display_info* tty, struct frame* f)
{
  Lisp_Object tem, val;
...
char* tty_type_name(Lisp_Object terminal)
{
  struct terminal* t = decode_tty_terminal(terminal);
...
(Lisp_Object tty)
{
  ...
  if (!t->display_info.tty->input)
  {
    if (get_named_terminal(t->display_info.tty->name))
      error("Cannot resume display while another display is active on the same "
            "device");
    ...
    if (!t->display_info.tty->input)
    {
      ...
      report_file_errno("Cannot reopen tty device",
                        build_string(t->display_info.tty->name), open_errno);
    }
...
void tty_draw_row_with_mouse_face(struct window* w,
...
static Lisp_Object tty_frame_at(int x, int y, int* cx, int* cy)
{
#ifndef HAVE_ANDROID
  for (Lisp_Object frames = Ftty_frame_list_z_order(Qnil); !NILP(frames);
       frames = Fcdr(frames))
  {
...
/* Return the current position of the mouse.
   ...
   event arrives.  */
static void term_mouse_position(struct frame** fp, int insist,
...
static Lisp_Object term_mouse_click(struct input_event* result,
                                    Gpm_Event* event, struct frame* f)
{
  int i, j;
  ...
  if (!(event->type & (GPM_MOVE | GPM_DRAG)))
  {
    ...
    /* 1 << KG_ALT || KG_ALTGR */
    if (event->modifiers & (1 << 3) || event->modifiers & (1 << 1))
...
int handle_one_term_event(struct tty_display_info* tty,
                          const Gpm_Event* event_in)
{
...
(void)
{
  ...
  struct tty_display_info* tty =
      ((f)->output_method == output_termcap ? (f)->terminal->display_info.tty
                                            : NULL);
...
DEFUN("gpm-mouse-stop", Fgpm_mouse_stop, Sgpm_mouse_stop, 0, 0, 0,
...
static tty_menu* ATTRIBUTE_MALLOC tty_menu_create(void)
{
...
static void tty_menu_make_room(tty_menu* menu)
{
  if (menu->allocated == menu->count)
  {
    ...
    menu->submenu = xrealloc(menu->submenu, allocated * sizeof *menu->submenu);
    menu->panenumber =
        xrealloc(menu->panenumber, allocated * sizeof *menu->panenumber);
    menu->help_text =
        xrealloc(menu->help_text, allocated * sizeof *menu->help_text);
...
static tty_menu* tty_menu_search_pane(tty_menu* menu, int pane)
{
  int i;
  tty_menu* try;
...
static void tty_menu_calc_size(tty_menu* menu, int* width, int* height)
{
...
static void mouse_get_xy(int* x, int* y)
{
...
static void tty_menu_display(tty_menu* menu, int x, int y, int pn, int* faces,
                             int mx, int my, int first_item, bool disp_help)
{
  ...
  for (i = 0; i < max_items; i++)
  {
    ...
    enabled = (!menu->submenu[j] && menu->panenumber[j]) || (menu->submenu[j]);
...
static int tty_menu_add_pane(tty_menu* menu, const char* txt)
{
  int len;
...
static bool tty_menu_add_selection(tty_menu* menu, int pane, char* txt,
                                   bool enable, char const* help_text)
{
  int len;
...
static void tty_menu_locate(tty_menu* menu, int x, int y, int* ulx, int* uly,
...
static struct glyph_matrix* save_and_enable_current_matrix(struct frame* f)
{
  int i;
...
static void restore_desired_matrix(struct frame* f, struct glyph_matrix* saved)
{
...
static void free_saved_screen(struct glyph_matrix* saved)
{
  int i;
...
static void screen_update(struct frame* f, struct glyph_matrix* mtx)
{
...
static mi_result read_menu_input(struct frame* sf, int* x, int* y, int min_y,
                                 int max_y, bool* first_time)
{
  if (*first_time)
  {
    *first_time = false;
...
static int tty_menu_activate(tty_menu* menu, int* pane, int* selidx, int x0,
                             int y0, char** txt,
                             ...
                             bool kbd_navigation)
{
  ...
  faces[0] = lookup_derived_face(NULL, sf, Qtty_menu_disabled_face,
                                 DEFAULT_FACE_ID, 1);
  faces[1] =
      lookup_derived_face(NULL, sf, Qtty_menu_enabled_face, DEFAULT_FACE_ID, 1);
  selectface = intern("tty-menu-selected-face");
  faces[2] = lookup_derived_face(NULL, sf, selectface, faces[0], 1);
  faces[3] = lookup_derived_face(NULL, sf, selectface, faces[1], 1);
  ...
  while (!leave)
  {
    ...
    int min_y = state[0].y;
    int max_y =
        min(min_y + state[0].menu->count, FRAME_TOTAL_LINES(sf) - 1) - 1;
        ...
        if (state[i].x <= x && x < state[i].x + state[i].menu->width + 2)
        {
          ...
          if (0 <= dy && dy < state[i].menu->count)
          {
            ...
            *selidx = dy;
            ...
            {
              tty_menu_display(state[i].menu, state[i].x, state[i].y,
                               state[i].pane, faces, x, y, first_item, 1);
              ...
              state[statecount].x = state[i].x + state[i].menu->width + 2;
              state[statecount].y = y;
              statecount++;
            }
          }
        }
      tty_menu_display(state[statecount - 1].menu, state[statecount - 1].x,
                       state[statecount - 1].y, state[statecount - 1].pane,
 ...
 item.  */
    ...
    {
      help_callback(menu_help_message, menu_help_paneno, menu_help_itemno);
...
static void tty_menu_destroy(tty_menu* menu)
{
  int i;
  if (menu->allocated)
  {
...
static void tty_menu_help_callback(char const* help_string, int pane, int item)
{
  ...
  show_help_echo(help_string ? build_string(help_string) : Qnil, Qnil,
                 menu_object, make_fixnum(item));
}
...
static void tty_pop_down_menu(void* arg)
{
...
static int tty_menu_last_menubar_item(struct frame* f)
{
  int i = 0;
...
/* Find in frame F's menu bar the menu item that is next or previous
   ...
   frames.  */
static void tty_menu_new_item_coords(struct frame* f, int which, int* x, int* y)
{
  eassert(FRAME_TERMCAP_P(f) && FRAME_LIVE_P(f));
  if (FRAME_TERMCAP_P(f) && FRAME_LIVE_P(f))
  {
...
Lisp_Object tty_menu_show(struct frame* f, int x, int y, int menuflags,
                          Lisp_Object title, const char** error_name)
{
 ...
 It's meaningful only for dialog boxes.  */
    ...
    else
    {
      ...
      if (lpane == TTYM_FAILURE ||
          (!tty_menu_add_selection(menu, lpane, item_data, !NILP(enable),
                                   help_string)))
      {
        tty_menu_destroy(menu);
 ...
 menu with help echo messages or vice versa.  */
  ...
  record_unwind_protect_ptr(
      tty_pop_down_menu, &((struct tty_pop_down_menu){menu, current_buffer}));
  ...
  case TTYM_SUCCESS:
    ...
    while (i < menu_items_used)
    {
      ...
      {
        if (pane == 0)
          pane_prefix = AREF(menu_items, i + MENU_ITEMS_PANE_PREFIX);
        pane--;
        i += MENU_ITEMS_PANE_LENGTH;
      }
      else
...
void create_tty_output(struct frame* f)
{
...
static void tty_free_frame_resources(struct frame* f)
{
  eassert(FRAME_TERMCAP_P(f));
  free_frame_faces(f);
...
struct terminal* init_tty(const char* name, const char* terminal_type,
                          bool must_succeed)
{
  ...
  if (!terminal_type)
    maybe_fatal(must_succeed, 0, "Unknown terminal type",
...

  {
    ...
    tty->input = tty->output =
        ((fd < 0 || !isatty(fd)) ? NULL : emacs_fdopen(fd, "w+"));
  ...
  {
#ifdef TERMINFO
    maybe_fatal(must_succeed, terminal, "Cannot open terminfo database file",
                "Cannot open terminfo database file");
#else
    maybe_fatal(must_succeed, terminal, "Cannot open termcap database file",
  ...
  {
    maybe_fatal(
        must_succeed, terminal, "Terminal type %s is not defined",
...
'setenv TERM ...') to specify the correct type.  It may be necessary\n"
  ...
  if (tty->TS_orig_pair)
  {
...
#ifdef TERMINFO
    {
      ...
      if (fg && bg && fg != (char*)(intptr_t)-1 && bg != (char*)(intptr_t)-1)
      {
      ...
      {
        tty->TS_set_foreground = "\033[%?%p1%{8}%<%t3%p1%d%e38;2;%p1%{65536}%/"
                                 "%d;%p1%{256}%/%{255}%&%d;%p1%{255}%&%d%;m";
        tty->TS_set_background = "\033[%?%p1%{8}%<%t4%p1%d%e48;2;%p1%{65536}%/"
                                 "%d;%p1%{256}%/%{255}%&%d;%p1%{255}%&%d%;m";
        tty->TN_max_colors = 16777216;
      }
    }
  ...
  if (FrameRows(tty) < 3 || FrameCols(tty) < 3)
    maybe_fatal(must_succeed, terminal, "Screen size %dx%d is too small",
                "Screen size %dx%d is too small", FrameCols(tty),
                FrameRows(tty));
...
'setenv TERM ...') to specify the correct type.  It may be necessary\n"
  ...
  if (FrameRows(tty) <= 0 || FrameCols(tty) <= 0)
    maybe_fatal(must_succeed, terminal, "Could not determine the frame size",
                "Could not determine the frame size");

  tty->delete_in_insert_mode = tty->TS_delete_mode && tty->TS_insert_mode &&
      !strcmp(tty->TS_delete_mode, tty->TS_insert_mode);

  UseTabs(tty) = tabs_safe_p(fileno(tty->input)) && TabWidth(tty) == 8;

  tty->scroll_region_ok = (tty->Wcm->cm_abs &&
  ...
  tty->line_ins_del_ok =
      (((tty->TS_ins_line || tty->TS_ins_multi_lines) &&
       ...
       (tty->scroll_region_ok && tty->TS_fwd_scroll && tty->TS_rev_scroll));

  tty->char_ins_del_ok =
      ((tty->TS_ins_char || tty->TS_insert_mode || tty->TS_pad_inserted_char ||
        tty->TS_ins_multi_chars) &&
...
static void maybe_fatal(bool must_succeed, struct terminal* terminal,
...
static void delete_tty(struct terminal* terminal)
{
  struct tty_display_info* tty;
...
static Lisp_Object tty_frame_geometry(Lisp_Object frame, Lisp_Object attribute)
{
  struct frame* f = decode_live_frame(frame);
  ...
  int tool_bar_width =
      (tool_bar_height ? native_width - 2 * internal_border_width : 0);
  ...
  if (EQ(attribute, Qouter_edges))
    ...
    return list(
        ...
        Fcons(Qexternal_border_size, Fcons(make_fixnum(0), make_fixnum(0))),
        ...
        Fcons(Qtab_bar_size,
              Fcons(make_fixnum(tab_bar_width), make_fixnum(tab_bar_height))),
        Fcons(Qtool_bar_external, Qnil),
        Fcons(Qtool_bar_position, FRAME_TOOL_BAR_POSITION(f)),
        Fcons(Qtool_bar_size,
              Fcons(make_fixnum(tool_bar_width), make_fixnum(tool_bar_height))),
        Fcons(Qinternal_border_width, make_fixnum(internal_border_width)));
}
...
(Lisp_Object frame) { return tty_frame_geometry(frame, Qnil); }
...
DEFUN("tty-frame-restack", Ftty_frame_restack, Stty_frame_restack, 2, 3, 0,
...
static void tty_display_dimension(Lisp_Object frame, int* width, int* height)
{
  if (!FRAMEP(frame))
...
void syms_of_term(void)
{
  DEFVAR_BOOL("system-uses-terminfo", system_uses_terminfo, doc:
                  /* Non-nil means the system uses terminfo rather than
         termcap. This variable can be used by terminal emulator packages.
       */);
...
non-nil to enable this optimization.  */);
  ...
  DEFSYM(Qf0, "f0");
  DEFSYM(Qf10, "f10");
  DEFSYM(Qtty_set_up_initial_frame_faces, "tty-set-up-initial-frame-faces");
