...
static void tty_set_terminal_modes(struct terminal* terminal) {
    ...
    if (tty->output) {
        ...
        OUTPUT_IF(tty,
                  visible_cursor ? tty->TS_cursor_visible
...
static void tty_set_scroll_region(struct frame* f, int start, int stop) {
    ...
    if (tty->TS_set_scroll_region)
        ...
        OUTPUT(tty,
               tparm(tty->TS_set_scroll_region_1, FRAME_TOTAL_LINES(f), start,
    ...
    if (encode_terminal_src_size < required)
        encode_terminal_src =
            xpalloc(encode_terminal_src, &encode_terminal_src_size,
                    required - encode_terminal_src_size, -1,
                    sizeof *encode_terminal_src);
        ...
        /* We must skip glyphs to be padded for a wide character.  */
            ...
            if (NILP(string)) {
                ...
                if (encode_terminal_src_size - nbytes < SBYTES(string)) {
                    encode_terminal_src = xpalloc(
                        encode_terminal_src, &encode_terminal_src_size,
                        (SBYTES(string) - (encode_terminal_src_size - nbytes)),
                        -1, 1);
                    buf = encode_terminal_src + nbytes;
                }
...
void calculate_costs(struct frame* frame) {
    ...
    if (FRAME_TERMCAP_P(frame)) {
        ...
        if (f && (!tty->TS_ins_line && !tty->TS_del_line))
            ...
            do_line_insertion_deletion_costs(
                frame, tty->TS_ins_line, tty->TS_ins_multi_lines,
                tty->TS_del_line, tty->TS_del_multi_lines, 0, 0, 1);
...
#define CONDITIONAL_REASSIGN(cap1, cap2, sym)                                  \
    if (!tgetstr(cap1, address)) {                                             \
        ...
        if (sequence)                                                          \
            Fdefine_key(KVAR(kboard, Vinput_decode_map),                       \
                        build_string(sequence), make_vector(1, intern(sym)),   \
                        Qnil);                                                 \
    }
...
void produce_glyphs(struct it* it) {
    ...
    if (it->char_to_display >= 040 && it->char_to_display < 0177) {
     ...
     total width of the physical line being wrapped.  */
        ...
        int next_tab_x =
            (((1 + absolute_x + it->tab_width - 1) / it->tab_width) *
             it->tab_width);
        if (!NILP(Vdisplay_line_numbers) && it->line_number_produced_p)
...
static void append_glyphless_glyph(struct it* it, int face_id,
                                   const char* str) {
    struct glyph *glyph, *end;
    int i;
     ...
     of canonical width.  */
        ...
        if (it->glyphless_method == GLYPHLESS_DISPLAY_ACRONYM) {
            ...
            if (STRINGP(acronym) && SCHARS(acronym) == 1) {
                ...
                buf[0] = '[';
                for (len = 0; len < 6 && str[len] && ASCII_CHAR_P(str[len]);
                     len++)
                    buf[1 + len] = str[len];
                buf[1 + len] = ']';
                len += 2;
...
static void turn_on_face(struct frame* f, struct face* face) {
    ...
    if (MAY_USE_WITH_COLORS_P(tty, NC_REVERSE) &&
        (inverse_video ? fg == FACE_TTY_DEFAULT_FG_COLOR ||
    ...
    if (tty->TS_exit_attribute_mode) {
     ...
     may not turn off alt-char-mode.  */
        if (face->tty_bold_p || face->tty_italic_p || face->tty_reverse_p ||
            ...
            if (strcmp(tty->TS_exit_attribute_mode,
...
#define TTY_CAPABLE_P_TRY(tty, cap, TS, NC_bit)                                \
    ...
    TTY_CAPABLE_P_TRY(tty, TTY_CAP_UNDERLINE, tty->TS_enter_underline_mode,
                      NC_UNDERLINE);
    TTY_CAPABLE_P_TRY(tty, TTY_CAP_UNDERLINE_STYLED,
...
(Lisp_Object tty) {
    ...
    if (!t->display_info.tty->input) {
        if (get_named_terminal(t->display_info.tty->name))
            error("Cannot resume display while another display is active on "
                  "the same "
                  "device");
...
int handle_one_term_event(struct tty_display_info* tty,
    ...
    if (event.type & (GPM_MOVE | GPM_DRAG)) {
     ...
     directly from GPM.  */
        ...
        ie.arg = tty_handle_tab_bar_click(
            f, event.x, event.y, (ie.modifiers & down_modifier) != 0, &ie);
...
static void tty_menu_make_room(tty_menu* menu) {
    if (menu->allocated == menu->count) {
        ...
        menu->submenu =
            xrealloc(menu->submenu, allocated * sizeof *menu->submenu);
        menu->panenumber =
...
static void tty_menu_display(tty_menu* menu, int x, int y, int pn, int* faces,
    ...
    int max_items =
        min(menu->count - first_item, FRAME_TOTAL_LINES(sf) - 1 - y);
...
static void restore_desired_matrix(struct frame* f,
...
static int tty_menu_activate(tty_menu* menu, int* pane, int* selidx, int x0,
    ...
    faces[0] = lookup_derived_face(NULL, sf, Qtty_menu_disabled_face,
                                   DEFAULT_FACE_ID, 1);
    faces[1] = lookup_derived_face(NULL, sf, Qtty_menu_enabled_face,
                                   DEFAULT_FACE_ID, 1);
    ...
    while (!leave) {
        ...
        if (sf->mouse_moved && input_status != MI_QUIT_MENU) {
            ...
            for (i = 0; i < statecount; i++)
                if (state[i].x <= x &&
                    x < state[i].x + state[i].menu->width + 2) {
                    ...
                    if (0 <= dy && dy < state[i].menu->count) {
                        ...
                        *selidx = dy;
                        ...
                        if (i == statecount - 1 && state[i].menu->submenu[dy]) {
                            tty_menu_display(state[i].menu, state[i].x,
                                             state[i].y, state[i].pane, faces,
                                             x, y, first_item, 1);
                            state[statecount].menu = state[i].menu->submenu[dy];
                            state[statecount].pane =
                                state[i].menu->panenumber[dy];
                            state[statecount].screen_behind =
            ...
            tty_menu_display(state[statecount - 1].menu,
                             state[statecount - 1].x, state[statecount - 1].y,
                             state[statecount - 1].pane, faces, x, y,
                             first_item, 1);
...
static void tty_menu_help_callback(char const* help_string, int pane,
                                   int item) {
...
/* Find in frame F's menu bar the menu item that is next or previous
   ...
   frames.  */
static void tty_menu_new_item_coords(struct frame* f, int which, int* x,
                                     int* y) {
    eassert(FRAME_TERMCAP_P(f) && FRAME_LIVE_P(f));
    if (FRAME_TERMCAP_P(f) && FRAME_LIVE_P(f)) {
        Lisp_Object items = FRAME_MENU_BAR_ITEMS(f);
...
#ifdef TERMINFO
        {
            ...
            /* Fall back to xterm+direct (semicolon version) if Tc is set
               ...
               the COLORTERM environment variable.  */
                ...
                tty->TS_set_foreground =
                    "\033[%?%p1%{8}%<%t3%p1%d%e38;2;%p1%{65536}%/"
                    "%d;%p1%{256}%/%{255}%&%d;%p1%{255}%&%d%;m";
                tty->TS_set_background =
                    "\033[%?%p1%{8}%<%t4%p1%d%e48;2;%p1%{65536}%/"
                    "%d;%p1%{256}%/%{255}%&%d;%p1%{255}%&%d%;m";
                tty->TN_max_colors = 16777216;
            }
        }
...
static Lisp_Object tty_frame_geometry(Lisp_Object frame,
