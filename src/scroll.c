...
static void do_direct_scrolling(struct frame*, struct glyph_matrix*,
                                struct matrix_elt*, int, int);
static void do_scrolling(struct frame*, struct glyph_matrix*,
                         struct matrix_elt*, int, int);
...
calculate_scrolling(struct frame* frame,
                    ...
                    struct matrix_elt* matrix, int window_size, int lines_below,
    ...
    int lines_moved =
        window_size + (FRAME_SCROLL_REGION_OK(frame) ? 0 : lines_below);
    ...
    /* Discourage long scrolls on fast lines.
       ...
       at least 1/4 second.  */
    int extra_cost = clip_to_bounds(1, baud_rate / (10 * 4) / frame_total_lines,
                                    INT_MAX / 2);
...
static void do_scrolling(struct frame* frame,
...
#define CHECK_BOUNDS                                                           \
    ...
    mirrored_line_dance(frame, unchanged_at_top, window_size, copy_from,
                        retained_p);
...
calculate_direct_scrolling(struct frame* frame,
                           /* matrix is of size window_size + 1 on each side. */
                           struct matrix_elt* matrix, int window_size,
                           int lines_below, int* draw_cost, int* old_draw_cost,
    ...
    int* first_insert_cost = &FRAME_INSERT_COST(frame)[frame_total_lines - 1];
    int* first_delete_cost = &FRAME_DELETE_COST(frame)[frame_total_lines - 1];
    int* next_insert_cost = &FRAME_INSERTN_COST(frame)[frame_total_lines - 1];
    int* next_delete_cost = &FRAME_DELETEN_COST(frame)[frame_total_lines - 1];
    ...
    scroll_overhead = FRAME_SCROLL_REGION_COST(frame) + extra_cost;
            ...
            if (i == j) {
                ...
                if (i > j) {
                    ...
                    cost1 += first_insert_cost[-j] - first_insert_cost[1 - j] +
                        (delta - 1) *
                            (next_insert_cost[-j] - next_insert_cost[1 - j]);
                }
                else {
                    delta = j - i;
                    ...
                    cost1 += first_delete_cost[-i] - first_delete_cost[1 - i] +
                        (delta - 1) *
                            (next_delete_cost[-i] - next_delete_cost[1 - i]);
                }
                if (cost1 < cost) {
                    cost = cost1;
                    p->writecount = p1->writecount + 1;
...
static void do_direct_scrolling(struct frame* frame,
                                struct glyph_matrix* current_matrix,
    ...
    while (i > 0 || j > 0) {
        ...
        else if (p->deletecost < p->writecost && (write_follows_p || i > j)) {
...
void scrolling_1(struct frame* frame, int window_size, int unchanged_at_top,
    ...
    if (FRAME_SCROLL_REGION_OK(frame)) {
        calculate_direct_scrolling(
            frame, matrix, window_size, unchanged_at_bottom, draw_cost,
            old_draw_cost, old_hash, new_hash, free_at_end);
        do_direct_scrolling(frame, frame->current_matrix, matrix, window_size,
                            unchanged_at_top);
    }
    else {
        calculate_scrolling(frame, matrix, window_size, unchanged_at_bottom,
                            draw_cost, old_hash, new_hash, free_at_end);
        do_scrolling(frame, frame->current_matrix, matrix, window_size,
                     unchanged_at_top);
    }
...
int scrolling_max_lines_saved(int start, int end, unsigned* oldhash,
                              unsigned* newhash, int* cost) {
    enum
    {
        LOG2_NLINES = 9
    };
...
static void line_ins_del(struct frame* frame, int ov1, int pf1, int ovn,
                         int pfn, int* ov, int* mf) {
    int i;
    int frame_total_lines = FRAME_TOTAL_LINES(frame);
...
static void ins_del_costs(struct frame* frame, const char* one_line_string,
                          const char* multi_string, const char* setup_string,
                          const char* cleanup_string, int* costvec,
                          int* ncostvec, int coefficient) {
    if (multi_string)
        line_ins_del(frame, string_cost(multi_string) * coefficient,
                     per_line_cost(multi_string) * coefficient, 0, 0, costvec,
                     ncostvec);
    else if (one_line_string)
        line_ins_del(frame,
                     string_cost(setup_string) + string_cost(cleanup_string), 0,
                     string_cost(one_line_string),
                     per_line_cost(one_line_string), costvec, ncostvec);
    else
        line_ins_del(frame, 9999, 0, 9999, 0, costvec, ncostvec);
}
...
void do_line_insertion_deletion_costs(
    struct frame* frame, const char* ins_line_string,
    const char* multi_ins_string, const char* del_line_string,
    const char* multi_del_string, const char* setup_string,
    const char* cleanup_string, int coefficient) {
    int frame_total_lines = FRAME_TOTAL_LINES(frame);
    ...
    ins_del_costs(frame, ins_line_string, multi_ins_string, setup_string,
                  cleanup_string, FRAME_INSERT_COST(frame),
                  FRAME_INSERTN_COST(frame), coefficient);
    ins_del_costs(frame, del_line_string, multi_del_string, setup_string,
                  cleanup_string, FRAME_DELETE_COST(frame),
                  FRAME_DELETEN_COST(frame), coefficient);
}
