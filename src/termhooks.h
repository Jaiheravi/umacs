...
enum event_kind
{
    ...
    SCROLL_BAR_CLICK_EVENT, /* .code gives the number of the mouse button
                   ...
                   .timestamp gives a timestamp (in
                   milliseconds) for the click.  */
    HORIZONTAL_SCROLL_BAR_CLICK_EVENT, /* .code gives the number of the mouse
                  button that was clicked. .modifiers holds the state of the
                  modifier keys. .part is a lisp symbol indicating which part of
                  the scroll bar got clicked. .x gives the distance from the
                  start of the scroll bar of the click; .y gives the total
                  length of the scroll bar.
                  .frame_or_window gives the window
                  whose scroll bar was clicked in.
                  .timestamp gives a timestamp (in
...
#define EVENT_INIT(event)                                                      \
    (memset(&(event), 0, sizeof(struct input_event)), (event).device = Qt)
...
struct terminal {
    ...
    void (*mouse_position_hook)(struct frame** f, int, Lisp_Object* bar_window,
    ...
    void (*set_vertical_scroll_bar_hook)(struct window* window, int portion,
    ...
    /* Set the horizontal scroll bar for WINDOW to have its upper left
       ...
       of WHOLE characters, starting at POSITION.  If WINDOW doesn't yet
       have a scroll bar, create one for it.  */
    void (*set_horizontal_scroll_bar_hook)(struct window* window, int portion,
                                           int whole, int position);
    ...
    /* Retrieve the string resource specified by NAME with CLASS from
       database RDB. */
    const char* (*get_string_resource_hook)(void* rdb, const char* name,
                                            const char* class);
...
INLINE bool TERMINALP(Lisp_Object a) { return PSEUDOVECTORP(a, PVEC_TERMINAL); }

INLINE struct terminal* XTERMINAL(Lisp_Object a) {
    eassert(TERMINALP(a));
...
/* Most code should use these functions to set Lisp fields in struct
   terminal.  */
INLINE void tset_charset_list(struct terminal* t, Lisp_Object val) {
    t->charset_list = val;
}
INLINE void tset_selection_alist(struct terminal* t, Lisp_Object val) {
