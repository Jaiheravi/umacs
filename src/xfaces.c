...
bool parse_color_spec(const char* spec, unsigned short* r, unsigned short* g,
    ...
    if (spec[0] == '#') {
        if ((len - 1) % 3 == 0) {
            int n = (len - 1) / 3;
            return (
                parse_hex_color_comp(spec + 1 + 0 * n, spec + 1 + 1 * n, r) &&
...
(Lisp_Object spec) {
    ...
    return (parse_color_spec(SSDATA(spec), &r, &g, &b) ? list3i(r, g, b)
                                                       : Qnil);
}
...
static void merge_face_vectors(struct window* w, struct frame* f,
    ...
    if (FONT_SPEC_P(from[LFACE_FONT_INDEX])) {
        if (!UNSPECIFIEDP(to[LFACE_FONT_INDEX]))
            font =
                merge_font_spec(from[LFACE_FONT_INDEX], to[LFACE_FONT_INDEX]);
        else
    ...
    if (!NILP(font)) {
        if (!NILP(AREF(font, FONT_FOUNDRY_INDEX)))
            to[LFACE_FOUNDRY_INDEX] =
                SYMBOL_NAME(AREF(font, FONT_FOUNDRY_INDEX));
...
static bool face_inheritance_cycle(struct frame* f, Lisp_Object face,
    ...
    if (CONSP(face)) {
        ...
        for (tail = face; CONSP(tail); tail = XCDR(tail)) {
            ...
            ok = get_lface_attributes(NULL, f, member_face, face_attrs, false,
                                      NULL);
            if (!ok)
                break;
        ...
        if (EQ(first, Qforeground_color) || EQ(first, Qbackground_color)) {
             ...
             list shall be merged.  */
                ...
                if (!attr_filter_seen) {
                    ...
                    if (!merge_face_ref(w, f, parent_face, scratch_attrs,
                        ...
                        add_to_log("Invalid face attribute %S %S", QCinherit,
                                   parent_face);
                        return false;
                    }
            ...
            while (CONSP(face_ref) && CONSP(XCDR(face_ref))) {
                ...
                if (EQ(value, Qunspecified))
                    ...
                    if (SYMBOLP(value) &&
                        FONT_WEIGHT_NAME_NUMERIC(value) >= 0) {
    ...
    if (EQ(attr, QCfamily)) {
        ...
        if (!UNSPECIFIEDP(value) && !IGNORE_DEFFACE_P(value) &&
            ...
            if (EQ(face, Qdefault)) {
                ...
                if (!FIXNUMP(value) || XFIXNUM(value) <= 0)
                    signal_error(
                        "Default face height not absolute and positive", value);
            }
            else {
     ...
     of the face.  */
        ...
        if (!UNSPECIFIEDP(value) && !IGNORE_DEFFACE_P(value) &&
            ...
            CHECK_SYMBOL(value);
            if (!EQ(value, Qt) && !NILP(value))
                signal_error("Invalid inverse-video face attribute value",
                             value);
        }
        ...
        if (EQ(value, face) || face_inheritance_cycle(f, value, face))
            signal_error("Face inheritance results in inheritance cycle",
                         value);
        else if (NILP(tail))
    ...
    if (EQ(face, Qdefault)) {
        ...
        if (oldface) {
            /* In some cases, realize_face below can call Lisp, which could
                     ...
                     the default face, and therefore are not ready to do
               display.  */
...
(Lisp_Object face1, Lisp_Object face2, Lisp_Object frame) {
    ...
    equal_p =
        lface_equal_p(XVECTOR(lface1)->contents, XVECTOR(lface2)->contents);
...
static int color_distance(Emacs_Color* x, Emacs_Color* y) {
    /* This formula is from a paper titled `Colour metric' by Thiadmer
     Riemersma. Quoting from that paper:
...
(Lisp_Object color1, Lisp_Object color2, Lisp_Object frame,
 Lisp_Object metric) {
    ...
    if (!(CONSP(color1) && parse_rgb_list(color1, &cdef1)) &&
        !(STRINGP(color1) &&
          FRAME_TERMINAL(f)->defined_color_hook(f, SSDATA(color1), &cdef1,
                                                false, true)))
        signal_error("Invalid color", color1);
    if (!(CONSP(color2) && parse_rgb_list(color2, &cdef2)) &&
        !(STRINGP(color2) &&
          FRAME_TERMINAL(f)->defined_color_hook(f, SSDATA(color2), &cdef2,
                                                false, true)))
...
static void cache_face(struct face_cache* c, struct face* face,
                       uintptr_t hash) {
...
int smaller_face(struct frame* f, int face_id, int steps) { return face_id; }
...
(Lisp_Object plist) {
    ...
    merge_face_ref(NULL, XFRAME(selected_frame), plist,
                   XVECTOR(lface)->contents, true, NULL, 0);
    return lface;
}
...
static bool tty_supports_face_attributes_p(struct frame* f,
    ...
    if (!UNSPECIFIEDP(val) &&
        ...
        int def_weight =
            FONT_WEIGHT_NAME_NUMERIC(def_attrs[LFACE_WEIGHT_INDEX]);
...
(Lisp_Object attributes, Lisp_Object display) {
    ...
    if (NILP(display))
        ...
        frame = Qnil;
        FOR_EACH_FRAME(tail, frame)
        if (!NILP(Fequal(Fcdr(Fassq(Qdisplay, XFRAME(frame)->param_alist)),
                         display)))
            break;
    }
    ...
    *endptr = endpos;

    {
        ...
        if (!default_face) {
            ...
            default_face =
                FACE_FROM_ID(f, lookup_basic_face(w, f, DEFAULT_FACE_ID));
        }
    }
...
void init_xfaces(void) {
    ...
    if (dumped_with_pdumper_p()) {
        ...
        if (nfaces > 0) {
            ...
            lface_id_to_name =
                xnmalloc(next_lface_id, sizeof *lface_id_to_name);

            /* Store the faces.  */
            struct Lisp_Hash_Table* table =
                XHASH_TABLE(Vface_new_frame_defaults);
...
:/* Hash table of global face definitions (for internal use only.)  */);
    ...
    DEFVAR_LISP("tty-defined-color-alist", Vtty_defined_color_alist, doc:
                    /* An alist of defined terminal colors and their RGB
            values. See the docstring of `tty-color-alist' for the details.
          */);
