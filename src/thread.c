...
static union aligned_thread_state main_thread = {{
    .header.size = PVECHEADERSIZE(
        PVEC_THREAD, PSEUDOVECSIZE(struct thread_state, event_object),
...
static void release_global_lock(void) { sys_mutex_unlock(&global_lock); }

static void rebind_for_thread_switch(void) {
    ptrdiff_t distance =
        current_thread->m_specpdl_ptr - current_thread->m_specpdl;
    specpdl_unrewind(specpdl_ptr, -distance, true);
}

static void unbind_for_thread_switch(struct thread_state* thr) {
...
static void post_acquire_global_lock(struct thread_state* self) {
...
static void acquire_global_lock(struct thread_state* self) {
...
/* This is called from keyboard.c when it detects that SIGINT was
   ...
   undefined behavior.  */
void maybe_reacquire_global_lock(void) {
...
static void lisp_mutex_init(lisp_mutex_t* mutex) {
    mutex->owner = NULL;
    mutex->count = 0;
...
/* Lock MUTEX for thread LOCKER, setting its lock count to COUNT, if
   ...
   zero otherwise.  */
static int lisp_mutex_lock_for_thread(lisp_mutex_t* mutex,
                                      struct thread_state* locker,
                                      int new_count) {
    ...
    while (mutex->owner != NULL && (new_count != 0 || NILP(self->error_symbol)))
...
static int lisp_mutex_lock(lisp_mutex_t* mutex, int new_count) {
...
/* Decrement MUTEX's lock count.  If the lock count becomes zero after
   ...
   zero otherwise.  */
static int lisp_mutex_unlock(lisp_mutex_t* mutex) {
...
static unsigned int lisp_mutex_unlock_for_wait(lisp_mutex_t* mutex) {
...
static void lisp_mutex_destroy(lisp_mutex_t* mutex) {
    sys_cond_destroy(&mutex->condition);
}

static int lisp_mutex_owned_p(lisp_mutex_t* mutex) {
...
static void mutex_lock_callback(void* arg) {
...
static void do_unwind_mutex_lock(void) { current_thread->event_object = Qnil; }
...
static void mutex_unlock_callback(void* arg) {
    struct Lisp_Mutex* mutex = arg;
    struct thread_state* self = current_thread;
...
void finalize_one_mutex(struct Lisp_Mutex* mutex) {
...
static void condition_wait_callback(void* arg) {
...
static void condition_notify_callback(void* arg) {
...
void finalize_one_condvar(struct Lisp_CondVar* condvar) {
...
static void really_call_select(void* arg) {
...
int thread_select(select_func* func, int max_fds, fd_set* rfds, fd_set* wfds,
                  fd_set* efds, struct timespec* timeout, sigset_t* sigmask) {
...
static void mark_one_thread(struct thread_state* thread) {
    ...
    for (struct handler* handler = thread->m_handlerlist; handler;
...
static void mark_threads_callback(void* ignore) {
...
void mark_threads(void) { flush_stack_call_func(mark_threads_callback, NULL); }

void unmark_main_thread(void) { main_thread.s.header.size &= ~ARRAY_MARK_FLAG; }


static void yield_callback(void* ignore) {
    struct thread_state* self = current_thread;
...
static Lisp_Object invoke_thread_function(void) {
    specpdl_ref count = SPECPDL_INDEX();
...
static Lisp_Object record_thread_error(Lisp_Object error_form) {
...
#if defined HAVE_ANDROID && !defined ANDROID_STUBIFY
    rc =
        (*android_jvm)->AttachCurrentThread(android_jvm, &self->java_env, NULL);
    if (rc != JNI_OK)
        emacs_abort();
#endif /* defined HAVE_ANDROID && !defined ANDROID_STUBIFY */
#endif /* THREADS_ENABLED */
...
static void free_search_regs(struct re_registers* regs) {
    if (regs->num_regs != 0) {
        xfree(regs->start);
        xfree(regs->end);
    }
...
void finalize_one_thread(struct thread_state* state) {
...
(Lisp_Object function, Lisp_Object name, Lisp_Object buffer_disposition) {
    ...
    struct thread_state* new_thread = ALLOCATE_ZEROED_PSEUDOVECTOR(
...
static void thread_signal_callback(void* arg) {
...
static void thread_set_error(struct thread_state* tstate,
...
static void thread_join_callback(void* arg) {
    struct thread_state* tstate = arg;
    struct thread_state* self = current_thread;
    Lisp_Object thread;
...
bool thread_check_current_buffer(struct buffer* buffer) {
    struct thread_state* iter;
...
void thread_all_before_buffer_killed(Lisp_Object current) {
    struct thread_state* iter;
...
bool main_thread_p(const void* ptr) { return ptr == &main_thread.s; }

bool in_current_thread(void) {
