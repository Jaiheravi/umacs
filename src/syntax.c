...
static bool SYNTAX_FLAGS_COMSTART_FIRST(int flags) { return (flags >> 16) & 1; }
static bool SYNTAX_FLAGS_COMSTART_SECOND(int flags) {
    return (flags >> 17) & 1;
}
static bool SYNTAX_FLAGS_COMEND_FIRST(int flags) { return (flags >> 18) & 1; }
static bool SYNTAX_FLAGS_COMEND_SECOND(int flags) { return (flags >> 19) & 1; }
static bool SYNTAX_FLAGS_COMSTARTEND_FIRST(int flags) {
    return (flags & 0x50000) != 0;
}
static bool SYNTAX_FLAGS_PREFIX(int flags) { return (flags >> 20) & 1; }
static bool SYNTAX_FLAGS_COMMENT_STYLEB(int flags) { return (flags >> 21) & 1; }
static bool SYNTAX_FLAGS_COMMENT_STYLEC(int flags) { return (flags >> 23) & 1; }
static int SYNTAX_FLAGS_COMMENT_STYLEC2(int flags) {
    return (flags >> 22) & 2; /* SYNTAX_FLAGS_COMMENT_STYLEC (flags) * 2 */
}
static bool SYNTAX_FLAGS_COMMENT_NESTED(int flags) { return (flags >> 22) & 1; }

/* FLAGS should be the flags of the main char of the comment marker, e.g.
   the second for comstart and the first for comend.  */
static int SYNTAX_FLAGS_COMMENT_STYLE(int flags, int other_flags) {
...
static bool SYNTAX_COMEND_FIRST(int c) {
...
static void scan_sexps_forward(struct lisp_parse_state*, ptrdiff_t, ptrdiff_t,
                               ptrdiff_t, EMACS_INT, bool, int);
...
static void bset_syntax_table(struct buffer* b, Lisp_Object val) {
    b->syntax_table_ = val;
}

/* Whether the syntax of the character C has the prefix flag set.  */
bool syntax_prefix_flag_p(int c) {
...
enum
{
    INTERVALS_AT_ONCE = 10
}; /* 1 + max-number of intervals
to scan to property-change.  */

/* Set the syntax entry VAL for char C in table TABLE.  */

static void SET_RAW_SYNTAX_ENTRY(Lisp_Object table, int c, Lisp_Object val) {
...
static void SET_RAW_SYNTAX_ENTRY_RANGE(Lisp_Object table, Lisp_Object range,
                                       Lisp_Object val) {
...
static Lisp_Object SYNTAX_MATCH(int c) {
...
static void SETUP_SYNTAX_TABLE(ptrdiff_t from, ptrdiff_t count) {
...
void RE_SETUP_SYNTAX_TABLE_FOR_OBJECT(Lisp_Object object, ptrdiff_t frombyte) {
    SETUP_BUFFER_SYNTAX_TABLE();
    ...
    if (BUFFERP(gl_state.object)) {
        ...
        gl_state.b_property = BEG;
        gl_state.e_property =
            ZV; /* FIXME: Why not +1 like in SETUP_SYNTAX_TABLE? */
    }
    ...
    if (parse_sexp_lookup_properties)
        update_syntax_table(RE_SYNTAX_TABLE_BYTE_TO_CHAR(frombyte), 1, 1,
                            gl_state.object);
}
...
void update_syntax_table(ptrdiff_t charpos, EMACS_INT count, bool init,
                         Lisp_Object object) {
    ...
    if (invalidate)
        invalidate =
            !EQ(tmp_table, gl_state.old_prop); /* Need to invalidate? */
    ...
    while (i) {
        if (cnt && !EQ(tmp_table, textget(i->plist, Qsyntax_table))) {
            if (count > 0) {
                ...
                gl_state.b_property = i->position + LENGTH(i);
                gl_state.backward_i = i;
            }
            return;
        }
...
static void parse_sexp_propertize(ptrdiff_t charpos) {
    EMACS_INT zv = ZV;
    if (syntax_propertize__done <= charpos && syntax_propertize__done < zv) {
        ...
        gl_state.e_property_truncated = true;
             ...
             gl_state.e_property <
                 syntax_propertize__done) { /* When moving backward, e_property
                                           might be set without resetting
                                           e_property_truncated, so the
                                           e_property_truncated flag may
                                           occasionally be left raised
                                           spuriously.  This should be rare.  */
        gl_state.e_property_truncated = false;
        update_syntax_table_forward(charpos, false, Qnil);
    }
}

void update_syntax_table_forward(ptrdiff_t charpos, bool init,
                                 Lisp_Object object) {
...
static bool char_quoted(ptrdiff_t charpos, ptrdiff_t bytepos) {
...
static ptrdiff_t dec_bytepos(ptrdiff_t bytepos) {
    return (bytepos -
...
static ptrdiff_t find_defun_start(ptrdiff_t pos, ptrdiff_t pos_byte) {
    ...
    if (current_buffer == find_start_buffer
       ...
       Our value may not be the best possible, but will still be usable.  */
        && pos <= find_start_pos + 1000 && pos >= find_start_value &&
        BEGV == find_start_begv && MODIFF == find_start_modiff)
...
static bool prev_char_comend_first(ptrdiff_t pos, ptrdiff_t pos_byte) {
    int c;
    bool val;
...
static bool back_comment(ptrdiff_t from, ptrdiff_t from_byte, ptrdiff_t stop,
    ...
    while (from != stop) {
        ...
        com2start =
            (SYNTAX_FLAGS_COMSTART_FIRST(syntax) &&
             ...
             (comstyle == SYNTAX_FLAGS_COMMENT_STYLE(prev_syntax, syntax)) &&
        ...
        case Scomment:
            ...
            if (!comnested) {
                ...
                /* nested comments have to be balanced, so we don't need to
                   keep looking for earlier ones.  We use here the same
                   (slightly incorrect) reasoning as below:  since it is
                   followed by uniform paired string quotes, this comment-start
                   has to be outside of strings, else the comment-end itself
                   would be inside a string. */
                goto done;
            break;
        ...
        case Sendcomment:
            ...
            /* This is the same style of comment ender as ours. */
            {
                ...
                /* We're mixing comment styles here, so we'd better be careful.
                   ...
                   correct (we should just always set comment_lossage), but
                   removing it would imply that any multiline comment in C would
                   go through lossage, which seems overkill. The failure should
                   only happen in the rare cases such as { (* } *)   */
                comment_lossage = 1;
            break;
    ...
    lossage:
    {
       ...
       last passed a comment starter.  */
        ...
        do {
            ...
            scan_sexps_forward(&state, defun_start, defun_start_byte,
                               comment_end, TYPE_MINIMUM(EMACS_INT), 0, 0);
            defun_start = comment_end;
            if (!adjusted) {
                adjusted = true;
                find_start_value = CONSP(state.levelstarts)
                    ? XFIXNUM(XCAR(state.levelstarts))
...
(Lisp_Object object) {
    if (CHAR_TABLE_P(object) && EQ(XCHAR_TABLE(object)->purpose, Qsyntax_table))
        return Qt;
    return Qnil;
}
...
(void) { return BVAR(current_buffer, syntax_table); }
...
unsigned char const syntax_spec_code[0400] = {
    ...
    0377,          0377,           0377,        0377,
    0377,          0377,           0377,        0377,
    Swhitespace,   Scomment_fence, Sstring,     0377,
    Smath,         0377,           0377,        Squote,
    Sopen,         Sclose,         0377,        0377,
    0377,          Swhitespace,    Spunct,      Scharquote,
    0377,          0377,           0377,        0377,
    0377,          0377,           0377,        0377,
    0377,          0377,           0377,        0377,
    Scomment,      0377,           Sendcomment, 0377,
    Sinherit,      0377,           0377,        0377,
    0377,          0377,           0377,        0377, /* @, A ... */
    0377,          0377,           0377,        0377,
    0377,          0377,           0377,        0377,
    0377,          0377,           0377,        0377,
    0377,          0377,           0377,        Sword,
    0377,          0377,           0377,        0377,
    Sescape,       0377,           0377,        Ssymbol,
    0377,          0377,           0377,        0377,
    0377,          0377,           0377,        0377, /* `, a, ... */
    0377,          0377,           0377,        0377,
    0377,          0377,           0377,        0377,
    ...
    0377,          0377,           0377,        Sword,
    0377,          0377,           0377,        0377,
    Sstring_fence, 0377,           0377,        0377};

/* Indexed by syntax code, give the letter that describes it.  */

static char const syntax_code_spec[16] = {' ',  '.',  'w', '_',  '(', ')',
                                          '\'', '\"', '$', '\\', '/', '<',
                                          '>',  '@',  '!', '|'};
...
(Lisp_Object syntax) {
    ...
    if (syn < 0 || syn >= sizeof syntax_code_spec)
        args_out_of_range(make_fixnum(sizeof syntax_code_spec - 1), syntax);
...
(Lisp_Object syntax) {
    ...
    case Swhitespace:
        ...
        break;
    case Spunct:
        insert_string("punctuation");
        break;
    case Sword:
        insert_string("word");
        break;
    case Ssymbol:
        insert_string("symbol");
        break;
    case Sopen:
        insert_string("open");
        break;
    ...
    case Smath:
        ...
        break;
    case Sescape:
        insert_string("escape");
        break;
    case Scharquote:
        insert_string("charquote");
        break;
    case Scomment:
        insert_string("comment");
        break;
    case Sendcomment:
        insert_string("endcomment");
        break;
    case Sinherit:
        insert_string("inherit");
        break;
    case Scomment_fence:
        insert_string("comment fence");
        break;
    case Sstring_fence:
        insert_string("string fence");
        break;
    default:
        insert_string("invalid");
        return syntax;
...
ptrdiff_t scan_words(ptrdiff_t from, EMACS_INT count) {
    ptrdiff_t beg = BEGV;
    ptrdiff_t end = ZV;
...
(Lisp_Object arg) {
    ...
    tmp = Fconstrain_to_field(make_fixnum(val), make_fixnum(PT), Qnil, Qnil,
                              Qnil);
    val = XFIXNAT(tmp);
...
(Lisp_Object string, Lisp_Object lim) { return skip_chars(1, string, lim); }

DEFUN ("skip-chars-backward", Fskip_chars_backward, Sskip_chars_backward, 1, 2, 0,
       doc: /* Move point backward, stopping after a char not in STRING, or at pos LIM.
See `skip-chars-forward' for details.
Returns the distance traveled, either zero or negative.  */)
(Lisp_Object string, Lisp_Object lim) { return skip_chars(0, string, lim); }
...
(Lisp_Object syntax, Lisp_Object lim) { return skip_syntaxes(1, syntax, lim); }
...
(Lisp_Object syntax, Lisp_Object lim) { return skip_syntaxes(0, syntax, lim); }

static Lisp_Object skip_chars(bool forwardp, Lisp_Object string,
                              Lisp_Object lim) {
    int c;
    char fastmap[0400];
    ...
    i_byte = 0;
    if (i_byte < size_byte && SREF(string, 0) == '^') {
        negate = 1;
        i_byte++;
    }
    ...
    if (!string_multibyte) {
        ...
        while (i_byte < size_byte) {
            ...
            /* Treat `-' as range character only if another character
               follows.  */
            if (i_byte + 1 < size_byte && str[i_byte] == '-') {
                int c2;
...

    {
        ...
        if (forwardp) {
            endp =
                (XFIXNUM(lim) == GPT) ? GPT_ADDR : CHAR_POS_ADDR(XFIXNUM(lim));
...
static Lisp_Object skip_syntaxes(bool forwardp, Lisp_Object string,
                                 Lisp_Object lim) {
    int c;
...

    {
        ...
        if (forwardp) {
            while (true) {
                ...
                endp = XFIXNUM(lim) == GPT ? GPT_ADDR
                                           : CHAR_POS_ADDR(XFIXNUM(lim));
                stop = pos < GPT && GPT < XFIXNUM(lim) ? GPT_ADDR : endp;
...
static bool in_classes(int c, int nclasses, const unsigned char* classes) {
...
static bool forw_comment(ptrdiff_t from, ptrdiff_t from_byte, ptrdiff_t stop,
    ...
    while (1) {
        if (from == stop) {
            ...
            *last_syntax_ptr =
                (code == Sescape || code == Scharquote ||
                 ...
                 (nesting > 0 && SYNTAX_FLAGS_COMSTART_FIRST(syntax)))
                ? syntax
                : Smax;
            return 0;
        }
        ...
        if (code == Sendcomment &&
            ...
            /* We have encountered a comment end of the same style
               ...
               section.  */
            break;
        if (code == Scomment_fence && style == ST_COMMENT_STYLE)
            /* We have encountered a comment end of the same style
               as the comment sequence which began this comment
               section.  */
            break;
        if (nesting > 0 && code == Scomment &&
            SYNTAX_FLAGS_COMMENT_NESTED(syntax) &&
            SYNTAX_FLAGS_COMMENT_STYLE(syntax, 0) == style)
    ...
    forw_incomment:
        ...
        if (nesting > 0 && from < stop && SYNTAX_FLAGS_COMSTART_FIRST(syntax) &&
            (c1 = FETCH_CHAR_AS_MULTIBYTE(from_byte),
             other_syntax = SYNTAX_WITH_FLAGS(c1),
...
static enum syntaxcode syntax_multibyte(int c, bool multibyte_symbol_p) {
    return ASCII_CHAR_P(c) || !multibyte_symbol_p ? SYNTAX(c) : Ssymbol;
}

static Lisp_Object scan_lists(EMACS_INT from0, EMACS_INT count, EMACS_INT depth,
                              bool sexpflag) {
    Lisp_Object val;
    ...
    while (count > 0) {
        while (from < stop) {
            ...
            case Scomment:
                ...
                UPDATE_SYNTAX_TABLE_FORWARD(from);
                found =
                    forw_comment(from, from_byte, stop, comnested, comstyle, 0,
                                 &out_charpos, &out_bytepos, &dummy, &dummy2);
                from = out_charpos, from_byte = out_bytepos;
                if (!found) {
                    if (depth == 0)
            ...
            case Sstring_fence:
                ...
                while (1) {
                    ...
                    if (code == Sstring ? c == stringterm && c_code == Sstring
    ...
    while (count < 0) {
        while (from > stop) {
            ...
            case Sstring_fence:
                ...
                if (code == Sstring_fence && !depth && sexpflag)
                    goto done2;
                break;
        ...
        /* Reached start of buffer.  Error if within object, return nil if
         * between */
        if (depth)
            goto lose;
...
lose:
    xsignal3(Qscan_error, build_string("Unbalanced parentheses"),
             make_fixnum(last_good), make_fixnum(from));
}
...
/* If the character at FROM_BYTE is the second part of a 2-character
   ...
   true.  */
static bool in_2char_comment_start(struct lisp_parse_state* state,
                                   int prev_from_syntax, ptrdiff_t prev_from,
                                   ptrdiff_t from_byte) {
    int c1, syntax;
    if (SYNTAX_FLAGS_COMSTART_FIRST(prev_from_syntax) &&
        ...
        state->comstyle = SYNTAX_FLAGS_COMMENT_STYLE(syntax, prev_from_syntax);
...
static void scan_sexps_forward(struct lisp_parse_state* state, ptrdiff_t from,
                               ptrdiff_t from_byte, ptrdiff_t end,
...
#define INC_FROM                                                               \
    ...
    if (state->instring >= 0) {
        ...
        goto startquoted;
             ...
             (in_2char_comment_start(state, prev_from_syntax, prev_from,
                                     from_byte))) {
        INC_FROM;
    ...
    while (from < end) {
        ...
        if ((from < end) &&
            ...
            INC_FROM;
            prev_from_syntax =
                Smax; /* the syntax has already been "used up". */
            goto atcomment;
        }
        ...
        symstarted:
            while (from < end) {
                if (in_2char_comment_start(state, prev_from_syntax, prev_from,
                                           from_byte)) {
                    INC_FROM;
                    prev_from_syntax =
                        Smax; /* the syntax has already been "used up". */
        ...
        case Scomment:
            ...
            state->incomment =
                (SYNTAX_FLAGS_COMMENT_NESTED(prev_from_syntax) ? 1 : -1);
            state->comstr_start = prev_from;
        atcomment:
        ...
        startincomment:
            ...
            found = forw_comment(
                from, from_byte, end, state->incomment, state->comstyle,
                from == BEGV ? 0 : prev_from_syntax, &out_charpos, &out_bytepos,
                &state->incomment, &prev_from_syntax);
            from = out_charpos;
            from_byte = out_bytepos;
        ...
        case Sstring_fence:
            ...
            curlevel->last = prev_from;
            state->instring =
                (code == Sstring ? (FETCH_CHAR_AS_MULTIBYTE(prev_from_byte))
                                 : ST_STRING_STYLE);
            if (boundary_stop)
                goto done;
        startinstring:
...
endquoted:
    ...
    while (curlevel > levelstart)
        state->levelstarts =
            Fcons(make_fixnum((--curlevel)->last), state->levelstarts);
    state->prev_syntax =
...
/* Convert a (lisp) parse state to the internal form used in
   scan_sexps_forward.  */
static void internalize_parse_state(Lisp_Object external,
                                    struct lisp_parse_state* state) {
    Lisp_Object tem;
    ...
    if (NILP(external)) {
        ...
        state->instring =
            (!NILP(tem) ? (CHARACTERP(tem) ? XFIXNAT(tem) : ST_STRING_STYLE)
                        : -1);

        external = Fcdr(external);
        tem = Fcar(external);
        state->incomment =
            (!NILP(tem) ? (FIXNUMP(tem) ? XFIXNUM(tem) : -1) : 0);
...
(Lisp_Object from, Lisp_Object to, Lisp_Object targetdepth,
 Lisp_Object stopbefore, Lisp_Object oldstate, Lisp_Object commentstop) {
    ...
    scan_sexps_forward(
        &state, XFIXNUM(from), CHAR_TO_BYTE(XFIXNUM(from)), XFIXNUM(to), target,
        !NILP(stopbefore),
        (NILP(commentstop) ? 0 : (EQ(commentstop, Qsyntax_table) ? -1 : 1)));

    SET_PT_BOTH(state.location, state.location_byte);

    return Fcons(
        make_fixnum(state.depth),
        Fcons(
            state.prevlevelstart < 0 ? Qnil : make_fixnum(state.prevlevelstart),
            Fcons(
                state.thislevelstart < 0 ? Qnil
                                         : make_fixnum(state.thislevelstart),
                Fcons(
                    state.instring >= 0 ? (state.instring == ST_STRING_STYLE
                                               ? Qt
                                               : make_fixnum(state.instring))
                                        : Qnil,
                    Fcons(
                        state.incomment < 0
                            ? Qt
...
void syms_of_syntax(void) {
    ...
    DEFVAR_LISP("comment-use-syntax-ppss", Vcomment_use_syntax_ppss,
                doc
:/* Non-nil means `forward-comment' can use `syntax-ppss' internally.  */);
    ...
    Fput(Qscan_error, Qerror_message, build_string("Scan error"));

    DEFVAR_BOOL("parse-sexp-ignore-comments", parse_sexp_ignore_comments, doc:/* Non-nil means `forward-sexp', etc., should treat comments as whitespace.  */);

    DEFVAR_BOOL(
        "parse-sexp-lookup-properties", parse_sexp_lookup_properties,
        doc:/* Non-nil means `forward-sexp', etc., obey `syntax-table'
property. Otherwise, that text property is simply ignored. See the info node
`(elisp)Syntax Properties' for a description of the `syntax-table' property.
*/);
...
In both cases, LIMIT bounds the search. */);
    ...
    DEFVAR_BOOL("comment-end-can-be-escaped", comment_end_can_be_escaped, doc:/* Non-nil means an escaped ender inside a comment doesn't end the comment.  */);
