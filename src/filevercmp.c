...
static idx_t file_prefixlen(char const* s, ptrdiff_t* len) {
...
static int order(char const* s, idx_t pos, idx_t len) {
    if (pos == len)
        return -1;
...
static int _GL_ATTRIBUTE_PURE verrevcmp(const char* s1, idx_t s1_len,
                                        const char* s2, idx_t s2_len) {
    idx_t s1_pos = 0;
    idx_t s2_pos = 0;
    while (s1_pos < s1_len || s2_pos < s2_len) {
        ...
        while (s2_pos < s2_len && s2[s2_pos] == '0')
            s2_pos++;
        while (s1_pos < s1_len && s2_pos < s2_len && c_isdigit(s1[s1_pos]) &&
               c_isdigit(s2[s2_pos])) {
            if (!first_diff)
