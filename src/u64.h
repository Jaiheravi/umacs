...
_GL_U64_INLINE u64 u64hilo(unsigned int hi, unsigned int lo) {
    u64 r;
    r.hi = hi;
    r.lo = lo;
    return r;
...
_GL_U64_INLINE u64 u64lo(unsigned int lo) {
    u64 r;
    r.hi = 0;
    r.lo = lo;
    return r;
...
_GL_U64_INLINE unsigned int u64getlo(u64 x) { return x.lo & _GL_U64_MASK32; }

/* Return a u64 value representing SIZE, where 0 <= SIZE < 2**64.  */
_GL_U64_INLINE u64 u64size(size_t size) {
    u64 r;
    r.hi = size >> 31 >> 1;
...
_GL_U64_INLINE u64 u64not(u64 x) {
    u64 r;
    r.hi = ~x.hi;
    r.lo = ~x.lo;
    return r;
...
/* Return X < Y.  */
_GL_U64_INLINE bool u64lt(u64 x, u64 y) {
    return x.hi < y.hi || (x.hi == y.hi && x.lo < y.lo);
}

/* Return X & Y.  */
_GL_U64_INLINE u64 u64and(u64 x, u64 y) {
    u64 r;
    r.hi = x.hi & y.hi;
    r.lo = x.lo & y.lo;
    return r;
...
/* Return X | Y.  */
_GL_U64_INLINE u64 u64or(u64 x, u64 y) {
    u64 r;
    r.hi = x.hi | y.hi;
    r.lo = x.lo | y.lo;
    return r;
...
/* Return X ^ Y.  */
_GL_U64_INLINE u64 u64xor(u64 x, u64 y) {
    u64 r;
    r.hi = x.hi ^ y.hi;
    r.lo = x.lo ^ y.lo;
    return r;
...
_GL_U64_INLINE u64 u64plus(u64 x, u64 y) {
    u64 r;
...
_GL_U64_INLINE u64 u64shl(u64 x, int n) {
    u64 r;
    if (n < 32) {
...
/* Return X >> N.  */
_GL_U64_INLINE u64 u64shr(u64 x, int n) {
    u64 r;
    if (n < 32) {
        r.hi = x.hi >> n;
...
_GL_U64_INLINE u64 u64bswap(u64 x) {
    ...
    _GL_U64_INLINE u64 u64rol(u64 x, int n) {
