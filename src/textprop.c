...
static AVOID text_read_only(Lisp_Object propval) {
    if (STRINGP(propval))
...
static void modify_text_properties(Lisp_Object buffer, Lisp_Object start,
                                   Lisp_Object end) {
...
static void CHECK_STRING_OR_BUFFER(Lisp_Object x) {
...
validate_interval_range(Lisp_Object object, Lisp_Object* begin,
    ...
    if (BUFFERP(object)) {
        ...
        if (!(BUF_BEGV(b) <= XFIXNUM(*begin) &&
              XFIXNUM(*begin) <= XFIXNUM(*end) && XFIXNUM(*end) <= BUF_ZV(b)))
...
static Lisp_Object validate_plist(Lisp_Object list) {
    if (NILP(list))
        return Qnil;
...
static bool interval_has_all_properties(Lisp_Object plist, INTERVAL i) {
...
static bool interval_has_some_properties(Lisp_Object plist, INTERVAL i) {
...
static bool interval_has_some_properties_list(Lisp_Object list, INTERVAL i) {
    Lisp_Object tail1, tail2, sym;
...
/* Return the value of PROP in property-list PLIST, or Qunbound if it
   has none.  */
static Lisp_Object property_value(Lisp_Object plist, Lisp_Object prop) {
    Lisp_Object value;
...
static void set_properties(Lisp_Object properties, INTERVAL interval,
                           Lisp_Object object) {
    ...
    if (BUFFERP(object)) {
       ...
       or has a different value in PROPERTIES, make an undo record.  */
        for (sym = interval->plist; PLIST_ELT_P(sym, value); sym = XCDR(value))
            if (!EQ(property_value(properties, XCAR(sym)), XCAR(value))) {
                record_property_change(interval->position, LENGTH(interval),
                                       XCAR(sym), XCAR(value), object);
            }
       ...
       make an undo record binding it to nil, so it will be removed.  */
        for (sym = properties; PLIST_ELT_P(sym, value); sym = XCDR(value))
            if (BASE_EQ(property_value(interval->plist, XCAR(sym)), Qunbound)) {
                record_property_change(interval->position, LENGTH(interval),
                                       XCAR(sym), Qnil, object);
            }
    }
...
static bool add_properties(Lisp_Object plist, INTERVAL i, Lisp_Object object,
    ...
    for (tail1 = plist; CONSP(tail1); tail1 = Fcdr(XCDR(tail1))) {
        ...
        for (tail2 = i->plist; CONSP(tail2); tail2 = Fcdr(XCDR(tail2)))
            if (EQ(sym1, XCAR(tail2))) {
                ...
                if (BUFFERP(object)) {
                    record_property_change(i->position, LENGTH(i), sym1,
                                           Fcar(this_cdr), object);
                }
        ...
        if (!found) {
            /* Record this change in the buffer, for undo purposes.  */
            if (BUFFERP(object)) {
                record_property_change(i->position, LENGTH(i), sym1, Qnil,
                                       object);
            }
...
static bool remove_properties(Lisp_Object plist, Lisp_Object list, INTERVAL i,
                              Lisp_Object object) {
    bool changed = false;
    ...
    while (CONSP(tail1)) {
        ...
        while (CONSP(current_plist) && EQ(sym, XCAR(current_plist))) {
            if (BUFFERP(object))
                record_property_change(i->position, LENGTH(i), sym,
                                       XCAR(XCDR(current_plist)), object);
...
/* Return the value of char's property PROP, in OBJECT at POSITION.
   ...
   with OBJECT. */
Lisp_Object get_char_property_and_overlay(Lisp_Object position,
    ...
    if (BUFFERP(object)) {
        ...
        if (!(BUF_BEGV(b) <= pos && pos <= BUF_ZV(b)))
...
(Lisp_Object position, Lisp_Object prop, Lisp_Object object,
 Lisp_Object limit) {
    if (STRINGP(object)) {
        position =
            Fprevious_single_property_change(position, prop, object, limit);
        if (NILP(position)) {
            if (NILP(limit))
        ...
        if (XFIXNUM(position) <= XFIXNUM(limit)) {
            ...
            Lisp_Object initial_value = Fget_char_property(
                make_fixnum(XFIXNUM(position) - (0 <= XFIXNUM(position))), prop,
                object);
            ...
            while (true) {
                ...
                if (XFIXNAT(position) <= XFIXNAT(limit)) {
                    ...
                    Lisp_Object value = Fget_char_property(
                        make_fixnum(XFIXNAT(position) - 1), prop, object);
...
(Lisp_Object position, Lisp_Object object, Lisp_Object limit) {
    ...
    if (EQ(limit, Qt)) {
        ...
        if (!next)
            XSETFASTINT(
                position,
                (STRINGP(object) ? SCHARS(object) : BUF_ZV(XBUFFER(object))));
        else
...
(Lisp_Object position, Lisp_Object prop, Lisp_Object object,
 Lisp_Object limit) {
    ...
    next = next_interval(i);
    while (next && EQ(here_val, textget(next->plist, prop)) &&
           (NILP(limit) || next->position < XFIXNUM(limit)))
        next = next_interval(next);
...
(Lisp_Object position, Lisp_Object prop, Lisp_Object object,
 Lisp_Object limit) {
    ...
    here_val = textget(i->plist, prop);
    previous = previous_interval(i);
    while (previous && EQ(here_val, textget(previous->plist, prop)) &&
           (NILP(limit) ||
            (previous->position + LENGTH(previous) > XFIXNUM(limit))))
        previous = previous_interval(previous);
...
static Lisp_Object add_text_properties_1(Lisp_Object start, Lisp_Object end,
...
retry:
    ...
    if (BUFFERP(object) && first_time) {
       ...
       data changed.  So we restart the interval analysis anew.  */
        if (TOTAL_LENGTH(i) != prev_total_length || i->position != prev_pos) {
            first_time = false;
            goto retry;
        }
    }
...
(Lisp_Object start, Lisp_Object end, Lisp_Object property, Lisp_Object value,
 Lisp_Object object) {
...
into it.  */)
(Lisp_Object start, Lisp_Object end, Lisp_Object face, Lisp_Object append,
 Lisp_Object object) {
    AUTO_LIST2(properties, Qface, face);
    add_text_properties_1(
        start, end, properties, object,
        (NILP(append) ? TEXT_PROPERTY_PREPEND : TEXT_PROPERTY_APPEND), false);
    return Qnil;
}
...
Lisp_Object set_text_properties(Lisp_Object start, Lisp_Object end,
    ...
    if (BUFFERP(object) && XBUFFER(object) != current_buffer) {
        ...
        set_buffer_internal(XBUFFER(object));
        return unbind_to(count,
                         set_text_properties(start, end, properties, object,
                                             coherent_change_p));
    }
...
void set_text_properties_1(Lisp_Object start, Lisp_Object end,
                           Lisp_Object properties, Lisp_Object object,
                           INTERVAL i) {
    /* Ensure we run the modification hooks for the right buffer,
       without switching buffers twice (bug 36190).  FIXME: Switching
...
(Lisp_Object start, Lisp_Object end, Lisp_Object properties,
 Lisp_Object object) {
    ...
    if (BUFFERP(object) && XBUFFER(object) != current_buffer) {
        ...
        set_buffer_internal(XBUFFER(object));
        return unbind_to(
            count, Fremove_text_properties(start, end, properties, object));
    }
...
(Lisp_Object start, Lisp_Object end, Lisp_Object list_of_properties,
 Lisp_Object object) {
    ...
    /* We are at the beginning of an interval, with len to scan.
       ...
       Call modify_text_properties before calling remove_properties if
       !MODIFIED, and call signal_after_change before returning if MODIFIED. */
    for (;;) {
        eassert(i != 0);
        ...
        if (LENGTH(i) >= len) {
            ...
            else { /* i has the properties, and goes past the change limit.  */
                unchanged = i;
                i = split_interval_left(i, len);
                copy_properties(unchanged, i);
                if (!modified && BUFFERP(object))
...
int text_property_stickiness(Lisp_Object prop, Lisp_Object pos,
                             Lisp_Object buffer) {
    ...
    if (ignore_previous_character || (CONSP(defalt) && !NILP(XCDR(defalt))))
        ...
        if (!NILP(CONSP(rear_non_sticky) ? Fmemq(prop, rear_non_sticky)
                                         : rear_non_sticky))
    ...
    if (EQ(front_sticky, Qt) ||
        (CONSP(front_sticky) && !NILP(Fmemq(prop, front_sticky))))
...
Lisp_Object copy_text_properties(Lisp_Object start, Lisp_Object end,
                                 Lisp_Object src, Lisp_Object pos,
                                 Lisp_Object dest, Lisp_Object prop) {
...
Lisp_Object text_property_list(Lisp_Object object, Lisp_Object start,
                               Lisp_Object end, Lisp_Object prop) {
    struct interval* i;
    Lisp_Object result;
    ...
    if (i) {
        ...
        while (s < e) {
            ...
            if (!NILP(plist))
                result = Fcons(
                    list3(make_fixnum(s), make_fixnum(s + len), plist), result);
...
void add_text_properties_from_list(Lisp_Object object, Lisp_Object list,
                                   Lisp_Object delta) {
...
Lisp_Object extend_property_ranges(Lisp_Object list, Lisp_Object old_end,
                                   Lisp_Object new_end) {
    ...
    for (; CONSP(list); prev = list, list = XCDR(list)) {
        ...
        else if ((end == XFIXNUM(old_end) && end != max) || end > max) {
...
static void call_mod_hooks(Lisp_Object list, Lisp_Object start,
                           Lisp_Object end) {
    while (!NILP(list)) {
...
void verify_interval_modification(struct buffer* buf, ptrdiff_t start,
                                  ptrdiff_t end) {
...
void report_interval_modification(Lisp_Object start, Lisp_Object end) {
...
character that does not have its own value for that property.  */);
    ...
    DEFVAR_LISP(
        "char-property-alias-alist", Vchar_property_alias_alist, doc:
...
returned. */);
    ...
    DEFVAR_LISP("inhibit-point-motion-hooks", Vinhibit_point_motion_hooks, doc:
                    /* If non-nil, don't run `point-left' and `point-entered'
    text properties. This also inhibits the use of the `intangible' text
    property.  */);
