         ...
         : (idx += (charset)->char_index_offset,                               \
            (((charset)->code_space[0] + (idx) % (charset)->code_space[2]) |   \
             ...
             (((charset)->code_space[8] +                                      \
              ...
              << 16) |                                                         \
             (((charset)->code_space[12] +                                     \
               ((idx) / (charset)->code_space[11]))                            \
              << 24))))
...
static void load_charset_map(struct charset* charset,
                             struct charset_map_entries* entries, int n_entries,
                             int control_flag) {
...
static unsigned read_hex(FILE* fp, int lookahead, int* terminator,
                         bool* overflow) {
...
static void load_charset_map_from_file(struct charset* charset,
                                       Lisp_Object mapfile, int control_flag) {
...
static void load_charset_map_from_vector(struct charset* charset,
                                         Lisp_Object vec, int control_flag) {
    unsigned min_code = CHARSET_MIN_CODE(charset);
    unsigned max_code = CHARSET_MAX_CODE(charset);
    struct charset_map_entries *head, *entries;
    int n_entries;
...
static void load_charset(struct charset* charset, int control_flag) {
    Lisp_Object map;
...
static void map_charset_for_dump(void (*c_function)(Lisp_Object, Lisp_Object),
...
void map_charset_chars(void (*c_function)(Lisp_Object, Lisp_Object),
                       Lisp_Object function, Lisp_Object arg,
...
(ptrdiff_t nargs, Lisp_Object* args) {
         ...
         (charset.code_space[2] == 256 &&
          (charset.dimension == 2 ||
           (charset.code_space[6] == 256 &&
            (charset.dimension == 3 || charset.code_space[10] == 256)))));

    if (!charset.code_linear_p) {
        charset.code_space_mask = xzalloc(256);
        for (i = 0; i < 4; i++)
            for (j = charset.code_space[i * 4];
                 j <= charset.code_space[i * 4 + 1]; j++)
    ...
    if (!NILP(args[charset_arg_code_offset])) {
        ...
        if (MAX_CHAR - charset.code_offset < i)
            error("Unsupported max char: %d + %ud > MAX_CHAR (%d)", i,
                  charset.max_code, MAX_CHAR);
    ...
    if (new_definition_p) {
        ...
        if (charset.supplementary_p)
            ...
            if (EQ(tail, Vcharset_ordered_list))
                ...
                Vcharset_ordered_list =
                    nconc2(Vcharset_ordered_list, list1i(id));
            else {
...
static int define_charset_internal(Lisp_Object name, int dimension,
                                   const char* code_space_chars,
                                   unsigned min_code, unsigned max_code,
                                   int iso_final, int iso_revision,
...
DEFUN("define-charset-alias", Fdefine_charset_alias, Sdefine_charset_alias, 2,
      2, 0, doc:/* Define ALIAS as an alias for charset CHARSET.  */)
...
static bool check_iso_charset_parameter(Lisp_Object dimension,
                                        Lisp_Object chars,
...
(Lisp_Object dimension, Lisp_Object chars) {
    bool chars_flag =
        check_iso_charset_parameter(dimension, chars, make_fixnum('0'));
...
int string_xstring_p(Lisp_Object string) {
...
static void find_charsets_in_text(const unsigned char* ptr, ptrdiff_t nchars,
...
(Lisp_Object str, Lisp_Object table) {
    ...
    find_charsets_in_text(SDATA(str), SCHARS(str), SBYTES(str), charsets, table,
...
/* Return a unified character code for C (>= 0x110000).  VAL is a
   ...
   charset symbol.  */
static int maybe_unify_char(int c, Lisp_Object val) {
    struct charset* charset;
...
int decode_char(struct charset* charset, unsigned int code) {
    int c, char_index;
...
unsigned encode_char(struct charset* charset, int c) {
    unsigned code;
    enum charset_method method = CHARSET_METHOD(charset);
...
is specified.  */)
(Lisp_Object charset, Lisp_Object code1, Lisp_Object code2, Lisp_Object code3,
 Lisp_Object code4) {
    int id, dimension;
    struct charset* charsetp;
    unsigned code;
    ...
    if (NILP(code1))
        code =
            (CHARSET_ASCII_COMPATIBLE_P(charsetp) ? 0
                                                  : CHARSET_MIN_CODE(charsetp));
    else {
        CHECK_FIXNAT(code1);
...
struct charset* char_charset(int c, Lisp_Object charset_list,
    ...
    while (CONSP(charset_list)) {
        ...
        if (!maybe_null && c <= MAX_UNICODE_CHAR &&
...
(Lisp_Object dimension, Lisp_Object chars, Lisp_Object final_char) {
    bool chars_flag = check_iso_charset_parameter(dimension, chars, final_char);
    int id =
        ISO_CHARSET_TABLE(XFIXNUM(dimension), chars_flag, XFIXNAT(final_char));
...
static int charset_compare(const void* d1, const void* d2) {
...
(Lisp_Object charsets) {
    ...
    for (done = 0, tail = Vcharset_ordered_list, i = 0; done < n && CONSP(tail);
         tail = XCDR(tail), i++) {
        elt = XCAR(tail);
        id = XFIXNAT(elt);
...
the value may be a list of mnemonics.  */);
    ...
    charset_iso_8859_1 = define_charset_internal(
        Qiso_8859_1, 1, "\x00\xFF\0\0\0\0\0", 0, 255, -1, -1, -1, 1, 0, 0);
