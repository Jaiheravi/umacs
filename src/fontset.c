...
static bool fontset_id_valid_p(int id) {
...
static void set_fontset_id(Lisp_Object fontset, Lisp_Object id) {
...
static void set_fontset_name(Lisp_Object fontset, Lisp_Object name) {
    set_char_table_extras(fontset, 1, name);
}

#define FONTSET_ASCII(fontset) XCHAR_TABLE(fontset)->extras[2]
static void set_fontset_ascii(Lisp_Object fontset, Lisp_Object ascii) {
...
static void set_fontset_base(Lisp_Object fontset, Lisp_Object base) {
    set_char_table_extras(fontset, 3, base);
}

#define FONTSET_FRAME(fontset) XCHAR_TABLE(fontset)->extras[4]
static void set_fontset_frame(Lisp_Object fontset, Lisp_Object frame) {
    set_char_table_extras(fontset, 4, frame);
}

#define FONTSET_NOFONT_FACE(fontset) XCHAR_TABLE(fontset)->extras[5]
static void set_fontset_nofont_face(Lisp_Object fontset, Lisp_Object face) {
    set_char_table_extras(fontset, 5, face);
}

#define FONTSET_DEFAULT(fontset) XCHAR_TABLE(fontset)->extras[6]
static void set_fontset_default(Lisp_Object fontset, Lisp_Object def) {
...
static void set_fontset_fallback(Lisp_Object fontset, Lisp_Object fallback) {
...
static Lisp_Object font_def_new(Lisp_Object font_spec, Lisp_Object encoding,
...
#define RFONT_DEF_SET_OBJECT(rfont_def, object) ASET(rfont_def, 2, object)
...
#define FONTSET_REF(fontset, c)                                                \
    (EQ(fontset, Vdefault_fontset) ? CHAR_TABLE_REF(fontset, c)                \
                                   : fontset_ref(fontset, c))

static Lisp_Object fontset_ref(Lisp_Object fontset, int c) {
    Lisp_Object elt;
...
#define FONTSET_ADD(fontset, range, elt, add)                                  \
    (NILP(add)                                                                 \
         ? (NILP(range) ? set_fontset_fallback(fontset, make_vector(1, elt))   \
                        : (void)Fset_char_table_range(fontset, range,          \
                                                      make_vector(1, elt)))    \
         : fontset_add(fontset, range, elt, add))

static void fontset_add(Lisp_Object fontset, Lisp_Object range, Lisp_Object elt,
                        Lisp_Object add) {
    Lisp_Object args[2];
    ...
    if (CONSP(range)) {
        ...
        do {
            ...
            char_table_set_range(
                fontset, from, to1,
                (NILP(args[idx]) ? args[1 - idx] : CALLMANY(Fvconcat, args)));
            from = to1 + 1;
        } while (from <= to);
    }
    else {
...
static int fontset_compare_rfontdef(const void* val1, const void* val2) {
...
static void reorder_font_vector(Lisp_Object font_group, struct font* font) {
        ...
        if (!NILP(otf_spec))
            ...
            if (!NILP(encoding)) {
                ...
                for (tail = Vcharset_ordered_list;
                    ...
                    if (EQ(encoding, XCAR(tail)))
                        break;
                             ...
                             min(INT_MAX, MOST_POSITIVE_FIXNUM) - 0x100)
                        score += 0x100;
            }
            else {
               ...
               larger.  */
                ...
                if (!NILP(lang) && !EQ(lang, Vcurrent_iso639_language) &&
...
static Lisp_Object fontset_get_font_group(Lisp_Object fontset, int c) {
...
static Lisp_Object fontset_find_font(Lisp_Object fontset, int c,
                                     struct face* face, int charset_id,
                                     bool fallback) {
    Lisp_Object vec, font_group;
    int i, charset_matched = 0, found_index;
    struct frame* f =
        (FRAMEP(FONTSET_FRAME(fontset)) ? XFRAME(FONTSET_FRAME(fontset))
    ...
    for (i = 0; i < ASIZE(vec); i++) {
       ...
       and supporting C. */
        ...
        font_entity =
            font_find_for_lface(f, face->lface, FONT_DEF_SPEC(font_def), c);
        if (!NILP(font_entity)) {
...
static Lisp_Object fontset_font(Lisp_Object fontset, int c, struct face* face,
                                int id) {
    Lisp_Object rfont_def;
    ...
    if (!EQ(base_fontset, Vdefault_fontset) && !EQ(default_rfont_def, Qt)) {
...
static Lisp_Object make_fontset(Lisp_Object frame, Lisp_Object name,
                                Lisp_Object base) {
    Lisp_Object fontset;
    ...
    /* Find a free slot in Vfontset_table.  Usually, next_fontset_id is
       ...
       id.  */
    while (!NILP(AREF(Vfontset_table, id)))
        id++;
...
Lisp_Object fontset_name(int id) {
    Lisp_Object fontset;
...
Lisp_Object fontset_ascii(int id) {
...
void free_face_fontset(struct frame* f, struct face* face) {
    Lisp_Object fontset;
...
int face_for_char(struct frame* f, struct face* face, int c, ptrdiff_t pos,
                  Lisp_Object object) {
...
Lisp_Object font_for_char(struct face* face, int c, ptrdiff_t pos,
                          Lisp_Object object) {
    Lisp_Object fontset, rfont_def, charset;
    int id;
    ...
    rfont_def = fontset_font(fontset, c, face, id);
    return (VECTORP(rfont_def) ? RFONT_DEF_OBJECT(rfont_def) : Qnil);
}
...
int make_fontset_for_ascii_face(struct frame* f, int base_fontset_id,
                                struct face* face) {
...
static Lisp_Object fontset_pattern_regexp(Lisp_Object pattern) {
    if (!strchr(SSDATA(pattern), '*') && !strchr(SSDATA(pattern), '?'))
    ...
    if (!CONSP(Vcached_fontset_data) ||
       ...
       expression matching.  */
        ptrdiff_t regexsize =
            (SBYTES(pattern) + (ndashes < 14 ? 2 : 5) * nstars + 2 * nescs + 3);
        USE_SAFE_ALLOCA;
        ...
        *p1++ = 0;
        ...
        Vcached_fontset_data =
            Fcons(build_string(SSDATA(pattern)), build_string((char*)regex));
        SAFE_FREE();
    }
...
int fs_query_fontset(Lisp_Object name, int name_pattern) {
    Lisp_Object tem;
    int i;
    ...
    for (i = 0; i < ASIZE(Vfontset_table); i++) {
        ...
        if (NILP(fontset) || !BASE_FONTSET_P(fontset))
            continue;
...
Lisp_Object list_fontsets(struct frame* f, Lisp_Object pattern, int size) {
    ...
    for (id = 0; id < ASIZE(Vfontset_table); id++) {
        ...
        if (STRINGP(regexp) ? (fast_string_match(regexp, name) < 0)
...
static void free_realized_fontsets(Lisp_Object base) {
    int id;
...
static Lisp_Object check_fontset_name(Lisp_Object name, Lisp_Object* frame) {
    int id;
...
static void accumulate_script_ranges(Lisp_Object arg, Lisp_Object range,
                                     Lisp_Object val) {
    if (EQ(XCAR(arg), val)) {
        if (CONSP(range))
...
static void set_fontset_font(Lisp_Object arg, Lisp_Object range) {
 ...
 Lisp_Object frame, Lisp_Object add) {
    ...
    if (charset) {
        ...
        map_charset_chars(set_fontset_font, Qnil, arg, charset,
                          CHARSET_MIN_CODE(charset), CHARSET_MAX_CODE(charset));
...
(Lisp_Object name, Lisp_Object fontlist) {
    ...
    if (id < 0) {
        ...
        Vfontset_alias_alist =
            Fcons(Fcons(name, SYMBOL_NAME(short_name)), Vfontset_alias_alist);
...
int fontset_from_font(Lisp_Object font_object) {
    ...
    fontset = make_fontset(Qnil, name, Qnil);
    Vfontset_alias_alist =
        Fcons(Fcons(name, SYMBOL_NAME(alias)), Vfontset_alias_alist);
...
static void update_auto_fontset_alist(Lisp_Object font_object,
                                      Lisp_Object fontset) {
...
(Lisp_Object fontset, Lisp_Object frame) {
    ...
    for (i = j = 0; i < ASIZE(Vfontset_table); i++) {
        ...
        if (!NILP(elt) && EQ(FONTSET_BASE(elt), fontset) &&
                ...
                for (j = 0; j < ASIZE(elt); j++) {
                    ...
                    if (FIXNUMP(repertory)) {
                        struct charset* charset =
                            CHARSET_FROM_ID(XFIXNUM(repertory));
...
Lisp_Object dump_fontset(Lisp_Object fontset) {
    ...
    if (BASE_FONTSET_P(fontset)) {
        ...
        if (FRAMEP(frame)) {
            ...
            if (FRAME_LIVE_P(f))
                ...
                ASET(vec, 1, Fcons(FONTSET_NAME(FONTSET_BASE(fontset)), Qnil));
        }
        if (!NILP(FONTSET_DEFAULT(fontset)))
...
void syms_of_fontset(void) {
    ...
    set_fontset_name(Vdefault_fontset,
...
alternate fontnames (if any) are tried instead.  */);
    ...
    Vfontset_alias_alist = list1(
        Fcons(FONTSET_NAME(Vdefault_fontset), build_string("fontset-default")));
