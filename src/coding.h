...
#define CODING_ID_NAME(id) HASH_KEY(XHASH_TABLE(Vcoding_system_hash_table), id)
...
#define CODING_SYSTEM_ID(coding_system_symbol)                                 \
    hash_find(XHASH_TABLE(Vcoding_system_hash_table), coding_system_symbol)
...
#define CODING_MAY_REQUIRE_DECODING(coding)                                    \
    (CODING_REQUIRE_DECODING(coding) || CODING_REQUIRE_DETECTION(coding))
...
#define SJIS_TO_JIS(code)                                                      \
    do {                                                                       \
        ...
        s1 = (code) >> 8, s2 = (code) & 0xFF;                                  \
                                                                               \
        if (s2 >= 0x9F)                                                        \
            (j1 = s1 * 2 - (s1 >= 0xE0 ? 0x160 : 0xE0), j2 = s2 - 0x7E);       \
        else                                                                   \
...
#define JIS_TO_SJIS(code)                                                      \
    do {                                                                       \
        ...
        if (j1 & 1)                                                            \
            ...
            (s1 = j1 / 2 + ((j1 < 0x5F) ? 0x70 : 0xB0), s2 = j2 + 0x7E);       \
        (code) = (s1 << 8) | s2;                                               \
    } while (false)
...
#define UTF_16_HIGH_SURROGATE_P(val) (((val) & 0xFC00) == 0xD800)

/* Return true if VAL is a low surrogate.  VAL must be a 16-bit code
   unit.  */

#define UTF_16_LOW_SURROGATE_P(val) (((val) & 0xFC00) == 0xDC00)
...
extern Lisp_Object code_convert_string(Lisp_Object, Lisp_Object, Lisp_Object,
                                       bool, bool, bool);
extern Lisp_Object code_convert_string_norecord(Lisp_Object, Lisp_Object, bool);
...
extern void decode_coding_object(struct coding_system*, Lisp_Object, ptrdiff_t,
                                 ptrdiff_t, ptrdiff_t, ptrdiff_t, Lisp_Object);
extern void encode_coding_object(struct coding_system*, Lisp_Object, ptrdiff_t,
                                 ptrdiff_t, ptrdiff_t, ptrdiff_t, Lisp_Object);
...
INLINE int surrogates_to_codepoint(int low, int high) {
...
INLINE Lisp_Object build_string_from_utf8(const char* str) {
