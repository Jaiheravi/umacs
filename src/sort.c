...
static void reverse_slice(Lisp_Object* lo, Lisp_Object* hi) {
    --hi;
    while (lo < hi) {
        Lisp_Object t = *lo;
        *lo = *hi;
...
static inline void sortslice_copy(sortslice* s1, ptrdiff_t i, sortslice* s2,
                                  ptrdiff_t j) {
    s1->keys[i] = s2->keys[j];
    if (s1->values != NULL)
...
static inline void sortslice_copy_incr(sortslice* dst, sortslice* src) {
    *dst->keys++ = *src->keys++;
    if (dst->values != NULL)
        *dst->values++ = *src->values++;
}

static inline void sortslice_copy_decr(sortslice* dst, sortslice* src) {
    *dst->keys-- = *src->keys--;
    if (dst->values != NULL)
        *dst->values-- = *src->values--;
}
...
static inline void sortslice_memcpy(sortslice* s1, ptrdiff_t i, sortslice* s2,
...
static inline void sortslice_memmove(sortslice* s1, ptrdiff_t i, sortslice* s2,
                                     ptrdiff_t j, ptrdiff_t n) {
...
static inline void sortslice_advance(sortslice* slice, ptrdiff_t n) {
    slice->keys += n;
...
static bool order_pred_lisp(merge_state* ms, Lisp_Object a, Lisp_Object b) {
    return !NILP(calln(ms->predicate, a, b));
}

static bool order_pred_valuelt(merge_state* ms, Lisp_Object a, Lisp_Object b) {
    return !NILP(Fvaluelt(a, b));
}

/* Return true iff A < B according to the order predicate.  */
static inline bool inorder(merge_state* ms, Lisp_Object a, Lisp_Object b) {
...
static void binarysort(merge_state* ms, sortslice lo, const Lisp_Object* hi,
                       Lisp_Object* start) {
...
static ptrdiff_t gallop_left(merge_state* ms, const Lisp_Object key,
                             Lisp_Object* a, const ptrdiff_t n,
                             const ptrdiff_t hint) {
...
static ptrdiff_t gallop_right(merge_state* ms, const Lisp_Object key,
                              Lisp_Object* a, const ptrdiff_t n,
                              const ptrdiff_t hint) {
    eassume(a && n > 0 && hint >= 0 && hint < n);
...
static void merge_init(merge_state* ms, const ptrdiff_t list_size,
                       Lisp_Object* allocated_keys, sortslice* lo,
                       Lisp_Object predicate) {
    eassume(ms != NULL);
...
static void merge_markmem(void* arg) {
    merge_state* ms = arg;
    eassume(ms != NULL);
    ...
    if (ms->reloc.size != NULL && *ms->reloc.size > 0) {
        Lisp_Object* src = (ms->reloc.src->values ? ms->reloc.src->values
                                                  : ms->reloc.src->keys);
        eassume(src != NULL);
...
static void cleanup_mem(void* arg) {
    merge_state* ms = arg;
    eassume(ms != NULL);
    ...
    if (ms->reloc.order != 0 && *ms->reloc.size > 0) {
        Lisp_Object* src = (ms->reloc.src->values ? ms->reloc.src->values
                                                  : ms->reloc.src->keys);
        Lisp_Object* dst = (ms->reloc.dst->values ? ms->reloc.dst->values
                                                  : ms->reloc.dst->keys);
...
static void merge_getmem(merge_state* ms, const ptrdiff_t need) {
    eassume(ms != NULL);
...
static inline void needmem(merge_state* ms, ptrdiff_t na) {
    if (na > ms->alloced)
        merge_getmem(ms, na);
}
...
static void merge_lo(merge_state* ms, sortslice ssa, ptrdiff_t na,
...
static void merge_hi(merge_state* ms, sortslice ssa, ptrdiff_t na,
                     sortslice ssb, ptrdiff_t nb) {
    eassume(ms && ssa.keys && ssb.keys && na > 0 && nb > 0);
    eassume(ssa.keys + na == ssb.keys);
    needmem(ms, nb);
...
static void merge_at(merge_state* ms, const ptrdiff_t i) {
    eassume(ms != NULL);
    eassume(ms->n >= 2);
    eassume(i >= 0);
...
static int powerloop(const ptrdiff_t s1, const ptrdiff_t n1, const ptrdiff_t n2,
                     const ptrdiff_t n) {
    eassume(s1 >= 0);
    eassume(n1 > 0 && n2 > 0);
    eassume(s1 + n1 + n2 <= n);
    ...
    for (;;) {
        ++result;
        if (a >= n) { /* Both quotient bits are now 1.  */
            eassume(b >= a);
            a -= n;
            b -= n;
        }
        else if (b >= n) { /* a/n bit is 0 and b/n bit is 1.  */
            break;
...
static void found_new_run(merge_state* ms, const ptrdiff_t n2) {
    eassume(ms != NULL);
    if (ms->n) {
        eassume(ms->n > 0);
...
static void merge_force_collapse(merge_state* ms) {
    struct stretch* p = ms->pending;
...
static ptrdiff_t merge_compute_minrun(ptrdiff_t n) {
...
static void reverse_sortslice(sortslice* s, const ptrdiff_t n) {
    reverse_slice(s->keys, &s->keys[n]);
    if (s->values != NULL)
        reverse_slice(s->values, &s->values[n]);
}

static Lisp_Object resolve_fun(Lisp_Object fun) {
    if (SYMBOLP(fun)) {
...
/* Sort the array SEQ with LENGTH elements in the order determined by
   ...
   optionally reversed.  */
void tim_sort(Lisp_Object predicate, Lisp_Object keyfunc, Lisp_Object* seq,
    ...
    do {
        ...
        ptrdiff_t n =
            count_run(&ms, lo.keys, lo.keys + nremaining, &descending);
        if (descending)
