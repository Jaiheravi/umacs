...
void sys_mutex_init(sys_mutex_t* m) { *m = 0; }

void sys_mutex_lock(sys_mutex_t* m) {}

void sys_mutex_unlock(sys_mutex_t* m) {}

void sys_cond_init(sys_cond_t* c) { *c = 0; }

void sys_cond_wait(sys_cond_t* c, sys_mutex_t* m) {}

void sys_cond_signal(sys_cond_t* c) {}

void sys_cond_broadcast(sys_cond_t* c) {}

void sys_cond_destroy(sys_cond_t* c) {}

sys_thread_t sys_thread_self(void) { return 0; }

bool sys_thread_equal(sys_thread_t t, sys_thread_t u) { return t == u; }
void sys_thread_set_name(const char* name) {}

bool sys_thread_create(sys_thread_t* t, thread_creation_function* func,
                       void* datum) {
    return false;
}
...
void sys_mutex_init(sys_mutex_t* sys_mutex) {
...
void sys_mutex_lock(sys_mutex_t* sys_mutex) {
    pthread_mutex_t* mutex = SYSTHREAD_ALIGN_PTR(pthread_mutex_t, sys_mutex);
    int error = pthread_mutex_lock(mutex);
    eassert(error == 0);
}

void sys_mutex_unlock(sys_mutex_t* sys_mutex) {
    pthread_mutex_t* mutex = SYSTHREAD_ALIGN_PTR(pthread_mutex_t, sys_mutex);
    int error = pthread_mutex_unlock(mutex);
    eassert(error == 0);
}

void sys_cond_init(sys_cond_t* sys_cond) {
...
void sys_cond_wait(sys_cond_t* sys_cond, sys_mutex_t* sys_mutex) {
    pthread_cond_t* cond = SYSTHREAD_ALIGN_PTR(pthread_cond_t, sys_cond);
    pthread_mutex_t* mutex = SYSTHREAD_ALIGN_PTR(pthread_mutex_t, sys_mutex);
...
void sys_cond_signal(sys_cond_t* sys_cond) {
    pthread_cond_t* cond = SYSTHREAD_ALIGN_PTR(pthread_cond_t, sys_cond);
    int error = pthread_cond_signal(cond);
    eassert(error == 0);
}

void sys_cond_broadcast(sys_cond_t* sys_cond) {
    pthread_cond_t* cond = SYSTHREAD_ALIGN_PTR(pthread_cond_t, sys_cond);
...
void sys_cond_destroy(sys_cond_t* sys_cond) {
    pthread_cond_t* cond = SYSTHREAD_ALIGN_PTR(pthread_cond_t, sys_cond);
    int error = pthread_cond_destroy(cond);
    eassert(error == 0);
}

sys_thread_t sys_thread_self(void) { return pthread_self(); }

bool sys_thread_equal(sys_thread_t t, sys_thread_t u) {
...
bool sys_thread_create(sys_thread_t* thread_ptr, thread_creation_function* func,
                       void* arg) {
    pthread_attr_t attr;
    bool result = false;
...
void sys_thread_yield(void) { sched_yield(); }
...
void sys_cond_wait(sys_cond_t* cond, sys_mutex_t* mutex) {
    ...
    cond->wait_count--;
    last_thread_waiting =
        wait_result == WAIT_OBJECT_0 + CONDV_BROADCAST && cond->wait_count == 0;
    LeaveCriticalSection((LPCRITICAL_SECTION)&cond->wait_count_lock);
...
typedef HRESULT(WINAPI* SetThreadDescription_Proc)(HANDLE hThread,
                                                   PCWSTR lpThreadDescription);
extern SetThreadDescription_Proc set_thread_description;

/* Set the name of the thread identified by its thread ID.  */
static void w32_set_thread_name(DWORD thread_id, const char* name) {
    if (!name || !*name)
        return;
...
static void ALIGN_STACK w32_beginthread_wrapper(void* arg) {
...
void sys_thread_yield(void) { Sleep(0); }
