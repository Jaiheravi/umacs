...
#if !defined HAVE_ANDROID || defined ANDROID_STUBIFY || (__ANDROID_API__ < 9)
...
static int file_get_char(file_stream stream) {
    int c;
    char byte;
    ssize_t rc;
...
static void init_source(source_t* src, Lisp_Object readcharfun) {
...
static int source_string_get(source_t* src) {
    ...
    int c;
    if (src->multibyte)
        c = fetch_string_char_advance_no_check(s, &read_from_string_index,
...
static void source_string_unget(source_t* src, int c) {
    read_from_string_index--;
    read_from_string_index_byte =
        string_char_to_byte(src->object, read_from_string_index);
}
...
static void source_file_unget(source_t* src, int c) { unread_char = c; }
...
static inline int readchar(source_t* src) {
    readchar_offset++;
    return src->get(src);
}

/* Unread C from (to?) SRC.  Only a single char can be unread at a time.  */
static inline void unreadchar(source_t* src, int c) {
    readchar_offset--;
...
static bool from_file_p(source_t* source) {
    return source->get == source_file_get;
}

static bool from_buffer_p(source_t* source) {
    return source->get == source_buffer_get;
}

static void skip_dyn_bytes(source_t* source, ptrdiff_t n) {
    ...
    else { /* We're not reading directly from a file.  In that case, it's
          difficult to reliably count bytes, since these are usually meant for
          the file's encoding, whereas we're now typically in the internal
          encoding. But luckily, skip_dyn_bytes is used to skip over a single
...
static void skip_dyn_eof(source_t* source) {
    if (from_file_p(source)) {
        block_input(); /* FIXME: Not sure if it's needed.  */
...
static void unreadbyte_from_file(unsigned char c) {
...
static AVOID invalid_syntax_lisp(Lisp_Object s, source_t* source) {
...
static AVOID invalid_syntax(const char* s, source_t* source) {
...
static lexical_cookie_t lisp_file_lexical_cookie(Lisp_Object readcharfun) {
    source_t source;
...
static int safe_to_load_version(Lisp_Object file, lread_fd fd) {
    struct stat st;
    char buf[512];
    int nbytes, i;
    int version = 1;
    ...
    if (nbytes > 0) {
       ...
       with NUL bytes following it, but note the version.  */
        ...
        if (i >= nbytes ||
            fast_c_string_match_ignore_case(Vbytecomp_version_regexp, buf + i,
                                            nbytes - i) < 0)
            version = 0;
    }
...
static void record_load_unwind(Lisp_Object old) { Vloads_in_progress = old; }

/* This handler function is used via internal_condition_case_1.  */

static Lisp_Object load_error_handler(Lisp_Object data) { return Qnil; }

static void load_warn_unescaped_character_literals(Lisp_Object file) {
    ...
    FOR_EACH_TAIL(suffixes) {
        ...
        FOR_EACH_TAIL(exts) {
            ...
            if (SCHARS(ext) > 0 &&
                ...
                !NILP(Fmember(ext, Fsymbol_value(Qjka_compr_load_suffixes))))
                continue;
#endif
...
bool suffix_p(Lisp_Object string, const char* suffix) {
...
static void close_infile_unwind(void* arg) {
...
static Lisp_Object compute_found_effective(Lisp_Object found) {
...
static void loadhist_initialize(Lisp_Object filename) {
...
static void close_file_unwind_android_fd(void* ptr) {
...
static Lisp_Object get_lexical_binding(Lisp_Object stream, Lisp_Object from) {
...
(Lisp_Object file, Lisp_Object noerror, Lisp_Object nomessage,
 Lisp_Object nosuffix, Lisp_Object must_suffix) {
    ...
    if (!NILP(handler))
        return calln(handler, Qload, file, noerror, nomessage, nosuffix,
                     must_suffix);
    ...
    if (SCHARS(file) == 0) {
        ...
        if (FUNCTIONP(Vload_path_filter_function))
            load_path =
                calln(Vload_path_filter_function, load_path, file, suffixes);

#if !defined USE_ANDROID_ASSETS
        fd = openp(load_path, file, suffixes, &found, Qnil, load_prefer_newer,
                   no_native, NULL);
#else
        asset = NULL;
        rc = openp(load_path, file, suffixes, &found, Qnil, load_prefer_newer,
                   no_native, &asset);
        fd.fd = rc;
        fd.asset = asset;
    ...
    Lisp_Object found_eff =
        is_native_elisp ? compute_found_effective(found) : found;

    hist_file_name =
        (!NILP(Vpurify_flag) ? concat2(Ffile_name_directory(file),
    ...
    if (is_elc
       ...
       treat it as not byte-compiled.  */
        || (lread_fd_p && (version = safe_to_load_version(file, fd)) > 1))
       ...
       remote and have no handler!  */
    {
        if (!lread_fd_cmp(-2)) {
            ...
            struct timespec epoch_timespec = {(time_t)0,
                                              0}; /* 1970-01-01T00:00 UTC */
                ...
                if (result == 0 &&
                    ...
                    if (!NILP(nomessage) &&
                       ...
                       the epoch.  See bug #58224.  */
                        ...
                        msg_file =
                            Fsubstring(found, make_fixnum(0), make_fixnum(-1));
                        message_with_string(
    ...
    if (NILP(nomessage) || force_load_messages) {
        if (is_module)
            ...
            message_with_string("Loading %s (native-compiled elisp)...", file,
                                1);
        else if (!compiled)
            message_with_string("Loading %s (source)...", file, 1);
        else if (newer)
            message_with_string(
                "Loading %s (compiled; note, source file is newer)...", file,
                1);
    ...
    if (is_module) {
        ...
        if (!version || version >= 22)
            readevalloop(Qget_file_char, &input, hist_file_name, 0, Qnil, Qnil,
                         Qnil);
        else {
            /* We can't handle a file which was compiled with
               byte-compile-dynamic by older version of Emacs.  */
            specbind(Qload_force_doc_strings, Qt);
            readevalloop(Qget_emacs_mule_file_char, &input, hist_file_name, 0,
                         Qnil, Qnil, Qnil);
        }
    }
    unbind_to(count, Qnil);
    ...
    if (!noninteractive && (NILP(nomessage) || force_load_messages)) {
        if (is_module)
            ...
            message_with_string("Loading %s (native-compiled elisp)...done",
                                file, 1);
        else if (!compiled)
...
Lisp_Object save_match_data_load(Lisp_Object file, Lisp_Object noerror,
...
#ifdef HAVE_NATIVE_COMP
static bool maybe_swap_for_eln1(Lisp_Object src_name, Lisp_Object eln_name,
                                Lisp_Object* filename, int* fd,
        ...
        if (sys_fstat(eln_fd, &eln_st) || S_ISDIR(eln_st.st_mode))
            ...
            if (timespec_cmp(eln_mtime, mtime) >= 0) {
                ...
                *filename = eln_name;
                ...
                Fputhash(Ffile_name_nondirectory(eln_name), src_name,
                         Vcomp_eln_to_el_h);
                return true;
            }
            else
...
static void maybe_swap_for_eln(bool no_native, Lisp_Object* filename, int* fd,
                               struct timespec mtime) {
#ifdef HAVE_NATIVE_COMP
    ...
    if (no_native || load_no_native || !suffix_p(*filename, ".elc"))
        return;

    /* Search eln in the eln-cache directories.  */
    Lisp_Object eln_path_tail = Vnative_comp_eln_load_path;
    Lisp_Object src_name = Fsubstring(*filename, Qnil, make_fixnum(-1));
    ...
    FOR_EACH_TAIL_SAFE(eln_path_tail) {
        ...
        Lisp_Object eln_name = Fexpand_file_name(
            eln_rel_name, Fexpand_file_name(Vcomp_native_version_dir, dir));
...
int openp(Lisp_Object path, Lisp_Object str, Lisp_Object suffixes,
...
static void build_load_history(Lisp_Object filename, bool entire) {
...
static AVOID end_of_file_error(source_t* source) {
    if (from_file_p(source))
...
static Lisp_Object readevalloop_eager_expand_eval(Lisp_Object val,
...
static void readevalloop(Lisp_Object readcharfun, struct infile* infile0,
                         Lisp_Object sourcename, bool printflag,
                         Lisp_Object readfun, Lisp_Object start,
                         Lisp_Object end) {
    int c;
    Lisp_Object val;
    specpdl_ref count = SPECPDL_INDEX();
...
(Lisp_Object buffer, Lisp_Object printflag, Lisp_Object filename,
 Lisp_Object unibyte, Lisp_Object do_allow_print) {
    ...
    BUF_TEMP_SET_PT(XBUFFER(buf), BUF_BEGV(XBUFFER(buf)));
    readevalloop(buf, 0, filename, !NILP(printflag), Qnil, Qnil, Qnil);
...
(Lisp_Object start, Lisp_Object end, Lisp_Object printflag,
 Lisp_Object read_function) {
    ...
    readevalloop(cbuf, 0, BVAR(XBUFFER(cbuf), filename), !NILP(printflag),
                 read_function, start, end);
...
/* Function to set up the global context we need in toplevel read
   ...
   position.  */
static Lisp_Object read_internal_start(Lisp_Object stream, Lisp_Object start,
                                       Lisp_Object end, bool locate_syms) {
    Lisp_Object retval;
    ...
    if (STRINGP(stream)) {
        ...
        validate_subarray(stream, start, end, SCHARS(stream), &startval,
                          &endval);
...
static int character_name_to_code(char const* name, ptrdiff_t name_len,
                                  source_t* source) {
    ...
    if (!RANGED_FIXNUMP(0, code, MAX_UNICODE_CHAR) || len != name_len - 1 ||
    ...
    mod_key:
        {
            ...
            if (c1 != '-') {
                if (c == 's') {
                    ...
                    error("Invalid escape char syntax: \\%c not followed by -",
                          c);
            }
            modifiers |= mod;
            c1 = readchar(source);
    ...
    case 'x':
        {
            ...
            if (count == 0)
                error("Invalid escape char syntax: \\x not followed by hex "
                      "digit");
            if (count < 3 && i >= 0x80)
                i = BYTE8_TO_CHAR(i);
    ...
    case 'N':
        {
            ...
            while (true) {
                ...
                if (c >= 0x80) {
                    ...
                    invalid_syntax_lisp(
                        CALLN(Fformat, format, make_fixed_natnum(c)), source);
                }
...
static void invalid_radix_integer(EMACS_INT radix, source_t* source) {
...
static NO_INLINE void readbuf_grow(readbuf_t* rb) {
...
static inline void add_char_to_buffer(readbuf_t* rb, int c, bool multibyte) {
...
static Lisp_Object read_integer(source_t* source, int radix) {
    specpdl_ref count = SPECPDL_INDEX();
    char stackbuf[20];
...
static Lisp_Object read_char_literal(source_t* source) {
    int ch = readchar(source);
    if (ch < 0)
        end_of_file_error(source);
...
static Lisp_Object read_string_literal(source_t* source) {
    specpdl_ref count = SPECPDL_INDEX();
    char stackbuf[1024];
    readbuf_t rb = {.start = stackbuf,
                    .end = stackbuf + sizeof stackbuf,
    ...
    Lisp_Object obj = make_specified_string(
        rb.start, nchars, nbytes, (force_multibyte || nbytes != nchars));
    return unbind_to(count, obj);
}

/* Make a hash table from the constructor plist.  */
static Lisp_Object hash_table_from_plist(Lisp_Object plist) {
...
static Lisp_Object record_from_list(Lisp_Object elems) {
    ptrdiff_t size = list_length(elems);
    Lisp_Object obj = Fmake_record(XCAR(elems), make_fixnum(size - 1), Qnil);
...
static Lisp_Object vector_from_rev_list(Lisp_Object elems) {
    ptrdiff_t size = list_length(elems);
...
static Lisp_Object bytecode_from_rev_list(Lisp_Object elems, source_t* source) {
    ...
    if (infile && size >= CLOSURE_CONSTANTS) {
        ...
        if (NILP(vec[CLOSURE_CONSTANTS]) && STRINGP(vec[CLOSURE_CODE])) {
            ...
            enc = Fdecode_coding_string(
                enc, source->emacs_mule_encoding ? Qemacs_mule : Qutf_8_emacs,
                Qt, Qnil);
    ...
    if (!(size >= CLOSURE_STACK_DEPTH && size <= CLOSURE_INTERACTIVE + 1 &&
          ...
          ((STRINGP(vec[CLOSURE_CODE]) /* Byte-code function.  */
            && VECTORP(vec[CLOSURE_CONSTANTS]) && size > CLOSURE_STACK_DEPTH &&
            (FIXNATP(vec[CLOSURE_STACK_DEPTH]))) ||
           (CONSP(vec[CLOSURE_CODE]) /* Interpreted function.  */
            &&
            (CONSP(vec[CLOSURE_CONSTANTS]) || NILP(vec[CLOSURE_CONSTANTS]))))))
        invalid_syntax("Invalid byte-code object", source);
...
static Lisp_Object char_table_from_rev_list(Lisp_Object elems,
                                            source_t* source) {
    Lisp_Object obj = vector_from_rev_list(elems);
...
static Lisp_Object sub_char_table_from_rev_list(Lisp_Object elems,
                                                source_t* source) {
...
static Lisp_Object string_props_from_rev_list(Lisp_Object elems,
                                              source_t* source) {
    elems = Fnreverse(elems);
...
static Lisp_Object read_bool_vector(source_t* source) {
    EMACS_INT length = 0;
    for (;;) {
        int c = readchar(source);
        if (c < '0' || c > '9') {
        ...
        if (ckd_mul(&length, length, 10) || ckd_add(&length, length, c - '0'))
            invalid_syntax("#&", source);
    }
    ...
    if (length != size_in_chars * BOOL_VECTOR_BITS_PER_CHAR)
        data[size_in_chars - 1] &=
            (1 << (length % BOOL_VECTOR_BITS_PER_CHAR)) - 1;
    return obj;
}
...
static bool skip_lazy_string(source_t* source) {
    ptrdiff_t nskip = 0;
    ptrdiff_t digits = 0;
    for (;;) {
        int c = readchar(source);
        if (c < '0' || c > '9') {
            if (nskip > 0)
                /* We can't use UNREAD here, because in the code below we
                   side-step READCHAR.  Instead, assume the first char after
                   #@NNN occupies a single byte, which is the case normally
                   since it's just a space.  */
                nskip--;
            else
                unreadchar(source, c);
            break;
...
static ptrdiff_t symbol_char_span(const char* s) {
    const char* p = s;
    while (*p == '^' || *p == '*' || *p == '+' || *p == '-' || *p == '/' ||
           *p == '<' || *p == '=' || *p == '>' || *p == '_' || *p == '|')
        p++;
...
static void skip_space_and_comments(source_t* source) {
    int c;
    do {
        c = readchar(source);
        if (c == ';')
...
static inline struct read_stack_entry* read_stack_top(void) {
    eassume(rdstack.sp > 0);
    return &rdstack.stack[rdstack.sp - 1];
}

static inline struct read_stack_entry* read_stack_pop(void) {
    eassume(rdstack.sp > 0);
    return &rdstack.stack[--rdstack.sp];
}

static inline bool read_stack_empty_p(ptrdiff_t base_sp) {
    return rdstack.sp <= base_sp;
}

NO_INLINE static void grow_read_stack(void) {
...
static inline void read_stack_push(struct read_stack_entry e) {
    if (rdstack.sp >= rdstack.size)
        grow_read_stack();
    rdstack.stack[rdstack.sp++] = e;
}

static void read_stack_reset(intmax_t sp) {
    eassert(sp <= rdstack.sp);
    rdstack.sp = sp;
}

static AVOID invalid_syntax_with_buffer(readbuf_t* rb, source_t* source) {
    *rb->cur = '\0';
    invalid_syntax(rb->start, source);
}

static inline int read_and_buffer(readbuf_t* rb, source_t* source) {
    int c = readchar(source);
    if (c < 0)
...
static Lisp_Object read0(source_t* source, bool locate_syms) {
    ...
    readbuf_t rb = {
        .start = stackbuf, .end = stackbuf + sizeof stackbuf, .heap = NULL};
    ...
    case '.':
        {
            int nch = readchar(source);
            unreadchar(source, nch);
            if (nch <= 32 || nch == NO_BREAK_SPACE || nch == '"' ||
                nch == '\'' || nch == ';' || nch == '(' || nch == '[' ||
                nch == '#' || nch == '?' || nch == '`' || nch == ',') {
                if (!read_stack_empty_p(base_sp) &&
            ...
            *rb.cur = '\0';
            ...
            if (uninterned_symbol) {
                Lisp_Object name = make_specified_string(
                    rb.start, nchars, nbytes, source->multibyte);
                ...
                if (skip_shorthand
                    ...
                    found = oblookup_considering_shorthand(
                        obarray, rb.start, nchars, nbytes, &longhand,
            ...
            if (locate_syms && !NILP(result))
                result =
                    build_symbol_with_pos(result, make_fixnum(start_position));
        ...
        case RE_list_dot:
            {
                ...
                if (load_force_doc_strings &&
                    BASE_EQ(XCAR(obj), Vload_file_name) && !NILP(XCAR(obj)) &&
                    FIXNUMP(XCDR(obj)))
        ...
        case RE_numbered:
            {
                ...
                if (CONSP(obj)) {
                    ...
                    Flread__substitute_object_in_subtree(
                        obj, placeholder, read_objects_completed);

                    /* ...and #n# will use the real value from now on.  */
                    struct Lisp_Hash_Table* h = XHASH_TABLE(read_objects_map);
                    hash_hash_t hash;
                    ptrdiff_t i =
                        hash_find_get_hash(h, e->u.numbered.number, &hash);
                    eassert(i >= 0);
...
static Lisp_Object substitute_object_recurse(struct subst* subst,
                                             Lisp_Object subtree) {
    ...
    case Lisp_Vectorlike:
        {
            ...
            if (BOOL_VECTOR_P(subtree))
                ...
                /* An unknown pseudovector may contain non-Lisp fields, so we
                   ...
                   `Flength' which signaled `sequencep', so I just preserved
                   this behavior.  */
    ...
    case Lisp_String:
        {
            ...
            traverse_intervals_noorder(root_interval, substitute_in_interval,
                                       subst);
            return subtree;
        }
...
static void substitute_in_interval(INTERVAL interval, void* arg) {
...
Lisp_Object string_to_number(char const* string, int base, ptrdiff_t* plen) {
    char const* cp = string;
    bool float_syntax = false;
    double value = 0;

    /* Negate the value ourselves.  This treats 0, NaNs, and infinity properly
       on IEEE floating point hosts, and works around a formerly-common bug
       where atof ("-0.0") drops the sign.  */
    bool negative = *cp == '-';
    bool positive = *cp == '+';
...
Lisp_Object check_obarray_slow(Lisp_Object obarray) {
...
static Lisp_Object intern_sym(Lisp_Object sym, Lisp_Object obarray,
                              Lisp_Object index) {
...
Lisp_Object intern_driver(Lisp_Object string, Lisp_Object obarray,
                          Lisp_Object index) {
...
Lisp_Object intern_1(const char* str, ptrdiff_t len) {
    Lisp_Object obarray = check_obarray(Vobarray);
    Lisp_Object tem = oblookup(obarray, str, len, len);

    return (BARE_SYMBOL_P(tem)
                ? tem
                /* The above `oblookup' was done on the basis of nchars==nbytes,
                   so the string has to be unibyte.  */
                : intern_driver(make_unibyte_string(str, len), obarray, tem));
}

Lisp_Object intern_c_string_1(const char* str, ptrdiff_t len) {
    Lisp_Object obarray = check_obarray(Vobarray);
    Lisp_Object tem = oblookup(obarray, str, len, len);
...
Lisp_Object intern_c_multibyte(const char* str, ptrdiff_t nchars,
                               ptrdiff_t nbytes) {
    Lisp_Object obarray = check_obarray(Vobarray);
    Lisp_Object sym = oblookup(obarray, str, nchars, nbytes);
    if (BARE_SYMBOL_P(sym))
        return sym;
    return intern_driver(make_multibyte_string(str, nchars, nbytes), obarray,
                         sym);
}

static void define_symbol(Lisp_Object sym, char const* str) {
...
(Lisp_Object string, Lisp_Object obarray) {
    ...
    tem = oblookup_considering_shorthand(
        obarray, SSDATA(string), SCHARS(string), SBYTES(string), &longhand,
        &longhand_chars, &longhand_bytes);
...
static ptrdiff_t obarray_index(struct Lisp_Obarray* oa, const char* str,
                               ptrdiff_t size_byte) {
...
(Lisp_Object name, Lisp_Object obarray) {
    ...
    if (SYMBOLP(name))
        ...
        ptrdiff_t longhand_chars = 0;
        ptrdiff_t longhand_bytes = 0;
        sym = oblookup_considering_shorthand(
            obarray, SSDATA(name), SCHARS(name), SBYTES(name), &longhand,
            &longhand_chars, &longhand_bytes);
        xfree(longhand);
        if (FIXNUMP(sym))
            return Qnil;
...
Lisp_Object oblookup(Lisp_Object obarray, register const char* ptr,
                     ptrdiff_t size, ptrdiff_t size_byte) {
    struct Lisp_Obarray* o = XOBARRAY(obarray);
...
Lisp_Object oblookup_considering_shorthand(Lisp_Object obarray, const char* in,
                                           ptrdiff_t size, ptrdiff_t size_byte,
                                           char** out, ptrdiff_t* size_out,
                                           ptrdiff_t* size_byte_out) {
...
static struct Lisp_Obarray* allocate_obarray(void) {
...
(Lisp_Object object) { return OBARRAYP(object) ? Qt : Qnil; }
...
void map_obarray(Lisp_Object obarray, void (*fn)(Lisp_Object, Lisp_Object),
                 Lisp_Object arg) {
    CHECK_OBARRAY(obarray);
...
static void mapatoms_1(Lisp_Object sym, Lisp_Object function) {
    calln(function, sym);
}
...
DEFUN("internal--obarray-buckets", Finternal__obarray_buckets,
      Sinternal__obarray_buckets, 1, 1, 0,
...
void defsubr(union Aligned_Lisp_Subr* aname) {
...
void defvar_int(struct Lisp_Fwd const* i_fwd, char const* namestring) {
...
void defvar_bool(struct Lisp_Fwd const* b_fwd, char const* namestring) {
...
void defvar_lisp_nopro(struct Lisp_Fwd const* o_fwd, char const* namestring) {
...
void defvar_lisp(struct Lisp_Fwd const* o_fwd, char const* namestring) {
    eassert(o_fwd->type == Lisp_Fwd_Obj);
...
void defvar_kboard(struct Lisp_Fwd const* ko_fwd, char const* namestring) {
...
static Lisp_Object load_path_default(void) {
    ...
    if (!NILP(Vinstallation_directory)) {
        ...
        tem = Fexpand_file_name(build_string("lisp"), Vinstallation_directory);
        ...
        /* That dir doesn't exist, so add the build-time
           Lisp dirs instead.  */
        {
            Lisp_Object dump_path = decode_env_path(0, PATH_DUMPLOADSEARCH, 0);
        ...
        if (NILP(Fequal(Vinstallation_directory, Vsource_directory))) {
            ...
            if (!NILP(tem1) && NILP(tem2)) {
                tem =
                    Fexpand_file_name(build_string("lisp"), Vsource_directory);
...
void dir_warning(char const* use, Lisp_Object dirname) {
    ...
    if (initialized) {
        ...
        if (!NILP(Vlocale_coding_system)) {
            Lisp_Object s = code_convert_string_norecord(
                diag, Vlocale_coding_system, false);
            diagnostic = SSDATA(s);
            diaglen = SBYTES(s);
        }
        USE_SAFE_ALLOCA;
        ...
        ptrdiff_t message_len =
            esprintf(buffer, format, use, SSDATA(dirname), diagnostic);
...
to the specified file name if a suffix is allowed or required.  */);
    Vload_suffixes = list2(build_string(".elc"), build_string(".el"));
#ifdef HAVE_MODULES
...
:/* Suffix of loadable module file, or nil if modules are not supported.  */);
    ...
    Vdynamic_library_suffixes =
        Fcons(build_string(DYNAMIC_LIB_SUFFIX), Vdynamic_library_suffixes);
#else
...
the .eln filename.  */);
    ...
    DEFVAR_LISP("user-init-file", Vuser_init_file, doc:
                    /* File name, including directory, of user's initialization
    file. If the file loaded had extension `.elc', and the corresponding
    source file exists, this variable contains the name of source file,
    suitable for use by functions like `custom-save-all' which edit the init
    file. While Emacs loads and evaluates any init file, value is the real
...
`load'.  The function should return t if the file was loaded.  */);
    ...
    DEFVAR_BOOL("load-force-doc-strings", load_force_doc_strings, doc:
                    /* Non-nil means `load' should force-load all dynamic doc
    strings. This is useful when the file being loaded is a temporary copy.
  */);
...
You cannot count on them to still be there!  */);
    Vsource_directory = Fexpand_file_name(
        build_string("../"), Fcar(decode_env_path(0, PATH_DUMPLOADSEARCH, 0)));
...
:/* List of all DEFVAR_BOOL variables, used by the byte code optimizer.  */);
    ...
    DEFVAR_BOOL("load-dangerous-libraries", load_dangerous_libraries, doc:
...
This overrides the value of the NOMESSAGE argument to `load'.  */);
    ...
    DEFVAR_LISP("bytecomp-version-regexp", Vbytecomp_version_regexp, doc:
                    /* Regular expression matching safe to load compiled Lisp
    files. When Emacs loads a compiled Lisp file, it reads the first 512
    bytes from the file, and matches them against this regular expression.
...
Only valid during macro-expansion.  Internal use only. */);
    ...
    DEFSYM(Qinternal_macroexpand_for_load, "internal-macroexpand-for-load");
