...
#define DEFINE_GDB_SYMBOL_BEGIN(type, id)                                      \
    extern DECLARE_GDB_SYM(type, id);                                          \
    DECLARE_GDB_SYM(type, id)
...
enum
{
    EMACS_INT_WIDTH = INT_WIDTH,
    EMACS_UINT_WIDTH = UINT_WIDTH
};
...
enum
{
    BITS_PER_BITS_WORD = BOOL_VECTOR_BITS_PER_CHAR
};
#endif
...
#define eassert(cond)                                                          \
    (suppress_checking || (cond) ? (void)0 : die(#cond, __FILE__, __LINE__))
#define eassume(cond)                                                          \
    (suppress_checking ? assume(cond)                                          \
         : (cond)      ? (void)0                                               \
...
#if (ALIGNOF_EMACS_INT < IDEAL_GCALIGNMENT && !HAVE_C_ALIGNASOF &&             \
...
INLINE bool will_bootstrap_p(void) {
#if HAVE_PDUMPER
...
INLINE bool will_dump_with_pdumper_p(void) {
#if HAVE_PDUMPER
...
INLINE bool dumped_with_pdumper_p(void) {
#if HAVE_PDUMPER
...
INLINE EMACS_INT(XLI)(Lisp_Object o) { return lisp_h_XLI(o); }

INLINE Lisp_Object(XIL)(EMACS_INT i) { return lisp_h_XIL(i); }

INLINE void*(XLP)(Lisp_Object o) { return lisp_h_XLP(o); }

/* Extract A's type.  */

INLINE enum Lisp_Type(XTYPE)(Lisp_Object a) {
#if USE_LSB_TAG
...
INLINE bool(TAGGEDP)(Lisp_Object a, enum Lisp_Type tag) {
    return lisp_h_TAGGEDP(a, tag);
}

INLINE void(CHECK_TYPE)(int ok, Lisp_Object predicate, Lisp_Object x) {
    ...
    union {
        struct {
           ...
           special (with `defvar' etc), and shouldn't be lexically bound.  */
            ...
            /* Value of the symbol or Qunbound if unbound.  Which alternative of
           the union is used depends on the `redirect' field above.  */
            union {
                Lisp_Object value;
...
#define LISP_WORD_TAG(tag) ((Lisp_Word_tag)(tag) << (USE_LSB_TAG ? 0 : VALBITS))
...
INLINE bool PSEUDOVECTORP(Lisp_Object a, int code) {
...
INLINE bool SYMBOLP(Lisp_Object x) {
    return (BARE_SYMBOL_P(x) ||
            (symbols_with_pos_enabled && SYMBOL_WITH_POS_P(x)));
}

INLINE struct Lisp_Symbol_With_Pos* XSYMBOL_WITH_POS(Lisp_Object a) {
    eassert(SYMBOL_WITH_POS_P(a));
    return XUNTAG(a, Lisp_Vectorlike, struct Lisp_Symbol_With_Pos);
}

INLINE Lisp_Object XSYMBOL_WITH_POS_SYM(Lisp_Object a) {
    Lisp_Object sym = XSYMBOL_WITH_POS(a)->sym;
    eassume(BARE_SYMBOL_P(sym));
    return sym;
}

INLINE Lisp_Object XSYMBOL_WITH_POS_POS(Lisp_Object a) {
    return XSYMBOL_WITH_POS(a)->pos;
}

INLINE Lisp_Object maybe_remove_pos_from_symbol(Lisp_Object x) {
...
/* Internal use only.  */
INLINE Lisp_Object make_lisp_symbol_internal(struct Lisp_Symbol* sym) {
...
INLINE Lisp_Object make_lisp_symbol(struct Lisp_Symbol* sym) {
...
INLINE bool c_symbol_p(struct Lisp_Symbol* sym) {
...
INLINE void(CHECK_SYMBOL)(Lisp_Object x) { lisp_h_CHECK_SYMBOL(x); }

/* True if the possibly-unsigned integer I doesn't fit in a fixnum.  */

#define FIXNUM_OVERFLOW_P(i)                                                   \
    (!((0 <= (i) || MOST_NEGATIVE_FIXNUM <= (i)) &&                            \
       (i) <= MOST_POSITIVE_FIXNUM))

#if USE_LSB_TAG

INLINE Lisp_Object(make_fixnum)(EMACS_INT n) {
    eassert(!FIXNUM_OVERFLOW_P(n));
    return lisp_h_make_fixnum_wrap(n);
}

INLINE EMACS_INT(XFIXNUM_RAW)(Lisp_Object a) { return lisp_h_XFIXNUM_RAW(a); }

INLINE Lisp_Object make_ufixnum(EMACS_INT n) {
...
INLINE EMACS_INT XFIXNUM_RAW(Lisp_Object a) {
    EMACS_INT i = XLI(a);
    if (!USE_LSB_TAG) {
        EMACS_UINT u = i;
        i = u << INTTYPEBITS;
...
INLINE EMACS_INT XFIXNUM(Lisp_Object a) {
    eassert(FIXNUMP(a));
    return XFIXNUM_RAW(a);
}

/* Extract A's value as an unsigned integer in the range 0..INTMASK.  */
INLINE EMACS_UINT XUFIXNUM_RAW(Lisp_Object a) {
    EMACS_UINT i = XLI(a);
    return USE_LSB_TAG ? i >> INTTYPEBITS : i & INTMASK;
}
INLINE EMACS_UINT XUFIXNUM(Lisp_Object a) {
    eassert(FIXNUMP(a));
    return XUFIXNUM_RAW(a);
}

/* Return A's hash, which is in the range 0..INTMASK.  */
INLINE EMACS_INT(XHASH)(Lisp_Object a) { return lisp_h_XHASH(a); }

/* Like make_fixnum (N), but may be faster.  N must be in nonnegative range.  */
INLINE Lisp_Object make_fixed_natnum(EMACS_INT n) {
...
INLINE bool(BASE_EQ)(Lisp_Object x, Lisp_Object y) {
...
INLINE Lisp_Object dead_object(void) {
...
#define XSETPSEUDOVECTOR(a, b, code)                                           \
    XSETTYPED_PSEUDOVECTOR(                                                    \
        a, b, (XUNTAG(a, Lisp_Vectorlike, union vectorlike_header)->size),     \
        code)
...
INLINE void* XFIXNUMPTR(Lisp_Object a) { return XUNTAG(a, Lisp_Int0, char); }

INLINE Lisp_Object make_pointer_integer_unsafe(void* p) {
    Lisp_Object a = TAG_PTR_INITIALLY(Lisp_Int0, p);
    return a;
}

INLINE Lisp_Object make_pointer_integer(void* p) {
...
INLINE bool(CONSP)(Lisp_Object x) { return lisp_h_CONSP(x); }

INLINE void CHECK_CONS(Lisp_Object x) { CHECK_TYPE(CONSP(x), Qconsp, x); }

INLINE struct Lisp_Cons* XCONS(Lisp_Object a) {
    eassert(CONSP(a));
...
INLINE Lisp_Object* xcar_addr(Lisp_Object c) { return &XCONS(c)->u.s.car; }
INLINE Lisp_Object* xcdr_addr(Lisp_Object c) { return &XCONS(c)->u.s.u.cdr; }

/* Use these from normal code.  */

INLINE Lisp_Object(XCAR)(Lisp_Object c) { return lisp_h_XCAR(c); }

INLINE Lisp_Object(XCDR)(Lisp_Object c) { return lisp_h_XCDR(c); }

/* Use these to set the fields of a cons cell.

   Note that both arguments may refer to the same object, so 'n'
   should not be read after 'c' is first modified.  */
INLINE void XSETCAR(Lisp_Object c, Lisp_Object n) { *xcar_addr(c) = n; }
INLINE void XSETCDR(Lisp_Object c, Lisp_Object n) { *xcdr_addr(c) = n; }

/* Take the car or cdr of something whose type is not known.  */
INLINE Lisp_Object CAR(Lisp_Object c) {
    if (CONSP(c))
        return XCAR(c);
    if (!NILP(c))
...
INLINE Lisp_Object CDR(Lisp_Object c) {
    if (CONSP(c))
        return XCDR(c);
    if (!NILP(c))
        wrong_type_argument(Qlistp, c);
...
/* Take the car or cdr of something whose type is not known.  */
INLINE Lisp_Object CAR_SAFE(Lisp_Object c) { return CONSP(c) ? XCAR(c) : Qnil; }
INLINE Lisp_Object CDR_SAFE(Lisp_Object c) { return CONSP(c) ? XCDR(c) : Qnil; }
...
struct Lisp_String {
    union {
        struct {
            ...
            ptrdiff_t size;
            /* If nonnegative, number of bytes in the string (which is
           multibyte). If negative, the string is unibyte: -1 for data normally
           allocated -2 for data in rodata (C string constants) -3 for data that
           must be immovable (used for bytecode)  */
            ptrdiff_t size_byte;
...
INLINE bool STRINGP(Lisp_Object x) { return TAGGEDP(x, Lisp_String); }

INLINE void CHECK_STRING(Lisp_Object x) { CHECK_TYPE(STRINGP(x), Qstringp, x); }

INLINE struct Lisp_String* XSTRING(Lisp_Object a) {
    eassert(STRINGP(a));
    return XUNTAG(a, Lisp_String, struct Lisp_String);
}

/* True if STR is a multibyte string.  */
INLINE bool STRING_MULTIBYTE(Lisp_Object str) {
...
INLINE void STRING_SET_MULTIBYTE(Lisp_Object str) {
...
INLINE void STRING_SET_CHARS(Lisp_Object string, ptrdiff_t newsize) {
    ...
    eassert(STRING_MULTIBYTE(string) ? 0 <= newsize && newsize <= SBYTES(string)
                                     : newsize == SCHARS(string));
    XSTRING(string)->u.s.size = newsize;
}

INLINE void CHECK_STRING_NULL_BYTES(Lisp_Object string) {
    CHECK_TYPE(memchr(SSDATA(string), '\0', SBYTES(string)) == NULL, Qfilenamep,
               string);
}

/* True if STR is immovable (whose data won't move during GC).  */
INLINE bool string_immovable_p(Lisp_Object str) {
...
INLINE bool(VECTORLIKEP)(Lisp_Object x) { return lisp_h_VECTORLIKEP(x); }

INLINE struct Lisp_Vector* XVECTOR(Lisp_Object a) {
    eassert(VECTORLIKEP(a));
    return XUNTAG(a, Lisp_Vectorlike, struct Lisp_Vector);
}

INLINE ptrdiff_t ASIZE(Lisp_Object array) {
    ptrdiff_t size = XVECTOR(array)->header.size;
    eassume(0 <= size);
    return size;
}

INLINE ptrdiff_t gc_asize(Lisp_Object array) {
    /* Like ASIZE, but also can be used in the garbage collector.  */
    return XVECTOR(array)->header.size & ~ARRAY_MARK_FLAG;
}

INLINE ptrdiff_t PVSIZE(Lisp_Object pv) {
    return ASIZE(pv) & PSEUDOVECTOR_SIZE_MASK;
}

INLINE bool VECTORP(Lisp_Object x) {
    return VECTORLIKEP(x) && !(ASIZE(x) & PSEUDOVECTOR_FLAG);
}

INLINE void CHECK_VECTOR(Lisp_Object x) { CHECK_TYPE(VECTORP(x), Qvectorp, x); }


/* A pseudovector is like a vector, but has other non-Lisp components.  */

INLINE enum pvec_type PSEUDOVECTOR_TYPE(const struct Lisp_Vector* v) {
...
INLINE bool PSEUDOVECTOR_TYPEP(const union vectorlike_header* a,
                               enum pvec_type code) {
...
INLINE EMACS_INT bool_vector_words(EMACS_INT size) {
    eassume(0 <= size && size <= EMACS_INT_MAX - (BITS_PER_BITS_WORD - 1));
    return (size + (BITS_PER_BITS_WORD - 1)) / BITS_PER_BITS_WORD;
}

INLINE EMACS_INT bool_vector_bytes(EMACS_INT size) {
    eassume(0 <= size && size <= EMACS_INT_MAX - (BITS_PER_BITS_WORD - 1));
    return (size + (BOOL_VECTOR_BITS_PER_CHAR - 1)) / BOOL_VECTOR_BITS_PER_CHAR;
}

INLINE bits_word bits_word_to_host_endian(bits_word val) {
...
INLINE bool BOOL_VECTOR_P(Lisp_Object a) {
    return PSEUDOVECTORP(a, PVEC_BOOL_VECTOR);
}

INLINE void CHECK_BOOL_VECTOR(Lisp_Object x) {
    CHECK_TYPE(BOOL_VECTOR_P(x), Qbool_vector_p, x);
}

INLINE struct Lisp_Bool_Vector* XBOOL_VECTOR(Lisp_Object a) {
    eassert(BOOL_VECTOR_P(a));
    return XUNTAG(a, Lisp_Vectorlike, struct Lisp_Bool_Vector);
}

INLINE EMACS_INT bool_vector_size(Lisp_Object a) {
    EMACS_INT size = XBOOL_VECTOR(a)->size;
    eassume(0 <= size);
    return size;
}

INLINE bits_word* bool_vector_data(Lisp_Object a) {
    return XBOOL_VECTOR(a)->data;
}

INLINE unsigned char* bool_vector_uchar_data(Lisp_Object a) {
    return (unsigned char*)bool_vector_data(a);
}

/* True if A's Ith bit is set.  */

INLINE bool bool_vector_bitref(Lisp_Object a, EMACS_INT i) {
    eassume(0 <= i);
...
INLINE Lisp_Object bool_vector_ref(Lisp_Object a, EMACS_INT i) {
    return bool_vector_bitref(a, i) ? Qt : Qnil;
}

/* Set A's Ith bit to B.  */

INLINE void bool_vector_set(Lisp_Object a, EMACS_INT i, bool b) {
    eassume(0 <= i);
    eassert(i < bool_vector_size(a));
...
INLINE Lisp_Object AREF(Lisp_Object array, ptrdiff_t idx) {
    eassert(0 <= idx && idx < gc_asize(array));
    return XVECTOR(array)->contents[idx];
}

INLINE Lisp_Object* aref_addr(Lisp_Object array, ptrdiff_t idx) {
    eassert(0 <= idx && idx <= gc_asize(array));
    return &XVECTOR(array)->contents[idx];
}

INLINE void ASET(Lisp_Object array, ptrdiff_t idx, Lisp_Object val) {
    eassert(0 <= idx && idx < ASIZE(array));
    XVECTOR(array)->contents[idx] = val;
}

INLINE void gc_aset(Lisp_Object array, ptrdiff_t idx, Lisp_Object val) {
...
INLINE void memclear(void* p, ptrdiff_t nbytes) {
    eassert(0 <= nbytes);
...
#define VECSIZE(type) ((sizeof(type) - header_size + word_size - 1) / word_size)
...
INLINE bool ASCII_CHAR_P(intmax_t c) { return 0 <= c && c < 0x80; }
...
INLINE bool CHAR_TABLE_P(Lisp_Object a) {
    return PSEUDOVECTORP(a, PVEC_CHAR_TABLE);
}

INLINE struct Lisp_Char_Table* XCHAR_TABLE(Lisp_Object a) {
...
INLINE bool SUB_CHAR_TABLE_P(Lisp_Object a) {
    return PSEUDOVECTORP(a, PVEC_SUB_CHAR_TABLE);
}

INLINE struct Lisp_Sub_Char_Table* XSUB_CHAR_TABLE(Lisp_Object a) {
    eassert(SUB_CHAR_TABLE_P(a));
    return XUNTAG(a, Lisp_Vectorlike, struct Lisp_Sub_Char_Table);
}

INLINE Lisp_Object CHAR_TABLE_REF_ASCII(Lisp_Object ct, ptrdiff_t idx) {
...
INLINE bool SUBRP(Lisp_Object a) { return PSEUDOVECTORP(a, PVEC_SUBR); }

INLINE struct Lisp_Subr* XSUBR(Lisp_Object a) {
    eassert(SUBRP(a));
...
INLINE bool VALID_DOCSTRING_P(Lisp_Object doc) {
...
INLINE int CHAR_TABLE_EXTRA_SLOTS(struct Lisp_Char_Table* ct) {
...
INLINE Lisp_Object SYMBOL_VAL(struct Lisp_Symbol* sym) {
    eassert(sym->u.s.redirect == SYMBOL_PLAINVAL);
    return sym->u.s.val.value;
}

INLINE struct Lisp_Symbol* SYMBOL_ALIAS(struct Lisp_Symbol* sym) {
    eassume(sym->u.s.redirect == SYMBOL_VARALIAS && sym->u.s.val.alias);
    return sym->u.s.val.alias;
}
INLINE struct Lisp_Buffer_Local_Value* SYMBOL_BLV(struct Lisp_Symbol* sym) {
    eassume(sym->u.s.redirect == SYMBOL_LOCALIZED && sym->u.s.val.blv);
    return sym->u.s.val.blv;
}
INLINE lispfwd SYMBOL_FWD(struct Lisp_Symbol* sym) {
    eassume(sym->u.s.redirect == SYMBOL_FORWARDED && sym->u.s.val.fwd);
    return sym->u.s.val.fwd;
}

INLINE void SET_SYMBOL_VAL(struct Lisp_Symbol* sym, Lisp_Object v) {
    eassert(sym->u.s.redirect == SYMBOL_PLAINVAL);
    sym->u.s.val.value = v;
}

INLINE void SET_SYMBOL_ALIAS(struct Lisp_Symbol* sym, struct Lisp_Symbol* v) {
    eassume(sym->u.s.redirect == SYMBOL_VARALIAS && v);
    sym->u.s.val.alias = v;
}
INLINE void SET_SYMBOL_BLV(struct Lisp_Symbol* sym,
                           struct Lisp_Buffer_Local_Value* v) {
    eassume(sym->u.s.redirect == SYMBOL_LOCALIZED && v);
    sym->u.s.val.blv = v;
}
INLINE void SET_SYMBOL_FWD(struct Lisp_Symbol* sym, lispfwd fwd) {
...
INLINE bool SYMBOL_INTERNED_P(Lisp_Object sym) {
    return XSYMBOL(sym)->u.s.interned != SYMBOL_UNINTERNED;
}

/* Value is true if SYM is interned in initial_obarray.  */

INLINE bool SYMBOL_INTERNED_IN_INITIAL_OBARRAY_P(Lisp_Object sym) {
...
INLINE int(SYMBOL_TRAPPED_WRITE_P)(Lisp_Object sym) {
...
INLINE int(SYMBOL_CONSTANT_P)(Lisp_Object sym) {
...
INLINE bool OBARRAYP(Lisp_Object a) { return PSEUDOVECTORP(a, PVEC_OBARRAY); }

INLINE struct Lisp_Obarray* XOBARRAY(Lisp_Object a) {
    eassert(OBARRAYP(a));
    return XUNTAG(a, Lisp_Vectorlike, struct Lisp_Obarray);
}

INLINE void CHECK_OBARRAY(Lisp_Object x) {
    CHECK_TYPE(OBARRAYP(x), Qobarrayp, x);
}

INLINE Lisp_Object make_lisp_obarray(struct Lisp_Obarray* o) {
    eassert(PSEUDOVECTOR_TYPEP(&o->header, PVEC_OBARRAY));
    return make_lisp_ptr(o, Lisp_Vectorlike);
}

INLINE ptrdiff_t obarray_size(const struct Lisp_Obarray* o) {
...
INLINE Lisp_Object check_obarray(Lisp_Object obarray) {
...
INLINE obarray_iter_t make_obarray_iter(struct Lisp_Obarray* oa) {
...
INLINE bool obarray_iter_at_end(obarray_iter_t* it) {
    if (it->symbol)
        return false;
...
INLINE void obarray_iter_step(obarray_iter_t* it) {
    it->symbol = it->symbol->u.s.next;
}

/* The Lisp symbol at IT, if obarray_iter_at_end returned false.  */
INLINE Lisp_Object obarray_iter_symbol(obarray_iter_t* it) {
...
INLINE bool hash_unused_entry_key_p(Lisp_Object key) {
    return BASE_EQ(key, HASH_UNUSED_ENTRY_KEY);
}

INLINE bool HASH_TABLE_P(Lisp_Object a) {
    return PSEUDOVECTORP(a, PVEC_HASH_TABLE);
}

INLINE struct Lisp_Hash_Table* XHASH_TABLE(Lisp_Object a) {
    eassert(HASH_TABLE_P(a));
    return XUNTAG(a, Lisp_Vectorlike, struct Lisp_Hash_Table);
}

INLINE Lisp_Object make_lisp_hash_table(struct Lisp_Hash_Table* h) {
    eassert(PSEUDOVECTOR_TYPEP(&h->header, PVEC_HASH_TABLE));
    return make_lisp_ptr(h, Lisp_Vectorlike);
}

/* Value is the key part of entry IDX in hash table H.  */
INLINE Lisp_Object HASH_KEY(const struct Lisp_Hash_Table* h, ptrdiff_t idx) {
    eassert(idx >= 0 && idx < h->table_size);
    return h->key_and_value[2 * idx];
}

/* Value is the value part of entry IDX in hash table H.  */
INLINE Lisp_Object HASH_VALUE(const struct Lisp_Hash_Table* h, ptrdiff_t idx) {
    eassert(idx >= 0 && idx < h->table_size);
    return h->key_and_value[2 * idx + 1];
}

/* Value is the hash code computed for entry IDX in hash table H.  */
INLINE hash_hash_t HASH_HASH(const struct Lisp_Hash_Table* h, ptrdiff_t idx) {
    eassert(idx >= 0 && idx < h->table_size);
    return h->hash[idx];
}

/* Value is the size of hash table H.  */
INLINE ptrdiff_t HASH_TABLE_SIZE(const struct Lisp_Hash_Table* h) {
    return h->table_size;
}

/* Size of the index vector in hash table H.  */
INLINE ptrdiff_t hash_table_index_size(const struct Lisp_Hash_Table* h) {
    return (ptrdiff_t)1 << h->index_bits;
}

/* Hash value for KEY in hash table H.  */
INLINE hash_hash_t hash_from_key(struct Lisp_Hash_Table* h, Lisp_Object key) {
...
#define DOHASH(h, k, v)                                                        \
    for (Lisp_Object* dohash_##k##_##v##_kv = (h)->key_and_value,              \
                      *dohash_##k##_##v##_end =                                \
                          dohash_##k##_##v##_kv + 2 * HASH_TABLE_SIZE(h),      \
                      *dohash_##k##_##v##_base = dohash_##k##_##v##_kv, k, v;  \
         dohash_##k##_##v##_kv < dohash_##k##_##v##_end &&                     \
         (k = dohash_##k##_##v##_kv[0], v = dohash_##k##_##v##_kv[1],          \
         /*maybe unused*/ (void)v, true);                                      \
         eassert(dohash_##k##_##v##_base == (h)->key_and_value &&              \
                 dohash_##k##_##v##_end ==                                     \
                     dohash_##k##_##v##_base + 2 * HASH_TABLE_SIZE(h)),        \
                      dohash_##k##_##v##_kv += 2)                              \
...
INLINE ptrdiff_t knuth_hash(hash_hash_t hash, unsigned bits) {
...
struct Lisp_Marker {
    ...
    /* This is the buffer that the marker points into, or 0 if it points
       nowhere. Note: a chain of markers can contain markers pointing into
       different buffers (the chain is per buffer_text rather than per buffer,
       so it's shared between indirect buffers).  */
...
INLINE bool FINALIZERP(Lisp_Object x) {
    return PSEUDOVECTORP(x, PVEC_FINALIZER);
}

INLINE struct Lisp_Finalizer* XFINALIZER(Lisp_Object a) {
    eassert(FINALIZERP(a));
    return XUNTAG(a, Lisp_Vectorlike, struct Lisp_Finalizer);
}

INLINE bool MARKERP(Lisp_Object x) { return PSEUDOVECTORP(x, PVEC_MARKER); }

INLINE struct Lisp_Marker* XMARKER(Lisp_Object a) {
    eassert(MARKERP(a));
    return XUNTAG(a, Lisp_Vectorlike, struct Lisp_Marker);
}

INLINE bool OVERLAYP(Lisp_Object x) { return PSEUDOVECTORP(x, PVEC_OVERLAY); }

INLINE struct Lisp_Overlay* XOVERLAY(Lisp_Object a) {
    eassert(OVERLAYP(a));
    return XUNTAG(a, Lisp_Vectorlike, struct Lisp_Overlay);
}

INLINE bool USER_PTRP(Lisp_Object x) { return PSEUDOVECTORP(x, PVEC_USER_PTR); }

INLINE struct Lisp_User_Ptr* XUSER_PTR(Lisp_Object a) {
    eassert(USER_PTRP(a));
    return XUNTAG(a, Lisp_Vectorlike, struct Lisp_User_Ptr);
}

INLINE bool SQLITEP(Lisp_Object x) { return PSEUDOVECTORP(x, PVEC_SQLITE); }

INLINE bool SQLITE(Lisp_Object a) { return PSEUDOVECTORP(a, PVEC_SQLITE); }

INLINE void CHECK_SQLITE(Lisp_Object x) { CHECK_TYPE(SQLITE(x), Qsqlitep, x); }

INLINE struct Lisp_Sqlite* XSQLITE(Lisp_Object a) {
    eassert(SQLITEP(a));
    return XUNTAG(a, Lisp_Vectorlike, struct Lisp_Sqlite);
}

INLINE bool BIGNUMP(Lisp_Object x) { return PSEUDOVECTORP(x, PVEC_BIGNUM); }

INLINE bool INTEGERP(Lisp_Object x) { return FIXNUMP(x) || BIGNUMP(x); }

/* Return a Lisp integer with value taken from N.  */
INLINE Lisp_Object make_int(intmax_t n) {
    return FIXNUM_OVERFLOW_P(n) ? make_bigint(n) : make_fixnum(n);
}
INLINE Lisp_Object make_uint(uintmax_t n) {
...
INLINE enum Lisp_Fwd_Type XFWDTYPE(lispfwd a) { return a->type; }

INLINE bool BUFFER_OBJFWDP(lispfwd a) {
    return XFWDTYPE(a) == Lisp_Fwd_Buffer_Obj;
}

INLINE int XBUFFER_OFFSET(lispfwd a) {
    eassert(BUFFER_OBJFWDP(a));
    return a->u.buf.offset;
}

INLINE bool KBOARD_OBJFWDP(lispfwd a) {
...
INLINE bool(FLOATP)(Lisp_Object x) { return lisp_h_FLOATP(x); }

INLINE struct Lisp_Float* XFLOAT(Lisp_Object a) {
    eassert(FLOATP(a));
    return XUNTAG(a, Lisp_Float, struct Lisp_Float);
}

INLINE double XFLOAT_DATA(Lisp_Object f) { return XFLOAT(f)->u.data; }
...
INLINE bool FIXNATP(Lisp_Object x) { return FIXNUMP(x) && 0 <= XFIXNUM(x); }

/* Like XFIXNUM (A), but may be faster.  A must be nonnegative.  */
INLINE EMACS_INT XFIXNAT(Lisp_Object a) {
    eassert(FIXNUMP(a));
    EMACS_INT int0 = Lisp_Int0;
...
INLINE bool NUMBERP(Lisp_Object x) { return INTEGERP(x) || FLOATP(x); }

INLINE bool RANGED_FIXNUMP(intmax_t lo, Lisp_Object x, intmax_t hi) {
...
INLINE bool AUTOLOADP(Lisp_Object x) {
...
INLINE bool WINDOW_CONFIGURATIONP(Lisp_Object a) {
    return PSEUDOVECTORP(a, PVEC_WINDOW_CONFIGURATION);
}

INLINE bool CLOSUREP(Lisp_Object a) { return PSEUDOVECTORP(a, PVEC_CLOSURE); }

INLINE bool FRAMEP(Lisp_Object a) { return PSEUDOVECTORP(a, PVEC_FRAME); }

INLINE bool RECORDP(Lisp_Object a) { return PSEUDOVECTORP(a, PVEC_RECORD); }

INLINE void CHECK_RECORD(Lisp_Object x) { CHECK_TYPE(RECORDP(x), Qrecordp, x); }

/* Test for image (image . spec)  */
INLINE bool IMAGEP(Lisp_Object x) { return CONSP(x) && EQ(XCAR(x), Qimage); }

/* Array types.  */
INLINE bool ARRAYP(Lisp_Object x) {
    return VECTORP(x) || STRINGP(x) || CHAR_TABLE_P(x) || BOOL_VECTOR_P(x);
}

INLINE void CHECK_LIST(Lisp_Object x) {
    CHECK_TYPE(CONSP(x) || NILP(x), Qlistp, x);
}

INLINE void CHECK_LIST_END(Lisp_Object x, Lisp_Object y) {
    CHECK_TYPE(NILP(x), Qlistp, y);
}

INLINE void(CHECK_FIXNUM)(Lisp_Object x) { lisp_h_CHECK_FIXNUM(x); }

INLINE void CHECK_STRING_CAR(Lisp_Object x) {
    CHECK_TYPE(STRINGP(XCAR(x)), Qstringp, XCAR(x));
}
/* This is a bit special because we always need size afterwards.  */
INLINE ptrdiff_t CHECK_VECTOR_OR_STRING(Lisp_Object x) {
    if (VECTORP(x))
        return ASIZE(x);
    if (STRINGP(x))
        return SCHARS(x);
...
INLINE void CHECK_ARRAY(Lisp_Object x, Lisp_Object predicate) {
    CHECK_TYPE(ARRAYP(x), predicate, x);
}
INLINE void CHECK_FIXNAT(Lisp_Object x) {
    CHECK_TYPE(FIXNATP(x), Qwholenump, x);
}

INLINE double XFLOATINT(Lisp_Object n) {
    return (FIXNUMP(n)      ? XFIXNUM(n)
                : FLOATP(n) ? XFLOAT_DATA(n)
                            : bignum_to_double(n));
}

INLINE void CHECK_NUMBER(Lisp_Object x) { CHECK_TYPE(NUMBERP(x), Qnumberp, x); }

INLINE void CHECK_INTEGER(Lisp_Object x) {
    CHECK_TYPE(INTEGERP(x), Qintegerp, x);
}

INLINE void CHECK_SUBR(Lisp_Object x) { CHECK_TYPE(SUBRP(x), Qsubrp, x); }
...
INLINE Lisp_Object call0(Lisp_Object fn) { return calln(fn); }
...
#define DEFVAR_LISP(lname, vname, doc)                                         \
    do {                                                                       \
        static struct Lisp_Fwd const o_fwd = {Lisp_Fwd_Obj,                    \
                                              .u.objvar = &globals.f_##vname}; \
...
#define DEFVAR_INT(lname, vname, doc)                                          \
    do {                                                                       \
        static struct Lisp_Fwd const i_fwd = {Lisp_Fwd_Int,                    \
                                              .u.intvar = &globals.f_##vname}; \
        defvar_int(&i_fwd, lname);                                             \
    } while (false)
#define DEFVAR_KBOARD(lname, vname, doc)                                       \
    do {                                                                       \
        static struct Lisp_Fwd const ko_fwd = {                                \
            Lisp_Fwd_Kboard_Obj, .u.kbdoffset = offsetof(KBOARD, vname##_)};   \
...
/* Internal use only.  */
INLINE specpdl_ref wrap_specpdl_ref(ptrdiff_t bytes) {
#ifdef WRAP_SPECPDL_REF
...
/* Internal use only.  */
INLINE ptrdiff_t unwrap_specpdl_ref(specpdl_ref ref) {
#ifdef WRAP_SPECPDL_REF
    return ref.bytes;
#else
    return ref;
...
/* Grow the specpdl stack by one entry.
   ...
   address is taken.  */
INLINE void grow_specpdl(void) {
    specpdl_ptr++;
    if (specpdl_ptr == specpdl_end)
        grow_specpdl_allocation();
}

INLINE specpdl_ref record_in_backtrace(Lisp_Object function, Lisp_Object* args,
                                       ptrdiff_t nargs) {
...
/* Define if the windowing system provides a menu bar.  */
    ...
    defined(USE_GTK) || defined(HAVE_HAIKU)
...
INLINE Lisp_Object* xvector_contents_addr(Lisp_Object a, ptrdiff_t i) {
...
INLINE Lisp_Object* xvector_contents(Lisp_Object a) {
    return xvector_contents_addr(a, 0);
}

/* Copy COUNT Lisp_Objects from ARGS to contents of V starting from OFFSET.  */

INLINE void vcopy(Lisp_Object v, ptrdiff_t offset, Lisp_Object const* args,
                  ptrdiff_t count) {
...
INLINE void set_hash_key_slot(struct Lisp_Hash_Table* h, ptrdiff_t idx,
                              Lisp_Object val) {
    eassert(idx >= 0 && idx < h->table_size);
    h->key_and_value[2 * idx] = val;
}

INLINE void set_hash_value_slot(struct Lisp_Hash_Table* h, ptrdiff_t idx,
                                Lisp_Object val) {
    eassert(idx >= 0 && idx < h->table_size);
...
INLINE void set_symbol_function(Lisp_Object sym, Lisp_Object function) {
    XSYMBOL(sym)->u.s.function = function;
}

INLINE void set_symbol_plist(Lisp_Object sym, Lisp_Object plist) {
    XSYMBOL(sym)->u.s.plist = plist;
}

INLINE void set_symbol_next(Lisp_Object sym, struct Lisp_Symbol* next) {
    XSYMBOL(sym)->u.s.next = next;
}

INLINE void make_symbol_constant(Lisp_Object sym) {
    XSYMBOL(sym)->u.s.trapped_write = SYMBOL_NOWRITE;
}

/* Buffer-local variable access functions.  */

INLINE bool blv_found(struct Lisp_Buffer_Local_Value* blv) {
...
INLINE void set_overlay_plist(Lisp_Object overlay, Lisp_Object plist) {
    XOVERLAY(overlay)->plist = plist;
}

/* Get text properties of S.  */

INLINE INTERVAL string_intervals(Lisp_Object s) {
    return XSTRING(s)->u.s.intervals;
}

/* Set text properties of S to I.  */

INLINE void set_string_intervals(Lisp_Object s, INTERVAL i) {
...
INLINE void set_char_table_defalt(Lisp_Object table, Lisp_Object val) {
    XCHAR_TABLE(table)->defalt = val;
}
INLINE void set_char_table_purpose(Lisp_Object table, Lisp_Object val) {
    XCHAR_TABLE(table)->purpose = val;
}

/* Set different slots in (sub)character tables.  */

INLINE void set_char_table_extras(Lisp_Object table, ptrdiff_t idx,
                                  Lisp_Object val) {
    eassert(0 <= idx && idx < CHAR_TABLE_EXTRA_SLOTS(XCHAR_TABLE(table)));
    XCHAR_TABLE(table)->extras[idx] = val;
}

INLINE void set_char_table_contents(Lisp_Object table, ptrdiff_t idx,
                                    Lisp_Object val) {
    eassert(0 <= idx && idx < (1 << CHARTAB_SIZE_BITS_0));
    XCHAR_TABLE(table)->contents[idx] = val;
}

INLINE void set_sub_char_table_contents(Lisp_Object table, ptrdiff_t idx,
                                        Lisp_Object val) {
...
INLINE bool integer_to_intmax(Lisp_Object num, intmax_t* n) {
    if (FIXNUMP(num)) {
        *n = XFIXNUM(num);
        return true;
    }
...
INLINE bool integer_to_uintmax(Lisp_Object num, uintmax_t* n) {
    if (FIXNUMP(num)) {
        *n = XFIXNUM(num);
...
INLINE int elogb(unsigned long long int n) {
...
INLINE modiff_count modiff_incr(modiff_count* a, ptrdiff_t len) {
    modiff_count a0 = *a;
...
INLINE Lisp_Object modiff_to_integer(modiff_count a) {
...
extern void notify_variable_watchers(Lisp_Object, Lisp_Object, Lisp_Object,
                                     Lisp_Object);
...
#define CONS_TO_INTEGER(cons, type, var)                                       \
    (TYPE_SIGNED(type) ? ((var) = cons_to_signed(cons, TYPE_MINIMUM(type),     \
                                                 TYPE_MAXIMUM(type)))          \
...
extern void validate_subarray(Lisp_Object, Lisp_Object, Lisp_Object, ptrdiff_t,
                              ptrdiff_t*, ptrdiff_t*);
extern Lisp_Object substring_both(Lisp_Object, ptrdiff_t, ptrdiff_t, ptrdiff_t,
                                  ptrdiff_t);
extern Lisp_Object merge(Lisp_Object, Lisp_Object, Lisp_Object);
extern Lisp_Object merge_c(Lisp_Object, Lisp_Object,
...
extern ptrdiff_t copy_text(const unsigned char*, unsigned char*, ptrdiff_t,
                           bool, bool);
extern int count_combining_before(const unsigned char*, ptrdiff_t, ptrdiff_t,
                                  ptrdiff_t);
extern int count_combining_after(const unsigned char*, ptrdiff_t, ptrdiff_t,
                                 ptrdiff_t);
extern void insert(const char*, ptrdiff_t);
extern void insert_and_inherit(const char*, ptrdiff_t);
extern void insert_1_both(const char*, ptrdiff_t, ptrdiff_t, bool, bool, bool);
extern void insert_from_gap_1(ptrdiff_t, ptrdiff_t, bool);
extern void insert_from_gap(ptrdiff_t, ptrdiff_t, bool, bool);
extern void insert_from_string(Lisp_Object, ptrdiff_t, ptrdiff_t, ptrdiff_t,
                               ptrdiff_t, bool);
...
extern void insert_from_string_before_markers(Lisp_Object, ptrdiff_t, ptrdiff_t,
...
extern void adjust_after_insert(ptrdiff_t, ptrdiff_t, ptrdiff_t, ptrdiff_t,
                                ptrdiff_t);
extern void adjust_markers_for_delete(ptrdiff_t, ptrdiff_t, ptrdiff_t,
                                      ptrdiff_t);
extern void adjust_markers_bytepos(ptrdiff_t, ptrdiff_t, ptrdiff_t, ptrdiff_t,
                                   int);
...
extern ptrdiff_t display_count_lines(ptrdiff_t start_byte, ptrdiff_t limit_byte,
...
extern bool pos_visible_p(struct window*, ptrdiff_t, int*, int*, int*, int*,
                          int*, int*);
...
/* Defined in character.c.  */
extern void parse_str_as_multibyte(const unsigned char*, ptrdiff_t, ptrdiff_t*,
                                   ptrdiff_t*);
...
#endif
#endif
INLINE void flush_stack_call_func(void (*func)(void* arg), void* arg) {
...
#define list(...) listn(ARRAYELTS(((Lisp_Object[]){__VA_ARGS__})), __VA_ARGS__)
...
INLINE Lisp_Object list1i(intmax_t a) { return list1(make_int(a)); }

INLINE Lisp_Object list2i(intmax_t a, intmax_t b) {
    return list2(make_int(a), make_int(b));
}

INLINE Lisp_Object list3i(intmax_t a, intmax_t b, intmax_t c) {
    return list3(make_int(a), make_int(b), make_int(c));
}

INLINE Lisp_Object list4i(intmax_t a, intmax_t b, intmax_t c, intmax_t d) {
...
INLINE ptrdiff_t vector_nbytes(const struct Lisp_Vector* v) {
    return vectorlike_nbytes(&v->header);
}

/* Make unibyte string from C string when the length isn't known.  */

INLINE Lisp_Object build_unibyte_string(const char* str) {
...
allocate_nil_vector(ptrdiff_t) ATTRIBUTE_RETURNS_NONNULL;
...
extern struct Lisp_Vector* allocate_vector(ptrdiff_t) ATTRIBUTE_RETURNS_NONNULL;

INLINE Lisp_Object make_uninit_vector(ptrdiff_t size) {
    return make_lisp_ptr(allocate_vector(size), Lisp_Vectorlike);
}

/* Like above, but special for sub char-tables.  */

INLINE Lisp_Object make_uninit_sub_char_table(int depth, int min_char) {
...
INLINE Lisp_Object make_nil_vector(ptrdiff_t size) {
    return make_lisp_ptr(allocate_nil_vector(size), Lisp_Vectorlike);
}

extern struct Lisp_Vector*
allocate_pseudovector(int, int, int, enum pvec_type) ATTRIBUTE_RETURNS_NONNULL;
...
#define ALLOCATE_PSEUDOVECTOR(type, field, tag)                                \
    ((type*)allocate_pseudovector(VECSIZE(type), PSEUDOVECSIZE(type, field),   \
...
extern void print_error_message(Lisp_Object, Lisp_Object, const char*,
                                Lisp_Object);
extern Lisp_Object
internal_with_output_to_temp_buffer(const char*, Lisp_Object (*)(Lisp_Object),
                                    Lisp_Object);
...
/* Defined in doprnt.c.  */
extern ptrdiff_t doprnt(char*, ptrdiff_t, const char*, const char*, va_list);
extern ptrdiff_t esprintf(char*, char const*, ...)
    ATTRIBUTE_FORMAT_PRINTF(2, 3);
extern ptrdiff_t exprintf(char**, ptrdiff_t*, char*, ptrdiff_t, char const*,
                          ...) ATTRIBUTE_FORMAT_PRINTF(5, 6);
extern ptrdiff_t evxprintf(char**, ptrdiff_t*, char*, ptrdiff_t, char const*,
                           va_list) ATTRIBUTE_FORMAT_PRINTF(5, 0);

/* Defined in lread.c.  */
extern Lisp_Object intern_1(const char*, ptrdiff_t);
extern Lisp_Object intern_c_string_1(const char*, ptrdiff_t);
extern Lisp_Object intern_driver(Lisp_Object, Lisp_Object, Lisp_Object);
extern Lisp_Object intern_c_multibyte(const char* str, ptrdiff_t nchars,
                                      ptrdiff_t nbytes);
extern void init_symbol(Lisp_Object, Lisp_Object);
extern Lisp_Object oblookup(Lisp_Object, const char*, ptrdiff_t, ptrdiff_t);
INLINE void LOADHIST_ATTACH(Lisp_Object x) {
    if (initialized)
...
INLINE AVOID xsignal(Lisp_Object error_symbol, Lisp_Object data) {
...
extern void define_error(Lisp_Object name, const char* message,
                         Lisp_Object parent);
...
extern Lisp_Object internal_condition_case_1(Lisp_Object (*)(Lisp_Object),
                                             ...
                                             Lisp_Object (*)(Lisp_Object));
extern Lisp_Object
    internal_condition_case_2(Lisp_Object (*)(Lisp_Object, Lisp_Object),
                              Lisp_Object, Lisp_Object, Lisp_Object,
                              Lisp_Object (*)(Lisp_Object));
...
extern Lisp_Object internal_catch_all(Lisp_Object (*)(void*), void*,
                                      Lisp_Object (*)(enum nonlocal_exit,
                                                      Lisp_Object));
extern struct handler* push_handler(Lisp_Object,
                                    enum handlertype) ATTRIBUTE_RETURNS_NONNULL;
...
extern void record_unwind_protect_ptr_mark(void (*function)(void*), void* arg,
                                           void (*mark)(void*));
...
extern void set_unwind_protect(specpdl_ref, void (*)(Lisp_Object), Lisp_Object);
...
extern AVOID verror(const char*, va_list) ATTRIBUTE_FORMAT_PRINTF(1, 0);
...
#define safe_calln(...) CALLMANY(safe_funcall, ((Lisp_Object[]){__VA_ARGS__}))

INLINE void CHECK_KEYWORD_ARGS(ptrdiff_t nargs) {
...
INLINE bool MODULE_FUNCTIONP(Lisp_Object o) {
    return PSEUDOVECTORP(o, PVEC_MODULE_FUNCTION);
}

INLINE struct Lisp_Module_Function* XMODULE_FUNCTION(Lisp_Object o) {
...
extern Lisp_Object
module_function_documentation(struct Lisp_Module_Function const*);
extern Lisp_Object
module_function_interactive_form(const struct Lisp_Module_Function*);
extern Lisp_Object
module_function_command_modes(const struct Lisp_Module_Function*);
extern module_funcptr
module_function_address(struct Lisp_Module_Function const*);
...
extern void find_field(Lisp_Object, Lisp_Object, Lisp_Object, ptrdiff_t*,
...
extern char* splice_dir_file(char*, char const*,
                             char const*) ATTRIBUTE_RETURNS_NONNULL;
...
extern void update_search_regs(ptrdiff_t oldstart, ptrdiff_t oldend,
                               ptrdiff_t newend);
...
INLINE ptrdiff_t fast_string_match(Lisp_Object regexp, Lisp_Object string) {
    return fast_string_match_internal(regexp, string, Qnil);
}

INLINE ptrdiff_t fast_string_match_ignore_case(Lisp_Object regexp,
                                               Lisp_Object string) {
    return fast_string_match_internal(regexp, string, Vascii_canon_table);
}

INLINE ptrdiff_t fast_c_string_match(Lisp_Object regexp, const char* string,
                                     ptrdiff_t len) {
    return fast_c_string_match_internal(regexp, string, len, Qnil);
}

INLINE ptrdiff_t fast_c_string_match_ignore_case(Lisp_Object regexp,
                                                 const char* string,
                                                 ptrdiff_t len) {
...
extern ptrdiff_t fast_looking_at(Lisp_Object, ptrdiff_t, ptrdiff_t, ptrdiff_t,
                                 ptrdiff_t, Lisp_Object);
extern ptrdiff_t find_newline1(ptrdiff_t, ptrdiff_t, ptrdiff_t, ptrdiff_t,
                               ptrdiff_t, ptrdiff_t*, ptrdiff_t*, bool);
extern ptrdiff_t find_newline(ptrdiff_t, ptrdiff_t, ptrdiff_t, ptrdiff_t,
                              ptrdiff_t, ptrdiff_t*, ptrdiff_t*, bool);
extern void scan_newline(ptrdiff_t, ptrdiff_t, ptrdiff_t, ptrdiff_t, ptrdiff_t,
                         bool);
...
extern ptrdiff_t find_before_next_newline(ptrdiff_t, ptrdiff_t, ptrdiff_t,
                                          ptrdiff_t*);
extern EMACS_INT search_buffer(Lisp_Object, ptrdiff_t, ptrdiff_t, ptrdiff_t,
                               ptrdiff_t, EMACS_INT, bool, Lisp_Object,
                               Lisp_Object, bool);
...
extern int emacs_spawn(pid_t*, int, int, int, char**, char**, const char*,
                       const char*, bool, bool, const sigset_t*);
...
INLINE struct bc_frame* get_act_rec(struct thread_state* th) {
    return th->bc.fp;
}

INLINE void set_act_rec(struct thread_state* th, struct bc_frame* act_rec) {
    th->bc.fp = act_rec;
}
...
extern void record_property_change(ptrdiff_t, ptrdiff_t, Lisp_Object,
                                   Lisp_Object, Lisp_Object);
...
#else
INLINE int maybe_disable_address_randomization(int argc, char** argv) {
    return argc;
}
#endif
...
#endif
extern FILE* emacs_fdopen(int, const char*) ATTRIBUTE_MALLOC
    ATTRIBUTE_DEALLOC(emacs_fclose, 1);
...
extern int emacs_renameat_noreplace(int, const char*, int, const char*);
...
extern void report_point_change(struct frame*, struct window*, struct buffer*);
...
#ifdef HAVE_NATIVE_COMP
INLINE bool NATIVE_COMP_FUNCTIONP(Lisp_Object a) {
    return SUBRP(a) && !NILP(XSUBR(a)->native_comp_u);
}

INLINE bool NATIVE_COMP_FUNCTION_DYNP(Lisp_Object a) {
    return NATIVE_COMP_FUNCTIONP(a) && !NILP(XSUBR(a)->lambda_list);
}

INLINE Lisp_Object SUBR_TYPE(Lisp_Object a) { return XSUBR(a)->type; }

INLINE struct Lisp_Native_Comp_Unit* allocate_native_comp_unit(void) {
    return ALLOCATE_ZEROED_PSEUDOVECTOR(struct Lisp_Native_Comp_Unit, data_vec,
                                        PVEC_NATIVE_COMP_UNIT);
}
#else
INLINE bool NATIVE_COMP_FUNCTIONP(Lisp_Object a) { return false; }

INLINE bool NATIVE_COMP_FUNCTION_DYNP(Lisp_Object a) { return false; }
...
extern void* xpalloc(void*, ptrdiff_t*, ptrdiff_t, ptrdiff_t,
                     ptrdiff_t) ATTRIBUTE_RETURNS_NONNULL;

extern char* xstrdup(char const*) ATTRIBUTE_MALLOC ATTRIBUTE_RETURNS_NONNULL;
...
INLINE char* lispstpcpy(char* dest, Lisp_Object string) {
...
INLINE void __lsan_ignore_object(void const* p) {}
#endif
...
INLINE char* egetenv(const char* var) {
...
INLINE void safe_free(specpdl_ref sa_count) {
...
INLINE Lisp_Object safe_free_unbind_to(specpdl_ref count, specpdl_ref sa_count,
                                       Lisp_Object val) {
...
/* If USE_STACK_LISP_OBJECTS, define macros and functions that
   ...
   Build with CPPFLAGS='-DUSE_STACK_LISP_OBJECTS=0' to disable it.  */
     ...
     !defined __clang__ && !GNUC_PREREQ(4, 3, 2))
...
enum
{
    defined_GC_CHECK_STRING_BYTES = true
};
#else
enum
{
    defined_GC_CHECK_STRING_BYTES = false
};
#endif
...
#define AUTO_CONS_EXPR(a, b) (USE_STACK_CONS ? STACK_CONS(a, b) : Fcons(a, b))
...
#define AUTO_LIST3(name, a, b, c)                                              \
    Lisp_Object name =                                                         \
        (USE_STACK_CONS ? STACK_CONS(a, STACK_CONS(b, STACK_CONS(c, Qnil)))    \
                        : list3(a, b, c))
#define AUTO_LIST4(name, a, b, c, d)                                           \
    Lisp_Object name =                                                         \
        (USE_STACK_CONS                                                        \
             ? STACK_CONS(a,                                                   \
                          STACK_CONS(b, STACK_CONS(c, STACK_CONS(d, Qnil))))   \
             : list4(a, b, c, d))
