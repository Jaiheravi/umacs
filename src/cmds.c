...
static Lisp_Object move_point(Lisp_Object n, bool forward) {
...
(Lisp_Object n) { return move_point(n, 1); }
...
right or to the left on the screen.  This is in contrast with
\\[left-char], which see.  */)
(Lisp_Object n) { return move_point(n, 0); }
...
(Lisp_Object n) {
    ...
    if (NILP(n))
        ...
        CHECK_INTEGER(n);
        if (FIXNUMP(n) && -BUF_BYTES_MAX <= XFIXNUM(n) &&
            XFIXNUM(n) <= BUF_BYTES_MAX)
            count = XFIXNUM(n);
        else {
    ...
    return (excessive ? CALLN(Fplus, make_fixnum(shortage - count), n)
...
DEFUN("end-of-line", Fend_of_line, Send_of_line, 0, 1, "^p", doc:
...
(Lisp_Object n) {
    ...
    while (1) {
        ...
        else if (PT > newpos && PT < ZV && FETCH_BYTE(PT_BYTE) != '\n')
...
(Lisp_Object n, Lisp_Object c) {
    ...
    if (!CHARACTERP(c))
        ...
        int character =
            translate_char(Vtranslation_table_for_input, XFIXNUM(c));
...
static int internal_self_insert(int c, EMACS_INT n) {
    int hairy = 0;
    Lisp_Object tem;
    ...
    if (!NILP(overwrite) && PT < ZV) {
       ...
       or before a tab if it doesn't use the whole width of the tab.  */
        if (EQ(overwrite, Qoverwrite_mode_binary))
            ...
            if (n <=
                (min(MOST_POSITIVE_FIXNUM, PTRDIFF_MAX) - curcol) / cwidth) {
    ...
    if (!NILP(BVAR(current_buffer, abbrev_mode)) && synt != Sword &&
        NILP(BVAR(current_buffer, read_only)) && PT > BEGV &&
        (SYNTAX(!NILP(BVAR(current_buffer, enable_multibyte_characters))
                    ? XFIXNAT(Fprevious_char())
                    : UNIBYTE_TO_CHAR(XFIXNAT(Fprevious_char()))) == Sword)) {
       ...
       return right away--don't really self-insert.  */
        if (SYMBOLP(sym) && !NILP(sym) && !NILP(XSYMBOL(sym)->u.s.function) &&
    ...
    if (chars_to_delete) {
        int mc = ((NILP(BVAR(current_buffer, enable_multibyte_characters)) &&
                   SINGLE_BYTE_CHAR_P(c))
                      ...
                      : c);
        Lisp_Object string =
            Fmake_string(make_fixnum(n), make_fixnum(mc), Qnil);

        if (spaces_to_insert) {
            tem = Fmake_string(make_fixnum(spaces_to_insert), make_fixnum(' '),
                               Qnil);
