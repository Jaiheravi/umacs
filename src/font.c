...
bool valid_font_driver(struct font_driver const* drv) {
...
static Lisp_Object font_make_spec(void) {
    ...
    struct font_spec* spec = ((struct font_spec*)allocate_pseudovector(
        VECSIZE(struct font_spec), FONT_SPEC_MAX, FONT_SPEC_MAX, PVEC_FONT));
    XSETFONT(font_spec, spec);
    return font_spec;
}

Lisp_Object font_make_entity(void) {
    Lisp_Object font_entity;
    struct font_entity* entity = ((struct font_entity*)allocate_pseudovector(
        VECSIZE(struct font_entity), FONT_ENTITY_MAX, FONT_ENTITY_MAX,
        PVEC_FONT));
...
Lisp_Object font_make_entity_android(int size) {
    Lisp_Object font_entity;
    struct font_entity* entity = ((struct font_entity*)allocate_pseudovector(
        size, FONT_ENTITY_MAX, FONT_ENTITY_MAX, PVEC_FONT));
...
Lisp_Object font_make_object(int size, Lisp_Object entity, int pixelsize) {
    Lisp_Object font_object;
    struct font* font = (struct font*)allocate_pseudovector(
        size, FONT_OBJECT_MAX, FONT_OBJECT_MAX, PVEC_FONT);
    int i;
...
Lisp_Object font_build_object(int vectorsize, Lisp_Object type,
...
Lisp_Object font_intern_prop(const char* str, ptrdiff_t len,
                             bool force_symbol) {
    ...
    if (SYMBOLP(tem))
        return tem;
    name =
        make_specified_string(str, nchars, len, len != nchars && len == nbytes);
    return intern_driver(name, obarray, tem);
}

/* Return a pixel size of font-spec SPEC on frame F.  */

static int font_pixel_size(struct frame* f, Lisp_Object spec) {
#ifdef HAVE_WINDOW_SYSTEM
...
int font_style_to_value(enum font_property_index prop, Lisp_Object val,
                        bool noerror) {
...
Lisp_Object font_style_symbolic(Lisp_Object font, enum font_property_index prop,
                                bool for_face) {
...
Lisp_Object find_font_encoding(Lisp_Object fontname) {
...
int font_registry_charsets(Lisp_Object registry, struct charset** encoding,
...
static Lisp_Object font_prop_validate_symbol(Lisp_Object prop,
                                             Lisp_Object val) {
    if (STRINGP(val))
...
static Lisp_Object font_prop_validate_style(Lisp_Object style,
                                            Lisp_Object val) {
...
static Lisp_Object font_prop_validate_non_neg(Lisp_Object prop,
                                              Lisp_Object val) {
    return (FIXNATP(val) || (FLOATP(val) && XFLOAT_DATA(val) >= 0) ? val
                                                                   : Qerror);
}

static Lisp_Object font_prop_validate_spacing(Lisp_Object prop,
                                              Lisp_Object val) {
...
static Lisp_Object font_prop_validate_otf(Lisp_Object prop, Lisp_Object val) {
...
static int get_font_prop_index(Lisp_Object key) {
    int i;
...
static Lisp_Object font_prop_validate(int idx, Lisp_Object prop,
                                      Lisp_Object val) {
...
Lisp_Object font_put_extra(Lisp_Object font, Lisp_Object prop,
                           Lisp_Object val) {
    ...
    if (NILP(slot)) {
        ...
        if (BASE_EQ(val, Qunbound))
            return val;
        while (CONSP(extra) && NILP(Fstring_lessp(prop, XCAR(XCAR(extra)))))
...
static int parse_matrix(const char* p) {
    double matrix[4];
    char* end;
    int i;
...
static int font_expand_wildcards(Lisp_Object* field, int n) {
    /* Copy of FIELD.  */
...
#define XLFD_LARGENUM_MASK                                                     \
    (XLFD_POINT_MASK | XLFD_RESX_MASK | XLFD_RESY_MASK | XLFD_AVGWIDTH_MASK)
    ...
    for (i = 0, range_from = 0, range_to = 14 - n; i < n;
        ...
        if (NILP(val)) {
            ...
            if (FIXNUMP(val)) {
                ...
                if (i + 1 == n)
                    from = to = XLFD_ENCODING_INDEX, mask = XLFD_ENCODING_MASK;
                else if (numeric == 0)
...
static int font_parse_xlfd_1(char* name, ptrdiff_t len, Lisp_Object font,
                             int segments) {
    int i, j, n;
...
#define INTERN_FIELD_SYM(N) font_intern_prop(f[N], f[(N) + 1] - 1 - f[N], 1)
    ...
    if (i == XLFD_LAST_INDEX) {
        ...
        if (!NILP(prop[XLFD_SPACING_INDEX])) {
            val =
                font_prop_validate_spacing(QCspacing, prop[XLFD_SPACING_INDEX]);
            if (!FIXNUMP(val))
                return -1;
            ASET(font, FONT_SPACING_INDEX, val);
        }
...
int font_parse_xlfd(char* name, ptrdiff_t len, Lisp_Object font) {
...
char* font_dynamic_unparse_xlfd(Lisp_Object font, int pixel_size) {
    char* p;
    ...
    name =
        xmalloc(strlen(f[XLFD_FOUNDRY_INDEX]) + strlen(f[XLFD_FAMILY_INDEX]) +
    ...
    sprintf(name, "-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s", f[XLFD_FOUNDRY_INDEX],
            f[XLFD_FAMILY_INDEX], f[XLFD_WEIGHT_INDEX], f[XLFD_SLANT_INDEX],
            f[XLFD_SWIDTH_INDEX], f[XLFD_ADSTYLE_INDEX], f[XLFD_PIXEL_INDEX],
            f[XLFD_RESX_INDEX], f[XLFD_SPACING_INDEX], f[XLFD_AVGWIDTH_INDEX],
...
ptrdiff_t font_unparse_xlfd(Lisp_Object font, int pixel_size, char* name,
                            int nbytes) {
    char* p;
    const char* f[XLFD_REGISTRY_INDEX + 1];
    Lisp_Object val;
    ...
    len = snprintf(
        name, nbytes, "-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s-%s",
        f[XLFD_FOUNDRY_INDEX], f[XLFD_FAMILY_INDEX], f[XLFD_WEIGHT_INDEX],
        f[XLFD_SLANT_INDEX], f[XLFD_SWIDTH_INDEX], f[XLFD_ADSTYLE_INDEX],
        f[XLFD_PIXEL_INDEX], f[XLFD_RESX_INDEX], f[XLFD_SPACING_INDEX],
        f[XLFD_AVGWIDTH_INDEX], f[XLFD_REGISTRY_INDEX]);
...
static int font_parse_fcname(char* name, ptrdiff_t len, Lisp_Object font) {
    char *p, *q;
    ...
    for (p = name; *p; p++) {
        if (*p == '\\' && p[1])
            ...
            for (q = p + 1; *q && *q != ':'; q++) {
                ...
                if (q == p + 1 &&
                    (strncmp(q, "ExtB", 4) == 0 || strncmp(q, "R", 1) == 0 ||
                     strncmp(q, "B", 1) == 0 || strncmp(q, "SB", 2) == 0)) {
                    size_found = 0;
                    break;
                }
#endif
...
#define PROP_MATCH(STR)                                                        \
    (word_len == strlen(STR) && memcmp(p, STR, strlen(STR)) == 0)
                    ...
                    if (PROP_MATCH("thin") || PROP_MATCH("ultra-light") ||
                        PROP_MATCH("light") || PROP_MATCH("semi-light") ||
                        PROP_MATCH("book") || PROP_MATCH("medium") ||
                        PROP_MATCH("normal") || PROP_MATCH("semibold") ||
                        PROP_MATCH("demibold") || PROP_MATCH("bold") ||
                        PROP_MATCH("ultra-bold") || PROP_MATCH("black") ||
                        PROP_MATCH("heavy") || PROP_MATCH("ultra-heavy"))
                        FONT_SET_STYLE(font, FONT_WEIGHT_INDEX, val);
                    else if (PROP_MATCH("roman") || PROP_MATCH("italic") ||
                             PROP_MATCH("oblique"))
...
#undef PROP_MATCH
                    ...
                    if (prop >= FONT_FOUNDRY_INDEX && prop < FONT_EXTRA_INDEX)
                        ...
                        extra_props =
                            nconc2(extra_props, list1(Fcons(key, val)));
                    }
                }
                p = q;
            }
...
#define PROP_MATCH(STR)                                                        \
    (word_len == strlen(STR) && memcmp(word_start, STR, strlen(STR)) == 0)
...
static int font_unparse_fcname(Lisp_Object font, int pixel_size, char* name,
                               int nbytes) {
    ...
    if (!NILP(AREF(font, FONT_FOUNDRY_INDEX))) {
        int len = snprintf(p, lim - p, ":foundry=%s",
                           SSDATA(SYMBOL_NAME(AREF(font, FONT_FOUNDRY_INDEX))));
        if (!(0 <= len && len < lim - p))
            return -1;
        p += len;
    }
...
static int font_parse_name(char* name, ptrdiff_t namelen, Lisp_Object font) {
...
void font_parse_family_registry(Lisp_Object family, Lisp_Object registry,
                                Lisp_Object font_spec) {
    ptrdiff_t len;
    char *p0, *p1;
...
static double font_rescale_ratio(Lisp_Object font_entity) {
    Lisp_Object tail, elt;
...
static unsigned font_score(Lisp_Object entity, Lisp_Object* spec_prop) {
    unsigned score = 0;
    int i;
...
static Lisp_Object font_vconcat_entity_vectors(Lisp_Object list) {
...
static int font_compare(const void* d1, const void* d2) {
...
static Lisp_Object font_sort_entities(Lisp_Object list, Lisp_Object prefer,
    ...
    if (FLOATP(prefer_prop[FONT_SIZE_INDEX]))
        prefer_prop[FONT_SIZE_INDEX] = make_fixnum(font_pixel_size(f, prefer));
...
void font_update_sort_order(int* order) {
    int i, shift_bits;
...
static bool font_check_otf_features(Lisp_Object script, Lisp_Object langsys,
...
static bool font_check_otf(Lisp_Object spec, Lisp_Object otf_capability) {
    ...
    if (!NILP(gsub) &&
        !font_check_otf_features(script, langsys, gsub, XCAR(otf_capability)))
        return 0;
    if (!NILP(gpos) &&
        !font_check_otf_features(script, langsys, gpos, XCDR(otf_capability)))
        return 0;
    return 1;
}
...
bool font_match_p(Lisp_Object spec, Lisp_Object font) {
...
static void font_prepare_cache(struct frame* f,
...
static void font_finish_cache(struct frame* f,
                              struct font_driver const* driver) {
...
static Lisp_Object font_get_cache(struct frame* f,
                                  struct font_driver const* driver) {
...
static void font_clear_cache(struct frame* f, Lisp_Object cache,
                             struct font_driver const* driver) {
    Lisp_Object tail, elt;
    Lisp_Object entity;
    ptrdiff_t i;
...
bool font_is_ignored(const char* name, ptrdiff_t namelen) {
    ...
    FOR_EACH_TAIL_SAFE(tail) {
        regexp = XCAR(tail);
        if (STRINGP(regexp) &&
            fast_c_string_match_ignore_case(regexp, name, namelen) >= 0)
            return true;
    }
    return false;
}
...
static Lisp_Object font_delete_unmatched(Lisp_Object vec, Lisp_Object spec,
                                         int size) {
    ...
    for (val = Qnil, i = ASIZE(vec) - 1; i >= 0; i--) {
        ...
        if (prop < FONT_SPEC_MAX && FIXNUMP(AREF(spec, FONT_DPI_INDEX)) &&
            ...
            prop = FONT_SPEC_MAX;
        if (prop < FONT_SPEC_MAX && FIXNUMP(AREF(spec, FONT_AVGWIDTH_INDEX)) &&
...
Lisp_Object font_list_entities(struct frame* f, Lisp_Object spec) {
    ...
    for (; driver_list; driver_list = driver_list->next)
        if (driver_list->on &&
            ...
            if (ASIZE(val) > 0 &&
                ...
                val = font_delete_unmatched(val, need_filtering ? spec : Qnil,
                                            size);
            if (ASIZE(val) > 0) {
                list = Fcons(val, list);
...
static Lisp_Object font_matching_entity(struct frame* f, Lisp_Object* attrs,
                                        Lisp_Object spec) {
    struct font_driver_list* driver_list = f->font_driver_list;
...
static Lisp_Object font_open_entity(struct frame* f, Lisp_Object entity,
                                    int pixel_size) {
...
static void font_close_object(struct frame* f, Lisp_Object font_object) {
...
int font_has_char(struct frame* f, Lisp_Object font, int c) {
    struct font* fontp;
...
static unsigned font_encode_char(Lisp_Object font_object, int c) {
    struct font* font;
...
Lisp_Object font_get_name(Lisp_Object font_object) {
    eassert(FONT_OBJECT_P(font_object));
...
Lisp_Object font_spec_from_name(Lisp_Object font_name) {
...
void font_clear_prop(Lisp_Object* attrs, enum font_property_index prop) {
...
static Lisp_Object font_select_entity(struct frame* f, Lisp_Object entities,
                                      Lisp_Object* attrs, int pixel_size,
                                      int c) {
    Lisp_Object font_entity;
    Lisp_Object prefer;
    int i;
...
Lisp_Object font_find_for_lface(struct frame* f, Lisp_Object* attrs,
    ...
    if (c >= 0 && !NILP(AREF(spec, FONT_REGISTRY_INDEX))) {
        ...
        if (font_registry_charsets(AREF(spec, FONT_REGISTRY_INDEX), &encoding,
                                   &repertory) < 0)
            return Qnil;
        if (repertory &&
                ...
                for (l = 0; SYMBOLP(adstyle[l]); l++) {
                    ...
                    if (!NILP(entities)) {
                       ...
                       best match for PIXEL_SIZE and attributes in ATTRS.  */
                        val = font_select_entity(f, entities, attrs, pixel_size,
                                                 c);
                        if (!NILP(val)) {
                            SAFE_FREE();
                            return val;
...
Lisp_Object font_open_for_lface(struct frame* f, Lisp_Object entity,
...
Lisp_Object font_load_for_lface(struct frame* f, Lisp_Object* attrs,
                                Lisp_Object spec) {
...
void font_prepare_for_face(struct frame* f, struct face* face) {
...
void font_done_for_face(struct frame* f, struct face* face) {
...
Lisp_Object font_open_by_spec(struct frame* f, Lisp_Object spec) {
...
Lisp_Object font_open_by_name(struct frame* f, Lisp_Object name) {
...
void register_font_driver(struct font_driver const* driver, struct frame* f) {
...
void free_font_driver_list(struct frame* f) {
...
Lisp_Object font_update_drivers(struct frame* f, Lisp_Object new_drivers) {
    ...
    for (Lisp_Object rest = all_drivers; CONSP(rest); rest = XCDR(rest)) {
        ...
        if (NILP(superseded_by) || NILP(Fmemq(superseded_by, all_drivers)))
    ...
    if (NILP(new_drivers))
        ...
        *next = NULL;
        ...
        if (!f->font_driver_list
                 ->on) { /* None of the drivers is enabled: enable them all.
                            Happens if you set the list of drivers to (xft x) in
                            your .emacs and then use it under w32 or ns.  */
            for (list = f->font_driver_list; list; list = list->next) {
                struct font_driver const* driver = list->driver;
                eassert(!list->on);
...
static void fset_font_data(struct frame* f, Lisp_Object val) {
    f->font_data = val;
}

void font_put_frame_data(struct frame* f, Lisp_Object driver, void* data) {
    ...
    if (!data)
        ...
        if (NILP(val))
            fset_font_data(
                f, Fcons(Fcons(driver, make_mint_ptr(data)), f->font_data));
        else
            XSETCDR(val, make_mint_ptr(data));
    }
}

void* font_get_frame_data(struct frame* f, Lisp_Object driver) {
    Lisp_Object val = assq_no_quit(driver, f->font_data);
...
void font_filter_properties(Lisp_Object font, Lisp_Object alist,
    ...
    for (i = 0; boolean_properties[i] != NULL; ++i)
        for (it = alist; !NILP(it); it = XCDR(it)) {
            ...
            if (strcmp(boolean_properties[i], keystr) == 0) {
                const char* str = FIXNUMP(val)
                    ? (XFIXNUM(val) ? "true" : "false")
...
static Lisp_Object font_at(int c, ptrdiff_t pos, struct face* face,
    ...
    multibyte =
        (NILP(string) ? !NILP(BVAR(current_buffer, enable_multibyte_characters))
    ...
    if (!face) {
        ...
        if (STRINGP(string))
            ...
            face_id = face_at_buffer_position(w, pos, &endptr, pos + 100, false,
                                              -1, 0);
...
static bool codepoint_is_emoji_eligible(int ch) {
...
Lisp_Object font_range(ptrdiff_t pos, ptrdiff_t pos_byte, ptrdiff_t* limit,
    ...
    if (!face) {
        ...
        if (NILP(string))
            face_id =
                face_at_buffer_position(w, pos, &ignore, *limit, false, -1, 0);
        else {
...
Lisp_Object copy_font_spec(Lisp_Object font) {
    enum
    {
...
/* Merge font-specs FROM and TO, and return a new font-spec.
   ...
   property in TO.  */
Lisp_Object merge_font_spec(Lisp_Object from, Lisp_Object to) {
...
(Lisp_Object font, Lisp_Object prop, Lisp_Object val) {
    ...
    if (idx >= 0 && idx < FONT_EXTRA_INDEX) {
        ...
        if (EQ(prop, QCname) || EQ(prop, QCscript) || EQ(prop, QClang) ||
            EQ(prop, QCotf))
            CHECK_FONT_SPEC(font);
        else
            CHECK_FONT(font);
...
void clear_font_cache(struct frame* f) {
    struct font_driver_list* driver_list = f->font_driver_list;
...
void font_fill_lglyph_metrics(Lisp_Object glyph, struct font* font,
                              unsigned int code) {
...
(Lisp_Object gstring, Lisp_Object direction) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (FIXNUMP(n))
            break;
        gstring = larger_vector(gstring, LGSTRING_GLYPH_LEN(gstring), -1);
    }
    if (i == 3 || XFIXNUM(n) == 0)
        return Qnil;
    ...
    for (i = 1; i < LGSTRING_GLYPH_LEN(gstring); i++) {
        ...
        if (!(LGLYPH_FROM(glyph) <= LGLYPH_TO(glyph) &&
              (LGLYPH_FROM(glyph) == from ? LGLYPH_TO(glyph) == to
...
(Lisp_Object position, Lisp_Object ch) {
    ...
    if (NILP(position)) {
        ...
        f = XFRAME(w->frame);
        face_id =
            face_at_buffer_position(w, pos, &dummy, pos + 100, false, -1, 0);
    }
    if (!CHAR_VALID_P(c))
        return Qnil;
...
DEFUN("open-font", Fopen_font, Sopen_font, 1, 3, 0, doc:/* Open FONT-ENTITY. */)
...
(Lisp_Object font_object) {
    ...
    return CALLN(Fvector, AREF(font_object, FONT_NAME_INDEX),
                 ...
                 make_fixnum(font->pixel_size), make_fixnum(font->max_width),
...
(Lisp_Object name, Lisp_Object frame) {
    ...
    info = CALLN(
        Fvector, AREF(font_object, FONT_NAME_INDEX),
        AREF(font_object, FONT_FULLNAME_INDEX), make_fixnum(font->pixel_size),
        make_fixnum(font->height), make_fixnum(font->baseline_offset),
        make_fixnum(font->relative_compose), make_fixnum(font->default_ascent),
        make_fixnum(font->max_width), make_fixnum(font->ascent),
        make_fixnum(font->descent), make_fixnum(font->space_width),
        make_fixnum(font->average_width), AREF(font_object, FONT_FILE_INDEX),
        (font->driver->otf_capability
             ? Fcons(Qopentype, font->driver->otf_capability(font))
             : Qnil));
...
static Lisp_Object build_style_table(const struct table_entry* entry,
                                     int nelement) {
...
void font_add_log(const char* action, Lisp_Object arg, Lisp_Object result) {
    Lisp_Object val;
    int i;
    ...
    if (CONSP(result) && VECTORP(XCAR(result)) && ASIZE(XCAR(result)) > 0 &&
    ...
    if (FONTP(result)) {
        ...
        if (!FONT_SPEC_P(result)) {
            AUTO_STRING(colon, ":");
            val =
                concat3(SYMBOL_NAME(AREF(result, FONT_TYPE_INDEX)), colon, val);
        }
        result = val;
    }
...
void font_deferred_log(const char* action, Lisp_Object arg,
                       Lisp_Object result) {
    if (EQ(Vfont_log, Qt))
        return;
...
void font_drop_xrender_surfaces(struct frame* f) {
    struct font_driver_list* list;
...
gets the repertory information by an opened font and ENCODING.  */);
    ...
    DEFVAR_LISP_NOPRO("font-slant-table", Vfont_slant_table, doc:
    ...
    DEFVAR_LISP_NOPRO("font-width-table", Vfont_width_table, doc:
...
cause Xft crashes.  Only has an effect in Xft builds.  */);
    ...
    DEFVAR_BOOL("query-all-font-backends", query_all_font_backends, doc:
                    /*
...
void init_font(void) { Vfont_log = egetenv("EMACS_FONT_LOG") ? Qnil : Qt; }
