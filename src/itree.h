...
INLINE bool itree_empty_p(struct itree_tree* tree) {
    return !tree || !tree->root;
}
extern intmax_t itree_size(struct itree_tree*);
extern void itree_clear(struct itree_tree*);
extern void itree_insert(struct itree_tree*, struct itree_node*, ptrdiff_t,
                         ptrdiff_t);
extern struct itree_node* itree_remove(struct itree_tree*, struct itree_node*);
...
extern struct itree_iterator* itree_iterator_start(struct itree_iterator*,
                                                   ...
                                                   ptrdiff_t, ptrdiff_t,
                                                   enum itree_order);
extern void itree_iterator_narrow(struct itree_iterator*, ptrdiff_t, ptrdiff_t);
...
#define ITREE_FOREACH(n, t, beg, end, order)                                   \
    ...
    if (!t) {                                                                  \
        ...
        for (struct itree_iterator itree_local_iter_,                          \
             *itree_iter_ = itree_iterator_start(&itree_local_iter_, t, beg,   \
                                                 end, ITREE_##order);          \
