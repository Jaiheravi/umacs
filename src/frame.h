...
INLINE void fset_buffer_list(struct frame* f, Lisp_Object val) {
    f->buffer_list = val;
}
INLINE void fset_buried_buffer_list(struct frame* f, Lisp_Object val) {
    f->buried_buffer_list = val;
}
INLINE void fset_condemned_scroll_bars(struct frame* f, Lisp_Object val) {
    f->condemned_scroll_bars = val;
}
INLINE void fset_face_hash_table(struct frame* f, Lisp_Object val) {
    f->face_hash_table = val;
}
#if defined(HAVE_WINDOW_SYSTEM)
INLINE void fset_parent_frame(struct frame* f, Lisp_Object val) {
    f->parent_frame = val;
}
#endif
INLINE void fset_focus_frame(struct frame* f, Lisp_Object val) {
    f->focus_frame = val;
}
INLINE void fset_icon_name(struct frame* f, Lisp_Object val) {
    f->icon_name = val;
}
INLINE void fset_menu_bar_items(struct frame* f, Lisp_Object val) {
    f->menu_bar_items = val;
}
INLINE void fset_menu_bar_vector(struct frame* f, Lisp_Object val) {
    f->menu_bar_vector = val;
}
#if defined HAVE_WINDOW_SYSTEM && !defined HAVE_EXT_MENU_BAR
INLINE void fset_menu_bar_window(struct frame* f, Lisp_Object val) {
    f->menu_bar_window = val;
}
#endif
INLINE void fset_name(struct frame* f, Lisp_Object val) { f->name = val; }
INLINE void fset_param_alist(struct frame* f, Lisp_Object val) {
    f->param_alist = val;
}
INLINE void fset_root_window(struct frame* f, Lisp_Object val) {
    f->root_window = val;
}
INLINE void fset_scroll_bars(struct frame* f, Lisp_Object val) {
    f->scroll_bars = val;
}
INLINE void fset_selected_window(struct frame* f, Lisp_Object val) {
    f->selected_window = val;
}
INLINE void fset_old_selected_window(struct frame* f, Lisp_Object val) {
    f->old_selected_window = val;
}
INLINE void fset_title(struct frame* f, Lisp_Object val) { f->title = val; }
INLINE void fset_tab_bar_items(struct frame* f, Lisp_Object val) {
    f->tab_bar_items = val;
}
#if defined(HAVE_WINDOW_SYSTEM)
INLINE void fset_tab_bar_window(struct frame* f, Lisp_Object val) {
    f->tab_bar_window = val;
}
INLINE void fset_current_tab_bar_string(struct frame* f, Lisp_Object val) {
    f->current_tab_bar_string = val;
}
INLINE void fset_desired_tab_bar_string(struct frame* f, Lisp_Object val) {
    f->desired_tab_bar_string = val;
}
#endif /* HAVE_WINDOW_SYSTEM */
INLINE void fset_tool_bar_items(struct frame* f, Lisp_Object val) {
    f->tool_bar_items = val;
}

#if defined(HAVE_WINDOW_SYSTEM) && !defined(HAVE_EXT_TOOL_BAR)

INLINE void fset_tool_bar_window(struct frame* f, Lisp_Object val) {
    f->tool_bar_window = val;
}
INLINE void fset_current_tool_bar_string(struct frame* f, Lisp_Object val) {
    f->current_tool_bar_string = val;
}
INLINE void fset_desired_tool_bar_string(struct frame* f, Lisp_Object val) {
    f->desired_tool_bar_string = val;
}

#endif /* HAVE_WINDOW_SYSTEM && !HAVE_EXT_TOOL_BAR */

INLINE void fset_tool_bar_position(struct frame* f, Lisp_Object val) {
    f->tool_bar_position = val;
}

INLINE double NUMVAL(Lisp_Object x) { return NUMBERP(x) ? XFLOATINT(x) : -1; }

INLINE double default_pixels_per_inch_x(void) {
    Lisp_Object v =
        (CONSP(Vdisplay_pixels_per_inch) ? XCAR(Vdisplay_pixels_per_inch)
                                         : Vdisplay_pixels_per_inch);
    return NUMVAL(v) > 0 ? NUMVAL(v) : 72.0;
}

INLINE double default_pixels_per_inch_y(void) {
    Lisp_Object v =
        (CONSP(Vdisplay_pixels_per_inch) ? XCDR(Vdisplay_pixels_per_inch)
                                         : Vdisplay_pixels_per_inch);
    return NUMVAL(v) > 0 ? NUMVAL(v) : 72.0;
}
...
#define FRAME_RES_X(f) (eassert(FRAME_WINDOW_P(f)), FRAME_DISPLAY_INFO(f)->resx)
#define FRAME_RES_Y(f) (eassert(FRAME_WINDOW_P(f)), FRAME_DISPLAY_INFO(f)->resy)
...
#define FRAME_RES(f)                                                           \
    (eassert(FRAME_WINDOW_P(f)), FRAME_DISPLAY_INFO(f)->font_resolution)
...
#define MOUSE_HL_INFO(F)                                                       \
    (FRAME_WINDOW_P(F) ? &FRAME_DISPLAY_INFO(F)->mouse_highlight               \
...
#endif
#else
#define MOUSE_HL_INFO(F) (&(F)->output_data.tty->display_info->mouse_highlight)
#endif
...
#define FRAME_TOOL_BAR_TOP_HEIGHT(f)                                           \
    ((BASE_EQ((f)->tool_bar_position, Qtop)) ? (f)->tool_bar_height : 0)

#define FRAME_TOOL_BAR_TOP_LINES(f)                                            \
    ((BASE_EQ((f)->tool_bar_position, Qtop)) ? (f)->tool_bar_lines : 0)

/* Size of F's tool bar if it is placed at the bottom of the
   frame.  */
#define FRAME_TOOL_BAR_BOTTOM_HEIGHT(f)                                        \
    ((BASE_EQ((f)->tool_bar_position, Qbottom)) ? (f)->tool_bar_height : 0)

#define FRAME_TOOL_BAR_BOTTOM_LINES(f)                                         \
    ((BASE_EQ((f)->tool_bar_position, Qbottom)) ? (f)->tool_bar_lines : 0)

/* Height of frame F's top margin in frame lines.  */
#define FRAME_TOP_MARGIN(F)                                                    \
    (FRAME_MENU_BAR_LINES(F) + FRAME_TAB_BAR_LINES(F) +                        \
...
#define FRAME_BOTTOM_MARGIN(f) FRAME_TOOL_BAR_BOTTOM_LINES(f)

/* Pixel height of frame F's bottom margin.  */

#define FRAME_BOTTOM_MARGIN_HEIGHT(f) FRAME_TOOL_BAR_BOTTOM_HEIGHT(f)
...
INLINE struct frame* FRAME_PARENT_FRAME(struct frame* f) {
...
#define FRAME_HAS_HORIZONTAL_SCROLL_BARS(f) ((f)->horizontal_scroll_bars)
#else
...
#define FRAME_SCROLL_BAR_AREA_HEIGHT(f)                                        \
    (FRAME_HAS_HORIZONTAL_SCROLL_BARS(f) ? FRAME_CONFIG_SCROLL_BAR_HEIGHT(f)   \
                                         : 0)

/* Width of vertical scroll bar in frame F, measured in columns.  */
#define FRAME_SCROLL_BAR_COLS(f)                                               \
    (FRAME_HAS_VERTICAL_SCROLL_BARS(f) ? FRAME_CONFIG_SCROLL_BAR_COLS(f) : 0)

/* Height of horizontal scroll bar in frame F, measured in lines.  */
#define FRAME_SCROLL_BAR_LINES(f)                                              \
    (FRAME_HAS_HORIZONTAL_SCROLL_BARS(f) ? FRAME_CONFIG_SCROLL_BAR_LINES(f) : 0)
...
INLINE void SET_FRAME_ICONIFIED(struct frame* f, int i) {
#ifdef HAVE_WINDOW_SYSTEM
    Lisp_Object frame;
#endif
...
extern int tty_child_pos_param(struct frame*, Lisp_Object, Lisp_Object, int,
                               int);
extern int tty_child_size_param(struct frame*, Lisp_Object, Lisp_Object, int);
#ifdef HAVE_WINDOW_SYSTEM
...
INLINE bool window_system_available(struct frame* f) {
#ifdef HAVE_WINDOW_SYSTEM
...
extern void frame_size_history_extra(struct frame*, Lisp_Object, int, int, int,
                                     int, int, int);
#ifdef NS_IMPL_COCOA
...
/* Set visibility of frame F.
   ...
   wasn't redisplayed).  */
INLINE void SET_FRAME_VISIBLE(struct frame* f, bool v) {
    if (v) {
        if (v == 1 && f->visible != 1)
...
INLINE int frame_dimension(int x) {
#ifdef HAVE_WINDOW_SYSTEM
    return x;
#else
    return 0;
...
INLINE int FRAME_FRINGE_COLS(struct frame* f) {
    return frame_dimension(f->fringe_cols);
}

/* Pixel-width of the left and right fringe.  */

INLINE int FRAME_LEFT_FRINGE_WIDTH(struct frame* f) {
    return frame_dimension(f->left_fringe_width);
}
INLINE int FRAME_RIGHT_FRINGE_WIDTH(struct frame* f) {
    return frame_dimension(f->right_fringe_width);
}

/* Total width of fringes in pixels.  */

INLINE int FRAME_TOTAL_FRINGE_WIDTH(struct frame* f) {
    return FRAME_LEFT_FRINGE_WIDTH(f) + FRAME_RIGHT_FRINGE_WIDTH(f);
}

INLINE int FRAME_CHILD_FRAME_BORDER_WIDTH(struct frame* f) {
...
/* Pixel-width of internal border.  Uses child_frame_border_width for
   ...
   otherwise.  */
INLINE int FRAME_INTERNAL_BORDER_WIDTH(struct frame* f) {
...
INLINE int FRAME_RIGHT_DIVIDER_WIDTH(struct frame* f) {
    return frame_dimension(f->right_divider_width);
}

INLINE int FRAME_BOTTOM_DIVIDER_WIDTH(struct frame* f) {
    return frame_dimension(f->bottom_divider_width);
}

/* Return a non-null pointer to the cached face with ID on frame F.  */

INLINE struct face* FACE_FROM_ID(struct frame* f, int id) {
...
INLINE struct face* FACE_FROM_ID_OR_NULL(struct frame* f, int id) {
...
INLINE struct image* IMAGE_FROM_ID(struct frame* f, int id) {
...
INLINE struct image* IMAGE_OPT_FROM_ID(struct frame* f, int id) {
...
#define FRAME_COL_TO_PIXEL_X(f, col)                                           \
    (FRAME_INTERNAL_BORDER_WIDTH(f) + (col) * FRAME_COLUMN_WIDTH(f))

/* Return the pixel width/height of frame F if it has
   COLS columns/LINES rows.  */
#define FRAME_TEXT_COLS_TO_PIXEL_WIDTH(f, cols)                                \
    ((cols) * FRAME_COLUMN_WIDTH(f) + FRAME_SCROLL_BAR_AREA_WIDTH(f) +         \
     FRAME_TOTAL_FRINGE_WIDTH(f) + 2 * FRAME_INTERNAL_BORDER_WIDTH(f))

#define FRAME_TEXT_LINES_TO_PIXEL_HEIGHT(f, lines)                             \
    ((lines) * FRAME_LINE_HEIGHT(f) + FRAME_TOP_MARGIN_HEIGHT(f) +             \
     FRAME_SCROLL_BAR_AREA_HEIGHT(f) + 2 * FRAME_INTERNAL_BORDER_WIDTH(f))
...
#define FRAME_TEXT_TO_PIXEL_WIDTH(f, width)                                    \
    ((width) + FRAME_SCROLL_BAR_AREA_WIDTH(f) + FRAME_TOTAL_FRINGE_WIDTH(f) +  \
     2 * FRAME_INTERNAL_BORDER_WIDTH(f))
...
#define FRAME_INNER_HEIGHT(f)                                                  \
    (FRAME_PIXEL_HEIGHT(f) - FRAME_MARGIN_HEIGHT(f) -                          \
     2 * FRAME_INTERNAL_BORDER_WIDTH(f))

/* Value is the smallest width of any character in any font on frame F.  */
#define FRAME_SMALLEST_CHAR_WIDTH(f) FRAME_DISPLAY_INFO(f)->smallest_char_width
...
extern void frame_set_mouse_pixel_position(struct frame* f, int pix_x,
                                           int pix_y);
...
INLINE void gui_set_bitmap_icon(struct frame* f) {
...
INLINE void flush_frame(struct frame* f) {
