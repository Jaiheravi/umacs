...
dynlib_handle_ptr dynlib_open(const char* dll_fname) {
    HMODULE hdll;
...
dynlib_handle_ptr dynlib_open_for_eln(const char* dll_fname) {
...
void dynlib_addr(void (*funcptr)(void), const char** fname,
                 const char** symname) {
       ...
       address where it's loaded in memory.  */
        ...
        if (g_b_init_get_module_handle_ex == 0) {
            ...
            /* We load the ANSI version of the function because the
               ...
               Unicode version.  */
            s_pfn_Get_Module_HandleExA = (GetModuleHandleExA_Proc)get_proc_addr(
                hm_kernel32, "GetModuleHandleExA");
        }
...
void dynlib_addr(void (*funcptr)(void), const char** path, const char** sym) {
    *path = NULL;
    *sym = NULL;
#ifdef HAVE_DLADDR
...
const char* dynlib_error(void) { return dlerror(); }

#ifdef HAVE_NATIVE_COMP
int dynlib_close(dynlib_handle_ptr h) { return dlclose(h) == 0; }
#endif
