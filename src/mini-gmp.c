...
mp_limb_t mpn_invert_3by2(mp_limb_t u1, mp_limb_t u0) {
...

    {
        ...
        */
        ...
        r = ((~u1 - (mp_limb_t)qh * uh) << (GMP_LIMB_BITS / 2)) |
            GMP_LLIMB_MASK;
...
static void mpn_div_qr_pi1(mp_ptr qp, mp_ptr np, mp_size_t nn, mp_limb_t n1,
    ...
    do {
        ...
        if (n1 == d1 && n0 == d0) {
            ...
            n1 = np[dn - 1 +
                    i]; /* update n1, last loop's value will now be invalid */
        }
        else {
...
static void mpz_div_r_2exp(mpz_t r, const mpz_t u, mp_bitcnt_t bit_index,
    ...
    if (rn > un) {
       ...
       non-zero */
        if (mode ==
            ((us > 0) ? GMP_DIV_CEIL : GMP_DIV_FLOOR)) /* us != 0 here. */
        {
...
void mpz_gcd(mpz_t g, const mpz_t u, const mpz_t v) {
    ...
    tu->_mp_size =
        mpn_gcd(tu->_mp_d, tu->_mp_d, tu->_mp_size, tv->_mp_d, tv->_mp_size);
