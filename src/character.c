...
int char_string(unsigned int c, unsigned char* p) {
    int bytes;
...
int translate_char(Lisp_Object table, int c) {
...
by the Unicode Standard.  */
          attributes : const)
(Lisp_Object unicode) {
    return (!NILP(unicode) ? make_fixnum(MAX_UNICODE_CHAR)
...
static ptrdiff_t char_width(int c, struct Lisp_Char_Table* dp) {
...
ptrdiff_t c_string_width(const unsigned char* str, ptrdiff_t len, int precision,
...
ptrdiff_t strwidth(const char* str, ptrdiff_t len) {
...
ptrdiff_t lisp_string_width(Lisp_Object string, ptrdiff_t from, ptrdiff_t to,
                            ptrdiff_t precision, ptrdiff_t* nchars,
                            ptrdiff_t* nbytes, bool auto_comp) {
...
#ifdef HAVE_WINDOW_SYSTEM
        else if (auto_comp && f && FRAME_WINDOW_P(f) && multibyte &&
                 find_automatic_composition(i, -1, i, &ignore, &end, &val,
                                            string) &&
                 end > i) {
            int j;
...
ptrdiff_t chars_in_text(const unsigned char* ptr, ptrdiff_t nbytes) {
...
ptrdiff_t multibyte_chars_in_text(const unsigned char* ptr, ptrdiff_t nbytes) {
...
ptrdiff_t str_as_multibyte(unsigned char* str, ptrdiff_t len, ptrdiff_t nbytes,
                           ptrdiff_t* nchars) {
...
static ptrdiff_t string_count_byte8(Lisp_Object string) {
...
Lisp_Object string_escape_byte8(Lisp_Object string) {
    ...
    if (multibyte)
        while (src < src_end) {
            ...
            if (CHAR_BYTE8_HEAD_P(c)) {
                ...
                c = CHAR_TO_BYTE8(c);
                dst +=
                    snprintf((char*)dst, sizeof((char*)dst), "\\%03o", c + 0u);
            }
            else
                while (len--)
                    *dst++ = *src++;
...
bool alphabeticp(int c) {
    ...
    return (gen_cat == UNICODE_CATEGORY_Lu || gen_cat == UNICODE_CATEGORY_Ll ||
            gen_cat == UNICODE_CATEGORY_Lt || gen_cat == UNICODE_CATEGORY_Lm ||
            gen_cat == UNICODE_CATEGORY_Lo || gen_cat == UNICODE_CATEGORY_Mn ||
            gen_cat == UNICODE_CATEGORY_Mc || gen_cat == UNICODE_CATEGORY_Me ||
            gen_cat == UNICODE_CATEGORY_Nl);
}

/* Return true if C is an alphabetic or decimal-number character.  */
bool alphanumericp(int c) {
    Lisp_Object category = CHAR_TABLE_REF(Vunicode_category_table, c);
    if (!FIXNUMP(category))
        return false;
    EMACS_INT gen_cat = XFIXNUM(category);
    ...
    return (gen_cat == UNICODE_CATEGORY_Lu || gen_cat == UNICODE_CATEGORY_Ll ||
            ...
            gen_cat == UNICODE_CATEGORY_Lo || gen_cat == UNICODE_CATEGORY_Mn ||
            gen_cat == UNICODE_CATEGORY_Mc || gen_cat == UNICODE_CATEGORY_Me ||
            gen_cat == UNICODE_CATEGORY_Nl || gen_cat == UNICODE_CATEGORY_Nd);
}
...
Such characters have the value t in this table.  */);
    ...
    Fset_char_table_range(Vprintable_chars,
                          Fcons(make_fixnum(160), make_fixnum(MAX_5_BYTE_CHAR)),
                          Qt);
...
It has one extra slot whose value is a list of script symbols.  */);
    ...
    SCRIPT. If it is a vector, one of the characters in the vector is
    necessary. This variable is used to find a font for a specific script.
  */);
