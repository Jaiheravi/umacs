...
static Lisp_Object sub_char_table_ref_and_range(Lisp_Object, int, int*, int*,
                                                Lisp_Object, bool);
...
static void CHECK_CHAR_TABLE(Lisp_Object x) {
    CHECK_TYPE(CHAR_TABLE_P(x), Qchar_table_p, x);
}

static void set_char_table_ascii(Lisp_Object table, Lisp_Object val) {
    XCHAR_TABLE(table)->ascii = val;
}
static void set_char_table_parent(Lisp_Object table, Lisp_Object val) {
...
static Lisp_Object make_sub_char_table(int depth, int min_char,
                                       Lisp_Object defalt) {
    int i;
...
static Lisp_Object char_table_ascii(Lisp_Object table) {
    Lisp_Object sub, val;
...
static Lisp_Object copy_sub_char_table(Lisp_Object table) {
...
Lisp_Object copy_char_table(Lisp_Object table) {
...
static Lisp_Object sub_char_table_ref(Lisp_Object table, int c,
                                      bool is_uniprop) {
...
Lisp_Object char_table_ref(Lisp_Object table, int c) {
...
char_table_ref_simple(Lisp_Object table, int idx, int c, int* from, int* to,
    ...
    if (SUB_CHAR_TABLE_P(val))
        val =
            sub_char_table_ref_and_range(val, c, from, to, defalt, is_uniprop);
    else if (NILP(val))
        val = defalt;
    return val;
}
...
static Lisp_Object sub_char_table_ref_and_range(Lisp_Object table, int c,
    ...
    while (((c = (chartab_idx + 1) * chartab_chars[depth]) <
           ...
           (c += min_char) <= *to) {
        chartab_idx++;
        Lisp_Object this_val = char_table_ref_simple(
            table, chartab_idx, c, from, to, defalt, is_uniprop, true);
...
Lisp_Object char_table_ref_and_range(Lisp_Object table, int c, int* from,
                                     int* to) {
    struct Lisp_Char_Table* tbl = XCHAR_TABLE(table);
    ...
    while (*from < idx * chartab_chars[0]) {
        ...
        idx--;
        Lisp_Object this_val = char_table_ref_simple(
            table, idx, c, from, to, tbl->defalt, is_uniprop, false);
    ...
    while (*to >= (chartab_idx + 1) * chartab_chars[0]) {
        ...
        Lisp_Object this_val = char_table_ref_simple(
            table, chartab_idx, c, from, to, tbl->defalt, is_uniprop, false);
...
static void sub_char_table_set(Lisp_Object table, int c, Lisp_Object val,
                               bool is_uniprop) {
    struct Lisp_Sub_Char_Table* tbl = XSUB_CHAR_TABLE(table);
    int depth = tbl->depth, min_char = tbl->min_char;
    ...
    if (depth == 3)
        ...
        if (!SUB_CHAR_TABLE_P(sub)) {
            if (is_uniprop && UNIPROP_COMPRESSED_FORM_P(sub))
                ...
                sub = make_sub_char_table(
                    depth + 1, min_char + i * chartab_chars[depth], sub);
...
void char_table_set(Lisp_Object table, int c, Lisp_Object val) {
    struct Lisp_Char_Table* tbl = XCHAR_TABLE(table);
...
static void sub_char_table_set_range(Lisp_Object table, int from, int to,
                                     Lisp_Object val, bool is_uniprop) {
    struct Lisp_Sub_Char_Table* tbl = XSUB_CHAR_TABLE(table);
    int depth = tbl->depth, min_char = tbl->min_char;
...
void char_table_set_range(Lisp_Object table, int from, int to,
                          Lisp_Object val) {
    struct Lisp_Char_Table* tbl = XCHAR_TABLE(table);
...
DEFUN("char-table-extra-slot", Fchar_table_extra_slot, Schar_table_extra_slot,
      2, 2, 0, doc:/* Return the value of CHAR-TABLE's extra-slot number N.  */)
...
DEFUN("set-char-table-extra-slot", Fset_char_table_extra_slot,
      Sset_char_table_extra_slot, 3, 3, 0,
...
static Lisp_Object optimize_sub_char_table(Lisp_Object table,
                                           Lisp_Object test) {
    struct Lisp_Sub_Char_Table* tbl = XSUB_CHAR_TABLE(table);
...
map_sub_char_table(void (*c_function)(Lisp_Object, Lisp_Object, Lisp_Object),
    ...
    for (c = min_char + chars_in_block * i; c <= max_char;
        ...
        if (SUB_CHAR_TABLE_P(this)) {
            ...
            val = map_sub_char_table(c_function, function, this, arg, val,
                                     range, top);
        }
        else {
            if (NILP(this))
...
void map_char_table(void (*c_function)(Lisp_Object, Lisp_Object, Lisp_Object),
                    Lisp_Object function, Lisp_Object table, Lisp_Object arg) {
...
static void map_sub_char_table_for_charset(
    void (*c_function)(Lisp_Object, Lisp_Object), Lisp_Object function,
    Lisp_Object table, Lisp_Object arg, Lisp_Object range,
...
void map_char_table_for_charset(void (*c_function)(Lisp_Object, Lisp_Object),
...
static Lisp_Object uniprop_table_uncompress(Lisp_Object table, int idx) {
...
static Lisp_Object uniprop_decode_value_run_length(Lisp_Object table,
                                                   Lisp_Object value) {
...
static uniprop_decoder_t uniprop_decoder[] = {uniprop_decode_value_run_length};

static const int uniprop_decoder_count = ARRAYELTS(uniprop_decoder);

/* Return the decoder of char-table TABLE or nil if none.  */

static uniprop_decoder_t uniprop_get_decoder(Lisp_Object table) {
    EMACS_INT i;
...
static Lisp_Object uniprop_encode_value_character(Lisp_Object table,
                                                  Lisp_Object value) {
...
static Lisp_Object uniprop_encode_value_run_length(Lisp_Object table,
                                                   Lisp_Object value) {
...
static Lisp_Object uniprop_encode_value_numeric(Lisp_Object table,
                                                Lisp_Object value) {
    Lisp_Object* value_table = XVECTOR(XCHAR_TABLE(table)->extras[4])->contents;
    int i, size = ASIZE(XCHAR_TABLE(table)->extras[4]);
    ...
    if (i == size)
        set_char_table_extras(table, 4,
                              CALLN(Fvconcat, XCHAR_TABLE(table)->extras[4],
                                    make_vector(1, value)));
    return make_fixnum(i);
}

static uniprop_encoder_t uniprop_encoder[] = {uniprop_encode_value_character,
...
static uniprop_decoder_t uniprop_get_encoder(Lisp_Object table) {
    EMACS_INT i;
...
Lisp_Object uniprop_table(Lisp_Object prop) {
...
Lisp_Object get_unicode_property(Lisp_Object char_table, int ch) {
