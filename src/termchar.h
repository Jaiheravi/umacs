...
struct tty_display_info {
    ...
    const char*
        TS_ins_multi_lines; /* "AL" (one parameter, # lines to insert) */
    ...
    const char*
        TS_set_scroll_region; /* "cs" (2 params, first line and last line) */
    ...
    const char*
        TS_del_multi_chars; /* "DC" (one parameter, # chars to delete) */
    const char* TS_del_line; /* "dl" */
    const char*
        TS_del_multi_lines; /* "DL" (one parameter, # lines to delete) */
    ...
    const char*
        TS_ins_multi_chars; /* "IC" (one parameter, # chars to insert) */
    ...
    const char*
        TS_enter_bold_mode; /* "md" -- turn on bold (extra bright mode).  */
    ...
    int TF_teleray; /* termcap xt flag: many weird consequences.
                       For t1061. */
    const char*
        TF_set_underline_style; /* termcap Smulx entry: Switches the underline
               style based on the parameter.  Param should
               be one of: 0 (none), 1 (straight), 2 (double-line),
               3 (wave), 4 (dots), or 5 (dashes).  */
    const char*
        TF_set_underline_color; /* Enabled when TF_set_underline_style is set:
