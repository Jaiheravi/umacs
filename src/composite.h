...
2007, 2008, 2009, 2010, 2011 National Institute of Advanced Industrial Science
and Technology (AIST) Registration Number H14PRO021 Copyright (C) 2003, 2006
...
INLINE bool composition_registered_p(Lisp_Object prop) {
...
#define COMPOSITION_LENGTH(prop)                                               \
    (composition_registered_p(prop) ? XFIXNUM(XCAR(XCDR(prop)))                \
                                    : XFIXNUM(XCAR(XCAR(prop))))

/* Return components of the composition.  */
#define COMPOSITION_COMPONENTS(prop)                                           \
    (composition_registered_p(prop) ? XCAR(XCDR(XCDR(prop))) : XCDR(XCAR(prop)))

/* Return modification function of the composition.  */
#define COMPOSITION_MODIFICATION_FUNC(prop)                                    \
    (composition_registered_p(prop) ? XCDR(XCDR(XCDR(prop)))                   \
         : CONSP(prop)              ? XCDR(prop)                               \
                                    : Qnil)
...
#define COMPOSITION_GLYPH(cmp, n)                                              \
    XFIXNUM(                                                                   \
        AREF(COMPOSITION_KEY(cmp),                                             \
             (cmp)->method == COMPOSITION_WITH_RULE_ALTCHARS ? (n) * 2 : (n)))
...
/* True if the global reference point GREF and new reference point NREF are
   valid.  */
INLINE bool COMPOSITION_ENCODE_RULE_VALID(int gref, int nref) {
...
#define COMPOSITION_ENCODE_RULE(gref, nref) ((gref) * 12 + (nref))
...
INLINE bool composition_valid_p(ptrdiff_t start, ptrdiff_t end,
                                Lisp_Object prop) {
    return (CONSP(prop) &&
            (composition_registered_p(prop)
                 ? (COMPOSITION_ID(prop) >= 0 &&
                    COMPOSITION_ID(prop) <= n_compositions && CONSP(XCDR(prop)))
                 : (CONSP(XCAR(prop)) &&
                    (NILP(XCDR(XCAR(prop))) || STRINGP(XCDR(XCAR(prop))) ||
                     VECTORP(XCDR(XCAR(prop))) || FIXNUMP(XCDR(XCAR(prop))) ||
...
INLINE Lisp_Object* lgstring_glyph_addr(Lisp_Object lgs, ptrdiff_t idx) {
...
#define LGLYPH_SET_LBEARING(g, val)                                            \
    ASET(g, LGLYPH_IX_LBEARING, make_fixnum(val))
#define LGLYPH_SET_RBEARING(g, val)                                            \
    ASET(g, LGLYPH_IX_RBEARING, make_fixnum(val))
...
#define LGLYPH_XOFF(g)                                                         \
    (VECTORP(LGLYPH_ADJUSTMENT(g)) ? XFIXNUM(AREF(LGLYPH_ADJUSTMENT(g), 0)) : 0)
#define LGLYPH_YOFF(g)                                                         \
    (VECTORP(LGLYPH_ADJUSTMENT(g)) ? XFIXNUM(AREF(LGLYPH_ADJUSTMENT(g), 1)) : 0)
#define LGLYPH_WADJUST(g)                                                      \
    (VECTORP(LGLYPH_ADJUSTMENT(g)) ? XFIXNUM(AREF(LGLYPH_ADJUSTMENT(g), 2)) : 0)
...
extern void composition_compute_stop_pos(struct composition_it*, ptrdiff_t,
                                         ptrdiff_t, ptrdiff_t, Lisp_Object,
                                         bool);
extern bool composition_reseat_it(struct composition_it*, ptrdiff_t, ptrdiff_t,
                                  ptrdiff_t, struct window*, signed char,
                                  struct face*, Lisp_Object);
extern int composition_update_it(struct composition_it*, ptrdiff_t, ptrdiff_t,
                                 Lisp_Object);
