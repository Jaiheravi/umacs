...
static EMACS_INT simple_search(EMACS_INT, unsigned char*, ptrdiff_t, ptrdiff_t,
                               Lisp_Object, ptrdiff_t, ptrdiff_t, ptrdiff_t,
                               ptrdiff_t);
static EMACS_INT boyer_moore(EMACS_INT, unsigned char*, ptrdiff_t, Lisp_Object,
                             Lisp_Object, ptrdiff_t, ptrdiff_t, int);

Lisp_Object re_match_object;

static AVOID matcher_overflow(void) {
    error("Stack overflow in regexp matcher");
}

static void freeze_buffer_relocation(void) {
#ifdef REL_ALLOC
...
static void compile_pattern_1(struct regexp_cache* cp, Lisp_Object pattern,
    ...
    cp->syntax_table =
        cp->buf.used_syntax ? BVAR(current_buffer, syntax_table) : Qt;
...
void clear_regexp_cache(void) {
    int i;

    for (i = 0; i < REGEXP_CACHE_SIZE; ++i)
        /* It's tempting to compare with the syntax-table we've actually
           changed, but it's not sufficient because char-table inheritance means
           that modifying one syntax-table can change others at the same time.
         */
        if (!searchbufs[i].busy && !BASE_EQ(searchbufs[i].syntax_table, Qt))
            searchbufs[i].regexp = Qnil;
}

static void unfreeze_pattern(void* arg) {
    struct regexp_cache* searchbuf = arg;
    searchbuf->busy = false;
}

static void freeze_pattern(struct regexp_cache* searchbuf) {
...
static struct regexp_cache* compile_pattern(Lisp_Object pattern,
       ...
       nil should never appear before a non-nil entry.  */
        ...
        if (SCHARS(cp->regexp) == SCHARS(pattern) && !cp->busy &&
            ...
            (BASE_EQ(cp->syntax_table, Qt) ||
             BASE_EQ(cp->syntax_table, BVAR(current_buffer, syntax_table))) &&
...
static Lisp_Object looking_at_1(Lisp_Object string, bool posix,
                                bool modify_data) {
    Lisp_Object val;
    unsigned char *p1, *p2;
    ptrdiff_t s1, s2;
    ...
    struct regexp_cache* cache_entry = compile_pattern(
        string, modify_match_data ? &search_regs : NULL,
        (!NILP(Vcase_fold_search) ? BVAR(current_buffer, case_canon_table)
                                  : Qnil),
        posix, !NILP(BVAR(current_buffer, enable_multibyte_characters)));
    ...
    i = re_match_2(&cache_entry->buf, (char*)p1, s1, (char*)p2, s2,
                   PT_BYTE - BEGV_BYTE, modify_match_data ? &search_regs : NULL,
                   ZV_BYTE - BEGV_BYTE);
...
static Lisp_Object string_match_1(Lisp_Object regexp, Lisp_Object string,
                                  Lisp_Object start, bool posix,
                                  bool modify_data) {
    ptrdiff_t val;
    EMACS_INT pos;
    ...
    specpdl_ref count = SPECPDL_INDEX();
    struct regexp_cache* cache_entry = compile_pattern(
        regexp, modify_match_data ? &search_regs : NULL,
        (!NILP(Vcase_fold_search) ? BVAR(current_buffer, case_canon_table)
                                  : Qnil),
        posix, STRING_MULTIBYTE(string));
    freeze_pattern(cache_entry);
    re_match_object = string;
    val = re_search(&cache_entry->buf, SSDATA(string), SBYTES(string), pos_byte,
...
ptrdiff_t fast_string_match_internal(Lisp_Object regexp, Lisp_Object string,
                                     Lisp_Object table) {
    re_match_object = string;
    specpdl_ref count = SPECPDL_INDEX();
    ...
    freeze_pattern(cache_entry);
    ptrdiff_t val = re_search(&cache_entry->buf, SSDATA(string), SBYTES(string),
                              0, SBYTES(string), 0);
    unbind_to(count, Qnil);
    return val;
}
...
ptrdiff_t fast_c_string_match_internal(Lisp_Object regexp, const char* string,
                                       ptrdiff_t len, Lisp_Object table) {
    ...
    specpdl_ref count = SPECPDL_INDEX();
    struct regexp_cache* cache_entry = compile_pattern(regexp, 0, table, 0, 0);
    freeze_pattern(cache_entry);
    re_match_object = Qt;
...
ptrdiff_t fast_looking_at(Lisp_Object regexp, ptrdiff_t pos, ptrdiff_t pos_byte,
    ...
    len = re_match_2(&cache_entry->buf, (char*)p1, s1, (char*)p2, s2, pos_byte,
                     NULL, limit_byte);
...
static struct region_cache* newline_cache_on_off(struct buffer* buf) {
    ...
    if (NILP(BVAR(buf, cache_long_scans))) {
        if (!indirect_p || NILP(BVAR(base_buf, cache_long_scans))) {
            /* It should be off.  */
...
ptrdiff_t find_newline(ptrdiff_t start, ptrdiff_t start_byte, ptrdiff_t end,
        ...
        while (start != end) {
            ...
            if (newline_cache) {
                ...
                while (start < end && result) {
                    ...
                    while (start < lim1 && FETCH_BYTE(start_byte) == '\n') {
                        start_byte++;
                        start++;
                        if (--count == 0) {
                            if (bytepos)
        ...
        while (start > end) {
            ...
            if (newline_cache) {
                ...
                while (start > end && result) {
                    ...
                    start_byte = CHAR_TO_BYTE(start);
                    while (start > lim1 && FETCH_BYTE(start_byte - 1) == '\n') {
                        if (++count == 0) {
                            if (bytepos)
                                *bytepos = start_byte;
                            return start;
...
void scan_newline(ptrdiff_t start, ptrdiff_t start_byte, ptrdiff_t limit,
                  ptrdiff_t limit_byte, ptrdiff_t count, bool allow_quit) {
    ptrdiff_t charpos, bytepos, counted;

    charpos = find_newline(start, start_byte, limit, limit_byte, count,
                           &counted, &bytepos, allow_quit);
    if (counted != count)
...
ptrdiff_t scan_newline_from_point(ptrdiff_t count, ptrdiff_t* charpos,
                                  ptrdiff_t* bytepos) {
    ptrdiff_t counted;
    ...
    if (count <= 0)
        ...
        *charpos =
            find_newline(PT, PT_BYTE, ZV, ZV_BYTE, count, &counted, bytepos, 1);
    return counted;
}

/* Like find_newline, but doesn't allow QUITting and doesn't return
   COUNTED.  */
ptrdiff_t find_newline_no_quit(ptrdiff_t from, ptrdiff_t frombyte,
...
ptrdiff_t find_before_next_newline(ptrdiff_t from, ptrdiff_t to, ptrdiff_t cnt,
                                   ptrdiff_t* bytepos) {
    ptrdiff_t counted;
...
static Lisp_Object search_command(Lisp_Object string, Lisp_Object bound,
                                  Lisp_Object noerror, Lisp_Object count,
                                  int direction, bool RE, bool posix) {
    EMACS_INT np;
    EMACS_INT lim;
    ptrdiff_t lim_byte;
    ...
    np = search_buffer(
        string, PT, PT_BYTE, lim, lim_byte, n, RE,
        (!NILP(Vcase_fold_search) ? BVAR(current_buffer, case_canon_table)
                                  : Qnil),
        (!NILP(Vcase_fold_search) ? BVAR(current_buffer, case_eqv_table)
                                  : Qnil),
        posix);
    if (np <= 0) {
        if (NILP(noerror))
...
static bool trivial_regexp_p(Lisp_Object regexp) {
    ...
    while (--len >= 0) {
        ...
        case '\\':
            ...
            case '_':
            case 'c':
            case 'C': /* for categoryspec and notcategoryspec */
            case '1':
            case '2':
            case '3':
            case '4':
...
static EMACS_INT search_buffer_re(Lisp_Object string, ptrdiff_t pos,
    ...
    struct regexp_cache* cache_entry = compile_pattern(
        string, preserve_match_data ? &search_regs : &search_regs_1, trt, posix,
...
static EMACS_INT search_buffer_non_re(Lisp_Object string, ptrdiff_t pos,
                                      ptrdiff_t pos_byte, ptrdiff_t lim,
                                      ptrdiff_t lim_byte, EMACS_INT n, bool RE,
                                      Lisp_Object trt, Lisp_Object inverse_trt,
                                      bool posix) {
    ...
    if (multibyte == STRING_MULTIBYTE(string)) {
        ...
        raw_pattern_size_byte =
            count_size_as_multibyte(SDATA(string), raw_pattern_size);
        raw_pattern = SAFE_ALLOCA(raw_pattern_size_byte + 1);
        copy_text(SDATA(string), raw_pattern, SCHARS(string), 0, 1);
    }
    else {
        ...
        copy_text(SDATA(string), raw_pattern, SBYTES(string), 1, 0);
    }
    ...
    EMACS_INT result =
        (boyer_moore_ok ? boyer_moore(n, pat, len_byte, trt, inverse_trt,
...
search_buffer(Lisp_Object string, ptrdiff_t pos, ptrdiff_t pos_byte,
    ...
    if (RE && !(trivial_regexp_p(string) && NILP(Vsearch_spaces_regexp)))
        ...
        pos = search_buffer_non_re(string, pos, pos_byte, lim, lim_byte, n, RE,
...
static EMACS_INT simple_search(EMACS_INT n, unsigned char* pat, ptrdiff_t len,
                               ptrdiff_t len_byte, Lisp_Object trt,
                               ptrdiff_t pos, ptrdiff_t pos_byte, ptrdiff_t lim,
                               ptrdiff_t lim_byte) {
    bool multibyte = !NILP(BVAR(current_buffer, enable_multibyte_characters));
    bool forward = n > 0;
                ...
                while (this_len > 0) {
                    ...
                    int buf_charlen,
                        buf_ch = string_char_and_length(
                            BYTE_POS_ADDR(this_pos_byte), &buf_charlen);
...
static EMACS_INT boyer_moore(EMACS_INT n, unsigned char* base_pat,
                             ptrdiff_t len_byte, Lisp_Object trt,
                             Lisp_Object inverse_trt, ptrdiff_t pos_byte,
                             ptrdiff_t lim_byte, int char_base) {
    ...
    while (n != 0) {
       ...
       Using an explicit ?1:0 prevents this.  */
        if ((lim_byte - pos_byte - ((direction > 0) ? 1 : 0)) * direction < 0)
        ...
        if ((limit - pos_byte) * direction > 20) {
            ...
            hit:
                ...
                if (i + direction == 0) {
                    ...
                    position = pos_byte + cursor - p2 +
                        ((direction > 0) ? 1 - len_byte : 0);
#ifdef REL_ALLOC
        ...
        {
            limit = ((direction > 0) ? BUFFER_CEILING_OF(pos_byte - dirlen + 1)
                                     : BUFFER_FLOOR_OF(pos_byte - dirlen - 1));
            limit = ((direction > 0) ? min(limit + len_byte, lim_byte - 1)
            ...
            hit2:
                ...
                while ((i -= direction) + direction != 0) {
                    ...
                    /* Translate only the last byte of a character.  */
                    if (!multibyte ||
                        ((ptr == tail_end_ptr || CHAR_HEAD_P(ptr[1])) &&
                         (CHAR_HEAD_P(ptr[0])
...
static void set_search_regs(ptrdiff_t beg_byte, ptrdiff_t nbytes) {
    ptrdiff_t i;
...
(Lisp_Object regexp, Lisp_Object bound, Lisp_Object noerror,
 Lisp_Object count) {
...
(Lisp_Object newtext, Lisp_Object fixedcase, Lisp_Object literal,
 Lisp_Object string, Lisp_Object subexp) {
       ...
       is more than one letter long. */
        ...
        while (pos < last) {
            if (NILP(string)) {
                ...
                c = fetch_string_char_as_multibyte_advance(string, &pos,
                                                           &pos_byte);
    ...
    if (!NILP(string)) {
       ...
       if desired.  */
        if (NILP(literal)) {
            ...
            if (pos != lastpos)
                middle = substring_both(newtext, lastpos, lastpos_byte, pos,
                                        pos_byte);
            else
                middle = Qnil;
    ...
    if (NILP(literal)) {
        ...
        substed_alloc_size =
            (length <= (STRING_BYTES_BOUND - 100) / 2 ? length * 2 + 100
                                                      : STRING_BYTES_BOUND);
       ...
       buffer.  */
        ...
        for (pos_byte = 0, pos = 0; pos_byte < length;) {
            ...
            if (substed_alloc_size - substed_len < add_len)
                substed = xpalloc(substed, &substed_alloc_size,
...
static Lisp_Object match_limit(Lisp_Object num, bool beginningp) {
    EMACS_INT n;
    ...
    for (i = 0; i < search_regs.num_regs; i++) {
        ...
        if (start >= 0) {
            if (BASE_EQ(last_thing_searched, Qt) || !NILP(integers)) {
                ...
                Fset_marker(data[2 * i], make_fixnum(start),
                            last_thing_searched);
                data[2 * i + 1] = Fmake_marker();
                Fset_marker(data[2 * i + 1], make_fixnum(search_regs.end[i]),
                            last_thing_searched);
            }
            else
    ...
    /* Allocate registers if they don't already exist.  */
    {
        ...
        for (i = 0; CONSP(list); i++) {
            ...
            if (NILP(marker)) {
                ...
                CHECK_FIXNUM_COERCE_MARKER(marker);
                if (PTRDIFF_MIN <= XFIXNUM(from) &&
                    XFIXNUM(from) <= PTRDIFF_MAX &&
...
DEFUN("match-data--translate", Fmatch_data__translate, Smatch_data__translate,
      1, 1, 0, doc:/* Add N to all positions in the match data.  Internal.  */)
(Lisp_Object n) {
    CHECK_FIXNUM(n);
...
void update_search_regs(ptrdiff_t oldstart, ptrdiff_t oldend,
                        ptrdiff_t newend) {
...
ptrdiff_t find_newline1(ptrdiff_t start, ptrdiff_t start_byte, ptrdiff_t end,
                        ptrdiff_t end_byte, ptrdiff_t count, ptrdiff_t* counted,
                        ptrdiff_t* bytepos, bool allow_quit) {
    if (count > 0) {
        if (!end)
...
(Lisp_Object buffer) {
    ...
    if (NILP(BVAR(buf, cache_long_scans)) || buf->newline_cache == NULL)
        return Qnil;
    ...
    find_newline(BEGV, BEGV_BYTE, ZV, ZV_BYTE, TYPE_MAXIMUM(ptrdiff_t),
                 &nl_count_cache, NULL, true);
    ...
    if (nl_count_cache) {
        for (from = BEGV, found = from, i = 0; from < ZV; from = found, i++) {
            ...
            found =
                find_newline(from, from_byte, 0, -1, 1, &counted, NULL, true);
    ...
    find_newline1(BEGV, BEGV_BYTE, ZV, ZV_BYTE, TYPE_MAXIMUM(ptrdiff_t),
                  &nl_count_buf, NULL, true);
    ...
    if (nl_count_buf) {
        for (from = BEGV, found = from, i = 0; from < ZV; from = found, i++) {
            ...
            found =
                find_newline1(from, from_byte, 0, -1, 1, &counted, NULL, true);
...
(Lisp_Object regexp, Lisp_Object raw) {
    ...
    struct regexp_cache* cache_entry = compile_pattern(
        ...
        (!NILP(Vcase_fold_search) ? BVAR(current_buffer, case_canon_table)
                                  : Qnil),
        false, !NILP(BVAR(current_buffer, enable_multibyte_characters)));
    if (!NILP(raw))
        return make_unibyte_string((char*)cache_entry->buf.buffer,
                                   cache_entry->buf.used);
    else { /* FIXME: Why ENABLE_CHECKING?  */
...
void syms_of_search(void) {
    ...
    Fput(Qsearch_failed, Qerror_conditions, list(Qsearch_failed, Qerror));
    Fput(Qsearch_failed, Qerror_message, build_string("Search failed"));

    Fput(Quser_search_failed, Qerror_conditions,
         list(Quser_search_failed, Quser_error, Qsearch_failed, Qerror));
    Fput(Quser_search_failed, Qerror_message, build_string("Search failed"));

    Fput(Qinvalid_regexp, Qerror_conditions, list(Qinvalid_regexp, Qerror));
    Fput(Qinvalid_regexp, Qerror_message, build_string("Invalid regexp"));
