...
function. For example, write (defun foo (arg buf) "Doc string" (interactive
"P\\nbbuffer: ") .... ) to make ARG be the raw prefix argument, and set BUF to
an existing buffer, when `foo' is called as a command.
...
usage: (interactive &optional ARG-DESCRIPTOR &rest MODES)  */
              attributes : const)
(Lisp_Object args) { return Qnil; }

/* Quotify EXP: if EXP is constant, return it.
   If EXP is not constant, return (quote EXP).  */
static Lisp_Object quotify_arg(register Lisp_Object exp) {
    if (CONSP(exp) || (SYMBOLP(exp) && !NILP(exp) && !EQ(exp, Qt)))
...
static Lisp_Object quotify_args(Lisp_Object exp) {
...
static void check_mark(bool for_region) {
    Lisp_Object tem;
...
static void fix_command(Lisp_Object function, Lisp_Object values) {
    ...
    if (FIXNUMP(XCAR(arity)) && FIXNUMP(XCDR(arity))) {
        ...
        for (Lisp_Object tail = values; CONSP(tail); tail = XCDR(tail), ++i) {
            if (!NILP(XCAR(tail))) {
                final = tail;
                final_i = i;
            }
...
static Lisp_Object read_file_name(Lisp_Object default_filename,
                                  Lisp_Object mustmatch, Lisp_Object initial,
                                  Lisp_Object predicate) {
    return calln(Qread_file_name, callint_message, Qnil, default_filename,
...
(Lisp_Object function, Lisp_Object record_flag, Lisp_Object keys) {
    ...
    Lisp_Object enable =
        (SYMBOLP(function) ? Fget(function, Qenable_recursive_minibuffers)
                           : Qnil);
    ...
    if (!STRINGP(specs)) {
        ...
        if (events != num_input_events || !NILP(record_flag)) {
            ...
            calln(Qadd_to_history, Qcommand_history, Fcons(function, values),
                  Qnil, Qt);
        }
        ...
        return unbind_to(
            speccount, CALLN(Fapply, Qfuncall_interactively, function, specs));
    }
    ...
    for (;; string++) {
        ...
        /* Ignore this for semi-compatibility with Lucid.  */
            ...
            Lisp_Object w,
                event =
                    (next_event < key_count ? AREF(keys, next_event) : Qnil);
            if (EVENT_HAS_PARAMETERS(event) && (w = XCDR(event), CONSP(w)) &&
                (w = XCAR(w), CONSP(w)) && (w = XCAR(w), WINDOWP(w))) {
    ...
    /* Count the number of arguments, which is two (the function itself and
       `funcall-interactively') plus the number of arguments the interactive
       spec would have us give to the function.  */
    ptrdiff_t nargs = 2;
    ...
    for (ptrdiff_t i = 2; tem < string_end; i++) {
        ...
        case 'a': /* Symbol defined as a function.  */
            visargs[i] = Fcompleting_read(callint_message, Vobarray, Qfboundp,
        ...
        case 'c': /* Character.  */
            ...
            Fput_text_property(make_fixnum(0),
                               make_fixnum(SCHARS(callint_message)), Qface,
                               Qminibuffer_prompt, callint_message);
        ...
        case 'C': /* Command: symbol with interactive function.  */
            visargs[i] = Fcompleting_read(callint_message, Vobarray, Qcommandp,
                                          Qt, Qnil, Qnil, Qnil, Qnil);
            args[i] = Fintern(visargs[i], Qnil);
            break;
        ...
        case 'k': /* Key sequence.  */
            {
                ...
                Fput_text_property(make_fixnum(0),
                                   make_fixnum(SCHARS(callint_message)), Qface,
                                   Qminibuffer_prompt, callint_message);
                args[i] = Fread_key_sequence(callint_message, Qnil, Qnil, Qnil,
                                             Qnil, Qnil);
        ...
        case 'K': /* Key sequence to be defined.  */
            {
                ...
                Fput_text_property(make_fixnum(0),
                                   make_fixnum(SCHARS(callint_message)), Qface,
                                   Qminibuffer_prompt, callint_message);
                args[i] = Fread_key_sequence_vector(callint_message, Qnil, Qt,
                                                    Qnil, Qnil, Qnil);
                visargs[i] = Fkey_description(args[i], Qnil);
                unbind_to(speccount1, Qnil);
        ...
        case 'e': /* The invoking event.  */
            if (next_event >= key_count)
                error("%s must be bound to an event with parameters",
                      (SYMBOLP(function) ? SSDATA(SYMBOL_NAME(function))
                                         : "command"));
        ...
        case 'M': /* String read via minibuffer with
            ...
            args[i] = Fread_string(callint_message, Qnil, Qnil, Qnil, Qt);
            break;
        ...
        case 'r': /* Region, point and mark as 2 args.  */
            {
                ...
                varies[i] = *tem == 'R' ? 5 : 3;
                args[++i] =
                    PT > mark ? point_marker : BVAR(current_buffer, mark);
                varies[i] = *tem == 'R' ? 6 : 4;
            }
            break;
        ...
        case 's': /* String read via minibuffer without
                 inheriting the current input method.  */
            args[i] = Fread_string(callint_message, Qnil, Qnil, Qnil, Qnil);
            break;

        case 'S': /* Any symbol.  */
            visargs[i] = Fread_string(callint_message, Qnil, Qnil, Qnil, Qnil);
            args[i] = Fintern(visargs[i], Qnil);
            break;
        ...
        case 'Z': /* Coding-system symbol, or ignore the
            ...
            if (NILP(prefix_arg)) {
                ...
                args[i] = Fread_non_nil_coding_system(callint_message);
                visargs[i] = last_minibuf_string;
            }
            break;
    ...
    if (arg_from_tty || !NILP(record_flag)) {
       ...
       an array of just the same size.  */
        ...
        for (ptrdiff_t i = 2; i < nargs; i++)
            visargs[i] =
                (varies[i] > 0 ? list1(intern(callint_argfuns[varies[i]]))
                               : quotify_arg(args[i]));
        calln(Qadd_to_history, Qcommand_history, Flist(nargs - 1, visargs + 1),
              Qnil, Qt);
    }
...
void syms_of_callint(void) {
    ...
    preserved_fns =
        list(intern_c_string("use-region-beginning"),
             intern_c_string("use-region-end"),
             intern_c_string("region-beginning"), intern_c_string("region-end"),
             intern_c_string("point"), intern_c_string("mark"));
    ...
    DEFVAR_KBOARD("prefix-arg", Vprefix_arg, doc:
                      /* The value of the prefix argument for the next editing
        command. It may be a number, or the symbol `-' for just a minus sign
        as arg, or a list whose car is a number for just one or more C-u's
...
See `prefix-arg' for the meaning of the value.  */);
    ...
    DEFVAR_LISP("current-prefix-arg", Vcurrent_prefix_arg, doc:
                    /* The value of the prefix argument for this editing
    command. It may be a number, or the symbol `-' for just a minus sign as
    arg, or a list whose car is a number for just one or more C-u's or nil
    if no argument has been specified. This is what `(interactive \"P\")'
    returns.  */);
