...
#define _GL_BYTESWAP_HAS_BUILTIN_BSWAP64 true
    ...
    _GL_BYTESWAP_INLINE uint_least16_t bswap_16(uint_least16_t x) {
#ifdef _GL_BYTESWAP_HAS_BUILTIN_BSWAP16
        return __builtin_bswap16(x);
#else
    uint_fast16_t mask = 0xff;
    return ((x & mask << 8 * 1) >> 8 * 1 | (x & mask << 8 * 0) << 8 * 1);
#endif
    }

    /* Given an unsigned 32-bit argument X, return the value corresponding to
       X with reversed byte order.  */
    _GL_BYTESWAP_INLINE uint_least32_t bswap_32(uint_least32_t x) {
#ifdef _GL_BYTESWAP_HAS_BUILTIN_BSWAP32
        return __builtin_bswap32(x);
#else
    uint_fast32_t mask = 0xff;
    return ((x & mask << 8 * 3) >> 8 * 3 | (x & mask << 8 * 2) >> 8 * 1 |
            (x & mask << 8 * 1) << 8 * 1 | (x & mask << 8 * 0) << 8 * 3);
#endif
    }
    ...
    /* Given an unsigned 64-bit argument X, return the value corresponding to
       X with reversed byte order.  */
    _GL_BYTESWAP_INLINE uint_least64_t bswap_64(uint_least64_t x) {
#ifdef _GL_BYTESWAP_HAS_BUILTIN_BSWAP64
        return __builtin_bswap64(x);
#else
        uint_fast64_t mask = 0xff;
        return ((x & mask << 8 * 7) >> 8 * 7 | (x & mask << 8 * 6) >> 8 * 5 |
                (x & mask << 8 * 5) >> 8 * 3 | (x & mask << 8 * 4) >> 8 * 1 |
                (x & mask << 8 * 3) << 8 * 1 | (x & mask << 8 * 2) << 8 * 3 |
                (x & mask << 8 * 1) << 8 * 5 | (x & mask << 8 * 0) << 8 * 7);
#endif
    }
#endif
