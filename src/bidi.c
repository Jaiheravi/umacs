...
static bidi_type_t bidi_get_type(int ch, bidi_dir_t override) {
...
static void bidi_check_type(bidi_type_t type) {
    eassert(UNKNOWN_BT <= type && type <= NEUTRAL_ON);
}

/* Given a bidi TYPE of a character, return its category.  */
static bidi_category_t bidi_get_category(bidi_type_t type) {
    switch (type) {
    case UNKNOWN_BT:
        return UNKNOWN_BC;
    case STRONG_L:
...
static bool bidi_isolate_fmt_char(bidi_type_t ch_type) {
...
static bidi_bracket_type_t bidi_paired_bracket_type(int c) {
...
static void bidi_set_sos_type(struct bidi_it* bidi_it, int level_before,
                              int level_after) {
    ...
    bidi_it->prev_for_neutral.type =
        (bidi_it->sos == R2L ? STRONG_R : STRONG_L);
...
static void bidi_push_embedding_level(struct bidi_it* bidi_it, int level,
                                      bidi_dir_t override,
                                      bool isolate_status) {
...
static int bidi_pop_embedding_level(struct bidi_it* bidi_it) {
    int level;
...
static void bidi_remember_char(struct bidi_saved_info* saved_info,
...
/* Effectively remove the cached states beyond the Nth state from the
   ...
   (buffer or string).  */
static void bidi_cache_reset_to(int n) {
...
static void bidi_cache_reset(void) { bidi_cache_reset_to(0); }
...
static void bidi_cache_shrink(void) {
...
static void bidi_cache_fetch_state(ptrdiff_t idx, struct bidi_it* bidi_it) {
...
static ptrdiff_t bidi_cache_search(ptrdiff_t charpos, int level, int dir) {
    ptrdiff_t i, i_start;
...
/* Find a cached state where the resolved level changes to a value
   ...
   true or false.  */
static ptrdiff_t bidi_cache_find_level_change(int level, int dir, bool before) {
    if (bidi_cache_idx) {
...
ptrdiff_t bidi_level_start(int level) {
...
static void bidi_cache_ensure_space(ptrdiff_t idx) {
    ...
    if (idx >= bidi_cache_size) {
        ...
        if (max(idx + 1, bidi_cache_size + chunk_size) <= bidi_cache_max_elts) {
...
static int bidi_cache_iterator_state(struct bidi_it* bidi_it, bool resolved,
                                     bool update_only) {
    ptrdiff_t idx;
    ...
    if (idx < 0) {
       ...
       useless and must be reset.  */
        if (bidi_cache_start < idx && idx < bidi_cache_size &&
            (bidi_it->charpos >
                 (bidi_cache[idx - 1].charpos + bidi_cache[idx - 1].nchars) ||
...
static bidi_type_t bidi_cache_find(ptrdiff_t charpos, bool resolved_only,
...
static int bidi_peek_at_next_level(struct bidi_it* bidi_it) {
    if (bidi_cache_idx == bidi_cache_start)
        emacs_abort();
...
void bidi_push_it(struct bidi_it* bidi_it) {
...
void bidi_pop_it(struct bidi_it* bidi_it) {
...
void* bidi_shelve_cache(void) {
    ...
    alloc = (bidi_shelve_header_size + bidi_cache_idx * sizeof(struct bidi_it));
    databuf = xmalloc(alloc);
    bidi_cache_total_alloc += alloc;

    memcpy(databuf, &bidi_cache_idx, sizeof(bidi_cache_idx));
    memcpy(databuf + sizeof(bidi_cache_idx), bidi_cache,
...
/* Restore the cache state from a copy stashed away by
   ...
   being restored.  */
void bidi_unshelve_cache(void* databuf, bool just_free) {
    ...
    if (!p) {
        ...
        if (just_free) {
            ...
            bidi_cache_total_alloc -= (bidi_shelve_header_size +
                                       bidi_cache_idx * sizeof(struct bidi_it));
        }
...
static void bidi_set_paragraph_end(struct bidi_it* bidi_it) {
...
void bidi_init_it(ptrdiff_t charpos, ptrdiff_t bytepos, bool frame_window_p,
                  struct bidi_it* bidi_it) {
    if (!bidi_initialized)
        bidi_initialize();
    if (charpos >= 0)
    ...
    bidi_it->prev_for_neutral.type = bidi_it->prev_for_neutral.orig_type =
        UNKNOWN_BT;
...
static void bidi_line_init(struct bidi_it* bidi_it) {
    ...
    bidi_it->bracket_pairing_pos = -1;
    bidi_set_sos_type(bidi_it, (bidi_it->paragraph_dir == R2L ? 1 : 0),
...
static ptrdiff_t bidi_count_bytes(const unsigned char* s, ptrdiff_t beg,
                                  ptrdiff_t begbyte, ptrdiff_t end,
                                  bool unibyte) {
    ptrdiff_t pos = beg;
...
/* Fetch and return the character at byte position BYTEPOS.  If S is
   ...
   unibyte string.  */
static int bidi_char_at_pos(ptrdiff_t bytepos, const unsigned char* s,
                            bool unibyte) {
    if (s) {
        s += bytepos;
        if (unibyte)
...
static int bidi_fetch_char(ptrdiff_t charpos, ptrdiff_t bytepos,
                           ptrdiff_t* disp_pos, int* disp_prop,
                           struct bidi_string_data* string, struct window* w,
                           bool frame_window_p, ptrdiff_t* ch_len,
                           ptrdiff_t* nchars) {
    int ch;
    ptrdiff_t endpos =
    ...
    if (charpos + *nchars <= endpos && charpos + *nchars > *disp_pos &&
        ...
        *disp_pos = compute_display_string_pos(&pos, string, w, frame_window_p,
        ...
        if (max_redisplay_ticks > 0 && *disp_pos > charpos + *nchars)
            update_redisplay_ticks((*disp_pos - charpos - *nchars) / 100 + 1,
                                   w);
    }
...
static int bidi_fetch_char_skip_isolates(ptrdiff_t charpos, ptrdiff_t bytepos,
                                         ptrdiff_t* disp_pos, int* disp_prop,
...
static ptrdiff_t bidi_at_paragraph_end(ptrdiff_t charpos, ptrdiff_t bytepos) {
    Lisp_Object sep_re;
...
static struct region_cache* bidi_paragraph_cache_on_off(void) {
    ...
    if (NILP(BVAR(current_buffer, cache_long_scans))) {
        if (!indirect_p || NILP(BVAR(cache_buffer, cache_long_scans))) {
...
static ptrdiff_t bidi_find_paragraph_start(ptrdiff_t pos, ptrdiff_t pos_byte) {
    ...
    while (pos_byte > BEGV_BYTE && n++ < MAX_PARAGRAPH_SEARCH &&
...
find_first_strong_char(ptrdiff_t pos, ptrdiff_t bytepos, ptrdiff_t end,
                       ptrdiff_t* disp_pos, int* disp_prop,
                       struct bidi_string_data* string, struct window* w,
                       bool string_p, bool frame_window_p, ptrdiff_t* ch_len,
                       ptrdiff_t* nchars, bool stop_at_pdi) {
    ptrdiff_t pos1;
    bidi_type_t type;
    int ch;
    ...
    ch =
        bidi_fetch_char_skip_isolates(pos, bytepos, disp_pos, disp_prop, string,
                                      w, frame_window_p, ch_len, nchars);
    type = bidi_get_type(ch, NEUTRAL_DIR);

    pos1 = pos;
    for (pos += *nchars, bytepos += *ch_len; bidi_get_category(type) != STRONG
...
void bidi_paragraph_init(bidi_dir_t dir, struct bidi_it* bidi_it,
                         bool no_default_p) {
    ...
    else if (dir == NEUTRAL_DIR) /* P2 */
    {
       ...
       because we need to reinitialize below in that case.  */
        if (!bidi_it->first_elt && bidi_it->charpos < bidi_it->separator_limit)
            return;
       ...
       exist.  */
        pos = bidi_it->charpos;
        s = (STRINGP(bidi_it->string.lstring) ? SDATA(bidi_it->string.lstring)
                                              : bidi_it->string.s);
        if (bytepos > begbyte &&
       ...
       and only if we are iterating on a buffer.  */
        do {
            ...
            type = find_first_strong_char(
                pos, bytepos, end, &disp_pos, &disp_prop, &bidi_it->string,
                bidi_it->w, string_p, bidi_it->frame_window_p, &ch_len, &nchars,
                false);
            if (type == STRONG_R || type == STRONG_AL) /* P3 */
                bidi_it->paragraph_dir = R2L;
            else if (type == STRONG_L)
                bidi_it->paragraph_dir = L2R;
            if (!string_p && no_default_p &&
    ...
    if (bidi_it->paragraph_dir != L2R && bidi_it->paragraph_dir != R2L)
        bidi_it->paragraph_dir =
            L2R; /* P3 and HL1 ``higher-level protocols'' */
...
static bool bidi_explicit_dir_char(int ch) {
    bidi_type_t ch_type;
    ...
    return (ch_type == LRE || ch_type == LRO || ch_type == RLE ||
            ch_type == RLO || ch_type == PDF);
}
...
static int bidi_resolve_explicit(struct bidi_it* bidi_it) {
    int curchar;
    ...
    int disp_prop;
    ptrdiff_t eob = ((bidi_it->string.s || STRINGP(bidi_it->string.lstring))
    ...
    if (bidi_it->next_en_pos >= 0 && bidi_it->charpos >= bidi_it->next_en_pos) {
        bidi_it->next_en_pos = 0;
        bidi_it->next_en_type = UNKNOWN_BT;
    }
    ...
    if (bidi_it->bytepos < (string_p ? 0 : BEGV_BYTE) || bidi_it->first_elt) {
        bidi_it->first_elt = 0;
        if (string_p) {
            const unsigned char* p = (STRINGP(bidi_it->string.lstring)
                                          ? SDATA(bidi_it->string.lstring)
                                          : bidi_it->string.s);

            if (bidi_it->charpos < 0)
                bidi_it->charpos = bidi_it->bytepos = 0;
            eassert(bidi_it->bytepos ==
                    bidi_count_bytes(p, 0, 0, bidi_it->charpos,
    ...
    if (bidi_it->charpos >= (string_p ? bidi_it->string.schars : ZV)) {
       ...
       that.  */
        ...
        {
        case RLI: /* X5a */
            if (current_level < BIDI_MAXDEPTH && bidi_it->invalid_levels == 0 &&
                bidi_it->invalid_isolates == 0) {
                new_level = ((current_level + 1) & ~1) + 1;
                bidi_it->isolate_level++;
                bidi_push_embedding_level(bidi_it, new_level, NEUTRAL_DIR,
                                          true);
            }
            else
       ...
       a single character u+FFFC.  */
        curchar = bidi_fetch_char(
            bidi_it->charpos, bidi_it->bytepos, &bidi_it->disp_pos,
            &bidi_it->disp_prop, &bidi_it->string, bidi_it->w,
            bidi_it->frame_window_p, &bidi_it->ch_len, &bidi_it->nchars);
    }
    bidi_it->ch = curchar;
    ...
    case RLO: /* X4 */
        ...
        if (new_level < BIDI_MAXDEPTH && bidi_it->invalid_levels == 0 &&
            bidi_it->invalid_isolates == 0) {
    ...
    case LRO: /* X5 */
        ...
        bidi_check_type(bidi_it->type_after_wn);
        type = WEAK_BN; /* X9/Retaining */
        if (new_level < BIDI_MAXDEPTH - 1 && bidi_it->invalid_levels == 0 &&
            bidi_it->invalid_isolates == 0) {
    ...
    case FSI: /* X5c */
        ...
        typ1 = find_first_strong_char(
            bidi_it->charpos, bidi_it->bytepos, end, &disp_pos, &disp_prop,
            &bidi_it->string, bidi_it->w, string_p, bidi_it->frame_window_p,
            &ch_len, &nchars, true);
...
static bidi_type_t bidi_resolve_weak(struct bidi_it* bidi_it) {
    bidi_type_t type;
    bidi_dir_t override;
    int prev_level = bidi_it->level_stack[bidi_it->stack_idx].level;
    ...
    ptrdiff_t eob = ((STRINGP(bidi_it->string.lstring) || bidi_it->string.s)
                         ? bidi_it->string.schars
                         : ZV);
    ...
    eassert(!(type == UNKNOWN_BT || type == LRE || type == LRO || type == RLE ||
              type == RLO || type == PDF));
    ...
    if (type == NEUTRAL_S || type == NEUTRAL_WS || type == WEAK_BN ||
        type == STRONG_AL)
    ...
    if (override == R2L) /* X6 */
        ...
        if (type == WEAK_EN /* W2 */
            ...
            if (type_of_next == WEAK_BN || bidi_explicit_dir_char(next_char)) {
            ...
            if (type == WEAK_ES && type_of_next == WEAK_EN &&
                bidi_it->last_strong.type != STRONG_AL)
                type = WEAK_EN;
                 ...
                 || type == WEAK_BN) /* W5/Retaining */
        {
            if (bidi_it->prev.type == WEAK_EN) /* ET/BN w/EN before it */
                ...
                if (bidi_it->next_en_type ==
                    WEAK_EN) /* ET/BN with EN after it */
                    type = WEAK_EN;
            }
               ...
               resolution.  */
                ...
                if (type_of_next == WEAK_ET || type_of_next == WEAK_BN ||
                    bidi_explicit_dir_char(next_char)) {
                    bidi_copy_it(&saved_it, bidi_it);
                    while (
                        bidi_resolve_explicit(bidi_it) == new_level &&
                        (bidi_it->type == WEAK_BN || bidi_it->type == WEAK_ET))
                        type_of_next = bidi_it->type;
                    if (type == WEAK_BN &&
                        bidi_it->charpos ==
                            saved_it.charpos + saved_it.nchars) {
    ...
    if (type == WEAK_ES || type == WEAK_ET || type == WEAK_CS /* W6 */
        ...
        (type == WEAK_BN &&
         (bidi_it->prev.type == WEAK_CS /* W6/Retaining */
          || bidi_it->prev.type == WEAK_ES || bidi_it->prev.type == WEAK_ET)))
        type = NEUTRAL_ON;
...
static bidi_type_t bidi_resolve_neutral_1(bidi_type_t prev_type,
                                          bidi_type_t next_type, int lev) {
...
enum
{
    MAX_BPA_STACK = max(
        1,
        ((MAX_ALLOCA - 2 * sizeof(struct bidi_it)) / sizeof(bpa_stack_entry)))
};
...
/* This function implements BPA, the Bidi Parenthesis Algorithm,
   ...
   zero otherwise.  */
static bool bidi_find_bracket_pairs(struct bidi_it* bidi_it) {
    ...
    if (btype == BIDI_BRACKET_OPEN) {
        ...
        bidi_type_t embedding_type =
            (embedding_level & 1) ? STRONG_R : STRONG_L;
        ...
        while (1) {
            ...
            if (btype == BIDI_BRACKET_OPEN)
                ...
                if (sp >= 0) {
                    ...
                    if (bpa_stack[sp].flags & FLAG_EMBEDDING_INSIDE)
                        ...
                        tem_it.bracket_enclosed_type /* N0c */
                            = (embedding_type == STRONG_L ? STRONG_R
                                                          : STRONG_L);
                    else /* N0d */
               ...
               all the slots on the stack.  */
                ...
                {
                case STRONG_L:
                    flag = ((embedding_level & 1) == 0 ? FLAG_EMBEDDING_INSIDE
                ...
                case WEAK_EN:
                case WEAK_AN:
                    flag = ((embedding_level & 1) == 1 ? FLAG_EMBEDDING_INSIDE
                                                       : FLAG_OPPOSITE_INSIDE);
                    r2l_seen = true;
                    break;
                default:
            ...
            if (bidi_it->level_stack[new_sidx].level > current_level &&
                ...
                while (bidi_it->level_stack[bidi_it->stack_idx].level >
                    ...
                    if (maxlevel <
                        ...
                        maxlevel =
                            bidi_it->level_stack[bidi_it->stack_idx].level;
                    if (!bidi_cache_iterator_state(bidi_it, type == NEUTRAL_B,
                                                   0)) {
            ...
            if (bidi_it->type_after_wn ==
                NEUTRAL_ON) /* Unicode 8.0 correction */
                btype = bidi_paired_bracket_type(bidi_it->ch);
            else
       ...
       forth many times).  */
        if (maxlevel == base_level && (l2r_seen || r2l_seen) /* N0d */
...
static void bidi_record_type_for_neutral(struct bidi_saved_info* info,
                                         int level, bool nextp) {
    int idx;
...
static bidi_type_t bidi_resolve_brackets(struct bidi_it* bidi_it) {
    int prev_level = bidi_it->level_stack[bidi_it->stack_idx].level;
    ...
    if (resolve_bracket) {
        ...
        if (bidi_it->bracket_enclosed_type == embedding_type) /* N0b */
            ...
            case WEAK_EN:
            case WEAK_AN:
                type = (bidi_it->bracket_enclosed_type == STRONG_R) /* N0c */
                    ? STRONG_R /* N0c1 */
                    : embedding_type; /* N0c2 */
                break;
            case STRONG_L:
                type = (bidi_it->bracket_enclosed_type == STRONG_L) /* N0c */
                    ? STRONG_L /* N0c1 */
                    : embedding_type; /* N0c2 */
                break;
            default:
       ...
       type as for the resolved opening bracket.  */
        if (type != NEUTRAL_ON) {
            ptrdiff_t idx =
                bidi_cache_search(bidi_it->bracket_pairing_pos, -1, 1);
...
static bidi_type_t bidi_resolve_neutral(struct bidi_it* bidi_it) {
    bidi_type_t type = bidi_resolve_brackets(bidi_it);
    int current_level;
    bool is_neutral;

    eassert(type == STRONG_R || type == STRONG_L || type == WEAK_BN ||
            type == WEAK_EN || type == WEAK_AN || type == NEUTRAL_B ||
            type == NEUTRAL_S || type == NEUTRAL_WS || type == NEUTRAL_ON ||
            type == LRI || type == RLI || type == PDI);
       ...
       entering the expensive loop in the "else" clause.  */
            ...
            do {
                ...
                if (!adjacent_to_neutrals &&
                    ...
                    adjacent_to_neutrals = true;
                       ...
                       (type != WEAK_BN && bidi_get_category(type) != NEUTRAL &&
                        !bidi_isolate_fmt_char(type))
            ...
            if ((bidi_it->level_stack[bidi_it->stack_idx].level !=
               ...
               member.  */
                ...
                switch (
                    INT_PROMOTE(type)) /* promotion to int avoids warnings */
                {
                case STRONG_L:
                case STRONG_R:
                case STRONG_AL:
...
/* Given an iterator state in BIDI_IT, advance one character position
   ...
   type.  */
static bidi_type_t bidi_type_of_next_char(struct bidi_it* bidi_it) {
    bidi_type_t type;
...
static int bidi_level_of_next_char(struct bidi_it* bidi_it) {
    bidi_type_t type = UNKNOWN_BT;
    int level;
    ...
    if (bidi_it->orig_type == NEUTRAL_B /* L1 */
        || bidi_it->orig_type == NEUTRAL_S || bidi_it->ch == '\n' ||
        bidi_it->ch == BIDI_EOB ||
...
/* Move to the other edge of a level given by LEVEL.  If END_FLAG,
   ...
   at point B.  */
static void bidi_find_other_level_edge(struct bidi_it* bidi_it, int level,
                                       bool end_flag) {
    ...
    if ((idx = bidi_cache_find_level_change(level, dir, end_flag)) >=
       ...
       iterating over a single buffer position.  */
        if (max_redisplay_ticks > 0 && bidi_it->charpos > pos0)
            update_redisplay_ticks((bidi_it->charpos - pos0) / 50 + 1,
                                   bidi_it->w);
    }
}
...
void bidi_move_to_visually_next(struct bidi_it* bidi_it) {
    ...
    if (!bidi_it->first_elt && (bidi_it->ch == '\n' || bidi_it->ch == BIDI_EOB))
        bidi_line_init(bidi_it);
       ...
       is always one more or one less than the previous one.  */
        ...
        while (next_level != expected_next_level) {
            ...
            /* If next_level is not consistent with incr, we might
               infloop.  */
            eassert(incr > 0 ? next_level > expected_next_level
...
ptrdiff_t bidi_find_first_overridden(struct bidi_it* bidi_it) {
    ptrdiff_t eob =
    ...
    } while (found_pos == eob && bidi_it->charpos < eob &&
