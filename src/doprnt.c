...
enum
{
    SIZE_BOUND_EXTRA = DBL_MAX_10_EXP + 50
};
...
static char const* parse_format_integer(char const* fmt, int* value) {
    int n = 0;
    bool overflow = false;
    for (; '0' <= *fmt && *fmt <= '9'; fmt++) {
...
static ptrdiff_t doprnt_non_null_end(char* buffer, ptrdiff_t bufsize,
                                     char const* format, char const* format_end,
                                     va_list ap) {
    USE_SAFE_ALLOCA;
...
ptrdiff_t doprnt(char* buffer, ptrdiff_t bufsize, const char* format,
                ...
                if (tem > bufsize) {
                    ...
                    do {
                        ...
                        if (CHAR_HEAD_P(string[tem])) {
                            if (BYTES_BY_CHAR_HEAD(string[tem]) <=
                                bufsize - tem)
                                tem = bufsize;
                            break;
                        }
...
/* Format to an unbounded buffer BUF.  This is like sprintf, except it
   ...
   '\0'.  */
ptrdiff_t esprintf(char* buf, char const* format, ...) {
    ptrdiff_t nbytes;
    va_list ap;
    va_start(ap, format);
...
/* Format to buffer *BUF of positive size *BUFSIZE, reallocating *BUF
   ...
   BUFSIZE_MAX.  */
ptrdiff_t exprintf(char** buf, ptrdiff_t* bufsize, char* nonheapbuf,
                   ptrdiff_t bufsize_max, char const* format, ...) {
    ptrdiff_t nbytes;
    va_list ap;
    va_start(ap, format);
...
ptrdiff_t evxprintf(char** buf, ptrdiff_t* bufsize, char* nonheapbuf,
                    ptrdiff_t bufsize_max, char const* format, va_list ap) {
    for (;;) {
        ptrdiff_t nbytes;
        va_list ap_copy;
        va_copy(ap_copy, ap);
