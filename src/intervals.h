...
#define AM_RIGHT_CHILD(i) (!NULL_PARENT(i) && INTERVAL_PARENT(i)->right == (i))
...
#define INTERVAL_PARENT(i) (eassert((i) != 0 && !(i)->up_obj), (i)->up.interval)
...
INLINE void set_interval_object(INTERVAL i, Lisp_Object obj) {
    eassert(BUFFERP(obj) || STRINGP(obj));
    i->up_obj = 1;
    i->up.obj = obj;
}

INLINE void set_interval_parent(INTERVAL i, INTERVAL parent) {
    i->up_obj = false;
    i->up.interval = parent;
}

INLINE void set_interval_plist(INTERVAL i, Lisp_Object plist) {
    i->plist = plist;
}
...
#define MERGE_INTERVAL_CACHE(from, to)                                         \
    do {                                                                       \
        if ((from)->write_protect)                                             \
            (to)->write_protect = true;                                        \
        if ((from)->visible)                                                   \
            (to)->visible = true;                                              \
        if ((from)->front_sticky)                                              \
            (to)->front_sticky = true;                                         \
        if ((from)->rear_sticky)                                               \
            (to)->rear_sticky = true;                                          \
    } while (false)

/* Is this interval visible?  Replace later with cache access.  */
#define INTERVAL_VISIBLE_P(i) (i && NILP(textget((i)->plist, Qinvisible)))
...
#define INTERVAL_GENERALLY_WRITABLE_P(i, ro)                                   \
    (NILP(ro) || (!NILP(Vinhibit_read_only) && !CONSP(Vinhibit_read_only)))
...
#define INTERVAL_EXPRESSLY_WRITABLE_P(i, ro)                                   \
    (!NILP(textget((i)->plist, Qinhibit_read_only)) ||                         \
     (!NILP(ro) && CONSP(Vinhibit_read_only) &&                                \
...
#if false
#define FRONT_STICKY_P(i) (i && !NILP(textget((i)->plist, Qfront_sticky)))
...
extern void traverse_intervals(INTERVAL, ptrdiff_t, void (*)(INTERVAL, void*),
                               void*);
extern void traverse_intervals_noorder(INTERVAL, void (*)(INTERVAL, void*),
                                       void*);
extern INTERVAL split_interval_right(INTERVAL,
                                     ptrdiff_t) ATTRIBUTE_RETURNS_NONNULL;
extern INTERVAL split_interval_left(INTERVAL,
                                    ptrdiff_t) ATTRIBUTE_RETURNS_NONNULL;
...
extern void graft_intervals_into_buffer(INTERVAL, ptrdiff_t, ptrdiff_t,
                                        struct buffer*, bool);
extern void verify_interval_modification(struct buffer*, ptrdiff_t, ptrdiff_t);
extern INTERVAL balance_intervals(INTERVAL);
extern void copy_intervals_to_string(Lisp_Object, struct buffer*, ptrdiff_t,
                                     ptrdiff_t);
...
extern INTERVAL validate_interval_range(Lisp_Object, Lisp_Object*, Lisp_Object*,
                                        bool);
...
extern Lisp_Object copy_text_properties(Lisp_Object, Lisp_Object, Lisp_Object,
                                        Lisp_Object, Lisp_Object, Lisp_Object);
extern Lisp_Object set_text_properties(Lisp_Object, Lisp_Object, Lisp_Object,
                                       Lisp_Object, Lisp_Object);
extern void set_text_properties_1(Lisp_Object, Lisp_Object, Lisp_Object,
                                  Lisp_Object, INTERVAL);
...
Lisp_Object get_char_property_and_overlay(Lisp_Object, Lisp_Object, Lisp_Object,
                                          Lisp_Object*);
