...
Lisp_Object get_doc_string(Lisp_Object filepos, bool unibyte) {
    ...
    Lisp_Object docdir = NILP(tem) ? ENCODE_FILE(dir) : empty_unibyte_string;
    ...
    while (true) {
        ptrdiff_t space_left =
            (get_doc_string_buffer_size - 1 - (p - get_doc_string_buffer));
...
Invalid data in documentation file -- %c followed by code %03o",
    ...
    if (unibyte)
        ...
        ptrdiff_t nchars = multibyte_chars_in_text(
            ((unsigned char*)get_doc_string_buffer + offset),
            to - (get_doc_string_buffer + offset));
        return make_string_from_bytes(get_doc_string_buffer + offset, nchars,
                                      to - (get_doc_string_buffer + offset));
    }
}
...
(Lisp_Object object) {
    return (STRINGP(object) || FIXNUMP(object) /* Reference to DOC.  */
                    || (CONSP(object) /* Reference to .elc.  */
                        && STRINGP(XCAR(object)) && FIXNUMP(XCDR(object)))
                ? Qt
                : Qnil);
}
...
DEFUN("internal-subr-documentation", Fsubr_documentation, Ssubr_documentation,
      1, 1, 0, doc:/* Return the raw documentation info of a C primitive.  */)
...
retry:
    ...
    if (EQ(prop, Qvariable_documentation) && NILP(tem)) {
...
static void store_function_docstring(Lisp_Object obj, EMACS_INT offset) {
    ...
    if (SUBRP(fun)) {
       ...
       we've found a docstring for it.  */
        ...
        AUTO_STRING(format,
                    "Ignoring DOC string on non-compiled "
                    "non-subr: %S");
        CALLN(Fmessage, format, obj);
    }
}
    ...
    while (true) {
        ...
        if (p) {
            ...
            */
            ...
            sym = oblookup(
                Vobarray, p + 2,
                multibyte_chars_in_text((unsigned char*)p + 2, end - p - 2),
                end - p - 2);
...
static bool default_to_grave_quoting_style(void) {
