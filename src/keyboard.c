...
static Lisp_Object read_char_x_menu_prompt(Lisp_Object, Lisp_Object, bool*);
static Lisp_Object read_char_minibuf_menu_prompt(int, Lisp_Object);
static Lisp_Object make_lispy_event(struct input_event*);
static Lisp_Object make_lispy_movement(struct frame*, Lisp_Object,
                                       enum scroll_bar_part, Lisp_Object,
                                       Lisp_Object, Time);
static Lisp_Object modify_event_symbol(ptrdiff_t, int, Lisp_Object, Lisp_Object,
                                       const char* const*, Lisp_Object*,
                                       ptrdiff_t);
...
static Lisp_Object xevent_start(Lisp_Object event) { return XCAR(XCDR(event)); }

/* These setters are used only in this file, so they can be private.  */
static void kset_echo_string(struct kboard* kb, Lisp_Object val)
{
  kb->echo_string_ = val;
}
static void kset_echo_prompt(struct kboard* kb, Lisp_Object val)
{
  kb->echo_prompt_ = val;
}
static void kset_kbd_queue(struct kboard* kb, Lisp_Object val)
{
  kb->kbd_queue_ = val;
}
static void kset_keyboard_translate_table(struct kboard* kb, Lisp_Object val)
{
  kb->Vkeyboard_translate_table_ = val;
}
static void kset_last_prefix_arg(struct kboard* kb, Lisp_Object val)
{
  kb->Vlast_prefix_arg_ = val;
}
static void kset_last_repeatable_command(struct kboard* kb, Lisp_Object val)
{
  kb->Vlast_repeatable_command_ = val;
}
static void kset_local_function_key_map(struct kboard* kb, Lisp_Object val)
{
  kb->Vlocal_function_key_map_ = val;
}
static void kset_overriding_terminal_local_map(struct kboard* kb,
                                               Lisp_Object val)
{
  kb->Voverriding_terminal_local_map_ = val;
}
static void kset_real_last_command(struct kboard* kb, Lisp_Object val)
{
  kb->Vreal_last_command_ = val;
}
static void kset_system_key_syms(struct kboard* kb, Lisp_Object val)
{
  kb->system_key_syms_ = val;
}


static bool echo_keystrokes_p(void)
{
...
static void echo_add_key(Lisp_Object c)
{
  ...
  if ((NILP(echo_string) || SCHARS(echo_string) == 0) && help_char_p(c))
  {
  ...
  kset_echo_string(current_kboard, concat2(echo_string, new_string));
  SAFE_FREE();
}
...
static void echo_dash(void)
{
  ...
  if (SCHARS(KVAR(current_kboard, echo_string)) > 1)
  {
    ...
    if ((XFIXNUM(last_char) == '-' && XFIXNUM(prev_char) != ' ')
        ...
        || (echo_keystrokes_help && XFIXNUM(last_char) == ')' &&
            XFIXNUM(prev_char) == 'p'))
      return;
  }
...
static void echo_truncate(ptrdiff_t nchars)
{
...
static void add_command_key(Lisp_Object key)
{
...
Lisp_Object recursive_edit_1(void)
{
...
void record_auto_save(void) { last_auto_save = num_nonmacro_input_events; }

/* Make an auto save happen as soon as possible at command level.  */

#ifdef SIGDANGER
void force_auto_save_soon(void) { last_auto_save = -auto_save_interval - 1; }
#endif
...
void not_single_kboard_state(KBOARD* kboard)
{
...
void push_kboard(struct kboard* k)
{
...
void temporarily_switch_to_single_kboard(struct frame* f)
{
...
static void restore_kboard_configuration(int was_locked)
{
...
static Lisp_Object cmd_error(Lisp_Object data)
{
  ...
  if (!NILP(executing_kbd_macro))
  {
    if (executing_kbd_macro_iterations == 1)
      snprintf(macroerror, sizeof macroerror, "After 1 kbd macro iteration: ");
    else
      snprintf(macroerror, sizeof macroerror,
               "After %" pI "d kbd macro "
               "iterations: ",
...
void cmd_error_internal(Lisp_Object data, const char* context)
{
...
Lisp_Object command_loop_2(Lisp_Object handlers)
{
...
static Lisp_Object top_level_2(void)
{
...
static Lisp_Object top_level_1(Lisp_Object ignore)
{
...
static AVOID user_error(const char* msg)
{
...
DEFUN("abort-recursive-edit", Fabort_recursive_edit, Sabort_recursive_edit, 0,
      0, "",
      doc
...
static void tracking_off(Lisp_Object old_track_mouse)
{
...
DEFUN("internal--track-mouse", Finternal_track_mouse, Sinternal_track_mouse, 1,
      1, 0, doc:/* Call BODYFUN with mouse movement events enabled.  */)
(Lisp_Object bodyfun)
{
  specpdl_ref count = SPECPDL_INDEX();
  Lisp_Object val;
...
static int read_key_sequence(Lisp_Object*, Lisp_Object, bool, bool, bool, bool,
                             bool);
...
static Lisp_Object command_loop_1(void)
{
 ...
 wait a short time and redraw minibuffer.  */
    ...
    if (minibuf_level && !NILP(echo_area_buffer[0]) &&
 ...
 reset it before we execute the command.  */
    ...
    safe_run_hooks_maybe_narrowed(Qpre_command_hook, XWINDOW(selected_window));
    ...
    safe_run_hooks_maybe_narrowed(Qpost_command_hook, XWINDOW(selected_window));
 ...
 Bug#34317.  */
    if (!NILP(echo_area_buffer[0]) &&
        (EQ(echo_area_window, FRAME_MINIBUF_WINDOW(XFRAME(selected_frame)))))
      resize_echo_area_exactly();
    ...
    if (!NILP(BVAR(current_buffer, mark_active)) && !NILP(Vrun_hooks))
    {
     ...
     `select-active-regions' is non-nil.  */
        if ((!NILP(Fwindow_system(Qnil)) ||
             ((symval = find_symbol_value(Qtty_select_active_regions),
        ...
        {
          Lisp_Object txt = calln(Vregion_extract_function, Qnil);
  ...
  finalize:
        ...
        last_point_position != PT)
    {
      if (NILP(Vdisable_point_adjustment) &&
          NILP(Vglobal_disable_point_adjustment) && !composition_break_at_point)
      {
        if (last_point_position > BEGV && last_point_position < ZV &&
...
Lisp_Object read_menu_command(void)
{
  specpdl_ref count = SPECPDL_INDEX();
  ...
  Lisp_Object keybuf[READ_KEY_ELTS];
  int i = read_key_sequence(keybuf, Qnil, false, true, true, true, false);
...
static void adjust_point_for_property(ptrdiff_t last_pt, bool modified)
{
  ptrdiff_t beg, end;
  ...
  while (check_composition || check_display || check_invisible)
  {
    ...
    if (check_composition && PT > BEGV && PT < ZV &&
    ...
    if (check_display && PT > BEGV && PT < ZV &&
        !NILP(val = get_char_property_and_overlay(make_fixnum(PT), Qdisplay,
                                                  selected_window, &overlay)) &&
        display_prop_intangible_p(val, overlay, PT, PT_BYTE) &&
        (!OVERLAYP(overlay)
             ? get_property_and_range(PT, Qdisplay, &val, &beg, &end, Qnil)
             : (beg = OVERLAY_START(overlay), end = OVERLAY_END(overlay))) &&
        (beg < PT /* && end > PT   <- It's always the case.  */
         || (beg <= PT && STRINGP(val) && SCHARS(val) == 0)))
    {
      eassert(end > PT);
      SET_PT(PT < last_pt
                 ? (STRINGP(val) && SCHARS(val) == 0 ? max(beg - 1, BEGV) : beg)
                 : end);
    ...
    if (check_invisible && PT > BEGV && PT < ZV)
    {
      ...
      while (end < ZV
		 ...
		 && (val = Fget_pos_property (make_fixnum (end),
             ...
             && !NILP(val = get_char_property_and_overlay(
                          make_fixnum(end), Qinvisible, selected_window,
                          &overlay)) &&
             (inv = TEXT_PROP_MEANS_INVISIBLE(val)))
      {
        ...
        tmp = Fnext_single_char_property_change(make_fixnum(end), Qinvisible,
                                                Qnil, Qnil);
...
#endif
             && !NILP(val = get_char_property_and_overlay(
                          make_fixnum(beg - 1), Qinvisible, selected_window,
                          &overlay)) &&
             (inv = TEXT_PROP_MEANS_INVISIBLE(val)))
      {
        ellipsis = ellipsis || inv > 1 ||
            (OVERLAYP(overlay) &&
             (!NILP(Foverlay_get(overlay, Qafter_string)) ||
              !NILP(Foverlay_get(overlay, Qbefore_string))));
        tmp = Fprevious_single_char_property_change(make_fixnum(beg),
                                                    Qinvisible, Qnil, Qnil);
   ...
   point-motion hooks, intangibility, etc.  */
      ...
      bool shown =
          !NILP(val = get_char_property_and_overlay(
                    make_fixnum(beg), Qdisplay, selected_window, &overlay)) &&
      ...
      if (!modified && !shown && !ellipsis && beg < end)
      {
        if (last_pt == beg && PT == end && end < ZV)
          ...
          ;
        else if (val = Fget_pos_property(make_fixnum(PT), Qinvisible,
                                         selected_window),
                 TEXT_PROP_MEANS_INVISIBLE(val) &&
                     (val =
                          (Fget_pos_property(make_fixnum(PT == beg ? end : beg),
...
static Lisp_Object safe_run_hooks_1(ptrdiff_t nargs, Lisp_Object* args)
{
  eassert(nargs >= 2);
...
static Lisp_Object safe_run_hooks_error(Lisp_Object error, ptrdiff_t nargs,
                                        Lisp_Object* args)
{
  eassert(nargs >= 2);
...
static Lisp_Object safe_run_hook_funcall(ptrdiff_t nargs, Lisp_Object* args)
{
  ...
  internal_condition_case_n(safe_run_hooks_1, nargs, newargs, Qt,
...
static void safe_run_hooks_maybe_narrowed(Lisp_Object hook, struct window* w)
{
  specpdl_ref count = SPECPDL_INDEX();
...
void safe_run_hooks_2(Lisp_Object hook, Lisp_Object arg1, Lisp_Object arg2)
{
  specpdl_ref count = SPECPDL_INDEX();
...
void poll_for_input_1(void)
{
  if (!input_blocked_p() && !waiting_for_input)
    gobble_input();
}
#endif
...
static void poll_for_input(struct atimer* timer)
{
...
void start_polling(void)
{
 ...
 a different interval, start a new one.  */
    if (NUMBERP(Vpolling_period) &&
        (poll_timer == NULL || NILP(Fequal(Vpolling_period, poll_timer_time))))
    {
      ...
      poll_timer =
          start_atimer(ATIMER_CONTINUOUS, interval, poll_for_input, NULL);
...
void bind_polling_period(int n)
{
#ifdef POLL_FOR_INPUT
  ...
  else if (FLOATP(Vpolling_period))
  {
    ...
    stop_other_atimers(poll_timer);
    stop_polling();
    specbind(Qpolling_period, (n > new ? make_int(n) : Vpolling_period));
  }
...
static Lisp_Object help_echo_substitute_command_keys(Lisp_Object help)
{
  if (STRINGP(help) && SCHARS(help) > 0 &&
      !NILP(Fget_text_property(make_fixnum(0), Qhelp_echo_inhibit_substitution,
                               help)))
    return help;
...
void show_help_echo(Lisp_Object help, Lisp_Object window, Lisp_Object object,
                    Lisp_Object pos)
{
...
static void read_char_help_form_unwind(void)
{
...
static Lisp_Object read_event_from_main_queue(struct timespec* end_time,
...
/* Like `read_event_from_main_queue' but applies keyboard-coding-system
   to tty input.  */
static Lisp_Object read_decoded_event_from_main_queue(struct timespec* end_time,
                                                      sys_jmp_buf local_getcjmp,
...
#define MAX_ENCODED_BYTES 16
  ...
  {
    Lisp_Object nextevt =
        read_event_from_main_queue(end_time, local_getcjmp, used_mouse_menu);
#ifdef WINDOWSNT
 ...
 Unicode.  See w32inevt.c:key_event, near its end.  */
    ...
    else
    {
      ...
      events[n++] = nextevt;
      if (FIXNATP(nextevt) && XFIXNUM(nextevt) < (meta_key == 1 ? 0x80 : 0x100))
        ...
        if (raw_text_coding_system_p(coding))
        {
          ...
          if (meta_key != 2)
          {
            for (i = 0; i < n; i++)
            {
              ...
              int modifier = (meta_key == 3 && c < 0x100 && (c & 0x80))
                  ? meta_modifier
                  : 0;
      ...
      while (n > 1)
        Vunread_command_events = Fcons(events[--n], Vunread_command_events);
      return events[0];
    }
#endif
  }
...
static Lisp_Object read_char(int commandflag, Lisp_Object map,
...
retry:
 ...
 additional keys returned by Fx_popup_menu.  */
    if (CONSP(c) && (SYMBOLP(XCAR(c)) || FIXNUMP(XCAR(c))) && NILP(XCDR(c)))
      c = XCAR(c);
 ...
 inside universal-argument.  */
    ...
    /* Undo what read_char_x_menu_prompt did when it unread
 additional keys returned by Fx_popup_menu.  */
    if (CONSP(c) && EQ(XCDR(c), Qdisabled) &&
 ...
 selected.  */
    ...
    if (STRINGP(Vexecuting_kbd_macro) && (XFIXNAT(c) & 0x80) &&
        (XFIXNAT(c) <= 0xff))
 ...
 from messing up echoing of the input after the prompt.  */
  ...
  c = Qnil;
  if (KEYMAPP(map) && INTERACTIVE && !NILP(prev_event) &&
      !EVENT_HAS_PARAMETERS(prev_event)
      /* Don't bring up a menu if we already have another event.  */
      && !CONSP(Vunread_command_events) && !detect_input_pending_run_timers(0))
  {
 ...
 don't do so more than once.  */
  ...
  if (minibuf_level == 0 && !end_time && !current_kboard->immediate_echo &&
      (this_command_key_count > 0 ||
       !NILP(call0(Qinternal_echo_keystrokes_prefix))) &&
      !noninteractive && echo_keystrokes_p() &&
      (/* No message.  */
       NILP(echo_area_buffer[0])
       /* Or empty message.  */
 ...
 and we don't want to delay before doing so.  */
  ...
  if (commandflag != 0 && commandflag != -2 && auto_save_interval > 0 &&
  ...
  if (KEYMAPP(map) && INTERACTIVE && !NILP(prev_event) &&
      EVENT_HAS_PARAMETERS(prev_event) && !EQ(XCAR(prev_event), Qmenu_bar) &&
 ...
 9 at 200k, 11 at 300k, and 12 at 500k.  It is 15 at 1 meg.  */
    ...
    if (commandflag != 0 && commandflag != -2 &&
        num_nonmacro_input_events > last_auto_save &&
        FIXNUMP(Vauto_save_timeout) && XFIXNUM(Vauto_save_timeout) > 0)
    {
      Lisp_Object tem0;
  ...
  if (NILP(c))
  {
    if (current_kboard->kbd_queue_has_data)
    {
      ...
      c_volatile = c;
      kset_kbd_queue(current_kboard, XCDR(KVAR(current_kboard, kbd_queue)));
...
wrong_kboard:
  ...
  if (NILP(c))
  {
    c = read_decoded_event_from_main_queue(end_time, local_getcjmp, prev_event,
                                           used_mouse_menu);
    if (NILP(c) && end_time && timespec_cmp(*end_time, current_timespec()) <= 0)
    {
      goto exit;
    }
...
non_reread:
...
#ifdef HAVE_NS
    if (CONSP(c) && (EQ(XCAR(c), Qns_unput_working_text)))
      input_was_pending = input_pending;
#endif
  ...
  if (FIXNUMP(c))
  {
    ...
    if ((STRINGP(KVAR(current_kboard, Vkeyboard_translate_table)) &&
        ...
        (VECTORP(KVAR(current_kboard, Vkeyboard_translate_table)) &&
         XFIXNAT(c) < ASIZE(KVAR(current_kboard, Vkeyboard_translate_table))) ||
  ...
  if (EVENT_HAS_PARAMETERS(c) && CONSP(XCDR(c)) && CONSP(xevent_start(c)) &&
 ...
 insert the dummy prefix event `menu-bar'.  */
    if (EQ(posn, Qmenu_bar) || EQ(posn, Qtab_bar) || EQ(posn, Qtool_bar))
    {
      ...
      if (end_time)
        Vunread_command_events = Fcons(Fcons(Qt, c), Vunread_command_events);
      else
      {
        also_record = c;
  ...
  if (FIXNUMP(c) && !NILP(Vinput_method_function) && ' ' <= XFIXNUM(c) &&
      XFIXNUM(c) < 256 && XFIXNUM(c) != 127)
  {
...
from_macro:
  ...
  if (FIXNUMP(c) &&
   ...
   after the first event of the key sequence.  */
      && NILP(prev_event) && ' ' <= XFIXNUM(c) && XFIXNUM(c) < 256 &&
      XFIXNUM(c) != 127)
  {
    Lisp_Object keys;
    ptrdiff_t key_count;
...
reread_first:
  ...
  if ((!reread || this_command_key_count == 0) && !end_time)
  {
  ...
  if (!NILP(Vhelp_form) && help_char_p(c))
  {
    ...
    help_form_saved_window_configs = Fcons(Fcurrent_window_configuration(Qnil),
...
static bool help_char_p(Lisp_Object c)
{
  if (EQ(c, Vhelp_char))
    return true;
 ...
 mouse-movement events.  */
    ...
    if (EQ(XCAR(c), Qhelp_echo))
    {
      ...
      if (!STRINGP(help))
        recorded = 1;
               ...
               (last_help = Fcar_safe(Fcdr_safe(XCDR(ev1))),
                EQ(last_help, help)))
        recorded = 1;
      else if (CONSP(ev1) && EQ(XCAR(ev1), Qmouse_movement) && CONSP(ev2) &&
               EQ(XCAR(ev2), Qhelp_echo) &&
               (last_help = Fcar_safe(Fcdr_safe(XCDR(ev2))),
                EQ(last_help, help)))
        recorded = -1;
      else if (CONSP(ev1) && EQ(XCAR(ev1), Qmouse_movement) && CONSP(ev2) &&
               EQ(XCAR(ev2), Qmouse_movement) && CONSP(ev3) &&
               EQ(XCAR(ev3), Qhelp_echo) &&
               (last_help = Fcar_safe(Fcdr_safe(XCDR(ev3))),
                EQ(last_help, help)))
        recorded = -2;
    }
    else if (EQ(XCAR(c), Qmouse_movement))
    {
      ...
      if (CONSP(ev1) && EQ(XCAR(ev1), Qmouse_movement) &&
          (last_window = Fcar_safe(Fcar_safe(XCDR(ev1))),
           EQ(last_window, window)) &&
          CONSP(ev2) && EQ(XCAR(ev2), Qmouse_movement) &&
          (last_window = Fcar_safe(Fcar_safe(XCDR(ev2))),
           EQ(last_window, window)))
      {
...
static void save_getcjmp(sys_jmp_buf temp)
{
  memcpy(temp, getcjmp, sizeof getcjmp);
}

static void restore_getcjmp(void* temp)
{
...
static bool readable_events(int flags)
{
      ...
      do
      {
        if (!(
                ...
                ((!input_pending_p_filter_events &&
                 ...
                 (input_pending_p_filter_events && is_ignored_event(event))))
#ifdef USE_TOOLKIT_SCROLL_BARS
            &&
            !((flags & READABLE_EVENTS_IGNORE_SQUEEZABLES) &&
              (event->kind == SCROLL_BAR_CLICK_EVENT ||
               event->kind == HORIZONTAL_SCROLL_BAR_CLICK_EVENT) &&
              event->ie.part == scroll_bar_handle && event->ie.modifiers == 0)
#endif
        )
          return 1;
...
static KBOARD* event_to_kboard(struct input_event* event)
{
...
static int kbd_buffer_nr_stored(void)
{
...
void kbd_buffer_store_buffered_event(union buffered_input_event* event,
                                     struct input_event* hold_quit)
{
  ...
  if (event->kind == ASCII_KEYSTROKE_EVENT)
  {
    ...
    c |= (event->ie.modifiers &
          (meta_modifier | alt_modifier | hyper_modifier | super_modifier));
    ...
    if (c == quit_char)
    {
      ...
      {
        kset_kbd_queue(kb,
                       list2(make_lispy_switch_frame(event->ie.frame_or_window),
                             make_fixnum(c)));
        kb->kbd_queue_has_data = true;
  ...
  if (!NILP(Vthrow_on_input) && !is_ignored_event(event))
    Vquit_flag = Vthrow_on_input;
}

/* Limit help event positions to this range, to avoid overflow problems.  */
#define INPUT_EVENT_POS_MAX                                                    \
  ((ptrdiff_t)min(PTRDIFF_MAX,                                                 \
                  min(TYPE_MAXIMUM(Time) / 2, MOST_POSITIVE_FIXNUM)))
#define INPUT_EVENT_POS_MIN                                                    \
  (PTRDIFF_MIN < -INPUT_EVENT_POS_MAX ? -1 - INPUT_EVENT_POS_MAX : PTRDIFF_MIN)

/* Return a Time that encodes position POS.  POS must be in range.  */

static Time position_to_Time(ptrdiff_t pos)
{
...
static ptrdiff_t Time_to_position(Time encoded_pos)
{
...
void gen_help_event(Lisp_Object help, Lisp_Object frame, Lisp_Object window,
...
void kbd_buffer_store_help_event(Lisp_Object frame, Lisp_Object help)
{
  struct input_event event;
  EVENT_INIT(event);
...
/* Discard any mouse events in the event buffer by setting them to
   NO_EVENT.  */
void discard_mouse_events(void)
{
  for (union buffered_input_event* sp = kbd_fetch_ptr; sp != kbd_store_ptr;
       sp = next_kbd_event(sp))
  {
    if (sp->kind == MOUSE_CLICK_EVENT || sp->kind == WHEEL_EVENT ||
        sp->kind == HORIZ_WHEEL_EVENT || sp->kind == SCROLL_BAR_CLICK_EVENT ||
...
bool kbd_buffer_events_waiting(void)
{
  for (union buffered_input_event* sp = kbd_fetch_ptr;; sp = next_kbd_event(sp))
...
static void clear_event(struct input_event* event) { event->kind = NO_EVENT; }

static Lisp_Object kbd_buffer_get_event_1(Lisp_Object arg)
{
  Lisp_Object coding_system = Fget_text_property(make_fixnum(0), Qcoding, arg);

  if (EQ(coding_system, Qt))
    return arg;

  return code_convert_string(
      arg, (!NILP(coding_system) ? coding_system : Vlocale_coding_system), Qnil,
      0, false, 0);
}

static Lisp_Object kbd_buffer_get_event_2(Lisp_Object val) { return Qnil; }
                                        ...
                                        struct timespec* end_time)
{
 ...
 interrupt handlers have not read it, read it now.  */
    ...
    if (end_time)
    {
      ...
      else
      {
        ...
        wait_reading_process_output(min(duration.tv_sec, WAIT_READING_MAX),
                                    duration.tv_nsec, -1, 1, Qnil, NULL, 0);
      }
    }
    else
    {
  ...
  if (had_pending_conversion_events)
  {
    ...
    if (conversion_disabled_p() || NILP(Vtext_conversion_edits))
      obj = Qnil;
  }
  else
#endif
   ...
   We return nil for them.  */
    ...
    case SELECTION_CLEAR_EVENT:
      ...
      case MONITORS_CHANGED_EVENT:
        {
          ...
          CALLN(Frun_hook_with_args, Qdisplay_monitors_changed_functions,
                event->ie.arg);
      ...
      case TOOLKIT_THEME_CHANGED_EVENT:
        ...
        CALLN(Frun_hook_with_args, Qtoolkit_theme_set_functions, event->ie.arg);
        break;
      ...
      default:
        {
          ...
          if (EQ(event->ie.device, Qt))
            Vlast_event_device =
                ((event->ie.kind == ASCII_KEYSTROKE_EVENT ||
         ...
         frame with the same modifiers, and send that instead.  */
            ...
            if (event->ie.kind == PINCH_EVENT
                ...
                && ((pinch_dx = XFLOAT_DATA(XCAR(event->ie.arg))) != 0.0 ||
                    ...
                    XFLOAT_DATA(Fnth(make_fixnum(3), event->ie.arg)) != 0.0))
            {
              ...
              while (maybe_event != kbd_store_ptr &&
                     ...
                     && (XFLOAT_DATA(XCAR(maybe_event->ie.arg)) != 0.0 ||
                         ...
                         XFLOAT_DATA(
                             Fnth(make_fixnum(3), maybe_event->ie.arg)) != 0.0))
              {
                event = maybe_event;
                ...
                pinch_angle +=
                    XFLOAT_DATA(Fnth(make_fixnum(3), maybe_event->ie.arg));

                XSETCAR(maybe_event->ie.arg, make_float(pinch_dx));
                XSETCAR(XCDR(maybe_event->ie.arg), make_float(pinch_dy));
                XSETCAR(Fnthcdr(make_fixnum(3), maybe_event->ie.arg),
                ...
                CONSP(event->ie.arg))
            {
              ...
              event->ie.code =
                  XFIXNUM(Faref(XCDR(event->ie.arg), XCAR(event->ie.arg)));
         ...
         that as the `event with parameters' for this selection.  */
            if (used_mouse_menu &&
                ...
                (event->kind == MENU_BAR_EVENT ||
                 event->kind == TAB_BAR_EVENT || event->kind == TOOL_BAR_EVENT))
              *used_mouse_menu = true;
#endif
#ifdef HAVE_NS
            /* Certain system events are non-key events.  */
            if (used_mouse_menu && event->kind == NS_NONKEY_EVENT)
              *used_mouse_menu = true;
#endif

            if (event->kind != MULTIBYTE_CHAR_KEYSTROKE_EVENT ||
                !CONSP(event->ie.arg) ||
                (XFIXNUM(XCAR(event->ie.arg)) >= SCHARS(XCDR(event->ie.arg))))
            {
   ...
   so x remains nil.  */
      ...
      if (f && FRAME_TERMINAL(f)->mouse_position_hook)
        (*FRAME_TERMINAL(f)->mouse_position_hook)(&f, 0, &bar_window, &part, &x,
                                                  &y, &t);
   ...
   frames.  */
      if (!NILP(x) && f)
      {
        ...
        if (!EQ(frame, internal_last_event_frame) && !EQ(frame, selected_frame))
          obj = make_lispy_switch_frame(frame);
        internal_last_event_frame = frame;
      }
...
void swallow_events(bool do_display)
{
...
/* Return the value of TIMER if it is a valid timer, an invalid struct
   timespec otherwise.  */
static struct timespec decode_timer(Lisp_Object timer)
{
  Lisp_Object* vec;
...
static struct timespec timer_check_2(Lisp_Object timers,
                                     Lisp_Object idle_timers)
{
 ...
 this timer becomes ripe.
    ...
    if (CONSP(timers))
    {
      ...
      timer_difference = (timer_ripe ? timespec_sub(now, timer_time)
 ...
 based on the next idle timer.  */
    if (CONSP(idle_timers))
    {
      ...
      idle_timer_difference =
          (idle_timer_ripe ? timespec_sub(idleness_now, idle_timer_time)
 ...
 Also step down the list where we found that timer.  */
    ...
    if (timespec_valid_p(timer_difference) &&
        (!timespec_valid_p(idle_timer_difference) ||
         ...
         (idle_timer_ripe == timer_ripe &&
          ((timer_ripe
                ? timespec_cmp(idle_timer_difference, timer_difference)
                : timespec_cmp(timer_difference, idle_timer_difference)) < 0))))
    {
      chosen_timer = timer;
      timers = XCDR(timers);
...
static const char* const lispy_accent_keys[] = {
    "dead-circumflex", "dead-grave",        "dead-tilde",
    "dead-diaeresis",  "dead-macron",       "dead-degree",
    "dead-acute",      "dead-cedilla",      "dead-breve",
    "dead-ogonek",     "dead-caron",        "dead-doubleacute",
    "dead-abovedot",   "dead-abovering",    "dead-iota",
    "dead-belowdot",   "dead-voiced-sound", "dead-semivoiced-sound",
    "dead-hook",       "dead-horn",
};
...
static const char* const lispy_multimedia_keys[] = {0,
                                                    "browser-back",
                                                    "browser-forward",
                                                    "browser-refresh",
                                                    "browser-stop",
...
static const char* const iso_lispy_function_keys[] = {
    ...
    "iso-set-margin-left",
    "iso-set-margin-right", /* 0xffe27, 28 */
    ...
    "iso-discontinuous-underline", /* 0xfe30, 31 */
    "iso-emphasize",
    "iso-center-object",
    "iso-enter", /* ... 0xfe34 */
};
...
static const char* const lispy_wheel_names[] = {"wheel-up", "wheel-down",
                                                "wheel-left", "wheel-right"};

/* drag-n-drop events are generated when a set of selected files are
   dragged from another application and dropped onto an Emacs window.  */
static const char* const lispy_drag_n_drop_names[] = {"drag-n-drop"};
...
static short const scroll_bar_parts[] = {SYMBOL_INDEX(Qnil),
                                         SYMBOL_INDEX(Qabove_handle),
                                         SYMBOL_INDEX(Qhandle),
                                         SYMBOL_INDEX(Qbelow_handle),
                                         SYMBOL_INDEX(Qup),
                                         SYMBOL_INDEX(Qdown),
                                         SYMBOL_INDEX(Qtop),
                                         ...
                                         SYMBOL_INDEX(Qhorizontal_handle),
                                         SYMBOL_INDEX(Qafter_handle),
                                         SYMBOL_INDEX(Qleft),
                                         SYMBOL_INDEX(Qright),
                                         SYMBOL_INDEX(Qleftmost),
                                         SYMBOL_INDEX(Qrightmost),
                                         SYMBOL_INDEX(Qend_scroll),
                                         SYMBOL_INDEX(Qratio)};
...
static short const internal_border_parts[] = {
    ...
    SYMBOL_INDEX(Qtop_left_corner),
    SYMBOL_INDEX(Qtop_edge),
    SYMBOL_INDEX(Qtop_right_corner),
    SYMBOL_INDEX(Qright_edge),
    SYMBOL_INDEX(Qbottom_right_corner),
...
static int make_lispy_tty_position(struct frame* r, int mx, int my,
                                   ...
                                   Lisp_Object* posn)
{
  ...
  FOR_EACH_FRAME(tail, frame)
  {
    ...
    if (root_frame(f) == r && f != r && !FRAME_UNDECORATED(f) &&
  ...
  if (side != ON_NONE)
  {
    enum internal_border_part part = frame_internal_border_part(f, ix, iy);
...
static Lisp_Object make_lispy_position(struct frame* f, Lisp_Object x,
                                       Lisp_Object y, Time t)
{
  ...
  Lisp_Object window_or_frame =
      (f != NULL ? window_from_coordinates(f, mx, my, &part, false, true, true)
                 : Qnil);
#ifdef HAVE_WINDOW_SYSTEM
  ...
  /* Report mouse events on the tab bar and (on GUI frames) on the
     tool bar.  */
  if (f &&
      ((WINDOWP(f->tab_bar_window) && EQ(window_or_frame, f->tab_bar_window))
#ifndef HAVE_EXT_TOOL_BAR
       ||
       (WINDOWP(f->tool_bar_window) && EQ(window_or_frame, f->tool_bar_window))
#endif
           ))
  {
 ...
 coordinates.  FIXME!  */
...
#endif
  if (f && !FRAME_WINDOW_P(f) && FRAME_TAB_BAR_LINES(f) > 0 &&
  ...
  if (WINDOWP(window_or_frame) && is_tty_frame(f) &&
      (is_tty_root_frame_with_visible_child(f) || is_tty_child_frame(f)) &&
 ...
 string on the click position.  */
             ...
             part == ON_HEADER_LINE)
    {
      ...
      posn = (part == ON_MODE_LINE
                  ? Qmode_line
                  : (part == ON_TAB_LINE ? Qtab_line : Qheader_line));

      /* Note that mode_line_string takes COL, ROW as pixels and
         converts them to characters.  */
      col = wx;
      row = wy;
      string = mode_line_string(w, part, &col, &row, &charpos, &object, &dx,
                                &dy, &width, &height);
      if (STRINGP(string))
 ...
 window's) top edge, while X is meaningless.  */
    else if (part == ON_LEFT_MARGIN || part == ON_RIGHT_MARGIN)
    {
      ...
      row = wy;
      string = marginal_area_string(w, part, &col, &row, &charpos, &object, &dx,
                                    &dy, &width, &height);
      if (STRINGP(string))
        string_info = Fcons(string, make_fixnum(charpos));
      xret = wx;
    ...
    else if (part == ON_RIGHT_FRINGE)
    {
      ...
      col = 0;
      xret = wx;
      dx = wx - window_box_width(w, LEFT_MARGIN_AREA) -
 ...
 the buffer position nearest to the click.  */
    if (!textpos)
    {
      ...
      string2 = buffer_posn_from_coords(w, &x2, &y2, &p, &object2, &dx2, &dy2,
                                        &width2, &height2);
      ...
      if (NILP(posn))
      {
        ...
        if (STRINGP(string2))
          string_info = Fcons(string2, make_fixnum(CHARPOS(p.string_pos)));
      }
      if (NILP(object))
        object = object2;
    }
    ...
    if (IMAGEP(object))
    {
      ...
      if ((image_map = plist_get(XCDR(object), QCmap), !NILP(image_map)) &&
          (hotspot = find_hot_spot(image_map, dx, dy), CONSP(hotspot)) &&
    ...
    /* Object info.  */
    extra_info = list3(object, Fcons(make_fixnum(dx), make_fixnum(dy)),
  ...
  else if (f)
  {
    ...
    if (FRAME_LIVE_P(f) && NILP(posn) && FRAME_INTERNAL_BORDER_WIDTH(f) > 0 &&
        !NILP(get_frame_param(f, Qdrag_internal_border)))
    {
  ...
  return Fcons(window_or_frame,
               Fcons(posn,
                     Fcons(Fcons(make_fixnum(xret), make_fixnum(yret)),
...
/* Return non-zero if F is a GUI frame that uses some toolkit-managed
   ...
   geometry.  */
static bool toolkit_menubar_in_use(struct frame* f)
{
#ifdef HAVE_EXT_MENU_BAR
...
static Lisp_Object make_scroll_bar_position(struct input_event* ev,
                                            Lisp_Object type)
{
...
static bool coords_in_menu_bar_window(struct frame* f, int x, int y)
{
  ...
  return (y >= WINDOW_TOP_EDGE_Y(window) && x >= WINDOW_LEFT_EDGE_X(window) &&
...
static bool coords_in_tab_bar_window(struct frame* f, int x, int y)
{
  struct window* window;
...
static void save_line_number_display_width(struct input_event* event)
{
  struct window* w;
  int pixel_width;
...
static bool line_number_mode_hscroll(Lisp_Object start_pos, Lisp_Object end_pos)
{
  ...
  return EQ(start_col, end_col) && down_mouse_line_number_width >= 0 &&
...
static Lisp_Object make_lispy_event(struct input_event* event)
{
  int i;
  ...
  case HELP_EVENT:
    {
      ...
      Lisp_Object position = make_fixnum(Time_to_position(event->timestamp));
      ...
      if (!WINDOWP(window))
        window = Qnil;
      return Fcons(Qhelp_echo, list5(frame, help, window, object, position));
    }
  ...
  case MULTIBYTE_CHAR_KEYSTROKE_EVENT:
    {
      ...
      if (event->modifiers & ~shift_modifier)
      {
        ...
        if (uppercasep(c) && !(event->modifiers & shift_modifier))
        {
          /* Got a capital letter without a shift.  The caps
             lock is on.   Un-capitalize the letter.  */
          c = downcase(c);
        }
        else if (lowercasep(c) && (event->modifiers & shift_modifier))
        {
      ...
      if ((event->code) == 040 && event->modifiers & shift_modifier)
        c |= shift_modifier;
      button_down_time = 0;
  ...
  case NON_ASCII_KEYSTROKE_EVENT:
    ...
    for (i = 0; i < ARRAYELTS(lispy_accent_codes); i++)
      if (event->code == lispy_accent_codes[i])
        return modify_event_symbol(i, event->modifiers, Qfunction_key, Qnil,
    ...
    if (event->code < FUNCTION_KEY_OFFSET &&
      ...
      return modify_event_symbol(event->code - ISO_FUNCTION_KEY_OFFSET,
                                 event->modifiers, Qfunction_key, Qnil,
 ...
 since we can't make a vector long enough.  */
    ...
    return modify_event_symbol(event->code, event->modifiers, Qfunction_key,
                               KVAR(current_kboard, Vsystem_key_alist), 0,
  ...
  case LANGUAGE_CHANGE_EVENT:
    ...
    return list4(Qlanguage_change, event->frame_or_window,
                 make_fixnum(event->code), make_fixnum(event->modifiers));

  case MULTIMEDIA_KEY_EVENT:
    if (event->code < ARRAYELTS(lispy_multimedia_keys) && event->code > 0 &&
        lispy_multimedia_keys[event->code])
    {
      return modify_event_symbol(event->code, event->modifiers, Qfunction_key,
                                 Qnil, lispy_multimedia_keys, &func_key_syms,
...
#endif
    {
      ...
      if (event->kind == MOUSE_CLICK_EVENT)
      {
        ...
        if (!toolkit_menubar_in_use(f)
            ...
            &&
            (!FRAME_WINDOW_P(f) ||
             coords_in_menu_bar_window(f, XFIXNUM(event->x), XFIXNUM(event->y)))
#endif
        )
        {
#if defined HAVE_WINDOW_SYSTEM && !defined HAVE_EXT_MENU_BAR
          if (FRAME_WINDOW_P(f))
          {
            ...
            x_y_to_column_row(
                ...
                FRAME_TO_WINDOW_PIXEL_Y(menu_w, XFIXNUM(event->y)), &column,
                &row);
          }
          else
#endif
              ...
              (event->modifiers & down_modifier))
          {
            ...
            /* Elisp manual 2.4b says (x y) are window
               ...
               frame-relative.  */
            position = list4(event->frame_or_window, Qmenu_bar,
        ...
        position = make_lispy_position(f, event->x, event->y, event->timestamp);
      ...
      if (button >= ASIZE(button_down_location))
      {
        ...
        button_down_location = larger_vector(button_down_location, incr, -1);
      ...
      *start_pos_ptr = Qnil;
      ...
      if (event->modifiers & down_modifier)
      {
        if (is_double)
        {
          double_click_count++;
          event->modifiers |=
              ((double_click_count > 2) ? triple_modifier : double_modifier);
        }
        else
      ...
      else if (event->modifiers & up_modifier)
      {
        ...
        else
        {
          ...
          xdiff = XFIXNUM(event->x) - XFIXNUM(XCAR(frame_relative_event_pos));
          ydiff = XFIXNUM(event->y) - XFIXNUM(XCDR(frame_relative_event_pos));

          if (!(0 < double_click_fuzz && -double_click_fuzz < xdiff &&
                xdiff < double_click_fuzz && -double_click_fuzz < ydiff &&
             ...
             it's probably OK to ignore it as well.  */
                ...
                (EQ(Fcar(Fcdr(start_pos)),
                 ...
                 !EQ(Fcar(start_pos), Fcar(position))))) /* Different window */
             ...
             being generated.  */
          {
            ...
            else if (new_y >= XFIXNUM(Fcar(Fcdr(Fcdr(Fcdr(edges))))))
              new_y = XFIXNUM(Fcar(Fcdr(Fcdr(Fcdr(edges))))) - 1;

            position = make_lispy_position(
                XFRAME(event->frame_or_window), make_fixnum(new_x),
                make_fixnum(new_y), event->timestamp);
          }
        }
        ...
        event->modifiers =
            ((event->modifiers & ~up_modifier) | click_or_drag_modifier |
...

      {
        ...
        head = modify_event_symbol(button, event->modifiers, Qmouse_click,
                                   Vlispy_mouse_stem, NULL, &mouse_syms,
                                   ASIZE(mouse_syms));
  ...
  case HORIZ_WHEEL_EVENT:
    {
      ...
      /* Set double or triple modifiers to indicate the wheel speed.  */
      {
        ...
        fuzz = FRAME_WINDOW_P(fr) ? double_click_fuzz : double_click_fuzz / 8;
        ...
        head = modify_event_symbol(symbol_num, event->modifiers, Qmouse_click,
                                   Qnil, lispy_wheel_names, &wheel_syms,
                                   ASIZE(wheel_syms));
      }

      if (CONSP(event->arg))
        return list5(
            head, position, make_fixnum(double_click_count), XCAR(event->arg),
            /* FIXME: When a mouse-click on a tab-bar is
                                 converted into a wheel-event we get here
               something of an unexpected shape...  */
            (CONSP(XCDR(event->arg)) && CONSP(XCDR(XCDR(event->arg))))
                ? Fcons(XCAR(XCDR(event->arg)), XCAR(XCDR(XCDR(event->arg))))
  ...
  case TOUCHSCREEN_BEGIN_EVENT:
    {
      ...
      if (coords_in_tab_bar_window(f, XFIXNUM(event->x), XFIXNUM(event->y))
          /* `get_tab_bar_item_kbd' returns 0 if the item was
             previously highlighted, 1 otherwise, and -1 if there is
             no tab bar item.  */
          && get_tab_bar_item_kbd(f, XFIXNUM(event->x), XFIXNUM(event->y),
                                  &tab_bar_item, &close) >= 0)
      {
        /* First, obtain the propertized string.  */
        x = Fcopy_sequence(
            AREF(f->tab_bar_items, (tab_bar_item + TAB_BAR_ITEM_CAPTION)));

        /* Next, add the key binding.  */
        AUTO_LIST2(
            y, Qmenu_item,
            list3(AREF(f->tab_bar_items, (tab_bar_item + TAB_BAR_ITEM_KEY)),
                  AREF(f->tab_bar_items, (tab_bar_item + TAB_BAR_ITEM_BINDING)),
                  close ? Qt : Qnil));

        /* And add the new properties to the propertized string.  */
        Fadd_text_properties(make_fixnum(0), make_fixnum(SCHARS(x)), y, x);
      ...
      return list2(Qtouchscreen_begin, Fcons(id, position));
    }
  ...
  case TOUCHSCREEN_END_EVENT:
    {
      ...
      if (EQ(menu_bar_touch_id, id))
      {
        ...
        {
          x_y_to_column_row(XWINDOW(f->menu_bar_window), XFIXNUM(x), XFIXNUM(y),
                            &column, &row);
  ...
  case PINCH_EVENT:
    {
      ...
      return Fcons(modify_event_symbol(0, event->modifiers, Qpinch, Qnil,
                                       (const char*[]){"pinch"}, &pinch_syms,
                                       1),
 ...
 index in scroll_bar_parts to get the appropriate symbol.  */
  ...
  case SCROLL_BAR_CLICK_EVENT:
    {
      ...
      if (event->code >= ASIZE(mouse_syms))
        mouse_syms =
            larger_vector(mouse_syms, event->code - ASIZE(mouse_syms) + 1, -1);

      /* Get the symbol we should use for the mouse click.  */
      head = modify_event_symbol(event->code, event->modifiers, Qmouse_click,
                                 Vlispy_mouse_stem, NULL, &mouse_syms,
                                 ASIZE(mouse_syms));
      return list2(head, position);
    }
  ...
  case DRAG_N_DROP_EVENT:
    {
      ...
      head = modify_event_symbol(0, event->modifiers, Qdrag_n_drop, Qnil,
                                 lispy_drag_n_drop_names, &drag_n_drop_syms, 1);
  ...
  case TOOL_BAR_EVENT:
    {
      ...
      if (SYMBOLP(res))
        res = apply_modifiers(event->modifiers, res);
  ...
  case CONFIG_CHANGED_EVENT:
    return list3(Qconfig_changed_event, event->arg, event->frame_or_window);
...
static Lisp_Object make_lispy_movement(struct frame* frame,
                                       Lisp_Object bar_window,
                                       ...
                                       Lisp_Object y, Time t)
{
  ...
  if (frame && !NILP(bar_window))
  {
    ...
    return list2(Qscroll_bar_movement,
                 list5(bar_window, Qvertical_scroll_bar, Fcons(x, y),
                       make_fixnum(t), part_sym));
  }
...
static Lisp_Object make_lispy_switch_frame(Lisp_Object frame)
{
  return list2(Qswitch_frame, frame);
}

static Lisp_Object make_lispy_focus_in(Lisp_Object frame)
{
  return list2(Qfocus_in, frame);
}

static Lisp_Object make_lispy_focus_out(Lisp_Object frame)
{
...
static int parse_modifiers_uncached(Lisp_Object symbol, ptrdiff_t* modifier_end)
{
  Lisp_Object name;
  ptrdiff_t i;
  int modifiers;
...
/* Return a symbol whose name is the modifier prefixes for MODIFIERS
   ...
   This doesn't use any caches.  */
static Lisp_Object apply_modifiers_uncached(int modifiers, char* base,
                                            int base_len, int base_len_byte)
{
...
static Lisp_Object lispy_modifier_list(int modifiers)
{
...
Lisp_Object parse_modifiers(Lisp_Object symbol)
{
...
/* Apply the modifiers MODIFIERS to the symbol BASE.
   ...
   the modified symbol.  */
static Lisp_Object apply_modifiers(int modifiers, Lisp_Object base)
{
  ...
  else
  {
    ...
    new_symbol = apply_modifiers_uncached(modifiers, SSDATA(SYMBOL_NAME(base)),
...
Lisp_Object reorder_modifiers(Lisp_Object symbol)
{
...
static Lisp_Object modify_event_symbol(ptrdiff_t symbol_num, int modifiers,
                                       ...
                                       ptrdiff_t table_size)
{
  ...
  {
    if (!VECTORP(*symbol_table) || ASIZE(*symbol_table) != table_size)
  ...
  if (NILP(value))
  {
    ...
    {
      char* buf;
      ptrdiff_t len = (SBYTES(name_alist_or_stem) + sizeof "-" +
                       INT_STRLEN_BOUND(EMACS_INT));
      USE_SAFE_ALLOCA;
      buf = SAFE_ALLOCA(len);
...
{
  Lisp_Object frame;
  if (!EQ(CAR_SAFE(event), Qfocus_in) || !CONSP(XCDR(event)) ||
  ...
  bool switching =
      (!EQ(frame, internal_last_event_frame) && !EQ(frame, selected_frame));
  internal_last_event_frame = frame;
...
int parse_solitary_modifier(Lisp_Object symbol)
{
  Lisp_Object name;
...
bool lucid_event_type_list_p(Lisp_Object object)
{
  if (!CONSP(object))
    return false;
...
static bool get_input_pending(int flags)
{
...
int tty_read_avail_input(struct terminal* terminal,
                         struct input_event* hold_quit)
{
  ...
  if (terminal->type != output_termcap && terminal->type != output_msdos_raw)
    emacs_abort();
...
#error "Cannot read without possibly delaying"
  ...
  for (i = 0; i < nread; i++)
  {
    ...
    if (!FRAMEP(tty->top_frame) ||
        (FRAME_PARENT_FRAME(XFRAME(selected_frame)) &&
         (root_frame(XFRAME(selected_frame)) == XFRAME(tty->top_frame))))
 ...
 something must have gone really wrong.  */
    ...
    if (buf.kind == ASCII_KEYSTROKE_EVENT && buf.code == quit_char)
      break;
  }
...
static void handle_async_input(void)
{
...
void unblock_input_to(int level)
{
...
void unblock_input(void) { unblock_input_to(interrupt_input_blocked - 1); }

/* Undo any number of BLOCK_INPUT calls,
   and also reinvoke any pending signal.  */

void totally_unblock_input(void) { unblock_input_to(0); }

#if defined(USABLE_SIGIO) || defined(USABLE_SIGPOLL)

void handle_input_available_signal(int sig)
{
#if defined HAVE_ANDROID && !defined ANDROID_STUBIFY
...
void add_user_signal(int sig, const char* name)
{
...
static void handle_user_signal(int sig)
{
  ...
  for (p = user_signals; p; p = p->next)
    if (p->sig == sig)
    {
      if (special_event_name && strcmp(special_event_name, p->name) == 0)
      {
...
static char* find_user_signal_name(int sig)
{
  struct user_signal_info* p;
...
bool menu_separator_name_p(const char* label)
{
  if (!label)
    return 0;
  else if (strnlen(label, 4) == 4 && memcmp(label, "--", 2) == 0 &&
           label[2] != '-')
  {
    int i;
    label += 2;
...
Lisp_Object menu_bar_items(Lisp_Object old)
{
     ...
     the initial keymaps from the current buffer.  */
  {
    ...
    if (!NILP(Voverriding_local_map_menu_flag) && !NILP(Voverriding_local_map))
    {
  ...
  for (mapno = nmaps - 1; mapno >= 0; mapno--)
    if (!NILP(maps[mapno]))
    {
      def = get_keymap(access_keymap(maps[mapno], Qmenu_bar, 1, 0, 1), 0, 1);
      if (CONSP(def))
      {
...
static void menu_bar_item(Lisp_Object key, Lisp_Object item, Lisp_Object dummy1,
                          void* dummy2)
{
  int i;
  bool parsed;
 ...
 discard any previously made menu bar item.  */
  ...
  if (i == menu_bar_items_index)
  {
    ...
    i++;
    ASET(menu_bar_items_vector, i, AREF(item_properties, ITEM_PROPERTY_NAME));
    i++;
    ASET(menu_bar_items_vector, i, list1(item));
    i++;
    ASET(menu_bar_items_vector, i, make_fixnum(0));
    i++;
    menu_bar_items_index = i;
  }
...
static Lisp_Object menu_item_eval_property_1(Lisp_Object arg)
{
...
static Lisp_Object eval_dyn(Lisp_Object form) { return Feval(form, Qnil); }

/* Evaluate an expression and return the result (or nil if something
   went wrong).  Used to evaluate dynamic parts of menu items.  */
Lisp_Object menu_item_eval_property(Lisp_Object sexpr)
{
  specpdl_ref count = SPECPDL_INDEX();
  Lisp_Object val;
...
bool parse_menu_item(Lisp_Object item, int inmenubar)
{
  ...
  if (STRINGP(item_string))
  {
    ...
    if (CONSP(item) && CONSP(XCAR(item)) &&
        (NILP(XCAR(XCAR(item))) || VECTORP(XCAR(XCAR(item)))))
      item = XCDR(item);
    ...
    if (CONSP(start))
    {
      ...
      FOR_EACH_TAIL(item)
      {
        ...
        else if (EQ(tem, QCbutton) && CONSP(XCAR(item)))
        {
          ...
          {
            ASET(item_properties, ITEM_PROPERTY_SELECTED, XCDR(tem));
  ...
  if (!NILP(filter))
  {
    def = menu_item_eval_property(list2(XCAR(filter), list2(Qquote, def)));
      ...
      if (CONSP(keyhint) && !NILP(XCAR(keyhint)))
      {
        ...
        if (NILP(tem) ||
            (!EQ(tem, def)
             ...
             && !(SYMBOLP(def) && EQ(tem, XSYMBOL(def)->u.s.function))))
          keys = Qnil;
      }
  ...
  */
  ...
  if (!NILP(tem))
    ASET(item_properties, ITEM_PROPERTY_SELECTED, menu_item_eval_property(tem));
...
static void process_tab_bar_item(Lisp_Object, Lisp_Object, Lisp_Object, void*);
...
Lisp_Object tab_bar_items(Lisp_Object reuse, int* nitems)
{
  Lisp_Object* maps;
  Lisp_Object mapsbuf[3];
  ptrdiff_t nmaps, i;
...
static void process_tab_bar_item(Lisp_Object key, Lisp_Object def,
                                 Lisp_Object data, void* args)
{
  int i;
      ...
      if (EQ(key, v[TAB_BAR_ITEM_KEY]))
      {
        if (ntab_bar_items > i + TAB_BAR_ITEM_NSLOTS)
          memmove(v, v + TAB_BAR_ITEM_NSLOTS,
                  ((ntab_bar_items - i - TAB_BAR_ITEM_NSLOTS) * word_size));
...
static void set_prop_tab_bar(ptrdiff_t idx, Lisp_Object val)
{
...
static bool parse_tab_bar_item(Lisp_Object key, Lisp_Object item)
{
  ...
  else if (!EQ(XCAR(item), Qmenu_item) || (item = XCDR(item), !CONSP(item)))
    return 0;
  ...
  if (!NILP(filter))
    set_prop_tab_bar(TAB_BAR_ITEM_BINDING,
                     (menu_item_eval_property(list2(
                         filter, list2(Qquote, PROP(TAB_BAR_ITEM_BINDING))))));
...
static void init_tab_bar_items(Lisp_Object reuse)
{
  if (VECTORP(reuse))
...
{
  ptrdiff_t incr =
      (ntab_bar_items - (ASIZE(tab_bar_items_vector) - TAB_BAR_ITEM_NSLOTS));
...
Lisp_Object tool_bar_items(Lisp_Object reuse, int* nitems)
{
  Lisp_Object* maps;
  Lisp_Object mapsbuf[3];
  ptrdiff_t nmaps, i;
...
static void process_tool_bar_item(Lisp_Object key, Lisp_Object def,
                                  Lisp_Object data, void* args)
{
  int i;
      ...
      if (EQ(key, v[TOOL_BAR_ITEM_KEY]))
      {
        if (ntool_bar_items > i + TOOL_BAR_ITEM_NSLOTS)
          memmove(v, v + TOOL_BAR_ITEM_NSLOTS,
                  ((ntool_bar_items - i - TOOL_BAR_ITEM_NSLOTS) * word_size));
...
static void set_prop(ptrdiff_t idx, Lisp_Object val)
{
...
static bool parse_tool_bar_item(Lisp_Object key, Lisp_Object item)
{
  Lisp_Object filter = Qnil;
  Lisp_Object caption;
  int i;
    ...
    else if (EQ(ikey, QCbutton) && CONSP(value))
    {
      ...
      if (EQ(type, QCtoggle) || EQ(type, QCradio))
      {
        ...
        set_prop(TOOL_BAR_ITEM_TYPE, type);
             ...
             (CONSP(value) || (VECTORP(value) && ASIZE(value) == 4)))
  ...
  if (!NILP(filter))
    set_prop(TOOL_BAR_ITEM_BINDING,
             (menu_item_eval_property(
                 list2(filter, list2(Qquote, PROP(TOOL_BAR_ITEM_BINDING))))));
...
static void init_tool_bar_items(Lisp_Object reuse)
{
  if (VECTORP(reuse))
...
{
  ptrdiff_t incr =
      (ntool_bar_items - (ASIZE(tool_bar_items_vector) - TOOL_BAR_ITEM_NSLOTS));
...
static Lisp_Object read_char_x_menu_prompt(Lisp_Object map,
                                           Lisp_Object prev_event,
                                           bool* used_mouse_menu)
{
  if (used_mouse_menu)
  ...
  if (EVENT_HAS_PARAMETERS(prev_event) && !EQ(XCAR(prev_event), Qmenu_bar) &&
      !EQ(XCAR(prev_event), Qtab_bar) && !EQ(XCAR(prev_event), Qtool_bar))
  {
    ...
    if (CONSP(value))
    {
      ...
      for (tem = XCDR(value); CONSP(tem); tem = XCDR(tem))
      {
        ...
        if (SYMBOLP(XCAR(tem)) || FIXNUMP(XCAR(tem)))
      ...
      Vunread_command_events = nconc2(XCDR(value), Vunread_command_events);
      value = XCAR(value);
    }
    else if (NILP(value))
      value = Qt;
...
static Lisp_Object follow_key(Lisp_Object keymap, Lisp_Object key)
{
  return access_keymap(get_keymap(keymap, 0, 1), key, 1, 0, 1);
}

static Lisp_Object read_char_minibuf_menu_prompt(int commandflag,
                                                 Lisp_Object map)
{
  Lisp_Object name;
  ptrdiff_t nlength;
...
#define PUSH_C_STR(str, listvar)                                               \
  ...
  while (1)
  {
     ...
     advanced past it, but start scanning its contents.  */
      ...
      else
      {
        ...
        if (FIXNUMP(event) && parse_menu_item(elt, -1))
        {
          ...
          Lisp_Object s = AREF(item_properties, ITEM_PROPERTY_NAME);
   ...
   the prefix is what the user has already typed.  */
          ...
          if ((SCHARS(s) + i + 2 + (char_matches ? 0 : SCHARS(desc) + 3)) <
                  width ||
              !notfirst)
          {
            ptrdiff_t thiswidth;
         ...
         first char, explicitly show what char to type.  */
            if (!char_matches)
            {
              ...
              menu_strings = Fcons(
                  Fsubstring(desc, make_fixnum(0), make_fixnum(thiswidth)),
                  menu_strings);
              i += thiswidth;
              PUSH_C_STR(" = ", menu_strings);
              i += 3;
            ...
            menu_strings =
                Fcons(Fsubstring(s, make_fixnum(0), make_fixnum(thiswidth)),
                      menu_strings);
            i += thiswidth;
          }
          else
                               ...
                               Lisp_Object second_event)
{
  ...
  if (SYMBOLP(first_event) && EVENT_HAS_PARAMETERS(second_event) &&
...
static Lisp_Object access_keymap_keyremap(Lisp_Object map, Lisp_Object key,
                                          Lisp_Object prompt, bool do_funcall,
...
static bool keyremap_step(Lisp_Object* keybuf, volatile keyremap* fkey,
                          int input, bool doit, int* diff, Lisp_Object prompt)
{
  ...
  if (KEYMAPP(fkey->parent))
    next = access_keymap_keyremap(fkey->map, key, prompt, doit, buf_start,
                                  buf_end, keybuf);
  else
    next = Qnil;
  ...
  if ((VECTORP(next) || STRINGP(next)) && doit)
  {
    ...
    *diff = len - (fkey->end - fkey->start);
    ...
    for (i = 0; i < len; i++)
      keybuf[fkey->start + i] = Faref(next, make_fixnum(i));
...
static bool test_undefined(Lisp_Object binding)
{
  return (NILP(binding) || EQ(binding, Qundefined) ||
          (SYMBOLP(binding) &&
           EQ(Fcommand_remapping(binding, Qnil, Qnil), Qundefined)));
}

void init_raw_keybuf_count(void) { raw_keybuf_count = 0; }
...
static Lisp_Object read_filtered_event(bool no_switch_frame,
                                       bool ascii_required, bool error_nonascii,
                                       bool input_method, Lisp_Object seconds)
{
...
retry:
  ...
  if (no_switch_frame && EVENT_HAS_PARAMETERS(val) &&
...
static void restore_reading_key_sequence(int old_reading_key_sequence)
{
...
static bool requeued_events_pending_p(void)
{
...
static int read_key_sequence(Lisp_Object* keybuf, Lisp_Object prompt,
                             bool dont_downcase_last,
                             ...
                             bool disable_text_conversion_p)
{
  ...
  record_unwind_protect_int(restore_reading_key_sequence, reading_key_sequence);
...
replay_sequence:
 ...
 unless there are unread events pending.  */
    else if (!NILP(Vexecuting_kbd_macro) && at_end_of_macro_p() &&
    ...
    {
      {
        ...
        key = read_char(prevent_redisplay ? -2 : NILP(prompt), current_binding,
                        last_nonmenu_event, &used_mouse_menu, NULL);
     ...
     of the command_loop_1.  */
      ...
      if (FIXNUMP(key) && XFIXNUM(key) == quit_char &&
    ...
    if (EVENT_HAS_PARAMETERS(key))
    {
      ...
      if (EQ(kind, Qmouse_click) || EQ(kind, Qtouchscreen))
      {
        ...
        if (last_real_key_start == 0)
        {
          ...
          if (WINDOWP(window) && BUFFERP(XWINDOW(window)->contents) &&
     ...
     use posn as a fake prefix key.  */
        if (SYMBOLP(posn) &&
            (NILP(fake_prefixed_keys) || NILP(Fmemq(key, fake_prefixed_keys))))
        {
      ...
      else if (CONSP(XCDR(key)) && CONSP(xevent_start(key)) &&
    ...
    else
    {
     ...
     event that is bound:
       ...
       Triple-downs reduce to double-downs, then to downs,
         then are eliminated.  */
        if (modifiers &
            (up_modifier | down_modifier | drag_modifier | double_modifier |
             triple_modifier))
        {
          while (modifiers &
                 (up_modifier | down_modifier | drag_modifier |
                  double_modifier | triple_modifier))
          {
            ...
            else
            {
              ...
              {
                indec.end = indec.start = min(last_real_key_start, indec.start);
                indec.map = indec.parent;
                if (fkey.end > last_real_key_start)
                {
                  fkey.end = fkey.start = min(last_real_key_start, fkey.start);
                  fkey.map = fkey.parent;
            ...
            new_head = apply_modifiers(modifiers, XCAR(breakdown));
 ...
 Scan from indec.end until we find a bound suffix.  */
    while (indec.end < t)
    {
      ...
      done = keyremap_step(keybuf, &indec, max(t, mock_input), true, &diff,
                           prompt);
      if (done)
      {
    ...
    if (!KEYMAPP(current_binding) && !test_undefined(current_binding) &&
        indec.start >= t)
       ...
       invariant that keytran.end <= fkey.start).  */
    {
      ...
      while (fkey.end < indec.start)
      {
        ...
        int diff;
            ...
            keybuf, &fkey, max(t, mock_input),
            /* If there's a binding (i.e.
               first_binding >= nmaps) we don't want
               to apply this function-key-mapping.  */
            (fkey.end + 1 == t && test_undefined(current_binding)), &diff,
            prompt);
        if (done)
        {
          mock_input = diff + max(t, mock_input);
 ...
 Scan from keytran.end until we find a bound suffix.  */
    while (keytran.end < fkey.start)
    {
      ...
      done = keyremap_step(keybuf, &keytran, max(t, mock_input), true, &diff,
                           prompt);
      if (done)
      {
        mock_input = diff + max(t, mock_input);
 ...
 use the corresponding lower-case letter instead.  */
    if (NILP(current_binding) &&
        /* indec.start >= t && fkey.start >= t && */ keytran.start >= t &&
        FIXNUMP(key) && translate_upper_case_key_bindings)
    {
      Lisp_Object new_key;
  ...
  not_upcase:
    if (NILP(current_binding) && help_char_p(EVENT_HEAD(key)) && t > 1)
    {
 ...
 use the corresponding unshifted function key instead.  */
        ...
        /* indec.start >= t && fkey.start >= t && */ keytran.start >= t)
    {
      ...
      int modifiers = CONSP(breakdown) ? (XFIXNUM(XCAR(XCDR(breakdown)))) : 0;
  ...
  if ((dont_downcase_last || NILP(current_binding)) && t > 0 &&
...
static Lisp_Object read_key_sequence_vs(Lisp_Object prompt,
                                        ...
                                        bool disable_text_conversion)
{
  ...
  specbind(Qinput_method_exit_on_first_char, (NILP(cmd_loop) ? Qt : Qnil));
  specbind(Qinput_method_use_echo_area, (NILP(cmd_loop) ? Qt : Qnil));
  ...
  return unbind_to(count,
                   ((allow_string ? make_event_array : Fvector)(i, keybuf)));
}
...
bool detect_input_pending(void)
{
...
bool detect_input_pending_run_timers(bool do_display)
{
...
void clear_input_pending(void) { input_pending = false; }

/* Return true if there are pending requeued command events.  */

bool requeued_command_events_pending_p(void)
{
...
(Lisp_Object check_timers)
{
  ...
  return (get_input_pending(
              (NILP(check_timers) ? 0 : READABLE_EVENTS_DO_TIMERS_NOW) |
              READABLE_EVENTS_FILTER_EVENTS)
              ? Qt
              : Qnil);
}
...
(Lisp_Object event)
{
  ...
  CHECK_CONS(event);
  if (NILP(access_keymap(get_keymap(Vspecial_event_map, 0, 1), event, 0, 0, 1)))
...
/* Reallocate recent_keys copying the recorded keystrokes
   in the right order.  */
static void update_recent_keys(int new_size, int kept_keys)
{
...
(Lisp_Object arg)
{
  ...
  if (new_size > MAX_NUM_RECENT_KEYS)
  {
    AUTO_STRING(fmt, "Value must be <= %d");
    Fsignal(Quser_error,
            list1(CALLN(Fformat, fmt, make_fixnum(MAX_NUM_RECENT_KEYS))));
  }
...
(Lisp_Object include_cmds)
{
  ...
  if (!total_keys || (cmds && total_keys < lossage_limit))
    return Fvector(total_keys, XVECTOR(recent_keys)->contents);
  else
  {
    Lisp_Object es = Qnil;
    int i = (total_keys < lossage_limit ? 0 : recent_keys_index);
...
(void)
{
  return Fvector(this_command_key_count, XVECTOR(this_command_keys)->contents);
}
...
The value is always a vector.  */)
(void) { return Fvector(raw_keybuf_count, XVECTOR(raw_keybuf)->contents); }
...
void set_waiting_for_input(struct timespec* time_to_clear)
{
...
void clear_waiting_for_input(void)
{
...
static void deliver_interrupt_signal(int sig)
{
...
static void write_stdout(char const* msg)
{
...
static void handle_interrupt(bool in_signal_handler)
{
  char c;
  ...
  if (!NILP(Vquit_flag) && get_named_terminal(dev_tty))
  {
     ...
     * original shell, this command causes the user to end up back at the shell.
     * The "Auto-save" and "Abort" questions are not asked until
     * the user elects to return to emacs, at which point he can save the
     * current job and either dump core or continue.
     */
    sys_suspend();
#else
...
static void quit_throw_to_read_char(bool from_signal)
{
  ...
  if (FRAMEP(internal_last_event_frame) &&
    ...
    do_switch_frame(make_lispy_switch_frame(internal_last_event_frame), 0, 0,
                    Qnil);
...
(Lisp_Object x, Lisp_Object y, Lisp_Object frame_or_window, Lisp_Object whole)
{
  ...
  if (WINDOWP(frame_or_window))
  {
    ...
    XSETINT(x,
            (XFIXNUM(x) + WINDOW_LEFT_EDGE_X(w) +
...
static void init_kboard(KBOARD* kb, Lisp_Object type)
{
...
KBOARD* allocate_kboard(Lisp_Object type)
{
 ...
 * it a second time.
 */
static void wipe_kboard(KBOARD* kb) { xfree(kb->kbd_macro_buffer); }

/* Free KB and memory referenced from it.  */

void delete_kboard(KBOARD* kb)
{
  KBOARD** kbp;
  ...
  *kbp = kb->next_kboard;
  ...
  if (kb == current_kboard && FRAMEP(selected_frame) &&
  ...
  for (thread = all_threads; thread; thread = thread->next_thread)
  {
    ...
    for (p = thread->m_specpdl_ptr; p > thread->m_specpdl;)
    {
      ...
      if (p->kind == SPECPDL_LET && p->let.where.kbd == kb)
...
static Lisp_Object init_while_no_input_ignore_events(void)
{
  Lisp_Object events =
      list(Qselect_window, Qhelp_echo, Qmove_frame, Qiconify_frame,
           Qmake_frame_visible, Qfocus_in, Qfocus_out, Qconfig_changed_event,
           Qselection_request, Qmonitors_changed, Qtoolkit_theme_changed);
...
static bool is_ignored_event(union buffered_input_event* event)
{
...
so that you can determine whether the command was run by mouse or not.  */);
  ...
  DEFVAR_LISP("unread-command-events", Vunread_command_events, doc:
                  /* List of events to be read as the command input.
    These events are processed first, before actual keyboard input.
    Events read from this list are not normally added to `this-command-keys',
    as they will already have been added once as they were read for the first
    time. An element of the form (t . EVENT) forces EVENT to be added to that
    list. An element of the form (no-record . EVENT) means process EVENT, but
    do not record it in the keyboard macros, recent-keys, and the dribble
    file.  */);
  Vunread_command_events = Qnil;

  DEFVAR_LISP("unread-post-input-method-events",
              Vunread_post_input_method_events, doc:
...
to see what command invoked the current minibuffer.  */);
  ...
  DEFVAR_LISP("this-command-keys-shift-translated",
              Vthis_command_keys_shift_translated, doc:
                  /* Non-nil if the key sequence activating this command was
    shift-translated. Shift-translation occurs when there is no binding for
    the key sequence as entered, but a binding was found by changing an
    upper-case letter to lower-case, or a shifted function key to an unshifted
    one.  */);
  Vthis_command_keys_shift_translated = Qnil;
...
to count as a drag.  */);
  ...
  DEFVAR_INT("num-input-keys", num_input_keys, doc:
...
came from `unread-command-events' instead).  */);
  ...
  DEFVAR_LISP("help-char", Vhelp_char, doc:
                  /* Character to recognize as meaning Help.
    When it is read, do `(eval help-form)', and display result if it's a
    string. If the value of `help-form' is nil, this char can be read
    normally.  */);
...
bindings.  The order of this list controls the order of the items.  */);
  ...
  DEFVAR_LISP("tab-bar-separator-image-expression",
              Vtab_bar_separator_image_expression, doc:
                  /* Expression evaluating to the image spec for a tab-bar
         separator. This is used internally by graphical displays that do not
         render tab-bar separators natively.  Otherwise it is unused (e.g. on
         GTK).  */);
  Vtab_bar_separator_image_expression = Qnil;

  DEFVAR_LISP("tool-bar-separator-image-expression",
              Vtool_bar_separator_image_expression, doc:
                  /* Expression evaluating to the image spec for a tool-bar
         separator. This is used internally by graphical displays that do not
         render tool-bar separators natively.  Otherwise it is unused (e.g. on
         GTK).  */);
...
for guidance on what to do.  */);
  ...
  DEFVAR_LISP("input-method-previous-message", Vinput_method_previous_message,
              doc:
                  /* When `input-method-function' is called, hold the previous
    echo area message. This variable exists because `read-event' clears the
    echo area before running the input method.  It is nil if there was no
    message.  */);
...
If nil, Emacs crashes immediately in response to fatal signals.  */);
  ...
  DEFVAR_LISP("while-no-input-ignore-events", Vwhile_no_input_ignore_events,
              doc:
...
Internal use only.  */);
  ...
  DEFVAR_BOOL("record-all-keys", record_all_keys, doc:
...
void keys_of_keyboard(void)
{
  initial_define_lispy_key(Vspecial_event_map, "delete-frame",
                           "handle-delete-frame");
#ifdef HAVE_NTGUI
  initial_define_lispy_key(Vspecial_event_map, "end-session", "kill-emacs");
#endif
#ifdef HAVE_NS
  ...
  initial_define_lispy_key(Vspecial_event_map, "config-changed-event",
                           "ignore");
#if defined(WINDOWSNT)
  initial_define_lispy_key(Vspecial_event_map, "language-change", "ignore");
#endif
  initial_define_lispy_key(Vspecial_event_map, "focus-in", "handle-focus-in");
  initial_define_lispy_key(Vspecial_event_map, "focus-out", "handle-focus-out");
  initial_define_lispy_key(Vspecial_event_map, "move-frame",
                           "handle-move-frame");
  initial_define_lispy_key(Vspecial_event_map, "sleep-event", "ignore");
}

/* Mark the pointers in the kboard objects.
   Called by Fgarbage_collect.  */
void mark_kboards(void)
{
