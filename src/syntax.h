...
INLINE Lisp_Object syntax_property_entry(int c, bool via_property) {
    if (via_property)
...
INLINE Lisp_Object SYNTAX_ENTRY(int c) {
...
INLINE int syntax_property_with_flags(int c, bool via_property) {
    Lisp_Object ent = syntax_property_entry(c, via_property);
    return CONSP(ent) ? XFIXNUM(XCAR(ent)) : Swhitespace;
}
INLINE int SYNTAX_WITH_FLAGS(int c) {
    return syntax_property_with_flags(c, false);
}

INLINE enum syntaxcode syntax_property(int c, bool via_property) {
    return syntax_property_with_flags(c, via_property) & 0xff;
}
INLINE enum syntaxcode SYNTAX(int c) { return syntax_property(c, false); }
...
INLINE ptrdiff_t RE_SYNTAX_TABLE_BYTE_TO_CHAR(ptrdiff_t byteoffset) {
    return (!parse_sexp_lookup_properties ? 0
                ...
                : NILP(gl_state.object) ? BYTE_TO_CHAR(byteoffset + BEGV_BYTE)
                                        : byteoffset);
}
...
INLINE void UPDATE_SYNTAX_TABLE_FORWARD(
    ptrdiff_t charpos) { /* Performs just-in-time syntax-propertization.  */
...
INLINE void UPDATE_SYNTAX_TABLE_BACKWARD(ptrdiff_t charpos) {
...
INLINE void UPDATE_SYNTAX_TABLE(ptrdiff_t charpos) {
...
INLINE void SETUP_BUFFER_SYNTAX_TABLE(void) {
