...
static Lisp_Object file_attributes(int, char const*, Lisp_Object, Lisp_Object,
                                   Lisp_Object);

/* Return the number of bytes in DP's name.  */
static ptrdiff_t dirent_namelen(struct dirent* dp) {
...
static int dirent_type(struct dirent* dp) {
...
static emacs_dir* open_directory(Lisp_Object dirname,
                                 Lisp_Object encoded_dirname, int* fdp) {
...
#ifdef WINDOWSNT
static void directory_files_internal_w32_unwind(Lisp_Object arg) {
    Vw32_get_true_file_attributes = arg;
}
#endif

static void directory_files_internal_unwind(void* d) { emacs_closedir(d); }
...
static struct dirent* read_dirent(emacs_dir* dir, Lisp_Object dirname) {
    while (true) {
        errno = 0;
...
Lisp_Object directory_files_internal(Lisp_Object directory, Lisp_Object full,
                                     Lisp_Object match, Lisp_Object nosort,
                                     bool attrs, Lisp_Object id_format,
                                     Lisp_Object return_count) {
    ...
    if (!NILP(full) &&
        !STRING_MULTIBYTE(
            directory)) { /* We will be concatenating 'directory' with local
                             file name. We always decode local file names, so in
                             order to safely concatenate them we need
                             'directory' to be decoded as well (bug#56469).  */
    ...
    for (struct dirent* dp; (dp = read_dirent(d, directory));) {
        ...
        if (!NILP(full)) {
            ...
            /* DECODE_FILE may return non-ASCII unibyte strings (e.g. when
                   file-name-coding-system is 'binary'), so we don't know for
               sure that the bytes we have follow our internal utf-8
               representation for multibyte strings.  If nchars == nbytes we
               don't need to care and just return a unibyte string; and if not,
               that means one of 'name' or 'directory' is multibyte, in which
               case we presume that the other one would also be multibyte if it
                   contained non-ASCII.
                   FIXME: This last presumption is broken when 'directory' is
                   multibyte (with non-ASCII), and 'name' is unibyte with
               non-ASCII (because file-name-coding-system is 'binary').  */
            ...
            memcpy(SDATA(finalname) + directory_nbytes + needsep, SDATA(name),
                   name_nbytes);
        }
        else
            finalname = name;
...
(Lisp_Object directory, Lisp_Object full, Lisp_Object match, Lisp_Object nosort,
 Lisp_Object count) {
    ...
    if (!NILP(handler))
        return calln(handler, Qdirectory_files, directory, full, match, nosort,
                     count);
...
which see.  */)
(Lisp_Object directory, Lisp_Object full, Lisp_Object match, Lisp_Object nosort,
 Lisp_Object id_format, Lisp_Object count) {
    directory = Fexpand_file_name(directory, Qnil);
    ...
    if (!NILP(handler))
        return calln(handler, Qdirectory_files_and_attributes, directory, full,
                     match, nosort, id_format, count);

    return directory_files_internal(directory, full, match, nosort, true,
                                    id_format, count);
}
...
(Lisp_Object file, Lisp_Object directory, Lisp_Object predicate) {
    ...
    if (!NILP(handler))
        return calln(handler, Qfile_name_completion, file, directory,
                     predicate);
...
static Lisp_Object file_name_completion(Lisp_Object file, Lisp_Object dirname,
                                        bool all_flag, Lisp_Object predicate) {
...
#define TRIVIAL_DIRECTORY_ENTRY(n) (!strcmp(n, ".") || !strcmp(n, ".."))
               ...
               actually in the way in a directory with only one file.  */
                if (TRIVIAL_DIRECTORY_ENTRY(dp->d_name))
                    ...
                    for (tem = Vcompletion_ignored_extensions; CONSP(tem);
                         tem = XCDR(tem)) {
                        ptrdiff_t elt_len;
                        char* p1;
                        ...
                        if (completion_ignore_case) {
                            ...
                            if (skip < 0 ||
                                !BASE_EQ(Fcompare_strings(
                                             name, make_fixnum(skip), Qnil, elt,
                                             zero, cmp_len, Qt),
                                         Qt))
                                continue;
                        }
            ...
            if (includeall &&
                !canexclude) { /* If we have one non-excludable file, we want to
                              exclude the excludable files.  */
                includeall = 0;
        ...
        Lisp_Object regexps,
            table = (completion_ignore_case ? Vascii_canon_table : Qnil);
        ...
        if (check_decoded && SCHARS(file) <= SCHARS(name)) {
            ...
            Lisp_Object cmp = Fcompare_strings(
                name, zero, make_fixnum(compare), file, zero,
                make_fixnum(compare), completion_ignore_case ? Qt : Qnil);
            if (!BASE_EQ(cmp, Qt))
                continue;
        }
        ...
        if (all_flag)
            ...
            Lisp_Object cmp = Fcompare_strings(
                bestmatch, zero, make_fixnum(compare), name, zero,
                make_fixnum(compare), completion_ignore_case ? Qt : Qnil);
            ptrdiff_t matchsize =
                BASE_EQ(cmp, Qt) ? compare : eabs(XFIXNUM(cmp)) - 1;
    ...
    if (matchcount == 1 && !NILP(Fequal(bestmatch, file)))
        return Qt;
    bestmatch =
        Fsubstring(bestmatch, make_fixnum(0), make_fixnum(bestmatchsize));
    return bestmatch;
}
...
static ptrdiff_t scmp(const char* s1, const char* s2, ptrdiff_t len) {
    ...
    if (completion_ignore_case) {
       ...
       comparing non-ASCII strings case-insensitively.  */
        while (
            l &&
            (downcase((unsigned char)*s1++) == downcase((unsigned char)*s2++)))
            l--;
    }
    else {
        while (l && *s1++ == *s2++)
...
static bool file_name_completion_dirp(int fd, struct dirent* dp,
                                      ptrdiff_t len) {
    USE_SAFE_ALLOCA;
    ...
    bool dirp = sys_faccessat(fd, subdir_name, F_OK, AT_EACCESS) == 0;
    SAFE_FREE();
    return dirp;
}

static char* stat_uname(struct stat* st) {
#ifdef WINDOWSNT
    return st->st_uname;
#else
...
static char* stat_gname(struct stat* st) {
#ifdef WINDOWSNT
    return st->st_gname;
#else
...
(Lisp_Object filename, Lisp_Object id_format) {
    ...
    filename = internal_condition_case_2(Fexpand_file_name, filename, Qnil, Qt,
                                         Fidentity);
    if (!STRINGP(filename))
        return Qnil;
    ...
    if (!NILP(handler)) { /* Only pass the extra arg if it is used to help
                         backward compatibility with old file name handlers
                         which do not implement the new arg.  --Stef */
        if (NILP(id_format))
...
static Lisp_Object file_attributes(int fd, char const* name,
    ...
    return CALLN(Flist, file_type, make_fixnum(s.st_nlink),
                 (uname ? DECODE_SYSTEM(build_unibyte_string(uname))
                        : INT_TO_INTEGER(s.st_uid)),
                 (gname ? DECODE_SYSTEM(build_unibyte_string(gname))
                 ...
                 /* If the file size is a 4-byte type, assume that
                    ...
                    32-bit platforms.  */
                 INT_TO_INTEGER(sizeof(s.st_size) == 4 ? s.st_size & 0xffffffffu
                                                       : s.st_size),

                 make_string(modes, 10), Qt, INT_TO_INTEGER(s.st_ino),
...
(Lisp_Object f1, Lisp_Object f2) { return Fstring_lessp(Fcar(f1), Fcar(f2)); }
...
void syms_of_dired(void) {
    ...
    DEFVAR_LISP("completion-ignored-extensions", Vcompletion_ignored_extensions,
                doc:
                    /* Completion ignores file names ending in any string in
    this list. It does not ignore them if all possible completions end in one
    of these strings or when displaying a list of completions. It ignores
    directory names if they match any string in this list which ends in a
    slash.  */);
